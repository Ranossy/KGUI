--this is the begin of this file
_ShieldWeak = {}
...
function ShieldFunction(t, szFunc, tResult, tParam, bInUserAction, szOnceKey, bInExcuteMacro)
	t[szFunc] = function(tOwner)
		return function(...)
            if bInExcuteMacro and not _g_CurrentExcuteMacro then
                return
            end
...
ShieldFunction(_ShieldList, "OnAddOnUseSkill", nil, nil, true, nil, true)
ShieldFunction(_ShieldList, "GetClientPlayer", nil, nil, nil, nil)
...
ShieldTable(_ShieldList, "PetActionBar", _PetActionBarShield)

local _PuppetActionBarShield = 
{
	["OnUseActionBarObject"] = true,
	["OnItemLButtonDrag"] = true,
	["OnItemLButtonDragEnd"] = true,
...
ShieldTable(_ShieldList, "PuppetActionBar", _PuppetActionBarShield)



function LogShieldList()
	Log("*************Log Shield List*************")
	for k, v in pairs(_ShieldList) do
		Log(tostring(k))
	end
	Log("*****************************************")
end
