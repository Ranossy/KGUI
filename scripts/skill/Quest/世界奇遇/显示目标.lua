---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/世界奇遇/显示目标.lua
-- 更新时间:	2015/7/1 16:07:48
-- 更新用户:	wangwei-pc2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/世界奇遇/显示目标.lua", -- 属性值1
		0														-- 属性值2
	);
	--[[
   	skill.AddAttribute(
	...
	skill.nTargetCountLimit = 500;				-- 技能作用目标数量限制,(小于0 代表目标数量不限制)
...
local tCrimeList = {
	...
	[43820] = {375, 376, GetEditorString(7, 7152)},
	[43821] = {475, 476, GetEditorString(7, 7153)},
	[43852] = {477, 478, GetEditorString(7, 7154)},
	[43853] = {479, 480, GetEditorString(7, 7155)},
	[43854] = {481, 482, GetEditorString(7, 7156)},
	
	-- 浩气盟
	[43855] = {389, 390, GetEditorString(7, 7157)},
	[43856] = {483, 484, GetEditorString(7, 7158)},
	[43857] = {485, 486, GetEditorString(7, 7159)},
	[43858] = {487, 488, GetEditorString(7, 7160)},
	[43859] = {489, 490, GetEditorString(7, 7161)},
}
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if IsPlayer(dwCharacterID) then
		...
		if player then
			...
			if npc then
				if GetJudgementState(player, npc) then
					RemoteCallToClient(player.dwID, "UpdateMinimapMark", 1, npc.dwTemplateID, false, npc.nX, npc.nY, npc.nZ, tCrimeList[npc.dwTemplateID][3], 3000)
				end
			end
		end
	end
end
function GetJudgementState(player, npc)
	if player.nCamp == 0 then
		return false
	elseif player.nCamp == 2 then
		if not (player.GetAdventureFlag(373) and not player.GetAdventureFlag(493)) then
			return false
		end
	else
		if not (player.GetAdventureFlag(387) and not player.GetAdventureFlag(497)) then
			return false
		end
	end

	if not tCrimeList[npc.dwTemplateID] then
		return false
	end

	if player.GetAdventureFlag(tCrimeList[npc.dwTemplateID][1]) or
		player.GetAdventureFlag(tCrimeList[npc.dwTemplateID][2]) then
		return false
	end
