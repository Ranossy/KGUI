---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/世界奇遇/阴阳两界_杨宁坐骑.lua
-- 更新时间:	2015/6/26 17:52:42
-- 更新用户:	kelin-PC
-- 脚本说明:
----------------------------------------------------------------------<
...
function OnRemove(dwCharacterID, dwBuffID, nBuffLevel, nLeftFrame)
	...
	if nLeftFrame == 0 then
		if player.CanAddItem(5, 23772, 1) == ADD_ITEM_RESULT_CODE.SUCCESS then
			if player.GetFreeRoomSize() < 1 then
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(7, 5379), 5)
				return
			end
			player.AddItem(5, 23772, 1)
			player.SetAdventureFlag(156, true)
			if player.GetBuff(7840, 1) then
				player.DelBuff(7840, 1)
			end
			if player.GetBuff(2587, 1) then
				player.DelBuff(2587, 1)
			end
			if player.GetBuff(9138, 1) then
				player.DelBuff(9138, 1)
			end
			if player.GetBuff(7840, 1) then
				player.DelBuff(7840, 1)
			end
			if player.GetBuff(7822, 1) then
				player.DelBuff(7822, 1)
			end
			local npc = scene.GetNpcByNickName("QY_YYLJ_Horse")
			if not npc then
				return
			end
			npc.SetDisappearFrames(1)
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(7, 5610), 5)
		end
	end
end
