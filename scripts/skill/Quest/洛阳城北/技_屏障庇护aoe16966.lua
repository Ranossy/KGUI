---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/洛阳城北/技_屏障庇护aoe16966.lua
-- 更新时间:	2016/9/30 16:02:12
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/洛阳城北/技_屏障庇护aoe16966.lua", -- 属性值1
		0																	-- 属性值2
	);
...
--	print(npc.dwForceID ,target.dwForceID)
	if not IsPlayer(dwCharacterID) and npc.dwForceID ~= target.dwForceID then--不是玩家，只要势力不同就算被入侵
		npc.AddBuff(npc.dwID, npc.nLevel, 11474, 1, 3)
		return
	end
	if IsPlayer(dwCharacterID) and npc.dwForceID == 28 then--狼牙屏障被玩家入侵
		if target.nMoveState ~= MOVE_STATE.ON_DEATH then
			npc.AddBuff(npc.dwID, npc.nLevel, 11474, 1, 3)
		end
		return
	end
	if not npc.GetBuff(11474, 1) then
		target.AddBuff(target.dwID, target.nLevel, 11355, 1, 3)
	end
end
