---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/洛阳城北/回纥文书_搜索.lua
-- 更新时间:	2016/10/4 21:36:43
-- 更新用户:	zhengfeng5
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/洛阳城北/回纥文书_搜索.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local npcWenshu = GetNpc(dwSkillSrcID)
	if not npcWenshu then
		return
	end
	...
	if not player.GetBuff(11374, 1) then -- 没有偷书buff
		return
	end
	...
	local npc1 = scene.GetNpcByNickName("toushu_" .. player.dwID)	
	if	not npc1 then
		npc1 = scene.CreateNpc(54168, 43592, 35392, 1091456, 7, GLOBAL.GAME_FPS * 10, "toushu_" .. player.dwID, player.dwID)
		npcWenshu.SayTo(player.dwID, GetEditorString(10, 8682))
		npc1.ModifyThreat(player.dwID, 1000)
		player.SetTimer(32, "scripts/skill/Quest/洛阳城北/回纥文书_搜索.lua", 0, 0)
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	if  nParam1 == 0 then
		if npc1 then
			npc1.Say(GetEditorString(10, 8683))
			player.SetTimer(32, "scripts/skill/Quest/洛阳城北/回纥文书_搜索.lua", 1, 0)
		end
	elseif nParam1 == 1 then
		player.SetTimer(32, "scripts/skill/Quest/洛阳城北/回纥文书_搜索.lua", 2, 0)
	elseif nParam1 == 2 then
		if npc1 then
			npc1.ClearAllThreat()
			npc1.SetDisappearFrames(16)
			if player.GetBuff(11337, 1) then
				player.DelBuff(11337, 1)
			end
			if player.GetBuff(11337, 2) then
				player.DelBuff(11337, 2)
			end
		end
	end
