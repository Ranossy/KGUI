---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/洛阳城北/轰炸总控搜索技能16825.lua
-- 更新时间:	2016/9/20 21:15:29
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/洛阳城北/轰炸总控搜索技能16825.lua", -- 属性值1
		0														-- 属性值2
	);
...
local MARKBUFF = 11256
local RANDOM = 10000
local OFFSET = 3000
local NPC_SHITOU_TEMPLATEID = 53365
--魔法属性应用时的执行函数,dwCharacterID是魔法属性作用的目标ID
function Apply(dwCharacterID, dwSkillSrcID)
	local npc = GetNpc(dwSkillSrcID)--释放者。轰炸总控
	if not npc  then
		return
	end
	local nBoomNum = npc.GetCustomUnsigned4(0)
	if nBoomNum <= 0 then
		return
	end
	local target = nil
	local nBaseBoomRate = 2800--被轰炸概率1%。随机数用1万。
	if IsPlayer(dwCharacterID) then
		target = GetPlayer(dwCharacterID)
	else
		nBaseBoomRate = 1400--是npc改成0.5%
		target = GetNpc(dwCharacterID)
	end
	if not target then
		return
	end
	if target.GetBuff(MARKBUFF, 1) then
		return
	end

	if Random(RANDOM) <= nBaseBoomRate then--炸他
		npc.SetCustomUnsigned4(0, nBoomNum - 1)
		target.AddBuff(target.dwID, target.nLevel, MARKBUFF, 1, 9)
		local scene = npc.GetScene()
		local npc1 = scene.CreateNpc(NPC_SHITOU_TEMPLATEID, target.nX + (Random(OFFSET) - OFFSET / 2), target.nY + (Random(OFFSET) - OFFSET / 2), target.nZ + 100, 0, - 1)
		if npc1 then
			--	print(npc1.dwID)
			if npc.dwForceID == 28 then
				npc1.SetForceID(28)
				npc1.SetModelID(55324)
			else
				npc1.SetForceID(27)
				npc1.SetModelID(55333)
			end
		end
--	else
	--	print("不炸他" .. npc.GetCustomUnsigned4(0))
	end
end
