---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/洛阳城北/御花园_狼牙暗狩_搜索AOE.lua
-- 更新时间:	2016/9/16 23:44:22
-- 更新用户:	zhengfeng5
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/洛阳城北/御花园_狼牙暗狩_搜索AOE.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	for i = 1, #tBuffList do
		if character.GetBuff(tBuffList[i][1], tBuffList[i][2]) then
			nFlag = true
		end
		if nFlag then
			break
	...
	npc.ModifyThreat(dwCharacterID, 100)
end
