---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/洛阳城北/技_举盾维持aoe16952.lua
-- 更新时间:	2016/9/25 18:43:10
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/洛阳城北/技_举盾维持aoe16952.lua", -- 属性值1
		0																	-- 属性值2
	);
...
--	local npc = GetNpc(dwSelfID)
	--if  npc and npc.dwForceID == 27 then
		--npc.PlaySfx(1516, npc.nX, npc.nY, npc.nZ + 20,math.floor(360 * ((npc.nFaceDirection-128) % 256) / 256))
	--end

	--npc1=player.GetSelectCharacter();npc1.PlaySfx(1516, npc1.nX, npc1.nY, npc1.nZ + 500)
	local target
	...
	if target then
		target.AddBuff(target.dwID,target.nLevel,11313,1,5)
	end
end
