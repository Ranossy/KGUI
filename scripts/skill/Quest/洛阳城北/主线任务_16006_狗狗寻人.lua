---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/洛阳城北/主线任务_16006_狗狗寻人.lua
-- 更新时间:	2016/9/23 16:47:57
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<

--------------脚本文件开始------------------------------------------------
Include("scripts/Map/洛阳城北/include/表现相关/路人WaySearch.lua")
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/洛阳城北/主线任务_16006_狗狗寻人.lua",											-- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcDog = GetNpc(dwSkillSrcID)
	if not npcDog then
		return
	end
	if npcDog.GetCustomInteger1(0) > 0 then
		return
	end
	local player = GetPlayer(npcDog.dwOwner)
	if not player then
		return
	end
	if npcTarget.dwTemplateID ~= 50966 or npcTarget.GetCustomBoolean(NPCCUSTOM.ISNT_LANGYA) == true then
		return
	end
	local tOutRepresentID = {
		37503, 37816, 37817, 37827, 37839,
		37849, 42158, 42163,
		37328, 42168, 42173,
	}
	for i = 1, #tOutRepresentID do
		if npcTarget.dwModelID == tOutRepresentID[i] then
			return
		end
	end
	local tOutCustomID = {1, 2, 3}
	for i = 1, #tOutCustomID do
		if npcTarget.GetCustomInteger4(NPCCUSTOM.TEAM_ID) == tOutCustomID[i] then
			return
		end
	end
	if Random(100) > 10 then
		npcDog.SetCustomInteger1(0, 1)
		npcDog.DelBuff(11302, 1)
		npcDog.SetTarget(TARGET.NPC, npcTarget.dwID)
		npcDog.SayTo(player.dwID, GetEditorString(10, 254))
		npcDog.FireAIEvent(2001, 0, 0)
	else
		npcTarget.SetCustomBoolean(NPCCUSTOM.ISNT_LANGYA, true)
		--npcDog.RunTo(npcTarget.nX, npcTarget.nY)
		--player.SetTimer(2 * GLOBAL.GAME_FPS, "scripts/skill/Quest/洛阳城北/主线任务_16006_狗狗寻人.lua", npcTarget.dwID, npcDog.dwID)
	end
end
...
--function OnTimer(player, nParam1, nParam2)
	...
	--if not scene then
		--return
	--end
	--local npcDog = GetNpc(nParam2)
	--if not npcDog then
		--return
	--end
	--local npcTarget = GetNpc(nParam1)
	--if not npcTarget then
		--return
	--end
	--npcDog.DoAction(0, 10150)	
	--npcDog.SayTo(player.dwID, "汪汪汪！")
	--npcDog.SetDisappearFrames(1 * GLOBAL.GAME_FPS)
	--scene.CreateNpc(8047, npcTarget.nX, npcTarget.nY, npcTarget.nZ, 0, 3 * GLOBAL.GAME_FPS)
	--local npcNew = scene.CreateNpc(52900, npcTarget.nX, npcTarget.nY, npcTarget.nZ, npcTarget.nFaceDirection, 3 * GLOBAL.GAME_FPS)
	--if Random(100) > 50 then
		--npcNew.SetModelID(54555)
	--end
	--npcTarget.SetDisappearFrames(1 * GLOBAL.GAME_FPS)
	--npcNew.ModifyThreat(player.dwID,10000)
--end
