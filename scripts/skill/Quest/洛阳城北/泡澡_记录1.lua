---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/洛阳城北/泡澡_记录1.lua
-- 更新时间:	2016/10/15 17:35:18
-- 更新用户:	zhengfeng5
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local  BuffID = 11462
	local buff = player.GetBuff(11462, 1)
	if not buff  then
		return
	end

	local n = buff.nLeftActiveCount
	local tText = {
		GetEditorString(11, 280),
		GetEditorString(11, 340),
		GetEditorString(11, 282),
		GetEditorString(11, 283),
		GetEditorString(11, 281),
		GetEditorString(11, 279),
	}
	if tText[n] then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", tText[n], 4)
	end
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	if  BuffID == 11462 then
		local nCHEST_STYLE = nCustomValue
		player.SetRepresentIDDirectly(EQUIPMENT_REPRESENT.CHEST_STYLE, nCHEST_STYLE)

		-- 记录上衣表现COLORID1
		local buff2 = player.GetBuff(11463, 1)
		local nCHEST_COLOR = buff2.nCustomValue
		player.DelBuff(11463, 1)
		player.SetRepresentIDDirectly(EQUIPMENT_REPRESENT.CHEST_COLOR, nCHEST_COLOR)

		player.CloseLonelyMode()
		player.Jump(true, 0)
		player.SetPosition(36428, 34711, 1097792)

		if player.GetQuestPhase(16009) < 3 then
			player.ForceFinishQuest(16009)
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(11, 276), 4)
		end
	end
