---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/血战预热_大唐魂.lua
-- 更新时间:	2013/10/25 13:50:59
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 663);	--技能普通CD
...
--player.PlaySfx(610, player.nX + 100 * math.cos(nDir), player.nY + 100 * math.sin(nDir), player.nZ, player.nFaceDirection * 600 / 255 - 192 * 600 / 255)
	--print(1111111)
--	player.SetTimer(20, "scripts/skill/Quest/血战预热_大唐魂.lua", 0, 0)
	return nPreResult;
end
...
--player.PlaySfx(610, player.nX + 100 * math.cos(nDir), player.nY + 100 * math.sin(nDir), player.nZ, player.nFaceDirection * 600 / 255 - 192 * 600 / 255)
	--print(1111111)
	--player.SetTimer(20, "scripts/skill/Quest/血战预热_大唐魂.lua", 0, 0)
	print(GetEditorString(6, 732))

	if youshoupanduan(2, dwSkillSrcID, dwCharacterID) then
		if player.GetBuff(7080, 1) then
			local n = player.GetBuff(7080, 1).nStackNum
			print(GetEditorString(6, 733) .. n)
			if n == 1 then
				player.CastSkill(8455 + n, 1, TARGET.NPC, dwCharacterID)
				npc.Say(11111111)
			elseif n == 2 then
				player.CastSkill(8455 + n, 1, TARGET.NPC, dwCharacterID)
				npc.Say(22222)
			...
			player.CastSkill(8455, 1, TARGET.NPC, dwCharacterID)
			npc.Say(0000)
		end
	end
end
...
--print(222222)
--local x = player.nX + math.cos(player.nFaceDirection * math.pi * 2 / 256 - math.pi / 2) * 64 * 1
--local y = player.nY + math.sin(player.nFaceDirection * math.pi * 2 / 256 - math.pi / 2) * 64 * 1
--player.PlaySfx(610, x, y, player.nZ + 100, player.nFaceDirection * 360 / 256 + 90)
...
function youshoupanduan(d, c1, c2)
	local player = GetPlayer(c1)
	if not player then
		return false
	end
	local npc = GetNpc(c2)
	...
	local dir1 = player.nFaceDirection
	npc.TurnToCharacter(c1)
	local dir2 = npc.nFaceDirection-128
	local chazhi = dir1-dir2
	if chazhi < 0 then
		chazhi = chazhi + 256
	elseif chazhi > 256 then
		chazhi = chazhi-256
	end
	if chazhi > 128 then
		return false
	end
	local l = d / math.sin(chazhi / 128 * math.pi)
	if GetCharacterDistance(c1, c2) > l then
		return true
	end
	return false
end
