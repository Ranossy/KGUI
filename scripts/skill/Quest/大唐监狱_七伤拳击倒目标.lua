---------------------------------------------------------------------->
-- 脚本名称:	F:/zhugan/client/scripts/skill/Quest/大唐监狱_七伤拳击倒目标.lua
-- 更新时间:	2013/2/27 15:52:30
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = npc.GetBuff(5292, 1)
	if  buff.nCustomValue > 5 or buff.nCustomValue < 1 then
		buff.nCustomValue = 1
	end
	local target
	...
	target.nCurrentLife = target.nCurrentLife * 0.2 * (5-buff.nCustomValue)
	if buff.nCustomValue == 5 then
		target.Die()
	end
