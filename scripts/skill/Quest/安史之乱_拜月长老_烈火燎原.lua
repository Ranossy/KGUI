...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/安史之乱_拜月长老_烈火燎原.lua",
		0														
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player.nMoveState == MOVE_STATE.ON_JUMP then
		player.AddBuff(0, 99, 5823, 1)
	else
		local tDamageList = {14894, 14894 * 2, 14894 * 3, 14894 * 4, 14894 * 5}
		local npc_Caster = GetNpc(dwSrcID)
		if not npc_Caster then
			if player.nCurrentLife <= 20000 then
				player.Die()
			else
				player.nCurrentLife = player.nCurrentLife -20000
			end
			return
		end
		
		local nDamageLevel = 1
		local buff = npc_Caster.GetBuff(5824, 1)
		if buff then
			nDamageLevel = buff.nStackNum
			if not tDamageList[nDamageLevel] then
				nDamageLevel = 1
			end
		end
		
		if player.nCurrentLife <= tDamageList[nDamageLevel] then
			player.Die()
		else
			player.nCurrentLife = player.nCurrentLife -tDamageList[nDamageLevel]
		end
	end
end
