---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/霸刀_周六_陷阱标记AOE.lua
-- 更新时间:	2016/11/14 17:28:00
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
Include("scripts/Include/Skill.lh")
Include("scripts/Include/Player.lh")
Include("scripts/Map/ACT_95级门派事件/include/霸刀/周六NPC变量头文件.lua")
local nPQID = 187
local tTrap = {55336, 55377, 55379}
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/霸刀_周六_陷阱标记AOE.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcMark = GetNpc(dwSkillSrcID)
	if not player or not npcMark then
		return
	end

	local scene = npcMark.GetScene()
	if not scene then
		return
	end
--[[
	if Random(1000) > 800 then
		return
	end
--]]
	for nIndex = 1, MARK_TRAP.MAXTRAP_PER do
		if not scene.IsNickNameNpcExist(npcMark.szName .. "_" .. nIndex) then
			CreateSoldiers(npcMark, tTrap, nIndex)
			break
		end
	end
end
...
function CreateSoldiers(npcMaster, tTable, nNum)
	...
	--print(nNum,nCode)
	--for i = 1, nNum do
	local szNewName = npcMaster.szName .. "_" .. nNum
	local npcSoldier = scene.GetNpcByNickName(szNewName)
	--print(i .. ":" .. Pos_X .. "," .. Pos_Y .. "," .. nFace)
	if not npcSoldier then
		local nAngle = Random(256)
		local Pos_X = npcMaster.nX + Random(10 * 10) / 10 * 64 * math.cos(nAngle)
		local Pos_Y = npcMaster.nY + Random(10 * 10) / 10 * 64 * math.sin(nAngle)
		npcSoldier = scene.CreateNpc(tTable[Random(#tTable)], Pos_X, Pos_Y, npcMaster.nZ + 64 * 8 * 10, Random(256), 2 * 60 * GLOBAL.GAME_FPS, szNewName)
		if npcSoldier then
