---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/七秀_玩家_飞花.lua
-- 更新时间:	2015/12/21 11:27:15
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
local nPQID = 90
local tReward = {
	{dwID = 50646, nAddPQValue = 1, nAddPlayerValue = 1},--慢速跳跃
	{dwID = 50511, nAddPQValue = 1, nAddPlayerValue = 1},--慢速跳跃
	{dwID = 50140, nAddPQValue = 2, nAddPlayerValue = 2},--快速跳跃
	{dwID = 50141, nAddPQValue = 2, nAddPlayerValue = 2},--快速跳跃
	{dwID = 50655, nAddPQValue = 2, nAddPlayerValue = 2},--快速跳跃
	{dwID = 50648, nAddPQValue = 8, nAddPlayerValue = 8},--特殊NPC
	{dwID = 50649, nAddPQValue = 8, nAddPlayerValue = 8},--特殊NPC
	{dwID = 50650, nAddPQValue = 8, nAddPlayerValue = 8},--特殊NPC
}
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/七秀_玩家_飞花.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(10261, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	--skill.AddSlowCheckDestBuff(10261, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	local npcTarget = GetNpc(dwTargetID)
	if nTargetType == TARGET.NPC and npcTarget then
		player.TurnToCharacter(dwTargetID)
	else
		return SKILL_RESULT_CODE.INVALID_TARGET
	end
	if npcTarget.GetBuff(10388, 0) then
		return SKILL_RESULT_CODE.INVALID_TARGET
	end
	local bIsTrue = false
	for i = 1, #tReward do
		if npcTarget.dwTemplateID == tReward[i].dwID then
			bIsTrue = true
			break
		end
	end
	...
	--local JTbuffCD = player.GetBuff(10387, 1)
	--if not JTbuffCD then
	--RemoteCallToClient(player.dwID, "SetMaxCameraDistance", 300)
	--player.AddBuff(0, 99, 10386, 1, 1) --镜头恢复
	--player.AddBuff(0, 99, 10387, 1, 1) --镜头恢复
	--end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npcTarget then
		for i = 1, #tReward do
			if npcTarget.dwTemplateID == tReward[i].dwID then
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(9, 2958), 4)
				CommonPQ_AddPlayerPQValueTips(player, nPQID, tReward[i].nAddPlayerValue)
				CommonPQ_AddValue(nPQID, 1, tReward[i].nAddPQValue, scene)
				if not player.IsAchievementAcquired(5245) and (npcTarget.dwTemplateID == 50648 or npcTarget.dwTemplateID == 50649 or npcTarget.dwTemplateID == 50650) then	--成就“飞花逐影”
					player.AddAchievementCount(5245, 1)					
				end
				if not player.IsAchievementAcquired(5246) then
					if not player.GetBuff(10408, 0) then
						player.AddBuff(0, 99, 10408, 1, 60)
					end
					local buff = player.GetBuff(10408, 1)
					if buff then
						buff.nCustomValue = buff.nCustomValue + 1
						if buff.nCustomValue >= 10 then
							player.AddAchievementCount(5246, 1)
							player.DelBuff(10408, 1)
						end
					end
				end
				return
