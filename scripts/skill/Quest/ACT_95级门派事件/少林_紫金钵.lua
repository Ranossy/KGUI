---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/少林_紫金钵.lua
-- 更新时间:	2016/1/12 13:29:13
-- 更新用户:	wangsongtao-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/少林_紫金钵.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	local npc = GetNpc(dwSkillSrcID) --仲虚念自己
	if not npc then
		return
	end
	if npc.GetBuff(10426, 1) then -- 最后无敌阶段就不再放了
		return
	end
	local scene = npc.GetScene()
	if scene.GetNpcByNickName("PQ_CWJL_jinbo") then
		return
	end
	scene.CreateNpc(50546, npc.nX, npc.nY, npc.nY, 64, 20 * 60 * 16, "PQ_CWJL_jinbo")
end
