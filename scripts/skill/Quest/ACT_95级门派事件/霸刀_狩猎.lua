---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/霸刀_狩猎.lua
-- 更新时间:	2016/10/21 11:40:47
-- 更新用户:	zhangkaiyuan
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/霸刀_狩猎.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	----------------- 设置Cool down --------------------------------------------
	-- 公共CD
	skill.SetPublicCoolDown(1163);								-- 公共CD 1.5秒
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		...
		local npcTarget = scene.GetNpcByNickName("sheji_" .. player.dwID)
		if not npcTarget then
			npcTarget = scene.CreateNpc(55321, nX, nY, nZ, 0, 4 * GLOBAL.GAME_FPS, "sheji_" .. player.dwID)			
			local nDistance = math.sqrt(GetDistanceSq(player.nX, player.nY,0, nX, nY,0)/64/64)
			local nSpeed = 15
			local nTime = nDistance / nSpeed
			player.SetTimer(nTime * GLOBAL.GAME_FPS, "scripts/skill/Quest/ACT_95级门派事件/霸刀_狩猎.lua", 0, 0)
		end
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	local npcTarget = scene.GetNpcByNickName("sheji_" .. player.dwID)
	if npcTarget then
		player.PlaySfx(38,npcTarget.nX,npcTarget.nY,npcTarget.nZ)
		player.CastSkillXYZ(17082, 1, npcTarget.nX, npcTarget.nY, npcTarget.nZ)
