...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	-- 加动态技能栏
	if player.GetDynamicSkillGroup() ~= 233 then
		player.SetDynamicSkillGroup(233)
	end
	...
	--local buff1 = player.GetBuff(10392, 1)
	--if buff1 then
	--return 
	--end
	...
	if buff.nLeftActiveCount < 2 then
		player.AddBuff(player.dwID,player.nLevel,10395,1,60)
	end
	...
	if math.sqrt(GetDistanceSq(player.nX,player.nY,0,npcMark.nX,npcMark.nY,0 ))> 22 * 64 then 
--	if GetCharacterDistance(player.dwID, npcMark.dwID) > 22 * 64 then -- 22尺外
		player.DelBuff(10395, 1)
		player.Stop()
		player.AddBuff(player.dwID, player.nLevel, 10463, 1, 1)
