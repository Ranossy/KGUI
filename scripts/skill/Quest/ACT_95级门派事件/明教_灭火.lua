---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/明教_灭火.lua
-- 更新时间:	2016/1/11 16:20:17
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/明教_灭火.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
	...
	skill.SetNormalCoolDown(1, 1026);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	--end
	--if npcTarget.dwTemplateID ~= 50870 then
		--return SKILL_RESULT_CODE.INVALID_TARGET
	--end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc.dwTemplateID ~= 50870 then
		return
	end
	if not player then
		--	print("取不到player")
	end
	npc.nCurrentLife = math.max(0, npc.nCurrentLife - 100)
	if npc.nCurrentLife <= 100 then
		npc.Die()
	end
	CommonPQ_AddPlayerPQValueTips(player, 144, 5, GetEditorString(9, 8849)) -- player，pqID,本次增加的个人贡献度,pq的名称
	if not player.IsAchievementAcquired(5318) then
		player.AddAchievementCount(5318, 1)
	end
--	CommonPQ_AddValue(144, 1, 5, scene)--改变PQ数值：表ID，位置，数值。后两个参数可以忽略
end
