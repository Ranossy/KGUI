---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/明教_救助.lua
-- 更新时间:	2015/12/30 16:45:41
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<

--------------脚本文件开始------------------------------------------------
Include("scripts/Map/ACT_通用PQ/include/通用PQ常用函数安全版.lua")
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/明教_救助.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nCurrentNum = 0
	for i = 1, nMaxNum do
		if scene.IsNickNameNpcExist("PQ104" .. i .. "P" .. player.dwID) then
			nCurrentNum = nCurrentNum + 1
		end
	end

	if nCurrentNum == 0 then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(9, 2746), 4)
		player.DelBuff(10391, 1)
		player.DelBuff(10392, 1)
		--print(1)
		return
	end
	...
	-- 释放链状技能，实现拉人的表现
	for j = 1, nMaxNum do
		if scene.IsNickNameNpcExist("PQ104" .. j .. "P" .. player.dwID) then
			local npc = scene.GetNpcByNickName("PQ104" .. j .. "P" .. player.dwID)
			if GetCharacterDistance(player.dwID, npc.dwID) <  10 * 64 then		-- 10尺以内
				player.CastSkill(15350, 1, TARGET.NPC, npc.dwID)--目标已经在前边判断过。	15136
			end
		end
	end

	-- 判断跟流沙漩涡的距离，超过22尺则救人成功
	local buff = player.GetBuff(10395, 1)
	if not buff  then
		player.DelBuff(10391, 1)
		player.DelBuff(10392, 1)
		return
	end
	local npcID = buff.nCustomValue
	local npcMark = GetNpc(npcID)
	if not npcMark then
		player.DelBuff(10391, 1)
		player.DelBuff(10392, 1)
		return
	...
	-- 成功结算
	for k = 1, nMaxNum do
		if scene.IsNickNameNpcExist("PQ104" .. k .. "P" .. player.dwID) then
			local npc = scene.GetNpcByNickName("PQ104" .. k .. "P" .. player.dwID)
			if npc and npc.GetCustomUnsigned4(12) == 0 and (GetCharacterDistance(npcMark.dwID, npc.dwID) > 17 * 64 or GetCharacterDistance(npcMark.dwID, player.dwID) > 21 * 64) then -- 19尺外
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(9, 2978), 4)
				if not player.IsAchievementAcquired(5317) then
					player.AddAchievementCount(5317, 1)
				end
			--	player.DelBuff(10391, 1)
		--		player.DelBuff(10392, 1)
				-- 在这里可以给玩家加PQ贡献度了
				CommonPQ_AddPlayerPQValueTips(player, 104, 8, GetEditorString(9, 8046)) -- player，pqID,本次增加的个人贡献度,pq的名称
				CommonPQ_AddValue(104, 1, 8, scene)--改变PQ数值：表ID，位置，数值。后两个参数可以忽略
				--	player.SendSystemMessage("你的明教门派事件周一到周五的个人贡献度增加了")
				npc.SetDisappearFrames(48, 1)
				npc.SetCustomUnsigned4(12,1)--标记已经救成功
				npc.Say(player.szName .. GetEditorString(9, 2979))
				nCurrentNum = nCurrentNum - 1
				if npcMark.GetCustomUnsigned4(0) >= 1 then
					npcMark.SetCustomUnsigned4(0, npcMark.GetCustomUnsigned4(0) - 1)
				end
			--else
				--print( npc.GetCustomUnsigned4(4))
			end
		end
	end
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	--local buff10392 = player.GetBuff(10392, 1)
	--if buff10392 then
	--player.DelBuff(10392, 1)
	--return
	--end
