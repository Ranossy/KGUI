---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/天策_加护送标记.lua
-- 更新时间:	2015/12/21 0:41:58
-- 更新用户:	mantong2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/天策_加护送标记.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.GetBuff(10384, 1) then
		player.AddBuff(0, 99, 10384, 1)
	end
	local buff = player.GetBuff(10384, 1)
	if buff.nStackNum < 50 then
		player.AddBuff(0, 99, 10384, 1)
	else
		return
	end
	player.SendSystemMessage(GetEditorString(9, 1878) )
	CommonPQ_AddPlayerPQValueTips(player, 136, 1)
end
