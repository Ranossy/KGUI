---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/丐帮_钓鱼_投竿.lua
-- 更新时间:	2016/1/15 14:54:15
-- 更新用户:	zhangtianhui1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/丐帮_钓鱼_投竿.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(10447, 1) then
		return
	end
	if player.bFightState then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(9, 8671), 4)
		return
	end
	if player.nMoveState ~= MOVE_STATE.ON_STAND then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(9, 8672), 4)
		return
	end
	local dwTrapHashID = GetFileNameHash("scripts/Map/ACT_95级门派事件/丐帮/trap/钓鱼范围.lua")
	if player.dwLastScriptID ~= dwTrapHashID or player.nZ > 1097472 then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(9, 8673), 4)
		return
	end
	RemoteCallToClient(player.dwID, "OnDynamicSkillHighlightChanged", 1, false)
	if player.GetFreeRoomSize() < 1 then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(9, 8358), 4)
		return
	end
	player.DoAction(0, 11355)
	player.AddBuff(0, 99, 10447, 1)
	player.AddBuff(0, 99, 6754, 1, 10)
	player.SetTimer(5 * GLOBAL.GAME_FPS, "scripts/Map/ACT_95级门派事件/丐帮/event/钓鱼随机处理.lua", 0, 0)
	player.SetTimer(3.5 * GLOBAL.GAME_FPS, "scripts/skill/Quest/ACT_95级门派事件/丐帮_钓鱼_投竿.lua", 0, 0)
end

function OnTimer(player, nParam1, nParam2)
	if player.GetBuff(10447, 1) then
		player.DoAction(0, 11354)
	end
end
