---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/长歌_跳动的音符.lua
-- 更新时间:	2015/11/30 16:09:16
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/长歌_跳动的音符.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	local character
	if GetNpc(dwCharacterID) then
		character = GetNpc(dwCharacterID)
	elseif GetPlayer(dwCharacterID) then
		character = GetPlayer(dwCharacterID)
	end
	local scene = character.GetScene()
	local tPosition = {
		{28866, 108934, 1060992, 118},
		{28650, 109586, 1061888, 146},
		{27433, 109142, 1061056, 239},
		{27913, 108614, 1061120, 225},
	}
	for i = 1, 4 do
		local npcYinfu = scene.GetNpcByNickName("PQ114_yinfu" .. i)
		if not npcYinfu then
			npcYinfu = scene.CreateNpc(50206, tPosition[i][1], tPosition[i][2], tPosition[i][3], tPosition[i][4], - 1, "PQ114_yinfu" .. i)
			npcYinfu.SetCustomUnsigned2(0, 114)
			break
		end
	end
end
