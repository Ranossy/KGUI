---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/明教_血染红衣.lua
-- 更新时间:	2016/1/12 10:56:41
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/明教_血染红衣.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not scene.IsNickNameNpcExist("ACT_PQ144") then
		player.DelBuff(10424, 1)
		if player.GetDynamicSkillGroup() == 238 then
			player.SetDynamicSkillGroup(0)		
		end
		return 
	end	
	
	local dwHashValue = GetFileNameHash("scripts/Map/世界奇遇/trap/三尺青锋_三生树.lua")
	if player.GetMapID() ~= 150 or player.dwLastScriptID ~= dwHashValue then
		--print("不在trap中")
		player.DelBuff(10424, 1)
		if player.GetDynamicSkillGroup() == 238 then
			player.SetDynamicSkillGroup(0)		
	...
	if player.GetDynamicSkillGroup() ~= 238 then
		player.SetDynamicSkillGroup(0)
		player.SetDynamicSkillGroup(238)
		return
	end
	--player.SetDynamicSkillGroup(238)
end
