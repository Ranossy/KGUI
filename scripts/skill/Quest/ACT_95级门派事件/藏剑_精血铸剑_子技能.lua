---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/藏剑_精血铸剑_子技能.lua
-- 更新时间:	2015/12/9 23:54:16
-- 更新用户:	wangwei-pc2
-- 脚本说明:	15275
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/藏剑_精血铸剑_子技能.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not scene.IsNickNameNpcExist("PQMarkNPC129Num1") then
		return
	end
	if CommonPQ_GetValue(129, 2, scene) < 1500 then
		if player.nCurrentLife > player.nMaxLife * 0.1 then
			player.nCurrentLife = player.nCurrentLife - player.nMaxLife * 0.1
			CommonPQ_AddPlayerPQValueTips(player, 129, 1)
			CommonPQ_AddValue(129, 2, 1, scene)
			--if not player.GetBuff(10332, 1) then
			player.AddBuff(0, 99, 10332, 1, 60)  --吸收治疗BUFF			
			player.AddBuff(0, 99, 10336, 1, 11)  --判断标记BUFF
			local buff = player.GetBuff(10336, 1)
			if buff then
				if buff.nStackNum >= 9 then
					player.DoAction(0, 60032)
				end
			end
			--end
		else
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(9, 1933), 4)
			player.CastSkill(15280, 1)  --打断自己的技能
			player.DoAction(0, 60032)
			player.PlaySfx(1261, 80775, 76590, 1113936)
		end
	else
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(9, 1934), 4)
		player.CastSkill(15280, 1)  --打断自己的技能
		player.DoAction(0, 60032)
		player.PlaySfx(1261, 80775, 76590, 1113936)
	end
