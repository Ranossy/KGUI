---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/七秀_通用圆形AOE.lua
-- 更新时间:	2015/12/27 22:07:21
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/七秀_通用圆形AOE.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene then
		local tBoss = {
			[50697] = "PQ91_XiaNian",
			[50698] = "PQ91_TongYaChen",
			[50699] = "PQ91_MuRongWan",
			}
		local npcBOSS = scene.GetNpcByNickName(tBoss[npc.dwTemplateID])
		if npcBOSS then
			dwCasterID = npcBOSS.dwID
		end
	end
	player.CustomDamage(dwCasterID, 15334, 1, 3, player.nMaxLife * 0.12)
end
