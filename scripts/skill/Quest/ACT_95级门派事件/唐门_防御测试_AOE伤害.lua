---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/唐门_防御测试_AOE伤害.lua
-- 更新时间:	2016/1/27 16:31:33
-- 更新用户:	wangwei-pc2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/唐门_防御测试_AOE伤害.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	--skill.AddSlowCheckDestBuff(10451, 1, eCompareFlag, 0, eLevelCompareFlag);		-- 需求目标Buff
	--skill.AddSlowCheckSelfOwnBuff(dwBuffID, nStackNum, eCompareFlag1 nLevel, eLevelCompareFlag);	-- 需求自身属于自己的Buff
	...
	----------------- 作用范围 -------------------------------------------------
	skill.nHeight = 18 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not scene.IsNickNameNpcExist("PQMarkNPC137Num1") then
		return
	end
	local buff1 = player.GetBuff(10473, 1)
	if buff1 then
		if CommonPQ_GetValue(137, 1, scene) < 1500 then
			CommonPQ_AddPlayerPQValueTips(player, 137, 2)
			CommonPQ_AddValue(137, 1, 2, scene)		
			--player.PlaySfx(1205, player.nX, player.nY, player.nZ)
			-----------用同时添加的禁止上马BUFF变量来记录成就--------
			local buff1_1 = player.GetBuff(2587, 1)
			buff1_1.nCustomValue = buff1_1.nCustomValue + 1			
			if buff1_1.nCustomValue == 10 then
				if not player.IsAchievementAcquired(5312) then     
					player.AcquireAchievement(5312)    
				end	
			end
		end
	end
	local buff2 = player.GetBuff(10473, 2)
	if buff2 then
		if CommonPQ_GetValue(137, 2, scene) < 1500 then
			CommonPQ_AddPlayerPQValueTips(player, 137, 2)
			CommonPQ_AddValue(137, 2, 2, scene)		
			--player.PlaySfx(1205, player.nX, player.nY, player.nZ)
			local buff2_1 = player.GetBuff(2587, 1)
			buff2_1.nCustomValue = buff2_1.nCustomValue + 1			
			if buff2_1.nCustomValue == 10 then
				if not player.IsAchievementAcquired(5312) then     
					player.AcquireAchievement(5312)    
				end	
			end
