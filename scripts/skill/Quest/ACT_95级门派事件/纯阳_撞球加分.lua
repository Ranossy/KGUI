---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/纯阳_撞球加分.lua
-- 更新时间:	2016/1/8 12:02:34
-- 更新用户:	zhangchennan
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/纯阳_撞球加分.lua", -- 属性值1
		0														-- 属性值2
	);
	--skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(10325, 1) then
		if npc.dwTemplateID == 50458 then
			CommonPQ_AddValue(116, 1, 10, scene)
			CommonPQ_AddPlayerPQValueTips(player, 116, 4)
			player.DelBuff(10325, 1)
			player.AddBuff(0, 99, 10331, 1)
		elseif npc.dwTemplateID == 50459 then
			CommonPQ_AddValue(116, 1, 8, scene)
			CommonPQ_AddPlayerPQValueTips(player, 116, 3)
			player.DelBuff(10325, 1)
			player.AddBuff(0, 99, 10331, 1)
		else
			CommonPQ_AddValue(116, 1, 5, scene)
			CommonPQ_AddPlayerPQValueTips(player, 116, 2)
			player.DelBuff(10325, 1)
			player.AddBuff(0, 99, 10331, 1)
		end
	else
		if player.GetBuff(10331, 1)then
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(9, 7982), 3)
		else
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(9, 1926), 3)
		end
		return
	end
end
