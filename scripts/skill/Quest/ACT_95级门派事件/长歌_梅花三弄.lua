---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/长歌_梅花三弄.lua
-- 更新时间:	2015/11/25 10:44:27
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local t = character.GetAbsorbInfo()
	local nAbsorbValue = t[1].nAbsorbType
	local nAbsorbUsedValue = t[1].nAbsorbUsedValue
	local nNewAbsorbValue = nAbsorbValue - nAbsorbUsedValue + nAddnAbsorbValue
	for i = 1, 10 do
		character.DelBuff(nBuffID, i)
	end
	local nNewLevel = 1
	for j = 10, 1 do
		if nNewAbsorbValue >= tLevel[j] then
			nNewLevel = j
			break
		end
	end
	character.AddBuff(character.dwID, 99, nBuffID, nNewLevel, 6 * 60) -- 60分钟的buff
end
