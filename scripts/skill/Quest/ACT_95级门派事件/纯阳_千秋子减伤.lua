---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/纯阳_千秋子减伤.lua
-- 更新时间:	2016/2/3 0:10:10
-- 更新用户:	zhangchennan
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	local npcQianQiuZi = GetNpc(dwCharacterID)
	if not npcQianQiuZi then
		return
	end
	local buff = npcQianQiuZi.GetBuff(10480, 1)
	if not buff then
		return
	end
	if buff.nStackNum > 0 then
		if npcQianQiuZi.GetBuff(10480, 1) then
			npcQianQiuZi.DelBuff(10480, 1)
		end
	end
end
