---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/霸刀_蛇仙女_蛇鼠一窝.lua
-- 更新时间:	2016/11/9 21:40:14
-- 更新用户:	zhengfeng5
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/霸刀_蛇仙女_蛇鼠一窝.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	npc.AddBuff(0, 99, 11505, 1, 10)
	npc.Yell(GetEditorString(11, 1869))
	...
	local nY = npc.nY
	local nZ = npc.nZ
	local szName = "ACT_BD_SXN_SHUWO_"
-- 原始点X，原始点Y,原始点朝向，半径，偏移角度（0表示正前方，180表示正后方）
	for i = 1, 3 do
		if not scene.GetNpcByNickName(szName .. i) then
			local nNx, nNy, nNfaceDirection = GetPositionByAngleAndRadius(nX, nY, nFaceDirection, 64 * Random(5, 10), 120 * i - 1)
			scene.CreateNpc(55357, nNx, nNy, nZ + 400, nFaceDirection, GLOBAL.GAME_FPS * 60 * 10, szName .. i)
		end
	end
end
