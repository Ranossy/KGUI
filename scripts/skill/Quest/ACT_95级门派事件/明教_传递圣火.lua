---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/明教_传递圣火.lua
-- 更新时间:	2016/1/8 10:44:45
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/明教_传递圣火.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--if not scene.IsNickNameNpcExist("ACT_PQ143") then
		--return 
	--end
	if target.GetDynamicSkillGroup() == 0 then
		target.AddBuff(0, 99, 10423, 1, 12 * 60)
		target.SetDynamicSkillGroup(237)
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(9, 8865) .. target.szName, 3)
	else
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOWe", target.szName .. GetEditorString(9, 8866), 3)
	end
