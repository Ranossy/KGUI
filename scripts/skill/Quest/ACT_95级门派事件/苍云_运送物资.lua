---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/苍云_运送物资.lua
-- 更新时间:	2015/12/9 16:03:29
-- 更新用户:	zhangtianhui1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/苍云_运送物资.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.SendSystemMessage(GetEditorString(9, 1878) )
	CommonPQ_AddPlayerPQValueTips(player, 109, 1)
	if not buff then
		player.AddBuff(0, 99, 10327, 1, 1)
	end
	local buff = player.GetBuff(10327, 1)
	if buff.nStackNum < 5 then
		player.AddBuff(0, 99, 10327, 1, 1)
	end
	if not player.GetBuff(10328, 1) then
		player.AddBuff(0, 99, 10328, 1)
	end
end
