---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/明教_安雨_阳极_灼热.lua
-- 更新时间:	2016/2/15 17:56:31
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/明教_安雨_阳极_灼热.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		if scene then
			local npcBOSS = scene.GetNpcByNickName("PQ144_anyu")
			if npcBOSS then
				dwCasterID = npcBOSS.dwID
			end
		end
		player.CustomDamage(dwCasterID, 15482, 1, 1, player.nMaxLife * 0.1)
		--if player.GetBuff(10501, 1) then
			--player.DelGroupBuff(10501, 1)
		--end
	end
end
