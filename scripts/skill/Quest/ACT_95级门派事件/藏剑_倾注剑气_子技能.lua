---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/藏剑_倾注剑气_子技能.lua
-- 更新时间:	2015/12/2 20:50:13
-- 更新用户:	wangwei-pc2
-- 脚本说明:	15275
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/藏剑_倾注剑气_子技能.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not scene.IsNickNameNpcExist("PQMarkNPC105Num1") then
		return
	end
	if CommonPQ_GetValue(105, 2, scene) < 1500 then
		if player.nCurrentRage >= 20 then
			player.nCurrentRage = player.nCurrentRage - 20
			CommonPQ_AddPlayerPQValueTips(player, 105, 1)
			CommonPQ_AddValue(105, 2, 1, scene)
		else
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(9, 1123), 4)
			player.CastSkill(15280, 1)  --打断自己的技能
		end
	else		
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(9, 1124), 4)
		player.CastSkill(15280, 1)  --打断自己的技能
	end
