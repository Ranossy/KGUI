---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/明教_套人.lua
-- 更新时间:	2015/12/30 16:34:53
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/明教_套人.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if not tNpcList[npcTarget.dwTemplateID] then
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if nTargetType ~= TARGET.NPC then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(9, 8861), 4)
		return
	end
	...
	if 50680 == target.dwTemplateID then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(9, 8863), 4)
		return
	end
	if not tNpcList[target.dwTemplateID] then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(9, 8861), 4)
		return
	end
	...
	if target.GetCustomInteger1(1) > 0 then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(9, 8830), 4)
		return
	end
	...
	if nCurrentNum >= nMaxNum then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(9, 2983) .. nMaxNum .. GetEditorString(9, 8837), 4)
		return
	end
	local npc
	for i = 1, nMaxNum do
		if not scene.IsNickNameNpcExist("PQ104" .. i .. "P" .. player.dwID) then
			npc = scene.CreateNpc(50680, target.nX, target.nY, target.nZ, target.nFaceDirection, 300 * GLOBAL.GAME_FPS, "PQ104" .. i .. "P" .. player.dwID)  --非私有NPC
			npc.SetCustomUnsigned4(0, player.dwID)
			npc.SetCustomUnsigned4(4, i)
			npc.SetModelID(target.dwModelID)
			...
			npc.SetCustomUnsigned4(8, npcMark.dwID)
			break
		end
	end
	
	--npc.SetModelID(target.dwModelID + 1082)  --3中表现都是对应的连续ID，直接加上差数
	target.SetDisappearFrames(16, 1)
	target.SetCustomInteger1(1, 1)
	player.AddBuff(0, 99, 10391, 1, 300 * 4)
	player.AddBuff(0, 99, 10392, 1, 300)
	--player.SetModelID(52199)
	--player.AddBuff(0, 99, 10395, 1, 300)
