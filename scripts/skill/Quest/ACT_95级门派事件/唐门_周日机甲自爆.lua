---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/唐门_周日机甲自爆.lua
-- 更新时间:	2016/2/2 20:07:37
-- 更新用户:	zhangchennan
-- 脚本说明:
----------------------------------------------------------------------<
...
Include("scripts/Map/ACT_95级门派事件/include/通用PQ杂兵头文件.lua")
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	npc.PlaySfx(1277, npc.nX, npc.nY, npc.nZ)
	if npc then
		npc.SetDisappearFrames(64, false)
	end
	local scene = npc.GetScene()
	if not scene then
		return
	end
	local npcQianQiuZi = scene.GetNpcByNickName("PQ140_bayeguhan")
	if npcQianQiuZi then
		CommonPQ_OnDeath(npcQianQiuZi, npc)
		npcQianQiuZi.Yell(GetEditorString(9, 8823))
		npcQianQiuZi.Die()
	end
	local npcShua = scene.GetNpcByNickName("PQ140_shuaguaizongkong")
	if npcShua then
		npcShua.SetDisappearFrames(1, false)
	end
	for j = 1, 100 do
		for k = 1, 10 do
			for  m = 1, 4 do
				local npcShua = scene.GetNpcByNickName("RKHMA" .. j .. "n" .. k .. m)
				if npcShua then
					npcShua.Die()
				end
			end
