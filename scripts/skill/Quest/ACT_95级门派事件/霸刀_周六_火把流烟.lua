---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/霸刀_周六_火把流烟.lua
-- 更新时间:	2016/11/14 17:01:58
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/霸刀_周六_火把流烟.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npcTarget and npcTarget.dwTemplateID == 55333 then
		player.PlaySfx(1549, npcTarget.nX, npcTarget.nY, npcTarget.nZ)		
		local scene = npcTarget.GetScene()
		if scene and not scene.IsNickNameNpcExist("PQMarkNPC187Num1") then
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(11, 1926), 4)
			return
		end
		if not npcTarget.GetBuff(11502, 1) then
			npcTarget.FireAIEvent(2001, 0, 0)
		end
	else
		player.PlaySfx(1549, 0, 0, 0)
	end
end
