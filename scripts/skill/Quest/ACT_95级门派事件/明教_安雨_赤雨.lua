---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/明教_安雨_赤雨.lua
-- 更新时间:	2016/2/15 18:02:29
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/明教_安雨_赤雨.lua",											-- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nAngle = Random(256) / 256 * math.pi * 2
	local Pos_X = npcBoss.nX + Random(20, 300) / 10 * 64 * math.cos(nAngle)
	local Pos_Y = npcBoss.nY + Random(20, 300) / 10 * 64 * math.sin(nAngle)
	local npcAOE = scene.CreateNpc(50882, Pos_X, Pos_Y, npcBoss.nZ, 0, 5 * GLOBAL.GAME_FPS)
	if npcAOE then
		--npcBoss.PlaySfx(1268, Pos_X, Pos_Y, npc.nZ)
		npcAOE.AddBuff(0, 99, 10503, 1, 3)
	end
end
