---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/纯阳_四象方星血量判断.lua
-- 更新时间:	2015/12/29 23:55:09
-- 更新用户:	zhangchennan
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	local npcSiXiangFangXingShi = GetNpc(dwCharacterID)
	if not npcSiXiangFangXingShi then
		return
	end
	local scene = npcSiXiangFangXingShi.GetScene()
	if not scene then
		return
	end
	local npcBiaoXianZongKong = scene.GetNpcByNickName("PQ117_zhouliuP1zhuanP2")
	if npcSiXiangFangXingShi.nCurrentLife > npcSiXiangFangXingShi.nMaxLife * 0.2 then
		--print(2222)
		npcSiXiangFangXingShi.AddBuff(0, 99, 10406, 1, 30)
	else
		npcSiXiangFangXingShi.nCurrentLife = npcSiXiangFangXingShi.nMaxLife
		npcSiXiangFangXingShi.DelBuff(10406, 1)
		npcSiXiangFangXingShi.SetCustomInteger1(0, 1)
	end
end
