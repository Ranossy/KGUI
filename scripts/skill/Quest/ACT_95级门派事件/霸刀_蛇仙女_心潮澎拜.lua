---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/霸刀_蛇仙女_心潮澎拜.lua
-- 更新时间:	2016/11/9 21:44:36
-- 更新用户:	zhengfeng5
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		ATTRIBUTE_TYPE.CALL_REPULSED, -- 魔法属性
		15, -- 属性值1
		0														-- 属性值2
	);
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/霸刀_蛇仙女_心潮澎拜.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
--	print(dwSkillSrcID)
	...
	npc.Say(GetEditorString(11, 1870))
	...
	local nY = npc.nY
	local nZ = npc.nZ
	local szName = "ACT_BD_SXN_DUYE_"
-- 原始点X，原始点Y,原始点朝向，半径，偏移角度（0表示正前方，180表示正后方）
	for i = 1, 20 do
		if not scene.GetNpcByNickName(szName .. i) then
			local nNx, nNy, nNfaceDirection = GetPositionByAngleAndRadius(nX, nY, nFaceDirection, 64 * Random(5, 25), 120 * i - 1)
			scene.CreateNpc(55358, nNx, nNy, nZ + 400, nFaceDirection, GLOBAL.GAME_FPS * 30, szName .. i)
		end
	end
end
--魔法属性反应用时的执行函数,dwCharacterID是魔法属性作用的目标ID
