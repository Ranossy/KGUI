---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/丐帮_巨龟_喷毒.lua
-- 更新时间:	2016/1/12 10:55:55
-- 更新用户:	zhangtianhui1
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 600, nDamageRand = 60, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/丐帮_巨龟_喷毒.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local npcJugui = GetNpc(dwSkillSrcID)
	if not npcJugui then
		return
	end
	local scene = npcJugui.GetScene()
	if not scene then
		return
	end
	local npcDupao = scene.GetNpcByNickName("GBPQ_Sta_Dupao")
	if not npcDupao then
		npcDupao = scene.CreateNpc(50948, 95000, 97152, 1093248, 236, 10 * GLOBAL.GAME_FPS, "GBPQ_Sta_Dupao")
	end
end
