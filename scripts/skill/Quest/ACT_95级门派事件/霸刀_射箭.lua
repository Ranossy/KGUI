---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/霸刀_射箭.lua
-- 更新时间:	2016/10/31 14:27:19
-- 更新用户:	zhangkaiyuan
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	--ATTRIBUTE_TYPE.GLOBAL_RESIST_PERCENT, -- 魔法属性
	--- 1024, -- 属性值1
	--0											-- 属性值2
	--);
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/霸刀_射箭.lua", -- 属性值1
		0											-- 属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	--skill.AddSlowCheckDestBuff(11489, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- 设置Cool down --------------------------------------------
	-- 公共CD
	skill.SetPublicCoolDown(1165);							-- 公共CD 1.5秒
	...
	--skill.nRectWidth = 5 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local ntheta = player.nFaceDirection / 256 * 360/180*math.pi
	--print("ahhhh" .. ntheta)
	local nR = 15 * 64
	local npcX,npcY = GetRectangular(nR,ntheta)
	local npcTarget = scene.GetNpcByNickName("shejian_" .. player.dwID)
	local npcOrigin = scene.GetNpcByNickName("Origin_" .. player.dwID)
	if not npcTarget then
		npcTarget = scene.CreateNpc(55343, player.nX + npcX, player.nY + npcY, player.nZ + 10000, 1, 10*16, "shejian_" .. player.dwID)
	end
	if not npcOrigin then
		npcOrigin = scene.CreateNpc(55343, player.nX, player.nY, player.nZ, 1, 10*16, "Origin_" .. player.dwID)
	end
	player.SetTimer(1, "scripts/skill/Quest/ACT_95级门派事件/霸刀_射箭.lua", 0, 0)
end
...
function GetSpherical(nx, ny)
	r = math.sqrt((nx * nx + ny * ny) / 64 / 64)
	if nx > 0 and ny > 0 then
		theta = math.atan(ny / nx)
	elseif nx < 0 and ny > 0 then
		theta = math.atan(ny / nx) + math.pi
	elseif nx < 0 and ny < 0 then
		theta = math.atan(ny / nx) + math.pi
	elseif nx > 0 and ny < 0 then 
		theta = math.atan(ny / nx) + (2 * math.pi)
	end	
	--print("r="..r)
	--print("theta="..theta)
	return r, theta
...
function GetRectangular(r, theta)
	x = r * math.cos(theta)
	y = r * math.sin(theta)
	--print("x="..x)
	--print("y="..y)
...
function OnTimer(player, nParam1, nParam2)
	...
	local nR, ntheta = GetSpherical(npcTarget.nX - npcOrigin.nX, npcTarget.nY - npcOrigin.nY)
	nR = math.floor(nR+0.5)
	local nPace = npcTarget.GetCustomUnsigned4(0) + 32
	if nPace / 64 <= nR then
		local nx, ny = GetRectangular(nPace, ntheta)
		player.PlaySfx(472, npcOrigin.nX + nx, npcOrigin.nY + ny, npcOrigin.nZ+500)
		player.CastSkillXYZ(17086,1,npcOrigin.nX + nx,npcOrigin.nY + ny, npcOrigin.nZ)
		npcTarget.SetCustomUnsigned4(0, nPace)
		player.SetTimer(0.1 * GLOBAL.GAME_FPS, "scripts/skill/Quest/ACT_95级门派事件/霸刀_射箭.lua", 0, 0)
	else
		npcTarget.SetDisappearFrames(1)
		npcOrigin.SetDisappearFrames(1)
	end
end
