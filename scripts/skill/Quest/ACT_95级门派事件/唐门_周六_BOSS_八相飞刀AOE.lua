---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/唐门_周六_BOSS_八相飞刀AOE.lua
-- 更新时间:	2016/2/1 17:24:41
-- 更新用户:	wangwei-pc2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddSlowCheckDestBuff(10451, 0, BUFF_COMPARE_FLAG.EQUAL, 2, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	local npc = GetPlayer(dwCharacterID)
	if not npc then
		return
	end
	...
	for i = 1, 6 do
		local feidao = scene.GetNpcByNickName("PQ_feidao" .. i)
		if not feidao then
			scene.CreateNpc(50828, npc.nX, npc.nY. npc.nZ, 192, 50 * GLOBAL.GAME_FPS, "PQ_feidao" .. i)
		end
	end
end
