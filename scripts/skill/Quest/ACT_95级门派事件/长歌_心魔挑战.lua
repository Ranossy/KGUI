---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/长歌_心魔挑战.lua
-- 更新时间:	2015/11/27 15:42:43
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = player.GetBuff(10253, 1)
	if not buff then
		return 
	end
	local nTiaoshu = buff.nLeftActiveCount
	if nTiaoshu == 60 then
		RemoteCallToClient(player.dwID, "CountDown", nTiaoshu) --屏幕数字
	elseif nTiaoshu == 30 then
		RemoteCallToClient(player.dwID, "CountDown", nTiaoshu) --屏幕数字
	elseif nTiaoshu == 20 then
		RemoteCallToClient(player.dwID, "CountDown", nTiaoshu) --屏幕数字
	elseif nTiaoshu <= 10 and nTiaoshu > 1 then
		RemoteCallToClient(player.dwID, "CountDown", nTiaoshu) --屏幕数字
	elseif nTiaoshu <= 1 then
		RemoteCallToClient(player.dwID, "CountDown", nTiaoshu) --屏幕数字
		RemoteCallToClient(player.dwID, "CountDown", "Close")
	end
end
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	local npcXinmo = scene.GetNpcByNickName("ACT308_XinShi_" .. player.dwID)
	if npcXinmo then
		npcXinmo.SetDisappearFrames(1)
	end

	RemoteCallToClient(player.dwID, "On_Trial_Screen_Black")
	RemoteCallToClient(player.dwID, "EnableColorShift", false) -- 退出滤镜模式
	player.CloseLonelyMode()
	player.SetPosition(93129, 53362, 1058304)
	player.TurnTo(108)
	--player.DelBuff(10253, 1)
end
