---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/七秀_玩家_飞花母技能.lua
-- 更新时间:	2015/12/29 22:30:00
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
local tReward = {
	{dwID = 50646, nAddPQValue = 2, nAddPlayerValue = 2},--慢速跳跃
	{dwID = 50511, nAddPQValue = 2, nAddPlayerValue = 2},--慢速跳跃
	{dwID = 50140, nAddPQValue = 5, nAddPlayerValue = 5},--快速跳跃
	{dwID = 50141, nAddPQValue = 5, nAddPlayerValue = 5},--快速跳跃
	{dwID = 50655, nAddPQValue = 5, nAddPlayerValue = 5},--快速跳跃
	{dwID = 50648, nAddPQValue = 10, nAddPlayerValue = 10},--特殊NPC
	{dwID = 50649, nAddPQValue = 10, nAddPlayerValue = 10},--特殊NPC
	{dwID = 50650, nAddPQValue = 10, nAddPlayerValue = 10},--特殊NPC
}
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/七秀_玩家_飞花母技能.lua",											-- 属性值1
		0											-- 属性值2
	);
	...
	skill.SetNormalCoolDown(1, 969);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		if nTargetType == TARGET.NPC and npcTarget then
			...
			if player.GetBuff(10408, 0) then
				player.DelBuff(10408, 1)
			end
			player.DoAction(0, 13983)
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(9, 2962), 4)
			return
		end
		if npcTarget.GetBuff(10388, 0) then
			...
			player.DoAction(0, 13983)
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(9, 2957), 4)
			return
		end
		local bIsTrue = false
		for i = 1, #tReward do
		...
		if not bIsTrue then
			...
			player.DoAction(0, 13983)
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(9, 2956), 4)
			return
		end
		local nSkillResult = player.CastSkill(15335, 1)
		if nSkillResult == SKILL_RESULT_CODE.TOO_FAR_TARGET then
			player.DoAction(0, 13983)
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(9, 2977), 4)
			if player.GetBuff(10408, 0) then
				player.DelBuff(10408, 1)
			end
		elseif nSkillResult ~= SKILL_RESULT_CODE.SUCCESS then
			player.DoAction(0, 13983)
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(9, 2955), 4)
			if player.GetBuff(10408, 0) then
				player.DelBuff(10408, 1)
			end
		end
