---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/苍云_周日_吐火圈.lua
-- 更新时间:	2015/12/21 16:06:54
-- 更新用户:	zhangtianhui1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/苍云_周日_吐火圈.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if Random(1, 10000) <= 5000 then
		for i = 1, 10 do
			local npcQuan = scene.GetNpcByNickName("CYPQ_Sun_Quan" .. i)
			if not npcQuan then
				npcQuan = scene.CreateNpc(50653, player.nX, player.nY, player.nZ, player.nFaceDirection, 60 * GLOBAL.GAME_FPS, "CYPQ_Sun_Quan" .. i)
				return
			end
		end
	end
