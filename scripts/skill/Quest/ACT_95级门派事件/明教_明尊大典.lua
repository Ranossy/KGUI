---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/明教_明尊大典.lua
-- 更新时间:	2016/1/11 16:07:40
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/明教_明尊大典.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--if not scene.IsNickNameNpcExist("ACT_PQ143") then
		...
		--player.DelBuff(10423, 1) 		
		--return 
	--end
	local dwHashValue = GetFileNameHash("scripts/Map/明教/trap/门派打坐.lua")
	if player.dwLastScriptID ~= dwHashValue then
		player.SetDynamicSkillGroup(0)
		player.DelBuff(10423, 1) 		
	end
	--print("在trap中")
	--if player.GetDynamicSkillGroup() == 237 then
		--player.SetDynamicSkillGroup(0)
		--player.SetDynamicSkillGroup(237)
		--return
	--end
	--player.SetDynamicSkillGroup(237)
end
