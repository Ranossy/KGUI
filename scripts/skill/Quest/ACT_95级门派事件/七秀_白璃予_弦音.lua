---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/七秀_白璃予_弦音.lua
-- 更新时间:	2015/12/29 10:45:59
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/七秀_白璃予_弦音.lua",
		0
	);
	...
	skill.nAreaRadius = 26 * LENGTH_BASE;		-- 技能作用半径
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player and not npcTarget then
		return
	end
	...
	if player then
		local dwCasterID = player.dwID
		local scene = player.GetScene()
		if scene then
			local npcBOSS = scene.GetNpcByNickName("PQ92_BaiLiYu")
			if npcBOSS then
				dwCasterID = npcBOSS.dwID
			end
		end
		player.CustomDamage(dwCasterID, 15337, 1, 2, player.nMaxLife * 0.02)
	elseif npcTarget then
		npcTarget.nCurrentLife = npcTarget.nCurrentLife - 4000
	end
end
