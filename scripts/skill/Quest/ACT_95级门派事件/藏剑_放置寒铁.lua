---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_95级门派事件/藏剑_放置寒铁.lua
-- 更新时间:	2015/11/27 16:27:22
-- 更新用户:	wangwei-pc2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_95级门派事件/藏剑_放置寒铁.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local doodad = scene.GetDoodadByNickName("hanyutie" .. player.dwID)
	player.DoAction(0, 60028)
	if not doodad then
		--DOODAD放到玩家面前2尺处。
		local nDir = player.nFaceDirection * 2 * math.pi / 256
		local N_X = player.nX + 1 * 64 * math.cos(nDir)
		local N_Y = player.nY + 1 * 64 * math.sin(nDir)	
		local newdoodad = scene.CreateDoodad(5901, N_X, N_Y, player.nZ + 800, 0, "hanyutie" .. player.dwID)
		if newdoodad then
			newdoodad.SetCustomInteger4(1, player.dwID)
			newdoodad.SetDisappearFrames(120 * GLOBAL.GAME_FPS)
		end
		player.SetDynamicSkillGroup(0)
		if player.GetItemAmountInAllPackages(ITEM_TABLE_TYPE.OTHER, 26168) >= 1 then
			player.CostItemInAllPackage(ITEM_TABLE_TYPE.OTHER, 26168, 1)
		end
	else
