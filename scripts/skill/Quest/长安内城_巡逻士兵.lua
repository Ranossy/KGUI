---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/长安内城_巡逻士兵.lua
-- 更新时间:	2013/9/11 11:38:38
-- 更新用户:	xutong3
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/长安内城_巡逻士兵.lua", -- 属性值1scripts/skill/Quest/长安内城_站岗.lua
		0														-- 属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player then
		if player.GetBuff(2587, 7) and not player.GetBuff(6506, 1)and not player.GetBuff(6524, 1) then
			local buff = player.GetBuff(6802, 1)
			if not buff or buff.nStackNum <= 1 then
				player.AddBuff(1, 1, 6802, 1, 3)
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(5, 9684), 3)
				return
			end
