---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/浮香丘击杀背箱者.lua
-- 更新时间:	2017/1/11 16:02:40
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.nMaxRadius		= 256 * LENGTH_BASE;		-- 技能施放的最大距离
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nCamp = npc.GetCustomInteger1(0)
	
	if nCamp == 2 then
		return
	end
	if nCamp == player.nBattleFieldSide then
		local nQuestIdx = player.GetQuestIndex(16453)
		if nQuestIdx then
			local nValue = player.GetQuestValue(nQuestIdx, 0)
			player.SetQuestValue(nQuestIdx, 0,  nValue + 1)
			player.ForceFinishQuest(16453)
		end
	end
end
