---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/梦回稻香/王婆婆_喂鸡子技能.lua
-- 更新时间:	2016/8/13 16:33:36
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/梦回稻香/王婆婆_喂鸡子技能.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc and tJi[npc.dwTemplateID] then
		...
		if player then
			...
			if nQuestIndex and nQuestPhase == 1 then
				...
				if player.GetQuestPhase(15678) == 1 and player.GetItemAmount(ITEM_TABLE_TYPE.OTHER, 26860) == 0 then
					RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(10, 4529), 4)
					player.SendSystemMessage( GetEditorString(10, 4529) )
				end
			elseif player.GetQuestPhase(15677) < 3 then
				...
				if npcWangPoPo then
					npcWangPoPo.SetCustomInteger1(0, npcWangPoPo.GetCustomInteger1(0) + 1)
					if npcWangPoPo.GetCustomInteger1(0) >= 5 and player.GetQuestPhase(15677) < 3 then
						npcWangPoPo.Say(GetEditorString(10, 4750), 1280)
						player.ForceFinishQuest(15677)
					end
				end
			end
		end
		if npc.dwTemplateID == 51812 then
			npc.Say(GetEditorString(10, 4525), 1280, true)
		else
			npc.Say(GetEditorString(10, 4526), 1280, true)
		end
	end
end
