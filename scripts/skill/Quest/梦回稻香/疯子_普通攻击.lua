---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/梦回稻香/疯子_普通攻击.lua
-- 更新时间:	2016/7/21 11:29:22
-- 更新用户:	zhengfeng5
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/梦回稻香/疯子_普通攻击.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local tText = {
		GetEditorString(10, 3067),
		GetEditorString(10, 3068),
		GetEditorString(10, 3070),
		GetEditorString(10, 3071),		
		GetEditorString(10, 3069),
		GetEditorString(10, 3109),
	}
	local character = nil
	local nFlag = 0
	if GetPlayer(dwCharacterID) then
		character = GetPlayer(dwCharacterID)
	elseif GetNpc(dwCharacterID) then
	...
	if nFlag == 0 then
		character.CustomDamage(dwSkillSrcID, 16174, 1, 1, character.nMaxLife * 0.15)
	elseif nFlag == 1 then
		local n  = character.nCurrentLife - character.nMaxLife * 0.15
		if n <= 0 then
			character.Die()
		else
			character.nCurrentLife =  n
		end
	end
	local nIndex = npc.GetCustomUnsigned4(0) or 0
	if nIndex % 2 == 0  then
		character.AddBuff(dwSkillSrcID, 1, 10983, 1)
	end	
	nIndex = nIndex % #tText
	npc.Say(tText[nIndex + 1], 1024, true)
	nIndex = nIndex + 1
	npc.SetCustomUnsigned4(0, nIndex)
end
