---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/梦回稻香/莫雨_情绪失控.lua
-- 更新时间:	2016/8/11 20:43:24
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc then
		...
		if buff then
			if npc.nCurrentLife <= buff.nCustomValue - 50000 then
				npc.Jump(true, 1)
				npc.DelBuff(11084, 1)
				npc.DoAction(0, 10056)
			elseif buff.nLeftActiveCount == 1 then
				npc.DoAction(0, 10056)
				npc.CastSkill(16434, math.max(1, 10 - buff.nLeftActiveCount + 1))				
			else
				npc.CastSkill(16434, math.max(1, 10 - buff.nLeftActiveCount + 1))				
				--npc.PlaySfx()
			end
