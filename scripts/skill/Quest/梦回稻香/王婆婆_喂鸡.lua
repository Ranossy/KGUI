---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/梦回稻香/王婆婆_喂鸡.lua
-- 更新时间:	2016/8/13 16:33:48
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/梦回稻香/王婆婆_喂鸡.lua", -- 属性值1
		0											-- 属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(10009, 0, 0, 1, 1);		-- 需求自身Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		...
		local nQuestID = 15678
		local nQuestIndex = player.GetQuestIndex(nQuestID)
		local nQuestPhase = player.GetQuestPhase(nQuestID)
		if nQuestIndex and nQuestPhase == 1 then
			local npcWangPoPo = scene.GetNpcByNickName("wangpopo")
			if npcWangPoPo then
				npcWangPoPo.SetCustomInteger1(1, npcWangPoPo.GetCustomInteger1(1) + 1)
			end
		end

		local nX, nY, nZ = player.GetCoordinateSkillTarget()
		local npcTarget = scene.GetNpcByNickName("JiShi_" .. player.dwID)
		if not npcTarget then
			npcTarget = scene.CreateNpc(47923, nX, nY, nZ, 0, 3 * GLOBAL.GAME_FPS, "JiShi_" .. player.dwID)
		end
		--player.CostItem(ITEM_TABLE_TYPE.OTHER, 26860, 1)
		player.SetTimer(40, "scripts/skill/Quest/梦回稻香/王婆婆_喂鸡.lua", 0, 0)
		--player.CastSkillXYZ(15059, 1, nX, nY, nZ)
		--player.PlaySfx(1225, nX, nY, nZ, 0)
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	local npcTarget = scene.GetNpcByNickName("JiShi_" .. player.dwID)
	if npcTarget then
		player.CastSkillXYZ(16436, 1, npcTarget.nX, npcTarget.nY, npcTarget.nZ)
		--player.PlaySfx(1225, npcTarget.nX, npcTarget.nY, npcTarget.nZ, 0)
