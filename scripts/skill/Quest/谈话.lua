---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/谈话.lua
-- 更新时间:	2013-3-25 17:22:53
-- 更新用户:	xiedixun1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
    ...
    if player then
        ...
        if nQuestIndex then
            local n = player.GetQuestValue(nQuestIndex, 0)
            local m = player.GetQuestValue(nQuestIndex, 1)
            if n < 1 and m < 3 then
                --先说三次话,引出狼牙伏兵
                if m == 0 then
                    player.Talk(GetEditorString(5, 1760))
                    player.SetQuestValue(nQuestIndex, 1, m + 1)
                    local shibing2 = scene.GetNpcByNickName("shibingyanyuan2")      
            
                    if shibing2 then
                        shibing2.SayTo(player.dwID, GetEditorString(5, 1763))
                    end
                    return                 
                elseif m == 1 then
                    player.Talk(GetEditorString(5, 1761))
                    player.SetQuestValue(nQuestIndex, 1, m + 1)
                    return   
                elseif m == 2 then
                    player.Talk(GetEditorString(5, 1762))
                    player.SetQuestValue(nQuestIndex, 1, m + 1)
                    player.SetQuestValue(nQuestIndex, 0, 1)
                    player.SetTimer(4 * GLOBAL.GAME_FPS, "scripts/Map/安史之乱_洛阳/quest/请君入瓮.lua", 0, 1)
                    player.SetDynamicSkillGroup(0)
                    local shibing1 = scene.GetNpcByNickName("shibingyanyuan1")    
                    if shibing1 then
                        shibing1.SayTo(player.dwID, GetEditorString(5, 1764))
                    end
                end              
            end           
        end
