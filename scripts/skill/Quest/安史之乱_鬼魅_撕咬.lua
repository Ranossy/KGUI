...
function Apply(dwCharacterID, dwSrcID)
	local player = GetPlayer(dwCharacterID) or GetNpc(dwCharacterID)
	if not player then
		return
	end
	local num = Random(1000000)
	if num < 100001 and player.CanApplySlay() then
		player.ClearAllThreat()
		player.AddBuff(0, 99, 5881, 1)
		player.PlaySfx(493, player.nX, player.nY, player.nZ)
		player.ApplySlay()
		player.SendSystemMessage(
			GetEditorString(5, 4908)
		)
		local nPlayer = GetPlayer(dwSrcID) or GetNpc(dwCharacterID)
		if not nPlayer then
			nPlayer.nCurrentLife = nPlayer.nMaxLife
		end
