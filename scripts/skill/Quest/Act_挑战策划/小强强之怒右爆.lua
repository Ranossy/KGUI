---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/Act_挑战策划/小强强之怒右爆.lua
-- 更新时间:	2014/7/15 16:43:01
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 104000, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/Act_挑战策划/小强强之怒右爆.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(8089, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	skill.nMinPrepareFrames  	= 4;				-- 急速效果最小吟唱帧数：默认是-1，吟唱不受急速影响。大于等于0则受急速影响，最小读条时间为填的值
	--skill.nChannelInterval	= 0; 				-- 通道技间隔时间
	--skill.nMinChannelInterval	= 16; 				-- 急速效果最小通道技间隔时间：默认是-1，通道不受急速影响。大于等于0则受急速影响，最通道间隔时间为填的值
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		if not player.GetBuff(8062, 1) then
			RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "ShowFullScreenSFX", "SkillHint")
		end
		player.AddBuff(0, 99, 8062, 1, 5)
		player.AddBuff(0, 99, 8063, 1, 5) --维持4秒的急速效果，可以叠加20层，每层提速20%
		local jsBuff = player.GetBuff(8063, 1)
		if jsBuff then
			if jsBuff.nStackNum >= 20 then
				player.DelGroupBuff(8063, 1)
				player.AddBuff(0, 99, 8079, 1, 5) --暴击
				RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "ShowFullScreenSFX", "SkillHint")
				player.AddBuff(0, 99, 8064, 1, 1) --出大招
			end
		end
	end
