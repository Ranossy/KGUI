---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/Act_挑战策划/步高现场加强实际.lua
-- 更新时间:	2014/7/25 18:55:43
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 30000, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/Act_挑战策划/步高现场加强实际.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
	...
	if player then
		if nNum < 1000 then
			if player.dwForceID == 1 then  --少林
				player.AddBuff(0, 99, 8125, 1, 10)
				--	npc.Say("加强少林")
				player.Talk(GetEditorString(6, 7349))
			
			end
		elseif nNum >= 1000 and nNum < 2000 then
			if player.dwForceID == 2 then  --万花
				player.AddBuff(0, 99, 8126, 1, 10)
			--	npc.Say("加强万花")
				player.Talk(GetEditorString(6, 7351))
				
			end
		elseif nNum >= 2000 and nNum < 3000 then
			if player.dwForceID == 3 then  --天策
				player.AddBuff(0, 99, 8124, 1, 10)
			--	npc.Say("加强天策")
				player.Talk(GetEditorString(6, 7353))
				
			end
		elseif nNum >= 3000 and nNum < 4000 then
			if player.dwForceID == 4 then  --纯阳
				player.AddBuff(0, 99, 8123, 1, 10)
				--	npc.Say("加强纯阳")
				player.Talk(GetEditorString(6, 7355))
			
			end
		elseif nNum >= 4000 and nNum < 5000 then
			if player.dwForceID == 5 then  --七秀
				player.AddBuff(0, 99, 8142, 1, 10)
			--	npc.Say("加强七秀")
				player.Talk(GetEditorString(6, 7357))
				
			end
		elseif nNum >= 5000 and nNum < 6000 then
			if player.dwForceID == 6 then  --五毒
				player.AddBuff(0, 99, 8127, 1, 10)
			--	npc.Say("加强五毒")
				player.Talk(GetEditorString(6, 7359))
				
			end
		elseif nNum >= 6000 and nNum < 7000 then
			if player.dwForceID == 7 then  --唐门
				player.AddBuff(0, 99, 8129, 1, 10)
			--	npc.Say("加强唐门")
				player.Talk(GetEditorString(6, 7361))
				
			end
		elseif nNum >= 7000 and nNum < 8000 then
			if player.dwForceID == 8 then  --藏剑
				player.AddBuff(0, 99, 8130, 1, 10)
			--	npc.Say("加强藏剑")
				player.Talk(GetEditorString(6, 7363))
				
			end
		elseif nNum >= 8000 and nNum < 9000 then
			if player.dwForceID == 9 then  --丐帮
				player.AddBuff(0, 99, 8131, 1, 10)
			--	npc.Say("加强丐帮")
				player.Talk(GetEditorString(6, 7365))
				
			end
		elseif nNum >= 9000  then
			if player.dwForceID == 10 then  --明教
				player.AddBuff(0, 99, 8128, 1, 10)
			--	npc.Say("加强明教")
				player.Talk(GetEditorString(6, 7367))
				
			end
		end
	end
	player.DelBuff(8119, 1) --避免2次打中占技能目标数
	player.SetTimer(48, "scripts/skill/Quest/Act_挑战策划/步高现场加强实际.lua", 0, 0)
