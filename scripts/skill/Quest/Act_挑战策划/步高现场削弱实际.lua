---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/Act_挑战策划/步高现场削弱实际.lua
-- 更新时间:	2014/7/25 20:00:32
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 40000, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/Act_挑战策划/步高现场削弱实际.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(8119, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(8119, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	if player then
		...
		if nNum < 1000 then
			if player.dwForceID == 1 then  --少林
				player.AddBuff(0, 99, 8135, 1, 10)
			--	npc.Say("削弱少林")
				player.Talk(GetEditorString(6, 7338))
				
			end
		elseif nNum >= 1000 and nNum < 2000 then
			if player.dwForceID == 2 then  --万花
				player.AddBuff(0, 99, 8136, 1, 10)
			--	npc.Say("削弱万花")
				player.Talk(GetEditorString(6, 7339))
				
			end
		elseif nNum >= 2000 and nNum < 3000 then
			if player.dwForceID == 3 then  --天策
				player.AddBuff(0, 99, 8133, 1, 10)
			--	npc.Say("削弱天策")
				player.Talk(GetEditorString(6, 7340))
				
			end
		elseif nNum >= 3000 and nNum < 4000 then
			if player.dwForceID == 4 then  --纯阳
				player.AddBuff(0, 99, 8132, 1, 10)
			--	npc.Say("削弱纯阳")
				player.Talk(GetEditorString(6, 7341))
				
			end
		elseif nNum >= 4000 and nNum < 5000 then
			if player.dwForceID == 5 then  --七秀
				player.AddBuff(0, 99, 8134, 1, 10)
			--	npc.Say("削弱七秀")
				player.Talk(GetEditorString(6, 7342))
				
			end
		elseif nNum >= 5000 and nNum < 6000 then
			if player.dwForceID == 6 then  --五毒
				player.AddBuff(0, 99, 8137, 1, 10)
			--	npc.Say("削弱五毒")
				player.Talk(GetEditorString(6, 7343))
				
			end
		elseif nNum >= 6000 and nNum < 7000 then
			if player.dwForceID == 7 then  --唐门
				player.AddBuff(0, 99, 8139, 1, 10)
			--	npc.Say("削弱唐门")
				player.Talk(GetEditorString(6, 7344))
				
			end
		elseif nNum >= 7000 and nNum < 8000 then
			if player.dwForceID == 8 then  --藏剑
				player.AddBuff(0, 99, 8140, 1, 10)
		--		npc.Say("削弱藏剑")
				player.Talk(GetEditorString(6, 7345))
				
			end
		elseif nNum >= 8000 and nNum < 9000 then
			if player.dwForceID == 9 then  --丐帮
				player.AddBuff(0, 99, 8141, 1, 10)
		--		npc.Say("削弱丐帮")
				player.Talk(GetEditorString(6, 7346))
				
			end
		elseif nNum >= 9000  then
			if player.dwForceID == 10 then  --明教
				player.AddBuff(0, 99, 8138, 1, 10)
			--	npc.Say("削弱明教")
				player.Talk(GetEditorString(6, 7347))
				
			end
		end
	end
	player.DelBuff(8119, 1) --避免2次打中占技能目标数
	player.SetTimer(48, "scripts/skill/Quest/Act_挑战策划/步高现场削弱实际.lua", 0, 0)

end

function OnTimer(player, nParam1, nParam2)
	player.AddBuff(0, 99, 8119, 1, 120)
end
