---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/Act_挑战策划/李小俊掉线实际.lua
-- 更新时间:	2014/7/25 22:43:25
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 45000, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/Act_挑战策划/李小俊掉线实际.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		if not player.GetBuff(8153, 1) then
			player.DelBuff(8150, 1) --刀客换摸
			player.DelBuff(8152, 1) --剑客换摸
			player.DelBuff(8154, 1) --胖妹子换摸
			player.DelBuff(8155, 1) --胖妹子换摸
			player.DelBuff(8151, 1) --防打扰
			player.AddBuff(0, 99, 8153, 1, 1030)
			player.SetDynamicSkillGroup(169)
			player.PlaySfx(320, 0, 0, 0)
			player.nCurrentLife = player.nMaxLife
		end
		player.SendSystemMessage(GetEditorString(6, 7517))
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "OpenDropLinePanel", true)
		player.DelBuff(8119, 1) --避免2次打中占技能目标数
		player.SetTimer(48, "scripts/skill/Quest/Act_挑战策划/李小俊掉线实际.lua", 0, 0)
	end
end
