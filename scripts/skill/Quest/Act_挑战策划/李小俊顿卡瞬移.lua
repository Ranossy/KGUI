---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/Act_挑战策划/李小俊顿卡瞬移.lua
-- 更新时间:	2014/7/24 15:29:39
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/Act_挑战策划/李小俊顿卡瞬移.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local Dian2 = {
		{33297, 38820, 1052800, 162},
		{23664, 21970, 1046784, 163},
		{6949, 22901, 1098112, 157},
		{45694, 32673, 1054528, 179},
		{14429, 21493, 1109632, 170},
		{32963, 19453, 1098304, 184},
		{19648, 32862, 1054528, 187},
	}
	local scene = npc.GetScene()
	local StateInfo = GetActivityState(273)
	if not StateInfo then
		return
	end
	if StateInfo == ACTIVITY_STATE.NORMAL_ON or StateInfo == ACTIVITY_STATE.RECOVER_ON  then
	for i = 1, #Dian2 do
			if scene.dwMapID == Dian2[i][4] then
				local Dian22 = SkillPosition1(npc, 1000, 5) --5尺
				local nNum = Random(5)
				npc.SetPosition(Dian22[nNum][1], Dian22[nNum][2], Dian22[nNum][3])
				break
			end	
		end
	else
		local nNum = Random(5)
		if npc then
			npc.SetPosition(Dian[nNum][1], Dian[nNum][2], Dian[nNum][3])
		end
	end
end
function SkillPosition(npc, dis, num)
	...
	for i = 1, num do
		...
		tPosition[i][3] = npc.nZ+100
	end
	return tPosition
end

function SkillPosition1(npc, dis, num)
	local tPosition = {}
	if num == 0 then
		return false
	end
	for i = 1, num do
		tPosition[i] = {}
		tPosition[i][1] = npc.GetCustomUnsigned4(50) + dis * math.cos(npc.nFaceDirection / 128 * math.pi + i * 2 * math.pi / num)
		tPosition[i][2] = npc.GetCustomUnsigned4(54) + dis * math.sin(npc.nFaceDirection / 128 * math.pi + i * 2 * math.pi / num)
		tPosition[i][3] = npc.GetCustomUnsigned4(58)+100
	end
	return tPosition
end
