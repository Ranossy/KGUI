---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/Act_挑战策划/步高教技能实际.lua
-- 更新时间:	2014/7/25 20:09:04
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/Act_挑战策划/步高教技能实际.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		if player.GetDynamicSkillGroup() == 171 then
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_GREEN", GetEditorString(6, 7526), 6)
			player.AddContribution(200)
			player.SendSystemMessage(GetEditorString(6, 7521))
			player.DelBuff(8119, 1) --避免2次打中占技能目标数
			player.SetTimer(48, "scripts/skill/Quest/Act_挑战策划/步高教技能实际.lua", 0, 0)
			return
		else
			player.AddBuff(0, 99, 8149, 1, 30) --题目1
			RemoteCallToClient(player.dwID, "OnUserInput", GetEditorString(6, 7530), "On_TZCHhuodong_1")
		end
	end
	player.DelBuff(8119, 1) --避免2次打中占技能目标数
	player.SetTimer(48, "scripts/skill/Quest/Act_挑战策划/步高教技能实际.lua", 0, 0)
