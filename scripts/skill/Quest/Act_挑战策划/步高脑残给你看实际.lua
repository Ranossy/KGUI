---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/Act_挑战策划/步高脑残给你看实际.lua
-- 更新时间:	2014/7/25 20:54:50
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 55000, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/Act_挑战策划/步高脑残给你看实际.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
	...
	if player then
		if nNum < 1000 then
			if player.dwForceID == 1 then  --少林
				player.AddBuff(0, 99, 8142, 1, 10)  --阴性气劲
				--npc.Say("少林增加阴性内功加成")
				player.Talk(GetEditorString(6, 7380))
				
			end
		elseif nNum >= 1000 and nNum < 2000 then
			if player.dwForceID == 2 then  --万花
				player.AddBuff(0, 99, 8124, 1, 10)  --天策气劲
			--	npc.Say("万花增加外功加成")
				player.Talk(GetEditorString(6, 7382))
				
			end
		elseif nNum >= 2000 and nNum < 3000 then
			if player.dwForceID == 3 then  --天策
				player.AddBuff(0, 99, 8126, 1, 10) --万花气劲
			--	npc.Say("天策增加混元内功加成")
				player.Talk(GetEditorString(6, 7384))
		...
		elseif nNum >= 3000 and nNum < 4000 then
			if player.dwForceID == 4 then  --纯阳
				player.AddBuff(0, 99, 8142, 1, 10) --阴性气劲
		--		npc.Say("纯阳增加阴性内功加成")
				player.Talk(GetEditorString(6, 7386))
				
			end
		elseif nNum >= 4000 and nNum < 5000 then
			if player.dwForceID == 5 then  --七秀
				player.AddBuff(0, 99, 8125, 1, 10) --阳性气劲
		--		npc.Say("七秀增加阳性内功加成")
				player.Talk(GetEditorString(6, 7388))
				
			end
		elseif nNum >= 5000 and nNum < 6000 then
			if player.dwForceID == 6 then  --五毒
				player.AddBuff(0, 99, 8124, 1, 10) --外功气劲
			--	npc.Say("五毒增加外功加成")
				player.Talk(GetEditorString(6, 7390))
		...
		elseif nNum >= 6000 and nNum < 7000 then
			if player.dwForceID == 7 then  --唐门
				player.AddBuff(0, 99, 8125, 1, 10) --阳性气劲
			--	npc.Say("唐门增加阳性内功加成")
				player.Talk(GetEditorString(6, 7392))
		...
		elseif nNum >= 7000 and nNum < 8000 then
			if player.dwForceID == 8 then  --藏剑
				player.AddBuff(0, 99, 8142, 1, 10) --阴性气劲
			--	npc.Say("藏剑增加阴性内功加成")
				player.Talk(GetEditorString(6, 7394))
		...
		elseif nNum >= 8000 and nNum < 9000 then
			if player.dwForceID == 9 then  --丐帮
				player.AddBuff(0, 99, 8125, 1, 10) --阳性气劲
			--	npc.Say("丐帮增加阳性内功加成")
				player.Talk(GetEditorString(6, 7396))
				
			end
		elseif nNum >= 9000  then
			if player.dwForceID == 10 then  --明教
				player.AddBuff(0, 99, 8126, 1, 10) --混元气劲
			--	npc.Say("明教增加混元内功加成")
				player.Talk(GetEditorString(6, 7525))
			end
		end
	end
	player.DelBuff(8119, 1) --避免2次打中占技能目标数
	player.SetTimer(48, "scripts/skill/Quest/Act_挑战策划/步高脑残给你看实际.lua", 0, 0)
