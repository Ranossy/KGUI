---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/Act_挑战策划/昆仑奴溢伤.lua
-- 更新时间:	2014/7/24 15:48:34
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 250000, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/Act_挑战策划/昆仑奴溢伤.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		if npc then
			local YSbuff = npc.GetBuff(8147, 1)
			npc.AddBuff(0, 99, 8147, 1, 10) --溢伤BUFF
			if YSbuff then
				if YSbuff.nStackNum >= 5 then
					npc.AddBuff(0, 99, 8146, 1, 10) --可释放大爆发
					npc.DelGroupBuff(8147, 1) --删除堆叠，重新计数
				end	
			end
		end
	end
