---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/Act_挑战策划/雄爷不二助我.lua
-- 更新时间:	2014/7/24 19:28:12
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/Act_挑战策划/雄爷不二助我.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local Dian = SkillPosition(npc, 192, 5) --15尺
	local Dian2 = SkillPosition(npc, 3000, 5) --15尺
	local scene = npc.GetScene()
	local ZZ = 200
	--local BaoXiang1 =
	local laonainai1 = scene.CreateNpc(36972, Dian[1][1], Dian[1][2], npc.nZ + ZZ, 125, 160, "XY_laonainai1")
	local laonainai2 = scene.CreateNpc(36972, Dian[2][1], Dian[2][2], npc.nZ + ZZ, 125, 160, "XY_laonainai2")
	local laonainai3 = scene.CreateNpc(36972, Dian[3][1], Dian[3][2], npc.nZ + ZZ, 125, 160, "XY_laonainai3")
	local laonainai4 = scene.CreateNpc(36972, Dian[4][1], Dian[4][2], npc.nZ + ZZ, 125, 160, "XY_laonainai4")
	local laonainai5 = scene.CreateNpc(36972, Dian[5][1], Dian[5][2], npc.nZ + ZZ, 125, 160, "XY_laonainai5")
	if laonainai1 then
		laonainai1.SetCustomUnsigned4(0, Dian2[1][1])
		laonainai1.SetCustomUnsigned4(4, Dian2[1][2])
	end
	if laonainai2 then
		laonainai2.SetCustomUnsigned4(0, Dian2[2][1])
		laonainai2.SetCustomUnsigned4(4, Dian2[2][2])
	end
	if laonainai3 then
		laonainai3.SetCustomUnsigned4(0, Dian2[3][1])
		laonainai3.SetCustomUnsigned4(4, Dian2[3][2])
	end
	if laonainai4 then
		laonainai4.SetCustomUnsigned4(0, Dian2[4][1])
		laonainai4.SetCustomUnsigned4(4, Dian2[4][2])
	end
	if laonainai5 then
		laonainai5.SetCustomUnsigned4(0, Dian2[5][1])
		laonainai5.SetCustomUnsigned4(4, Dian2[5][2])
	end
end
...
function SkillPosition(npc, dis, num)
	local tPosition = {}
	if num == 0 then
		return false
	end
	for i = 1, num do
		tPosition[i] = {}
		tPosition[i][1] = npc.nX + dis * math.cos(npc.nFaceDirection / 128 * math.pi + i * 2 * math.pi / num)
		tPosition[i][2] = npc.nY + dis * math.sin(npc.nFaceDirection / 128 * math.pi + i * 2 * math.pi / num)
		tPosition[i][3] = npc.nZ
	end
	return tPosition
end
