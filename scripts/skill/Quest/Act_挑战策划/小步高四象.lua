---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/Act_挑战策划/小步高四象.lua
-- 更新时间:	2014/7/16 16:29:23
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 98000, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/Act_挑战策划/小步高四象.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- 时间相关 -------------------------------------------------
	skill.nPrepareFrames = 21;				-- 吟唱帧数
	--skill.nMinPrepareFrames = 16;				-- 急速效果最小吟唱帧数：默认是-1，吟唱不受急速影响。大于等于0则受急速影响，最小读条时间为填的值
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		player.AddBuff(0, 99, 8071, 1, 5) --四象叠加
		local SXbuff = player.GetBuff(8071, 1)
		if SXbuff then
			if SXbuff.nStackNum >= 3 then
				RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "ShowFullScreenSFX", "SkillHint")
				player.AddBuff(0, 99, 8072, 1, 3) -- 有此BUFF后玩家可以释放冲刺
				player.DelGroupBuff(8071, 1)
			end
		end
	end
end
