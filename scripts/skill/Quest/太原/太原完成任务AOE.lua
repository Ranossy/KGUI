---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/太原/太原完成任务AOE.lua
-- 更新时间:	2014/10/28 11:30:53
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/太原/太原完成任务AOE.lua", -- 属性值1
		0														-- 属性值2
	);
...
local tNPC_Quest = {
	[37107] = {12535, 0, 1},
	[37321] = {12596, 0, 1}, -- 澄寞放技能，完成[12596]生死无界任务
	[37317] = {12592, 0, 1}, -- 道衍放技能，完成[12592]相救道衍任务
	[42819] = {13490, 0, 1}, -- 唐拂放技能，完成[13490]击杀内鬼任务
	[43680] = {13877, 0, 1}, -- 女飞贼放技能，完成[13877]击杀内鬼任务
	[43164] = {13623, 0, 1}, -- 神秘的游荡者放技能，完成[13623]击杀内鬼任务
	[56031] = {16648, 0, 1}, -- 风雷刀谷预热,欧阳风
	[56030] = {16648, 1, 1}, -- 风雷刀谷预热,欧阳金
	[56038] = {16648, 2, 1}, -- 风雷刀谷预热,欧阳雷
	}
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not tNPC_Quest[npc.dwTemplateID] then
		return
	end
	...
	local nQuestIndex = player.GetQuestIndex(tNPC_Quest[npc.dwTemplateID][1])
	if not nQuestIndex then
		return
	end
	if player.GetQuestValue(nQuestIndex, tNPC_Quest[npc.dwTemplateID][2]) ~= tNPC_Quest[npc.dwTemplateID][3] then
		player.SetQuestValue(nQuestIndex,tNPC_Quest[npc.dwTemplateID][2],tNPC_Quest[npc.dwTemplateID][3])
	end
	if tNPC_Quest[npc.dwTemplateID][1] == 13623 and player.GetItemAmount(ITEM_TABLE_TYPE.OTHER, 23629) < 1 then
		if player.CanAddItem(ITEM_TABLE_TYPE.OTHER, 23629, 1) == ITEM_RESULT_CODE.SUCCESS then
			player.AddItem(ITEM_TABLE_TYPE.OTHER, 23629, 1)
		else
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(7, 6512), 4)
		end
	end
end
