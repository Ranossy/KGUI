...
function GetSkillLevelData(skill)
  ...
  --  skill.SetNormalCoolDown(1, 571);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	local n = Random(1, 6)
	if n == 1 then 
		player.Talk(GetEditorString(5, 493), 5)
	end
	if n == 2 then 
		player.Talk(GetEditorString(5, 494), 5)
	end
	if n == 3 then 
		player.Talk(GetEditorString(5, 495), 5)
	end
	player.AddBuff(0,99,6115,1)
    return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	--if not player then 
		--return
	--end
	--player.nCurrentLife = player.nCurrentLife + 5000000
end
