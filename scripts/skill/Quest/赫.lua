---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/赫.lua
-- 更新时间:	2013/3/21 18:14:27
-- 更新用户:	wangwei-pc
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
	...
	if player then
		...
		if nQuestIndex then
			if  player.GetQuestValue(nQuestIndex, 1) == 2 then
				player.SetQuestValue(nQuestIndex, 1, 0)             
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(5, 1730), 3)
				player.SendSystemMessage(GetEditorString(5, 1749))
				player.AddExp(2000)
			end
			player.SetQuestValue(nQuestIndex, 1, 0)
		end
