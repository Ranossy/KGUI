---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/长安内城_东市驱逐.lua
-- 更新时间:	2013/9/11 21:32:25
-- 更新用户:	zhengfeng4
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/长安内城_东市驱逐.lua", -- 属性值1
		0														-- 属性值2
	);
...
local tPosition = {
	[1] = {106876, 47876, 1051712},
	[2] = {112981, 47786, 1051456},
}
local tNpcPosition = {
	["DSJQSW1"] = 1,
	["DSJQSW2"] = 1,
...
function Apply(dwCharacterID, dwSrcID)
	...
	-- 清仇恨
	npc.ClearAllThreat()
	npc.SayTo(player.dwID, GetEditorString(5, 9947))
	-- 添加隐身眩晕buff
	player.AddBuff(0, 99, 6813, 1, 3)
	RemoteCallToClient(player.dwID, "On_Trial_Screen_Black")   --黑屏
	player.SetTimer(1 * GLOBAL.GAME_FPS, "scripts/skill/Quest/长安内城_东市驱逐.lua", 0, tNpcPosition[npc.szName])
end
...
function OnTimer(player, nParam1, nParam2)
	if tPosition[nParam2] then
		player.SetPosition(tPosition[nParam2][1], tPosition[nParam2][2], tPosition[nParam2][3])
	end 
end
