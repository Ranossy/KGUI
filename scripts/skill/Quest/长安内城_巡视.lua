---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/长安内城_巡视.lua
-- 更新时间:	2013/8/14 14:46:18
-- 更新用户:	zhouyixiao1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, nTarget)
	local NPCbj = GetNpc(dwCharacterID)
	local Playerbj = GetPlayer(dwCharacterID)
	if NPCbj then
		if NPCbj.GetBuff(6494, 1) then
			NPCbj.DelBuff(6494, 1) -- 删除杀字特效
			NPCbj.AddBuff(0, 99, 6495, 1, 6) --添加杀字延迟显示BUFF
		end
		NPCbj.AddBuff(0, 99, 6495, 1, 6) --添加杀字延迟显示BUFF
	end
	if Playerbj then
		if not Playerbj.GetBuff(6499, 1) then
			if not  Playerbj.GetBuff(6834, 1) then
				RemoteCallToClient(Playerbj.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(5, 8516), 4)
				Playerbj.SetTimer(16, "scripts/skill/Quest/长安内城_巡视.lua", 0, 0)
				Playerbj.AddBuff(0, 99, 6499, 1, 10) -- 添加被发现BUFF
				Playerbj.PlaySfx(548, Playerbj.nX, Playerbj.nY, Playerbj.nZ)	
			end	
		end
		--黑屏设置玩家位置，玩家计时器
	end
...
function OnTimer(player, nParam1, nParam2)
	local step = nParam1
	if step == 1 then
		RemoteCallToClient(player.dwID, "On_Trial_Screen_Black") --黑屏
	end
	if step == 2 then
		player.SetPosition(121554, 50796, 1051328)
	end
	if step < 5 then
		player.SetTimer(16, "scripts/skill/Quest/长安内城_巡视.lua", step + 1, 0)
	end
end
