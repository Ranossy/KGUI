---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/90小橙武_大范围AOE.lua
-- 更新时间:	2015/1/8 9:57:39
-- 更新用户:	zhangtianhui1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	--skill.AddAttribute(
		...
		--ATTRIBUTE_TYPE.CALL_KNOCKED_OFF_PARABOLA, -- 魔法属性
		--160, -- 属性值1
		--5																-- 属性值2
	--);
	skill.AddAttribute(
	...
	skill.AddAttribute(
		...
		"skill/Quest/90小橙武_大范围AOE.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.GetBuff(8721, 1) then
		player.AddBuff(0, 99, 8721, 1, 1)
	end
	--player.CustomDamage(dwSkillSrcID, 13635, 1, 1, player.nMaxLife * 0.06)
	--local buffFengjian = player.GetBuff(8537, 1)
	--if not buffFengjian and not player.GetBuff(8577, 1) then
	--player.AddBuff(0, 99, 8537, 1)
	--return
	--end
	--if buffFengjian.nStackNum < 2 then
	--if not player.GetBuff(8577, 1) then
	--player.AddBuff(0, 99, 8537, 1)
	--end
	--else
	--player.AddBuff(0, 99, 8577, 1, 3)
	--RemoteCallToClient(player.dwID, "On_Trial_Screen_Black")
	--player.SetTimer(1 * GLOBAL.GAME_FPS, "scripts/skill/Quest/苍云_炸弹爆炸.lua", player.dwID, 0)
	--end
end

--function OnTimer(player, nParam1, nParam2)
--RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", "你被炸弹炸伤了。", 5)
--player.SetPosition(12340, 13519, 1057216)
--player.DelBuff(8537, 1)
--end
