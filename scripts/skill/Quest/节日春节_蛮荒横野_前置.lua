---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/节日春节_蛮荒横野_前置.lua
-- 更新时间:	2013/12/25 16:20:00
-- 更新用户:	zhouyixiao1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/节日春节_蛮荒横野_前置.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	for i = 1, 4 do
		if #tZuoBiao >= 1 then
			local nNum = Random(1, #tZuoBiao)
			if GetDistanceSq(npc.nX, npc.nY, npc.nZ, tZuoBiao[nNum][1], tZuoBiao[nNum][2], tZuoBiao[nNum][3]) >= (20 * 64 )^ 2 then
				FaceToPoint(npc.dwID, tZuoBiao[nNum][1], tZuoBiao[nNum][2])
				npc.AddBuff(0, 99, 7324, 1, 20)
			--	npc.AddBuff(0, 99, 7345, 1, 3)
				npc.CastSkillXYZ(8682, 1,  tZuoBiao[nNum][1],  tZuoBiao[nNum][2],  tZuoBiao[nNum][3])--放技能
				break
			else
				table.remove(tZuoBiao, nNum)
			end
		end
	end
end
function FaceToPoint(npcid, nTX, nTY)
	...
	elseif nTX > nX then	
		dir = math.atan((nTY-nY) / (nTX-nX)) / math.pi * 128
	else
		dir = math.atan((nTY-nY) / (nTX-nX)) / math.pi * 128+128		
	end
	npc.TurnTo(dir)
end
