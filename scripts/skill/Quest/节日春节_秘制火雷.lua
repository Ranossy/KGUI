---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/节日春节_秘制火雷.lua
-- 更新时间:	2014/1/23 19:30:06
-- 更新用户:	zhouyixiao1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/节日春节_秘制火雷.lua", -- 属性值1
		0														-- 属性值2
	);
	...
	skill.SetNormalCoolDown(1, 682);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local StateInfo = GetActivityState(222)
	local scene = player.GetScene()
	if StateInfo then
		if StateInfo == ACTIVITY_STATE.NORMAL_ON or StateInfo == ACTIVITY_STATE.RECOVER_ON then
			if GetDistanceSq(player.nX, player.nY, player.nZ, 61435, 89507, 1084352) <= 6000 ^ 2 then
				local NPChuolei = scene.CreateNpc(27607, player.nX, player.nY, player.nZ, 120, 160) --创建释放秘制火雷技能的NPC
				if NPChuolei then
					NPChuolei.SetCustomUnsigned4(0, player.dwID)
				end
			else
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(6, 2562), 8)
				return
			end
		else
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(6, 2520), 8)
			return
		end
	else
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(6, 2520), 8)
