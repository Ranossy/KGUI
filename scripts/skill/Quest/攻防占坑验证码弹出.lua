---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/攻防占坑验证码弹出.lua
-- 更新时间:	2016/12/5 18:10:38
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
...
--	print("1111", nLeftFrame, nCustomValue)
	local player = GetPlayer(nCharacterID)
	if BuffID == 11558 then
		if player.nCamp == 0 then
			return
		end
		if tCampCheckDate.IsInQuestionWhitelist(player) then
			return 
		end
		if nLeftFrame == 0 then
			--[[if nCustomValue < 3 then
				print("nCustomvalue")
				player.AddBuff(0, 99, 11558, 1, 5 * 60)
				local buff = player.GetBuff(11558, 1)
				buff.nCustomValue = nCustomValue + 1
			end--]]
	
			-- 弹出界面
			--{szQuestion = "恶人谷有几个据点？", tAnswer = {"1个", "没有据点", "2个", "3个",} nKey = 1},
			local nRand = Random(#tCampCheckDate.tQuestion)
			RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "OpenCastleFightCleanup", tCampCheckDate.tQuestion[nRand].szQuestion, tCampCheckDate.tQuestion[nRand].tAnswer)
			player.AddBuff(0, 99, 11559, 1, 32)
			local buffAnswer = player.GetBuff(11559, 1)
			buffAnswer.nCustomValue = tCampCheckDate.tQuestion[nRand].nKey
			RemoteCallToClient(player.dwID, "FlashTaskBar", 5) -- 客户端不在前台时任务栏闪烁通知
		end
	elseif BuffID == 11559 then
		if tCampCheckDate.IsInQuestionWhitelist(player) then
			return
		end
		if nLeftFrame == 0 then
			player.SendSystemMessage(GetEditorString(11, 2183))
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(11, 2183), 5)
			player.SetTimer(16 * 5, "scripts/Map/逐鹿中原/skill/扰乱次序结束.lua", 1, 0)
		end
	end
