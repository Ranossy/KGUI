---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/活动_震地炎.lua
-- 更新时间:	2014/4/18 15:11:36
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc then
		npc.AddBuff(0, 99, 7811, 1, 10)
	end
	local player = GetPlayer(dwCharacterID)
	if player then
		local scene = player.GetScene()
		if scene then
			player.AddBuff(0, 99, 7346, 1, 10)
			player.PlaySfx(653, player.nX, player.nY, player.nZ)
			scene.CreateNpc(36468, player.nX, player.nY, player.nZ, 125, 160) --创建火焰
		end
	end
end
