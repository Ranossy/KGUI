---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/剑胆琴心预热/杨逸飞_梅花三弄_敌方子技能.lua
-- 更新时间:	2015/9/15 19:55:26
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/Quest/剑胆琴心预热/杨逸飞_梅花三弄_敌方子技能.lua",											-- 属性值1
   		0											-- 属性值2
   		);
	...
	skill.nHeight = 16 * 8 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--local tNpcDie = {46188, 46189, 45977, 46047, }
	local tNpcHarm = {45975, 45976,46188, 46189, 45977, 46047, }
	--local nHarm = 100000
	--for i = 1, #tNpcDie do
		--if npcTarget.dwTemplateID == tNpcDie[i] then
			--npcTarget.Die()
			--break
		--end
	--end
	for i = 1, #tNpcHarm do
		if npcTarget.dwTemplateID == tNpcHarm[i] then
			npcTarget.AddBuff(0, 99, 9764, 1)
			--if npcTarget.nCurrentLife > nHarm then
				--npcTarget.nCurrentLife = npcTarget.nCurrentLife-nHarm
			--else
				--npcTarget.Die()
			--end
			break
