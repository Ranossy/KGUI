---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/剑胆琴心预热/挂件_凰吟白首_江逐月天.lua
-- 更新时间:	2015/9/17 16:10:56
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/Quest/剑胆琴心预热/挂件_凰吟白首_江逐月天.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if player.dwBackItemIndex ~= 16682 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND 
		return SKILL_RESULT_CODE.FAILED
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.DoAction(0, 13972)
	local scene = player.GetScene()
	if not scene then
		return
	end
	local npcAOE = scene.GetNpcByNickName("Act312_JiangZhu" .. player.dwID)
	if not npcAOE then
		npcAOE = scene.CreateNpc(46412, player.nX, player.nY, player.nZ, player.nFaceDirection, 20 * GLOBAL.GAME_FPS, "Act312_JiangZhu" .. player.dwID)
	end
end
