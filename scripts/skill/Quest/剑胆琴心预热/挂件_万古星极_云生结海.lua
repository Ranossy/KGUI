---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/剑胆琴心预热/挂件_万古星极_云生结海.lua
-- 更新时间:	2015/9/17 18:07:49
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/剑胆琴心预热/挂件_万古星极_云生结海.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	skill.SetNormalCoolDown(1, 951);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if (IsRemotePlayer(player.dwID)) then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(2, 2693), 4)
		player.SendSystemMessage(GetEditorString(2, 2693))
		return SKILL_RESULT_CODE.FAILED
	end
	--阵营攻防战
	local StateInfo = GetActivityState(41)
	if not StateInfo then
		return
	end
	if StateInfo == ACTIVITY_STATE.NORMAL_ON or StateInfo == ACTIVITY_STATE.RECOVER_ON then
		if player.GetMapID() == 25 or player.GetMapID() == 27 then
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(8, 4479), 4)
			player.SendSystemMessage(GetEditorString(8, 4479))
			return SKILL_RESULT_CODE.FAILED
		end
	end
	--狼牙首领
	if player.GetMapID() == 151 or player.GetMapID() == 156 then
		StateInfo = GetActivityState(188)
		if not StateInfo then
			return
		end
		if StateInfo == ACTIVITY_STATE.NORMAL_ON or StateInfo == ACTIVITY_STATE.RECOVER_ON then
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(8, 4480), 4)
			player.SendSystemMessage(GetEditorString(8, 4480))
			return SKILL_RESULT_CODE.FAILED
		end
		StateInfo = GetActivityState(219)
		if not StateInfo then
			return
		end
		if StateInfo == ACTIVITY_STATE.NORMAL_ON or StateInfo == ACTIVITY_STATE.RECOVER_ON then
	...
	if player.GetMapID() == 139 or player.GetMapID() == 172 then
		StateInfo = GetActivityState(193)
		if not StateInfo then
			return
		end
		if StateInfo == ACTIVITY_STATE.NORMAL_ON or StateInfo == ACTIVITY_STATE.RECOVER_ON then
	...
	--逐鹿中原
	StateInfo = GetActivityState(225)
	if not StateInfo then
		return
	end
	if StateInfo == ACTIVITY_STATE.NORMAL_ON or StateInfo == ACTIVITY_STATE.RECOVER_ON then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(8, 4481), 4)
		player.SendSystemMessage(GetEditorString(8, 4481))
		return SKILL_RESULT_CODE.FAILED
	end

	if not player.nMoveState == MOVE_STATE.ON_STAND then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(2, 8380), 4)
		player.SendSystemMessage(GetEditorString(2, 8380))
		return SKILL_RESULT_CODE.FAILED
	end
	if player.dwBackItemIndex ~= 16683 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND
		return SKILL_RESULT_CODE.FAILED
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.DoAction(0, 13973)
	local scene = player.GetScene()
	if not scene then
		return
	end
	local npcAOE = scene.GetNpcByNickName("Act312_YunSheng" .. player.dwID)
	if not npcAOE then
		npcAOE = scene.CreateNpc(46413, player.nX, player.nY, player.nZ, player.nFaceDirection, 20 * GLOBAL.GAME_FPS, "Act312_YunSheng" .. player.dwID)
	end
end
