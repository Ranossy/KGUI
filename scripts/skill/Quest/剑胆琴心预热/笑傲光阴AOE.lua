---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/剑胆琴心预热/笑傲光阴AOE.lua
-- 更新时间:	2015/9/15 18:16:20
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/剑胆琴心预热/笑傲光阴AOE.lua",											-- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcYangYiFei = scene.GetNpcByNickName("PQ62_YangYiFei")
	if not npcYangYiFei then
		return
	end
	if scene.CheckPlayerInKillersSet(playerTarget.dwID) then
		if playerTarget.GetBuff(9772, 1) then
			playerTarget.DelBuff(9772, 1)
		end
		if playerTarget.GetBuff(9772, 2) then
			playerTarget.DelBuff(9772, 2)
		end
		--RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", "你现在处于江湖追杀阶段，无法获得战效果", 5)
		return
	else
		if playerTarget.nLevel > playerTarget.nMaxLevel-5 then
			--if not playerTarget.GetBuff(9772, 1) then
			playerTarget.AddBuff(npcYangYiFei.dwID, npcYangYiFei.nLevel, 9772, 1)
			return
			--end
		else
			--if not playerTarget.GetBuff(9772, 2) then
			playerTarget.AddBuff(npcYangYiFei.dwID, npcYangYiFei.nLevel, 9772, 2)
			return
			--end
		end
	end
