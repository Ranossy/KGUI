...
tSkillData =
{
	{nDamageBase = 38462, nDamageRand = 0, nCostMana = 0},		--level 1
...
function Apply(dwCharacterID, dwSrcID)
	...
	local dwBossID = npc.GetCustomUnsigned4(Prince_AShiNa.Wizard.ValueName.dwBossID)
	
	local npc_Boss = GetNpc(dwBossID)
	if npc_Boss and npc_Boss.GetBuff(6508, 1) then
		local player = GetPlayer(dwCharacterID)
		if player then
			player.AddBuff(0, 99, 6535, 1)
		end
	end
