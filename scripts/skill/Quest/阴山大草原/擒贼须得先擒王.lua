------------------------------------------------
-- 脚本名称:	scripts/skill/Quest/阴山大草原/擒贼须得先擒王.lua
-- 更新时间:	2015/7/22
-- 更新用户:	zouhui
-- 脚本说明:
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/阴山大草原/擒贼须得先擒王.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID)
	...
	local nQuestID = 13687
	local nQuestPhase = player.GetQuestPhase(nQuestID)
	local nQuestIndex = player.GetQuestIndex(nQuestID)
	if nQuestPhase == 1 then
		player.SetQuestValue(nQuestIndex, 0, 1)
		local npcChenliushi = scene.GetNpcByNickName("chenliushi_heishirenwu")
		if not npcChenliushi then
			npcChenliushi = scene.CreateNpc(43219, player.nX + 300, player.nY + 300, player.nZ, 0, 8 * GLOBAL.GAME_FPS, "chenliushi_heishirenwu")
			npcChenliushi.WalkTo(player.nX, player.nY)
		end
	end
