---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/天策门派跟宠_反复捶打.lua
-- 更新时间:	2016/4/26 16:24:04
-- 更新用户:	zhangtianhui1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/天策门派跟宠_反复捶打.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.GetBuff(10745, 1) then
		return
	end
	if player.GetBuff(10743, 1) then
		player.DelBuff(10743, 1)
		player.AddBuff(0, 99, 10740, 1, 1)
		RemoteCallToClient(player.dwID, "OnDynamicSkillHighlightChanged", 3, false)
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "QTEPanel_HitNotify", 15863)
		player.AddBuff(0, 99, 10749, 1, 1)
	else
		local nQuestId = 15322
		local nQuestIndex = player.GetQuestIndex(nQuestId)
		if nQuestIndex then
			player.SetQuestFailedFlag(nQuestIndex, true)
		end
		local buff = player.GetBuff(10746, 1) --设置计时器标识
		if not buff then
			return
		end
		player.RemoveTimer(buff.nCustomValue)
		local tbBuff = {
			10740,
			10741,
			10742,
		...
		for _, nBuffId in pairs(tbBuff) do
			if player.GetBuff(nBuffId, 1) then
				player.DelBuff(nBuffId, 1)
			end
		end
		player.DelGroupBuff(10749, 1) --可堆叠buff特殊处理
		player.DoAction(0, 10030)
	end
end
