---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/千岛湖/惩戒凶吏完成任务.lua
-- 更新时间:	2015/10/26 17:56:14
-- 更新用户:	wangwei-pc2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nQuestID2 = 14036 --心有忌惮
	local nQuestPhase2 = player.GetQuestPhase(nQuestID2)
	local nQuestIndex2 = player.GetQuestIndex(nQuestID2)
	if nQuestIndex2 and nQuestPhase2 == 1 then
		local npc = GetNpc(dwSkillSrcID)
		if npc then
			if npc.dwTemplateID == 44609 then  --"白头翁"姚兆丰
				player.SetQuestValue(nQuestIndex2, 0, 1)
			elseif npc.dwTemplateID == 44599 then --乐忠
				player.SetQuestValue(nQuestIndex2, 1, 1)
			elseif	npc.dwTemplateID == 44222 then --“无面鬼”俞泾舟
				player.SetQuestValue(nQuestIndex2, 2, 1)
			end
		end
