---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/赛马_纵马驰骋.lua
-- 更新时间:	2017/5/9 15:13:02
-- 更新用户:	qingfupei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 4086, 1);		-- 设置1号位Buff
...
--判断玩家的状态，以判断是否可以发出技能
	...
	if item and item.dwIndex ~= 566 and item.dwIndex ~= 567 then
		return nPreResult;
	else
		return false;
	end
...
function Apply(dwCharacterID)
	...
	if item and item.dwIndex ~= 566 and item.dwIndex ~= 567 then
		...
		player.AddBuff(dwCharacterID, player.nLevel, 8475, 1)	--马匹属性·监控处理
	else
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_RED", GetEditorString(5, 7418), 10)
	end
end
