---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/丝绸占旗.lua
-- 更新时间:	2017/1/17 12:53:12
-- 更新用户:	wangsongtao-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nID = npc.GetCustomInteger1(0)
	local tXYZ = {
		{"JunQi_1_", 36144, 15939, 1175690},
		{"JunQi_2_", 35651, 28951, 1180019},
		{"JunQi_3_", 32380, 44502, 1178228},
		{"JunQi_4_", 48687, 29481, 1176111},
		{"JunQi_5_", 20324, 27620, 1175879},
	}
	if not tXYZ[nID] then
		return
	end
	local doodad = scene.GetDoodadByNickName(tXYZ[nID][1] .. "1")
	if not doodad then
		doodad = scene.GetDoodadByNickName(tXYZ[nID][1] .. "2")
	end
	--print(dwCharacterID, dwSkillSrcID,nID,doodad and doodad.szName or "nodoodad")
	if not doodad then
		return
	end
	if doodad.dwTemplateID == 2901 then -- 中立旗子不算争夺
		return
	end
	local nQuestID = 16462 -- 默认争夺
	if doodad.dwTemplateID == 2899 and player.nBattleFieldSide == 0 then -- 浩气的旗 浩气防守
		nQuestID = 16473
	elseif doodad.dwTemplateID == 2900 and player.nBattleFieldSide == 1 then -- 恶人的旗 恶人防守
		nQuestID = 16473
	end

	tBattleFunction.AddPlayerQuestValue(player, nQuestID, 0, 1, 30)
end
