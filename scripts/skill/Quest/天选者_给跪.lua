---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/天选者_给跪.lua
-- 更新时间:	2013/11/28 15:33:21
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.SetNormalCoolDown(1, 678);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if nPreResult then
		player.AddBuff(player.dwID,player.nLevel,7242,1,1)
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local caster = GetPlayer(dwSkillSrcID)
	
	if dwCharacterID == dwSkillSrcID then
		if caster then
			caster.AddBuff(dwSkillSrcID,caster.nLevel,7242,1,1)
		end
	else
		if target and caster and not target.GetBuff(7247, 1) then
			target.Stop()
			target.SetTarget(TARGET.PLAYER, dwSkillSrcID)
			target.TurnToCharacter(dwSkillSrcID)
			target.DoAction(0, 10150)
			target.Talk(caster.szName .. GetEditorString(6, 1781))
			target.AddBuff(0, 99, 7247, 1, 10)
		end
	end
end
