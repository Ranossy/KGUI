---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/七夕-缘缘找人.lua
-- 更新时间:	2016/6/27 18:00:25
-- 更新用户:	xutong3
-- 脚本说明:	16092
----------------------------------------------------------------------<
------------------------------------------------
...
Include("scripts/Map/节日七夕/include/缘缘头文件.lua")
local NPCCUSTOM = GetCustom()
--tSkillData =
--{
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc.GetCustomUnsigned4(NPCCUSTOM.TARGETLOCK) == 1 then--已经找到
		return
	end
	if npc.GetCustomUnsigned4(NPCCUSTOM.MASTERID) == dwCharacterID then
	--	print("是主人")
		return
	end

	for i = 0, 2 do
		if npc.GetCustomUnsigned4(NPCCUSTOM.IGNOREID1 + i*4) == dwCharacterID then--在嫌弃列表里面
			return
		end
	end
	...
	if npc.GetCustomUnsigned4(NPCCUSTOM.TARGETROLETYPE) ~= 10086 and player.nRoleType ~= npc.GetCustomUnsigned4(NPCCUSTOM.TARGETROLETYPE) then
	--	print( player.nRoleType.."体型不合适"..npc.GetCustomUnsigned4(NPCCUSTOM.TARGETROLETYPE))
		return
	end
	if npc.GetCustomUnsigned4(NPCCUSTOM.TARGETMENPAI) ~= 10086 and player.dwForceID ~= npc.GetCustomUnsigned4(NPCCUSTOM.TARGETMENPAI) then
	--	print(player.dwForceID.."门派不合适"..npc.GetCustomUnsigned4(NPCCUSTOM.TARGETMENPAI))
		return
	end

	if player.GetItemAmount(5, 26832) >= 1 then
		return
	end
	if player.bFightState == true then
		return
	end
	if player.nMoveState ~= MOVE_STATE.ON_SIT and player.nMoveState ~= MOVE_STATE.ON_STAND and player.nMoveState ~= MOVE_STATE.ON_WALK then
		return
	end
	
	npc.SetCustomUnsigned4(NPCCUSTOM.TARGETID, player.dwID)
	npc.SetCustomUnsigned4(NPCCUSTOM.TARGETLOCK,1)
	npc.FireAIEvent(2002,1,1)
end
