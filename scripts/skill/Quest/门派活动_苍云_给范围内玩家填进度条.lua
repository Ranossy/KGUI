---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/门派活动_苍云_给范围内玩家填进度条.lua
-- 更新时间:	2014/12/2 17:29:33
-- 更新用户:	wangwei-pc2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/门派活动_苍云_给范围内玩家填进度条.lua",
		0
	);
...
function Apply(dwCharacterID)
	...
	local npc = scene.GetNpcByNickName("MPRQ_CY_ZDZK")
	if not npc then
		return
	end
	local step = npc.GetCustomInteger1(1)
	if step == 0 then
		return
	elseif step == 1 then
		local score = npc.GetCustomInteger4(2)
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "GeneralProgressBar_Create", "cangyunshiqi", 9, GetEditorString(7, 769), "", score, 1000)
	elseif step == 2 then
		local score = npc.GetCustomInteger4(2)
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "GeneralProgressBar_Create", "cangyunshiqi", 9, GetEditorString(7, 770), "", score, 1000)
	elseif step == 3 then
		local score = npc.GetCustomInteger4(2)
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "GeneralProgressBar_Create", "cangyunshiqi", 9, GetEditorString(7, 771), "",  score, 1000)
	end
	player.AddBuff(0, 99, 8687, 1)  --添加标记BUFF，结束后关闭界面。
end
