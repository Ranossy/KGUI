---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/苍山洱海_带领跳舞.lua
-- 更新时间:	2012-7-9 16:33:47
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
	...
	if player.nRoleType == ROLE_TYPE.STANDARD_MALE then
		...
		if n < 30 then
			player.DoAction(0, 10151) --30%几率鼓掌
			player.AddBuff(0, 99, 3633, 1)
			player.SendSystemMessage(GetEditorString(4, 325))
		else player.DoAction(0, 10927)--70%几率跳舞
			player.SendSystemMessage(GetEditorString(4, 326))
		end
	elseif player.nRoleType == ROLE_TYPE.STANDARD_FEMALE then
		...
		if m < 30 then
			...
			player.AddBuff(0, 99, 3633, 1)
			player.SendSystemMessage(GetEditorString(4, 325))
		else player.DoAction(0, 10939)--70%几率跳舞
			player.SendSystemMessage(GetEditorString(4, 326))
		end
	elseif player.nRoleType == ROLE_TYPE.LITTLE_GIRL then
		local q = Random(1, 100)
		if q < 30 then
			player.DoAction(0, 10151) --30%几率鼓掌
			player.AddBuff(0, 99, 3633, 1)
			player.SendSystemMessage(GetEditorString(4, 325))
