---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/方士_挖宝.lua
-- 更新时间:	2016/6/14 17:45:53
-- 更新用户:	zhangtianhui1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	--skill.SetNormalCoolDown(1, 1059);	--技能普通CD，入定和还魂CD一样，防止反复切换
	...
	--skill.nCostItemType	= 5;									-- 技能消耗的物品类型
	--skill.nCostItemIndex	= 26760;									-- 技能消耗的物品索引ID
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local dwPlayerId = npc.GetCustomUnsigned4(0)
	local player = GetPlayer(dwPlayerId)
	if not player then
		return
	end
	local eTargetType, nTargetdwID = player.GetTarget()
	if eTargetType == TARGET.NO_TARGET then
		player.SendSystemMessage(GetEditorString(10, 2274))
		return
	end
	...
	local npcYinyang = GetNpc(nTargetdwID)
	if not npcYinyang then
		return
	end
	if npcYinyang.dwTemplateID ~= npc.dwTemplateID then
		player.SendSystemMessage(GetEditorString(10, 2274))
		return
	end
	...
	if not npcFuzhou then
		npcFuzhou = scene.CreateNpc(51582, npc.nX, npc.nY, npc.nZ, player.nFaceDirection, 10 * GLOBAL.GAME_FPS, "idcharm" .. player.dwID)
		player.SetNpcEffectVisible(51582, 53)
	end
	player.CastSkill(16061, 1)
end
