-- 脚本名称:	scripts/skill/Quest/江湖百态/照明弹.lua
---------------------------------------------------------------------->
-- 更新时间:	2016/7/13 22:25:09
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if (manager and manager.dwCurrentIdentityType == PLAYER_IDENTITY_TYPE.BODYGUARD) then
		LuLinIdentity.OpenCamp(player, GetEditorString(10, 2964))
		return nPreResult;
	end
	RemoteCallToClient(player.dwID, "OnSendSystemAnnounce", GetEditorString(10, 2501), "red")
	return false;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npc = scene.CreateNpc(51974, nX, nY, nZ, 1, 6 * 16)
	local nTime = 2.2
	npc.SetActiveMark(true)
	npc.SetForceID(28)
	player.CastSkillXYZ(16031, 1, npc.nX, npc.nY, npc.nZ)
	...
	player.SetTimer(nTime * 16, "scripts/skill/Quest/江湖百态/照明弹.lua", npc.dwID, 1)
end
function OnTimer(player, dwNpcID, n2)
--	print("timer",player,dwNpcID, n2,GetNpc(dwNpcID))
	local npc = GetNpc(dwNpcID)
	if not npc then
		return
	end
	if n2 == 1 then
		player.PlaySfx(1377, npc.nX, npc.nY, npc.nZ)
	end
end
