---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/冰域邪王虎_找人.lua
-- 更新时间:	2016/9/22 15:36:26
-- 更新用户:	kelin-PC
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.nTargetCountLimit	= 1000;				-- 技能作用目标数量限制,(小于0 代表目标数量不限制)
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local Boss = GetNpc(dwSkillSrcID)
	if not player then
		return
	end
	if not Boss then
	...
	if Boss.GetCustomUnsigned4(0) == 0 then
		Boss.SetCustomUnsigned4(0, player.dwID)
		if not player.GetBuff(11296, 1) then
		    player.AddBuff(player.dwID, player.nLevel, 11296, 1, 60)
		end
	elseif Boss.GetCustomUnsigned4(5) == 0 then
		Boss.SetCustomUnsigned4(5, player.dwID)
		if not player.GetBuff(11296, 1) then
		    player.AddBuff(player.dwID, player.nLevel, 11296, 1, 60)
		end
	elseif Boss.GetCustomUnsigned4(9) == 0 then
		Boss.SetCustomUnsigned4(9, player.dwID)
		if not player.GetBuff(11296, 1) then
		    player.AddBuff(player.dwID, player.nLevel, 11296, 1, 60)
		end
	end
	if not player.GetBuff(11287, 1) then
		player.AddBuff(player.dwID, player.nLevel, 11287, 1, 600)
	end
end
