---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/招小弟.lua
-- 更新时间:	2016/7/10 20:28:48
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
--	skill.AddSlowCheckSelfBuff(10962, 0, eCompareFlag, 1, eLevelCompareFlag);		-- 需求自身Buff
	...
	skill.SetNormalCoolDown(1, 745);	--技能普通CD
	...
	----------------- 时间相关 -------------------------------------------------
	skill.nPrepareFrames  	= 16 * 3;				-- 吟唱帧数
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if player.GetBuff(10962, 1) then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_YELLOW", GetEditorString(10, 2661), 4)
		return false
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local tPlace = CustomFunction.GetFrontNpcPosition(player.nX, player.nY, player.nFaceDirection, 2 * 64)
	local scene = player.GetScene()
	local tPlace1 = CustomFunction.GetFrontNpcPosition(player.nX, player.nY, player.nFaceDirection, 22 * 64)
	if scene.IsNickNameNpcExist("lulinxiaodi" .. player.dwID) then
		scene.DestroyNpcByNickName("lulinxiaodi" .. player.dwID)
	end
	local npc = scene.CreateNpc(51942, tPlace1.nX, tPlace1.nY, player.nZ, tPlace1.nDir, 2 * 60 * 16, "lulinxiaodi" .. player.dwID)
	if npc then
		npc.dwOwner = player.dwID
		npc.RunTo(tPlace.nX, tPlace.nY)
		npc.SayTo(player.dwID, player.szName .. GetEditorString(10, 2628))
	--	player.PlaySfx(190, npc.nX, npc.nY, npc.nZ)
		npc.SetDialogFlag(0)
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_YELLOW", GetEditorString(10, 2645), 5)
		npc.JumpTo(tPlace.nX, tPlace.nY, player.nZ, 0, 16 * 3)
		player.SetTimer(5 * 16, "scripts/skill/Quest/江湖百态/招小弟.lua", npc.dwID, 0)
	end
end

function OnTimer(player, n1, n2)
	local npc = GetNpc(n1)
	if not npc then
		return
	end
	npc.SetDialogFlag(1)
	npc.SayTo(player.dwID,  GetEditorString(10, 2626))
end
