---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/方士_锁灵符.lua
-- 更新时间:	2016/8/10 15:05:27
-- 更新用户:	lilin101
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/江湖百态/方士_锁灵符.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	--skill.AddSlowCheckDestBuff(11080, 1, COMPARE.EQUAL, dwSkillLevel, COMPARE.EQUAL);		-- 需求目标Buff
	skill.AddSlowCheckDestBuff(11081, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 11081, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npc_Dig = scene.CreateNpc(52295, npc.nX, npc.nY, npc.nZ, 0, 600 * GLOBAL.GAME_FPS, "IdentityWizardDig" .. player.dwID)	-- 用于标记贴符位置
	if npc_Dig then
		npc_Dig.SetCustomUnsigned4(1, npc.dwID)
		npc.SetCustomUnsigned4(Identity_Wizard.Ghost.ValueName.dwDigPosition, npc_Dig.dwID)
		if npc.dwTemplateID == 51501 then
			npc_Dig.SetCustomUnsigned1(0, 1)	-- 1表示藏宝堆标记。用于挖宝判断
		end
	end
