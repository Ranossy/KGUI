---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/镖师承伤时间到.lua
-- 更新时间:	2016/6/11 21:57:36
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
Include("scripts/Map/江湖百态/镖师/include/镖师_data.lua")
--------------脚本文件开始------------------------------------------------
...
-- 下线消失 不走这里，如何处理？
function OnRemove(dwCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	--[[
	if nCustomValue == 1 or nLeftFrame == 0 then -- 打过标记或者时间到的不进行判断， 只处理因为条件不符合而被删除BUFF的	情况。
		return
	end
	local player = GetPlayer(dwCharacterID) -- 镖师
	local dwOwnerID = player.GetPlayerIdentityManager().GetCustomUnsigned4(PLAYER_IDENTITY_TYPE.BODYGUARD, BIAOSHI_IDENTITY_CUSTOM_VALUE.OWNER_ID)
	local owner = dwOwnerID > 0 and GetPlayer(dwOwnerID) or nil
	if not owner then
		return
	end
	
	local tBiaoshiList = BiaoShiIdentity.GetOwnerAllBiaoShi(owner)
	local tDisInfo = {}
	for i = 1, #tBiaoshiList do
		if tBiaoshiList.nMoveState ~= MOVE_STATE.ON_DEATH then
			local manager = tBiaoshiList[i].GetPlayerIdentityManager()
			local nDis1 = (tBiaoshiList[i].nX - owner.nX) ^ 2 + (tBiaoshiList[i].nY - owner.nY) ^ 2
			local nProtectDis = BiaoShiIdentity.tProtectDis[GetIdentityLevel(1, manager.GetIdentityExp(1))] ^ 2
			if nDis1 <= nProtectDis then
				table.insert(tDisInfo, {nID = i, nDis = nDis1})
			end
		end
		
		for j = 1, 4 do
			local buff1 = tBiaoshiList[i].GetBuff(10834, j)
			if buff1 then
				buff1.nCustomValue = 1 -- 有此标记，不走OnRemove
				tBiaoshiList[i].DelBuff(10834, j)
			end
		end
	end
	...
	if #tDisInfo > 0 then
		if #tDisInfo > 1 then 
			table.sort(tDisInfo, BiaoshiDisSort)
		end
		tBiaoshiList[tDisInfo[1].nID].AddBuff(dwOwnerID, dwOwnerID.nLevel, 10834, math.max(#tBiaoshiList - 1, 1))
	end
	--]]
end

-- t1 = { nDis = 1, nId =1}
function BiaoshiDisSort(t1, t2)
	if t1.nDis > t2.nDis then
		return false
	end
	return true
end
