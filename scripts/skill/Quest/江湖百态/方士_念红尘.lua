---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/方士_念红尘.lua
-- 更新时间:	2016/10/12 1:25:41
-- 更新用户:	lilin101
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 11438, 1);		-- 设置1号位Buff
	...
	----------------- 作用范围 -------------------------------------------------
	--skill.nProtectRadius = 30 * LENGTH_BASE;                     -- 环形和矩形AOE的保护距离，范围内不受伤害
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.dwIdentityVisiableID ~= 3 then
		return
	end
	if player.GetBuff(10827, 1) then
		player.AddBuff(0, 99, 11438, 1, 10)
		player.DelBuff(10827, 1)
		if player.GetBuff(11494, 1) then
			player.DelBuff(11494, 1) --删除方士半透明buff
		end
		player.SendSystemMessage(GetEditorString(10, 9433))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(10, 9433), 3)
		if npc.dwTemplateID ~= 52199 then
			if not player.IsAchievementAcquired(5394) then
				player.AddAchievementCount(5394, 1)
			end
			local tAchList = {[6] = 5406, [150] = 5407, [102] = 5408, [193] = 5646, [158] = 5409}
			local scene = player.GetScene()
			if scene and tAchList[scene.dwMapID] then
				if not player.IsAchievementAcquired(tAchList[scene.dwMapID]) then
					player.AcquireAchievement(tAchList[scene.dwMapID])
				end
			end
			local buff = npc.GetBuff(11080, 0)
			if buff then
				local tExp = {[2] = 30, [3] = 60, [4] = 120, [5] = 240}
				if tExp[buff.nLevel] then
					player.GetPlayerIdentityManager().AddIdentityExp(3, tExp[buff.nLevel])	-- 增加方士经验
					CheckIdentityRankingData(player, PLAYER_IDENTITY_TYPE.WIZARD, 1)	-- 身份排行榜
				end
			end
		end
	end
