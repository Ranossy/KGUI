---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/方士_宠物发现魂灵.lua
-- 更新时间:	2016/7/11 10:06:09
-- 更新用户:	zhangtianhui2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/江湖百态/方士_宠物发现魂灵.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local npcLight = GetNpc(dwSkillSrcID)
	if not npcLight then
		return
	end
	local npcFind = GetNpc(dwCharacterID)
	if not npcFind then
		return
	end
	local tChooseNpc = {
		51495, 51501, --暂时只搜索弱小的魂灵和挖宝点
	}
	for _, dwChooseId in pairs(tChooseNpc) do
		if dwChooseId == npcFind.dwTemplateID then
			local nNewDistance = GetDistanceSq(npcLight.nX, npcLight.nY, 0, npcFind.nX, npcFind.nY, 0)
			if nNewDistance <= (100 * 64) ^ 2 then
				local dwOldId = npcLight.GetCustomUnsigned4(8)
				local npcOld = GetNpc(dwOldId)
				if not npcOld then
					npcLight.SetCustomUnsigned4(8, dwCharacterID)
					return
				end
				local nOldDistance = GetDistanceSq(npcLight.nX, npcLight.nY, 0, npcOld.nX, npcOld.nY, 0)
				if nOldDistance > nNewDistance then
					npcLight.SetCustomUnsigned4(8, dwCharacterID)
					return
				end
			end
