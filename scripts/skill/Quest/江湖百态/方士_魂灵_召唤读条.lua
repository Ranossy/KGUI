---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/方士_魂灵_召唤读条.lua
-- 更新时间:	2016/7/4 19:40:32
-- 更新用户:	wangwei2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/江湖百态/方士_魂灵_召唤读条.lua", -- 属性值1
		0														-- 属性值2
	)
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc.dwIdentityVisiableID ~= 0 then
		return
	end
	local scene = npc.GetScene()
	if scene then
		scene.SetTimer(0.1 * GLOBAL.GAME_FPS, "scripts/skill/Quest/江湖百态/方士_魂灵_召唤读条.lua", npc.dwID, 0)
	end
end
...
function OnTimer(scene, nParam1, nParam2)
	...
	npc.SetNpcTeamID(999)
	
	local radius = 4 --半径待定
	--local nDir = npc.nFaceDirection * 2 * math.pi / 256 --面向从256转换成360°
	local nDir = 0
	local seDir = nDir + (nParam2 * 2 * math.pi / 4)
	local N_X = npc.nX + radius * 64 * math.cos(seDir)
	local N_Y = npc.nY + radius * 64 * math.sin(seDir)
	--local guard = scene.GetNpcByNickName("IdentityWizard25Ghost_Guard" .. nParam2 .. npc.dwID)  --"IdentityWizard25Ghost_Guard" .. nParam2 .. npc.dwID
	--if not guard then
	local guard = scene.CreateNpc(51718, N_X, N_Y, npc.nZ, nParam2 * 64 + 128, 60 * GLOBAL.GAME_FPS)
	if guard then
		guard.SetNpcTeamID(999)
		guard.TurnToCharacter(npc.dwID)
		npc.PlaySfx(1353, N_X, N_Y, npc.nZ + 700)
		guard.TurnToCharacter(npc.dwID)
		guard.AddBuff(guard.dwID, guard.nLevel, 10846, 1)
		--[[
		local player = GetPlayer(npc.dwDropTargetPlayerID)
		if player then
			npc.ModifyThreat(npc.dwDropTargetPlayerID, 100)	-- 没卵用	
		end
		--]]
	end
	if nParam2 <= 2 then
		scene.SetTimer(1 * GLOBAL.GAME_FPS, "scripts/skill/Quest/江湖百态/方士_魂灵_召唤读条.lua", npc.dwID, nParam2 + 1)
	end
end
