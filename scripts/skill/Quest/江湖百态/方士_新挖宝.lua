---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/方士_新挖宝.lua
-- 更新时间:	2016/8/13 22:08:45
-- 更新用户:	lilin101
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddSlowCheckDestBuff(11081, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npc_Dst = GetNpc(dwCharacterID)
	if not (npc_Dst and npc) then
		return
	end
	if npc_Dst.dwTemplateID ~= 51501 then
		return
	end
	...
	local dwOwnerID = npc_Dst.GetCustomUnsigned4(Identity_Wizard.Ghost.ValueName.dwOwnerID)
	local dwPlayerID = npc.GetCustomUnsigned4(Identity_Wizard.DigControl.ValueName.dwOwnerID)
	...
	if player.GetBuff(Identity_Wizard.BuffList.Dig.dwID, Identity_Wizard.BuffList.Dig.nLevel) then
		return
	end
	player.AddBuff(player.dwID, player.nLevel, Identity_Wizard.BuffList.Dig.dwID, Identity_Wizard.BuffList.Dig.nLevel, 3)
	local buff = player.GetBuff(Identity_Wizard.BuffList.Dig.dwID, Identity_Wizard.BuffList.Dig.nLevel)
	if buff then
		buff.nCustomValue = dwCharacterID
	end
	if npc_Dst.GetBuff(Identity_Wizard.BuffList.SummonFlag.dwID, Identity_Wizard.BuffList.SummonFlag.nLevel) then
		npc_Dst.DelBuff(Identity_Wizard.BuffList.SummonFlag.dwID, Identity_Wizard.BuffList.SummonFlag.nLevel)
	end
	if npc_Dst.GetBuff(Identity_Wizard.BuffList.Rune.dwID, Identity_Wizard.BuffList.Rune.nLevel) then
		npc_Dst.DelBuff(Identity_Wizard.BuffList.Rune.dwID, Identity_Wizard.BuffList.Rune.nLevel)
	end
	
	local npc_Dig = scene.GetNpcByNickName("IdentityWizardDig" .. player.dwID)
	if npc_Dig then
		npc_Dig.SetDisappearFrames(1)
	end
	npc.SetDisappearFrames(1)
	player.AddMapMark(-10000, -10000, -10000, 1, "")
	--local nCount = npc.GetCustomUnsigned4(Identity_Wizard.DigControl.ValueName.dwNum)
	--npc.SetCustomUnsigned4(Identity_Wizard.DigControl.ValueName.dwNum, nCount + 1)
end
