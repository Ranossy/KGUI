---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/艺人_区域判断.lua
-- 更新时间:	2016/7/7 20:43:05
-- 更新用户:	YANGYUBO
...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(10819, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	skill.SetNormalCoolDown(1, 1079);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if not player.GetBuff(10819, 1) then
		return nPreResult;
		end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local player2 = GetPlayer(dwSkillSrcID)
	if not player2 then
		return
	end
	if dwCharacterID == dwSkillSrcID then
		player2.SetTimer(0.1 * GLOBAL.GAME_FPS, "scripts/skill/Quest/江湖百态/艺人_区域判断.lua", 0, 0)
	else
		if player and player.GetBuff(10819, 1) then
			player2.AddBuff(player2.dwID, player.nLevel, 10952, 1)		
		end
	end
...
function OnTimer(player, nParam1, nParam2)
	if player.GetBuff(10952, 1) then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(10, 2499), 5)
		player.SendSystemMessage(GetEditorString(10, 2499))
		player.ClearCDTime(1079)
		player.DelBuff(10952, 1)
	else
		player.CastSkill(16145, 1)
	end
end
