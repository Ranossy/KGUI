---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/冰域邪王虎_怒吼.lua
-- 更新时间:	2016/9/28 14:10:34
-- 更新用户:	kelin-PC
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.GetBuff(10864, 1) then
		player.AddBuff(player.dwID, player.nLevel, 11487, 1, 10)
	end
	local dwHashValue = GetFileNameHash("scripts/Map/江湖百态/驭兽师/trap/冰域邪王虎_草丛.lua")
	if player.dwLastScriptID == dwHashValue then
		if not player.IsAchievementAcquired(5679) then
			player.AddAchievementCount(5679, 1)
		end
		return
	end
	if not player.GetBuff(11285, 1) then
		player.AddBuff(player.dwID, player.nLevel, 11285, 1, 5)
	end
