---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/穿云箭.lua
-- 更新时间:	2016/7/8 17:26:03
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<

--------------脚本文件开始------------------------------------------------
Include("scripts/Map/世界奇遇/include/科霖奇遇小剧场.lua")
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if player.dwWaistItemIndex ~= 18308 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND 
		return SKILL_RESULT_CODE.FAILED
	end
	if not player.GetAdventureFlag(1115) then
		player.SendSystemMessage(GetEditorString(10, 3844))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(10, 3844), 4)
		return SKILL_RESULT_CODE.FAILED
	end
	if player.GetAdventureFlag(1115) and not player.GetAdventureFlag(1116) then
		local dwHashValue = GetFileNameHash("scripts/Map/世界奇遇/护佑苍生/trap/终极决战.lua")
		if player.dwLastScriptID == dwHashValue then
			SmallTheater_CreatDirector(player, 22)
		end
	end
	return nPreResult;
end
