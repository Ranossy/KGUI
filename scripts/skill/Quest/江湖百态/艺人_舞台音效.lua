---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/艺人_舞台音效.lua
-- 更新时间:	2016/7/13 20:44:03
-- 更新用户:	lilin101
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/江湖百态/艺人_舞台音效.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckDestBuff(10979, 0, COMPARE.EQUAL, 0, COMPARE.EQUAL);	
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local dwArtistID = npc.GetCustomUnsigned4(0)
	local player_Artist = GetPlayer(dwArtistID)
	if not player_Artist then
		return
	end
	if player.GetBuff(10979, 1) then
		return
	end
	player.AddBuff(0, 99, 10979, 1, 10)
	local buff = player.GetBuff(10979, 1)
	if not buff then
		return
	end
	buff.nCustomValue = dwSkillSrcID
	local tMusicList = {
		[ROLE_TYPE.LITTLE_BOY] = GetEditorString(10, 3039),
		[ROLE_TYPE.LITTLE_GIRL] = GetEditorString(10, 3040),
		[ROLE_TYPE.STANDARD_MALE] = GetEditorString(10, 3039),
		[ROLE_TYPE.STANDARD_FEMALE] = GetEditorString(10, 3040),
	}
	RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "PlayBgMusic", tMusicList[player_Artist.nRoleType], 0)
end
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	RemoteCallToClient(nCharacterID, "CallUIGlobalFunction", "BgMusic_TryPlayBack")
end
