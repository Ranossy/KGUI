---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/驭兽师_驭兽标记.lua
-- 更新时间:	2016/6/22 17:07:13
-- 更新用户:	lilin101
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.CanCostStamina() then
		if player.GetBuff(10864, 1) then
			player.DelBuff(10864, 1)
		end
		player.SendSystemMessage(GetEditorString(10, 1844))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(10, 1845), 5)
		return
	end

	if player.nCurrentStamina < 10 then
		if player.GetBuff(10864, 1) then
			player.DelBuff(10864, 1)
		end
		player.SendSystemMessage(GetEditorString(10, 1846))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(10, 1847), 5)
		return
	end
	...
	IdentityManager.AddIdentityExp(PLAYER_IDENTITY_TYPE.HUNTER, 6)
	CheckIdentityRankingData(player, PLAYER_IDENTITY_TYPE.HUNTER, 1)
end
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	Identity_YuShou.EndPetView(player)
end
