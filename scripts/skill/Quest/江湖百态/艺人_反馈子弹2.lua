---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/艺人_反馈子弹2.lua
-- 更新时间:	2016/7/13 23:12:24
-- 更新用户:	wangwei2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/江湖百态/艺人_反馈子弹2.lua", -- 属性值1
		0														-- 属性值2
	)
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 10969, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not actor.GetBuff(10819, 1) then
		return
	end
	...
	local tBuff = {10969, 10971, 10972, 10973}
	for i = 1, #tBuff do
		local buff = player.GetBuff(tBuff[i], 1)
		if buff then
			player.DelBuff(tBuff[i], 1)
		end
	end
	player.AddBuff(actor.dwID, actor.nLevel, 10971, 1)
	actor.Talk(GetEditorString(10, 3032) .. player.szName .. GetEditorString(10, 3033))
end
