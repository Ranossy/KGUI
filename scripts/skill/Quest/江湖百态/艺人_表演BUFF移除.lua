---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/艺人_表演BUFF移除.lua
-- 更新时间:	2016/6/22 21:33:12
-- 更新用户:	YANGYUBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	local buff = player.GetBuff(10951, 1)
	if buff then
		player.DelBuff(10951, 1)
	end
	ArtistEnd(player)
	--[[local player = GetPlayer(nCharacterID)
	local NowIdentity = player.GetPlayerIdentityManager().dwCurrentIdentityType
	if NowIdentity == PLAYER_IDENTITY_TYPE.ARTIST then
