---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/释放隐身.lua
-- 更新时间:	2016/6/16 17:24:34
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/江湖百态/释放隐身.lua", -- 属性值1
		0														-- 属性值2
		)
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not manager or manager.dwCurrentIdentityType ~= PLAYER_IDENTITY_TYPE.ROB then
		return
	end

	local dwID = manager.GetCustomUnsigned4(PLAYER_IDENTITY_TYPE.ROB, LVLIN_IDENTITY_CUSTOM_VALUE.INVISIBLE_TRAP_ID)
	local npc = GetNpc(dwID)
	if not npc then
		return
	end
	local nRet = player.CastSkillXYZ(16012, 1, npc.nX, npc.nY, npc.nZ)
	if nRet == 1 then
		npc.DoAction(0, 10150) -- 触发动作
		player.SetTimer(20, "scripts/skill/Quest/江湖百态/释放隐身.lua", npc.dwID, 0)
		--player.SetDynamicSkillGroup(286)
	elseif nRet == 18 then
		player.SendSystemMessage(GetEditorString(10, 1392))
	end
end

function OnTimer(player, dwID, n2)
	local npc = GetNpc(dwID)
	if npc then
		npc.Die()
	end
