---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/驭兽_静静.lua
-- 更新时间:	2016/7/15 19:19:55
-- 更新用户:	zhangtianhui2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 1064);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.CastSkill(16278, 1)
	local nSign = Random(1, 50000)
	local buff = player.GetBuff(10864, 1)
	if not buff then
		return
	end
	local dwRoushenId = buff.nCustomValue
	local npcRoushen = GetNpc(dwRoushenId)
	if not npcRoushen then
		return
	end
	npcRoushen.SetCustomUnsigned2(9, nSign)
	player.SetTimer(0.6 * GLOBAL.GAME_FPS, "scripts/skill/Quest/江湖百态/驭兽_哈皮.lua", 0, nSign)
end
...
function OnTimer(player, nParam1, nParam2)
	...
	local nSign = npcRoushen.GetCustomUnsigned2(9)
	if nSign ~= nParam2 then
		return
	end
	if nParam1 < 4 then
		player.CastSkill(16279, 1)
		player.SetTimer(0.4 * GLOBAL.GAME_FPS, "scripts/skill/Quest/江湖百态/驭兽_哈皮.lua", nParam1 + 1, nParam2)
	end
end
