---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/方士_招魂_判断周围.lua
-- 更新时间:	2016/7/11 23:32:53
-- 更新用户:	wangwei2
-- 脚本说明:	对自身周围区域释放技能，判断是否有表演中的玩家，如果有，则无法开启自身表演
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/江湖百态/方士_招魂_判断周围.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(10958, 0, COMPARE.EQUAL, 0, COMPARE.EQUAL);		-- 需求目标Buff
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if player.GetItemAmount(5, 26760) >= 20 then
		local tPartyMember = player.GetPartyMemberList()
		if not tPartyMember then
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(10, 2180), 3)
			player.SendSystemMessage(GetEditorString(10, 2180))
			return false
		end
		return nPreResult
	else
...
function Apply(dwCharacterID, dwSkillSrcID)
	local player_fangshi = GetPlayer(dwCharacterID)
	local player = GetPlayer(dwSkillSrcID)
	if not player or not player_fangshi then
		return
	end
	if player.nCurrentStamina < 120 then
		player.SendSystemMessage(GetEditorString(1, 4030))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(10, 4693), 5)
		return
	end
	local nQuestId = 15462
	local nQuestIndex = player.GetQuestIndex(nQuestId)
	...
	local npc = scene.CreateNpc(52258, player.nX, player.nY, player.nZ, 0, 3 * GLOBAL.GAME_FPS)
	if npc then
		npc.SetCustomUnsigned4(0, player.dwID)
		npc.CastSkill(16562, 1)
	end
	if dwCharacterID ~= dwSkillSrcID then
		if player_fangshi.GetBuff(10964, 1) then    --持续施展法阵和标记不能使用法阵，用同一个BUFF，都是24帧一跳
			player.AddBuff(player.dwID, player.nLevel, 10964, 1)
			return
		end
	else		
		player.SetTimer(0.2 * GLOBAL.GAME_FPS, "scripts/skill/Quest/江湖百态/方士_招魂_判断周围.lua", 0, 0)
	end
end
...
function OnTimer(player, nParam1, nParam2)
	if player.GetBuff(10964, 1) then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(10, 2668), 5)
		player.SendSystemMessage(GetEditorString(10, 2668))
		player.DelBuff(10964, 1)
	elseif not player.GetBuff(11129, 1) then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(10, 5223), 5)
		player.SendSystemMessage(GetEditorString(10, 5224))
	else
		player.CastSkill(15999, 1)
	end
end
