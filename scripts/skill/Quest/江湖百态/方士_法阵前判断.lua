---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/江湖百态/方士_法阵前判断.lua
-- 更新时间:	2016/8/25 17:14:07
-- 更新用户:	lilin101
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/江湖百态/方士_法阵前判断.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(11081, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--[[local dwFellowID = npc_Dst.GetCustomInteger4(Identity_Wizard.Ghost.ValueName.nFellowID)
	if dwFellowID ~= 0 then
		if not IsParty(dwFellowID, dwPlayerID) then
			return
		end
	end--]]
	...
	if player.GetBuff(11129, 1) then
		return
	end
	player.AddBuff(0, 99, 11129, 1, 3)
	npc.SetCustomUnsigned1(Identity_Wizard.DigControl.ValueName.dwGhostNum, 1)
