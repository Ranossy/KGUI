---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_四月洛阳/PQ160_狗狗召人.lua
-- 更新时间:	2016/4/21 16:46:14
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_四月洛阳/PQ160_狗狗召人.lua",											-- 属性值1
		0											-- 属性值2
	);
...
local EXITTIME = 5 * 60 * GLOBAL.GAME_FPS
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	for i = 1, 4 do
		if not scene.GetNpcByNickName(npc.szName .. "_" .. i) then
			local npcNew = scene.CreateNpc(51182, npc.nX, npc.nY, npc.nZ, Random(256),EXITTIME , npc.szName .. "_" .. i)
			if npcNew then
				npc.Say(GetEditorString(10, 254), 2560, true)
				npcNew.JumpTo(player.nX, player.nY, player.nZ, 0, 48)
				npcNew.SetCustomInteger2(0, 160)
				npcNew.SetCustomUnsigned4(16, npc.dwID)
				player.SetTimer(48, "scripts/skill/Quest/ACT_四月洛阳/PQ160_狗狗召人.lua", npcNew.dwID, 0)
			end
			return
		end
	end
	--local nIndex = npc.GetCustomInteger4(8) + 1
	--if nIndex >= 10 then
	--npc.Say("汪汪汪！", 2560, true)
	--return
	--end
	--local npcNew = scene.GetNpcByNickName(npc.szName .. "_" .. nIndex)
	--if not npcNew then
	--npcNew = scene.CreateNpc(51182, npc.nX, npc.nY, npc.nZ, Random(256), 10 * 60 * GLOBAL.GAME_FPS, npc.szName .. "_" .. nIndex)
	--npc.SetCustomInteger4(8, nIndex)
	--end
	--if npcNew then
	--npc.Say("汪汪汪！", 2560, true)
	--npcNew.JumpTo(player.nX, player.nY, player.nZ, 0, 48)
...
function OnTimer(player, nParam1, nParam2)
	...
	npc.Say(GetEditorString(10, 249), 2560, true)
	npc.ModifyThreat(player.dwID, 1000)
end
