---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_四月洛阳/曹炎烈_旋风戟_母技能.lua
-- 更新时间:	2016/12/2 17:45:43
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_四月洛阳/曹炎烈_旋风戟_母技能.lua", --属性值1
		0															--属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
	...
	--skill.AddSlowCheckDestBuff(10622, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npcBoss then
		npcBoss.CastSkill(17181, 1)
		npcBoss.CastSkill(17182, 1)
	end
	--[[
	...
	local npcBOSS = scene.GetNpcByNickName("PQ163_caoyanlie")
	if npcBOSS then
		dwCasterID = npcBOSS.dwID
	end

	if not player.GetBuff(10622, 1) then
		player.PlaySfx(553, player.nX, player.nY, player.nZ + 500)
		player.CustomDamage(dwCasterID, 15761, 1, 1, 21000)
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", "你没有穿戴独孤九的秘制软甲，被曹炎烈的旋风戟造成了大量伤害。", 4)
		return
	end
	player.CustomDamage(dwCasterID, 15761, 1, 1, 2100)
--]]
end
