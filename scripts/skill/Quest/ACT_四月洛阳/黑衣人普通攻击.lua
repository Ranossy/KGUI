---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_四月洛阳/黑衣人普通攻击.lua
-- 更新时间:	2016/4/21 21:39:25
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_四月洛阳/黑衣人普通攻击.lua", --属性值1
		0															--属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcBOSS = scene.GetNpcByNickName("zaizhandishoubossP" .. player.dwID)
	if npcBOSS then
		dwCasterID = npcBOSS.dwID
	end
	
	if not player.GetBuff(10708, 1) then
		player.PlaySfx(553, player.nX, player.nY, player.nZ + 500)
		--player.Die()
		player.CustomDamage(dwCasterID, 15819, 1, 1, player.nMaxLife * 0.4)
	else
		player.PlaySfx(553, player.nX, player.nY, player.nZ + 500)
		--player.Die()
		player.CustomDamage(dwCasterID, 15819, 1, 1, Random(120,500))
