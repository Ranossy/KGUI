---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/苍云_射箭.lua
-- 更新时间:	2014/11/6 21:35:16
-- 更新用户:	zhangtianhui1
-- 脚本说明:
----------------------------------------------------------------------<
---------------------------------------------------------------------->
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/苍云_射箭.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(8544, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	skill.SetNormalCoolDown(1, 630);	--技能普通CD
	...
	skill.nMaxRadius = 3000 * LENGTH_BASE;		-- 技能施放的最大距离
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	local MuBiao = GetNpc(dwTargetID)
	local tSZname = {
		{"daocaoren1", 1},
		{"daocaoren2", 7},
		{"daocaoren3", 1},
	...
	if nTargetType == TARGET.NPC then
		for i = 1, #tSZname do
			if MuBiao then
				if MuBiao.szName == tSZname[i][1] then
					player.TurnTo(tSZname[i][2])
				end
			end
		end
	end
	local JTbuffCD = player.GetBuff(8546, 1)
	if not JTbuffCD then
		RemoteCallToClient(player.dwID, "SetMaxCameraDistance", 300)
		player.AddBuff(0, 99, 8545, 1, 1) --镜头恢复
		player.AddBuff(0, 99, 8546, 1, 1) --镜头恢复
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, nTarget)
	...
	local nQuestId_12645 = 12645
	local nQuestIndex_12645 = player.GetQuestIndex(nQuestId_12645)
	local nQuestPhase_12645 = player.GetQuestPhase(nQuestId_12645)
	local nQuestId_12647 = 12647
	local nQuestIndex_12647 = player.GetQuestIndex(nQuestId_12647)
	local nQuestPhase_12647 = player.GetQuestPhase(nQuestId_12647)
	local nQuestId_14973 = 14973
	local nQuestIndex_14973 = player.GetQuestIndex(nQuestId_14973)
	local nQuestPhase_14973 = player.GetQuestPhase(nQuestId_14973)
	
	if nQuestIndex_12645 then
		local nQuestValue_12645 = player.GetQuestValue(nQuestIndex_12645, 1)
		player.SetQuestValue(nQuestIndex_12645, 1, nQuestValue_12645 + 1)
		if nQuestValue_12645 == 2 then
	...
	if nQuestIndex_12647 then
		local nQuestValue_12647 = player.GetQuestValue(nQuestIndex_12647, 1)
		player.SetQuestValue(nQuestIndex_12647, 1, nQuestValue_12647 + 1)
		if nQuestValue_12647 == 2 then
			RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "CloseSniperPanel")
			RemoteCallToClient(player.dwID, "RestoreMaxCameraDistance")
			player.SetDynamicSkillGroup(0)
			player.CloseLonelyMode()
	...
	--勤修不辍强行插入
	local nQuestId_12705 = 12705
	local nQuestIndex_12705 = player.GetQuestIndex(nQuestId_12705)
	if nQuestIndex_12705 then
		local nQuestValue_12705 = player.GetQuestValue(nQuestIndex_12705, 1)
		player.SetQuestValue(nQuestIndex_12705, 1, nQuestValue_12705 + 1)
		if nQuestValue_12705 == 2 then
			RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "CloseSniperPanel")
			RemoteCallToClient(player.dwID, "RestoreMaxCameraDistance")
			player.SetDynamicSkillGroup(0)
			player.CloseLonelyMode()
	...
	if nQuestIndex_14973 then
		local nQuestValue_14973 = player.GetQuestValue(nQuestIndex_14973, 1) + 1
		player.SetQuestValue(nQuestIndex_14973, 1, nQuestValue_14973)
		if nQuestValue_14973 == 1 then
			RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "CloseSniperPanel")
			RemoteCallToClient(player.dwID, "RestoreMaxCameraDistance")
			player.SetDynamicSkillGroup(0)
			player.CloseLonelyMode()
