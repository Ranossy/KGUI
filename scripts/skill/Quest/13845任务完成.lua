---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/13845任务完成.lua
-- 更新时间:	2015/9/17 14:39:11
-- 更新用户:	zhangchennan
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/13845任务完成.lua", -- 属性值1
		0														-- 属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID)
	...
	if player then
		...
		if nQuestIndex and nQuestPhase == 1 then
			RemoteCallToClient(player.dwID, "OnPlayProtocolMovie", 150, false)
			player.SetTimer(180 * GLOBAL.GAME_FPS, "scripts/Map/五台山/event/狼牙大将定时器.lua", 0, 0)
			player.SetTimer(4 * GLOBAL.GAME_FPS, "scripts/Map/五台山/event/狼牙大将传送.lua", 0, 0)
			player.SetQuestValue(nQuestIndex, 0, 1)
			player.AddBuff(0, 99, 10091, 1)
			--player.RemoteCallToClient(player.dwID, "OnPlayProtocolMovie", 22, false)
			--player.AddBuff(1899, 1)
			--player.DelBuff(10091, 1)
		end
	end
end
