---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/长安内城_站岗.lua
-- 更新时间:	2013/9/10 23:54:58
-- 更新用户:	xutong3
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
Include("scripts/Map/长安内城/include/运送货物.lua")
tSkillData = 
{
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/长安内城_站岗.lua", -- 属性值1scripts/skill/Quest/长安内城_站岗.lua
		0														-- 属性值2
	);
...
--	skill.nAngleRange = 85;					-- 攻击范围的扇形角度范围
	skill.nAngleRange = 75;					-- 攻击范围的扇形角度范围105度
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player then
		if player.GetBuff(2587, 7) and not player.GetBuff(6506, 1) then
			local buff = player.GetBuff(6802, 1)
			if not buff or buff.nStackNum <= 1 then
				player.AddBuff(1, 1, 6802, 1, 3)
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(5, 9685), 3)
				return
			end
			npc.FireAIEvent(2003, 1, player.dwID)
			CreatYeZhiMei(player)
			player.AddAchievementCount(3868,1)
		end	
	end
end
