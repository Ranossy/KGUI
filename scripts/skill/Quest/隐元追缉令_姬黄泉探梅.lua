...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/隐元追缉令_姬黄泉探梅.lua",				-- 属性值1
		0														-- 属性值2
	);
...
local character = nil
	if IsPlayer(dwCharacterID) then
		...
		target = character.GetSelectCharacter()
		target.CastSkill(7570, 1, TARGET.PLAYER, character.dwID)
	else	
		character = GetNpc(dwCharacterID)
		local nTaregetType,nTaregetID = character.GetTarget()
		local playerA = GetPlayer(nTaregetID)
		playerA.CastSkill(7570, 1, TARGET.NPC, character.dwID)
	end
end
