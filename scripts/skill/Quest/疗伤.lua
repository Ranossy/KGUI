---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/疗伤.lua
-- 更新时间:	2013-4-2 10:37:21
-- 更新用户:	xiedixun1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddSlowCheckDestBuff(5527, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	local jing = GetNpc(dwCharacterID)
	local player = GetPlayer(dwSkillSrcID)
	local scene = player.GetScene()
	if jing then
		if jing.dwTemplateID == 17925 then
			local nQuestID = 8520
			jing.SayTo(player.dwID, GetEditorString(5, 2290))
	...
	if scene then
		scene.SetTimer(2 * GLOBAL.GAME_FPS, "scripts/skill/Quest/疗伤.lua", player.dwID, 0)
	end
end

function OnTimer(scene, nParam1, nParam2)
	local jing = scene.GetNpcByNickName("jing")
	local player = GetPlayer(nParam1)
	if jing and player then
		jing.SayTo(player.dwID, GetEditorString(4, 6663))
	end
end
