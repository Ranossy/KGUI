...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(5470, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
...
function Apply(dwCharacterID)
	...
	if player then
		...
		if nQuestIndex then
			...
			--  print("m=" .. m)
			if n < 5 then
				player.DelBuff(5470, 1)
				local scene = player.GetScene()
				local npc = scene.GetNpcByNickName("wlg_guizishou")
				if npc then
					local a = npc.GetCustomInteger4(7)
					npc.SetCustomInteger4(7, a + 1)
				end
				player.SetQuestValue(nQuestIndex, 0, n + 1)
				player.SetQuestValue(nQuestIndex, 1, 2)
				player.Talk(GetEditorString(5, 6591))
			end
		end
	end
end
