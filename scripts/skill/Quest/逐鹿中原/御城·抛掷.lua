---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/逐鹿中原/御城·抛掷.lua
-- 更新时间:	2014/2/13 11:13:20
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/逐鹿中原/御城·抛掷守将AOE.lua", -- 属性值1
		0														-- 属性值2
	);
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,
		ATTRIBUTE_TYPE.EXECUTE_SCRIPT,
		"skill/Quest/逐鹿中原/御城·抛掷.lua",
		0
	);
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 7495, 1);		-- 设置1号位Buff
	skill.BindBuff(2, 7496, 1);		-- 设置2号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.DoAction(dwCharacterID, 13200)
end
...
function UnApply(dwCharacterID)
	...
	--local buff = player.GetBuff(7496, 1)
	--if not buff then
	--	return
	--end
	--print(33333)
	--local npc = GetNpc(buff.dwSkillSrcID)
	--if not npc then
	--	return
	--end
	--
	--print(2222)
	--npc.PlaySfx(697, player.nX, player.nY, player.nZ)
	--npc.CastSkillXYZ(8836, 1, player.nX, player.nY, player.nZ)
	--player.DelBuff(7496, 1)
end
