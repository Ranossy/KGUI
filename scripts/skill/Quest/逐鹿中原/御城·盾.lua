---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/逐鹿中原/御城·盾.lua
-- 更新时间:	2014/2/13 10:17:25
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
    ...
    skill.AddAttribute(
		...
		"skill/Quest/逐鹿中原/御城·盾.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7491, 1);		-- 设置1号位Buff
	--skill.BindBuff(2, nBuffID, nBuffLevel);		-- 设置2号位Buff
	--skill.BindBuff(3, nBuffID, nBuffLevel);		-- 设置3号位Buff
	--skill.BindBuff(4, nBuffID, nBuffLevel);		-- 设置4号位Buff

	----------------- 设置Cool down --------------------------------------------
	skill.SetNormalCoolDown(1, 690);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nDir = 2 * math.pi * player.nFaceDirection / 256
	local dwDstX = math.floor(4 * 64 * math.cos(nDir) + player.nX)
	local dwDstY = math.floor(4 * 64 * math.sin(nDir) + player.nY)
	
	local nDir_R = 2 * math.pi * (256 / 4 + player.nFaceDirection) / 256
	--local nDir_L = 2 * math.pi * (256 * 3 / 4 + player.nFaceDirection) / 256
	for i = 1, 10 do
		local nDis = 10 - i * 2
		local nX = math.floor(nDis * 64 * math.cos(nDir_R) + player.nX)
		local nY = math.floor(nDis * 64 * math.sin(nDir_R) + player.nY)
		local npc = scene.CreateNpc(30627, nX, nY, player.nZ, player.nFaceDirection, 5 * GLOBAL.GAME_FPS)
		npc.SetForceID(player.nCamp == 1 and 50 or 49)
		npc.SetTongForceID(player.dwTongID)
	end
end
