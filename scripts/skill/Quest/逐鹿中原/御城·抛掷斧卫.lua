---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/逐鹿中原/御城·抛掷斧卫.lua
-- 更新时间:	2014/2/13 11:36:46
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function UnApply(dwCharacterID)
	...
	local buff = player.GetBuff(7499)
	if not buff then
		return
	end
	local npc = GetNpc(buff.nSkillSrcID)
	if not npc then
		return
	end
	npc.CastSkillXYZ(8837, 1, player.nX, player.nY, player.nZ)
	player.DelBuff(7499, 1)
end
