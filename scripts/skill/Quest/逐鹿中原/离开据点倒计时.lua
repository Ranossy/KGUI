---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/逐鹿中原/离开据点倒计时.lua
-- 更新时间:	2014/3/15 18:58:38
-- 更新用户:	lilin-kingsoft
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function OnRemove(dwCharacterID, dwBuffID, nBuffLevel, nLeftFrame)
	...
	local buff = player.GetBuff(CastleFight.BuffList.ProtectFlag.dwBuffID, 0)
	if buff then
		player.DelBuff(CastleFight.BuffList.ProtectFlag.dwBuffID, buff.nLevel)
	end
	
	buff = player.GetBuff(CastleFight.BuffList.Flag.dwBuffID, 0)
	if buff then
		player.DelBuff(CastleFight.BuffList.Flag.dwBuffID, buff.nLevel)
	end
end
