---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/逐鹿中原/拉镖车.lua
-- 更新时间:	2015/10/14 0:46:14
-- 更新用户:	lilin-kingsoft
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 140);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npc = scene.GetNpcByNickName("TongTradeHorse" .. player.dwTongID)
	if not npc then
		player.SendSystemMessage(GetEditorString(8, 6602))
		return
	end
	local dwDistance = CustomFunction.GetSpace3D(npc, player)
	if dwDistance > 10 * 64 then
		player.SendSystemMessage(GetEditorString(8, 6788))
		return
	end
	
	npc.SetPosition(player.nX, player.nY, player.nZ)
end
