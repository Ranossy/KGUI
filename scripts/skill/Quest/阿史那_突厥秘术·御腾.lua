...
function Apply(dwCharacterID)
	...
	local nDstX = 15 * 64 * math.cos(nDir) + npc.nX
	local nDstY = 15 * 64 * math.sin(nDir) + npc.nY
	
	local nIndex = 0
	for i = 1, Prince_AShiNa.Totem.nMaxCount do
		if not scene.GetNpcByNickName(Prince_AShiNa.Totem.szNickName .. i) then
			nIndex = i
		end
	end
	...
	local npc_Totem = scene.CreateNpc(Prince_AShiNa.Totem.dwTemplateID, nDstX, nDstY, npc.nZ, npc.nFaceDirection, -1, Prince_AShiNa.Totem.szNickName .. nIndex)
	if npc_Totem then
		npc_Totem.SetCustomUnsigned4(Prince_AShiNa.Totem.ValueName.dwBossID, npc.dwID)
	end
end
