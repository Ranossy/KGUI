...
tSkillData =
{
	{nDamageBase = 53846, nDamageRand = 0, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	  skill.BindBuff(1, 5839, 1);		-- 设置1号位Buff
	...
	--skill.nRectWidth = 0 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
	  skill.nAngleRange		= 114;					-- 攻击范围的扇形角度范围
...
function Apply(dwCharacterID)
	...
	local nBuff = player.GetBuff(5839, 1)
	if not nBuff then
		return
	end
	if nBuff.nStackNum == 5 then
		--player.nCurrentLife = player.nCurrentLife - 128205
		--player.DelGroupBuff(5839, 1)
		if not player.GetBuff(6133, 1) then
			player.AddBuff(0, 99, 6133, 1)
		end
	end
end
