---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/洛道_读家书.lua
-- 更新时间:	2014/1/8 14:35:34
-- 更新用户:	zhangzhong1-pc1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
-- 创建人   :  王洋	
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7348, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		player.AddBuff(0, 99, 7348, 1, 1)
		local buff = player.GetBuff(7348, 1)
		if buff.nStackNum == 1 then
			player.Talk(GetEditorString(6, 2420))
		elseif buff.nStackNum == 2 then
			player.Talk(GetEditorString(6, 2421))
		elseif buff.nStackNum == 3 then
			player.Talk(GetEditorString(6, 2422))
		elseif buff.nStackNum == 4 then
			player.Talk(GetEditorString(6, 2423))
			if player.GetItemAmount(ITEM_TABLE_TYPE.OTHER, 894) == 0 then
				player.SendSystemMessage(GetEditorString(3, 5137))
				return
			end
			local nQuestID = 1517;		--阅读家书
