---------------------------------------------------------------------->
-- 脚本名称:	F:/zhugan/client/scripts/skill/Quest/大唐监狱_绝空链.lua
-- 更新时间:	2012/12/26 14:46:13
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData = 
{
	{nDamageBase = 7043 * 0.8, nDamageRand = 7043 * 0.4, nCostMana = 0}, --level 1
...
function Apply(dwCharacterID)
	...
	local tbuff = target.GetBuff(5029, 1)
	if not tbuff then
		return
	end
	local n = tbuff.nStackNum
	if tbuff then
		for i = 1, n do
			target.DelBuff(5029, 1)
			target.AddBuff(19765, 80, 5030, 1, 20)
		end
	end
end
