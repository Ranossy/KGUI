---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/端午节活动/端午_龙舟冲撞.lua
-- 更新时间:	2017/5/23 16:38:31
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/端午节活动/端午_龙舟冲撞.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	skill.SetNormalCoolDown(1, 1212);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nBuff = player.GetBuff(11983, 1)
	if not nBuff then
		player.AddBuff(0, 99, 11992, 1, 5)
	elseif nBuff then
		player.AddBuff(0, 99, 11992, nBuff.nStackNum + 1, 5 + nBuff.nStackNum)
	end
end
