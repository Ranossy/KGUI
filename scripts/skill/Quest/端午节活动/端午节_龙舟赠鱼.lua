---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/端午节活动/端午节_龙舟赠鱼.lua
-- 更新时间:	2017/5/23 22:47:22
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/端午节活动/端午节_龙舟赠鱼.lua", -- 属性值1
		0											-- 属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(11958, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	local playerDest = GetPlayer(dwCharacterID)
	...
	local nBuff = player.GetBuff(11989, 1)
	if not nBuff then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(11, 6623), 4)
		return
	end
	player.DelBuff(11989,1)
	LongZhou_AddPoint(player, 24)
	LongZhou_AddPoint(playerDest, 10)
	LongZhou_AddUpgradepoint(playerDest, 40)
	RemoteCallToClient(playerDest.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(11, 6715), 2)
	RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(11, 6758), 2)
	player.PlaySfx(17, playerDest.nX, playerDest.nY, playerDest.nZ)
	player.PlaySfx(17, player.nX, player.nY, player.nZ)
end
