---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/端午节活动/端午节_鳄鱼之吻母技能.lua
-- 更新时间:	2015/5/15 16:59:27
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/端午节活动/端午节_鳄鱼之吻母技能.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nFaceDirection = npc.nFaceDirection
	local nX = npc.nX
	local nY = npc.nY
	local nZ = npc.nZ
-- 原始点X，原始点Y,原始点朝向，半径，偏移角度（0表示正前方，180表示正后方）
	local nNx, nNy, nNfaceDirection = GetPositionByAngleAndRadius(nX, nY, nFaceDirection, 64 * Random(15, 30), Random(315,405))
	local scene = npc.GetScene()
	if not scene.GetNpcByNickName("Eryu_paopao") then
		scene.CreateNpc(43091, nNx, nNy, nZ, nNfaceDirection, 7 * GLOBAL.GAME_FPS, "Eryu_paopao")
	end
	scene.SetTimer(16, "scripts/skill/Quest/端午节活动/端午节_鳄鱼之吻母技能.lua", npc.dwID, 0)
end
...
function OnTimer(scene, nParam1, nParam2)
	...
	if npc then
		local npcPaopao = scene.GetNpcByNickName("Eryu_paopao")
		if nParam2==0 then
			if npcPaopao then
				--npc.SetTarget(TARGET.NPC, npcPaopao.dwID)
				--print("释放鳄鱼之吻技能")
				npc.CastSkillXYZ(13862, 1, npcPaopao.nX, npcPaopao.nY, npcPaopao.nZ)
				scene.SetTimer(48, "scripts/skill/Quest/端午节活动/端午节_鳄鱼之吻母技能.lua", npc.dwID, 1)
			end
		else
			if npcPaopao then
				npc.PlaySfx(1128, npcPaopao.nX, npcPaopao.nY, npcPaopao.nZ)
				npcPaopao.CastSkill(13860, 1)
				npcPaopao.SetDisappearFrames(0.5 * GLOBAL.GAME_FPS)
			end
		end		
	end
end
