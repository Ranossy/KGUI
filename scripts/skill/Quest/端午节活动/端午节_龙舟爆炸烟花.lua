---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/端午节活动/端午节_龙舟爆炸烟花.lua
-- 更新时间:	2017/5/17 16:01:28
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/端午节活动/端午节_龙舟爆炸烟花.lua", -- 属性值1
		0											-- 属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(11958, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL) --游侠buff
	skill.AddSlowCheckDestBuff(11959, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		...
		player.PlaySfx(8785, player.nX - npcX, player.nY - npcY, player.nZ)
		player.SetTimer(1.5 * GLOBAL.GAME_FPS, "scripts/skill/Quest/端午节活动/端午节_龙舟爆炸烟花.lua", nX, nY)
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	local nY = nParam2
	local nZ = player.nZ
	player.PlaySfx(8786, nX, nY, nZ)
	player.PlaySfx(8778, nX, nY, nZ)
	local nBuff = player.GetBuff(11986, 1)
	if not nBuff then
		player.CastSkillXYZ(17668, 1, nX, nY, nZ)
		player.CastSkillXYZ(17772, 1, nX, nY, nZ)
	elseif nBuff then
		player.CastSkillXYZ(17668, nBuff.nStackNum + 1, nX, nY, nZ)
		player.CastSkillXYZ(17772, nBuff.nStackNum + 1, nX, nY, nZ)
	end
end
