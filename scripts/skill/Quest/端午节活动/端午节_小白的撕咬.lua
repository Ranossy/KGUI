---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/端午节活动/端午节_小白的撕咬.lua
-- 更新时间:	2015/6/12 10:13:30
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		local npc = scene.GetNpcByNickName("duanwu_XiaoBai")
		if npc then
			dwCasterID = npc.dwID
		end
		player.CustomDamage(dwCasterID, 13975, 1, 0, player.nMaxLife * 0.2)
	end
end
