---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/端午节活动/端午节_龙舟主炮对玩家.lua
-- 更新时间:	2017/5/25 20:02:13
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/端午节活动/端午节_龙舟主炮对玩家.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(11770, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(12007, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	skill.AddSlowCheckDestBuff(244, 1, BUFF_COMPARE_FLAG.EQUAL, 18, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	skill.nHeight = 5 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
	--skill.nRectWidth = 4 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
	skill.nAngleRange = 256 * 60 / 360;					-- 攻击范围的扇形角度范围
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local playerSelf = GetPlayer(dwSkillSrcID)
	...
	local tYunzhaoPosition = {
		[213] = {40709, 37035, 1057920},
		[16] = {43271, 42606, 1056256},
		[159] = {72103, 34639, 1094144},
	}
	local nSkillLevel = 0
	local nLevelBuff = playerSelf.GetBuff(11985, 1)
	if not nLevelBuff then
		nSkillLevel = 1
	elseif nLevelBuff then
		nSkillLevel = nLevelBuff.nStackNum + 1
	end
	local nbuff = player.GetBuff(12006, 1)
	if nbuff then
		nbuff.nCustomValue = math.max(0, nbuff.nCustomValue - (10 * nSkillLevel + 70))
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "GeneralProgressBar_Create", "longzhou3", 19, GetEditorString(11, 6605), GetEditorString(11, 6687), nbuff.nCustomValue, 1000)
		if nbuff.nCustomValue == 0 then
			local yxBuff = player.GetBuff(244, 17)
			local yzBuff = player.GetBuff(244, 18)
			if not yxBuff and not yzBuff then
				return
			end
			if yxBuff then
				player.DelBuff(244, 17)
			end
			if yzBuff then
				player.DelBuff(244, 18)
			end
			RemoteCallToClient(player.dwID, "On_Trial_Screen_Black")
			player.SetPosition(tYunzhaoPosition[scene.dwMapID][1],tYunzhaoPosition[scene.dwMapID][2],tYunzhaoPosition[scene.dwMapID][3])
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(11, 6728) .. playerSelf.szName .. GetEditorString(11, 6729), 4)
			RemoteCallToClient(playerSelf.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", "你击沉了"..player.szName.."的龙舟，获得积分奖励", 4)
			player.AddBuff(0,99,12004,1,15)
			LongZhou_AddPoint(playerSelf, 40)
			LongZhou_AddUpgradepoint(playerSelf, 160)
			if not playerSelf.IsAchievementAcquired(5831) then
				playerSelf.AddAchievementCount(5831, 1)
			end
		end
	end
end
