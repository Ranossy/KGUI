...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local Buff = player.GetBuff(5198, 1)
	local nQuestID = 8569
	local nQuestPhase = player.GetQuestPhase(nQuestID)
	local nQuestIndex = player.GetQuestIndex(nQuestID)
	if not Buff then
		player.AddBuff(0,0,5198,1,1)
		return
	end
	
	if Buff.nStackNum < 11 then
		player.AddBuff(0,0,5198,1,1)
	else
		if nQuestPhase == 1 then
			local nValue = player.GetQuestValue(nQuestIndex, 0)
			local hudie = GetNpc(dwCharacterID)
			player.SetQuestValue(nQuestIndex, 0, nValue + 1)
			if hudie.dwTemplateID == 18415 then
				hudie.SetDisappearFrames(1, 1)
			end
		end
	end
end
