---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/长安内城_锻打.lua
-- 更新时间:	2013/8/2 15:50:26
-- 更新用户:	zhouyixiao1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 629);	--技能普通CD
	skill.SetCheckCoolDown(1, 628)	
...
function Apply(dwCharacterID)
	...
	if player then
		...
		local nvalue = player.GetQuestValue(nQuestIndex, 6)
		local nvalue1 = player.GetQuestValue(nQuestIndex1, 6)
		if nvalue ~= 1 and nvalue ~= 1 then
			player.CastSkill(7972, 1)
		else
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(5, 8481), 8)
			return
		end
		if nQuestIndex then
			player.SetQuestValue(nQuestIndex, 6, 1) --避免玩家ESC后无限创建NPC
		end
		if nQuestIndex1 then
			player.SetQuestValue(nQuestIndex1, 6, 1) --避免玩家ESC后无限创建NPC
		end
	else
		return
	end
	...
	for i = 1, #tjueseList do
		...
		if jueseID then
			...
			if Member then --遍历队伍内的所有成员均在距离范围内才放剑
				...
				if bQuestIndex then
					valueJs = Member.GetQuestValue(bQuestIndex, 0) --结束时的任务变量
				end
				if bQuestIndex1 then
					valueJs = Member.GetQuestValue(bQuestIndex1, 0) --结束时的任务变量
				end
				if buff then
					if valueJs >= 9 or jieshu >= 9 then
						...
						if nNum >= 5000 then
							...
							local NPCjian = scene.CreateNpc(25785, tZuoBiao[scene.dwMapID][1], tZuoBiao[scene.dwMapID][2], tZuoBiao[scene.dwMapID][3], 191, 20 * GLOBAL.GAME_FPS, Member.dwID .. "jianshen" .. shuzhi, Member.dwID)
							buff.nCustomValue = shuzhi
						else
							local shuzhi = buff.nCustomValue + 1
							local NPCjian = scene.CreateNpc(26547, tZuoBiao[scene.dwMapID][1], tZuoBiao[scene.dwMapID][2], tZuoBiao[scene.dwMapID][3], 191, 20 * GLOBAL.GAME_FPS, Member.dwID .. "jianshen2" .. shuzhi, Member.dwID)
							buff.nCustomValue = shuzhi
							Member.AddBuff(0, 99, 6679, 1, 120)
							local buffhuang = Member.GetBuff(6679, 1) 
							buffhuang.nCustomValue = shuzhi
						end
					end
				end
			end
