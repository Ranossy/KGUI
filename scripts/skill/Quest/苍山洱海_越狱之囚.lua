...
function GetSkillLevelData(skill)
    ...
    skill.nTargetCountLimit	= 99;				-- 技能作用目标数量限制,(小于0 代表目标数量不限制) 
...
function Apply(dwCharacterID)
	...
	local nQUEST_ID = 7761
	local dwQuestIndex = tplayer.GetQuestIndex(nQUEST_ID)
	local dwQuestPhase = tplayer.GetQuestPhase(nQUEST_ID)
	
	if dwQuestIndex and dwQuestPhase == 1 then
		tplayer.SetQuestValue(dwQuestIndex, 0, 1)
		return
	end
end
