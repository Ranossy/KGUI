---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/长安内城_射箭.lua
-- 更新时间:	2013/8/6 16:35:09
-- 更新用户:	zhouyixiao1
-- 脚本说明:	
----------------------------------------------------------------------<
---------------------------------------------------------------------->
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/长安内城_射箭.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(6478, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
		...
		for i = 1, #tSZname do
			if MuBiao then
				if MuBiao.szName == tSZname[i][1] then
					...
					if not player.GetBuff(6827, 1) then
						player.AddBuff(0, 99, 6827, 1, 8)
						player.AddBuff(0, 99, 6828, 1, 1)
					end	
				end
			end
		end
	end
	local JTbuffCD = player.GetBuff(6485, 1)
	if not JTbuffCD then
		RemoteCallToClient(player.dwID, "SetMaxCameraDistance", 300)
		player.AddBuff(0, 99, 6484, 1, 1) --镜头恢复
		player.AddBuff(0, 99, 6485, 1, 1) --镜头恢复
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, nTarget)
	...
	player.AddBuff(0, 99, 6829, 1, 1)
	local buffcj2 = player.GetBuff(6829, 1)
	if buffcj2.nStackNum >= 12 then
		if player.GetAchievementCount(3866) < 1 then 
			player.AddAchievementCount(3866, 1) --连环夺命
		end 			
	end
	if nQuestIndex and nQuestPhase == 1 then
		local value = player.GetQuestValue(nQuestIndex, 1)
		player.SetQuestValue(nQuestIndex, 1, value + 1) --任务计数用1号任务变量位
		if  player.GetBuff(6827, 1) then
			if player.GetBuff(6828, 1) then
				player.DelBuff(6827, 1)
				local buffcj = player.GetBuff(6828, 1)
				if buffcj.nStackNum >= 6 then
					if player.GetAchievementCount(3865) < 1 then 
						player.AddAchievementCount(3865, 1) --箭无虚发
					end 			
				end
			end
		end
	...
	if nQuestIndex1 and nQuestPhase1 == 1 then
		...
		player.SetQuestValue(nQuestIndex1, 1, value + 1) --任务计数用1号任务变量位
		if  player.GetBuff(6827, 1) then
			if player.GetBuff(6828, 1) then
				player.DelBuff(6827, 1)
				local buffcj = player.GetBuff(6828, 1)
	...
	if nQuestIndex and nQuestPhase == 2 then
		if not player.GetBuff(6801, 1) then
			player.AddBuff(0, 99, 6801, 1, 60)
		end
		local valueJS = player.GetBuff(6801, 1)
		valueJS.nCustomValue = valueJS.nCustomValue + 1
		local valueJL2 = valueJS.nCustomValue
		if valueJL2 > 0 then
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(5, 8076), 2)
		end
	end
	if nQuestIndex1 and nQuestPhase1 == 2 then
		if not player.GetBuff(6801, 1) then
			player.AddBuff(0, 99, 6801, 1, 60)
		end
		local valueJS = player.GetBuff(6801, 1)
