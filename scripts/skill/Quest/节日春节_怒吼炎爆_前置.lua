---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/节日春节_怒吼炎爆_前置.lua
-- 更新时间:	2013/12/27 11:17:12
-- 更新用户:	zhouyixiao1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/节日春节_怒吼炎爆_前置.lua", -- 属性值1
		0														-- 属性值2
	);
...
--	npc.AddBuff(0, 99, 7326, 1, 5)  --用于落地后放击飞效果的技能
	npc.AddBuff(0, 99, 7328, 1, 3)  --用在等小白兔表现出来后喷火球
	local tZuoBiao = CalPoint(npc.nX, npc.nY, 22 * 64, npc.nFaceDirection, 30 / 360 * 256)
	for i = 1, 3 do
		local SZbaitu = scene.GetNpcByNickName(tZuoBiao[i][3])
		if not SZbaitu then
			local NPCbaitu = scene.CreateNpc(27599, tZuoBiao[i][1], tZuoBiao[i][2], npc.nZ, 1, 160, tZuoBiao[i][3])
			if NPCbaitu then
				NPCbaitu.AddBuff(0, 99, 7327, 1, 10)
			end	
		end
	end
end

function CalPoint(nx, ny, radius, dir, angle)
	--参数分别表示 初始点x，初始点y，半径，初始面向，总角的角度
	return {
		{nx + radius * math.cos((dir-angle) / 128 * math.pi), ny + radius * math.sin((dir-angle) / 128 * math.pi), "CJNS_baitu1"}, 
		{nx + radius * math.cos((dir) / 128 * math.pi), ny + radius * math.sin((dir) / 128 * math.pi), "CJNS_baitu2"},
		{nx + radius * math.cos((dir+angle) / 128 * math.pi), ny + radius * math.sin((dir+angle) / 128 * math.pi), "CJNS_baitu3"}, 	
		}
end
