---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/节日春节_浮空反制.lua
-- 更新时间:	2013/12/30 17:36:18
-- 更新用户:	zhouyixiao1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(7332, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	--skill.AddSlowCheckDestBuff(6478, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	skill.AddSlowCheckDestBuff(7303, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if player.GetBuff(7332, 1) then
		player.TurnToCharacter(npc.dwID)
		player.AddBuff(0,99,7340,1,1)
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.PlayCameraAnimation(101, 0)
	RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "QTEPanel_HitNotify", 8692)
	player.DelBuff(7332,1)
	player.AddBuff(0, 99, 7351, 1, 119) --成功释放技能后有资格拾取残骸和奖励
end
