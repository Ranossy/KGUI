---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_峰值201611/安禄山魂魄_两界之门.lua
-- 更新时间:	2016/11/19 21:17:37
-- 更新用户:	zhengfeng5
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_峰值201611/安禄山魂魄_两界之门.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
	...
	skill.nHeight = 35 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	player.Talk(GetEditorString(11, 2062), 5)
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.Talk(GetEditorString(11, 1983), 5)
	...
	local nY = player.nY
	local nZ = player.nZ
	local szName = "ACT_FZ1221_csm"
-- 原始点X，原始点Y,原始点朝向，半径，偏移角度（0表示正前方，180表示正后方）
	local nCurrentTime = GetCurrentTime()
	local nEndTime = DateToTime(2016, 11, 22, 3, 59, 59)
	local nTime = nEndTime - nCurrentTime
	if nTime < 60 then
		nTime = 60
	end
	for i = 1, 10 do
		if not scene.GetNpcByNickName(szName .. i) then
			local nNx, nNy, nNfaceDirection = GetPositionByAngleAndRadius(nX, nY, nFaceDirection, 64 * 8, 0)
			local nNx1, nNy1, nNfaceDnNfaceDirection1irection1 = GetPositionByAngleAndRadius(nX, nY, nFaceDirection, 64 * 8, 180)
			local npc1 = scene.CreateNpc(55414, nNx, nNy, nZ + 400, nFaceDirection, GLOBAL.GAME_FPS * nTime, szName .. i)

			scene.UnbindMapMarkByNpc(npc1.dwID)
			local dwMarkID = scene.CreateMapMark(339, false, 0, 0, 0)
			scene.MapMarkBindNpc(dwMarkID, npc1.dwID)

			szName = "ACT_FZ1221_csm_lsj"
			npc1 = scene.CreateNpc(55415, nNx1, nNy1, nZ + 400, nNfaceDirection1, -1, szName .. i, 0, 0, 2)
			scene.UnbindMapMarkByNpc(npc1.dwID)
			local dwMarkID = scene.CreateMapMark(339, false, 0, 0, 0)
			scene.MapMarkBindNpc(dwMarkID, npc1.dwID)
			local nNx2, nNy2, nNfaceDnNfaceDirection1irection2 = GetPositionByAngleAndRadius(nX, nY, nFaceDirection, 64 * 16, 180)
				local yuanmingya = scene.CreateNpc(55443, nNx2, nNy2, nZ + 400, nFaceDirection, GLOBAL.GAME_FPS * nTime,nil,0, 0, 2)
			break
		end
	end
end
