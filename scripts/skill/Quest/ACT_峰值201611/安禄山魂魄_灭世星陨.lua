---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_峰值201611/安禄山魂魄_灭世星陨.lua
-- 更新时间:	2016/11/20 15:44:46
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_峰值201611/安禄山魂魄_灭世星陨.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	--player.Talk("你们还记得被两百个小铁支配的恐惧吗？！", 5)
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.Talk(GetEditorString(11, 1977), 5)
	...
	local nY = player.nY
	local nZ = player.nZ
	local szName = "ACT_FZ1221_msxy"
-- 原始点X，原始点Y,原始点朝向，半径，偏移角度（0表示正前方，180表示正后方）
	for i = 1, 5 do
		if not scene.GetNpcByNickName(szName .. i) then
			local nNx, nNy, nNfaceDirection = GetPositionByAngleAndRadius(nX, nY, nFaceDirection, 64 * Random(5, 15), 360 / 5 * i - 1)
			if player.dwIdentityVisiableID == 0 then
				scene.CreateNpc(55410, nNx, nNy, nZ + 400, nFaceDirection, GLOBAL.GAME_FPS * 20, szName .. i)
			else
				local npc1 = scene.CreateNpc(55410, nNx, nNy, nZ + 400, nFaceDirection, GLOBAL.GAME_FPS * 20, szName .. i, 0, 0, player.dwIdentityVisiableID)
				RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "rlcmd", "set character draw after post render " .. npc1.dwID .. " " .. 1)	-- 改变里世界特效颜色白名单  第一个是dwID，第二个参数，打开的话写1，关闭写0
