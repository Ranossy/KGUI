---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_峰值201611/安禄山_魂魄_进出里世界.lua
-- 更新时间:	2016/11/17 16:41:01
-- 更新用户:	zhengfeng5
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	skill.AddAttribute(
		...
		"skill/Quest/ACT_峰值201611/安禄山_魂魄_进出里世界.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	player.Talk(GetEditorString(11, 1987), 5)
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.Talk(GetEditorString(11, 1988), 5)
...
-- 进入里世界前的一系列判定。
		...
		--local nIndex = tList[player.GetMapID()] or 1
		local nIndex = 2
		player.GetPlayerIdentityManager().CloseIdentity()
		player.SendSystemMessage(GetEditorString(11, 1944))
		...
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "rlcmd", "set character draw after post render " .. player.dwID .. " ".. 1)	-- 改变里世界特效颜色白名单  第一个是dwID，第二个参数，打开的话写1，关闭写0
		--有宠物则删除
		...
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "rlcmd", "set character draw after post render " .. player.dwID .. " ".. 0)	-- 改变里世界特效颜色白名单  第一个是dwID，第二个参数，打开的话写1，关闭写0
