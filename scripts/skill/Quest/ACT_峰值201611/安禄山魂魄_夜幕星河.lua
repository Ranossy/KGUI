---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_峰值201611/安禄山魂魄_夜幕星河.lua
-- 更新时间:	2016/11/19 22:02:11
-- 更新用户:	zhengfeng5
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_峰值201611/安禄山魂魄_夜幕星河.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
	...
	skill.SetNormalCoolDown(1, 1172);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	player.Talk(GetEditorString(11, 1975), 5)
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.Talk(GetEditorString(11, 1958), 5)
	...
	local nY = player.nY
	local nZ = player.nZ
	local szName = "ACT_FZ1221_ymxh"
-- 原始点X，原始点Y,原始点朝向，半径，偏移角度（0表示正前方，180表示正后方）
	for i = 1, 4 do
		local nNx, nNy, nNfaceDirection = GetPositionByAngleAndRadius(nX, nY, nFaceDirection, 64 * Random(10, 20), 90 * i - 1)
		if player.dwIdentityVisiableID == 0 then
			scene.CreateNpc(55409, nNx, nNy, nZ + 400, nFaceDirection, GLOBAL.GAME_FPS * 20, szName .. i)
		else
			local npc1 = scene.CreateNpc(55409, nNx, nNy, nZ + 400, nFaceDirection, GLOBAL.GAME_FPS * 30, nil, 0, 0, player.dwIdentityVisiableID)
