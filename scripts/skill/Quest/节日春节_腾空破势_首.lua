---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/节日春节_腾空破势_首.lua
-- 更新时间:	2013/12/30 11:56:13
-- 更新用户:	zhouyixiao1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
	...
	if player.nMoveState == 27 then
		if player then
			player.AddBuff(0,99,7338,1) --锁操作
			player.DoAction(dwCharacterID, 12112) --冲上天空的动作
			player.AddBuff(0, 99, 7330, 1, 8)  --反制机会BUFF
			player.SetDynamicSkillGroup(106)  --为反制做准备屏蔽技能栏
		end	
	end
end
