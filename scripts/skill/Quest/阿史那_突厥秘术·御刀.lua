...
Include("scripts/Map/野外首领/枫华谷·战乱/include/阿史那_Data.lh")
...
--[[	local npc = GetNpc(dwCharacterID)
	...
	if not scene then
		...
		local npcYb = scene.CreateNpc(18403, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
		if npcYb then
			npcYb.RunTo(nX, nY)
		end--]]
	...
	local nDir = 2 * math.pi * npc.nFaceDirection / 256
	local dwStartX = math.floor(4 * 64 * math.cos(nDir) + npc.nX)
	local dwStartY = math.floor(4 * 64 * math.sin(nDir) + npc.nY)
	local dwDstX = math.floor(35 * 64 * math.cos(nDir) + npc.nX)
	local dwDstY = math.floor(35 * 64 * math.sin(nDir) + npc.nY)
	
	local npc_Knife = scene.CreateNpc(Prince_AShiNa.Knife.dwTemplateID, dwStartX, dwStartY, npc.nZ, npc.nFaceDirection, -1)
	if npc_Knife then
		npc_Knife.SetCustomUnsigned4(Prince_AShiNa.Knife.ValueName.dwStartX, dwStartX)
		npc_Knife.SetCustomUnsigned4(Prince_AShiNa.Knife.ValueName.dwStartY, dwStartY)
		npc_Knife.SetCustomUnsigned4(Prince_AShiNa.Knife.ValueName.dwDstX, dwDstX)
		npc_Knife.SetCustomUnsigned4(Prince_AShiNa.Knife.ValueName.dwDstY, dwDstY)
		npc_Knife.SetCustomUnsigned4(Prince_AShiNa.Knife.ValueName.dwBossID, npc.dwID)
		npc_Knife.RunTo(dwDstX, dwDstY)
	end
