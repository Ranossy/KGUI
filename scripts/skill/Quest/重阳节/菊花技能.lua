---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/重阳节/菊花技能.lua
-- 更新时间:	2014/9/11 17:22:22
-- 更新用户:	renjei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 8223, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcId)
	...
	local npcBossBuff1 = npcBoss.GetBuff(8235, 1)      ---防御层数的buff
	if npcBossBuff1 then
		local npcBossBuffn = npcBossBuff1.nStackNum
		if npcBossBuffn then
			npcBoss.AddBuff(0, 99, 8238, 1)           --- 打boss的伤害buff
			npcBoss.DelBuff(8235,1)                -----没打一次删除一层
		end
	else
		npcBoss.AddBuff(0, 99, 8238, 1)
	end
	return
end
