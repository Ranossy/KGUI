---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/重阳节/节日_重阳boss吼叫技能.lua
-- 更新时间:	2014/9/2 16:49:50
-- 更新用户:	renjei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/重阳节/节日_重阳boss吼叫技能.lua",
		0
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 8236, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcId)
	...
	for i = 1,10 do
		npc.AddBuff(0, 99, 8235, 1)
	end
	local nCount = npc.GetBuff(8235,1).nStackNum
		if nCount and nCount == 30 then
			npc.AddBuff(0,99,8301,1)                     -----boss加速buff
		end
	playerT.CustomDamage(npc.dwID, 13032, 1, 0, playerT.nMaxLife * 0.05)
end
