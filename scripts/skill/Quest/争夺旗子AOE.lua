---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/争夺旗子AOE.lua
-- 更新时间:	2017/1/13 21:33:09
-- 更新用户:	linjiajia2
-- 脚本说明:	九宫
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nIndex = npc.GetCustomInteger1(0)
	player.AddBuff(0, 99, 11623, 1)
	local buff = player.GetBuff(11623, 1)
	if buff then
		buff.nCustomValue = nIndex
	end
	--[[local nOldValue = 0
	local buff = player.GetBuff(11623, 1)
	if buff then
		nOldValue = buff.nCustomValue
	else
		player.AddBuff(0, 99, 11623, 1)
		buff = player.GetBuff(11623, 1)
	end

	if buff and CustomFunction.GetValueByBit(buff.nCustomValue, nIndex) ~= 1 then
		buff.nCustomValue = nOldValue + 2 ^ nIndex
	end]]
end
