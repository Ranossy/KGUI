---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/长安内城_轻锤后续.lua
-- 更新时间:	2013/8/30 10:12:14
-- 更新用户:	zhouyixiao1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/长安内城_轻锤后续.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	skill.SetNormalCoolDown(1, 637);	--技能普通CD
...
function Apply(dwCharacterID)
	...
	if nQuestIndex then
		player.SetQuestValue(nQuestIndex, 5, 0)
	end
	if nQuestIndex1 then
		player.SetQuestValue(nQuestIndex1, 5, 0)
	end
	local tZuoBiao = {
		...
		if jueseID then
			...
			if Member then --遍历队伍内的所有成员均在距离范围内才放剑
				if not Member.GetBuff(6831, 1) then
					local buff = Member.GetBuff(6679, 1)
					...
					if buff then
						...
						local szNPCjian = scene.GetNpcByNickName(Member.dwID .. "jianshen2" .. shuzhi)
						if szNPCjian then
					...
					if Member.GetBuff(6679, 1) then			
						if Member.GetBuff(6475, 1) then
							...
							local valueJs = 0
							if nQuestIndex then
								local value = Member.GetQuestValue(nQuestIndex, 0)
								if value then
							...
							if nQuestIndex1 then
								local value = Member.GetQuestValue(nQuestIndex1, 0)
								if value then
							...
							if valueJs >= 10 or jieshu >= 10 then
								...
								if buff then
									...
									--	scene.CreateNpc(26608, tZuoBiao[scene.dwMapID][1], tZuoBiao[scene.dwMapID][2], tZuoBiao[scene.dwMapID][3], 191, 10 * GLOBAL.GAME_FPS, Member.dwID .. "wancheng" .. shuzhi, Member.dwID)
						...
						local buff = player.GetBuff(6679, 1)
						if buff then
							local shuzhi = buff.nCustomValue - 1
