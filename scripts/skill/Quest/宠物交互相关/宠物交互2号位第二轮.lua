---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/宠物交互相关/宠物交互2号位第二轮.lua
-- 更新时间:	2015/10/14 17:35:00
-- 更新用户:	FENGMENGJIE
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
Include("scripts/RemoteFromClient/On_FellowPet.lua")
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/宠物交互相关/宠物交互2号位第二轮.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		for i = 1, 99 do
			local buff = player.GetBuff(10071, i)
			if buff then
				nActionIndex = GetFellowPetActionSkillInfo(13799, i)
			end
		end
		if nActionIndex == 0 then
			return
		end
		--判断当前宠物是否在恐惧中
		if Identity_YuShou.Get_Fear(player, npc) > 0 then
			return
		end
		player.CastSkill(16024, 1)--放检测同类技能
		On_FellowPet_AnimalAction(player, 0, nActionIndex)
	end
end
