---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴山大草原_摔跤比试_玩家先手.lua
-- 更新时间:	2015/10/22 21:56:32
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/阴山大草原_摔跤比试_玩家先手.lua",											-- 属性值1
		0											-- 属性值2
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 10190, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nAPCount = math.min(math.floor(player.nPhysicsOvercome / 50), 100)
	--print(11111,nAPCount)
	if nAPCount > 0 then
		for i = 1, nAPCount do
			player.AddBuff(0, 99, 10178, 1, 310)
		end
	end
	local nTPCount = math.min(math.floor(player.nTherapyPower / 100), 100)	
	--print(22222,nTPCount)
	if nTPCount > 0 then
		for i = 1, nTPCount do
			player.AddBuff(0, 99, 10179, 1, 310)
		end
	end
	player.SetDynamicSkillGroup(200)
	if not player.GetBuff(9318, 1) then
		player.AddBuff(0, 99, 9318, 1, 310)
	end
	if not player.GetBuff(10159, 1) then
		player.AddBuff(0, 99, 10159, 1, 310)
	end
	if not npc.GetBuff(10147, 1) then
		npc.AddBuff(0, 99, 10147, 1, 310)
	end
	if not npc.GetBuff(10159, 2) then
		npc.AddBuff(0, 99, 10159, 2, 310)
	end
	if not player.GetBuff(3448, 1) then
		player.AddBuff(0, 99, 3448, 1, 11)
	end
	if not player.GetBuff(7913, 1) then
		player.AddBuff(0, 99, 7913, 1, 310)
	end
end
