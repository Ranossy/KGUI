---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴山大草原_喂食_紫花苜蓿.lua
-- 更新时间:	2015/8/18 20:39:01
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
local nPQID = 42
local nAddPQValue = 1
local nAddPlayerValue = 2
local nBuffDel = 3	--DEBUFF最大消除层数
local nDoubleAward = 1.5	--奖励倍数
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/阴山大草原_喂食_紫花苜蓿.lua", -- 属性值1
		0											-- 属性值2
	);
	--[[
   	skill.AddAttribute(
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(9527, 1, 1, 1, 1);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(9543, 1, 1, 1, 1);		-- 需求目标Buff
	...
	skill.SetNormalCoolDown(1, 926);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = player.GetBuff(9527, 1)
	if not buff then
		return
	end
	local npcTarget = GetNpc(dwCharacterID)
	local nBuffID1 = 9344
	local nBuffID2 = 9543 -- 隐形BUFF
	if npcTarget then
		local buff1 = npcTarget.GetBuff(nBuffID1, 1)
		local buff2 = npcTarget.GetBuff(nBuffID2, 1)
		local nCount = math.min(buff1.nStackNum, nBuffDel)
		if not buff1 then
			RemoteCallToClient(player.dwID, "OnSendSystemAnnounce", GetEditorString(8, 507), "red")
			return
		else
			local nDouble = nDoubleAward
			local buffMark = npcTarget.GetBuffByOwner(9468, 1, player.dwID)
			if buffMark and buffMark.dwSkillSrcID == player.dwID then
				nDouble = 1
			end
			if player.GetItemAmount(ITEM_TABLE_TYPE.OTHER, 17158) >= 1 then
				npcTarget.AddBuff(player.dwID, player.nLevel, 9468, 1, 30)
				player.CostItem(ITEM_TABLE_TYPE.OTHER, 17158, 1)
				player.DelBuff(9527, 1)
				CommonPQ_AddPlayerPQValueTips(player, nPQID, math.ceil(nAddPlayerValue * nCount * nDouble))
				CommonPQ_AddValue(nPQID, 1, math.ceil(nAddPQValue * nCount), scene)
				if nDouble == nDoubleAward then
					RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(8, 9862), 4)
				end
			elseif player.GetItemAmount(ITEM_TABLE_TYPE.OTHER, 17031) >= 1 then
				npcTarget.AddBuff(player.dwID, player.nLevel, 9468, 1, 30)
				player.CostItem(ITEM_TABLE_TYPE.OTHER, 17031, 1)
				player.DelBuff(9527, 1)
				CommonPQ_AddPlayerPQValueTips(player, nPQID, math.ceil(nAddPlayerValue * nCount * nDouble))
				CommonPQ_AddValue(nPQID, 1, math.ceil(nAddPQValue * nCount), scene)
				if nDouble == nDoubleAward then
				...
				RemoteCallToClient(player.dwID, "OnSendSystemAnnounce", GetEditorString(8, 8685), "red")
				return
			end
			for i = 1, nCount do
				if npcTarget.GetBuff(nBuffID1, 1) then					
					npcTarget.DelBuff(nBuffID1, 1)			
				end
			end
			if not npcTarget.GetBuff(nBuffID1, 1) then
				npcTarget.DelBuff(nBuffID2, 1)
			end
		end
	end
end
