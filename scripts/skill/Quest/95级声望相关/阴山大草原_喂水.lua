---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴山大草原_喂水.lua
-- 更新时间:	2015/10/24 21:26:59
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/阴山大草原_喂水.lua", -- 属性值1
		0											-- 属性值2
	);
	--[[
   	skill.AddAttribute(
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(9528, 1, 1, 1, 1);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(9542, 1, 1, 1, 1);		-- 需求目标Buff
	...
	skill.SetNormalCoolDown(1, 927);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = player.GetBuff(9528, 1)
	if not buff then
		return
	end
	local npcTarget = GetNpc(dwCharacterID)
	local nBuffID1 = 9345
	local nBuffID2 = 9542 -- 隐形BUFF
	if npcTarget then
		local buff1 = npcTarget.GetBuff(nBuffID1, 1)
		local buff2 = npcTarget.GetBuff(nBuffID2, 1)
		if not buff2 then
			RemoteCallToClient(player.dwID, "OnSendSystemAnnounce", GetEditorString(8, 852), "red")
			return
		else
			...
			player.DelBuff(9528, 1)
			player.CostItemInAllPackage(5, 23978, 1)
