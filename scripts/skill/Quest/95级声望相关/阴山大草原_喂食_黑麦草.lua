---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴山大草原_喂食_黑麦草.lua
-- 更新时间:	2015/10/24 21:25:39
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/阴山大草原_喂食_黑麦草.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(9525, 1, 1, 1, 1);		-- 需求自身Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = player.GetBuff(9524, 1)
	if not buff then
		return
	end
	...
	if npcTarget then
		...
		if not buff1 then
			...
			player.DelBuff(9525, 1)
			player.CostItemInAllPackage(5, 23979, 1)
			CommonPQ_AddPlayerPQValueTips(player, nPQID, math.ceil(nAddPlayerValue * nDouble))
			CommonPQ_AddValue(nPQID, 1, math.ceil(nAddPQValue), scene)
