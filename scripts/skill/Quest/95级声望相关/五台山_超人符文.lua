---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/五台山_超人符文.lua
-- 更新时间:	2015/8/12 10:42:07
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/五台山_超人符文.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = player.GetBuff(9474, 1)
	-- buff持续时间大于2秒则不再加buff
	if buff and buff.nLeftActiveCount > 2 then
		return 
	end
	if player.GetBuff(9471, 1) then	--佛像
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(8, 9507), 4)
		player.AddBuff(player.dwID, player.nLevel, 9474, 1, 2)
	elseif player.GetBuff(9470, 1) or player.GetBuff(9520, 1) or player.GetBuff(9521, 1) then	--香烛
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(8, 9507), 4)
		player.AddBuff(player.dwID, player.nLevel, 9474, 1, 3)
	else
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(8, 9507), 4)
		player.AddBuff(player.dwID, player.nLevel, 9474, 1, 4)
	end
end
