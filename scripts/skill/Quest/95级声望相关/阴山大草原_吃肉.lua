---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴山大草原_吃肉.lua
-- 更新时间:	2015/10/18 20:32:16
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/阴山大草原_吃肉.lua",											-- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(8, 50), 4)
	local buff = player.GetBuff(buffID1, 1)
	if not buff then
		return
	end
	if buff.nStackNum >= 10 then
		player.AddBuff(0, 99, buffID2, 1, 20)
		player.AddBuff(0, 99, 10120, 1)
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(8, 51), 4)
		CommonPQ_AddPlayerPQValueTips(player, nPQID, nAddPlayerValue)
		CommonPQ_AddValue(nPQID, 1, nAddPQValue, scene)
		if not player.IsAchievementAcquired(5138) then	--成就“大快朵颐”
			player.AddAchievementCount(5138, 1)					
		end
	else
		player.DoAction(0, 10156)
