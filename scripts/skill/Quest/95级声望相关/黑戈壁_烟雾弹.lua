---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/黑戈壁_烟雾弹.lua
-- 更新时间:	2015/10/6 17:30:25
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/黑戈壁_烟雾弹.lua",											-- 属性值1
		0											-- 属性值2
	);
	...
	----------------- 施放距离 -------------------------------------------------
	skill.nMinRadius		= 12 * LENGTH_BASE;		-- 技能施放的最小距离
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		...
		local npcTarget = scene.GetNpcByNickName("PQ82_Target_" .. player.dwID)
		if not npcTarget then
			npcTarget = scene.CreateNpc(47923, nX, nY, nZ, 0, 3 * GLOBAL.GAME_FPS, "PQ82_Target_" .. player.dwID)
		end
		player.DelBuff(10009, 1)
		player.SetTimer(40, "scripts/skill/Quest/95级声望相关/黑戈壁_烟雾弹.lua", 0, 0)
...
function OnTimer(player, nParam1, nParam2)
	...
	local npcTarget = scene.GetNpcByNickName("PQ82_Target_" .. player.dwID)
	if npcTarget then
		player.CastSkillXYZ(15059, 1, npcTarget.nX, npcTarget.nY, npcTarget.nZ)
