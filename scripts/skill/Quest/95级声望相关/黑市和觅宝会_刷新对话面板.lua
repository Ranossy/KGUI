---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/黑市和觅宝会_刷新对话面板.lua
-- 更新时间:	2016/1/26 18:03:21
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
Include("scripts/Map/95级声望相关/npc/黑市和觅宝会/高级拍卖师.lua")
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = player.GetBuff(10459, 1)
	if player.GetBuff(10464, 1) then
		return
	end
	
	local sz1 = "PMS_HS_GJ"
	local sz2 = "PMS_MBH_GJ"
	local npc1 = scene.GetNpcByNickName(sz1)
	local npc2 = scene.GetNpcByNickName(sz2)
	if npc1 and GetCharacterDistance(player.dwID, npc1.dwID) <= 3 * 64 then
		OnDialogue(npc1, player)
	end
	local npc2 = scene.GetNpcByNickName(sz2)
	if npc2 and GetCharacterDistance(player.dwID, npc2.dwID) <= 3 * 64 then
		OnDialogue(npc2, player)
	end
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	if player.nX ~= tNpcList[nCustomValue][1] then
		player.SetPosition(tNpcList[nCustomValue][1], tNpcList[nCustomValue][2], tNpcList[nCustomValue][3])
	end
end
