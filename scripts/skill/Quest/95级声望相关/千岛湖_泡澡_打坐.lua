---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/千岛湖_泡澡_打坐.lua
-- 更新时间:	2015/8/24 11:49:52
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/千岛湖_泡澡_打坐.lua",				
		0														
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(9556, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	--skill.SetNormalCoolDown(1, 192);	--技能普通CD
	...
	skill.nChannelFrame		= 9600;	 			-- 通道技持续时间，单位帧数
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if not scene.GetNpcByNickName("PQMarkNPC50Num1") then
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not scene.GetNpcByNickName("PQMarkNPC50Num1") and not scene.GetNpcByNickName("Act300_Counter") then
		player.AddBuff(0, 99, 10120, 1)
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(8, 9687), 4)
		return
	end
	local buffCount = player.GetBuff(9557, 1)
	if not buffCount then
		local nTime = GetNextDay7()--获得次日7时分钟数
		if nTime == 0 then
			nTime = 1
		end
		player.AddBuff(0, 99, 9557, 1, nTime)
		buffCount = player.GetBuff(9557, 1)
	end
	if buffCount then
		if buffCount.nCustomValue >= 100 then
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(8, 7747), 4)
			return
		else
			buffCount.nCustomValue = buffCount.nCustomValue + 1
			if buffCount.nCustomValue == 100 then
				if not player.IsAchievementAcquired(5141) then	--成就“小懒虫”
					player.AddAchievementCount(5141, 1)					
				end
			end
		end
	end
	local nExpPercent = 0.1
	--穿浴袍涨经验 9308
	if player.GetBuff(9308, 1) then
		nExpPercent = nExpPercent + 0.1
	end
	--喝清水涨经验
	if player.GetBuff(9555, 1) then
		nExpPercent = nExpPercent + 0.1
	end
	--涨经验
	local tLevelUpData = GetLevelUpData(player.nRoleType, player.nLevel)
	if not tLevelUpData then
		return
	end
	if player.nLevel < player.nMaxLevel then
		local nAddExp = math.min(math.floor(tLevelUpData.Experience * nExpPercent / 100), 1000)
		player.AddExp(nAddExp)
	end
	-- 吃鸡蛋涨修为
	if player.GetBuff(9554, 1) then
		player.AddTrain(20)
	end
	CommonPQ_AddValue(nPQID, 1, nAddPQValue, scene)
	CommonPQ_AddPlayerPQValueTips(player, nPQID, nAddPlayerValue)
end
