---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/黑戈壁_发起偷袭.lua
-- 更新时间:	2015/10/9 15:19:38
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
local nPQID = 83
local tHuman = {46634, 46635, 46636, 46637}	--主动怪，一窝一个
local tBoss = {44874, 44876, }	--BOSS怪，多窝一个
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/黑戈壁_发起偷袭.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcMaster = GetNpc(dwSkillSrcID)
	if not npcTarget or not npcMaster then
		return
	end
	...
	local bIsCreateBoss = false	--判断是否刷BOSS
	if scene.GetCustomInteger4(SCENE_CUSTOM_VALUE_NAME.HEIGEBI.PQ83_ISCREATEBOSS) >= 200 then
		bIsCreateBoss = true
	end

	if npcTarget.dwTemplateID == 47772 and npcTarget.GetCustomInteger1(63) ~= 1 then	--单个刷怪
		npcTarget.SetCustomInteger1(63, 1)
		npcTarget.PlaySfx(1225, npcTarget.nX, npcTarget.nY, npcTarget.nZ, 0)
		CreateSoldiers(npcTarget, tHuman, 1)
		--RandomSoldierSpecy(npcTarget, 1, player)
		npcTarget.SetDisappearFrames(3 * GLOBAL.GAME_FPS, 1)	--烟雾标记NPC要进重生组
	elseif npcTarget.dwTemplateID == 47773 and npcTarget.GetCustomInteger1(63) ~= 1 then
		npcTarget.SetCustomInteger1(63, 1)
		npcTarget.PlaySfx(1225, npcTarget.nX, npcTarget.nY, npcTarget.nZ, 0)
		CreateSoldiers(npcTarget, tHuman, Random(3, 6))

		if bIsCreateBoss then	--创建BOSS处理
			local npcBOSS = scene.GetNpcByNickName("PQ83_DaJieBoss")
			if not npcBOSS then
				CreateSoldiers(npcTarget, tBoss, 1)
				bIsCreateBoss = false
			end
		end
...
function CreateSoldiers(npcMaster, tTable, nNum)
	...
	--print(nNum,nCode)
	for i = 1, nNum do
		local szNewName = "PQ83_" .. npcMaster.dwID .. "_" .. i
		if tTable == tBoss then
			szNewName = "PQ83_DaJieBoss"
		end
		local npcSoldier = scene.GetNpcByNickName(szNewName)
		...
		if not npcSoldier then
			...
			--print(nFace)
			npcSoldier = scene.CreateNpc(tTable[Random(#tTable)], Pos_X, Pos_Y, npcMaster.nZ, nFace, 60 * GLOBAL.GAME_FPS * 30, szNewName)
			if npcSoldier then
				npcSoldier.SetCustomInteger2(0, nPQID)
				npcSoldier.AddBuff(0, 99, 10034, 1, 600)
			end
		end
	end
end
