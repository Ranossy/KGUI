---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴山大草原_PQ终结AOE.lua
-- 更新时间:	2015/8/17 11:27:07
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/阴山大草原_PQ终结AOE.lua",											-- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local tNpcID = {44850, 44851, 44854, 44855, 44857, 44858, 44866, 44867}
	local npcTarget = GetNpc(dwCharacterID)
	--if npcTarget and (npcTarget.dwTemplateID == 44850 or npcTarget.dwTemplateID == 44851 or npcTarget.dwTemplateID == 44854 or npcTarget.dwTemplateID == 44855 or npcTarget.dwTemplateID == 44857 or npcTarget.dwTemplateID == 44858 or npcTarget.dwTemplateID == 44866 or npcTarget.dwTemplateID == 44867) then
	--npcTarget.Die()
	if npcTarget then
		for i = 1, #tNpcID do
			if npcTarget.dwTemplateID == tNpcID[i] then
				npcTarget.Die()
				return
			end
		end
