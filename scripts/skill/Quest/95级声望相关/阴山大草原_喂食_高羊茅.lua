---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴山大草原_喂食_高羊茅.lua
-- 更新时间:	2015/8/18 20:39:17
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/阴山大草原_喂食_高羊茅.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(9526, 1, 1, 1, 1);		-- 需求自身Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = player.GetBuff(9526, 1)
	if not buff then
		return
	end
	...
	if npcTarget then
		...
		if not buff1 then
			...
			if player.GetItemAmount(ITEM_TABLE_TYPE.OTHER, 23980) >= 1 then
				...
				player.DelBuff(9526, 1)
				player.CostItemInAllPackage(5, 23980, 1)
			...
			if nDouble == nDoubleAward then
				...
				RemoteCallToClient(player.dwID, "OnSendSystemAnnounce", GetEditorString(8, 9690), "red")
				return
			end
		end
	end
