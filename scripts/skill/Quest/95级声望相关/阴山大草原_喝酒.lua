---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴山大草原_喝酒.lua
-- 更新时间:	2015/10/18 20:41:03
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
local nPQID = 46
local nAddPQValue = 20
local nAddPlayerValue = 10
local nAddPQValue_Min = 2
local nAddPlayerValue_Min = 0

local buffID1 = 9366
local buffID2 = 9368
local nTime = 60 * 3 -- 10分钟
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/阴山大草原_喝酒.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	skill.nChannelFrame		= 800;	 			-- 通道技持续时间，单位帧数
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	--if not player then
		--return false
	--end
	--local buff = player.GetBuff(buffID1, 1)
	--if buff and buff.nStackNum >= 10 then
		--RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", "你已经连喝十杯美酒，去别处玩玩吧。", 4)
		--return false
	--end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.AddBuff(0, 99, buffID1, 1, nTime)
	RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(8, 45), 4)
	local buff = player.GetBuff(buffID1, 1)
	if not buff then
		return
	end
	if buff.nStackNum >= 10 then
		player.AddBuff(0, 99, buffID2, 1, 60 * 5)
		player.AddBuff(0, 99, 10120, 1)
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(8, 46), 4)
		CommonPQ_AddPlayerPQValueTips(player, nPQID, nAddPlayerValue)
		CommonPQ_AddValue(nPQID, 1, nAddPQValue, scene)
	else
		player.DoAction(0, 10157)
		--CommonPQ_AddPlayerPQValueTips(player, nPQID, nAddPlayerValue_Min)
		CommonPQ_AddValue(nPQID, 1, nAddPQValue_Min, scene)
	end
end
