---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/五台山_飞人符文.lua
-- 更新时间:	2015/8/12 10:41:40
-- 更新用户:	zhengfeng4
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/五台山_飞人符文.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = player.GetBuff(9473, 1)
	if player.GetBuff(9471, 1) then
		buff = player.GetBuff(9473, 3)
		if buff and buff.nLeftActiveCount> 1 then
			return 
		end
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(8, 9508), 4)
		player.AddBuff(0, 99, 9473, 3, 10)		
	elseif player.GetBuff(9470, 1) or player.GetBuff(9520, 1) or player.GetBuff(9521, 1) then
		buff = player.GetBuff(9473, 2)
		if buff and buff.nLeftActiveCount> 1 then
			return 
		end
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(8, 9508), 4)
		player.AddBuff(0, 99, 9473, 2, 10)		
	else
		if buff and buff.nLeftActiveCount> 1 then
			return 
		end
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(8, 9508), 4)
		player.AddBuff(0, 99, 9473, 1, 10)		
	end
	-- buff持续时间大于2秒则不再加buff
	--if buff and buff.nLeftActiveCount> 1 then
	--return 
	--end
	--player.AddBuff(0, 99, 9473, 1, 6)
end
