---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴山大草原_勇士脱战BUFF.lua
-- 更新时间:	2015/10/20 22:00:39
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID)
	...
	if npc.dwTemplateID == 44877 then
		npc.AddBuff(0, 99, 10155, 1, 60)
		return
	end
	npc.AddBuff(0, 99, 10155, 1, 15)
	--npc.SetCustomUnsigned4(0, 0)
	--npc.SetCustomUnsigned1(4, 0)
end
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	local player = GetPlayer(npc.GetCustomUnsigned4(0))
	if player then
		local buffDing = player.GetBuff(3448, 1)
		if buffDing and buffDing.nLeftActiveCount > 1 then
			player.DelBuff(3448, 1)
		end
		local buffTingFeng = player.GetBuff(7913, 1)
		if buffTingFeng and buffTingFeng.nLeftActiveCount > 1 then
			player.DelBuff(7913, 1)
		end
	end
	npc.AddBuff(0, 99, 10155, 1, 15)
	--npc.SetCustomUnsigned4(0, 0)
