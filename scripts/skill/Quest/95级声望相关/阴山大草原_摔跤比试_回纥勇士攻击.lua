---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴山大草原_摔跤比试_回纥勇士攻击.lua
-- 更新时间:	2015/10/22 21:57:24
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	...
	{nDamageBase = 7500, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/阴山大草原_摔跤比试_回纥勇士攻击.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc.GetCustomUnsigned4(0) ~= 0 and player.dwID ~= npc.GetCustomUnsigned4(0) then
		player.CustomDamage(npc.dwID, 15137, 1, 0, player.nMaxLife * 10)
		local playerMaster = GetPlayer(npc.GetCustomUnsigned4(0))
		if playerMaster then
			playerMaster.CustomDamage(npc.dwID, 15137, 1, 0, playerMaster.nMaxLife * 10)
		end
		local tSay = {GetEditorString(9, 564),GetEditorString(9, 565),GetEditorString(9, 566)}
		npc.Say(tSay[Random(#tSay)], 2560, true)
		npc.ClearAllThreat()
		return
	end
	if player and not player.GetBuff(9548, 0) then
		player.CustomDamage(npc.dwID, 15137, 1, 0, player.nMaxLife * 0.18)
	end
end
