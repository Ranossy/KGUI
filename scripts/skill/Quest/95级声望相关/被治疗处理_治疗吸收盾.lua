---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/被治疗处理_治疗吸收盾.lua
-- 更新时间:	2015/8/12 20:19:52
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/被治疗处理_治疗吸收盾.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc.GetCustomUnsigned4(59) == 0 then		--满员就不判了
		for i = 1, 10 do
			if npc.GetCustomUnsigned4(23 + 4 * (i-1)) == player.dwID then
				break
				--已有就不计了
			end
			if npc.GetCustomUnsigned4(23 + 4 * (i-1)) == 0 then
				npc.SetCustomUnsigned4(23 + 4 * (i-1), player.dwID)
				break
			end
		end
	end
	if npc.GetMapID() == 216 then--阴山大草原
		if npc.GetBuff(9346, 1) then
			local tAbsorbInfo = npc.GetAbsorbInfo()
			--print(tAbsorbInfo[1].nAbsorbType,tAbsorbInfo[1].nAbsorbValue,tAbsorbInfo[1].nAbsorbUsedValue)
			if tAbsorbInfo[1].nAbsorbUsedValue >= 80000 then
				npc.DelBuff(9346,1)
			end
		end
	end
