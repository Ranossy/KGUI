---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴风峡日常_负重表现移除.lua
-- 更新时间:	2015/12/7 17:15:09
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	if nLeftFrame == 0 then
		...
		local buffFuZhong = player.GetBuff(10229, 1)
		if buffFuZhong then
			local nSum = buffFuZhong.nStackNum
			if nSum > 1 then
				player.DelGroupBuff(10229, 1)
			else
				player.DelBuff(10229, 1)
			end
		end
		local nQuestIndex = player.GetQuestIndex(14608)
		local nQuestPhase = player.GetQuestPhase(14608)
		local nQuestIndex1 = player.GetQuestIndex(14614)
		local nQuestPhase1 = player.GetQuestPhase(14614)
		if nQuestIndex and nQuestPhase == 1 and not player.GetBuff(10228, 0) then
			player.SetQuestValue(nQuestIndex, 0, 0)
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(9, 1870), 4)
		elseif nQuestIndex1 and nQuestPhase1 == 1 and not player.GetBuff(10228, 0) then
