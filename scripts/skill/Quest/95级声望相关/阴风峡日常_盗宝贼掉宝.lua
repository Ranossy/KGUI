---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴风峡日常_盗宝贼掉宝.lua
-- 更新时间:	2015/12/7 21:53:09
-- 更新用户:	jiangzhefei1
-- 脚本说明:	╰(￣▽￣)╭打我会掉宝贝呢~
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/95级声望相关/阴风峡日常_盗宝贼掉宝.lua",											-- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc.GetBuff(10329, 1) then
		return
	end
	local buff = npc.GetBuff(10322, 1)
	if not buff then
		return
	end
	npc.DelBuff(10322, 1)
	local tSay = {GetEditorString(9, 1894), GetEditorString(9, 1895), GetEditorString(2, 1774), GetEditorString(9, 1896), GetEditorString(9, 1897), GetEditorString(9, 1898)}
	npc.Say(tSay[Random(#tSay)], 2240, true)
	local Pos_X = npc.nX + Random(30) / 10 * 64 * math.cos(Random(npc.nFaceDirection + 86, npc.nFaceDirection + 171))
	local Pos_Y = npc.nY + Random(30) / 10 * 64 * math.sin(Random(npc.nFaceDirection + 86, npc.nFaceDirection + 171))
	local nFace = Random(256)
	--print(nFace)
	npc.AddBuff(0, 99, 10329, 1, 1)
	local doodadNew = scene.CreateDoodad(Random(5833, 5848), Pos_X, Pos_Y, npc.nZ, nFace)
	if doodadNew then
		doodadNew.SetDisappearFrames(60 * GLOBAL.GAME_FPS, 0)
	end
end
