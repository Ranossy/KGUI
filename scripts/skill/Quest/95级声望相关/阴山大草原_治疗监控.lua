---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/95级声望相关/阴山大草原_治疗监控.lua
-- 更新时间:	2015/8/12 19:00:40
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
Include("scripts/Map/ACT_通用PQ/include/通用PQ数值.lua")
local tWord =
{
	{GetEditorString(7, 295)},
}
...
function OnRemove(dwCharacterID, dwBuffID, dwBuffLevel, nLeftFrame)
	...
	if nLeftFrame ~= 0 and scene.IsNickNameNpcExist("PQMarkNPC42Num1") then
		for i = 1, 10 do
			local player
			player = GetPlayer(npc.GetCustomUnsigned4(23 + 4 * (i-1)))
			if player then
				AddBothValue(player, npc)
				if not player.IsAchievementAcquired(5131) then	--成就“按摩能手”
					player.AddAchievementCount(5131, 1)
				end					
			end
		end
	end
	for i = 1, 10 do
		npc.SetCustomUnsigned4(23 + 4 * (i-1), 0)
	end
	npc.AddBuff(0, 99, 10158, 1, 15)
end

function AddBothValue(player, npc)
	local nPQID = 42
	local nAddPQValue = 3
	local nAddPlayerValue = 5
	local scene = npc.GetScene()
	if scene then
		CommonPQ_AddValue(nPQID, 1, nAddPQValue, scene)
	end
	CommonPQ_AddPlayerPQValueTips(player, nPQID, nAddPlayerValue)-- player，pqID,本次增加的个人贡献度，当前PQ的个人贡献度总分
end
