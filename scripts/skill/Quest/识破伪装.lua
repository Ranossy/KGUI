---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/识破伪装.lua
-- 更新时间:	2013/2/18 11:26:51
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
	...
	local dwPlayerID = npc.GetCustomInteger4(1)
	if not IsPlayer(dwPlayerID) then
		npc.SetCustomInteger4(1, 0)
		return
	end
	...
	if player.GetBuff(5249, 1) then
		player.DelBuff(5249, 1)
	elseif player.GetBuff(5249, 2) then
		player.DelBuff(5249, 2)
	end
	npc.Say(GetEditorString(4, 9297), 768)
	npc.ModifyThreat(player.dwID, 2)
	npc.SetCustomInteger4(1, 0)
end
