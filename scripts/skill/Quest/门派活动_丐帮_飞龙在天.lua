---------------------------------------------------------------------->
-- 脚本名称:	G:/Head/client/scripts/skill/Quest/门派活动_丐帮_飞龙在天.lua
-- 更新时间:	2013/5/19 21:35:37
-- 更新用户:	chenbugao
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	--skill.SetDelaySubSkill(40, 5371, 1)	
	-----------------技能渐变相关-------------------------------------------
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 159 then--丐帮门派事件郭岩处理
		--print(10086)
		local npcGuoYan = scene.GetNpcByNickName("MPRQ_GB_GuoYan")
		local npcYuwenMie = scene.GetNpcByNickName("MPRQ_GB_YuwenMie")
		if not npcGuoYan or not npcYuwenMie then
			return
		end
		local _, dwTargetID1 = npcGuoYan.GetTarget()
		local npcTargetOfGuoYan = GetNpc(dwTargetID1)
		if npcTargetOfGuoYan.szName ~= "MPRQ_GB_YuwenMie" then
			Log(GetEditorString(5, 5369))
			npcGuoYan.SetTarget(TARGET.NPC,npcYuwenMie.dwID)
		end
		local _, dwTargetID2 = npcYuwenMie.GetTarget()
		local npcTargetForYuwenMie = GetNpc(dwTargetID2)
		if not npcTargetForYuwenMie.szName == "MPRQ_GB_GuoYan" then
			npcTargetForYuwenMie.SetTarget(TARGET.NPC,npcGuoYan.dwID)
		end
		npcYuwenMie.DoAction(0, 0)
		npcYuwenMie.DoAction(0,10058)
		--npcYuwenMie.BeHitted(npcGuoYan.dwID,7937,1,85)
	end
	local guoyan = scene.GetNpcByNickName("gaibangzidi9")
	if not guoyan then 
		return
	end
	local targetNPC = scene.GetNpcByNickName("lingxuecike9")	
	if not targetNPC then
		return
	end
	targetNPC.DoAction(0, 10062)
	print(targetNPC.DoAction(0, 10072))
end
