---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_峰值201611玩家变身/易魂.lua
-- 更新时间:	2016/11/17 19:37:55
-- 更新用户:	qinfupi
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_峰值201611玩家变身/易魂.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID)
	local tWeightLv = {1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000}
	local tSkillGroup= {315, 315, 315, 315, 315, 315, 315, 315}
	local player = GetPlayer(dwCharacterID)
	local nRandBuffLv = Random(1, 8000)

	--随机技能等级
	local nBuffLv = 0
	for i, nWeight in pairs(tWeightLv) do
		if nRandBuffLv < nWeight and nBuffLv == 0 then
			nBuffLv = i
		end
	end
	print("nRandBuffLv =" .. nRandBuffLv .. " nBuffLv=" .. nBuffLv)

	--先删除旧buff
	local buff = player.GetBuff(11514, 0)
	if buff then
		player.DelBuff(11514, buff.nLevel)
	end
	
	--加上新buff顺便更换动态技能栏
	player.AddBuff(0, 99, 11514, nBuffLv, 10)
	player.SetDynamicSkillGroup(tSkillGroup[nBuffLv])
end
