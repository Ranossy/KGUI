...
function GetSkillLevelData(skill)
    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	skill.AddAttribute(
   		...
   		"skill/Quest/苍山洱海_连杀_内功.lua",				
   		0														
   		);
	----------------- 技能施放Buff需求 ---------------------------------------------
    skill.AddSlowCheckSelfBuff(3631, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
...
function Apply(dwCharacterID)
	...
	player.AddBuff(0,99,3630,1)
	local buff = player.GetBuff(3630,1)
	if buff.nStackNum == 4 then
		for i = 1,4 do  
			player.DelBuff(3630,1)
		end
		player.AddBuff(0,99,3631,3)
	end
end
