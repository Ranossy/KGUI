...
function Apply(dwCharacterID)
	...
	if scene then
		if player then
			...
			local npc1 = scene.GetNpcByNickName("PDC_LingDui")
			if npc1 then
				npc1.SayTo(player.dwID, GetEditorString(5, 1945))
			end
			if nQuestIndex and nQuestPhase == 1 then
				--player.SetQuestValue(nQuestIndex, 0, 1)
				--RemoteCallToClient(player.dwID, "On_Trial_Screen_Black")
				player.SetTimer(3 * GLOBAL.GAME_FPS, "scripts/Map/枫华谷_安史之乱/event/叛军猛如虎-释放.lua", 0, 0)
			end
		end
	end
end
