...
function Apply(dwCharacterID)
	...
	if player then
		...
		if nQuestIndex and nQuestPhase == 1 then
			...
			local buff = player.GetBuff(3995, 1)
			if buff then
				player.DelBuff(3995, 1)
			end
			player.AddBuff(0, 99, 1899, 1, 51)
			RemoteCallToClient(player.dwID, "OnPlayProtocolMovie", 20, true)
		end
	end
end
