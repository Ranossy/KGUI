---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/霸刀预热/霸刀预热_独孤修_黄泉浊浪释放.lua
-- 更新时间:	2016/9/7 11:39:14
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/Quest/霸刀预热/霸刀预热_独孤修_黄泉浊浪释放.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		if scene then
			local npcBOSS = scene.GetNpcByNickName("PQ164_DuGuiXiu")
			if npcBOSS then
				dwCasterID = npcBOSS.dwID
			end
		end
		player.CustomDamage(dwCasterID, 16698, 1, 3, player.nMaxLife * 0.05)
		player.AddBuff(dwCasterID, 99, 11190, 1, 7)
