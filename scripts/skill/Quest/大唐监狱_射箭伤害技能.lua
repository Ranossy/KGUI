---------------------------------------------------------------------->
-- 脚本名称:	F:/zhugan/client/scripts/skill/Quest/大唐监狱_射箭伤害技能.lua
-- 更新时间:	2013/2/25 15:45:36
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = npc.GetBuff(5282, 1)
	if not buff then
		return
	end
	local tID = buff.nCustomValue
	if IsPlayer(tID) then
		npc.CastSkill(7840, 1, TARGET.PLAYER, tID)
	else
		npc.CastSkill(7840, 1, TARGET.NPC, tID)
	end
	npc.DelBuff(5282,1)
end
