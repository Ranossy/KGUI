---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/苍云_100层撞币_20.lua
-- 更新时间:	2014/11/17 22:39:53
-- 更新用户:	zhangtianhui1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/苍云_100层撞币_20.lua", -- 属性值1
		0														-- 属性值2
	);
	--skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	----苍云门派事件处理----
	local nQuestId_14972 = 14972
	local nQuestIndex_14972 = player.GetQuestIndex(nQuestId_14972)
	local nQuestPhase_14972 = player.GetQuestPhase(nQuestId_14972)
	if nQuestIndex_14972 and nQuestPhase_14972 == 1 then
		local nQuestValue_14972 = player.GetQuestValue(nQuestIndex_14972, 0) + 3
		player.SetQuestValue(nQuestIndex_14972, 0, nQuestValue_14972)
		npc.PlaySfx(439, npc.nX, npc.nY, npc.nZ)
		npc.SetDisappearFrames(1)
	end
	----苍云门派事件结束----
	local nQuestId = 12740
	local nQuestIndex = player.GetQuestIndex(nQuestId)
	local nQuestPhase = player.GetQuestPhase(nQuestId)
	if not (nQuestIndex and nQuestPhase == 1) then
		return
	end
	local nQuestValue = player.GetQuestValue(nQuestIndex, 0) + 5
	player.SetQuestValue(nQuestIndex, 0, nQuestValue)
	npc.PlaySfx(439, npc.nX, npc.nY, npc.nZ)
	npc.SetDisappearFrames(1)
end
