---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/长安内城_巡逻狗.lua
-- 更新时间:	2013/9/11 11:36:36
-- 更新用户:	xutong3
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/长安内城_巡逻狗.lua", -- 属性值1scripts/skill/Quest/长安内城_站岗.lua
		0														-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
		...
		if player.nMoveState ~= MOVE_STATE.ON_STAND then
			if player.GetBuff(2587, 7) and not player.GetBuff(6506, 1) then
				...
				if not buff or buff.nStackNum <= 1 then
					...
					RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(5, 9686), 3)
					return
				end
