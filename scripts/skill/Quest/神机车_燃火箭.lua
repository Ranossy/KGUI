---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/神机车_燃火箭.lua
-- 更新时间:	2014/3/10 21:55:31
-- 更新用户:	gaoweilong1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if target then
		if GetRelation(player.dwID, target.dwID) == RELATION_TYPE.ENEMY or GetRelation(player.dwID, target.dwID) == RELATION_TYPE.NEUTRALITY then
			if GetCharacterDistance(player.dwID, target.dwID) / 64 <= 38 then
				player.SetTimer(2, "scripts/skill/Quest/神机车_燃火箭.lua", 0, 0)
			end		
		end
	end
	...
	if player.nCurrentRage < 50 then
...
--	print("UnApply")
	...
	if IsPlayer(dwID) then
		...
		if tPlayer then
			player.CastSkillXYZ(8957, 1, tPlayer.nX, tPlayer.nY, tPlayer.nZ)
		end
	else
		local tNPC = GetNpc(dwID)
		if tNPC then
			player.CastSkillXYZ(8957, 1, tNPC.nX, tNPC.nY, tNPC.nZ)
		end
	end
