---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/长安内城_乘风而逝.lua
-- 更新时间:	2013/8/19 21:40:30
-- 更新用户:	zhouyixiao1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/长安内城_乘风而逝.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID)
	...
	if not player.GetBuff(6499, 1) then
		player.DoAction(player.dwID, 10068)
		player.AddBuff(0, 99, 6834, 1, 1)
		player.SetTimer(12, "scripts/skill/Quest/长安内城_乘风而逝.lua", 0, 0)
	else
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(6, 173), 4)	
	end
end
function OnTimer(player, nParam1, nParam2)
	...
	if step == 0 then
		player.Stop()
		player.AddBuff(0, 99, 6504, 1, 3)	
		player.PlaySfx(548, player.nX, player.nY, player.nZ)
		if nQuestIndex then
			player.SetQuestValue(nQuestIndex, 4, 1)	
		end
		if nQuestIndex1 then
			player.SetQuestValue(nQuestIndex1, 4, 1)	
		end
		player.DoAction(player.dwID, 10055)
	end
	if step == 3 then
		RemoteCallToClient(player.dwID, "On_Trial_Screen_Black") --黑屏
	end
	if step == 4 then
		player.SetPosition(124139, 52350, 1056960)
		player.SetDynamicSkillGroup(0)
		player.DelBuff(6478, 1)
		if nQuestIndex then
			if player.GetBuff(6505, 1) then
				player.DelBuff(6505, 1)
				player.SetQuestValue(nQuestIndex, 4, 0)
			end	
		end
		if nQuestIndex1 then
			if player.GetBuff(6505, 1) then
				player.DelBuff(6505, 1)
				player.SetQuestValue(nQuestIndex1, 4, 0)
			end	
		end
	end
	if step < 10 then
		player.SetTimer(16, "scripts/skill/Quest/长安内城_乘风而逝.lua", step + 1, 0)
	end
end
