---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/节日春节_利爪猛击.lua
-- 更新时间:	2014/1/2 11:02:55
-- 更新用户:	zhouyixiao1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Quest/节日春节_利爪猛击.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if youshshangpanduan(0, dwSkillSrcID, dwCharacterID, 90) then
		npc.CastSkill(8695, 1, TARGET.PLAYER, dwCharacterID)
	else
	end
end
function youshshangpanduan(d, c1, c2, angle)
	--参数分别为盲点距离，释放者（npc）id，被击者（player）id,释放者右侧角度（从面向开始）
	local player = GetPlayer(c2)
	if not player then
		return false
	end
	local npc = GetNpc(c1)
	...
	local dir1 = npc.nFaceDirection
	local dir2 
	if npc.nX == player.nX then
		if npc.nY > player.nY then
			dir2 = 256 * 0.75
		else
			dir2 = 256 * 0.25
		end
	elseif npc.nX >= player.nX then
		dir2 = math.atan((player.nY-npc.nY) / (player.nX-npc.nX)) * 128 / math.pi + 128
	else
		dir2 = math.atan((player.nY-npc.nY) / (player.nX-npc.nX)) * 128 / math.pi
	end
