---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_风雷刀谷预热/柳愚坚壁清野子技能.lua
-- 更新时间:	2017/3/20 11:38:24
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		tSkillData[1].nDamageBase, -- 属性值1
		0											-- 属性值2
	);
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK, -- 属性作用模式
		ATTRIBUTE_TYPE.SKILL_PHYSICS_DAMAGE_RAND, -- 魔法属性
		tSkillData[1].nDamageRand, -- 属性值1
		0											-- 属性值2
	);
	skill.AddAttribute(
	...
	skill.AddAttribute(
		...
		"skill/Quest/ACT_风雷刀谷预热/柳愚坚壁清野子技能.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	--if nTime ~= 0 and GetCurrentTime() + 5 < nTime then
	--return
	--end
	...
	npcBoss.AddBuff(0, 99, 11770, 1, 1)
	local scene = npcBoss.GetScene()
	if not scene then
		return
	end
	local szname = nTime % 20
	local szNickName = "bdyr_jianbiqingye" .. szname
	---范围残余伤害
	if not scene.IsNickNameNpcExist(szNickName) then
		local npc_daoqi = scene.CreateNpc(55995, npcBoss.nX, npcBoss.nY, npcBoss.nZ, npcBoss.nFaceDirection, 11 * 16, szNickName)
		if npc_daoqi then
			npc_daoqi.SetRelationProxyID(npcBoss.dwID)
			npc_daoqi.AddBuff(npc_daoqi.dwID, npc_daoqi.nLevel, 11755, 1, 11)
			npcBoss.PlaySfx(1522, npcBoss.nX, npcBoss.nY, npcBoss.nZ, npcBoss.nFaceDirection * 360 / 256)
		end
	end
