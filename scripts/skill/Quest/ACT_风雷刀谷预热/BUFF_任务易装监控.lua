---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/ACT_风雷刀谷预热/BUFF_任务易装监控.lua
-- 更新时间:	2017/3/1 17:55:39
-- 更新用户:	JIANGZHEFEI-PC
-- 脚本说明:
----------------------------------------------------------------------<
...
local tChangeModelID = {
	[ROLE_TYPE.STANDARD_MALE] = 1,
	[ROLE_TYPE.STANDARD_FEMALE] = 2,
	[ROLE_TYPE.LITTLE_BOY] = 4,
	[ROLE_TYPE.LITTLE_GIRL] = 3,
}
...
function Apply(dwCharacterID)
	...
	if player.GetMapID() ~= 243 or (player.nX > 31324 or player.nX < 20748 or player.nY > 63875 or player.nY < 49543) then
		player.DelBuff(11764, tChangeModelID[player.nRoleType])
	end
end
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	if player.GetBuff(11765, 1) then
		player.DelBuff(11765, 1)
	end
	local scene = player.GetScene()
	if not scene then
		return
	end
	local npcActor = scene.GetNpcByNickName("16665_LangYa")
	if npcActor and GetCharacterDistance(npcActor.dwID, player.dwID) < 64 * 15 then
		npcActor.ModifyThreat(player.dwID, 100)
		local npcBaDao = scene.GetNpcByNickName("16665_BaDao")
		if npcBaDao then
			npcBaDao.ModifyThreat(player.dwID, 100)
		end
	end
end
