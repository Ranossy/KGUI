---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/五台山/哈迪血量判断.lua
-- 更新时间:	2015/10/28 17:09:57
-- 更新用户:	zhangchennan
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	local npcHaDi = GetNpc(dwCharacterID)
	if not npcHaDi then
		return
	end
	local n = npcHaDi.GetCustomInteger4(1)
	local player = GetPlayer(n)
	if not player then
		npcHaDi.SetDisappearFrames(1)
		return
	end
	...
	local nQuestID = 13715
	local nQuestIndex = player.GetQuestIndex(nQuestID)
	local nQuestPhase = player.GetQuestPhase(nQuestID)
	local npcHaDiTeXiao = scene.GetNpcByNickName("haditexiao" .. player.dwID)
	local nX, nY, nZ = player.GetAbsoluteCoordinate()
	if not (nQuestIndex and nQuestPhase == 1) then
		return
	end
	--if player.GetQuestValue(nQuestIndex, 7) == 1 then
		--return
	--end
	--print(11111)
	if npcHaDi.nCurrentLife > npcHaDi.nMaxLife * 0.98 then
		--print(2222)
		npcHaDi.AddBuff(0, 99, 10226, 1, 30)
	else npcHaDi.DelBuff(10226, 1)
		--print(33333)
		npcHaDi.AddBuff(0, 99, 3745, 1)
		npcHaDi.SetForceID(37)
		npcHaDi.ClearAllThreat()
		player.AddBuff(0, 99, 9370, 1)
		player.AddBuff(0, 99, 9847, 1)
		npcHaDi.DoAction(0, 10059)
		npcHaDi.SayTo(player.dwID, GetEditorString(9, 202))
		player.SetTimer(3 * GLOBAL.GAME_FPS, "scripts/Map/五台山/npc/缘居林_哈迪.lua", 0, 0)
		--player.SetQuestValue(nQuestIndex, 7, 1)
		if not npcHaDiTeXiao then
			scene.CreateNpc(48196, nX + 50, nY + 50, nZ, 0, 40 * GLOBAL.GAME_FPS, "haditexiao" .. player.dwID, player.dwID)
		end
