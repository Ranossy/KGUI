...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene then
		if player then
			local nQuestID = 13862
			local nQuestPhase = player.GetQuestPhase(nQuestID)
			local nQuestIndex = player.GetQuestIndex(nQuestID)
			local feiluoshe = scene.GetNpcByNickName("feiluoshe" .. player.dwID)
			if feiluoshe then
				feiluoshe.SayTo(player.dwID, GetEditorString(7, 9764))
			end
			
			if nQuestIndex and nQuestPhase == 1 then
				--player.SetQuestValue(nQuestIndex, 0, 1)
				--RemoteCallToClient(player.dwID, "On_Trial_Screen_Black")
				player.SetTimer(3 * GLOBAL.GAME_FPS, "scripts/Map/五台山/event/吠罗舍释放.lua", 0, 0)
			end
		end
	end
end
