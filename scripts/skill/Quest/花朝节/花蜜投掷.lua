---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Quest/花朝节/花蜜投掷.lua
-- 更新时间:	2016/3/8 12:47:56
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		...
		local npcTarget = scene.GetNpcByNickName("Act117_Target_" .. player.dwID)
		if not npcTarget then
			npcTarget = scene.CreateNpc(47923, nX, nY, nZ, 0, 3 * GLOBAL.GAME_FPS, "Act117_Target_" .. player.dwID)
		end
		player.SetTimer(0.1 * GLOBAL.GAME_FPS, "scripts/skill/Quest/花朝节/花蜜投掷.lua", 0, 0)
...
function OnTimer(player, nParam1, nParam2)
	...
	local npcTarget = scene.GetNpcByNickName("Act117_Target_" .. player.dwID)
	if npcTarget then
		player.CastSkillXYZ(15499, 1, npcTarget.nX, npcTarget.nY, npcTarget.nZ)
		player.PlaySfx(1278, npcTarget.nX, npcTarget.nY, npcTarget.nZ - 64 * 8 * 3, 0)
