---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/藏剑/藏剑_西子情_云栖松.lua
-- 更新时间:	2013/8/7 14:09:46
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/藏剑/藏剑_西子情_云栖松.lua", -- 属性值1
		0														-- 属性值2
	);
	--skill.BindBuff(1,1730,dwSkillLevel);
	--skill.BindBuff(2, 2065, dwSkillLevel);

	----------------- 设置CoolDown ---------------------------------------------------------------------
	--SetPublicCoolDown(id)设置公共CD，SetNormalCoolDown(posi, id)设置技能CD，其中posi为CoolDown位(共3位)，id需要查CoolDownList.tab
	skill.SetPublicCoolDown(16);		
	skill.SetNormalCoolDown(1, 352);
...
function OnSkillLevelUp(skill, player)
	-- if skill.dwLevel == 5 then
		-- player.AcquireAchievement(1364)
	-- end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local num = 0
	if player.IsSkillRecipeActive(628, 1) then
		num = num + 1
	end
	if player.IsSkillRecipeActive(629, 1) then
		num = num + 1
	end
	if num == 2 then
		player.AddBuff(dwCharacterID, player.nLevel, 2065, 7, 1)
	elseif num == 1 then
		player.AddBuff(dwCharacterID, player.nLevel, 2065, 6, 1)
	else
		player.AddBuff(dwCharacterID, player.nLevel, 2065, 5, 1)
	end
	--]]
	...
	player.AddBuff(player.dwID, player.nLevel, 2065, 1, 20 + nCount)

	ModityCDToUI(player, 1668, 0, 0)
end
