---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/藏剑/藏剑_君子风_泉韵_被击判定血量.lua
-- 更新时间:	2015/9/12 15:49:23
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/藏剑/藏剑_君子风_泉韵_被击判定血量.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.nCurrentLife <= player.nMaxLife * 0.4 and not player.GetBuff(9709,1) then
		local nlev = player.GetSkillLevel(1666)
		if nlev ~= 0 then
			local rest, nleftframe, ntotalframe = player.GetSkillCDProgress(1666, nlev)
			if rest == true and nleftframe ~= 0 then
				player.ClearCDTime(351)
				player.AddBuff(player.dwID, player.nLevel, 9709, 1)
			end
		end
	end	
end
