---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/藏剑/镇派/片玉_添加假死效果.lua
-- 更新时间:	2015/9/12 9:32:28
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/藏剑/镇派/片玉_添加假死效果.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.AddBuff(player.dwID, player.nLevel, 9702, 1, 3600)
	if not player.GetBuff(9704, 1) and not player.GetBuff(9703, 1) and not player.GetBuff(9933,1) then
		player.AddBuff(player.dwID, player.nLevel, 9703, 1)
	end

	if player.GetBuff(9933,1)and player.GetBuff(9703,1) then
		player.DelBuff(9703,1)
	end

	if player.nCurrentLife <= player.nMaxLife * 0.01 and not player.GetBuff(9704, 1) and player.nMoveState ~= MOVE_STATE.ON_DEATH and  player.bFightState == true then
		--添加内置CDbuff
		player.DelBuff(9703, 1)
		player.AddBuff(player.dwID, player.nLevel, 9704, 1)
		player.nCurrentLife = player.nMaxLife * 0.15
		player.nCurrentLife = player.nCurrentLife + player.nMaxLife * 0.002 * player.nCurrentRage
