---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/藏剑/藏剑_灵峰剑式_松舍问霞.lua
-- 更新时间:	2017/3/24 18:35:16
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/藏剑/藏剑_灵峰剑式_松舍问霞.lua", -- 属性值1
		0											-- 属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(11821, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	skill.SetNormalCoolDown(1, 1201);	--技能普通CD
	skill.SetCheckCoolDown(1, 444);	--只检查不走的CD
	--skill.SetCheckCoolDown(2, 960);
	----------------- 经验升级相关 ---------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	local _, dwCharacterID = player.GetTarget()
	local target
	--print(dwCharacterID)
	--if dwCharacterID == nil  or dwCharacterID == player.dwID then
	...
	--print(target.nCurrentLife)
	if not player.GetBuff(11821, 1)then
		if target.nCurrentLife > target.nMaxLife * 0.4 then
			return SKILL_RESULT_CODE.FAILED
		else
			player.AddBuff(player.dwID, player.nLevel, 11821, 1)
			local buff = player.GetBuff(11821, 1)
			buff.nCustomValue = dwCharacterID
		end
	else
		
		local buff = player.GetBuff(11821, 1)
		if buff.nLeftActiveCount == 10 then
			return SKILL_RESULT_CODE.FAILED
		end
		--local nlev = math.max(math.min(math.floor((player.nCurrentRage + 10) / 10), 10), 1)
		player.SetTimer(1, "scripts/skill/藏剑/藏剑_灵峰剑式_松舍问霞.lua", buff.nCustomValue, player.dwID)
		--[[
		print(player.CastSkill(14911, nlev, player.GetSkillTarget()))
		player.AddBuff(dwCharacterID, target.nLevel, 10393, 1, 1)
		--]]
		player.DelBuff(11821, 1)
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.CastSkill(14911, nlev, player.GetSkillTarget())
	player.AddBuff(dwCharacterID, target.nLevel, 10393, 1, 1)
	--]]
end
...
function OnTimer(player, dwCharacterID, dwSkillSrcID)
	local player = GetPlayer(dwSkillSrcID)
	if not player then
		return
	end
	...
	local nlev = math.max(math.min(math.floor((player.nCurrentRage + 10) / 10), 10), 1)
	--print(player.GetCDLeft(960))
	player.CastSkill(14911, nlev, targettype,dwCharacterID)
	player.AddBuff(dwCharacterID, target.nLevel, 10393, 1, 1)
end
