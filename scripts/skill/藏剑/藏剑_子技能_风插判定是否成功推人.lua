---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/藏剑/藏剑_子技能_风插判定是否成功推人.lua
-- 更新时间:	2015/9/12 17:18:26
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	...
   	skill.AddAttribute(
   		...
   		"skill/藏剑/藏剑_子技能_风插判定是否成功推人.lua",											-- 属性值1
   		0											-- 属性值2
   		);
	...
	-----------------技能施放技能需求-------------------------------------------
	skill.AddCheckSelfLearntSkill(6545, 1, SKILL_COMPARE_FLAG.EQUAL);     --检查比较Caster自己已学习的技能ID和等级
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nlev = player.GetSkillLevel(1596)
	if nlev ~= 0 and not player.GetBuff(9712,1) then
		local rest, nleftframe, ntotalframe = player.GetSkillCDProgress(1596, nlev)
		if rest == true and nleftframe ~= 0 then
			player.ClearCDTime(337)
			player.AddBuff(player.dwID, player.nLevel, 9712, 1)
		end
	end
	--]]
end
