---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/藏剑/藏剑_西子情_泉凝月长致消失给减伤.lua
-- 更新时间:	2015/9/11 19:11:07
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	--[[
	if player.GetSkillLevel(14603) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 9699, 1, nLeftFrame / 16)
	end
	--]]
	if player.GetSkillLevel(14601) == 1 then
		local nTime = math.floor(nLeftFrame / 16, 1)
		for i = 1, nTime do
			player.CastSkill(14815, 1)	--降低5秒调息时间
		end
	end
end;
