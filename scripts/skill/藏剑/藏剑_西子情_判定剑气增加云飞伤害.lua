---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/藏剑/藏剑_西子情_判定剑气增加云飞伤害.lua
-- 更新时间:	2015/9/25 19:08:07
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.nCurrentRage <= 50 and player.GetKungfuMount() ~= nil and player.GetKungfuMount().dwSkillID == 10145 and player.bFightState == true and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		if not player.GetBuff(9903, 1) then
			player.AddBuff(player.dwID, player.nLevel, 9903, 1)
		end
	else
		if player.GetBuff(9903, 1) then
			player.DelBuff(9903, 1)
		end
	end
end
