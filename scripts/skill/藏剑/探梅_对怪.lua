---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/藏剑/探梅_对怪.lua
-- 更新时间:	2013/8/28 17:38:33
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
	...
	if tplayer and IsPlayer(dwCharacterID) then
		local character = tplayer.GetSelectCharacter()
		if character then 
			local eTargetType, dwTargetID = character.GetTarget()
			if eTargetType == TARGET.PLAYER  then
				N = tplayer.CastSkill(3795, 1, eTargetType, dwTargetID)
				if N ~= 1 then
					tplayer.CastSkill(3797, 1)			--子技能失败清母技能CD
				end		
			else
				tplayer.CastSkill(3797, 1)			--子技能失败清母技能CD
			end
