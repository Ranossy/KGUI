---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/藏剑/橙武听雷额外伤害.lua
-- 更新时间:	2017/3/15 15:01:41
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
--	);
	...
	skill.AddSlowCheckSelfBuff(6471, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);
	----------------- 设置CoolDown ---------------------------------------------------------------------
	...
	if dwSkillLevel < 9 then
		skill.nChannelInterval = 16 * 1.1 / 2;
	elseif dwSkillLevel < 33 then
		skill.nChannelInterval = (16 + (dwSkillLevel - 8) * 5) * 1.1 / 2
	else
		skill.nChannelInterval = 144 * 1.1 / 2;     -- 通道技间隔时间
	end
