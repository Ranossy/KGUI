---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/藏剑/90级奇穴/100剑气必定会心.lua
-- 更新时间:	2015/5/29 11:47:43
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/藏剑/90级奇穴/100剑气必定会心.lua", -- 属性值1
		0														-- 属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(6141, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 6140, 1);		-- 设置1号位Buff
	--skill.BindBuff(1, 6141, 1);		-- 设置2号位Buff
...
--	print(111111)
	if player.nCurrentRage >= 100 then
		--player.AddBuff(dwCharacterID, player.nLevel, 6157, 1, 1)
		--print(22222)
		player.SetTimer(8, "scripts/skill/藏剑/90级奇穴/100剑气必定会心.lua", 0, 0)
		--[[
		if player.GetCDLeft(347) ~= 0 then
			print(33333)
			player.ClearCDTime(347)
			player.AddBuff(dwCharacterID, player.nLevel, 6141, 1, 1)
		end
		--]]
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	--print(33333)
	if player.GetCDLeft(347) ~= 0 and player.GetKungfuMount() ~= nil and player.GetKungfuMount().dwSkillID == 10144 then
			--print(4444)
			--print("----------------")
			player.ClearCDTime(347)
			player.AddBuff(dwCharacterID, player.nLevel, 6141, 1, 1)
