---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/藏剑/90级奇穴/闪避回血.lua
-- 更新时间:	2013/6/7 0:47:35
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/藏剑/90级奇穴/闪避回血.lua", -- 属性值1
		0															-- 属性值2
	);
    
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(2065, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL);		-- 需求自身Buff
...
function Apply(dwCharacterID)
	...
	local buff = player.GetBuff(6353, 1)
	if player.GetSkillLevel(6802) == 1 then
		if not player.GetBuff(7913,1) then
			if not buff or buff.nStackNum < 5 then
				--if player.GetBuff(2065)
				player.nCurrentLife = player.nCurrentLife + player.nMaxLife * 0.04
				player.nCurrentRage = player.nCurrentRage + player.nMaxRage * 0.05
				player.AddBuff(dwCharacterID, player.nLevel, 6353, 1, 1)
			end
		end
	end
end
