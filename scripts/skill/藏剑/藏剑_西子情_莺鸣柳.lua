---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/藏剑/藏剑_西子情_莺鸣柳.lua
-- 更新时间:	2013/5/29 21:09:18
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/藏剑/藏剑_西子情_莺鸣柳.lua", -- 属性值1
		0											                                -- 属性值2
	);	
	skill.BindBuff(1, 1728, 1);

	----------------- 设置CoolDown ---------------------------------------------------------------------
	--SetPublicCoolDown(id)设置公共CD，SetNormalCoolDown(posi, id)设置技能CD，其中posi为CoolDown位(共3位)，id需要查CoolDownList.tab
	--skill.SetPublicCoolDown(16);		
	skill.SetNormalCoolDown(1, 348);
...
function Apply(dwCharacterID)
	...
	if player and IsPlayer(dwCharacterID) then
		local buff5 = player.GetBuff(1924, 5)
		local buff6 = player.GetBuff(1924, 6)
		local buff7 = player.GetBuff(1924, 7)
		if buff5 or buff6 or buff7 then
			player.CastSkill(2315, 1, TARGET.PLAYER, player.dwID)
		end
	end
	player.AddBuff(dwCharacterID, player.nLevel, 1728, 1, 1)
	player.nCurrentRage = player.nMaxRage

	ModityCDToUI(player, 1663, 0, 0)
end
