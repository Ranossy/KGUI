...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/藏剑/藏剑_君子风_子技能_问水诀_啸日.lua",
		0
	);
...
function Apply(dwCharacterID)
	...
	if nShanjujianyiLvl== 0 or not player.bCanUseBigSword then
		 bResult = false;
	end
	if bResult then
		player.UmountKungfu()
		player.MountKungfu(10145,nShanjujianyiLvl)
		player.DelBuff(2245, 1)
		player.DelBuff(1685, 1)
		player.DelBuff(1686, 1)
		player.DelBuff(1802, 1)
		player.DelBuff(1802, 2)
		player.DelBuff(1802, 3)
		--[[
		if player.GetBuff(7700, 1) then
			local nbuff = player.GetBuff(7700, 1)
			local nbuffcount = nbuff.nStackNum
			if nbuffcount ~= 0 then
				for i = 1, nbuffcount do
					player.AddBuff(player.dwID,player.nLevel,7701,1,35)
				end
			end
			player.DelGroupBuff(7700,1)
		end
		--]]
	end
