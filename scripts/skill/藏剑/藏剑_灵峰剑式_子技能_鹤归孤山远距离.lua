...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/藏剑/藏剑_灵峰剑式_子技能_鹤归孤山远距离.lua", -- 属性值1
		0														-- 属性值2
	);  
	skill.BindBuff(1, 1904, 1);	
	----------------- 设置CoolDown ---------------------------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--[[
	if player.GetSkillLevel(6534) == 1 then
		player.nCurrentRage = player.nCurrentRage + 6
		target.AddBuff(dwSkillSrcID, player.nLevel, 6136, 1, 1)
	end
	--]]
end
--
