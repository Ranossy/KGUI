...
function Apply(dwCharacterID)
	...
	if player and IsPlayer(dwCharacterID) then
		...
		if character then 
			...
			if eTargetType == TARGET.PLAYER or eTargetType == TARGET.NPC then
				N = player.CastSkill(1891, 1, eTargetType, dwTargetID)
				--print(N)
				if N ~= 1 then
					player.CastSkill(1893, 1)			--子技能失败清母技能CD
				end		
			else 
				player.CastSkill(1893, 1)			--子技能失败清母技能CD
			end
