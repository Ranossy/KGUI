---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/低血量监控重置无相诀.lua
-- 更新时间:	2016/9/21 19:55:11
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.GetBuff(11294, 1) and player.nCurrentLife <= player.nMaxLife * 0.4 and player.nMoveState ~= MOVE_STATE.ON_DEATH and player.bFightState == true then
		local nLeftFrame = player.GetCDLeft(47)
		if nLeftFrame ~= 0 then
			player.ClearCDTime(47)
			player.AddBuff(player.dwID, player.nLevel, 11294, 1)
		end
	end
end
