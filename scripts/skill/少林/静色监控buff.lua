---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/静色监控buff.lua
-- 更新时间:	2015/9/17 1:14:40
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nlev = player.GetSkillLevel(242)	--捉影技能ID
	if nlev ~= 0 then
		local rest, nleftframe, ntotalframe = player.GetSkillCDProgress(242, nlev)
		if rest == true and nleftframe ~= 0 and player.bFightState == true and player.nMoveState ~= MOVE_STATE.ON_DEATH then
			if not player.GetBuff(9812, 1) then
				--print(111111)
				player.AddBuff(player.dwID, player.nLevel, 9812, 1, 4)
			end
		else
			--player.DelBuff(9812, 1)
		end
	end
end
