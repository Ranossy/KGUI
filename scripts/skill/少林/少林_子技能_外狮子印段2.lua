---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/少林_子技能_外狮子印段2.lua
-- 更新时间:	2015/9/17 0:00:34
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
--   		);
	...
	skill.AddAttribute(
		...
		"skill/少林/少林_子技能_外狮子印段2.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 2796, dwSkillLevel);			-- 设置Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(14755) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 9806, 1, 12)		--15秒40%反弹，40%几率解控
	elseif player.GetSkillLevel(14752) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 9809, 1, 12)		--15秒60%反弹
	else
		player.AddBuff(player.dwID, player.nLevel, 2797, 6, 12)		--10秒40%反弹
	end
end
