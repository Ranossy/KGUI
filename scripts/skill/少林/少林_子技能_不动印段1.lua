---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/少林_子技能_不动印段1.lua
-- 更新时间:	2015/9/16 20:42:13
-- 更新用户:	mengxiangfei11
-- 脚本说明:
...
function GetSkillLevelData(skill)
	...
	--  		);
   	...
   	skill.AddAttribute(
   		...
   		"skill/少林/少林_子技能_不动印段1.lua",											-- 属性值1
   		0											-- 属性值2
   		);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 401, dwSkillLevel)			-- 设置Buff
	--skill.BindBuff(1, 1093, dwSkillLevel);		-- 设置Debuff
...
function Apply(dwCharacterID)
	...
	if nlev ~= 0 then
		if player.GetSkillLevel(14747) == 1 then
			for i = 1, 8 do 
				player.AddBuff(player.dwID, player.nLevel, 401, nlev, 30)--延长10秒持续时间
			end
		else
			for i = 1, 8 do 
				player.AddBuff(player.dwID, player.nLevel, 401, nlev, 20)	--正常持续时间
			end
		end
	end
end
