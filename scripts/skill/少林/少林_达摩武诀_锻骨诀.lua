---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/少林_达摩武诀_锻骨诀.lua
-- 更新时间:	2015/9/17 9:09:56
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 0, nDamageRand = 0, nCostMana = 29}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(													--------特殊处理，锻骨决驱散兰催后，不会被沉默，20110725郑海星
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK,
...
function Apply(dwCharacterID)
	...
	player.AddBuff(player.dwID, player.nLevel, 1186, 1, 8)
	

	if player.GetSkillLevel(14795) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 9835, 1)
	end

	ModityCDToUI(player, 257, 0, 0)
end
