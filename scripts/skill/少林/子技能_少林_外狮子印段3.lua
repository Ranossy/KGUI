---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/子技能_少林_外狮子印段3.lua
-- 更新时间:	2015/9/17 0:03:23
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK, -- 属性作用模式
		ATTRIBUTE_TYPE.EXECUTE_SCRIPT, -- 魔法属性
		"skill/少林/子技能_少林_外狮子印段3.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 2797, dwSkillLevel);		-- 设置Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(14755) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 9806, 1, 18)		--时间延长并解控
	elseif player.GetSkillLevel(14752) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 9809, 1, 18)		--效果\时间延长50%
	else
		player.AddBuff(player.dwID, player.nLevel, 2797, 6, 18)		--10秒40%反弹
	end
end
