---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/少林_罗汉棍法_横扫六合.lua
-- 更新时间:	2013/4/24 20:56:40
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/少林/少林_罗汉棍法_横扫六合.lua", -- 属性值1
		0														-- 属性值2
	);
	...
	if dwSkillLevel <= 6 then
		skill.nCostManaBasePercent = 433*0.2*0.7;	-- 技能消耗的内力
...
function Apply(dwCharacterID)
	...
	if tplayer.GetSkillLevel(14745) == 1 then
		tplayer.CastSkill(3806, tlevel1)	--AOE人数判定添加标记buff B_3868
		local nbuff_mark = tplayer.GetBuff(3868, 1)
		if nbuff_mark then
			local nCount = nbuff_mark.nStackNum
			if nCount ~= 0 then
				for i = 1, nCount do
					--tplayer.CastSkill(14746, 1)	--命中一个目标清1秒调息时间
	...
	if tlevel2 == 1 then
		...
		if tplayer.GetSkillLevel(9280) == 1 and nSkill ~= nil and nSkill.dwSkillID == 10003 then
			...
			tplayer.CastSkill(3810, tlevel1)	--易经下,只打中一个目标
			tplayer.CastSkill(3808, tlevel1)	--双心法下正常命中目标
			tplayer.CastSkill(3812, tlevel1)	--洗髓下单目标
		end
