---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/90级奇穴/舍身期间给自己回血.lua
-- 更新时间:	2015/9/19 12:39:18
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/少林/90级奇穴/舍身期间给自己回血.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nbuff = player.GetBuff(1242, 1)
	if nbuff then
		local HS_ID = nbuff.dwSkillSrcID
		local target = GetPlayer(HS_ID)
		if target and target.GetSkillLevel(14796) == 1 and not target.GetBuff(7913,1) then
