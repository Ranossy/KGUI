---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/90级奇穴/低血量加血.lua
-- 更新时间:	2013/6/1 3:32:08
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/少林/90级奇穴/低血量加血.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(6208, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL );		-- 需求自身Buff
...
function Apply(dwCharacterID)
	...
	if player.GetSkillLevel(5933) == 1 and not player.GetBuff(7913,1) and player.dwShapeShiftID == 0 and player.nCurrentLife <= player.nMaxLife * 0.3 then
		player.AddBuff(dwCharacterID, player.nLevel, 6207, 1, 1)
		player.nCurrentLife = player.nCurrentLife + player.nMaxLife * 0.2
		player.AddBuff(dwCharacterID, player.nLevel, 6208, 1, 1)
	end
end
