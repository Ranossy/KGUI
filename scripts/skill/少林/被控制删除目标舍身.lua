...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.GetBuff(10174, 1) then
		...
		if nbuff then
			local playerID = nbuff.nCustomValue
			local target
			if IsPlayer(playerID) then
				target = GetPlayer(playerID)
			else
				target = GetNpc(playerID)
			end
			if not target then
				return
			end
			if target.GetBuffByOwner(1242, 1, player.dwID) then
				target.DelBuff(1242, 1)
			end
		end
		--]]
		local nbuff = player.GetBuff(10747, 1)
		if nbuff then
			...
			--[[
			local nbuff_1 = target.GetBuffByOwner(1242, 1, player.dwID)
			if nbuff_1 then
