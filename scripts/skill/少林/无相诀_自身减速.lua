...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
   skill.AddSlowCheckSelfBuff(3084, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);			-- 需求自身Buff
	...
	----------------- BUFF相关 -------------------------------------------------
    skill.BindBuff(1, 996, 1);		-- 设置1号位Buff
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame)
    ...
    if nLeftFrame == 0 then -- 正常结束，剩余时间为0
    	...
    	local result = character.DelBuff(996,1)      -- 非正常结束
    end
end;
function OnDetach(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	local player = GetPlayer(nCharacterID)
	if not player then
		return
	end
	if player.IsSkillRecipeActive(301,1) then
		local nTime = math.floor(nLeftFrame / 16, 1)
		for i = 1, nTime do
			player.CastSkill(15744, 1)	
		end
