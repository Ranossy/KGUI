---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/判定距离重置捉影.lua
-- 更新时间:	2013/10/30 10:34:23
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local Distance = GetCharacterDistance(dwCharacterID, dwSkillSrcID)
	if Distance >= 8 * 64 then
		if player.GetCDLeft(49) ~= 0 then
			if not player.GetBuff(7122, 1) then
				player.ClearCDTime(49)
				player.AddBuff(dwCharacterID, player.nLevel, 7122, 1, 1)
			end
		end
	end
end
