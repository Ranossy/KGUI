---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/少林_罗汉金身被击回复豆并且添加伤害buff.lua
-- 更新时间:	2015/9/28 15:56:59
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/少林/少林_罗汉金身被击回复豆并且添加伤害buff.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nMark = 0
	if player.GetBuff(9804, 1) or player.GetBuff(2795, 6) then
		nMark = 2000
	elseif player.GetBuff(9807, 1) or player.GetBuff(9805, 1) or player.GetBuff(2796, 6) then
		nMark = 3000
	elseif player.GetBuff(9806, 1) or player.GetBuff(2797, 6) then
		nMark = 4000
	elseif player.GetBuff(9808, 1) then
		nMark = 4500
	elseif player.GetBuff(9809, 1) then
		nMark = 6000
	end

	if nRnd <= nMark and player.GetSkillLevel(14820) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 9931, 1)	--添加伤害buff
		player.CastSkill(14956, 1)	--回豆
	end
end
