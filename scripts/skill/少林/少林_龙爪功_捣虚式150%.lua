---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/少林_龙爪功_捣虚式150%.lua
-- 更新时间:	2013/7/25 15:26:51
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/少林/少林_龙爪功_捣虚式150%.lua", -- 属性值1
		0														-- 属性值2
		);
	...
	if dwSkillLevel <= 9 then
		skill.nChannelInterval = (200 + (dwSkillLevel-1) * 5)*1.15;
	else
		skill.nChannelInterval = 240 * 1.2*1.15;	 			-- 通道技持续时间，单位帧数 
	end
	...
	--skill.nPrepareFrames    = 120;										--吟唱帧数,16帧等于1秒
...
function Apply(dwCharacterID)
	...
	if player.GetSkillLevel(6788) == 1 then
		player.CastSkill(6787, 1)
		player.DelBuff(3880, 1)
		player.DelBuff(3880, 1)
	end
