---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/子技能_少林_不动印段3.lua
-- 更新时间:	2015/9/16 20:49:17
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
--   		);
	skill.AddAttribute(
   	...
   	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK,					-- 属性作用模式
		ATTRIBUTE_TYPE.EXECUTE_SCRIPT,								-- 魔法属性
		"skill/少林/子技能_少林_不动印段3.lua",											-- 属性值1
		0											-- 属性值2
		);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 403, dwSkillLevel)			-- 设置Buff
	--skill.BindBuff(1, 1095, dwSkillLevel);			-- 设置Debuff
...
function Apply(dwCharacterID)
	...
	if nlev ~= 0 then
		...
		for i = 1, 8 do 
				player.AddBuff(player.dwID, player.nLevel, 403, nlev, 30)--延长10秒持续时间
			end
		else
			for i = 1, 8 do 
				player.AddBuff(player.dwID, player.nLevel, 403, nlev, 20)	--正常持续时间
			end
		end
	end
end
