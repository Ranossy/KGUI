---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/少林/少林_龙爪功_二业依缘.lua
-- 更新时间:	2015/10/23 1:58:11
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
   ...
   skill.AddAttribute(
		...
		"skill/少林/少林_龙爪功_二业依缘.lua",											-- 属性值1
		0											-- 属性值2
		);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(10023, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	skill.AddSlowCheckSelfBuff(10024, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 10024, 1);		-- 设置1号位Buff
	...
	skill.SetNormalCoolDown(1, 982);	--技能普通CD
	...
	skill.nCostManaBasePercent = 180;
	----------------- 聚气相关 -------------------------------------------------
...
function OnSkillLevelUp(skill, player)
	if player.GetSkillLevel(15166) ~= 1 then
		player.LearnSkillLevel(15166, 1, player.dwID)
	end
end
...
function OnSkillForgotten(skill, player)
	if player.GetSkillLevel(15166) == 1 then
		player.ForgetSkill(15166)
	end

	if player.GetBuff(10023, 1) then
		player.DelBuff(10023, 1)
	end

	if player.GetBuff(10024, 1) then
		player.DelBuff(10024, 1)
	end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.SetTimer(4, "scripts/skill/少林/少林_龙爪功_二业依缘.lua", 0, 0)
end
