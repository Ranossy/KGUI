---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/技能道具/成就系统趣味道具_笛子.lua
-- 更新时间:	12/07/09 19:12:41
-- 更新用户:	liuhengzhi1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function OnUse(player, item, dwSourceBox, dwSourceSlot, dwTargetBox, dwTargetSlot)	
	...
	--player.DoAction(0, 10965) --播放吹笛子动作
	if player.CheckEmotionAction(38) == true then
		player.SendSystemMessage(GetEditorString(8, 2481))
	else
		player.AddEmotionAction(38)
	end
	--player.AddAchievementCount(321, 1)
	return false, true
end
