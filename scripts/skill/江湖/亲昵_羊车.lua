---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/江湖/亲昵_羊车.lua
-- 更新时间:	2015/3/29 15:46:54
-- 更新用户:	zhangyan3
-- 脚本说明:
----------------------------------------------------------------------<
-------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 751);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local target1 = GetPlayer(caster.dwFollowID)
	if target1 then
		if target1.nRoleType == ROLE_TYPE.STANDARD_MALE then
			caster.CastSkill(9423, 1)
		elseif target1.nRoleType == ROLE_TYPE.STANDARD_FEMALE then
			caster.CastSkill(9424, 1)
		elseif target1.nRoleType == ROLE_TYPE.LITTLE_BOY then
			caster.CastSkill(9425, 1)
		else
			caster.CastSkill(9446, 1)
		end
	end
	local target2 = GetPlayer(caster.dwControllerID)
	if target2 then
		if target2.nRoleType == ROLE_TYPE.STANDARD_MALE then
			caster.CastSkill(9447, 1)
		elseif target2.nRoleType == ROLE_TYPE.STANDARD_FEMALE then
			caster.CastSkill(9448, 1)
		elseif target2.nRoleType == ROLE_TYPE.LITTLE_BOY then
			caster.CastSkill(9449, 1)
		else
			caster.CastSkill(9450, 1)
		end
	end
