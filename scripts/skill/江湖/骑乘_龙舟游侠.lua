---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/江湖/骑乘_龙舟游侠.lua
-- 更新时间:	2017/5/20 16:47:44
-- 更新用户:	xutong1-pc
-- 脚本说明:
----------------------------------------------------------------------<
-------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 244, 8);		-- 设置1号位Buff
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if player.nMoveState ~= MOVE_STATE.ON_FLOAT then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_RED", GetEditorString(7, 2875), 5)
		return false;
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not IfBattle(player) then--用于判断本场景的战斗是否开启 true:进行中。false：关闭。
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_RED", GetEditorString(11, 6659), 5)
		return
	end
	if not LongZhou_IfBoatCorrect_ForSkill(player, 11958) then
		return
	end
	if player.GetBuff(12004, 1) then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_RED", GetEditorString(11, 6688), 5)
		return
	end
	if player.nMoveState == MOVE_STATE.ON_FLOAT then
		player.SetDynamicSkillGroup(338)
		player.AddBuff(0, 99, 244, 18)
	end
