---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/江湖/90级奇穴切换用.lua
-- 更新时间:	2017/3/28 22:06:14
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
		if player.GetBuff(11840, 1) then
			...
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(11, 4757), 4)
			return false
		end
		return nPreResult;
	end
...
function Apply(dwCharacterID)
	...
	player.SelectNewTalentPoint(nX, nY)
	--player.PlaySfx(511,player.nX,player.nY,player.nZ)
end
