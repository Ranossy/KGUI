...
function GetSkillLevelData(skill)
    ...
    skill.AddSlowCheckSelfBuff(7831, 30, BUFF_COMPARE_FLAG.LESS, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--  skill.BindBuff(1, 7831, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if height then
		--local npc = scene.CreateNpc(36482, player.nX, player.nY, player.nZ, 0, -1)
		Creat1(player)
	end
end
