---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/江湖/90级奇穴整体切换用.lua
-- 更新时间:	2016/11/17 10:24:24
-- 更新用户:	chenchen6-pc
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
		if player.GetBuff(11840, 1) then
			player.SendSystemMessage(GetEditorString(11, 4758))
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(11, 4758), 4)
			return false
		end
		return nPreResult;
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(6004, 1) then
		...
		nCount = nbuff.nCustomValue
		player.ChangeNewTalentSet(nCount)
	end

	if player.GetBuff(6004, 1) then
		player.DelBuff(6004, 1)
	end
	--用于霸刀技能切换后的秘籍替换
	player.SetTimer(4, "scripts/skill/江湖/90级奇穴整体切换用.lua", 0, 0)
end
...
function OnTimer(player, nParam1, nParam2)
	...
	if player.dwForceID == 23 then
		...
		local nCurrentSkill = 0
		local nFormerSkill = 0
		...
		if nCurrentSkill == 0 or nFormerSkill == 0 then
			return
		end
		--print("nCurrentSkill"..nCurrentSkill)
		--print("nFormerSkill"..nFormerSkill)

		--先判定当前技能是否有秘籍激活
		local nCurrentCount = 0
		local nFormerCount = 0
		local lv = player.GetSkillLevel(nCurrentSkill)

		for i = 1, 8 do
			if player.IsSkillRecipeActive(nTable[nCurrentSkill][i], 1) then
				nCurrentCount = nCurrentCount + 1
			end

			if player.IsSkillRecipeActive(nTable[nFormerSkill][i], 1) then
				nFormerCount = nFormerCount + 1
			end
		end
		--print("nCurrentCount"..nCurrentCount)
		--print("nFormerCount"..nFormerCount)
		if nCurrentCount ~= 0 then
			--print(111111)
			--当前技能秘籍有激活
			return
		elseif nFormerCount ~= 0 then
			--print(2222)
			if lv ~= 0 then
				RecipeCopy(nFormerSkill, lv, player, nCurrentSkill)
			end
		end
	end
end
