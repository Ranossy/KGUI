---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/江湖/骑乘_龙舟隐者.lua
-- 更新时间:	2017/5/20 16:47:47
-- 更新用户:	xutong1-pc
-- 脚本说明:
----------------------------------------------------------------------<
-------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if player.nMoveState ~= MOVE_STATE.ON_FLOAT then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_RED", GetEditorString(7, 2875), 10)
		return false;
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not LongZhou_IfBoatCorrect_ForSkill(player, 11959) then
		return
	end
	if player.nMoveState == MOVE_STATE.ON_FLOAT then
		player.SetDynamicSkillGroup(337)
		player.AddBuff(0, 99, 244, 17)
	end
