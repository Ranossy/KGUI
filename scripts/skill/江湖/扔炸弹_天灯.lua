...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nDir = player.nFaceDirection + 64
	if nDir > 255 then
		nDir = nDir - 255
	end
--	local doodad = scene.CreateDoodad(4725, player.nX, player.nY, player.nZ, player.nFaceDirection, "FZHD_shenjianxinbaoxiang_"..nCount)
	local npc = scene.CreateNpc(36490, player.nX, player.nY, player.nZ + 3500, nDir,-1)
	if npc then
		npc.AddBuff(0, 0, 4910, 1, 28800)
	end	
end
