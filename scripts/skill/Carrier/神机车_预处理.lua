---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/神机车_预处理.lua
-- 更新时间:	2014/4/15 23:24:24
-- 更新用户:	gaoweilong1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Carrier/神机车_预处理.lua", -- 属性值1
		0														-- 属性值2
	);
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	...
	if not buff_GuiFu and not buff_CuiCheng and not buff_LiQi then
		player.CastSkill(8969, 1, type, dwID)
		player.CastSkill(9041, 1, type, dwID)
	elseif buff_GuiFu and not buff_CuiCheng and not buff_LiQi then
		player.CastSkill(8969, 2, type, dwID)
		player.CastSkill(9041, 2, type, dwID)
	elseif buff_GuiFu and buff_CuiCheng and not buff_LiQi then
		player.CastSkill(8969, 2, type, dwID)
		player.CastSkill(9041, 3, type, dwID)
	elseif buff_GuiFu and buff_CuiCheng and buff_LiQi then
		player.CastSkill(8969, 3, type, dwID)
		player.CastSkill(9041, 3, type, dwID)
	end
	
	if player.nCurrentMana < 100 then
		player.nCurrentMana = player.nCurrentMana + 1
	end
