---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/摧城车_战斗状态.lua
-- 更新时间:	2014/6/30 15:48:02
-- 更新用户:	lilin-kingsoft
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.dwShapeShiftID == 4 then
		player.Stop()
		...
		if scene then
			if player.nCamp == 1 then
				scene.CreateNpc(36823, player.nX, player.nY, player.nZ, player.nFaceDirection, -1, player.dwID .. 36718)
			else
				scene.CreateNpc(36816, player.nX, player.nY, player.nZ, player.nFaceDirection, -1, player.dwID .. 36718)
			end
		end
		--player.SetTimer(30, "scripts/skill/Carrier/摧城车_战斗状态.lua", CurrentLife, 0)
		player.SetTimer(32, "scripts/skill/Carrier/摧城车_战斗状态.lua", CurrentLife, 0)
	end
end
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	if nParam2 == 0 then
		...
		player.StartShapeShift(5)
		player.SetDynamicSkillGroup(tCarrier[5].SkillBarID)
