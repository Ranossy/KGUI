---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/攻防神机台_穿心弩子技能.lua
-- 更新时间:	2015/10/9 21:41:35
-- 更新用户:	wangsongtao-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Carrier/攻防神机台_穿心弩子技能.lua", -- 属性值1
		0														-- 属性值2
	);
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	...
	local buff_RanHuo = player.GetBuff(7756, 1)
	player.SetTarget(type, dwID)
	player.nCurrentLife = math.max(1, player.nCurrentLife - player.nMaxLife * 0.005)
	...
	if buff_RanHuo then
		player.CastSkill(14481, nTargetPlayerLevel, type, dwID)
		player.CastSkill(14483, nTargetNpcLevel, type, dwID)
	else
		player.CastSkill(14479, nTargetPlayerLevel, type, dwID)
		player.CastSkill(14480, nTargetNpcLevel, type, dwID)
	end
