---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/神机车_过热.lua
-- 更新时间:	2014/3/12 15:34:15
-- 更新用户:	gaoweilong1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.dwShapeShiftID == 2 then
		player.Stop()
		player.AddBuff(0, 99, 7616, 1, 100)
		local scene = player.GetScene()
		if scene then
			local npc = scene.GetNpcByNickName(player.dwID .. 27617)
			if npc then
				npc.SetDisappearFrames(1)
			end
		end
		player.EndShapeShiftNoDelay()
		player.StartShapeShift(1)
		player.SetTimer(1, "scripts/skill/Carrier/神机车_过热.lua", 1, 0)
	end
end
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	if nParam1 == 1 then
		...
		player.SetTimer(108, "scripts/skill/Carrier/神机车_过热.lua", 2, 0)
	elseif nParam1 == 2 then
		...
		player.CastSkill(9001, nTargetPlayerLevel)
		player.CastSkill(9058, nTargetNpcLevel)
