---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/神机车_战斗状态.lua
-- 更新时间:	2014/3/14 9:27:29
-- 更新用户:	gaoweilong1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.dwShapeShiftID == 1 then
		player.Stop()
		...
		if scene then
			if player.nCamp == 1 then
				scene.CreateNpc(27617, player.nX, player.nY, player.nZ, player.nFaceDirection, -1, player.dwID .. 27617)
			else
				scene.CreateNpc(36467, player.nX, player.nY, player.nZ, player.nFaceDirection, -1, player.dwID .. 27617)
			end
		end
		player.SetTimer(28, "scripts/skill/Carrier/神机车_战斗状态.lua", CurrentLife, 0)
	end
end
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	if nParam2 == 0 then
		...
		player.StartShapeShift(2)
		player.SetDynamicSkillGroup(tCarrier[2].SkillBarID)
