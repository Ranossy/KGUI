---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/神机台_照明弹.lua
-- 更新时间:	2014/4/20 21:48:58
-- 更新用户:	gaoweilong1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Carrier/神机台_照明弹.lua", -- 属性值1
		0														-- 属性值2
	);
	...
	skill.SetNormalCoolDown(1, 722);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.CastSkillXYZ(9151, 1, nX, nY, nZ)--子弹表现
	local npc = scene.CreateNpc(27566, nX, nY, nZ, nF, 16)
	if npc then
		npc.AddBuff(0, 99, 3246, 1, 999)
		npc.SetPosition(nX, nY, nZ)
		player.CastSkillXYZ(9168, 1, nX, nY, nZ)
	end
	
	local buff = player.GetBuff(7713, 1)
	if buff then
		npc = scene.CreateNpc(36454, nX, nY, nZ, nF, 160)
		...
		if npc then
			npc.SetCustomUnsigned1(0, 2)
		end
	else
		npc = scene.CreateNpc(36454, nX, nY, nZ, nF, 80)
		if player.nCamp == 1 then
			npc.SetForceID(50)
		elseif player.nCamp == 2 then
			npc.SetForceID(49)
