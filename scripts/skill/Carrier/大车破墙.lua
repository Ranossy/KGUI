---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/大车破墙.lua
-- 更新时间:	2016/12/26 17:30:33
-- 更新用户:	wangsongtao-pc
-- 脚本说明:	skill 17211
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local tMidFollow, tLeft, tRight = CustomFunction.GetFollowsPosition(player.nX, player.nY, math.fmod(player.nFaceDirection + 128, 256), nDis, 2 * 64)
	if tMidFollow.nX < 0 or tMidFollow.nY < 0 or tLeft.nX < 0 or tLeft.nY < 0 or tRight.nX < 0 or tRight.nY < 0 then
		return
	end
	local QQ_AllList = {}
	QQ_AllList[1] = player.GetAdvancedDynamicObstacleList(player.nX, player.nY, tMidFollow.nX, tMidFollow.nY, player.nZ - 2 * 64 * 8, player.nZ + 8 * 64 * 8)  -- 正前方
	QQ_AllList[2] = player.GetAdvancedDynamicObstacleList(player.nX, player.nY, tLeft.nX, tLeft.nY, player.nZ - 2 * 64 * 8, player.nZ + 8 * 64 * 8) -- 左边
	QQ_AllList[3] = player.GetAdvancedDynamicObstacleList(player.nX, player.nY, tRight.nX, tRight.nY, player.nZ - 2 * 64 * 8, player.nZ + 8 * 64 * 8) -- 右边前方
	for i = 1, #QQ_AllList do
		local QQ_List = QQ_AllList[i]
		if #QQ_List ~= 0 then
			for i = 1, #QQ_List do
