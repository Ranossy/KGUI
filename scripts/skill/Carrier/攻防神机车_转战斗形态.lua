---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/攻防神机车_转战斗形态.lua
-- 更新时间:	2015/9/8 0:58:28
-- 更新用户:	wangsongtao-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.dwShapeShiftID == 9 then
		player.Stop()
		...
		if scene then
			if player.nCamp == 1 then
				npcDizuo = scene.CreateNpc(45173, player.nX, player.nY, player.nZ, player.nFaceDirection, -1, "GFzaijudizuo" .. player.dwID)
			else
				npcDizuo = scene.CreateNpc(45172, player.nX, player.nY, player.nZ, player.nFaceDirection, -1, "GFzaijudizuo" .. player.dwID)
			end
			if npcDizuo then
				npcDizuo.SetCustomUnsigned4(0, player.dwID)
			end
		end
		player.SetTimer(28, "scripts/skill/Carrier/攻防神机车_转战斗形态.lua", CurrentLife, 0)
	end
end
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	if nParam2 == 0 then
		...
		player.StartShapeShift(10)
		player.SetDynamicSkillGroup(tCarrier[10].SkillBarID)
