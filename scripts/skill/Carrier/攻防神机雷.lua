---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/攻防神机雷.lua
-- 更新时间:	2015/10/22 16:37:06
-- 更新用户:	wangsongtao-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nCount = player.GetItemAmount(ITEM_TABLE_TYPE.OTHER, 24422)
	if nCount < 1 then
		return
	end
	player.CostItemInAllPackage(ITEM_TABLE_TYPE.OTHER, 24422, 1)
	if not player.IsAchievementAcquired(5172) then
		player.AddAchievementCount(5172, 1)
	end
	local nX, nY, nZ = target.nX, target.nY, target.nZ
	local scene = player.GetScene()
	if not scene then
		return
	end
	local npcTemp = scene.CreateNpc(27566, nX, nY, nZ, 1, 18)
	if npcTemp then
		npcTemp.AddBuff(0, 99, 3246, 1, 999)
		npcTemp.SetPosition(nX, nY, nZ)
		player.SetTimer(16, "scripts/skill/Carrier/攻防神机雷.lua", npcTemp.dwID, 0)
	end
end
...
function OnTimer(player, nParam1, nParam2)
	local npcTemp = GetNpc(nParam1)
	if not npcTemp then
		return
	end
	local nX, nY, nZ = npcTemp.nX, npcTemp.nY, npcTemp.nZ

	local dwSkillID = 14565
	local dwSkillID_Up = 14566
	player.CastSkillXYZ(dwSkillID, 1, nX, nY, nZ)
	player.CastSkillXYZ(dwSkillID_Up, 1, nX, nY, nZ)

	player.PlaySfx(649, nX, nY, nZ)
