---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/神机台_扫射子技能.lua
-- 更新时间:	2014/4/16 10:16:36
-- 更新用户:	gaoweilong1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Carrier/神机台_扫射子技能.lua", -- 属性值1
		0														-- 属性值2
	);
	...
	----------------- 设置Cool down --------------------------------------------
	-- 公共CD
	skill.SetPublicCoolDown(8);							-- 公共CD 1.5秒
	...
	skill.nHeight = 70 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if target then
		if GetRelation(player.dwID, target.dwID) == RELATION_TYPE.ENEMY or GetRelation(player.dwID, target.dwID) == RELATION_TYPE.NEUTRALITY then
			if GetCharacterDistance(player.dwID, target.dwID) <= 2240 then
				local buff1 = player.GetBuff(7372, 1)
				local buff2 = player.GetBuff(7373, 1)
				buff1.nCustomValue = GetCharacterDistance(player.dwID, target.dwID)
				buff2.nCustomValue = target.dwID
				player.SetTimer(2, "scripts/skill/Carrier/神机台_扫射子技能.lua", 0, 0)
			end		
		end		
	end
	return nPreResult;
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	local buff_7373 = player.GetBuff(7373, 1)
	if not buff_7373 then
		return
	end
	local dwID = buff_7373.nCustomValue
	 local type = nil
	...
	local buff_GuiFu = player.GetBuff(7714, 1)
	local buff_CuiCheng = player.GetBuff(7716, 1)
	local buff_LiQi = player.GetBuff(7718, 1)
	local buff_Fire = player.GetBuff(9085, 1)
	
	local nTargetNpcLevel = 1
	local nTargetPlayerLevel = 1
	
	if buff_GuiFu then
		nTargetNpcLevel = nTargetNpcLevel + 2 ^ 1
		nTargetPlayerLevel = nTargetPlayerLevel + 2 ^ 1
	end
	if buff_CuiCheng then
		nTargetNpcLevel = nTargetNpcLevel + 2 ^ 2
	end
	if buff_LiQi then
		nTargetPlayerLevel = nTargetPlayerLevel + 2 ^ 2
	end
	
	if buff_Fire then
		player.CastSkill(9082, nTargetPlayerLevel, type, dwID)
	 	player.CastSkill(9084, nTargetNpcLevel, type, dwID)
	 else
	 	player.CastSkill(9081, nTargetPlayerLevel, type, dwID)
	 	player.CastSkill(9083, nTargetNpcLevel, type, dwID)
	end
