---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/黑戈壁神机车_箭弩连射子技能.lua
-- 更新时间:	2015/10/1 22:02:52
-- 更新用户:	wangsongtao-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Carrier/黑戈壁神机车_箭弩连射子技能.lua", -- 属性值1
		0														-- 属性值2
	);
	...
	-- skill.nHeight = 80 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if nDistance > 64 * 38 then
		return
	end

	if nDistance >= buff1.nCustomValue then
		local npc = GetNpc(buff2.nCustomValue)
		if npc and npc.dwTemplateID == 27566 then
			if nDistance - buff1.nCustomValue < 320 then
		...
		if npc and npc.dwTemplateID == 27566 then
			if buff1.nCustomValue - nDistance < 320 then
				return
			end
		end
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	...
	player.CastSkill(15007, nTargetPlayerLevel, type, dwID)
	player.CastSkill(15008, nTargetNpcLevel, type, dwID)

	if player.nCurrentMana < 1000 then
		player.nCurrentMana = player.nCurrentMana + 10
	end

	if player.nCurrentMana >= 1000 then
		player.AddBuff(0, 99, 7615, 1, 54)
		player.Stop()
		player.DoAction(0, 10102)
		player.CastSkill(15012, 1)
	end

	local buff_7373 = player.GetBuff(7373, 1)
	local buff_7372 = player.GetBuff(7372, 1)
	if buff_7373 then
		buff_7373.nCustomValue = 0
	end
	if buff_7372 then
		buff_7372.nCustomValue = 0
	end
end
