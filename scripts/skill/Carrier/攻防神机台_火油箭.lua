---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/攻防神机台_火油箭.lua
-- 更新时间:	2015/9/26 19:35:02
-- 更新用户:	wangsongtao-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Carrier/攻防神机台_火油箭.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID)
	...
	if buff_RanHuo then
		player.DelBuff(7756, 1)
	end
	player.CastSkillXYZ(14979, 1, nX, nY, nZ)--子弹表现
	local npcFire = scene.CreateNpc(46879, nX, nY, nZ, 0, 5 * GLOBAL.GAME_FPS)
	if npcFire then
		npcFire.SetCustomUnsigned4(0, player.dwID)
		--npcFire.SetForceID(player.nCamp == 1 and 50 or 49)
	end
end
