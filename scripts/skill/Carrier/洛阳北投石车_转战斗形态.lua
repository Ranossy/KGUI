---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/洛阳北投石车_转战斗形态.lua
-- 更新时间:	2016/2/24 17:20:36
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Carrier/洛阳北投石车_转战斗形态.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.dwShapeShiftID == 17 then
		player.Stop()
		...
		local npcDizuo
		if scene then
			npcDizuo = scene.CreateNpc(50967, player.nX, player.nY, player.nZ, player.nFaceDirection, - 1, "GFzaijudizuo" .. player.dwID)
			print(player.dwID)
			if npcDizuo then
				npcDizuo.SetCustomUnsigned4(0, player.dwID)
			end
		end
		player.SetTimer(32, "scripts/skill/Carrier/洛阳北投石车_转战斗形态.lua", CurrentLife, 0)
	end
end
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	if nParam2 == 0 then
		...
		player.StartShapeShift(18)
		player.SetDynamicSkillGroup(tCarrier[18].SkillBarID)
