---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/神机台_点火.lua
-- 更新时间:	2014/4/19 14:58:58
-- 更新用户:	gaoweilong1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 721);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.nCurrentMana < 100 then
		player.nCurrentMana = player.nCurrentMana + 5
	end

	if player.nCurrentMana >= 100 then
		local buff = player.GetBuff(7756, 1)
		if not buff then
			player.AddBuff(0, 99, 7756, 1)
		end
	end
end
