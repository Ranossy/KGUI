---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/攻防神机雷子技能高伤.lua
-- 更新时间:	2015/9/9 0:55:37
-- 更新用户:	wangsongtao-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
local nDamageStandard = 240000

tSkillData =
{
	{nDamageBase = nDamageStandard, nDamageRand = 0, nCostMana = 0}, --level 1 原始
	{nDamageBase = nDamageStandard, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = nDamageStandard * 1.1, nDamageRand = 0, nCostMana = 0}, --level 3 鬼斧 提高10%伤害
	{nDamageBase = nDamageStandard * 1.1, nDamageRand = 0, nCostMana = 0}, --level 4
	{nDamageBase = nDamageStandard * 1.5, nDamageRand = 0, nCostMana = 0}, --level 5 利器 提高50%伤害
	{nDamageBase = nDamageStandard * 1.5, nDamageRand = 0, nCostMana = 0}, --level 6
	{nDamageBase = nDamageStandard * 1.6, nDamageRand = 0, nCostMana = 0}, --level 7 鬼斧+利器 提高 60%伤害
};
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/Carrier/攻防神机雷子技能高伤.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(7734, 1, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	-- 攻防任务16416
	local nQuestIdx = player.GetQuestIndex(16416)
	if nQuestIdx and not player.GetBuff(11591, 1) then
		player.AddBuff(0, 99, 11591, 1, 1) -- 加个短CD防止打到多个人加多次
		local nValue = player.GetQuestValue(nQuestIdx, 0)
		player.SetQuestValue(nQuestIdx, 0, nValue + 1)
		if nValue >= 2 then
			player.ForceFinishQuest(16416)
		end
	end
end
