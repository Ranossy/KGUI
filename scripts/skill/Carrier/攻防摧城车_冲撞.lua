---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/攻防摧城车_冲撞.lua
-- 更新时间:	2015/9/12 22:28:46
-- 更新用户:	wangsongtao-pc
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 732);	--技能普通CD
	...
	skill.nHeight = 16 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	local npc = scene.GetNpcByNickName("GFzaijudizuo" .. player.dwID)
	if not npc then
		return nPreResult
	end

	player.PlayCameraAnimation(108, 0)
	player.AddBuff(0, 99, 8021, 1, 6)
	npc.DoAction(0, 10069)
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	npc.DoAction(0, 10068)
	local dwSkillID = 14689
	local dwSkillID_Up = 14690
	...
	player.CastSkillXYZ(dwSkillID, nTargetPlayerLevel, player.nX, player.nY, player.nZ)
	player.CastSkillXYZ(dwSkillID_Up, nTargetNpcLevel, player.nX, player.nY, player.nZ)

	player.SetTimer(38, "scripts/skill/Carrier/攻防摧城车_冲撞.lua", npc.dwID, 0)
end
...
function OnTimer(player, dwNpcID, nParam2)
