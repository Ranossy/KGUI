---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/Carrier/攻防摧城车_转战斗状态.lua
-- 更新时间:	2015/9/13 10:30:39
-- 更新用户:	wangsongtao-pc
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
Include("scripts/Carrier/Include/Data.lua")
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 734);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local CurrentLife = player.nCurrentLife
	if player and player.dwShapeShiftID == 12 then
		player.Stop()
		player.AddBuff(0, 99, 7616, 1, 100)
		player.AddBuff(0, 99, 7615, 1, 100)
		player.Stop()
		player.PlayCameraAnimation(107, 0)
		local scene = player.GetScene()
		local npcDizuo
		if scene then
			if player.nCamp == 1 then
				npcDizuo = scene.CreateNpc(46195, player.nX, player.nY, player.nZ, player.nFaceDirection, -1, "GFzaijudizuo" .. player.dwID)
			else
				npcDizuo = scene.CreateNpc(46194, player.nX, player.nY, player.nZ, player.nFaceDirection, -1, "GFzaijudizuo" .. player.dwID)
			end
			if npcDizuo then
				npcDizuo.SetCustomUnsigned4(0, player.dwID)
			end
		end
		player.SetTimer(32, "scripts/skill/Carrier/攻防摧城车_转战斗状态.lua", CurrentLife, 0)
	end
end
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	if nParam2 == 0 then
		...
		player.StartShapeShift(13)
		player.SetDynamicSkillGroup(tCarrier[13].SkillBarID)
		player.DelBuff(7616, 1)
		player.DelBuff(7615, 1)
		player.nCurrentLife = nParam1
		--player.SetTurnRange(player.nFaceDirection, 21, 64)
	end
end
