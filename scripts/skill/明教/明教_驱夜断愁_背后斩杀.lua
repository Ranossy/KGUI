---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/明教_驱夜断愁_背后斩杀.lua
-- 更新时间:	2013/6/4 2:45:04
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		tSkillData[dwSkillLevel].nDamageBase * 1.7/5,
		0
	);
	...
	skill.AddAttribute(
		...
		tSkillData[dwSkillLevel].nDamageRand * 1.7/5,
		0
	);
...
--增加武器伤害 
	...
	if dwSkillLevel < 10 then
		skill.nChannelInterval = 195;
	elseif dwSkillLevel < 29 then
		skill.nChannelInterval = 195 + (dwSkillLevel - 9) * 7
	else
		skill.nChannelInterval =260*1.3;     -- 通道技间隔时间   
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(5989) == 1 then
		target.AddBuff(dwSkillSrcID, player.nLevel, 562, 24, 1)
	end
end
