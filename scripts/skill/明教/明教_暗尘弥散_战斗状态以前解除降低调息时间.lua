---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/明教_暗尘弥散_战斗状态以前解除降低调息时间.lua
-- 更新时间:	2017/4/15 21:47:57
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	if scene.bIsArenaMap then
		player.SetTimer(8, "scripts/skill/明教/明教_暗尘弥散_战斗状态以前解除降低调息时间.lua", 0, 0)
	end
	--]]

	if player.GetSkillLevel(14695) == 1 and nLeftFrame ~= 0 then
		local nTime = math.floor(nLeftFrame / 16, 1)
		for i = 1, nTime do
			player.CastSkill(14696, 1)	--每次降低5秒调息时间
		end
	end
	--[[
	local scene = player.GetScene()
...
function OnTimer(player, nP1, nP2)
	--[[
	if nP1 == 0 then
		player.SetTimer(16,"scripts/skill/明教/明教_暗尘弥散_战斗状态以前解除降低调息时间.lua",1,0)
	end
