---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/明教_添加12层奇穴判定buff.lua
-- 更新时间:	2015/9/26 13:24:19
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	if dwSkillLevel == 1 then
		skill.AddAttribute(
			...
			"skill/明教/明教_添加12层奇穴判定buff.lua", -- 属性值1
			0											-- 属性值2
			);
	end
	...
	if dwSkillLevel == 2 then
		skill.AddAttribute(
			ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK, -- 属性作用模式
			ATTRIBUTE_TYPE.EXECUTE_SCRIPT, -- 魔法属性
			"skill/明教/90级奇穴/添加12层奇穴月系.lua", -- 属性值1
			0											-- 属性值2
			);
	end
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(14915) == 1 or player.GetSkillLevel(14922) == 1 then
		if not player.GetBuff(9909, 1) then
			if player.GetBuff(9911, 1) then
				player.AddBuff(player.dwID, player.nLevel, 9909, 1)
				player.DelBuff(9911, 1)
			else
				player.AddBuff(player.dwID, player.nLevel, 9910, 1)
			end
		end
	end
end
