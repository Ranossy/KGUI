---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/明教_幽月轮.lua
-- 更新时间:	2015/2/4 13:59:15
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	...
	{nLunarDamage = 285 * 2 * 0.98, nLunarDamageRand = 285 * 2 * 0.04, nCostMana = 186},
};
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK, --第一下
	...
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK, --第二下
		ATTRIBUTE_TYPE.CAST_SKILL,
		4025,
		dwSkillLevel
	);
	...
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK, --第三下
		ATTRIBUTE_TYPE.CAST_SKILL,
		4026,
		dwSkillLevel
		);
...
function OnSkillLevelUp(skill, player)
	if skill.dwLevel == skill.dwMaxLevel then
		player.AcquireAchievement(3285)
	end
end
...
function Apply(dwCharacterID)
	...
	local nlev = player.GetSkillLevel(3959)
	if nlev == 0 then
		return
	end
	...
	if player.GetSkillLevel(5988) == 1 then
		--有奇穴\释放眩晕目标三段攻击
		player.CastSkill(13663, nlev, player.GetSkillTarget())
		player.CastSkill(13664, nlev, player.GetSkillTarget())
		player.CastSkill(13665, nlev, player.GetSkillTarget())
		--有奇穴打非眩晕目标
		player.CastSkill(13666, nlev, player.GetSkillTarget())
		player.CastSkill(13667, nlev, player.GetSkillTarget())
		player.CastSkill(13668, nlev, player.GetSkillTarget())
	else
		--释放无奇穴的普通目标三段攻击
		player.CastSkill(4024, nlev, player.GetSkillTarget())
		player.CastSkill(4025, nlev, player.GetSkillTarget())
		player.CastSkill(4026, nlev, player.GetSkillTarget())
	end
end
