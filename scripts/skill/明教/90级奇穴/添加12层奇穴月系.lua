---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/90级奇穴/添加12层奇穴月系.lua
-- 更新时间:	2015/9/26 13:25:41
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(14915) == 1 or player.GetSkillLevel(14922) == 1 then
		if not player.GetBuff(9909, 1) then
			if player.GetBuff(9910, 1) then
				player.AddBuff(player.dwID, player.nLevel, 9909, 1)
				player.DelBuff(9910, 1)
			else
				player.AddBuff(player.dwID, player.nLevel, 9911, 1)
			end
		end
	end
end
