---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/90级奇穴/明教_净世破魔击几率回能.lua
-- 更新时间:	2014/6/27 10:06:38
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/明教/90级奇穴/明教_净世破魔击几率回能.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local nplayer = GetPlayer(dwCharacterID)
	if not nplayer then
		return
	end
	--print(nplayer.dwID)
	nplayer.SetTimer(2,"scripts/skill/明教/90级奇穴/明教_净世破魔击几率回能.lua",0,0)
	--[[
	local n = Random(1, 10000)
	if n <= 1000 then
		nplayer.nCurrentSunEnergy = nplayer.nCurrentSunEnergy + 1000
		nplayer.nCurrentMoonEnergy = nplayer.nCurrentMoonEnergy + 1000
	elseif n <= 4000 then
		nplayer.nCurrentSunEnergy = nplayer.nCurrentSunEnergy + 2000
		nplayer.nCurrentMoonEnergy = nplayer.nCurrentMoonEnergy + 2000
	elseif n <= 6000 then
		nplayer.nCurrentSunEnergy = nplayer.nCurrentSunEnergy + 3000
		nplayer.nCurrentMoonEnergy = nplayer.nCurrentMoonEnergy + 3000
	elseif n <= 7500 then
		nplayer.nCurrentSunEnergy = nplayer.nCurrentSunEnergy + 4000
		nplayer.nCurrentMoonEnergy = nplayer.nCurrentMoonEnergy + 4000
	elseif n <= 8500 then
		nplayer.nCurrentSunEnergy = nplayer.nCurrentSunEnergy + 5000
		nplayer.nCurrentMoonEnergy = nplayer.nCurrentMoonEnergy + 5000
	elseif n <= 9000 then
		nplayer.nCurrentSunEnergy = nplayer.nCurrentSunEnergy + 6000
		nplayer.nCurrentMoonEnergy = nplayer.nCurrentMoonEnergy + 6000
	elseif n <= 9500 then
		nplayer.nCurrentSunEnergy = nplayer.nCurrentSunEnergy + 7000
		nplayer.nCurrentMoonEnergy = nplayer.nCurrentMoonEnergy + 7000
	else
		nplayer.nCurrentSunEnergy = nplayer.nCurrentSunEnergy + 8000
		nplayer.nCurrentMoonEnergy = nplayer.nCurrentMoonEnergy + 8000
	--]]	
	end
...
function OnTimer(nplayer, nparam1, nparam2)
	if not nplayer then
		return
	end
		local n = Random(1, 10000)
