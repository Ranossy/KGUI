---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/90级奇穴/满日AOE.lua
-- 更新时间:	2013/6/7 1:26:45
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData = 
{
	{nDamageBase = 286 * 0.95, nDamageRand = 286 * 0.1, nCostMana = 0}, --level 5
};
...
function GetSkillLevelData(skill)
	...
	skill.AddSlowCheckDestOwnBuff(6288, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);	-- 需求目标属于自己的Buff   
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(5978) == 1 then
		--target.AddBuff(dwSkillSrcID, player.nLevel, 6288, 1, 1)
		if not player.GetBuff(6358, 1) then
			player.AddBuff(dwSkillSrcID, player.nLevel, 6356, 1, 1)
			player.AddBuff(dwSkillSrcID, player.nLevel, 6358, 1, 1)
		end
	end
end
