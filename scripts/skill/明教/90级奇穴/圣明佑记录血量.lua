---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/90级奇穴/圣明佑记录血量.lua
-- 更新时间:	2017/4/9 16:30:05
-- 更新用户:	xueyan3
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if nLeftFrame > 0 then
		local buff = player.GetBuff(11874, 1)
		...
		if table then
			...
			for k, v in ipairs(table) do
				...
				if v.nAbsorbType == 8 then
					if v.nAbsorbUsedValue < v.nAbsorbValue then
						return
					end
				end
			end
...
--	print("nLeftFrame=" .. nLeftFrame)
	if nLeftFrame > 0 then
		...
		--]]
		if player.GetBuff(11875, 1) then
			player.CastSkill(4044, 1)
			player.DelBuff(11875, 1)
		end
