---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/90级奇穴/贪魔体驱散dot.lua
-- 更新时间:	2013/6/4 4:43:01
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local n = 0
	local lv = player.GetSkillLevel(3973)
	if player.IsSkillRecipeActive(1039, 1) then
		n = 1
	end
	--换图标
	player.AddBuff(player.dwID, player.nLevel, 10122, 1)
	
	if player.GetSkillLevel(6718) == 1 then
		player.CastSkill(6736, 1)
		player.AddBuff(dwSkillSrcID, player.nLevel, 6278, 1, 5+n)
	end
	if player.GetSkillLevel(6719) == 1 then
		--player.CastSkill(6736, 1)
		player.AddBuff(dwSkillSrcID, player.nLevel, 6279, 1, 5+n)
		player.AddBuff(dwSkillSrcID, player.nLevel, 6280, 1, 10+2*n)
	else
		player.AddBuff(dwSkillSrcID, player.nLevel, 4033, 1, 5+n)
		player.AddBuff(dwSkillSrcID, player.nLevel, 4439, lv, 5+n)
	end
	if player.GetSkillLevel(6721) == 1 then
		player.AddBuff(dwSkillSrcID, player.nLevel, 6282, 1, 5+n)
	end

	--附魔贪魔体被击回血
	local nSkill = player.GetKungfuMount()
	if player.GetSkillLevel(9272) == 1 and nSkill ~= nil and nSkill.dwSkillID == 10243 then
		player.AddBuff(dwSkillSrcID, player.nLevel, 7964, 1, 5 + n)
	end
end
