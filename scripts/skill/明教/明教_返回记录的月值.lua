---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/明教_返回记录的月值.lua
-- 更新时间:	2015/9/21 14:54:26
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nbuff_sun = player.GetBuff(9858, 1)	--日
	local nbuff_moon = player.GetBuff(9859, 1)	--月
	
	if nbuff_sun then
		nbuff_sun.nCustomValue = 0
	end
	--print("moon>>>>>>>>>>")
	if nbuff_moon then
		player.nCurrentMoonEnergy = player.nCurrentMoonEnergy + nbuff_moon.nCustomValue
		--print("moon="..player.nCurrentMoonEnergy.." moonbuff="..nbuff_moon.nCustomValue)
		nbuff_moon.nCustomValue = 0
	end
end
