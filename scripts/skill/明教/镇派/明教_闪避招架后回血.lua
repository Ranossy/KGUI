...
function GetSkillLevelData(skill)
    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	skill.AddAttribute(
   		...
   		"skill/明教/镇派/明教_闪避招架后回血.lua",				
   		0														
   		);
    
	----------------- 技能施放Buff需求 ---------------------------------------------
skill.AddSlowCheckSelfBuff(4485, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);
	...
	----------------- BUFF相关 -------------------------------------------------
   	  -- skill.BindBuff(1, 4532, dwSkillLevel);		-- 设置1号位Buff
	skill.BindBuff(1, 4485, 1);		-- 设置2号位Buff
...
function Apply(dwCharacterID)
	...
	if not player.GetBuff(7913, 1) and player.dwShapeShiftID == 0 then
		...
		--player.AddBuff(player.dwID,player.nLevel,1111,1)
	end
end
