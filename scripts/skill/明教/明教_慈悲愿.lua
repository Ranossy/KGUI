---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/明教_慈悲愿.lua
-- 更新时间:	2015/9/14 15:56:47
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 0, nDamageRand = 0, nThreat = 1754}, --level 1
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 4198, dwSkillLevel);		-- 设置1号位Buff
	...
	skill.SetNormalCoolDown(1, 524);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(14674) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 9736, 2)	 --加强版60%闪避持续3秒
	elseif player.GetSkillLevel(14675) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 9736, 3)	 --加强版10%闪避持续10秒,30%威胁值
	else
		player.AddBuff(player.dwID, player.nLevel, 9736, 1)		--正常30%闪避	
	end
end
