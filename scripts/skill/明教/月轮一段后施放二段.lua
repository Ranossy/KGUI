...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local lv = player.GetSkillLevel(3959)
	if lv ~= 0 then
		if player.GetCDLeft(504) ~= 0 then
			player.ClearCDTime(504)
		end
		if player.GetSkillLevel(5988) == 1 then
			--有奇穴\释放眩晕目标三段攻击
			player.CastSkill(13664, lv, player.GetSkillTarget())
			--有奇穴打非眩晕目标
			player.CastSkill(13667, lv, player.GetSkillTarget())
		else
			--释放无奇穴的普通目标三段攻击
			player.CastSkill(4025, lv, player.GetSkillTarget())
		end
	end
end
