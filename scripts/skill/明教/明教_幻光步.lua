---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/明教_幻光步.lua
-- 更新时间:	2013/8/19 16:41:49
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 512);	--技能普通CD
	skill.SetNormalCoolDown(2, 88);  	--轻功间CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	--if player.GetSkillLevel(6893) == 1 then
	--player.AddBuff(0,99,6414,1)
	--end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	player.AddBuff(0, 99, 4407, 1, 1)
	if player.GetSkillLevel(6893) == 1 then
		player.AddBuff(0, 99, 6414, 1)
		player.DelMultiGroupBuffByFunctionType(5)
	end
	--player.PlaySfx(234, player.nX, player.nY, player.nZ)
	
	ModityCDToUI(player, 3970, 0, 0)
end
