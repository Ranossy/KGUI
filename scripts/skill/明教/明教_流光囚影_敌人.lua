...
function GetSkillLevelData(skill)
    ...
    skill.AddSlowCheckDestBuff(7927, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
   ...
   -- skill.BindBuff(1, 4055, 1);		-- 锁足
	...
	--skill.SetNormalCoolDown(1, 519);	--技能普通CD
...
function Apply(dwCharacterID,dwSkillSrcID)
	...
	if IsPlayer(dwCharacterID) then
		cNpc = GetPlayer(dwCharacterID)		--对玩家有锁足
		cNpc.AddBuff(0,99,4055,1)
	else
	...
	--if nResaultB ~= 1 then			--释放失败后补偿
		...
		--return
	--end 
	if nResaultA == 1 and nRX == 0 and nRY ==0 and nRZ ==0 then		--若3D障碍可达，则传至目标后面
		cPlayer.Stop()
		cPlayer.SetPosition(nCX, nCY, cNpc.nZ)
		cPlayer.TurnTo(cNpc.nFaceDirection)
		--cPlayer.SetTimer()
		--cPlayer.SetTimer(1, "scripts/skill/明教/明教_流光囚影.lua", 0, 0)--走CD
	else						--若3D障碍不可达，则传至目标位置
		cPlayer.Stop()
		cPlayer.SetPosition(cNpc.nX, cNpc.nY, cNpc.nZ)
		cPlayer.TurnTo(cNpc.nFaceDirection)
		--cPlayer.SetTimer(3, "scripts/skill/明教/明教_流光囚影.lua", 0, 0)--走CD
	...
	cPlayer.AddBuff(0, 99, 3473, 1)		--抗摔


	--奇穴敌对目标锁足2秒
	if cPlayer.GetSkillLevel(5982) == 1 then
		cNpc.AddBuff(cPlayer.dwID, cPlayer.nLevel, 9753, 1)
	end
end
