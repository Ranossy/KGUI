---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/明教_光明相.lua
-- 更新时间:	2015/9/14 16:53:59
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 4423, 1);		-- 设置1号位Buff
	...
	skill.SetNormalCoolDown(1, 511);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(6771) == 1 then
		if target.dwShapeShiftID == 0 and not target.GetBuff(7913,1) then
			target.nCurrentLife = target.nCurrentLife + target.nMaxLife * 0.2
			target.AddBuff(player.dwID, player.nLevel, 6308, 1)
		end
	end

	player.AddBuff(player.dwID, player.nLevel, 4423, 1)	--正常添加一层
	if player.GetSkillLevel(6720) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 4423, 1)	--奇穴额外添加一层
	end

	ModityCDToUI(player, 3969, 0, 0)
end
...
function UnApply(dwCharacterID)
	...
	player.DelBuff(4723, 1) --特效buff
	player.DelBuff(4724, 1) --特效buff
	player.DelBuff(4914, 1) --特效buff
end
