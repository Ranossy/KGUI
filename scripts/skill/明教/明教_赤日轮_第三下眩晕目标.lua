---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/明教_赤日轮_第三下眩晕目标.lua
-- 更新时间:	2015/2/4 14:39:48
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		tSkillData[dwSkillLevel].nDamageBase*1.2,
		0
	);
	...
	skill.AddAttribute(
		...
		tSkillData[dwSkillLevel].nDamageRand*1.2,
		0
	);
...
--增加武器伤害
	...
	skill.AddSlowCheckSelfBuff(4025, 1, BUFF_COMPARE_FLAG.EQUAL, 2, BUFF_COMPARE_FLAG.EQUAL);
	----------------- 设置技能消耗 ---------------------------------------------------------------------
	...
	if dwSkillLevel < 10 then
		skill.nChannelInterval = 32 * 1.2 * 1.15;
	elseif dwSkillLevel < 33 then
		skill.nChannelInterval = (32 + (dwSkillLevel - 9) * 2.4) * 1.2 * 1.15
	else
		skill.nChannelInterval = 96 * 1.2 * 1.15;     -- 通道技间隔时间
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(5988) == 1 then
		if	player.nSunPowerValue ~= 1 and player.nMoonPowerValue ~= 1 then
			player.nCurrentSunEnergy = player.nCurrentSunEnergy + 1000
			--player.AddBuff(dwSkillSrcID, player.nLevel, 6274, 1, 1)
		end
	end
