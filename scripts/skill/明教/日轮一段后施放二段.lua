---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/日轮一段后施放二段.lua
-- 更新时间:	2014/7/20 12:54:37
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local lv = player.GetSkillLevel(3962)
	if lv ~= 0 then
		if player.GetCDLeft(507) ~= 0 then
			player.ClearCDTime(507)
		end
		if player.GetSkillLevel(5988) == 1 then
			--有奇穴\释放眩晕目标三段攻击
			player.CastSkill(13670, lv, player.GetSkillTarget())
			--有奇穴打非眩晕目标
			player.CastSkill(13673, lv, player.GetSkillTarget())
		else
			--释放无奇穴的普通目标三段攻击
			player.CastSkill(4029, lv, player.GetSkillTarget())
		end
	end
end
