---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/明教_幽月轮_第三下眩晕目标.lua
-- 更新时间:	2015/2/4 14:42:31
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK, --额外加20%会心
...
--增加武器伤害
	...
	skill.AddAttribute(
		...
		"skill/明教/明教_幽月轮_第三下眩晕目标.lua",
		0
	);
	--skill.BindBuff(1,4020,1) 		--锁足
	----------------- 设置CoolDown ---------------------------------------------------------------------
	...
	skill.AddSlowCheckSelfBuff(4019, 1, BUFF_COMPARE_FLAG.EQUAL, 2, BUFF_COMPARE_FLAG.EQUAL);
	----------------- 设置技能消耗 ---------------------------------------------------------------------
	...
	if dwSkillLevel < 10 then
		skill.nChannelInterval = 40 * 1.2 * 1.15;
	elseif dwSkillLevel < 24 then
		skill.nChannelInterval = (40 + (dwSkillLevel - 9) * 3) * 1.2 * 1.15
	else
		skill.nChannelInterval = 80 * 1.2 * 1.15;     -- 通道技间隔时间
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(5988) == 1 then
		if player.nMoonPowerValue ~= 1 and player.nSunPowerValue ~= 1 then
