---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/隐身叠加会心判定.lua
-- 更新时间:	2015/9/15 0:05:28
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(4052, 1) or player.GetBuff(4052, 2) then
		player.AddBuff(player.dwID, player.nLevel, 9755, 1)		--每层5%会心，5层
	else
		if player.GetBuff(9755, 1) then
			local nbuff_mark = player.GetBuff(9754, 1)
			if nbuff_mark then
				nbuff_mark.nCustomValue = nbuff_mark.nCustomValue + 1
				print("nbuff_mark.nCustomValue"..nbuff_mark.nCustomValue)
				if nbuff_mark.nCustomValue % 2 == 0 then
					player.DelBuff(9755, 1)
				end
			end
		else
			local nbuff_mark = player.GetBuff(9754, 1)
			if nbuff_mark and nbuff_mark.nCustomValue ~= 0 then
				nbuff_mark.nCustomValue = 0
			end
		end
	end
end
