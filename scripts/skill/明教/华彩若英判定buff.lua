---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/明教/华彩若英判定buff.lua
-- 更新时间:	2015/9/14 17:27:20
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nbuff = player.GetBuff(9742, 1)
	local nCount_old = 0
	if nbuff then
		nCount_old = nbuff.nStackNum
	end
	
	local nCount_new = 9 - math.floor((player.nCurrentLife / player.nMaxLife) * 10, 1)

	if player.nMoveState ~= MOVE_STATE.ON_DEATH and player.bFightState == true then
		if nCount_new > nCount_old then
			local nTime = nCount_new - nCount_old
			for i = 1, nTime do
				player.AddBuff(player.dwID, player.nLevel, 9742, 1)
			end
		elseif nCount_new < nCount_old then
			local nTime = nCount_old - nCount_new
			for i = 1, nTime do
				player.DelBuff(9742, 1)
			end
		end
	else
		player.DelGroupBuff(9742, 1)
	end
