---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_双刀_刀C.lua
-- 更新时间:	2016/7/11 15:23:37
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 3333, nDamageRand = 4444, nCostMana = 5555}, --level 1
};
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.GetBuff(10814, 1) then
		player.ResetCD(1062)
	end

	player.nCurrentEnergy = math.min(player.nCurrentEnergy + 10, 100)
	player.nCurrentRage = math.max(math.min(player.nCurrentRage, 100 - player.nCurrentEnergy), 0)

	--[[
	if player.nCurrentEnergy + player.nCurrentRage >= 100 then
		player.nCurrentRage = player.nCurrentRage - (player.nCurrentEnergy + player.nCurrentRage - 100)
	end
	--]]
end
