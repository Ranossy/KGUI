---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_双刀_崩雪潮.lua
-- 更新时间:	2016/9/12 9:45:25
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(10814, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 双刀姿态
	skill.AddSlowCheckSelfBuff(11077, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);	--霸刀大刀旋转
	--skill.AddSlowCheckSelfBuff(10816, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	--skill.nChannelInterval = 28; 				-- 通道技间隔时间
...
function OnSkillLevelUp(skill, player)
	...
	if player.GetSkillLevel(16027) ~= 0 and player.GetSkillLevel(16027) ~= lv then
		player.LearnSkillLevel(16027, lv, player.dwID)
	end
	--]]
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	---崩雪潮Buff替换
	player.SetTimer(2, "scripts/skill/霸刀/套路及子技能/霸刀_双刀_崩雪潮.lua", 0, 0)

	---冲刺标记
	player.AddBuff(player.dwID, player.nLevel, 11156, 1, 1)

	----崩雪潮触发碎江天
	if player.GetSkillLevel(16706) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 11172, 1)
		local nbuff_mark = player.GetBuff(11172, 1)
		if nbuff_mark then
			local nCount = nbuff_mark.nStackNum
			RemoteCallToClient(player.dwID, "ChangeSkillSurfaceNum", 16100, nCount)
			if nCount == 3 then
				if player.GetSkillLevel(16716) == 1 then
					player.AddBuff(player.dwID, player.nLevel, 11001, 1)
				elseif not player.GetBuff(11001, 1) then
					player.AddBuff(player.dwID, player.nLevel, 11001, 1)
				end
				player.DelGroupBuff(11172, 1)
				RemoteCallToClient(player.dwID, "ChangeSkillSurfaceNum", 16100, 0)
			end
		end
	end
	---绝沧海调息期间崩雪潮不触发调息
	if player.GetSkillLevel(16721) == 1 then
		--取绝沧海调息时间
		local CD_LeftFrame, _ = player.GetCDLeft(1062)
		if CD_LeftFrame == 0 then
			player.AddBuff(player.dwID, player.nLevel, 11189, 1)
		elseif CD_LeftFrame ~= 0 then
			--重置崩雪潮调息时间
			player.ClearCDTime(1061)
		end
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	if player.GetBuff(11194, 1) then
		player.DelBuff(11194, 1)
	elseif player.GetBuff(11193, 1)then
		player.DelBuff(11193, 1)
		player.AddBuff(player.dwID, player.nLevel, 11194, 1, 1)
	else
		player.AddBuff(player.dwID, player.nLevel, 11193, 1, 1)
	end
end
