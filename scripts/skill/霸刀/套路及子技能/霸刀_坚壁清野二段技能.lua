---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_坚壁清野二段技能.lua
-- 更新时间:	2016/9/23 21:33:53
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_坚壁清野二段技能.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(11306, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);	--霸刀
	skill.AddSlowCheckSelfBuff(10816, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);	--霸刀
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nbuff = player.GetBuff(11306, 1)
	if nbuff then
		local npc_ID = nbuff.dwSkillSrcID
		local npc_cast = GetNpc(npc_ID)
		if npc_cast then
			--[[
			if player.GetSkillLevel(16730) == 1 then
				npc_cast.CastSkill(16929, 1)
			else
				npc_cast.CastSkill(16919, 1)	
			end
			--]]
			npc_cast.CastSkill(16919, 1)
			player.DelBuff(11306, 1)
		end
	end
end
