---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_鞘刀_坚壁清野20尺伤害全距离.lua
-- 更新时间:	2016/10/7 16:12:55
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_鞘刀_坚壁清野20尺伤害全距离.lua",
		0
	);
	--[[
	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	--释放者是表现npc
	local npc_self = GetNpc(dwSkillSrcID)
	...
	if lv ~= 0 then
		if nHPcof > 0.8 then
			if IsPlayer(dwCharacterID) then
				player.CastSkill(16792, 1, TARGET.PLAYER, dwCharacterID)
				player.CastSkill(16787, lv, TARGET.PLAYER, dwCharacterID)
			else
				player.CastSkill(16792, 1, TARGET.NPC, dwCharacterID)
				player.CastSkill(16787, lv, TARGET.NPC, dwCharacterID)
			end
		elseif nHPcof > 0.6 then
			if IsPlayer(dwCharacterID) then
				player.CastSkill(16792, 1, TARGET.PLAYER, dwCharacterID)
				player.CastSkill(16788, lv, TARGET.PLAYER, dwCharacterID)
			else
				player.CastSkill(16792, 1, TARGET.NPC, dwCharacterID)
				player.CastSkill(16788, lv, TARGET.NPC, dwCharacterID)
			end
		elseif nHPcof > 0.4 then
			if IsPlayer(dwCharacterID) then
				player.CastSkill(16792, 1, TARGET.PLAYER, dwCharacterID)
				player.CastSkill(16789, lv, TARGET.PLAYER, dwCharacterID)
			else
				player.CastSkill(16792, 1, TARGET.NPC, dwCharacterID)
				player.CastSkill(16789, lv, TARGET.NPC, dwCharacterID)
			end
		elseif nHPcof > 0.2 then
			if IsPlayer(dwCharacterID) then
				player.CastSkill(16792, 1, TARGET.PLAYER, dwCharacterID)
				player.CastSkill(16790, lv, TARGET.PLAYER, dwCharacterID)
			else
				player.CastSkill(16792, 1, TARGET.NPC, dwCharacterID)
				player.CastSkill(16790, lv, TARGET.NPC, dwCharacterID)
			end
		else
			if IsPlayer(dwCharacterID) then
				player.CastSkill(16792, 1, TARGET.PLAYER, dwCharacterID)
				player.CastSkill(16791, lv, TARGET.PLAYER, dwCharacterID)
			else
				player.CastSkill(16792, 1, TARGET.NPC, dwCharacterID)
				player.CastSkill(16791, lv, TARGET.NPC, dwCharacterID)
			end
		end
	end
end
