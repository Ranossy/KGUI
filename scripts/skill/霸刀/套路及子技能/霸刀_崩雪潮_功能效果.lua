---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_崩雪潮_功能效果.lua
-- 更新时间:	2016/8/17 19:58:27
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_崩雪潮_功能效果.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--大刀状态下崩雪潮变透支
	if player.GetBuff(10815, 1) then
		local nMaxOverDraftCount, nDraftCount = player.GetOverDraftCoolDown(1080)
		if nDraftCount ~= 0 then
			player.AddCDTime(1080, 48)
		end
		player.SetOverDraftCoolDown(1080, nDraftCount + 1)

		if player.GetBuff(11026, 1)then
			player.DelBuff(11026, 1)
			player.AddBuff(player.dwID, player.nLevel, 11032, 1)
		elseif player.GetBuff(11032, 1)then
			player.DelBuff(11032, 1)
			player.SetTimer(20, "scripts/skill/霸刀/套路及子技能/霸刀_崩雪潮_功能效果.lua", 0, 0)
		else
			player.AddBuff(player.dwID, player.nLevel, 11026, 1)
		end
	end

	if player.GetBuff(10814, 1) and player.IsSkillRecipeActive(4205, 1) then
		player.nCurrentEnergy = math.min(player.nCurrentEnergy + 15, player.nMaxEnergy)
		player.nCurrentRage = math.max(math.min(player.nCurrentRage, player.nMaxEnergy - player.nCurrentEnergy), 0)
	else
		player.nCurrentEnergy = math.min(player.nCurrentEnergy + 10, player.nMaxEnergy)
		player.nCurrentRage = math.max(math.min(player.nCurrentRage, player.nMaxEnergy - player.nCurrentEnergy), 0)
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	local nlv = player.GetSkillLevel(16026)
	if nlv ~= 0 then
		player.CastSkill(16381, nlv)
