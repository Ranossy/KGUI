---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_鞘刀_封渊震煞.lua
-- 更新时间:	2016/9/20 10:26:28
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_鞘刀_封渊震煞.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	skill.SetNormalCoolDown(1, 1113);	--技能普通CD
	--skill.SetNormalCoolDown(2, 1144);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npc_daoqi = scene.CreateNpcWithEmployerID(53233, player.nX, player.nY, player.nZ, player.nFaceDirection, 4 * 16, player.dwID, false, player.dwIdentityVisiableID)
	if npc_daoqi then
		player.SetTimer(2, "scripts/skill/霸刀/套路及子技能/霸刀_鞘刀_封渊震煞.lua", npc_daoqi.dwID, 0)
		--气墙推人眩晕
		if player.GetSkillLevel(16911) == 1 then
			npc_daoqi.AddBuff(player.dwID, player.nLevel, 11450, 1)

			local nbuff = npc_daoqi.GetBuff(11450, 1)
			if nbuff then
	...
	player.Stop()
	player.CastSkill(16763, 1)
		
	--霸体效果
	player.AddBuff(player.dwID, player.nLevel, 11310, 1)
	player.AddBuff(player.dwID, player.nLevel, 11358, 1)
	
	local nMaxOverDraftCount, nDraftCount = player.GetOverDraftCoolDown(1113)
	if nDraftCount ~= 0 then
		player.AddCDTime(1113, 480)
	end
	player.SetOverDraftCoolDown(1113, nDraftCount + 1)
end
...
function OnTimer(player, nParam1, nParam2)
	...
	player.PlaySfx(1530, npc_daoqi.nX, npc_daoqi.nY, npc_daoqi.nZ, npc_daoqi.nFaceDirection * 360 / 256)

	local Sou_X, Sou_Y, Dst_X, Dst_Y = GetSourceAndDestinationPoint(npc_daoqi.nX, npc_daoqi.nY, npc_daoqi.nFaceDirection, 5 * 64)
	local nIndex = player.AddAdvancedDynamicObstacle(Sou_X, Sou_Y, Dst_X, Dst_Y, npc_daoqi.nZ - 2 * 64 * 8, npc_daoqi.nZ + 8 * 64 * 8, 64, ADVANCED_DYNAMIC_OBSTACLE_TYPE.ONLY_ENEMY)
	...
	if player.GetSkillLevel(16911) == 1 then
		for i = 1, 4 do
			if not player.GetBuff(11465 + i, 1) then
				...
				if nbuff_mark then
					nbuff_mark.nCustomValue = npc_daoqi.dwID
				end
