---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_腿_雷走风切命中运功伤害效果.lua
-- 更新时间:	2016/9/17 22:55:37
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_腿_雷走风切命中运功伤害效果.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	  skill.BindBuff(1, 11171, 1);		-- 设置1号位Buff
	...
	if dwSkillLevel == 1 then
		skill.nChannelInterval = 45*0.8; 				-- 通道技间隔时间
	elseif dwSkillLevel < 10 then
		skill.nChannelInterval = (45 + (dwSkillLevel - 1) * 17)*0.8; 				-- 通道技间隔时间
	else
		skill.nChannelInterval = 224*0.8; 				-- 通道技间隔时间
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(16797) == 1 then
		target.AddBuff(player.dwID, player.nLevel, 11169, 1)
	end
	
	---秘籍附带驱散
	if player.IsSkillRecipeActive(4130, 1) then
		player.CastSkill(16805, 1)
	end
	---秘籍附带加速
	if player.IsSkillRecipeActive(4131, 1) then
		player.AddBuff(player.dwID, player.nLevel, 11244, 1)
	end
end
