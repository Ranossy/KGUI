---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_鞘刀_坚壁清野Npc矩形技能.lua
-- 更新时间:	2016/9/16 19:46:08
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_鞘刀_坚壁清野Npc矩形技能.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if lv ~= 0 and IsEnemy(player.dwID, dwCharacterID) then
		if IsPlayer(dwCharacterID) then
			player.CastSkill(16794, lv, TARGET.PLAYER, dwCharacterID)
		else
			player.CastSkill(16794, lv, TARGET.NPC, dwCharacterID)
		end
	end

	if player.GetSkillLevel(16912) == 1 then
		target.AddBuff(player.dwID, player.nLevel, 11308, 1)
	end
    ---坚壁清野添加内置计时
	if not target.GetBuff(11550, 1) then
		target.AddBuff(player.dwID, player.nLevel, 11550, 1)
	end
	---坚壁清野添加内置计数
	target.AddBuff(player.dwID, player.nLevel, 11551, 1)

	---坚壁清野每跳15%几率触发2段技能
	if player.GetSkillLevel(16906) == 1 then
		if not player.GetBuff(11304, 1) then
			local nRnd = Random(1, 10000)
			if nRnd <= 1500 then
				if not player.GetBuff(11306, 1)then
					player.AddBuff(player.dwID, player.nLevel, 11304, 1)
					player.AddBuff(npc_self.dwID, npc_self.nLevel, 11306, 1)
				end	
			end
		end
	end
