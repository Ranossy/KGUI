---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_割据秦宫_二段眩晕.lua
-- 更新时间:	2016/9/14 19:53:36
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	...
	{nDamageBase = 175, nDamageRand = 15, nCostMana = 0}, --level 10
	{nDamageBase = 190, nDamageRand = 15, nCostMana = 0}, --level 6
	{nDamageBase = 205, nDamageRand = 15, nCostMana = 0}, --level 7
	{nDamageBase = 220, nDamageRand = 15, nCostMana = 0}, --level 8
	{nDamageBase = 235, nDamageRand = 15, nCostMana = 0}, --level 9
	{nDamageBase = 245, nDamageRand = 15, nCostMana = 0}, --level 10
};
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_割据秦宫_二段眩晕.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 11231, 1);		-- 设置1号位Buff
	...
	if dwSkillLevel == 1 then
		skill.nChannelInterval = 32*1.05; 				-- 通道技间隔时间
	elseif dwSkillLevel < 15 then
		skill.nChannelInterval = (32 + (dwSkillLevel - 1) * 9)*1.05; 				-- 通道技间隔时间
	else
		skill.nChannelInterval = 176*1.05; 				-- 通道技间隔时间
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	---增加项王击鼎旋转效果持续时间
	if player.GetSkillLevel(16750) == 1 then
		if not player.GetBuff(11299, 1) then
			local nMark = 0
			for i = 1, 9 do
				if player.GetBuff(11208, i) then
					if i == 9 then
						player.DelBuff(11208, i)
						player.AddBuff(player.dwID, player.nLevel, 11208, i)
						player.AddBuff(player.dwID, player.nLevel, 11333, 1)
					elseif i == 8 then
						player.DelBuff(11208, i)
						player.AddBuff(player.dwID, player.nLevel, 11208, i + 1)
						player.AddBuff(player.dwID, player.nLevel, 11333, 1)
					else
						player.DelBuff(11208, i)
						player.AddBuff(player.dwID, player.nLevel, 11208, i + 2)
						player.AddBuff(player.dwID, player.nLevel, 11333, 1)
						player.AddBuff(player.dwID, player.nLevel, 11333, 1)
					end
					nMark = 1
			...
			if nMark == 0 then
				player.AddBuff(player.dwID, player.nLevel, 11208, 2)
				player.AddBuff(player.dwID, player.nLevel, 11333, 1)
				player.AddBuff(player.dwID, player.nLevel, 11333, 1)
			end
			
			player.AddBuff(player.dwID, player.nLevel, 11299, 1)
		end
	end
	
	player.SetTimer(2, "scripts/skill/霸刀/套路及子技能/霸刀_割据秦宫_二段眩晕.lua", dwCharacterID, 0)
end
...
function OnTimer(player, nParam1, nParam2)
	...
	if IsPlayer(nParam1) then
		target = GetPlayer(nParam1)
	else
		target = GetNpc(nParam1)
	end
	if not target then
		return
	end
	...
	if IsPlayer(nParam1) then
		player.CastSkill(16769, 1, TARGET.PLAYER, nParam1)
	else
		player.CastSkill(16769, 1, TARGET.NPC, nParam1)
	end
end
