---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_拔刀_风体.lua
-- 更新时间:	2016/9/15 14:41:28
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_拔刀_风体.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	--skill.AddSlowCheckSelfBuff(11164, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);  --能量高于10点切换需求
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 10816, 1);		-- 设置1号位Buff
	...
	skill.SetNormalCoolDown(1, 1142);	--姿态3秒调息
	skill.SetNormalCoolDown(2, 1143);	--姿态主被动1秒调息
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	--[[
	if player.nCurrentSunEnergy < 10 then
		return SKILL_RESULT_CODE.FAILED
	end
	--]]
	return nPreResult;
...
function OnSkillLevelUp(skill, player)
	...
	if player.GetSkillLevel(16459) ~= lv then
		player.LearnSkillLevel(16459, lv, player.dwID)
	end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	RemoteCallToClient(player.dwID, "OnSkillReplace", 16169, 16459, 1, 16169) ---鞘刀被动切主动
	RemoteCallToClient(player.dwID, "OnSkillReplace", 16454, 16168, 1, 16168) ---大刀主动切被动
	RemoteCallToClient(player.dwID, "OnSkillReplace", 16608, 16166, 1, 16166) ---双刀主动切被动
	--[[
	---切为双刀腿
	local lv_SDlongxianghubu = player.GetSkillLevel(16028)
	local lv_SDleizoufengqie = player.GetSkillLevel(16598)
	local lv_SDtayanyangqi = player.GetSkillLevel(16460)
	
	if lv_SDlongxianghubu ~= 0 then
		if player.GetBuff(10174, 1) then
			RemoteCallToClient(player.dwBeProxySkillPlayerID, "OnSkillReplace", 16628, 16028, lv_SDlongxianghubu, 16028)
		end
	end
	
	if lv_SDtayanyangqi ~= 0 then
		if player.GetBuff(10174, 1) then
			RemoteCallToClient(player.dwBeProxySkillPlayerID, "OnSkillReplace", 16633, 16460, lv_SDtayanyangqi, 16460)
