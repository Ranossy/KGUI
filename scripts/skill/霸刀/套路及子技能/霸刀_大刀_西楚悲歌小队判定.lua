---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_大刀_西楚悲歌小队判定.lua
-- 更新时间:	2016/10/13 19:03:59
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/霸刀/套路及子技能/霸刀_大刀_西楚悲歌小队判定.lua",											-- 属性值1
   		0											-- 属性值2
   		);
	...
	--bKeyUpSkill = false	--为true表示keyup响应技能
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if dwCharacterID == dwSkillSrcID then
		...
		--大附魔标记buff
		if player.GetSkillLevel(17071) == 1 then
			if player.GetBuff(11475, 1) then
				player.DelGroupBuff(11475, 1)
			end
			player.AddBuff(player.dwID, player.nLevel, 11475, 1)
		end
	end
	

	if not target.GetBuff(10212, 1) then
		--30%基础攻击力
		if player.GetSkillLevel(17054) == 1 then
			target.AddBuff(player.dwID, player.nLevel, 11456, 1)
		end
		--30%减伤
		if player.GetSkillLevel(16958) == 1 then
			target.AddBuff(player.dwID, player.nLevel, 11457, 1)
		end
	end
