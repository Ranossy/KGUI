---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_双刀_散流霞.lua
-- 更新时间:	2016/10/9 15:16:57
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 1114);	--主动技能调息
	--skill.SetNormalCoolDown(2, 1133);
	skill.SetCheckCoolDown(1, 444);	--只检查不走的CD
	--skill.SetCheckCoolDown(2, 1133);	--只检查不走的CD
	skill.SetCheckCoolDown(2, 1139);	--主被动切换内置CD
	----------------- 经验升级相关 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	--skillmove动作
	--player.CastSkill(16963, 1)
	player.Stop()
	player.CastSkill(16963, 1)
	--player.SetTimer(2, "scripts/skill/霸刀/套路及子技能/霸刀_双刀_散流霞.lua", 0, 0)
	--player.ClearAllThreat()
	
	local nMaxOverDraftCount, nDraftCount = player.GetOverDraftCoolDown(1114)
	if nDraftCount ~= 0 then
		if player.IsSkillRecipeExist(4292, 1) then
			player.AddCDTime(1114, 400)
		else
			player.AddCDTime(1114, 480)
		end
	end
	player.SetOverDraftCoolDown(1114, nDraftCount + 1)

	---添加之前判定一次，避免Buff连续添加时间减少
	if player.GetBuff(11151, 1) then
		player.DelBuff(11151, 1)
	end

	---先添加监控Buff
	player.AddBuff(player.dwID, player.nLevel, 11152, 1, 1)

	---添加腿的冲刺效果
	player.AddBuff(player.dwID, player.nLevel, 11156, 1, 1)

	---空痕奇穴驱散不利效果回血
	if player.GetSkillLevel(16735) == 1 then
		player.CastSkill(16891, 1)
	end
end
