---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_双刀_擒王六式·一.lua
-- 更新时间:	2016/9/21 20:54:47
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_双刀_擒王六式·一.lua",
		0
	);
	...
	--skill.SetDelaySubSkill(4, 16933, 1)
	--skill.SetDelaySubSkill(10, 16934, 1)
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 11369, 1);		-- 设置1号位Buff
...
function OnSkillLevelUp(skill, player)
	...
	for i = 1 ,5 do
		if player.GetSkillLevel(16870 + i) ~= lv then
			player.LearnSkillLevel(16870 + i, lv, player.dwID)
		end
	end
	if skill.dwLevel == skill.dwMaxLevel and not player.IsAchievementAcquired(5478) then
		player.AcquireAchievement(5478)
	end
end
...
function UnApply(dwCharacterID)
	...
	local nlev = player.GetSkillLevel(16870)
	if nlev ~= 0 then
		player.CastSkill(16970, nlev)
	end
end
