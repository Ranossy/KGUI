---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_大刀_冰凝.lua
-- 更新时间:	2016/7/13 22:40:33
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_大刀_冰凝.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(10815, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);	-- 需求自身Buff
	skill.AddSlowCheckSelfBuff(11077, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);	--霸刀大刀旋转
	--skill.AddSlowCheckSelfBuff(11162, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);  --能量高于10点切换需求
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 10815, 1);		-- 设置1号位Buff
	...
	--skill.SetNormalCoolDown(1, 1133);	--技能普通CD
	skill.SetNormalCoolDown(1, 1140);	--技能普通CD
	skill.SetNormalCoolDown(2, 1141);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	--[[
	if player.nCurrentRage < 10 then
		return SKILL_RESULT_CODE.FAILED
	end
	--]]
	return nPreResult;
...
function OnSkillLevelUp(skill, player)
	...
	if player.GetSkillLevel(16454) ~= lv then
		player.LearnSkillLevel(16454, lv, player.dwID)
	end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--双刀腿切为大刀腿
	local lv_DDlongxianghubu = player.GetSkillLevel(16628)
	local lv_DDleizoufengqie = player.GetSkillLevel(16629)
	local lv_DDtayanyangqi = player.GetSkillLevel(16633)
	local lv_DDzitaizhudong = player.GetSkillLevel(16454)

	RemoteCallToClient(player.dwID, "OnSkillReplace", 16459, 16169, 1, 16169) ---鞘刀主动切被动
	RemoteCallToClient(player.dwID, "OnSkillReplace", 16168, 16454, 1, 16168) ---大刀被动切主动
	RemoteCallToClient(player.dwID, "OnSkillReplace", 16608, 16166, 1, 16166) ---双刀主动切被动
	--[[
	if lv_DDzitaizhudong ~= 0 then
		RemoteCallToClient(player.dwID, "OnSkillReplace", 16168, 16454, lv_DDzitaizhudong, 16168)
	end
	--]]
	if lv_DDlongxianghubu ~= 0 then
		if player.GetBuff(10174, 1) then
			RemoteCallToClient(player.dwBeProxySkillPlayerID, "OnSkillReplace", 16028, 16628, lv_DDlongxianghubu, 16028)
		else
			RemoteCallToClient(player.dwID, "OnSkillReplace", 16028, 16628, lv_DDlongxianghubu, 16028)
		end
	end
	
	if lv_DDleizoufengqie ~= 0 then
		RemoteCallToClient(player.dwID, "OnSkillReplace", 16598, 16629, lv_DDleizoufengqie, 16598)
		CDTransfer(player, 1110, 0, 1117, 1)
		RecipeCopy(16598, lv_DDleizoufengqie, player, 16629)
	end

	if lv_DDtayanyangqi ~= 0 then
		if player.GetBuff(10174, 1) then
			RemoteCallToClient(player.dwBeProxySkillPlayerID, "OnSkillReplace", 16460, 16633, lv_DDtayanyangqi, 16460)
			CDTransfer(player, 1098, 0, 1118, 1)
		else
			RemoteCallToClient(player.dwID, "OnSkillReplace", 16460, 16633, lv_DDtayanyangqi, 16460)
			CDTransfer(player, 1098, 0, 1118, 1)
		end
	end
	--踏宴扬旗透支单次切换为透支OB通知
	ModityCDToUI(player, 16633, 0, 0)
end
