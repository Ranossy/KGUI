---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_大刀_上将军印.lua
-- 更新时间:	2016/9/15 16:46:53
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_大刀_上将军印.lua",
		0
	);
	...
	skill.SetDelaySubSkill(2, 17072, 1)		--清标记buff功能
	skill.SetDelaySubSkill(4, 16786, 1)		--前方
	skill.SetDelaySubSkill(10, 16960, 1)	--右侧
	skill.SetDelaySubSkill(17, 16961, 1)	--左侧
	skill.SetDelaySubSkill(21, 16960, 1)	--右侧
	skill.SetDelaySubSkill(28, 16786, 1)
	----------------- 设置Cool down --------------------------------------------
	-- 公共CD
	skill.SetPublicCoolDown(16);							-- 公共CD 1.5秒
	-- 技能CD, CoolDownIndex为CD位(共3个), nCoolDownID为CoolDownList.tab内的CDID
	skill.SetNormalCoolDown(1, 1126);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if nPreResult == SKILL_RESULT_CODE.SUCCESS then
		...
		--免控锁操作
		player.AddBuff(player.dwID, player.nLevel, 11322, 1)
	end
	return nPreResult;
end
...
function OnSkillLevelUp(skill, player)
	if skill.dwLevel == skill.dwMaxLevel and not player.IsAchievementAcquired(5475) then
		player.AcquireAchievement(5475)
	end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nbuff_mark = player.GetBuff(11234, 1)
	if nbuff_mark then
		local nFaceDir = nbuff_mark.nCustomValue
		local table_new = { - 30, - 15, 0, 15, 30}
		local i = math.random(1, 5)
		local nDir = nFaceDir + table_new[i]
		if nDir < 0 then
			nDir = nDir + 256
		elseif nDir > 256 then
			nDir = nDir - 256
		end
		player.TurnTo(nDir)
	end
	player.CastSkill(16786, 1)
	--]]
end
