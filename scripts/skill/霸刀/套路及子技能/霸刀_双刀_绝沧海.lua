---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_双刀_绝沧海.lua
-- 更新时间:	2016/9/12 9:49:35
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	---绝沧海Buff替换
	player.SetTimer(2, "scripts/skill/霸刀/套路及子技能/霸刀_双刀_绝沧海.lua", 0, 0)
	...
	if player.GetSkillLevel(16706) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 11178, 1)
		local nbuff_mark = player.GetBuff(11178, 1)
		if nbuff_mark then
			local nCount = nbuff_mark.nStackNum
			RemoteCallToClient(player.dwID, "ChangeSkillSurfaceNum", 16101, nCount)
			if nCount == 3 then
				...
				player.DelGroupBuff(11178, 1)
				RemoteCallToClient(player.dwID, "ChangeSkillSurfaceNum", 16101, 0)
			end
		end
	end
---绝沧海调息期间崩雪潮不触发调息
	if player.GetSkillLevel(16721) == 1 then
		local CD_LeftFrame, _ = player.GetCDLeft(1061)
		if CD_LeftFrame == 0 then
			player.AddBuff(player.dwID, player.nLevel, 11188, 1)
		elseif CD_LeftFrame ~= 0 then
			--重置绝沧海调息时间
			player.ClearCDTime(1062)
		end
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	if player.GetBuff(11196, 1) then
		player.DelBuff(11196, 1)
	elseif player.GetBuff(11195, 1)then
		player.DelBuff(11195, 1)
		player.AddBuff(player.dwID, player.nLevel, 11196, 1, 1)
	else
		player.AddBuff(player.dwID, player.nLevel, 11195, 1, 1)
	end
end
