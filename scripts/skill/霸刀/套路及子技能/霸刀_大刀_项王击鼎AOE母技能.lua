---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_大刀_项王击鼎AOE母技能.lua
-- 更新时间:	2016/9/14 15:31:20
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_大刀_项王击鼎AOE母技能.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function OnClientCast(skill, player, nClientParam)
	if not player then
		return SKILL_RESULT_CODE.FAILED
	end	
		
	if nClientParam == 1 then
		skill.nChannelFrame = 8 * 4
	end
	if nClientParam == 2 then
		skill.nChannelFrame = 8 * 5
	end
	if nClientParam == 3 then
		skill.nChannelFrame = 8 * 6
	end
	if nClientParam == 4 then
		skill.nChannelFrame = 8 * 7
	end
	if nClientParam == 5 then
		skill.nChannelFrame = 8 * 8
	end
	if nClientParam == 6 then
		skill.nChannelFrame = 8 * 9
	end
	if nClientParam == 7 then
		skill.nChannelFrame = 8 * 10
	end
	if nClientParam == 8 then
		skill.nChannelFrame = 8 * 11
	end
	if nClientParam == 9 then
		skill.nChannelFrame = 8 * 12
	end
	if nClientParam == 10 then
		skill.nChannelFrame = 8 * 13
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(16777) == 1 then
		local nCount = 0
		for i = 1, 11 do
			if player.GetBuff(11222, i) then
				player.DelBuff(11222, i)
				player.AddBuff(player.dwID, player.nLevel, 11222, i + 1)
				nCount = nCount + 1
				break
			end
		end
		...
		if nCount == 0 then
			player.AddBuff(player.dwID, player.nLevel, 11222, 1)
		end
	end
end
