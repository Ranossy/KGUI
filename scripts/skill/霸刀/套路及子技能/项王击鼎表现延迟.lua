---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/项王击鼎表现延迟.lua
-- 更新时间:	2016/9/28 11:40:00
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	local nlv = player.GetSkillLevel(16601)
	--其中2跳是延迟效果
	local nCount = 13
	---执行奇穴效果和延迟表现

	if player.GetBuff(11148, 1)then
		player.DelBuff(11148, 1)
		player.AddBuff(player.dwID, player.nLevel, 11149, 1)
		RemoteCallToClient(player.dwID, "ChangeSkillSurfaceNum", 17079, 3)
	elseif player.GetBuff(11149, 1)then
		player.DelBuff(11149, 1)
		RemoteCallToClient(player.dwID, "ChangeSkillSurfaceNum", 16601, 1)
		player.SetTimer(16, "scripts/skill/霸刀/套路及子技能/项王击鼎表现延迟.lua", 0, 0)
		...
		--添加通道期间免控4帧一跳
		player.AddBuff(player.dwID, player.nLevel, 11077, 1, nCount)
		player.AddBuff(player.dwID, player.nLevel, 11336, 1)
	else
		player.AddBuff(player.dwID, player.nLevel, 11148, 1)
		RemoteCallToClient(player.dwID, "ChangeSkillSurfaceNum", 17078, 2)
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	local nlv = player.GetSkillLevel(16601)
	if nlv ~= 0 and player.GetBuff(10815, 1) then
		--释放通道AOE之前，删除奇穴加层buff
		for i = 1, 9 do
