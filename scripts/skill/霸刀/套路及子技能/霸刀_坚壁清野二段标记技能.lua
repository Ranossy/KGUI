---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_坚壁清野二段标记技能.lua
-- 更新时间:	2016/9/23 21:49:44
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	...
   	skill.AddAttribute(
   		...
   		"skill/霸刀/套路及子技能/霸刀_坚壁清野二段标记技能.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if lv ~= 0 and IsEnemy(player.dwID, dwCharacterID) then
		if player.GetSkillLevel(16912) == 1 then
			if IsPlayer(dwCharacterID) then
				player.CastSkill(16932, lv, TARGET.PLAYER, dwCharacterID)
			else
				player.CastSkill(16932, lv, TARGET.NPC, dwCharacterID)
			end
		else
			if IsPlayer(dwCharacterID) then
				player.CastSkill(16931, lv, TARGET.PLAYER, dwCharacterID)
			else
				player.CastSkill(16931, lv, TARGET.NPC, dwCharacterID)
			end
		end
	end
end
