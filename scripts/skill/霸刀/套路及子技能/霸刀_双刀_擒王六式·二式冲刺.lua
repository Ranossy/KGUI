---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_双刀_擒王六式·二式冲刺.lua
-- 更新时间:	2016/10/2 19:46:16
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_双刀_擒王六式·二式冲刺.lua",
		0
	);
	...
	--skill.AddSlowCheckSelfBuff(11278, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	skill.SetDelaySubSkill(4, 16935, dwSkillLevel)
	skill.SetDelaySubSkill(10, 16936, dwSkillLevel)
	...
	--skill.SetPublicCoolDown(1133);							-- 公共CD 1.5秒
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if target.nImmuneSkillMoveCount == 0 then
		target.BeHitted(player.dwID, 16871, 1, 39)
	end

	if player.GetSkillLevel(16954) == 1 then
		target.AddBuff(player.dwID, player.nLevel, 11318, 1)
	end

	---施展六式触发碎江天
	if player.GetSkillLevel(16930) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 11001, 1)
	end

	player.DelBuff(11278, 1)
	player.AddBuff(player.dwID, player.nLevel, 11279, 1)
	player.AddCDTime(1130, 160)

	---冲刺标记
	player.AddBuff(player.dwID, player.nLevel, 11156, 1, 1)

	---六式秘籍回复狂意
	if player.IsSkillRecipeActive(4186, 1) then
		player.nCurrentRage = player.nCurrentRage + math.min(10, player.nMaxRage - player.nCurrentRage)
	end

	---六式秘籍回复气劲
	if player.IsSkillRecipeActive(4187, 1) then
		player.nCurrentSunEnergy = player.nCurrentSunEnergy + math.min(10, player.nMaxSunEnergy - player.nCurrentSunEnergy)
	end
end
