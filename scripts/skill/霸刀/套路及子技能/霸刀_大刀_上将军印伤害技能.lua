---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_大刀_上将军印伤害技能.lua
-- 更新时间:	2016/9/17 15:48:54
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
--]]
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_大刀_上将军印伤害技能.lua",
		0
	);
	...
	--skill.nMinChannelInterval	= 100; 				-- 急速效果最小通道技间隔时间：默认是-1，通道不受急速影响。大于等于0则受急速影响，最通道间隔时间为填的值
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(16847) == 1 and not player.GetBuff(11478, 1) then
		player.AddBuff(player.dwID, player.nLevel, 11478, 1)
		player.ModifyCoolDown(1126, - 32)
	end
	
	local lv = player.GetSkillLevel(16627)
	if lv ~= 0 then
		if IsPlayer(dwCharacterID) then
			for i = 1, 6 do
				local nCanCast = 0
				if i == 1 then
					nCanCast = player.CastSkill(17043, lv, TARGET.PLAYER, dwCharacterID)
				else
					nCanCast = player.CastSkill(16798 + i, lv, TARGET.PLAYER, dwCharacterID)	
				end
				if nCanCast == 1 then
					break
				end
			...
			for i = 1, 6 do
				local nCanCast = 0
				if i == 1 then
					nCanCast = player.CastSkill(17043, lv, TARGET.NPC, dwCharacterID)
				else
					nCanCast = player.CastSkill(16798 + i, lv, TARGET.NPC, dwCharacterID)	
				end
				if nCanCast == 1 then
					break
				end
