---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_大刀_雷走风切普通伤害.lua
-- 更新时间:	2016/9/17 22:55:44
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_大刀_雷走风切普通伤害.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	if dwSkillLevel == 1 then
		skill.nChannelInterval = 45*0.8*0.7; 				-- 通道技间隔时间
	elseif dwSkillLevel < 10 then
		skill.nChannelInterval = (45 + (dwSkillLevel - 1) * 17)*0.8*0.7; 				-- 通道技间隔时间
	else
		skill.nChannelInterval = 224*0.8*0.7; 				-- 通道技间隔时间
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	----秘籍附带驱散
	if player.IsSkillRecipeActive(4138, 1) then
		player.CastSkill(16805, 1)
	end
	---秘籍附带加速
	if player.IsSkillRecipeActive(4139, 1) then
