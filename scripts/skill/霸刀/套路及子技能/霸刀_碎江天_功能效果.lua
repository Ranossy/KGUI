---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_碎江天_功能效果.lua
-- 更新时间:	2016/7/19 17:32:46
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_碎江天_功能效果.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--霸刀碎江天能量消耗
	local lv_Suijiangtian = player.GetSkillLevel(16304)
	
	local nRageCost = 20
	local nEnergyCost = 20
	if player.IsSkillRecipeActive(4196, 1) or player.IsSkillRecipeActive(4198, 1) or player.IsSkillRecipeActive(4200, 1) then
		nEnergyCost = 15
	end

	if player.IsSkillRecipeActive(4197, 1) or player.IsSkillRecipeActive(4199, 1) or player.IsSkillRecipeActive(4201, 1) then
		nRageCost = 15
	end
	
	if lv_Suijiangtian ~= 0 then
		if player.nCurrentEnergy >= nEnergyCost and player.nCurrentRage >= nRageCost then
			player.nCurrentEnergy = math.max(player.nCurrentEnergy - nEnergyCost, 0)
			player.nCurrentRage = math.max(player.nCurrentRage - nRageCost, 0)
			--实际双重伤害
			--双刀狂意伤害
			player.CastSkill(16316, lv_Suijiangtian, player.GetSkillTarget())
			--双刀刀魂伤害
			player.CastSkill(16317, lv_Suijiangtian, player.GetSkillTarget())
			--鞘刀狂意伤害
			player.CastSkill(16412, lv_Suijiangtian, player.GetSkillTarget())
			--鞘刀刀魂伤害
			player.CastSkill(16413, lv_Suijiangtian, player.GetSkillTarget())
			--大刀刀魂伤害
			player.CastSkill(16442, lv_Suijiangtian, player.GetSkillTarget())
			--大刀狂意伤害
			player.CastSkill(16443, lv_Suijiangtian, player.GetSkillTarget())
			
			--表现技能
			player.CastSkill(16337, lv_Suijiangtian, player.GetSkillTarget()) --双刀
			player.CastSkill(16394, lv_Suijiangtian, player.GetSkillTarget()) --大刀
			player.CastSkill(16429, lv_Suijiangtian, player.GetSkillTarget()) --鞘刀
		elseif player.nCurrentEnergy >= nEnergyCost then
			player.nCurrentEnergy = math.max(player.nCurrentEnergy - nEnergyCost, 0)
			--实际伤害
			player.CastSkill(16317, lv_Suijiangtian, player.GetSkillTarget())
			player.CastSkill(16442, lv_Suijiangtian, player.GetSkillTarget())
			player.CastSkill(16413, lv_Suijiangtian, player.GetSkillTarget())
			--表现技能
			player.CastSkill(16338, lv_Suijiangtian, player.GetSkillTarget()) --双刀
			player.CastSkill(16395, lv_Suijiangtian, player.GetSkillTarget()) --大刀
			player.CastSkill(16409, lv_Suijiangtian, player.GetSkillTarget()) --鞘刀
		else
			player.nCurrentRage = math.max(player.nCurrentRage - nRageCost, 0)
			--实际伤害
			player.CastSkill(16316, lv_Suijiangtian, player.GetSkillTarget())
			...
			player.CastSkill(16412, lv_Suijiangtian, player.GetSkillTarget())
			--表现技能
			player.CastSkill(16339, lv_Suijiangtian, player.GetSkillTarget()) --双刀
			player.CastSkill(16396, lv_Suijiangtian, player.GetSkillTarget()) --大刀
			player.CastSkill(16410, lv_Suijiangtian, player.GetSkillTarget()) --鞘刀
		end
	end
end
