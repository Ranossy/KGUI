---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_踏宴扬旗_无奇穴伤害控制效果.lua
-- 更新时间:	2017/3/18 22:37:08
-- 更新用户:	chenchen6-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	...
	{nDamageBase = 170, nDamageRand = 15, nCostMana = 0}, --level 10
};
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_踏宴扬旗_无奇穴伤害控制效果.lua", -- 属性值1
		0														-- 属性值2
	);
	skill.AddAttribute(
	...
	--skill.AddSlowCheckDestBuff(10815, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 不是大刀姿态
	--skill.AddSlowCheckDestOwnBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);	-- 需求目标属于自己的Buff

	-----------------技能施放技能需求-------------------------------------------
	skill.AddCheckSelfLearntSkill(17052, 0, SKILL_COMPARE_FLAG.EQUAL);     --检查比较Caster自己已学习的技能ID和等级
	...
	if dwSkillLevel == 1 then
		skill.nChannelInterval = 36 * 0.7; 				-- 通道技间隔时间
	elseif dwSkillLevel < 10 then
		skill.nChannelInterval = (36 + (dwSkillLevel - 1) * 13) * 0.7; 				-- 通道技间隔时间
	else
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	target.AddBuff(player.dwID, player.nLevel, 11110, 1, 2)
end
