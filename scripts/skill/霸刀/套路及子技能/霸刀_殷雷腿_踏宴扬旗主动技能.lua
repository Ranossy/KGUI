---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_殷雷腿_踏宴扬旗主动技能.lua
-- 更新时间:	2016/8/18 20:26:45
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_殷雷腿_踏宴扬旗主动技能.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	  skill.AddSlowCheckSelfBuff(11077, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);	--霸刀大刀旋转
	  skill.AddSlowCheckSelfBuff(11112, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);	--踏宴扬旗被动效果
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local lv = player.GetSkillLevel(16479)
	local nMaxOverDraftCount, nDraftCount = player.GetOverDraftCoolDown(1098)
	if nDraftCount == 1 then
		player.CastSkill(16481, lv, player.GetSkillTarget())
		player.AddCDTime(1098, 240)
		player.SetTimer(5, "scripts/skill/霸刀/套路及子技能/霸刀_殷雷腿_踏宴扬旗主动技能.lua", 0, 0)
	elseif nDraftCount == 2 then
		player.CastSkill(16482, lv, player.GetSkillTarget())
		player.AddCDTime(1098, 240)
	else
		player.CastSkill(16480, lv, player.GetSkillTarget())
	end
	
	player.SetOverDraftCoolDown(1098, nDraftCount + 1)
end
...
function OnTimer(player, nParam1, nParam2)
	...
	local lv = player.GetSkillLevel(16479)
	player.CastSkill(16483, lv, player.GetSkillTarget())
end
