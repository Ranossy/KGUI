---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_绝沧海_功能效果.lua
-- 更新时间:	2016/8/17 20:04:08
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_绝沧海_功能效果.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--双刀状态下崩雪潮和绝沧海不共CD
	if player.GetBuff(10815, 1) then
		player.ResetCD(1082)
	elseif player.GetBuff(10816, 1) then
		player.ResetCD(1061)
	end

	--大刀状态下崩雪潮变透支
	if player.GetBuff(10815, 1) then
		local nMaxOverDraftCount, nDraftCount = player.GetOverDraftCoolDown(1081)
		if nDraftCount ~= 0 then
			player.AddCDTime(1081, 48)
		end
		player.SetOverDraftCoolDown(1081, nDraftCount + 1)
		--RemoteCallToClient(player.dwID, "ChangeSkillSurfaceNum", 16101, (nMaxOverDraftCount - 1) - nDraftCount)
		if player.GetBuff(11028, 1)then
			player.DelBuff(11028, 1)
			player.AddBuff(player.dwID, player.nLevel, 11033, 1)
		elseif player.GetBuff(11033, 1)then
			player.DelBuff(11033, 1)
			player.SetTimer(20, "scripts/skill/霸刀/套路及子技能/霸刀_绝沧海_功能效果.lua", 0, 0)
		else
			player.AddBuff(player.dwID, player.nLevel, 11028, 1)
		end
	end
	if player.GetBuff(10814, 1) and player.IsSkillRecipeActive(4206, 1) then
		player.nCurrentRage = math.min(player.nCurrentRage + 15, player.nMaxRage)
		player.nCurrentEnergy = math.max(math.min(player.nCurrentEnergy, player.nMaxRage - player.nCurrentRage), 0)
	else
		player.nCurrentRage = math.min(player.nCurrentRage + 10, player.nMaxRage)
		player.nCurrentEnergy = math.max(math.min(player.nCurrentEnergy, player.nMaxRage - player.nCurrentRage), 0)
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	local nlv = player.GetSkillLevel(16345)
	if nlv ~= 0 then
		player.CastSkill(16379, nlv)
		player.AddBuff(player.dwID, player.nLevel, 11077, 1)
	end
end
