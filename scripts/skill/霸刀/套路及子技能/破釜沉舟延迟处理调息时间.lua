---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/破釜沉舟延迟处理调息时间.lua
-- 更新时间:	2016/10/12 2:39:12
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	if player.GetSkillLevel(16724) == 1 then
		if player.GetBuff(11191,1) then
			local nMaxOverDraftCount, nDraftCount = player.GetOverDraftCoolDown(1157)
			if nDraftCount == 0 then
				player.ModifyCoolDown(1157, - 160)
			elseif nDraftCount == 1 then
				player.AddCDTime(1157, 160)
			end
			player.SetOverDraftCoolDown(1157, nDraftCount + 1)
		else
			local nMaxOverDraftCount, nDraftCount = player.GetOverDraftCoolDown(1157)
			if nDraftCount == 0 then
				--player.ModifyCoolDown(1157, - 160)
			elseif nDraftCount == 1 then
				player.AddCDTime(1157, 320)
			end
			player.SetOverDraftCoolDown(1157, nDraftCount + 1)
		end
	end
	...
	--橙武效果重置技能调息
	if player.GetBuff(11376, 1) or player.GetBuff(11376, 2) or player.GetBuff(11376, 3) or player.GetBuff(11376, 4) then
		if player.GetSkillLevel(16724) == 1 then
			player.ClearCDTime(1157) --破釜沉舟
		else
			player.ClearCDTime(1112) --破釜沉舟
		end
	end
end
