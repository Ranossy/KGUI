---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/霸刀/套路及子技能/霸刀_双刀_雪意.lua
-- 更新时间:	2016/9/15 15:18:07
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/霸刀/套路及子技能/霸刀_双刀_雪意.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(10814, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);	-- 需求自身Buff
	skill.AddSlowCheckSelfBuff(11077, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);	--霸刀大刀旋转
	--skill.AddSlowCheckSelfBuff(11163, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);  --能量高于10点切换需求
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 10814, 1);		-- 设置1号位Buff
	...
	skill.SetNormalCoolDown(1, 1138);	--姿态技能3秒调息
	skill.SetNormalCoolDown(2, 1139);	--姿态主被动内置CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	--[[
	if player.nCurrentEnergy < 10 then
		return SKILL_RESULT_CODE.FAILED
	end
	--]]
	return nPreResult;
...
function OnSkillLevelUp(skill, player)
	...
	if player.GetSkillLevel(16608) ~= lv then
		player.LearnSkillLevel(16608, lv, player.dwID)
	end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	RemoteCallToClient(player.dwID, "OnSkillReplace", 16166, 16608, 1, 16166) ---双刀被动切主动
end
