...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/npc/不可复活技能.lua",				
   		0														
   		);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	--skill.AddSlowCheckDestBuff(5197, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 5197, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	local buff = player.GetBuff(5197, 1)
	if  buff then
		if buff.nLeftActiveCount < 5 then
			player.AddBuff(0,99,5197,1,60)
		end
	else
		player.AddBuff(0,99,5197,1,60)
	end
