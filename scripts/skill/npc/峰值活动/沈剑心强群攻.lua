---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/峰值活动/沈剑心强群攻.lua
-- 更新时间:	2014/12/2 16:07:15
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 80);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	local n = Random(1, 3)
	if n == 1 then 
		player.Talk(GetEditorString(7, 1070))
	end
	if n == 2 then 
		player.Talk(GetEditorString(7, 1071))
	end
	if n == 3 then 
		player.Talk(GetEditorString(7, 1075))
	end
	return nPreResult;
end
