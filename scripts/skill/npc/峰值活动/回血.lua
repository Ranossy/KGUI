---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/峰值活动/回血.lua
-- 更新时间:	2014/12/9 14:48:11
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	--skill.SetNormalCoolDown(1, 241);	--技能普通CD
...
function Apply(dwCharacterID, dwScourceID)
	local player = GetPlayer(dwCharacterID);
	if not player then
		return;
	end
	player.nCurrentLife = math.min(player.nMaxLife, player.nCurrentLife + player.nMaxLife * 0.2);
	
	if dwCharacterID ~= dwScourceID then
		player.AddBuff(0, 99, 8698, 1);
		local player1 = GetPlayer(dwScourceID);
		local n = Random(1, 3);
		if player1 then 
			if n == 1 then 
				player1.Talk(GetEditorString(7, 1083))
			end
			if n == 2 then 
				player1.Talk(GetEditorString(7, 1084))
			end
			if n == 3 then 
				player1.Talk(GetEditorString(7, 1085))
			end
		end
	else
		local n = Random(1, 6)
		if n == 1 then 
			player.Talk(GetEditorString(5, 493))
		end
		if n == 2 then 
			player.Talk(GetEditorString(5, 494))
		end
		if n == 3 then 
			player.Talk(GetEditorString(5, 495))
		end
	end
end
