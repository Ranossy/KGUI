---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/峰值活动/自爆.lua
-- 更新时间:	2014/12/2 16:31:17
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- 时间相关 -------------------------------------------------
	skill.nPrepareFrames  	= 5 * 16;				-- 吟唱帧数
...
function Apply(dwCharacterID)
	...
	local	dwSmokeNpcId = 12221;
	if not player then
		return;
	end

	if player.nMoveState ~= MOVE_STATE.ON_DEATH then
	 	player.GetScene().SetTimer(16, "scripts/skill/npc/峰值活动/自爆.lua", dwCharacterID, 0);
	end
end
...
function OnTimer(KScene, nParam1, nParam2)
