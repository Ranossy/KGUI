---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/峰值活动/杨逸飞_江逐月天.lua
-- 更新时间:	2015/9/7 16:39:04
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/峰值活动/杨逸飞_江逐月天.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.szName == GetEditorString(8, 1875) or player.GetBuff(4964, 2) then
		if Random(1, 3) == 1 then
			player.Talk(GetEditorString(8, 2102), 5)
		end
		player.ClearCDTime(948)
	end
	...
	--创建江逐月天气场NPC
	local npc = scene.CreateNpc(44764, target.nX, target.nY, target.nZ, 0, 8 * 16)
	...
	if npc then
		...
		--持续打效果的buff
		npc.AddBuff(player.dwID, player.nLevel, 9676, 1, 8)

		--对范围内小队目标添加暂停CD效果
		player.CastSkillXYZ(14583, 1, npc.nX, npc.nY, npc.nZ)
