---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/峰值活动/峰值沈剑心招玩家.lua
-- 更新时间:	2015/8/18 16:25:27
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/峰值活动/峰值沈剑心招玩家.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(8695, 0,  BUFF_COMPARE_FLAG.EQUAL, 0,  BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
--	skill.AddSlowCheckDestBuff(8695, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	skill.AddSlowCheckDestBuff(4964, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 8686, 1);		-- 设置1号位Buff
	...
	skill.SetNormalCoolDown(1, 382);	--技能普通CD
	...
	----------------- 时间相关 -------------------------------------------------
	skill.nPrepareFrames = 3 * 16;				-- 吟唱帧数
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if n == 1 then
		if player.szName == GetEditorString(8, 1875) then
			player.Talk(GetEditorString(8, 2108))
		else
			player.Talk(GetEditorString(7, 1076))
		end
		
	end
	if n == 2 then
		if player.szName == GetEditorString(8, 1875) then
			player.Talk(GetEditorString(8, 2109))
		else
			player.Talk(GetEditorString(7, 1077))
		end
	...
	if n == 3 then
		...
		if player.szName == GetEditorString(8, 1875) then
			player.Talk(GetEditorString(8, 2110))
		else
			player.Talk(GetEditorString(7, 1088))
		end
	end
	return nPreResult;
end
...
--	print(dwCharacterID)
--	print(dwSourceID)
	if dwCharacterID == dwSourceID then
		return;
	end
	...
	if not scene.CheckPlayerInKillersSet(dwCharacterID) then
		scene.AddPlayerToKillersSet(dwCharacterID);
	end
	--local tBuffLevel = {
		--[ROLE_TYPE.LITTLE_BOY] = 5,
		--[ROLE_TYPE.LITTLE_GIRL] = 4,
		--[ROLE_TYPE.STANDARD_FEMALE] = 3,
		--[ROLE_TYPE.STANDARD_MALE] = 1,
	--};
	--if not tBuffLevel[target.nRoleType] then
		--return;
	--end
	target.AddBuff(0, 99, 8686, 6, 600);
	--动态技能栏
	target.SetDynamicSkillGroup(210);
	target.AddBuff(0, 99, 4969, 1);	--免控
	target.AddBuff(0, 99, 8696, 1);
	target.SetCurrentKillPoint(0);
end
