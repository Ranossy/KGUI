---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/峰值活动/招策划.lua
-- 更新时间:	2014/12/5 11:59:45
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	--local n = Random(1, 3)
	--if n == 1 then 
		--player.Talk("找个志同道合的小伙伴！biu~biu~biu~")
	--end
	--if n == 2 then 
		--player.Talk("变身吧！我的小伙伴！")
	--end
	--if n == 3 then 
		--player.Talk("变身吧！我的小伙伴们！")
	--end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSourceID)
	...
	target.AddBuff(0, 99, 8686, tBuffLevel[target.nRoleType], 600);
	--动态技能栏
	target.SetDynamicSkillGroup(190);
	target.AddBuff(0, 99, 4969, 1);	--免控
	target.AddBuff(0, 99, 8696, 1);
	target.DelBuff(9686,1)
