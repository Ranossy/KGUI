---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/峰值活动/杨逸飞-云生结海.lua
-- 更新时间:	2015/9/7 16:50:21
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/峰值活动/杨逸飞-云生结海.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	--skill.SetNormalCoolDown(1, 901);	--音域技能单独100sGCD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if Random(1, 3) == 1 then
		player.Talk(GetEditorString(8, 2103), 5)
	end
	...
	--创建云生结海气场NPC
	local npc = scene.CreateNpc(44765, target.nX, target.nY, target.nZ, 0, 8 * 16)

	if npc then
		--防止其它NPC把气场搜到仇恨范围
		npc.AddBuff(0, 99, 9256, 1, 1)

		--持续打效果的buff
		npc.AddBuff(player.dwID, player.nLevel, 9680, 1, 8)

		--if player.GetSkillLevel(14152) == 1 then
		--该buff用于记录平分范围内玩家的百分比值
		--player.AddBuff(player.dwID, player.nLevel, 9267, 1)

		--S_14153释放音域的时候平衡音域范围内所有友方目标奇穴百分比值
		--player.CastSkillXYZ(14153, 1, npc.nX, npc.nY, npc.nZ)
		--end

		--对范围内小队目标添加平摊伤害和平摊治疗效果buff(一个技能脚本判定加2个buff)
		player.CastSkillXYZ(14584, 1, npc.nX, npc.nY, npc.nZ)
	...
	local fenshen = nil
	for i = 1, #tNPC do
		fenshen = scene.GetNpcByNickName(player.dwID.."yangyifei_"..i)
		if fenshen then
			scene.DestroyNpc(fenshen.dwID)
		end
		fenshen = scene.CreateNpc(46033, player.nX + tNPC[i][1], player.nY + tNPC[i][2], player.nZ, 0, 30* GAME_FPS,player.dwID.."yangyifei_"..i)
		if fenshen then
			fenshen.AddBuff(player.dwID,player.nLevel,9678,1,30)
		end
	end
	player.AddBuff(player.dwID,player.nLevel,9678,1,30)
end
