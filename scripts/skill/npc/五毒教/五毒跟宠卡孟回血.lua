...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
   		...
   		"skill/npc/五毒教/五毒跟宠卡孟回血.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local kameng = GetNpc(dwCharacterID)
	if kameng then
		local ownerID = kameng.GetCustomInteger4(1)
		if ownerID == dwSkillSrcID then
			kameng.nCurrentLife = kameng.nCurrentLife + kameng.nMaxLife * 0.03
		else
			print("owmerID IS :"..tostring(ownerID))
		end
	else
		print("no kameng here!")
	end
