---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/太原/月华_幻月流光.lua
-- 更新时间:	2014/11/22 15:31:32
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npcA and playerA and playerA.nMoveState ~= MOVE_STATE.ON_DEATH then
		...
		if not buffA then
			npcA.CastSkill(13494, 1, TARGET.PLAYER, dwCharacterID)
		end
	end

	local npcb = GetNpc(dwCharacterID)
	if npcb then
		npcb.AddBuff(0, 99, 8671, 1)
	end
end
