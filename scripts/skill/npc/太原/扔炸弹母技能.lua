---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/太原/扔炸弹母技能.lua
-- 更新时间:	2014/12/18 18:01:49
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.nMoveState~=MOVE_STATE.ON_DEATH then
		...
		player.CastSkillXYZ(13625, 1, nX, nY, nZ)
		player.CastSkillXYZ(13626, 1, nX, nY, nZ)
		player.PlaySfx(1091, nX, nY, nZ, 0)
		player.PlaySfx(1092,nX, nY, nZ-3000,0)
		local tRadius = {1 * 64, 3 * 64, 5 * 64, 7 * 64, 9 * 64}
		--for i = 1, #tRadius do
		local i = Random(#tRadius)
		local xx = 0
		local yy = 0
		local yy2 = 0
		xx = math.floor(Random(1, 2 * tRadius[i]*1000)/1000)-tRadius[i]
		yy2 = math.floor(math.sqrt(tRadius[i] * tRadius[i]-xx * xx))
		yy = Random(1000)
		if yy < 500 then
			yy = yy2
		else
		...
		for j = 1,50 do
			local npcNickName = "12989_MoGu_".. j
			local scene = player.GetScene()
			if not scene.GetNpcByNickName(npcNickName) then
				local npcNew = scene.CreateNpc(41676, nX + xx, nY + yy, nZ, Random(256), 60 * 16, npcNickName)
				break
			end
		end
