---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/太原/炸弹子技能减益.lua
-- 更新时间:	2014/12/20 11:23:44
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
Include("scripts/Map/太原/include/PQ加分提示头文件.lua")
Include("scripts/Map/太原/include/PQ头文件.lua")
local nPQID = 13
local nAddPlayerValue = 1 -- 个人贡献度
local nQuestID = 12988
local nValueIndex = 1
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--local tPQValue, _ = TaiYuanPQ_GetPQTableByID(nPQID)
	--if tPQValue then
		--SetPlayerPQValueTips(player, nPQID, nAddPlayerValue, nValue)-- player，pqID,本次增加的个人贡献度，当前PQ的个人贡献度总分
	--end
	local nQuestIndex = player.GetQuestIndex(nQuestID)
	local nQuestPhase = player.GetQuestPhase(nQuestID)
	if nQuestIndex and nQuestPhase == 1 then
		player.SetQuestValue(nQuestIndex, nValueIndex, player.GetQuestValue(nQuestIndex, nValueIndex) + 1)
	end
end
