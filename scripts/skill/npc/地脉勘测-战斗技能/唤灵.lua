---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/地脉勘测-战斗技能/唤灵.lua
-- 更新时间:	2015/6/1 11:25:44
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/地脉勘测-战斗技能/唤灵.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID)
	...
	for i = 1, 3 do
		if not scene.GetNpcByNickName(npc.szName.."clone"..i) then
			--	local clone = scene.CreateNpc(43285, npc.nX, npc.nY + 100, npc.nZ, npc.nFaceDirection, 720, npc.szName.."clone"..i)
			local clone = scene.CreateNpc(43285, npc.nX, npc.nY + 100 * i, npc.nZ, npc.nFaceDirection, 720, npc.szName.."clone"..i)
			if clone then
				clone.PlaySfx(dwSFXID, clone.nX, clone.nY, clone.nZ)
				npc.CopyThreatList(clone.dwID)
				clone.nCurrentLife = math.ceil(npc.nCurrentLife/10) + 1
				clone.SetModelID(npc.dwModelID)
			end
			break
		end
	end
