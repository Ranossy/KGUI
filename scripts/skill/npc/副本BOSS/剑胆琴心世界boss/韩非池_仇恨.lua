---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/剑胆琴心世界boss/韩非池_仇恨.lua
-- 更新时间:	2015/10/29 10:13:14
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
 ...
 -- ]]
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/剑胆琴心世界boss/韩非池_仇恨.lua", -- 属性值1
		0 -- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local hanfeichi = scene.GetNpcByNickName("shijiebosshanfeichi")
	if hanfeichi then
		if playertarget then
			hanfeichi.ModifyThreat(playertarget.dwID, 1)
			npc.SetCustomInteger1(8, 1)
		end
	end
	...
	if hanfeichi then
		if npctarget then
			hanfeichi.ModifyThreat(npctarget.dwID, 1)
			npc.SetCustomInteger1(8, 1)
		end
	end
end
