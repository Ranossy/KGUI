...
tSkillData =
{
	...
	{nDamageBase = 11550, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/BOSS1_樱返.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if target then
		if target.GetBuff(11616, 1) then
			target.DelBuff(11616, 1)
		end
		target.AddBuff(npc.dwID, npc.nLevel, 11617, 1)
		target.PlaySfx(1461, target.nX, target.nY, target.nZ+512)
	end
end
