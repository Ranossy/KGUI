---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/刀轮海厅/一剑决·山崩.lua
-- 更新时间:	2017/2/17 15:57:51
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/一剑决·山崩.lua", 
		0											
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if targetType == TARGET.PLAYER then--玩家目标
		...
		if playerTarget.nCurrentMana >= 0.6 * playerTarget.nMaxMana then
			npc.CastSkill(17385,1)--三倍
		elseif playerTarget.nCurrentMana >= 0.3 * playerTarget.nMaxMana then
			npc.CastSkill(17384,1)--两倍
		else
			npc.CastSkill(17383,1)--单倍
		end
	end
