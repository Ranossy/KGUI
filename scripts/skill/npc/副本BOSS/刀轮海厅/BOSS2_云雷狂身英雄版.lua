---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/刀轮海厅/BOSS2_云雷狂身英雄版.lua
-- 更新时间:	2017/3/23 20:16:45
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/BOSS2_云雷狂身英雄版.lua", -- 属性值1
		0															--属性值2
	);
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc_daoqi then
		--延迟2秒做动作和范围伤害
		player.SetTimer(2, "scripts/skill/霸刀/套路及子技能/霸刀_大刀_秀明尘身主动技能.lua", npc_daoqi.dwID, 0)
	...
	--playerTarget.AddAdvancedDynamicObstacleInCircle(npc.nX, npc.nY, npc.nZ - 2 * 64 * 8, 6 * 64, npc.nZ + 7 * 8 * 64, 4 * 16, ADVANCED_DYNAMIC_OBSTACLE_TYPE.ENEMY_PARTY)
	--playerTarget.PlaySfx(1543, npc.nX, npc.nY, npc.nZ)
	if  playerTarget then
		npc.CastSkill(17357, 2, TARGET.PLAYER, dwCharacterID)
		playerTarget.AddBuff(npc.dwID, npc.nLevel, 11833, 1)		--割裂英雄版
		playerTarget.AddBuff(npc.dwID, npc.nLevel, 11709, 1)		--割裂监控
	end		--打人
	if npcTarget then
		npc.CastSkill(17357, 2, TARGET.NPC, dwCharacterID)
		npcTarget.AddBuff(npc.dwID, npc.nLevel, 11833, 1)		--割裂英雄版
		npcTarget.AddBuff(npc.dwID, npc.nLevel, 11709, 1)		--割裂监控
	end		--打影子
end
