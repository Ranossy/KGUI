---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/刀轮海厅/BOSS1_阳炎.lua
-- 更新时间:	2017/3/21 10:36:24
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 39000, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 59400, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/BOSS1_阳炎.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if target then
		target.AddBuff(npc.dwID, npc.nLevel, 11848, 1)
		if target.GetBuff(11615, 1) then
			target.DelGroupBuff(11615, 1)
		end
		if target.GetBuff(11616, 1) then
			target.DelGroupBuff(11616, 1)
		end
		if target.GetBuff(11617, 1) then
			target.DelGroupBuff(11617, 1)
		end
		target.PlaySfx(1533, target.nX, target.nY, target.nZ)
		target.PlaySfx(1336, target.nX, target.nY, target.nZ)
		target.PlaySfx(1306, target.nX, target.nY, target.nZ)
	end
end
