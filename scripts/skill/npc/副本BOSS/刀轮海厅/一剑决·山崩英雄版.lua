---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/刀轮海厅/一剑决·山崩英雄版.lua
-- 更新时间:	2017/3/23 21:02:01
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/一剑决·山崩英雄版.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local npc = GetNpc(dwSkillSrcID)--目前并非选定别名目标 而是技能释放目标
	if not npc then
		return
	end

	local targetType, targetId = npc.GetTarget()
	if targetType == TARGET.PLAYER then--玩家目标
		local playerTarget = GetPlayer(targetId)
		if not playerTarget then
			return
		end
		if playerTarget.nCurrentMana >= 0.6 * playerTarget.nMaxMana then
			npc.CastSkill(17385, 2)--三倍
		elseif playerTarget.nCurrentMana >= 0.3 * playerTarget.nMaxMana then
			npc.CastSkill(17384, 2)--两倍
		else
			npc.CastSkill(17383, 2)--单倍
		end
	end

	if targetType == TARGET.NPC then--NPC目标
		local npcTarget = GetNpc(targetId)
		if not npcTarget then
			return
		end
		if npcTarget.nCurrentMana >= 0.6 * npcTarget.nMaxMana then
			npc.CastSkill(17385, 2)--三倍
		elseif npcTarget.nCurrentMana >= 0.3 * npcTarget.nMaxMana then
			npc.CastSkill(17384, 2)--两倍
		else
			npc.CastSkill(17383, 2)--单倍
		end
