---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/刀轮海厅/BOSS2_闹须弥·望.lua
-- 更新时间:	2017/2/16 21:52:56
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/BOSS2_闹须弥·望.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local Npc = scene.GetNpcByNickName("liuxueqian")
	if not npc then
		return
	end
	...
	if targetType == TARGET.NPC then
		...
		--npc目标回血
		if npcTarget.nCurrentLife >= npcTarget.nMaxLife * 0.95 then
		npcTarget.nCurrentLife = npcTarget.nMaxLife
		else
		npcTarget.nCurrentLife = npcTarget.nCurrentLife + npcTarget.nMaxLife * 0.05
		end
		Npc.SetCustomBoolean(51, true)
	end
