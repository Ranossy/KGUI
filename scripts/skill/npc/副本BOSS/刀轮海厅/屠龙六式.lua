---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/刀轮海厅/屠龙六式.lua
-- 更新时间:	2017/3/13 15:08:31
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/屠龙六式.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nID = npc.GetCustomInteger4(20)
	local player = GetPlayer(nID)
	if not player then
		return
	end
	npc.SetTarget(TARGET.PLAYER, nID)
	npc.TurnToCharacter(player.dwID)
	npc.DoAction(0, 10060)
	npc.CastSkill(17415, 1)		--冲
	npc.AddBuff(npc.dwID, npc.nLevel, 11732, 1)		--打击一监控buff
end
