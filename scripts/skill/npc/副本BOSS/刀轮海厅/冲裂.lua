---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/刀轮海厅/冲裂.lua
-- 更新时间:	2017/2/17 13:03:05
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
		...
		"skill/npc/副本BOSS/刀轮海厅/冲裂.lua", -- 属性值1
		0												--属性值2
		);    
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcYw = scene.CreateNpc(55923, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1, "yuanwei")--创建NPC并使其目标为npc的目标
	if not npcYw then
		return
	end
	npcYw.SetTarget(TARGET.PLAYER, player.dwID)
	npcYw.nCurrentMana=npcYw.nCurrentMana* (npc.nCurrentMana/npc.nMaxMana)--设置NPCYw的蓝量百分比与原NPC统一
	npc.CastSkill(17381, 1)--原NPC冲刺
end
