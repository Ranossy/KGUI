---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/刀轮海厅/胧返.lua
-- 更新时间:	2017/2/17 9:49:20
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/胧返.lua", -- 属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetOTActionState() == 1 or player.GetOTActionState() == 2 then
		player.AddBuff(npc.dwID,npc.nLevel,11714,1)--胧返沉默BUFF
		----npc回蓝部分↓
		local healManaPercent = 10 / 100 --回蓝百分比更改
		if npc.nCurrentMana >= npc.nMaxMana-npc.nMaxMana * healManaPercent then
			npc.nCurrentMana = npc.nMaxMana
		else
			npc.nCurrentMana = npc.nCurrentMana + npc.nMaxMana * healManaPercent
		end
	end
end
