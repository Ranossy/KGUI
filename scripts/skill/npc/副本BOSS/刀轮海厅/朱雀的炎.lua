---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/刀轮海厅/朱雀的炎.lua
-- 更新时间:	2017/3/22 16:30:40
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/朱雀的炎.lua", -- 属性值1
		0															--属性值2
	);
	--[[
	...
	skill.nHeight = 12 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 262 then
		if  playerTarget then
			playerTarget.AddBuff(npc.dwID, npc.nLevel, 11602, 1)
			npc.CastSkill(17365, 1, TARGET.PLAYER, dwCharacterID)
		end		--打人
		if npcTarget then
			npcTarget.AddBuff(npc.dwID, npc.nLevel, 11602, 1)
			npc.CastSkill(17365, 1, TARGET.NPC, dwCharacterID)
		end		--打影子
	else
		if  playerTarget then
			playerTarget.AddBuff(npc.dwID, npc.nLevel, 11830, 1)
			npc.CastSkill(17365, 2, TARGET.PLAYER, dwCharacterID)
		end		--打人
		if npcTarget then
			npcTarget.AddBuff(npc.dwID, npc.nLevel, 11830, 1)
			npc.CastSkill(17365, 2, TARGET.NPC, dwCharacterID)
		end		--打影子
	end	
end
