---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/刀轮海厅/月泉流引英雄版.lua
-- 更新时间:	2017/4/5 9:55:51
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/月泉流引英雄版.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if playertarget.nCurrentMana >= 0.05 then		--蓝量多于5%
		playertarget.nCurrentMana = playertarget.nCurrentMana - playertarget.nMaxMana * 0.015
		npc.AddBuff(npc.dwID, npc.nLevel, 11595, 1)
		npc.nCurrentMana = npc.nCurrentMana + npc.nMaxMana * 0.015
	else
		playertarget.nCurrentLife = playertarget.nCurrentLife - playertarget.nMaxLife * 0.03
		npc.nCurrentMana = npc.nCurrentMana + npc.nMaxMana * 0.03
		playertarget.AddBuff(npc.dwID, npc.nLevel, 11858, 1)
	end
	local Nbuff = npc.GetBuff(11595, 1)
	local Num = Nbuff.nStackNum
	if Num >= 50 then
		playertarget.AddBuff(npc.dwID, npc.nLevel, 11847, 1)
	end		--红月阳50层以上每层给玩家加一个赤炎焦土dot
end
