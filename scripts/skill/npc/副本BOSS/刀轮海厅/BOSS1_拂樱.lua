...
tSkillData =
{
	...
	{nDamageBase = 11880, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/BOSS1_拂樱.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if target then
		if target.GetBuff(11617, 1) then
			target.DelBuff(11617, 1)
		end
		target.AddBuff(npc.dwID, npc.nLevel, 11615, 1)
		target.PlaySfx(1431, 0, 0, 0)
		target.PlaySfx(1417, target.nX, target.nY, target.nZ+512)
		target.PlaySfx(1363, target.nX, target.nY, target.nZ - 512)
		target.PlaySfx(8643, target.nX, target.nY, target.nZ)
	end
end
