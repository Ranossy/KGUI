---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/刀轮海厅/BOSS1_落樱.lua
-- 更新时间:	2017/3/23 19:52:06
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 6500, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 9900, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/BOSS1_落樱.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if target then
		if target.GetBuff(11615, 1) then
			target.DelBuff(11615, 1)
		end
		target.AddBuff(npc.dwID, npc.nLevel, 11616, 1)
		target.AddBuff(npc.dwID, npc.nLevel, 11846, 1)
		target.PlaySfx(8644, 0, 0, 0)
		target.PlaySfx(1376, 0, 0, 0)
		target.PlaySfx(1417, target.nX, target.nY, target.nZ + 512)
		target.PlaySfx(1327, target.nX, target.nY, target.nZ)
	end
end
