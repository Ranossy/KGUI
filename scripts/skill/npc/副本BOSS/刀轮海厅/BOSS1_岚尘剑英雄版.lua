---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/刀轮海厅/BOSS1_岚尘剑英雄版.lua
-- 更新时间:	2017/3/23 19:55:31
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/刀轮海厅/BOSS1_岚尘剑英雄版.lua", -- 属性值1
		0															--属性值2
	);
	--[[
	...
	--skill.nRectWidth = 7 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcTarget = GetNpc(dwCharacterID)		--宝宝，影子等
	local npc = GetNpc(dwSkillSrcID)			--Boss
	if not npc then
		return
	end
	if  player and player.nMoveState ~= MOVE_STATE.ON_DEATH then

		if player.GetBuff(11634, 1) or player.GetBuff(11635, 1)
			or player.GetBuff(11636, 1) or player.GetBuff(11637, 1)
			or player.GetBuff(11641, 1) then
			npc.CastSkill(17370, 4, TARGET.PLAYER, dwCharacterID)
		else
			npc.CastSkill(17370, 3, TARGET.PLAYER, dwCharacterID)
		end
		if GetPlayerTDH(player) ~= nil then
			if GetPlayerTDH(player) == 1 then
				player.AddBuff(npc.dwID, npc.nLevel, 11636, 1)
			end		--外功
			if GetPlayerTDH(player) == 2 then
				player.AddBuff(npc.dwID, npc.nLevel, 11634, 1)
			end		--阳性内功
			if GetPlayerTDH(player) == 3 then
				player.AddBuff(npc.dwID, npc.nLevel, 11635, 1)
			end		--阴性内功
			if GetPlayerTDH(player) == 4 then
				player.AddBuff(npc.dwID, npc.nLevel, 11637, 1)
			end		--混元内功
			if GetPlayerTDH(player) == 5 then
				player.AddBuff(npc.dwID, npc.nLevel, 11641, 1)
			end		--毒性内功
			if GetPlayerTDH(player) == 6 then
				player.AddBuff(npc.dwID, npc.nLevel, 11915, 1)
			end		--奶妈
		end
		--加buff
	end		--打人
	if npcTarget then
		npc.CastSkill(17370, 1, TARGET.NPC, dwCharacterID)
	end		--打影子
end
