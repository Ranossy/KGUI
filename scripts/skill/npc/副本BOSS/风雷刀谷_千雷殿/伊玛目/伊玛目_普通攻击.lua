---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/伊玛目/伊玛目_普通攻击.lua
-- 更新时间:	2017/1/9 16:27:08
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	...
	{nDamageBase = 57200, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/伊玛目/伊玛目_普通攻击.lua", -- 属性值1
		0															--属性值2
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 5397, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 264 or scene.dwMapID == 272 then
		player.AddBuff(player.dwID, player.nLevel, 11612, 1, 20)
	end

	if scene.dwMapID == 270 then
		if npc and npc.GetBuff(3819, 1) then
			npc.AddBuff(npc.dwID, npc.nLevel, 11599, 2)
		end
		player.AddBuff(player.dwID, player.nLevel, 11612, 2, 40)
	end
end
