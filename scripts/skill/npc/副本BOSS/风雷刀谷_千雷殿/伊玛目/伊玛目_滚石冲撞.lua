---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/伊玛目/伊玛目_滚石冲撞.lua
-- 更新时间:	2017/1/17 10:02:14
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/伊玛目/伊玛目_滚石冲撞.lua", --属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
	...
	skill.AddSlowCheckDestBuff(11633, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if playertarget then
		playertarget.Stop()
		--playertarget.AddBuff(playertarget.dwID, playertarget.nLevel, 11633, 1, 3)
		if npc then
			local scene = npc.GetScene()
			if scene.dwMapID == 264 or scene.dwMapID == 272 then
				npc.CastSkill(17640, 1, TARGET.PLAYER, playertarget.dwID)
			end

			if scene.dwMapID == 270 then
				npc.CastSkill(17640, 2, TARGET.PLAYER, playertarget.dwID)
			end
		end
	end
end
