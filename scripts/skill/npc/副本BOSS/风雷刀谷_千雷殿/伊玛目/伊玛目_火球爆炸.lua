---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/伊玛目/伊玛目_火球爆炸.lua
-- 更新时间:	2017/1/9 15:37:49
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 23400, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 31500, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/伊玛目/伊玛目_火球爆炸.lua", --属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local yimamu = scene.GetNpcByNickName("yimamu")
	if yimamu then
		if scene.dwMapID == 264 or scene.dwMapID == 272 then
			yimamu.AddBuff(yimamu.dwID, yimamu.nLevel, 11599, 1)
		end

		if scene.dwMapID == 270 then
			yimamu.AddBuff(yimamu.dwID, yimamu.nLevel, 11599, 2)
		end
	end

	if scene.dwMapID == 264 or scene.dwMapID == 270 then
		local diaoluohuoqiukongzhi = scene.GetNpcByNickName("diaoluohuoqiukongzhi")
		if diaoluohuoqiukongzhi then
			diaoluohuoqiukongzhi.SetCustomInteger1(1, 1)--烈焰焚身成就没了
		end
	end
end
