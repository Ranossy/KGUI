---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_水流连接.lua
-- 更新时间:	2017/5/11 14:29:42
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_水流连接.lua", -- 属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		if caster then
			caster.CastSkill(17665, 1, TARGET.PLAYER, player.dwID)
			return
		end
	end
	...
	if npc then
		...
		if npc.szName == "kuangshi_huo"  and npcKc1 then
			...
			if dis <= 128 then
				if npcKc1 then
					caster.CastSkill(17663, 1, TARGET.NPC, npcKc1.dwID)
					return 
				end
			else
				if npcTzt then
					caster.CastSkill(17663, 1, TARGET.NPC, npcTzt.dwID)
					return 
				end
			end
		end
		...
		if npc.szName == "kuangshi_shui"  and npcKc2 then
			...
			if dis <= 128 then
				if npcKc2 then
					caster.CastSkill(17663, 1, TARGET.NPC, npcKc2.dwID)
					return 
				end
			else
				if npcTzt then
		...
		if npc.szName == "kuangshi_mu"  and npcKc3 then
			...
			if dis <= 128 then
				if npcKc3 then
					caster.CastSkill(17663, 1, TARGET.NPC, npcKc3.dwID)
					return 
				end
			else
				if npcTzt then
		...
		if npc.szName == "kuangshi_tu"  and npcKc4 then
			...
			if dis <= 128 then
				if npcKc4 then
					caster.CastSkill(17663, 1, TARGET.NPC, npcKc4.dwID)
					return 
				end
			else
				if npcTzt then
		...
		if npc.szName == "kuangshi_jin"  and npcKc5 then
			...
			if dis <= 128 then
				if npcKc5 then
					caster.CastSkill(17663, 1, TARGET.NPC, npcKc5.dwID)
					return 
				end
			else
				if npcTzt then
		...
		if npc then
			if npc.szName == "duanzaotai" or  npc.szName == "kuangche_huo" or  npc.szName == "kuangche_shui" or  npc.szName == "kuangche_mu" or  npc.szName == "kuangche_tu" or  npc.szName == "kuangche_jin" then
				...
				if caster then
					--print("水1")
					caster.CastSkill(17663, 1, TARGET.NPC, npc.dwID)
					return 
				end
			else
				if caster and npcTzt then
					--print("水2")
					caster.CastSkill(17680, 1, TARGET.NPC, npcTzt.dwID)
					return 
				end
			end
		end
