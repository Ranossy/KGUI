---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_云凰斩3火AOE.lua
-- 更新时间:	2017/4/17 14:37:22
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_云凰斩3火AOE.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.PlaySfx(1549, player.nX, player.nY, player.nZ + 512)
	player.PlaySfx(1361, player.nX, player.nY, player.nZ + 512)
	player.PlaySfx(1336, player.nX, player.nY, player.nZ+512)
	if scene.dwMapID == 264 or scene.dwMapID == 272 then
		player.AddBuff(npc.dwID, npc.nLevel, 11772, 1)
		if player.GetBuff(11772, 1) then
			local Nbuff = player.GetBuff(11772, 1)
			local Num = Nbuff.nStackNum
			npc.CastSkill(17601, Num, TARGET.PLAYER, player.dwID)		--灼烧点爆高伤
		end
	else
		player.AddBuff(npc.dwID, npc.nLevel, 11804, 2)
		if player.GetBuff(11804, 2) then
			local Nbuff = player.GetBuff(11804, 2)
			local Num = Nbuff.nStackNum
			npc.CastSkill(17601, Num + 5, TARGET.PLAYER, player.dwID)		--灼烧点爆高伤
		end	
	end
end
