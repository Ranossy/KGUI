---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_疾走麒麟.lua
-- 更新时间:	2017/3/3 15:38:09
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_疾走麒麟.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
	...
	skill.nHeight = 200 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local tDirection = npc.nFaceDirection
	local nY = npc.nY + math.floor(math.sin(tDirection / 128 * math.pi) * 10 * 64)
	local nX = npc.nX + math.floor(math.cos(tDirection / 128 * math.pi) * 10 * 64)
	if scene.dwMapID == 264 or scene.dwMapID == 272 then
		local npcDt = scene.CreateNpc(56048, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, - 1)
		if npcDt then
			npcDt.RunTo(nX, nY)
		end
	end
	...
	if scene.dwMapID == 270 then
		local npcDt = scene.CreateNpc(56909, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, - 1)
		if npcDt then
			npcDt.RunTo(nX, nY)
		end
	end
