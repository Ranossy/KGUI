---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_熔断伤害不可被打断.lua
-- 更新时间:	2017/3/10 15:21:13
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	...
	{nDamageBase = 26400, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 26400, nDamageRand = 0, nCostMana = 0}, --level 3
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_熔断伤害不可被打断.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local itemwuqi = player.GetItem(0, 0)		--取到武器
	if not itemwuqi then
		return
	end
	local nCurrent = itemwuqi.nCurrentDurability		--当前耐久度
	local nMax = itemwuqi.nMaxDurability		--最大耐久度
	if scene.dwMapID == 264 or scene.dwMapID == 272 then

		if nCurrent <= nMax * 0.001 then
			nCurrent = 0
			player.SetItemCurrentDurability(0, 0, nCurrent)
		else
			if player.GetBuff(11777, 1) then
				nCurrent = math.floor(nCurrent - nMax * 0.0015)
				player.SetItemCurrentDurability(0, 0, nCurrent)
			else
				nCurrent = math.floor(nCurrent - nMax * 0.001)
				player.SetItemCurrentDurability(0, 0, nCurrent)
			end
		end
	else

		if nCurrent <= nMax * 0.002 then
			nCurrent = 0
			player.SetItemCurrentDurability(0, 0, nCurrent)
		else
			if player.GetBuff(11777, 2) then
				nCurrent = math.floor(nCurrent - nMax * 0.0025)
				player.SetItemCurrentDurability(0, 0, nCurrent)
			else
				nCurrent = math.floor(nCurrent - nMax * 0.002)
				player.SetItemCurrentDurability(0, 0, nCurrent)
			end
		end
	end
