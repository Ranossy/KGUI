---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_火焰连接.lua
-- 更新时间:	2017/5/11 16:32:13
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_火焰连接.lua", -- 属性值1
		0															--属性值2
	);
	...
	skill.AddSlowCheckDestBuff(11090, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	skill.nMaxRadius = 2000 * LENGTH_BASE;		-- 技能施放的最大距离
	--------------------弹射配置参数----------------------------------------------
	...
	skill.nAreaRadius = 2000 * LENGTH_BASE;		-- 技能作用半径
...
function Apply(dwCharacterID, dwSkillSrcID)
	--print("火成功连接")
	local caster = GetNpc(dwSkillSrcID)
	local player = GetPlayer(dwCharacterID)
	local npc = GetNpc(dwCharacterID)
	
	if player then
		if caster then
			caster.CastSkill(17664, 1, TARGET.PLAYER, player.dwID)
			return
		end
	end
	if npc then
		local scene = npc.GetScene()
		local npcTzt = scene.GetNpcByNickName("duanzaotai")
	
		local npcKc1 = scene.GetNpcByNickName("kuangche_huo")
		local npcKc2 = scene.GetNpcByNickName("kuangche_shui")
		local npcKc3 = scene.GetNpcByNickName("kuangche_mu")
		local npcKc4 = scene.GetNpcByNickName("kuangche_tu")
		local npcKc5 = scene.GetNpcByNickName("kuangche_jin")
		
		if npc.szName == "kuangshi_huo"  and npcKc1 then
			local nX = npc.nX
			local nY = npc.nY
			local cX = npcKc1.nX
			local cY = npcKc1.nY
			local dis = ((nX - cX) ^ 2 + (nY - cY) ^ 2) ^ 0.5
			if dis <= 128 then
				if npcKc1 then
					caster.CastSkill(17662, 1, TARGET.NPC, npcKc1.dwID)
					return 
				end
			else
				if npcTzt then
					caster.CastSkill(17662, 1, TARGET.NPC, npcTzt.dwID)
					return 
				end
			end
		end

		if npc.szName == "kuangshi_shui"  and npcKc2 then
			local nX = npc.nX
			local nY = npc.nY
			local cX = npcKc2.nX
			local cY = npcKc2.nY
			...
			if dis <= 128 then
				if npcKc2 then
					caster.CastSkill(17662, 1, TARGET.NPC, npcKc2.dwID)
					return 
				end
			else
				if npcTzt then
		...
		if npc.szName == "kuangshi_mu"  and npcKc3 then
			local nX = npc.nX
			local nY = npc.nY
			local cX = npcKc3.nX
			local cY = npcKc3.nY
			...
			if dis <= 128 then
				if npcKc3 then
					caster.CastSkill(17662, 1, TARGET.NPC, npcKc3.dwID)
					return 
				end
			else
				if npcTzt then
		...
		if npc.szName == "kuangshi_tu"  and npcKc4 then
			local nX = npc.nX
			local nY = npc.nY
			local cX = npcKc4.nX
			local cY = npcKc4.nY
			...
			if dis <= 128 then
				if npcKc4 then
					caster.CastSkill(17662, 1, TARGET.NPC, npcKc4.dwID)
					return 
				end
			else
				if npcTzt then
		...
		if npc.szName == "kuangshi_jin"  and npcKc5 then
			local nX = npc.nX
			local nY = npc.nY
			local cX = npcKc5.nX
			local cY = npcKc5.nY
			...
			if dis <= 128 then
				if npcKc5 then
					caster.CastSkill(17662, 1, TARGET.NPC, npcKc5.dwID)
					return 
				end
			else
				if npcTzt then
		...
		if npc.szName == "duanzaotai" or  npc.szName == "kuangche_huo" or  npc.szName == "kuangche_shui" or  npc.szName == "kuangche_mu" or  npc.szName == "kuangche_tu" or  npc.szName == "kuangche_jin" then
			
			if caster then
				--print("火1")
				caster.CastSkill(17662, 1, TARGET.NPC, npc.dwID)
				return 
			end
		else
			if caster and npcTzt then
				--print("火2")
				caster.CastSkill(17679, 1, TARGET.NPC, npcTzt.dwID)
				return 
			end
		end
	end
