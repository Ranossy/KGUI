---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_加土Buff.lua
-- 更新时间:	2017/3/10 10:19:53
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_加土Buff.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 264 or scene.dwMapID == 272 then
		npc.AddBuff(npc.dwID, npc.nLevel, 11776, 1)		--加土buff
		if npc.GetBuff(11778, 1) then
			npc.DelBuff(11778, 1)
			npc.DelBuff(11776, 1)
			npc.AddBuff(npc.dwID, npc.nLevel, 11934, 1)
		end		--水+土=0
		...
		if npc.GetBuff(11779, 1) then
			local nNum = npc.GetBuff(11779, 1).nStackNum
			npc.DelGroupBuff(11779, 1)
			for i = 1, nNum do
				npc.AddBuff(npc.dwID, npc.nLevel, 11776, 1)
			end
		end	--火+土=3土(火全消，土加满)
	...
	if scene.dwMapID == 270 then
		npc.AddBuff(npc.dwID, npc.nLevel, 11776, 2)		--加土buff
		if npc.GetBuff(11778, 2) then
			npc.DelBuff(11778, 2)
			npc.DelBuff(11776, 2)
			npc.AddBuff(npc.dwID, npc.nLevel, 11934, 2)
		end		--水+土=0
		...
		if npc.GetBuff(11779, 2) then
			local nNum = npc.GetBuff(11779, 2).nStackNum
			npc.DelGroupBuff(11779, 2)
			for i = 1, nNum do
				npc.AddBuff(npc.dwID, npc.nLevel, 11776, 2)
			end
		end		--火+土=3土(火全消，土加满)
