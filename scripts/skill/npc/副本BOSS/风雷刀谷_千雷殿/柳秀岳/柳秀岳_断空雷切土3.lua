---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_断空雷切土3.lua
-- 更新时间:	2017/4/28 14:48:11
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_断空雷切土3.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 264 or scene.dwMapID == 272 then
		...
		player.AddBuff(npc.dwID, npc.nLevel, 11949, 1)
	else
		if not player.GetBuff(11909, 2) then
			player.Stop()
			player.AddBuff(npc.dwID, npc.nLevel, 11909, 2)
		end
		player.AddBuff(npc.dwID, npc.nLevel, 11949, 2)
	end
end
