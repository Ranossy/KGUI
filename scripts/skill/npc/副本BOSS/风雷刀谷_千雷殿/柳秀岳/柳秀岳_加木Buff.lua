---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_加木Buff.lua
-- 更新时间:	2017/3/10 10:07:48
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_加木Buff.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 264 or scene.dwMapID == 272 then
		npc.AddBuff(npc.dwID, npc.nLevel, 11775, 1)		--加木buff
		if npc.GetBuff(11776, 1) then
			npc.DelBuff(11776, 1)
			npc.DelBuff(11775, 1)
			npc.AddBuff(npc.dwID, npc.nLevel, 11933, 1)
		end		--土+木=0
		if npc.GetBuff(11779, 1) then
			npc.DelBuff(11775, 1)
			npc.AddBuff(npc.dwID, npc.nLevel, 11779, 1)
			
		end		--火+木=3火
		if npc.GetBuff(11778, 1) then
			local nNum = npc.GetBuff(11778, 1).nStackNum
			npc.DelGroupBuff(11778, 1)
			for i = 1, nNum do
				npc.AddBuff(npc.dwID, npc.nLevel, 11775, 1)
			end
		end	
		--水+木=3木(水全消，木加满)
		if npc.GetBuff(11777, 1) then
			npc.DelBuff(11775, 1)
		end		--金+木=金
	end

	if scene.dwMapID == 270 then
		npc.AddBuff(npc.dwID, npc.nLevel, 11775, 2)		--加木buff
		if npc.GetBuff(11776, 2) then
			npc.DelBuff(11776, 2)
			npc.DelBuff(11775, 2)
			npc.AddBuff(npc.dwID, npc.nLevel, 11933, 2)
		end		--土+木=0
		if npc.GetBuff(11779, 2) then
			npc.DelBuff(11775, 2)
			npc.AddBuff(npc.dwID, npc.nLevel, 11779, 2)
		end		--火+木=3火
		if npc.GetBuff(11778, 2) then
			local nNum = npc.GetBuff(11778, 2).nStackNum
			npc.DelGroupBuff(11778, 2)
			for i = 1, nNum do
				npc.AddBuff(npc.dwID, npc.nLevel, 11775, 2)
			end
		end		--水+木=3木(水全消，木加满)
		if npc.GetBuff(11777, 2) then
			npc.DelBuff(11775, 2)
		end		--金+木=金
	end
end
