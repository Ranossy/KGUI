---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_熔断.lua
-- 更新时间:	2017/5/9 16:21:59
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_熔断.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 264 or scene.dwMapID == 272 then
		if npc.GetBuff(11777, 1) then
			local Nbuff = npc.GetBuff(11777, 1)
			local Num = Nbuff.nStackNum
			if Num == 1 then
				npc.CastSkill(17447, 1)
			end		--一层金
			if Num == 2 then
				npc.CastSkill(17447, 1)
				local nRan1 = Random(10000)
				if nRan1 <= 5000 then
					player.AddBuff(player.dwID, player.nLevel, 11782, 1)
				end
			end		--二层金带随机缴械
			if Num == 3 then
				npc.CastSkill(17447, 1)
				local nRan2 = Random(10000)
				if nRan2 <= 5000 then
					if not player.GetBuff(11909, 1) then
						player.Stop()
						player.AddBuff(player.dwID, player.nLevel, 11783, 1)
					end
				end
				player.AddBuff(player.dwID, player.nLevel, 11784, 1)
			end		--三层金带随即眩晕，降低防御
		else
			npc.CastSkill(17447, 1)
		end
		...
		if npc.GetBuff(11777, 2) then
			local Nbuff = npc.GetBuff(11777, 2)
			local Num = Nbuff.nStackNum
			if Num == 1 then
				npc.CastSkill(17446, 2)
			end		--一层金
			if Num == 2 then
				npc.CastSkill(17446, 2)
				local nRan1 = Random(10000)
				if nRan1 <= 5000 then
					player.AddBuff(player.dwID, player.nLevel, 11782, 2)
				end
			end		--二层金带随机缴械
			if Num == 3 then
				npc.CastSkill(17446, 2)
				...
				if nRan2 <= 5000 then
					if not player.GetBuff(11909, 2) then
						player.Stop()
						player.AddBuff(player.dwID, player.nLevel, 11783, 2)
					end
				end
				player.AddBuff(player.dwID, player.nLevel, 11784, 2)
			end		--三层金带随即眩晕，降低防御
		else
			npc.CastSkill(17446, 2)
		end
