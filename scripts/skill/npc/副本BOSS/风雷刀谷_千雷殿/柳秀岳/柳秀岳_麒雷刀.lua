---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_麒雷刀.lua
-- 更新时间:	2017/5/10 14:27:45
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_麒雷刀.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 264 or scene.dwMapID == 272 then
		if not player.GetBuff(11909, 1) then
			player.Stop()
			player.AddBuff(player.dwID, player.nLevel, 11762, 1)
		end
	else
		if not player.GetBuff(11909, 2) then
			player.Stop()
			player.AddBuff(player.dwID, player.nLevel, 11762, 2)
		end
	end

	if scene then
		scene.SetTimer(2 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_麒雷刀.lua", player.dwID, npc.dwID)
	end
end
function OnTimer(scene, nParam1, nParam2)
	...
	--if nParam3 == 1 then
	if playertarget and npc then
		if scene.dwMapID == 264 or scene.dwMapID == 272 then
			npc.CastSkillXYZ(17451, 1, playertarget.nX, playertarget.nY, playertarget.nZ)
		else
			npc.CastSkillXYZ(17451, 2, playertarget.nX, playertarget.nY, playertarget.nZ)
		end
	end
	--end
end
