---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_断空雷切木3.lua
-- 更新时间:	2017/4/18 19:32:31
-- 更新用户:	CHENGSIHAN
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 10400, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 14700, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳秀岳/柳秀岳_断空雷切木3.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local LifePercent = player.nCurrentLife / player.nMaxLife
	if not LifePercent then
		return
	end
	local AddLife = npc.nMaxLife * LifePercent * 0.01
	if npc.nCurrentLife <= npc.nMaxLife - AddLife then
		npc.nCurrentLife = npc.nCurrentLife + AddLife
	else
		npc.nCurrentLife = npc.nMaxLife
	end
	
	if player.nMaxMana then
		local ManaPercent = player.nCurrentMana / player.nMaxMana
		if not ManaPercent then
			return
		end
		local AddMana = npc.nMaxMana * ManaPercent * 0.01
		if  npc.nCurrentMana <= npc.nMaxMana - AddMana then
			npc.nCurrentMana = npc.nMaxMana + AddMana
		else
			npc.nCurrentMana = npc.nMaxMana
		end
		if player.nCurrentMana >= 0.6 then		--蓝量多于30%
			player.nCurrentMana = player.nCurrentMana - player.nMaxMana * 0.6
		else
