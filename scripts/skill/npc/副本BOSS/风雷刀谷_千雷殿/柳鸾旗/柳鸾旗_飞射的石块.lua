---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳鸾旗/柳鸾旗_飞射的石块.lua
-- 更新时间:	2017/2/20 14:45:43
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 38500, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳鸾旗/柳鸾旗_飞射的石块.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if playertarget then
		...
		if scene then
			scene.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳鸾旗/柳鸾旗_飞射的石块.lua", playertarget.dwID, dwSkillSrcID)
		end
	end
end
...
function OnTimer(scene, nParam1, nParam2)
	...
	if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH then
		...
		playertarget.PlaySfx(8682, playertarget.nX, playertarget.nY, playertarget.nZ)
		if playertarget.GetBuff(11556, 2) then
			playertarget.AddBuff(playertarget.dwID, playertarget.nLevel, 11556, 2, 5)
		end

		if not playertarget.GetBuff(11556, 2) then
			playertarget.Stop()
			playertarget.AddBuff(playertarget.dwID, playertarget.nLevel, 11556, 2, 5)
		end
	end
