---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳鸾旗/柳鸾旗_幻境分身爆炸.lua
-- 更新时间:	2017/2/15 11:52:32
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳鸾旗/柳鸾旗_幻境分身爆炸.lua", --属性值1
		0															--属性值2
	);
	...
	if playertarget then
		if npc then
			...
			if scene.dwMapID == 264 or scene.dwMapID == 272 then
				playertarget.AddBuff(npc.dwID, npc.nLevel, 11699, 1, 20)
			end

			if scene.dwMapID == 270 then
				playertarget.AddBuff(npc.dwID, npc.nLevel, 11699, 2, 20)
			end
		end
	end
end
