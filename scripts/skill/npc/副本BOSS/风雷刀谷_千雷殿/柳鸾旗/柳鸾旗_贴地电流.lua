---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/柳鸾旗/柳鸾旗_贴地电流.lua
-- 更新时间:	2016/12/6 15:22:02
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 7800, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 15750, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/柳鸾旗/柳鸾旗_贴地电流.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if playertarget then
		...
		if scene.dwMapID == 264 or scene.dwMapID == 272 then
			if playertarget.GetBuff(11556, 1) then
				playertarget.AddBuff(playertarget.dwID, playertarget.nLevel, 11556, 1, 3)
			end
		
			if not playertarget.GetBuff(11556, 1) then
				playertarget.Stop()
				playertarget.AddBuff(playertarget.dwID, playertarget.nLevel, 11556, 1, 3)
			end
		end
		...
		if scene.dwMapID == 270 then
			if playertarget.GetBuff(11556, 2) then
				playertarget.AddBuff(playertarget.dwID, playertarget.nLevel, 11556, 2, 10)
			end
