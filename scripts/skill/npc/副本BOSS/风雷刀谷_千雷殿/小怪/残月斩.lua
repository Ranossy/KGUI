---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_千雷殿/小怪/残月斩.lua
-- 更新时间:	2017/5/4 12:04:07
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 9100, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 12250, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_千雷殿/小怪/残月斩.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 11926, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		if scene.dwMapID == 264 or scene.dwMapID == 272 then
			player.AddBuff(npc.dwID, npc.nLevel, 11926, 1)
			if player.GetBuff(11926, 1) then
				local nNum = player.GetBuff(11926, 1).nStackNum
				local npc = GetNpc(dwSkillSrcID)
				if npc then
					npc.nCurrentLife = npc.nCurrentLife + npc.nMaxLife * 0.005 * nNum
				end
			end
		else
			player.AddBuff(npc.dwID, npc.nLevel, 11926, 2)
			if player.GetBuff(11926, 2) then
				local nNum = player.GetBuff(11926, 2).nStackNum
				local npc = GetNpc(dwSkillSrcID)
				if npc then
					npc.nCurrentLife = npc.nCurrentLife + npc.nMaxLife * 0.01 * nNum
				end
			end
		end
