---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/春明门/春明门_崔乾佑_战车碾压.lua 拷贝
-- 更新时间:	2014/6/3 20:37:52
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/春明门/春明门_崔乾佑_战车判定.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	--skill.AddSlowCheckDestBuff(7856, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7856, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npczj = GetNpc(dwSkillSrcID)
	if npc and npczj then
		if npc.dwTemplateID == 36524 then
			npc.Die()
			npczj.SetCustomUnsigned1(1, 3)
		elseif npc.dwTemplateID == 36522 then
			npc.FireAIEvent(2004, dwSkillSrcID, 2)
		end
	end
	--[[local player = GetPlayer(dwCharacterID)
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH and not player.GetBuff(7910, 1) then
	--	print("jiabuff")
		player.AddBuff(dwSkillSrcID, 99, 7897, 1)
		player.AddBuff(dwSkillSrcID, 99, 7902, 1, 2)
		player.AddBuff(dwSkillSrcID, 99, 7910, 1, 10)
	end--]]
end
