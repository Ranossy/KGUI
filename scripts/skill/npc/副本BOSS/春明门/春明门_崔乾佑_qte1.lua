---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/春明门/春明门_崔乾佑_qte1.lua
-- 更新时间:	2014/6/11 17:47:33
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/春明门/春明门_崔乾佑_qte1.lua",
		0
	);
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		...
		if buff then
			...
			if buff.nCustomValue == 1 then
				--player.Stop()
				if GetNpc(buff.dwSkillSrcID) then
					player.TurnToCharacter(buff.dwSkillSrcID)
				end
				player.DoAction(0,13957)
				player.DelBuff(7897, 1)
				player.DelBuff(7902, 1)
				player.DelBuff(7910, 1)
				player.SetTimer(0.8 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/春明门/春明门_崔乾佑_qte1.lua", 1, 1)
