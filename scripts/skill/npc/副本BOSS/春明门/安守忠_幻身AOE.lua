---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/春明门/安守忠_幻身AOE.lua
-- 更新时间:	2014/6/5 17:35:26
-- 更新用户:	liguanghao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/春明门/安守忠_幻身AOE.lua",
		0
	)
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 7914, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		if not player.GetBuff(7856, 1) then
			player.AddBuff(0, 99, 7856, 1)
		end
		local scene = player.GetScene()
		if scene then
			local npcBoss = scene.GetNpcByNickName("BOSS2_anshouzhong")
			if npcBoss then
				npcBoss.AddBuff(0, 99, 7905, 1)
			end
		end
	end
	local npc = GetNpc(dwCharacterID)
	if npc then
		local npcHuanshen = GetNpc(dwSkillSrcID)
		if npcHuanshen then
			npcHuanshen.SetDisappearFrames(1, 0)
		end
	end
end
