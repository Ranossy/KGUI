    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	----------------- 魔法属性 -------------------------------------------------
		skill.AddAttribute(
			...
			"skill/npc/副本BOSS/25人谢云流/居合·贯诚.lua", 
			0
			);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	local xieyunliu = scene.GetNpcByNickName("xieyunliu_qichang")
	if not xieyunliu then
		return
	end
	if player.nZ - 	xieyunliu.nZ <= nDis then
		xieyunliu.CastSkill(1817, 1, TARGET.PLAYER, player.dwID)
		scene.SetCustomBoolean(12, true)
	end
end
