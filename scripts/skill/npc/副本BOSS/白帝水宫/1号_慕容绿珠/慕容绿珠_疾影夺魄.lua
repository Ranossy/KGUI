---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/白帝水宫/1号_慕容绿珠/慕容绿珠_疾影夺魄.lua
-- 更新时间:	2016/7/6 16:03:33
-- 更新用户:	yangfan14
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 60000, nDamageRand = 3000, nCostMana = 0}, --level 1
	{nDamageBase = 80000, nDamageRand = 4000, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/白帝水宫/1号_慕容绿珠/慕容绿珠_疾影夺魄.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if  player then
		...
		player.PlaySfx(1306, player.nX, player.nY, player.nZ)
	end

	local scene = player.GetScene()
	if scene.dwMapID == 242 then
		if  player then
			player.AddBuff(0, 99, 10797, 1)
		end
	end
	if scene.dwMapID == 244 then
		if  player then
			player.AddBuff(0, 99, 10797, 2)
		end
	end
end
