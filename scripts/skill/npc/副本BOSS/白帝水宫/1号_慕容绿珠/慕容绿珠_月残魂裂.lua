---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/白帝水宫/1号_慕容绿珠/慕容绿珠_月残魂裂.lua
-- 更新时间:	2016/7/6 20:46:34
-- 更新用户:	yangfan14
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 2820, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 3510, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/白帝水宫/1号_慕容绿珠/慕容绿珠_月残魂裂.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 242 then
		if  player then
			player.PlaySfx(1310, player.nX, player.nY, player.nZ + 300)
			player.AddBuff(0, 99, 10789, 1)
		end
	end
	if scene.dwMapID == 244 then
		if  player then
			player.PlaySfx(1310, player.nX, player.nY, player.nZ + 300)
			player.AddBuff(0, 99, 10789, 2)
		end
	end
end
