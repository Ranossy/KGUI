---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/白帝水宫/小怪/小怪_烈风残.lua
-- 更新时间:	2016/5/16 10:46:57
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 4600, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/白帝水宫/小怪/小怪_烈风残.lua", --属性值1
		0															--属性值2
	);
	...
	--skill.BindBuff(1, 8834, 1, 5);		-- 设置1号位Buff
	--elseif dwSkillLevel == 2 then
	--skill.BindBuff(1, 8834, 2, 5)
	--end
...
function Apply(dwCharacterID)
	...
	player.Stop()
	scene.SetTimer(8, "scripts/skill/npc/副本BOSS/白帝水宫/小怪/小怪_烈风残.lua", player.dwID, 1)
end
...
function OnTimer(scene, nParam1, nParam2)
	...
	player.BeHitted(player.dwID, 5356, 1, 30)
	if scene.dwMapID == 242 then
		player.AddBuff(0, 99, 10790, 1)
	end

	if scene.dwMapID == 244 then
		player.AddBuff(0, 99, 10790, 2)
	end
end
