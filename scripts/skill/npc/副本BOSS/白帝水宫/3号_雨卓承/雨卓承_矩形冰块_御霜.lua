---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/白帝水宫/3号_雨卓承/雨卓承_矩形冰块_御霜.lua
-- 更新时间:	2016/6/22 20:59:06
-- 更新用户:	yangfan14
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/白帝水宫/3号_雨卓承/雨卓承_矩形冰块_御霜.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 242 then
		if  player then
			player.AddBuff(0, 99, 10777, 1)
		end
	end
	if scene.dwMapID == 244 then
		if  player then
			player.AddBuff(0, 99, 10777, 2)
		end
	end
end
