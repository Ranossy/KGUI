---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/白帝水宫/3号_雨卓承/雨卓承_四象混沌·雪暴.lua
-- 更新时间:	2016/6/30 18:58:14
-- 更新用户:	yangfan14
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 21950, nDamageRand = 548, nCostMana = 0}, --level 1
	{nDamageBase = 51600, nDamageRand = 1290, nCostMana = 0}, --level 2
...
----------------- 穿透属性 -------------------------------------------------
 ...
 - 1024,
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/白帝水宫/3号_雨卓承/雨卓承_四象混沌·雪暴.lua", --属性值1
		0															--属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(10777, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 10853, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 242 then
		if  player then
			player.AddBuff(0, 99, 10853, 1)
		end
	end
	if scene.dwMapID == 244 then
		if  player then
			player.AddBuff(0, 99, 10853, 2)
		end
	end

	local boss = scene.GetNpcByNickName("yuzhuocheng")
	if boss then
		boss.SetCustomInteger1(50, 1)
	end
end
