---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/白帝水宫/2号_宫天蝉/宫天蝉_九曲飞流.lua
-- 更新时间:	2016/7/6 20:03:29
-- 更新用户:	yangfan14
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 4600, nDamageRand = 115, nCostMana = 0}, --level 1
	{nDamageBase = 8700, nDamageRand = 217, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/白帝水宫/2号_宫天蝉/宫天蝉_九曲飞流.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 242 then
		if  player then
			player.PlaySfx(1318, player.nX, player.nY, player.nZ)
			player.AddBuff(0, 99, 10772, 1)
			if player.GetBuff(10928, 1) then
				player.DelBuff(10928, 1)
			end
		end
	end
