---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/副本通用技能/小怪10尺范围友方加仇恨.lua
-- 更新时间:	2016/9/14 14:10:39
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/副本通用技能/小怪10尺范围友方加仇恨.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npc2 = GetNpc(dwSkillSrcID)
	if not npc2 then
		return
	end
	...
	for i = 1, #playerlist do
		...
		if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH then
			npc1.ModifyThreat(playertarget.dwID, 0)
			npc2.ModifyThreat(playertarget.dwID, 0)
		end
	end
end
