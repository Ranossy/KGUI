---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_剑俑突刺.lua
-- 更新时间:	2014/1/21 16:12:34
-- 更新用户:	ks13376-PC
-- 脚本说明:	
----------------------------------------------------------------------<
...
tSkillData = 
{
	{nDamageBase = 14220, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 39185, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_剑俑突刺.lua",								--属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 183 then
		if not player.GetBuff(7745, 1) then
			player.AddBuff(0, 99, 7745, 1, 20)
		end
	end
end
