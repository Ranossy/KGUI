...
tSkillData = 
{
	{nDamageBase = 35600, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 46400, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 7576, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	local npc = scene.GetNpcByNickName("huoyan_1")
	if not npc then
		return
	end
	if scene.dwMapID == 182 then
		npc.CastSkillXYZ(8918, 1, player.nX, player.nY, player.nZ)
	else
		local x1 = player.nX + 640
		local x2 = player.nX - 640
		local x3 = player.nX +512
		local x4 = player.nX - 367
		local y1 = player.nY - 256
		local y2 = player.nY +640
		local y3 = player.nY - 367
		local y4 = player.nY + 367
		local z1 = 0
		local tLocation = { {x1 ,y1, z1 }, {x2, y2, z1}, {x3, y3, z1}, {x2, y3, z1}, {x1, y3, z1}, {x4, y4, z1},{x1, x2, z1}, {x4, x3, z1}}
		local tSelectedLocation = RandomSelectLocation(tLocation, 1)
		for k, v in ipairs(tSelectedLocation) do
			local x = v[1]
			local y = v[2]
			local z = v[3]
			player.RunTo(x, y)
		end
		npc.CastSkillXYZ(8918, 2, player.nX, player.nY, player.nZ)
	end
end
...
function RandomSelectLocation(tLocation, nLocationCount)
	local tLocationCopy = {}
	
	for i = 1, #tLocation do
		tLocationCopy[#tLocationCopy + 1] = tLocation[i]
	end
	
	if #tLocationCopy <= nLocationCount then
		return tLocationCopy
	end
	
	local tSelectedLocation = {}	
	
	for i = 1, nLocationCount do
		local randomIndex = Random(#tLocationCopy)
		tSelectedLocation[#tSelectedLocation + 1] = tLocationCopy[randomIndex]
		table.remove(tLocationCopy, randomIndex)
	end
	
	return tSelectedLocation
end
