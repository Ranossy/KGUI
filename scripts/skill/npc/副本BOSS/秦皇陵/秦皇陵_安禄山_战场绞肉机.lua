---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_战场绞肉机.lua
-- 更新时间:	2014/2/10 14:57:52
-- 更新用户:	ks13376-PC
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_战场绞肉机.lua",								--属性值1
		0															--属性值2
	);
    
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(9005, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID)
	...
	--if scene.dwMapID == 182 then
		--if player.GetBuff(7435, 1) then
			--if player.GetBuff(7435, 1).nStackNum >= 5 then
				--player.PlaySfx(553, player.nX, player.nY, player.nZ + 500)
				--player.Die()
			--end
		--end
		--player.AddBuff(0, 99, 7435, 1)
	--end

	--[[if scene.dwMapID == 183 then
		if player.GetBuff(7435, 2) then
			if player.GetBuff(7435, 2).nStackNum >= 5 then
				player.PlaySfx(553, player.nX, player.nY, player.nZ + 500)
				player.Die()
			end
		end
		player.AddBuff(0, 99, 7435, 2)
	end--]]
	if player and not player.GetBuff(9005, 1) then
		player.AddBuff(0, 99, 9005, 1, 3)
	end
end
