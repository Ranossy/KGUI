---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_亡者气息.lua
-- 更新时间:	2014/1/23 11:38:16
-- 更新用户:	ks13376-PC
-- 脚本说明:	
----------------------------------------------------------------------<
...
tSkillData = 
{
	{nDamageBase = 16500, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 17400, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_亡者气息.lua",								--属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	if not player.GetBuff(7396, 1) then
		if jieduanjiemiankongzhi then
			if jieduanjiemiankongzhi.GetCustomInteger1(2) ~= 4 then
				--player.Stop()
			end
		end
	end
	
	if not player.GetBuff(7464, 1) then--没有拉勾冲刺buff
		if jieduanjiemiankongzhi then
			if jieduanjiemiankongzhi.GetCustomInteger1(2) ~= 4 then
				--player.AddBuff(0, 99, 7396, 1, 2)
			end
		end
	end
end
