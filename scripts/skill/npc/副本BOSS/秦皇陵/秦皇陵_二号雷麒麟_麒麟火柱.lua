---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_二号雷麒麟_麒麟火柱.lua
-- 更新时间:	2014/2/11 17:14:56
-- 更新用户:	zhouyixiao1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData = 
{
	{nDamageBase = 26100, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 34500, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_二号雷麒麟_麒麟火柱.lua",
		0
	);
	--skill.AddAttribute(
	--ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,
	...
	skill.AddSlowCheckDestBuff(7575, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 7454, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nTiaoshu = 8
	if player then
		if not player.GetBuff(7575, 1) and not player.GetBuff(7575, 2) then
			player.AddBuff(0, 99, 7346, 1, 3) --击飞换动作
			player.AddBuff(0, 99, 7467, 1, nTiaoshu)  --反制机会BUFF
			player.SetDynamicSkillGroup(119)  --为反制做准备屏蔽技能栏
			--player.PlaySfx(653, player.nX, player.nY, player.nZ)
		end

		local scene = player.GetScene()
		local npc = GetNpc(dwSkillSrcID)
		if npc then
			npc.SetCustomInteger1(5, 1)   --火柱自身的标记位置
		end		
	end
end
