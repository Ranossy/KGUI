---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_二号雷麒麟_火盾AOE加buff.lua
-- 更新时间:	2014/2/11 15:50:07
-- 更新用户:	mengxiangfei
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_二号雷麒麟_火盾AOE加buff.lua", --属性值1
		0															--属性值2
	);
...
function InTriangle1(nplayer,tplayer,qilin)
	--根据nplayer求三角形另外2个点
	local nDirect = {math.floor((nplayer.nFaceDirection - 64+256) % 256),math.floor((nplayer.nFaceDirection + 64/3+256) % 256)}
	local Distance1 = 2
	
	local npc1_x = nplayer.nX + math.floor(math.cos(nDirect[1] / 128 * math.pi) * Distance1 * 64)
	local npc1_y = nplayer.nY + math.floor(math.sin(nDirect[1] / 128 * math.pi) * Distance1 * 64)
	
	local npc2_x = nplayer.nX + math.floor(math.cos(nDirect[2] / 128 * math.pi) * Distance1 * 64)
	local npc2_y = nplayer.nY + math.floor(math.sin(nDirect[2] / 128 * math.pi) * Distance1 * 64)
	
	local S1 = Sarea(npc1_x, npc1_y, npc2_x, npc2_y, tplayer.nX, tplayer.nY)
	local S2 = Sarea(npc1_x, npc1_y, qilin.nX, qilin.nY, tplayer.nX, tplayer.nY)
	local S3 = Sarea(npc2_x, npc2_y, qilin.nX, qilin.nY, tplayer.nX,tplayer.nY)
	local S = Sarea(npc1_x, npc1_y, npc2_x, npc2_y, qilin.nX, qilin.nY)
	
	if math.abs(S-S1-S2-S3) <= 100 then
		return true
	else
		return false
	end
...
function Sarea(X1,Y1,X2,Y2,X3,Y3)
	local S = math.abs((X1 * Y2 + X2 * Y3 + X3 * Y1-X1 * Y3-X2 * Y1-X3 * Y2) / 2)
	return S	
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local qilin = scene.GetNpcByNickName("BOSS_QILIN")
	if not qilin then
		return
	end
	
	if not tplayer.GetBuff(7502, 1) and not tplayer.GetBuff(7502, 2)then
		return
	end
	
	if InTriangle1(nplayer, tplayer,qilin) then
		return
	end
	--这里没写完，要判定一系列角度来觉得给哪些玩家加buff
	tplayer.AddBuff(0,99,7456,1)
end
