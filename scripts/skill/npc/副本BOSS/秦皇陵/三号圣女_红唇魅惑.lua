...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame)
	...
	nplayer.SetDynamicSkillGroup(0)
	
	local nBOSS = scene.GetNpcByNickName("BOSS_SHENGNV")
	
	if nLeftFrame == 0 or (nLeftFrame ~= 0 and nplayer.nMoveState == MOVE_STATE.ON_DEATH) then
		if nBOSS and nBOSS.nMoveState ~= MOVE_STATE.ON_DEATH then
			nplayer.Die()
			if nBOSS.nCurrentLife < nBOSS.nMaxLife * 0.85 then
				nBOSS.nCurrentLife = nBOSS.nCurrentLife + nBOSS.nMaxLife * 0.15
			else
				nBOSS.nCurrentLife = nBOSS.nMaxLife
			end
		end
	end
end
