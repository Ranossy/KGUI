...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame)
	...
	if nLeftFrame ~= 0 then
		for i = 1, 100 do
			local huolaohuayu = scene.GetNpcByNickName("huolaohuayu"..i)
			if not huolaohuayu then
				if scene.dwMapID == 182 then
					scene.CreateNpc(27698, player.nX, player.nY, player.nZ, 0, -1, "huolaohuayu"..i)
				else
					scene.CreateNpc(36163, player.nX, player.nY, player.nZ, 0, -1, "huolaohuayu"..i)
				end
				break
			end
		end
	else
		player.PlaySfx(689, player.nX, player.nY, player.nZ)
		if scene.dwMapID == 182 then
			player.CastSkill(8811, 1)
		else
			player.CastSkill(8811, 2)
		end
	end
end
