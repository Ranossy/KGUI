---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_一号后小怪_毒气.lua
-- 更新时间:	2014/2/20 14:23:28
-- 更新用户:	ZHOUMING
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_一号后小怪_毒气.lua", -- 属性值1
		0														-- 属性值2
	);
	...
	skill.nHeight = 108 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
	skill.nRectWidth = 50 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
...
function Apply(dwCharacterID, dwSrc)
	...
	if player then
		...
		if tBuffLevel[scene.dwMapID] and not player.GetBuff(7528,tBuffLevel[scene.dwMapID]) then
			player.AddBuff(0,99,7528,tBuffLevel[scene.dwMapID],7 * 1200 * 24)
		end
	end
	if npc then
		local scene = npc.GetScene()
		if tBuffLevel[scene.dwMapID] and not npc.GetBuff(7528, tBuffLevel[scene.dwMapID]) then
			npc.AddBuff(0,99,7528,tBuffLevel[scene.dwMapID], 7 * 1200 * 24)
		end
	end
end
