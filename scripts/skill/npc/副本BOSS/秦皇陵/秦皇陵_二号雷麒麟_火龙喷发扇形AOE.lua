---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_二号雷麒麟_火龙喷发扇形AOE.lua
-- 更新时间:	2014/4/8 18:01:46
-- 更新用户:	wangwei-pc2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_二号雷麒麟_火龙喷发扇形AOE.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(7456, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	--skill.AddSlowCheckDestBuff(7456, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 7502, dwSkillLevel);		-- 设置1号位Buff
	...
	skill.nAngleRange = 66;					-- 攻击范围的扇形角度范围
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = Cplayer.GetBuff(7456, 1)
	if not buff then
		return
	end

	if Cplayer.GetScene().dwMapID == 182 then
		Cplayer.CustomDamage(dwSkillSrcID, 8840, 1, 1, 3700*0.14)
	else
		Cplayer.CustomDamage(dwSkillSrcID, 8840, 2, 1, 4500*0.14)
	end
end
