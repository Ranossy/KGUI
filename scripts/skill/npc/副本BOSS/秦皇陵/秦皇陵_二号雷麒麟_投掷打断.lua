...
function GetSkillLevelData(skill)
	...
	skill.AddSlowCheckDestBuff(7462, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 7463, dwSkillLevel);		-- 设置1号位Buff
	...
	skill.SetNormalCoolDown(1, 686);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local playerbuff = player.GetBuff(7608, 1)
	...
	local nZ = 0
	for i = 1, 4 do
		local doodad = scene.GetDoodadByNickName("BOSS2_qilinnu" .. i)
		if doodad then
			local doodad_dis = GetDistanceSq(player.nX, player.nY, player.nZ, doodad.nX, doodad.nY, doodad.nZ) 
			if doodad_dis >= dis then
				dis = doodad_dis
				nX = doodad.nX
				nY = doodad.nY
				nZ = doodad.nZ
	...
	scene.CreateNpc(30632, nX, nY, nZ, player.nFaceDirection, 20 * 16, "nujian" .. player.dwID)
	local npc = scene.GetNpcByNickName("nujian" .. player.dwID)
	local boss = scene.GetNpcByNickName("BOSS_QILIN")
	print(1111)
	if npc and boss then
		print(22222)
		print(npc.CastSkill(9064, 1, TARGET.NPC, boss.dwID))
		npc.RunTo(boss.nX, boss.nY)
	end
