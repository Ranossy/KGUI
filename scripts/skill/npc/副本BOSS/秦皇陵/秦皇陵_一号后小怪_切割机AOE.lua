---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_一号后小怪_切割机AOE.lua
-- 更新时间:	2014/4/25 9:53:25
-- 更新用户:	ZHOUMING
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData = 
{
	{nDamageBase = 9980, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 11980, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_一号后小怪_切割机AOE.lua", -- 属性值1
		0														-- 属性值2
	);
	skill.AddAttribute( 
...
function Apply(dwCharacterID, dwSrcID)
	...
	if npcTarget and npcSrc and (npcTarget.dwTemplateID == 36152 or npcTarget.dwTemplateID == 27687 ) then
		npcSrc.ModifyThreat(npcTarget.dwID, 1)
		npcTarget.Die()
	end
	--print(dwSrcID)
end
