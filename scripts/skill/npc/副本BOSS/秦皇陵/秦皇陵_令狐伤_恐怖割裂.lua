...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_令狐伤_恐怖割裂.lua",
		0
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	  skill.BindBuff(1, 7556, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	local plyayer = GetPlayer(dwCharacterID)
	if not player then
		return
	end
	player.AddBuff(0,99,7557,1)
	player.AddBuff(0,99,5197,1,12)
end
