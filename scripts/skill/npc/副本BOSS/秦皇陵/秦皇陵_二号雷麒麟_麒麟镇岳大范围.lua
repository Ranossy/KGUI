---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_二号雷麒麟_麒麟镇岳大范围.lua
-- 更新时间:	2014/3/20 10:06:55
-- 更新用户:	wangwei-pc2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 8700, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_二号雷麒麟_麒麟镇岳大范围.lua",
		0
	);
	--]]
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7460, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		if player.GetBuff(7575, 1) or  player.GetBuff(7575, 2) then
			return
		end
		player.AddBuff(0, 99, 7460, 1)
	else
		local petnpc = GetNpc(dwCharacterID)
		if petnpc then
			petnpc.AddBuff(0, 99, 7460, 1)
		end
	end
	--]]
end
