---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_铁勾.lua
-- 更新时间:	2014/2/9 14:49:35
-- 更新用户:	ks13376-PC
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_铁勾.lua",								--属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 182 then
		if not npc.GetBuff(7384, 1) then
			npc.AddBuff(0, 99, 7384, 1, 3)--加摇摆判定buff
			npc.DoAction(0, 10054)
		end
		
		if npc.nCurrentLife <= 60 then
			npc.Die()
		end
		npc.nCurrentLife = npc.nCurrentLife - 60
		
		npc.AddBuff(0, 99, 7416, 1, 2)--顶灯承重buff
		player.DelBuff(7396, 1)--亡者气息锁足
		player.AddBuff(0, 99, 7464, 1, 5)--冲刺维持buff
		local chongcibuff = player.GetBuff(7464, 1)
		if chongcibuff then
			chongcibuff.nCustomValue = npc.dwID
		end
	end
	...
	if scene.dwMapID == 183 then
		...
		if npc.nCurrentLife <= 100 then
			npc.Die()
		end
		npc.nCurrentLife = npc.nCurrentLife - 100
		
		npc.AddBuff(0, 99, 7416, 2, 2)--顶灯承重buff
		player.DelBuff(7396, 1)--亡者气息锁足
		player.AddBuff(0, 99, 7464, 1, 5)--冲刺维持buff
		local chongcibuff = player.GetBuff(7464, 1)
		if chongcibuff then
