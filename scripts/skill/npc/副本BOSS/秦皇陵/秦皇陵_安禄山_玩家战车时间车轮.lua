---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_玩家战车时间车轮.lua
-- 更新时间:	2014/3/6 15:38:03
-- 更新用户:	ks13376-PC
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData = 
{
	{nDamageBase = 113000, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 180000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_玩家战车时间车轮.lua",								--属性值1
		0															--属性值2
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7383, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 182 then
		if not player.GetBuff(7677, 1) then
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(6, 3158), 2)
		end

		if player.GetBuff(7677, 1) then
			if player.GetBuff(7677, 1).nStackNum < 20 then
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(6, 3158), 2)
			end
		end
	end
	...
	if scene.dwMapID == 183 then
		if not player.GetBuff(7677, 2) then
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(6, 3158), 2)
		end

		if player.GetBuff(7677, 2) then
			if player.GetBuff(7677, 2).nStackNum < 20 then
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(6, 3158), 2)
			end
		end
	end
	...
	if scene.dwMapID == 182 then
		if player.GetBuff(7677, 1) then
			if player.GetBuff(7677, 1).nStackNum >= 20 then
				--scene.CreateNpc(30614, 47982, 48291, 1065472, 0, 160, "shijianchelunbiaoxian")
				------小怪暂停
				local jieduanjiemiankongzhi = scene.GetNpcByNickName("jieduanjiemiankongzhi")
				local shuaiguaikongzhi = scene.GetNpcByNickName("anlushanshuaguaikongzhi")
				if jieduanjiemiankongzhi then
					if jieduanjiemiankongzhi.GetCustomInteger1(2) == 1 then
						if shuaiguaikongzhi then
							shuaiguaikongzhi.AddBuff(0, 99, 7391, 1, 10)---刷怪控制暂停
							local num = shuaiguaikongzhi.GetCustomInteger4(4)
							for i = 1, num do
								local xiaoguai = scene.GetNpcByNickName("anlushanbingyong" .. i)
								if xiaoguai then
									xiaoguai.AddBuff(0, 99, 7391, 1, 10)
								end
							end
						end
					end
				...
				local nengliangqiukongzhi = scene.GetNpcByNickName("nengliangqiukongzhi")
				if jieduanjiemiankongzhi then
					if jieduanjiemiankongzhi.GetCustomInteger1(2) == 1 then
						if nengliangqiukongzhi then
							nengliangqiukongzhi.AddBuff(0, 99, 7391, 1, 10)--能量球控制暂停
							local num = nengliangqiukongzhi.GetCustomInteger4(1)
							for i = 1, num do
								local nengliangqiu = scene.GetNpcByNickName("yousenengliangti" .. i)
								if nengliangqiu then
									nengliangqiu.AddBuff(0, 99, 7391, 1, 10)
								end
							end
						end
					end
				...
				local jieduanjiemiankongzhi = scene.GetNpcByNickName("jieduanjiemiankongzhi")
				if jieduanjiemiankongzhi then
					jieduanjiemiankongzhi.AddBuff(0, 99, 7391, 1, 10)
				end
				...
				for i = 1, #playerlist do
					...
					if playertarget then
						playertarget.DelGroupBuff(7382, 1)
						playertarget.DelGroupBuff(7383, 1)
						playertarget.DelGroupBuff(7677, 1)
					end
				end

				player.PlaySfx(692, 47982, 48291, 1070472)
			end
		end
	end
		...
		if player.GetBuff(7677, 2) then
			if player.GetBuff(7677, 2).nStackNum >= 20 then
				...
				for i = 1, #playerlist do
					local playertarget = GetPlayer(playerlist[i])
					if playertarget then
						playertarget.DelGroupBuff(7382, 2)
						playertarget.DelGroupBuff(7383, 2)
