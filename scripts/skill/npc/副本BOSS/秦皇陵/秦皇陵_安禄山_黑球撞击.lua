---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_黑球撞击.lua
-- 更新时间:	2014/1/21 11:01:26
-- 更新用户:	ks13376-PC
-- 脚本说明:	
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
tSkillData = 
{
	...
	{nDamageBase = 17200, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_黑球撞击.lua",								--属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7382, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 182 then
		local baiyunBuff = player.GetBuff(7383, 1)
		if not baiyunBuff then
			player.AddBuff(0, 99, 7382, 1)
		else
			local num = baiyunBuff.nStackNum
			player.DelGroupBuff(7383, 1)
			for i = 1, num do
				player.AddBuff(0, 99, 7382, 1)
			end
		end

		if player.GetBuff(7590, 1) then
			player.AddBuff(0, 99, 7677, 1)
		end
	end
	
	if scene.dwMapID == 183 then
		local baiyunBuff = player.GetBuff(7383, 2)
		if not baiyunBuff then
			player.AddBuff(0, 99, 7382, 2)
		else
			local num = baiyunBuff.nStackNum
			player.DelGroupBuff(7383, 2)
			for i = 1, num do
				player.AddBuff(0, 99, 7382, 2)
			end
		end
