---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_抛石.lua
-- 更新时间:	2014/2/19 9:58:28
-- 更新用户:	ks13376-PC
-- 脚本说明:	
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 160000, nDamageRand = 0, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
    ...
    skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_抛石特效.lua",
		0
	);
	...
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,
		ATTRIBUTE_TYPE.EXECUTE_SCRIPT,
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_安禄山_抛石.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 182 then
		if npctarget.GetBuff(7392, 1) then
			npctarget.DelBuff(7392, 1)
		end
	end

	if scene.dwMapID == 183 then
		if npctarget.GetBuff(7392, 2) then
			npctarget.DelBuff(7392, 2)
		end
	end
end
