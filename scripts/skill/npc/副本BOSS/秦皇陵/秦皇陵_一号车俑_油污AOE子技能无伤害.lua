---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_一号车俑_油污AOE子技能无伤害.lua
-- 更新时间:	2014/2/10 15:16:54
-- 更新用户:	mengxiangfei
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_一号车俑_油污AOE子技能无伤害.lua", --属性值1
		0															--属性值2
	);
    
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(7437, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7436, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local tnpc = GetNpc(dwSkillSrcID)
	if not tnpc then
		return
	end
	
	if tplayer.GetBuff(7436, 1) and not tnpc.GetBuff(7437,1) then
		tnpc.AddBuff(0,99,7437,1)
	end
	
	if tplayer.GetBuff(7436,1) or tplayer.GetBuff(7439,1) then
		return
	else
		tplayer.AddBuff(0,99,7436,1)
	end
