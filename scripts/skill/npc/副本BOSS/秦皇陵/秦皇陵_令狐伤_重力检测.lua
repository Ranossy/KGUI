...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_令狐伤_重力检测.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		npc.AddBuff(0, 99, 7604, 1)
		npc.FireAIEvent(2001,0,0)
		if player.GetBuff(7538, 1) then
			npc.AddBuff(0,99,7604,1)
		end
	end
	local tnpc = GetNpc(dwCharacterID)
	if tnpc then
		if tnpc.dwTemplateID == 27715 or tnpc.dwTemplateID == 36167 then
			npc.AddBuff(0, 99, 7604, 1)
			npc.AddBuff(0, 99, 7604, 1)
		end
		if tnpc.dwTemplateID == 27728 or tnpc.dwTemplateID == 36180 or tnpc.dwTemplateID == 36411 then
			npc.AddBuff(0, 99, 7604, 1)
			npc.AddBuff(0, 99, 7604, 1)
			npc.AddBuff(0, 99, 7604, 1)
		end
		...
		if npcLhs then
			local nXulong = npcLhs.GetCustomInteger1(7)
			if nXulong > 0 then
				for i = 1, nXulong do
					npc.AddBuff(0, 99, 7604, 1)
				end
