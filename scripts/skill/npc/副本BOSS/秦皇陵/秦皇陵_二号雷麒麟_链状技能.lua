---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_二号雷麒麟_链状技能.lua
-- 更新时间:	2014/2/20 15:24:39
-- 更新用户:	mengxiangfei
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_二号雷麒麟_链状技能.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local n = string.sub(npc.szName, 7)
	local i = tonumber(n)
	local targetType, targetID = npc.GetTarget()
	local targetT = nil

	if targetType == TARGET.PLAYER then
		targetT = GetPlayer(targetID)
	elseif targetType == TARGET.NPC then
		targetT = GetNpc(targetID)
	else
		return
	end
	
	targetT.AddBuff(0, 99, 7531, 1)
	if targetT.GetBuff(7531, 1) then
		local buff = targetT.GetBuff(7531, 1)
		buff.nCustomValue = i
	end
end
