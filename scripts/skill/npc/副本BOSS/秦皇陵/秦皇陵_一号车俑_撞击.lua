---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/秦皇陵/秦皇陵_一号车俑_撞击.lua
-- 更新时间:	2014/3/20 14:37:59
-- 更新用户:	liguanghao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 21700, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 29000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/秦皇陵/秦皇陵_一号车俑_撞击.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(7814, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 7814, 1);		-- 设置1号位Buff
	...
	skill.nRectWidth = 14 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		if scene and scene.dwMapID == 182 then
			if player.GetBuff(7581, 1) then
			...
			player.DelGroupBuff(7721, 1)
			player.DelGroupBuff(7581, 1)
			player.DelGroupBuff(7425, 1)
			player.SetModelID(0)
			player.SetDynamicSkillGroup(0)
		elseif scene and scene.dwMapID == 183 then
			if player.GetBuff(7581, 2) then
				local npc = GetNpc(dwSkillSrcID)
				if npc then
					npc.Die()
				end
			...
			player.DelGroupBuff(7721, 2)
			player.DelGroupBuff(7581, 2)
