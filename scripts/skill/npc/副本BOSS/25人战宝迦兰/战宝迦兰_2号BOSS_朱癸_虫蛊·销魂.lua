...
-- 武功套路  :  无
-- 技能名称  :  虫蛊·销魂
-- 技能效果  : 	随机一个玩家受到毒性DOT伤害，每2秒掉1200血，持续10秒，不能驱散，当DOT时间结束时，在该玩家脚下生成一个销魂烟效果。
------------------------------------------------
...
function GetSkillLevelData(skill)
  ...
  -- 	skill.AddAttribute(
	...
	----------------- BUFF相关 -------------------------------------------------
  skill.BindBuff(1,1267,2);		-- 设置1号位Buff 作用于自身的HOT
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame)
	local pPlayer = GetPlayer(nCharacterID)
	if pPlayer then
		...
		if scene and  scene.dwMapID == 46 then
		    ...
		    pPlayer.AddBuff(0,99,1746,1)
		    nCount = nCount + 1
		 	scene.CreateNpc(5805,pPlayer.nX,pPlayer.nY,pPlayer.nZ,0,2560, "boss2_"..nCount);--此处写Boss名字+"_",只要加入名字,其他信息不变
		end
	end
end
