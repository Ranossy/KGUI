...
function GetSkillLevelData(skill)
	...
	"skill/npc/副本BOSS/25人战宝迦兰/战宝迦兰_4号BOSS_掌火_掌火的注视.lua",				-- 属性值1
	0														-- 属性值2
	); 
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
  skill.BindBuff(1,1750,1);		-- 设置1号位Buff 
	...
	if IsPlayer(dwCharacterID) then
	    ...
	    if player then
			...
			if scene and scene.dwMapID == 46 then
	            local npc = scene.GetNpcByNickName("boss4")
	            if npc then
	                npc.Yell(player.szName..GetEditorString(2, 8036))
	            end
			end
	    end
	end
