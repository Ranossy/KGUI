...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/战宝军械库/战宝军械库_卓力格图_掷锤_施放.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	local npc = scene.GetNpcByNickName("zhuoligetu")
	local npcCZ = scene.GetNpcByNickName("zhuoligetu_gongchengchui")
	if npcCZ then
		npc.PlaySfx(60, npc.nX, npc.nY, npc.nZ)
		npcCZ.SetPosition(player.nX, player.nY, player.nZ)
	else
		npc.PlaySfx(60, npc.nX, npc.nY, npc.nZ)
		if scene.dwMapID == 160 then
			scene.CreateNpc(21885, player.nX, player.nY, player.nZ, 0, -1, "zhuoligetu_gongchengchui")
		elseif scene.dwMapID == 171 then
			scene.CreateNpc(24871, player.nX, player.nY, player.nZ, 0, -1, "zhuoligetu_gongchengchui")
		end
	end
	if npc then
		--npc.AddBuff(0, 99, 5436, 1)
		scene.SetTimer(24, "scripts/Map/战宝军械库/event/换模延迟处理.lua", 0, 0)
	end
end
