...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/战宝军械库/战宝军械库_澹台枯火_骨牢_施放.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	tDirection[2] = (npc.nFaceDirection + 51) % 256
	tDirection[3] = (npc.nFaceDirection + 102) % 256
	tDirection[4] = (npc.nFaceDirection + 153) % 256
	tDirection[5] = (npc.nFaceDirection + 204) % 256
  for	i = 1, 5 do  
		...
		if not scene then
  		...
  		if scene.dwMapID == 160 then
  			local npcYb = scene.CreateNpc(21886, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
  			if npcYb then
				npcYb.RunTo(nX, nY)
			end
  		elseif scene.dwMapID == 171 then
  			local npcYb = scene.CreateNpc(24872, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
  			if npcYb then
				npcYb.RunTo(nX, nY)
			end
  		end
