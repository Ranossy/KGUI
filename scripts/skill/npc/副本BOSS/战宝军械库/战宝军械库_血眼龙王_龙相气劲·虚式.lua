...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/战宝军械库/战宝军械库_血眼龙王_龙相气劲·虚式.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	local tDirection = {}
	tDirection[1] = npc.nFaceDirection + 60 % 256
	tDirection[2] = npc.nFaceDirection - 60 % 256
	tDirection[3] = npc.nFaceDirection 
	...
	if scene.dwMapID == 160 then
		for	i = 1, 2 do  
			...
			local npcYb = scene.CreateNpc(23191, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
			if npcYb then
				npcYb.RunTo(nX, nY)
			end
		end
		...
		for	i = 1, 5 do  
			...
			local npcYb = scene.CreateNpc(24919, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
			if npcYb then
				npcYb.RunTo(nX, nY)
			end
		end
