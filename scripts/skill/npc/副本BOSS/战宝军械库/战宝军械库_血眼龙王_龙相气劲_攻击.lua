...
tSkillData =
{
	...
	{nDamageBase = 2300, nDamageRand = 0, nCostMana = 0},		--level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/战宝军械库/战宝军械库_血眼龙王_龙相气劲_攻击.lua",								--属性值1
		0															--属性值2
	);	
	...
	----------------- BUFF相关 -------------------------------------------------
	  skill.BindBuff(1, 5343, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID,dwSrcID)
	...
	local nBoss = scene.GetNpcByNickName("xiaosha")
	if not nBoss then
		return
	end
	if scene.dwMapID == 160 then
		if player.nCurrentMana < player.nMaxMana * 0.03 then
			player.nCurrentMana = 0
		else
			player.nCurrentMana = player.nCurrentMana - player.nMaxMana * 0.03
		end	
		if player.nCurrentMana < player.nMaxMana * 0.2 then
			npc.CastSkill(5386, 3, TARGET.NPC, dwSrcID)
			nBoss.AddBuff(0, 99, 5342, 1)
			nBoss.AddBuff(0, 99, 5342, 1)
			nBoss.AddBuff(0, 99, 5342, 1)
		elseif  player.nCurrentMana < player.nMaxMana * 0.6 then
			npc.CastSkill(5386, 2, TARGET.NPC, dwSrcID)
			nBoss.AddBuff(0, 99, 5342, 1)
			nBoss.AddBuff(0, 99, 5342, 1)
		else
			npc.CastSkill(5386, 1, TARGET.NPC, dwSrcID)
			nBoss.AddBuff(0, 99, 5342, 1)
		end
	else
		...
		if player.nCurrentMana < player.nMaxMana * 0.2 then
			npc.CastSkill(5386, 6, TARGET.NPC, dwSrcID)
			nBoss.AddBuff(0, 99, 5342, 1)
			nBoss.AddBuff(0, 99, 5342, 1)
			nBoss.AddBuff(0, 99, 5342, 1)
		elseif  player.nCurrentMana < player.nMaxMana * 0.6 then
			npc.CastSkill(5386, 5, TARGET.NPC, dwSrcID)
			nBoss.AddBuff(0, 99, 5342, 1)
			nBoss.AddBuff(0, 99, 5342, 1)
		else
			npc.CastSkill(5386, 4, TARGET.NPC, dwSrcID)
			nBoss.AddBuff(0, 99, 5342, 1)
		end		
