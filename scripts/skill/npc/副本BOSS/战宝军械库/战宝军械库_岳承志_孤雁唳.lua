...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/战宝军械库/战宝军械库_岳承志_孤雁唳.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	local npcB = GetNpc(dwCharacterID)
	if not npcB then
		return
	end
	local scene = npcB.GetScene()
	if not scene then
		return
	end
	local player_onscene = scene.GetAllPlayer()
	local tplayer_close = {}
	local tplayer_far = {}
	if player_onscene then
		for i = 1, #player_onscene do
			local playerthis = GetPlayer(player_onscene[i])
			if playerthis and playerthis.nMoveState ~= MOVE_STATE.ON_DEATH then
				local kungfu = playerthis.GetKungfuMount()
				if kungfu then
					if kungfu.dwSkillID == 10224 or kungfu.dwSkillID == 10225 or kungfu.dwSkillID == 10014
						or kungfu.dwSkillID == 10080 or kungfu.dwSkillID == 10081 or kungfu.dwSkillID == 10175
						or kungfu.dwSkillID == 10176 or kungfu.dwSkillID == 10021 or kungfu.dwSkillID == 10028 then
						table.insert(tplayer_far, playerthis)
					else
						table.insert(tplayer_close, playerthis)
					end
				else
					table.insert(tplayer_close, playerthis)
				end
	...
	local tRandom = {}
	if #tplayer_far ~= 0 then
		local nRandomSub = #tplayer_far
		for i = 1, #tplayer_far do
			if nRandomSub >= 1 then
				local nRandomFar = Random(1, nRandomSub)
				table.insert(tRandom, tplayer_far[nRandomFar])
				nRandomSub = nRandomSub - 1
			end
		end
	end
	if #tplayer_close ~= 0 then
		local nRandomSub = #tplayer_close
		for i = 1, #tplayer_close do
			if nRandomSub >= 1 then
				local nRandomClose = Random(1, nRandomSub)
				table.insert(tRandom, tplayer_close[nRandomClose])
				nRandomSub = nRandomSub - 1
			end
		end
	end
	...
	if scene.dwMapID == 160 then
		buff = npcB.GetBuff(5443, 1)
	else
		buff = npcB.GetBuff(5443, 2)
	end
		
	if buff then
		nCount = nCount + buff.nStackNum  
	end
	for i = 1, nCount do
		local player_select = tRandom[i]
		if player_select then
			if scene.dwMapID == 160 then
				player_select.AddBuff(0, 99, 5871, 1)
			elseif scene.dwMapID == 171 then
				player_select.AddBuff(0, 99, 5872, 1)
			end
		end
	end
end
