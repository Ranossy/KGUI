...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(                                                           
		...
		"skill/npc/副本BOSS/战宝军械库/战宝军械库_卓力格图_敲山震虎_判断.lua",								--属性值1
		0	                                                                      
	);
    
	----------------- 技能施放Buff需求 ---------------------------------------------
	  skill.AddSlowCheckSelfBuff(5436, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 5437, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSrcID)
	local npcB = GetNpc(dwSrcID)
	if not npcB then
		return
	end
	npcB.AddBuff(0, 99, 5437, 1)
end
