...
tSkillData =
{
	...
	{nDamageBase = 22100, nDamageRand = 0, nCostMana = 0},		--level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/战宝军械库/战宝军械库_血眼龙王_龙相缚镖.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	local npc = scene.GetNpcByNickName("xiaosha")
	if not npc then
		return
	end
	if scene.dwMapID == 160 then
		if player.nCurrentMana < player.nCurrentMana * 0.2 then
			npc.CastSkill(5386, 3, TARGET.NPC, dwCharacterID)
		elseif  player.nCurrentMana < player.nCurrentMana * 0.6 then
			npc.CastSkill(5386, 2, TARGET.NPC, dwCharacterID)
		else
			npc.CastSkill(5386, 1, TARGET.NPC, dwCharacterID)
		end
		if player.nCurrentMana < player.nCurrentMana * 0.02 then
		...
		if player.nCurrentMana < player.nCurrentMana * 0.2 then
			npc.CastSkill(5386, 6, TARGET.NPC, dwCharacterID)
		elseif  player.nCurrentMana < player.nCurrentMana * 0.6 then
			npc.CastSkill(5386, 5, TARGET.NPC, dwCharacterID)
		else
			npc.CastSkill(5386, 4, TARGET.NPC, dwCharacterID)
		end
		if player.nCurrentMana < player.nCurrentMana * 0.05 then
			player.nCurrentMana = 0
		else
