...
tSkillData =
{
	{nDamageBase = 12600, nDamageRand = 0, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/战宝军械库/战宝军械库_闻人彦范_乘风箭.lua",								--属性值1
		0															--属性值2
	);	
	
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK, 
		ATTRIBUTE_TYPE.CONSUME_BUFF, 
		5870, 
		dwSkillLevel																
	);	
	...
	----------------- BUFF相关 -------------------------------------------------
	  skill.BindBuff(1, 5880, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	local npcBoss = scene.GetNpcByNickName("wenrenyanfan")
	if npcBoss and npcBoss.GetCustomBoolean(62) == false then
		npcBoss.SetCustomBoolean(62, true)
	end
	player.Stop()
	player.AddBuff(0,99,5441,1)
end
