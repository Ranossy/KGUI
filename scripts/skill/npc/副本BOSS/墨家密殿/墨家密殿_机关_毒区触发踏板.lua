---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/墨家密殿/墨家密殿_机关_毒区触发踏板.lua
-- 更新时间:	2014/3/21 12:01:29
-- 更新用户:	ZHOUMING
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/墨家密殿/墨家密殿_机关_毒区触发踏板.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSrc)
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		local doodad = scene.GetDoodadByNickName("kaimentaban5")
		if doodad and doodad.IsDoorClose() then
			doodad.OpenDoor()
		end
		for i = 3, 4 do
			local npc = scene.GetNpcByNickName("jiyong"..i)
			if npc and not npc.bFightState then--添加戟俑参战的逻辑
				npc.SetTarget(TARGET.PLAYER, player.dwID)
				npc.FireAIEvent(2001,0,0)
				--npc.SetForceID(28)
				npc.ModifyThreat(player.dwID,500)
			end
		end
	end
end
