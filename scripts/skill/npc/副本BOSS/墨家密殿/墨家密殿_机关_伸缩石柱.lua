---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/墨家密殿/墨家密殿_机关_伸缩石柱.lua
-- 更新时间:	2014/3/13 14:45:17
-- 更新用户:	ZHOUMING
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/墨家密殿/墨家密殿_机关_伸缩石柱.lua", -- 属性值1
		0														-- 属性值2
	);
	--skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and npc then
		for i = 1,10 do--删掉唐门、明教隐身buff
			if player.GetBuff(3203, i) then
				player.DelBuff(3203,i)
			end
		end
		for j = 1, 10 do
			if player.GetBuff(4052, j) then
				player.DelBuff(4052,j)
			end
		end
		local nDis = math.sqrt(GetDistanceSq(npc.nX, npc.nY, npc.nZ, player.nX, player.nY, player.nZ)) / 64
		local nTime = math.ceil(nDis / 12 * 4)
		player.SetTimer(nTime,"scripts/Map/墨家秘殿/event/伸缩石柱击飞玩家.lua",npc.dwID,0)
	end
end
