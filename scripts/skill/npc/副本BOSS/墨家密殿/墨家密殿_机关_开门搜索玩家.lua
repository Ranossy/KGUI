---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/墨家密殿/墨家密殿_机关_看门搜索玩家.lua
-- 更新时间:	2014/3/12 15:37:55
-- 更新用户:	ZHOUMING
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/墨家密殿/墨家密殿_机关_开门搜索玩家.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSrc)
	...
	local playerChr = GetPlayer(dwCharacterID)
	local npcSrc = GetNpc(dwSrc)
	if npc and npc.dwTemplateID == 30582 then
		npcSrc.SetCustomUnsigned1(0, 1)
		npcSrc.SetCustomUnsigned1(1, 1)
	elseif playerChr then
		if playerChr.nGender == CHARACTER_GENDER.FEMALE then
			npcSrc.SetCustomUnsigned1(0, 1)
		elseif playerChr.nGender == CHARACTER_GENDER.MALE then
                     npcSrc.SetCustomUnsigned1(1, 1)
		end
	end
end
