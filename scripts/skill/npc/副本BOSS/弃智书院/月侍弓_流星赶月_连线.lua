---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/安雨_能量输出链状表现.lua
-- 更新时间:	2014/10/8 21:35:41
-- 更新用户:	guozhaoxing1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
--[[	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/安雨_能量输出链状表现.lua", --属性值1
		0															--属性值2
	);
--]]
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- 时间相关 -------------------------------------------------
	skill.nPrepareFrames = 1 * GLOBAL.GAME_FPS;				-- 吟唱帧数
...
--[[	print("!@$%&*^%$&^*%")
	...
	local nType, nID = npcAnyu.GetSkillTarget()
	if nID then
		npcAnyu.TurnToCharacter(nID)
	end
	npcAnyu.CastSkill(10336, 1)
--]]
end
