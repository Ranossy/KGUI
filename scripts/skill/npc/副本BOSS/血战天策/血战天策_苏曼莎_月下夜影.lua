...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_苏曼莎_月下夜影.lua",								--属性值1
		0															--属性值2
	);	
    
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(6917, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	if scene.dwMapID == 175 then
		...
		if nResult then
			if player.nCurrentLife < 14200 then
				...
				player.AddBuff(0, 99, 6857, 1)
			end
		end
	else
		local nResult = FacePoint(player, npc, 180)
		if nResult then
			if player.GetBuff(7630, 1) then
				player.DelBuff(7630, 1)
			end
			if player.nCurrentLife < 28000 then
				player.Die()
			else
				player.nCurrentLife = player.nCurrentLife - 28000
				player.AddBuff(0, 99, 6857, 1)
			end
		end
	end
