...
---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_黑齿元佑_黑齿巫师_腐骨释放.lua
-- 更新时间:	2013/10/16 19:38:46
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
Include("scripts/Map/血战天策/include/BOSS6.lh")
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_黑齿元佑_黑齿巫师_腐骨释放.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 4310, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if tplayer then
		local tplayer_select = PriorityTable(tplayer)
		if tplayer_select then
			if #tplayer_select[1] ~= 0 then
				local nRandomSeed = math.floor(Random(1, (#tplayer_select[1] - 1) * 10000 + 9999) / 10000) + 1
				local player_select = tplayer_select[1][nRandomSeed]
				if player_select then
					if scene.dwMapID == 175 then
						scene.CreateNpc(26666, player_select.nX, player_select.nY, player_select.nZ, 0, 10 * GLOBAL.GAME_FPS)
					else
						scene.CreateNpc(27225, player_select.nX, player_select.nY, player_select.nZ, 0, 10 * GLOBAL.GAME_FPS)
					end
				end
			end
		end
