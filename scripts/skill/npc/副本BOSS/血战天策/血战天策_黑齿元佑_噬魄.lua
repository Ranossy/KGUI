---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_黑齿元佑_噬魄.lua
-- 更新时间:	2013/9/18 16:09:02
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function Apply(dwCharacterID)
	...
	local buff = player.GetBuff(6861, 1)
	if not buff then
		return
	end
	...
	local num = buff.nStackNum
	if scene.dwMapID == 175 then
		if player.nCurrentMana <= player.nMaxMana * 0.01 * num then
			if npcBoss.GetCustomBoolean(1) == true then
				player.CastSkill(8192, 1)
				local nCount = npcBoss.GetCustomInteger1(6)
				nCount = nCount + 1
				--scene.CreateNpc(26772, player.nX, player.nY, player.nZ, 0, -1, "guimei_" .. nCount)
				npcBoss.SetCustomInteger1(6, nCount)
				--player.Die()
			end
		else
			player.nCurrentMana = player.nCurrentMana - player.nMaxMana * 0.01 * num
		end
	else
		if player.nCurrentMana <= player.nMaxMana * 0.01 * num then
			if npcBoss.GetCustomBoolean(1) == true then
				player.CastSkill(8192, 2)
				local nCount = npcBoss.GetCustomInteger1(6)
				nCount = nCount + 1
				--scene.CreateNpc(27262, player.nX, player.nY, player.nZ, 0, -1, "guimei_" .. nCount)
				npcBoss.SetCustomInteger1(6, nCount)
				--player.Die()
			end
		else
