---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/英雄血战天策_雷狼_枪.lua
-- 更新时间:	2014/5/23 16:17:29
-- 更新用户:	wangpengfei1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/英雄血战天策_雷狼_枪.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc then
		if npc.dwTemplateID ~= 27123 and npc.dwTemplateID ~= 27125 then
			local player = GetPlayer(dwSkillSrcID)
			if player then
				player.DelGroupBuff(7106, 1)
				player.DelGroupBuff(7106, 2)
				player.DownHorse()
				player.SetDynamicSkillGroup(0)
				player.SetRepresentID(EQUIPMENT_REPRESENT.HORSE_STYLE, 0)
				local nNum = player.GetItemAmountInAllPackages(ITEM_TABLE_TYPE.OTHER, 20762)
				if nNum >= 1 then
					player.CostItemInAllPackage(ITEM_TABLE_TYPE.OTHER, 20762, nNum)
				end
			end
		end
	end
