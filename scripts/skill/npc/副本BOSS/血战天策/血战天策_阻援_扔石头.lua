...
tSkillData =
{
	{nDamageBase = 180000, nDamageRand = 0, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
    ...
    skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_前线_扔石头特效.lua",
		0
	);
	...
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,
		ATTRIBUTE_TYPE.EXECUTE_SCRIPT,
		"skill/npc/副本BOSS/血战天策/血战天策_阻援_扔石头.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	  --skill.BindBuff(1, 6673, 1);		-- 设置1号位Buff
	...
	skill.SetNormalCoolDown(1, 703);	--技能普通CD
...
function Apply(dwCharacterID)
	...
	if npctarget.GetBuff(6737, 1) then
		npctarget.DelBuff(6737, 1)
	end
	
	local scene = npctarget.GetScene()
	if scene.dwMapID == 175 then
		if npctarget.dwTemplateID == 26244 or npctarget.dwTemplateID == 26257 or npctarget.dwTemplateID == 26258 then
			npctarget.AddBuff(0, 99, 6673, 1, 15)
		end
	end
	
	if scene.dwMapID == 176 then
		if npctarget.dwTemplateID == 27153 or npctarget.dwTemplateID == 27158 or npctarget.dwTemplateID == 27159 then
			npctarget.AddBuff(0, 99, 6673, 1, 15)
		end
	end
end
