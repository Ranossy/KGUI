---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_黑齿元佑_生命汲取.lua
-- 更新时间:	2013/9/15 12:00:03
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function Apply(dwCharacterID)
	...
	if npcthis.nCurrentLife == npcthis.nMaxLife then
		npcthis.DelBuff(6838, 1)
		return
	end
	local npcBoss = scene.GetNpcByNickName("BOSS_heichiyuanyou")
	if scene.dwMapID == 175 then
		if npcBoss then
			if npcthis.nCurrentLife > npcthis.nMaxLife * 0.01 then
				npcthis.nCurrentLife = npcthis.nCurrentLife - npcthis.nMaxLife * 0.01
				npcBoss.nCurrentMana = npcBoss.nCurrentMana + npcBoss.nMaxMana * 0.0067
				npcthis.SetTarget(TARGET.NPC, npcBoss.dwID)
				npcthis.CastSkill(8187, 1)
			else
				if npcBoss.GetCustomBoolean(1) == true then
					local nCount = npcBoss.GetCustomInteger1(6)
					nCount = nCount + 1
					scene.CreateNpc(26772, npcthis.nX, npcthis.nY, npcthis.nZ, 0, -1, "guimei_" .. nCount)
					npcBoss.SetCustomInteger1(6, nCount)
					npcBoss.SetCustomBoolean(61, true)
					npcthis.Die()
					npcBoss.nCurrentMana = npcBoss.nCurrentMana + npcBoss.nMaxMana * 0.0067
				end
			end
		...
		if npcBoss then
			if npcthis.nCurrentLife > npcthis.nMaxLife * 0.015 then
				npcthis.nCurrentLife = npcthis.nCurrentLife - npcthis.nMaxLife * 0.015
				npcBoss.nCurrentMana = npcBoss.nCurrentMana + 147000
				npcthis.SetTarget(TARGET.NPC, npcBoss.dwID)
				npcthis.CastSkill(8187, 1)
			else
				if npcBoss.GetCustomBoolean(1) == true then
					local nCount = npcBoss.GetCustomInteger1(6)
					nCount = nCount + 1
					scene.CreateNpc(27262, npcthis.nX, npcthis.nY, npcthis.nZ, 0, -1, "guimei_" .. nCount)
					npcBoss.SetCustomInteger1(6, nCount)
					npcBoss.SetCustomBoolean(61, true)
					npcthis.Die()
					npcBoss.nCurrentMana = npcBoss.nCurrentMana + 147000
