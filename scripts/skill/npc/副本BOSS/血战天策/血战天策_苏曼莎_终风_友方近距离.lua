...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_苏曼莎_终风_友方近距离.lua",								--属性值1
		0															--属性值2
	);		
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 6906, dwSkillLevel);		-- 设置1号位Buff
	skill.BindBuff(2, 6969, dwSkillLevel);		-- 设置2号位Buff
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 175 then
		if npc.dwTemplateID == 26459 then
			return
		else
			npc.nCurrentLife = npc.nCurrentLife + npc.nMaxLife * 0.15
		end
	elseif scene.dwMapID == 176 then
		if npc.dwTemplateID == 27198 then
			return
		else
			npc.nCurrentLife = npc.nCurrentLife + npc.nMaxLife * 0.15
		end
