...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_杨宁_判断敌方个数.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID,dwSrcID)
	...
	if scene.dwMapID == 175 then
		if player.nCurrentLife >= player.nMaxLife * 0.75 and player.nCurrentMana >= player.nMaxMana * 0.75 then
			npc.AddBuff(0, 99, 7210, 1)
		end
		if player.nCurrentLife >= player.nMaxLife * 0.75 then
			npc.AddBuff(0, 99, 7108, 1)
		end
	elseif scene.dwMapID == 176 then
		if player.nCurrentLife >= player.nMaxLife * 0.75 and player.nCurrentMana >= player.nMaxMana * 0.75 then
			npc.AddBuff(0, 99, 7210, 2)
		end
		if player.nCurrentLife >= player.nMaxLife * 0.75 then
			npc.AddBuff(0, 99, 7108, 2)
		end
	end
