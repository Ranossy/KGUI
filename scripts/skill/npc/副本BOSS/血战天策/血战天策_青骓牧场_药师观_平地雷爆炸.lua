---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_青骓牧场_药师观_平地雷爆炸.lua
-- 更新时间:	2013/12/2 15:15:04
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
tSkillData = 
{
	{nDamageBase = 315000, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 1700000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_青骓牧场_药师观_平地雷爆炸.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 6980, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if tplayer then
		for i = 1, #tplayer do
			...
			if playerthis then
				if playerthis.nX <= 22109 and playerthis.nX >= 14801 and playerthis.nY <= 72330 and playerthis.nY >= 59577 and playerthis.nMoveState ~= MOVE_STATE.ON_DEATH then
					npcthis.ModifyThreat(playerthis.dwID, 100)
				end
			end
		end
	end
