...
tSkillData =
{
	{nDamageBase = 167000, nDamageRand = 0, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_阻援_投石特效.lua",
		0
	);
	...
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,
		ATTRIBUTE_TYPE.EXECUTE_SCRIPT,
		"skill/npc/副本BOSS/血战天策/血战天策_阻援_投石.lua",
		0
	);
	...
	-- 技能CD, CoolDownIndex为CD位(共3个), nCoolDownID为CoolDownList.tab内的CDID
	  skill.SetNormalCoolDown(1, 644);	--技能普通CD
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 175 then
		if npctarget.dwTemplateID == 26352 or npctarget.dwTemplateID == 26353 or npctarget.dwTemplateID == 26354 then
			npctarget.FireAIEvent(2001, 0, 0)
		end
	end
	
	if scene.dwMapID == 176 then
		if npctarget.dwTemplateID == 27180 or npctarget.dwTemplateID == 27181 or npctarget.dwTemplateID == 27182 then
			npctarget.FireAIEvent(2001, 0, 0)
		end
	end
end
