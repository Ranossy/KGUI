---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_雷狼_加速.lua
-- 更新时间:	2013/9/26 10:58:52
-- 更新用户:	liguanghao2
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_黑齿元佑_加速.lua", --属性值1
		0															--属性值2
	);	
    
	----------------- 技能施放Buff需求 ---------------------------------------------
	  skill.AddSlowCheckSelfBuff(6896, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
...
function Apply(dwCharacterID)
	...
	if player then
		if not player.GetBuff(6896, 1) and not player.GetBuff(6894, 1) and not player.GetBuff(6895, 1) then
			player.AddBuff(0, 99, 6894, 1)
		elseif not player.GetBuff(6896, 1) and player.GetBuff(6894, 1) then
			player.AddBuff(0, 99, 6895, 1)
		end
	end
end
