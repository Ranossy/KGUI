---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_青骓牧场_药师观_油罐投掷.lua
-- 更新时间:	2013/10/14 16:36:20
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_青骓牧场_药师观_油罐投掷.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	if tplayer then
		for i = 1, #tplayer do
			...
			if playerthis then
				if playerthis.nX <= 21725 and playerthis.nX >= 14958 and playerthis.nY <= 71481 and playerthis.nY >= 64233 and playerthis.nMoveState ~= MOVE_STATE.ON_DEATH then
					table.insert(tplayerselect, playerthis)
				end
			end
		end
	end
	if #tplayerselect == 0 then
		return
	end
	local nRandom = Random(1, #tplayerselect)
	if nRandom then
		local playerselect = tplayerselect[nRandom]
		if playerselect then
			if scene.dwMapID == 175 then
				scene.CreateNpc(26919, playerselect.nX, playerselect.nY, playerselect.nZ, 0, 10 * GLOBAL.GAME_FPS)
			else
				scene.CreateNpc(27355, playerselect.nX, playerselect.nY, playerselect.nZ, 0, 10 * GLOBAL.GAME_FPS)
			end
		end
	end
end
