...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_杨宁_豪·守如山.lua",								--属性值1
		0															--属性值2
	);	
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7076, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if player.GetKungfuMount() and player.GetKungfuMount().dwSkillID == 10026 or player.GetKungfuMount().dwSkillID == 10062 then
		player.AddBuff(0,99,7117,1)
	end
end
