---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策-八阵图-AOE熊.lua
-- 更新时间:	2013/11/14 21:54:33
-- 更新用户:	ZHOUMING
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策-八阵图-AOE熊.lua",
		0														
	);	
	--skill.AddAttribute( 
	...
	--skill.AddSlowCheckDestBuff(6945, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSrcID)
	--print(10086)
	local npcMeat = GetNpc(dwSrcID)
	local npcTarget = GetNpc(dwCharacterID)
	if not npcMeat then
		return 
	end
	local scene = npcMeat.GetScene()
	if not scene or not tBearTemplateID[scene.dwMapID] then
		return
	end
	--print(10089)
	if not npcTarget or npcTarget.dwTemplateID ~= tBearTemplateID[scene.dwMapID] then
		return
	end
	--print(10090)
	npcMeat.ModifyThreat(npcTarget.dwID, 100000)
	npcTarget.ModifyThreat(npcMeat.dwID,100000)
	--npcTarget.ModifyThreat(npcMeat.dwID,1)
end
