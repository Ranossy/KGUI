...
tSkillData = 
{
	...
	{nDamageBase = 8000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_小怪_暗花_正面.lua",								--属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID,dwSrcID)
	...
	if scene.dwMapID == 175 then
		if player.GetOTActionState() == CHARACTER_OTACTION_TYPE.ACTION_SKILL_PREPARE or player.GetOTActionState() == CHARACTER_OTACTION_TYPE.ACTION_SKILL_CHANNEL then
			player.AddBuff(0, 99, 6998, 1)
			player.AddBuff(0, 99, 6999, 1)
		end
		local nResult = FacePoint(player, npc, 180)
		if not nResult then
			npc.CastSkill(8353,1,TARGET.PLAYER,dwCharacterID)
		end		
	else
		if player.GetOTActionState() == CHARACTER_OTACTION_TYPE.ACTION_SKILL_PREPARE or player.GetOTActionState() == CHARACTER_OTACTION_TYPE.ACTION_SKILL_CHANNEL then
			player.AddBuff(0, 99, 6998, 1)
			player.AddBuff(0, 99, 6999, 2)
		end
		local nResult = FacePoint(player, npc, 180)
		if not nResult then
			npc.CastSkill(8353,2,TARGET.PLAYER,dwCharacterID)
		end		
	end
end
