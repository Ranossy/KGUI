...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_小怪_一段射杀.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player.GetBuff(6988, 1) then
		local nBuff = player.GetBuff(6988,1).nStackNum
		npc.CastSkill(8570,nBuff,TARGET.PLAYER,dwCharacterID)
	end
	if player.GetBuff(6988, 2) then
		local nBuff = player.GetBuff(6988,2).nStackNum
		npc.CastSkill(8570,nBuff+10,TARGET.PLAYER,dwCharacterID)
	end	
end
