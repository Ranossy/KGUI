...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(                                                           
		...
		"skill/npc/副本BOSS/血战天策/血战天策_狼牙据点_炮击成就.lua",                                                                        
		0                                                                        
	);    
...
function Apply(dwCharacterID,dwSrcID)
	...
	local npcPaoTai = scene.GetNpcByNickName("XZTC_LYJD_PaoTai1")
	if not npcPaoTai then
		return
	end
	local player = GetPlayer(npcPaoTai.GetCustomUnsigned4(0))
	if not player then
		return
	end
	local buff = player.GetBuff(7232, 1)
	if buff then
		local nCustomValue = buff.nCustomValue
