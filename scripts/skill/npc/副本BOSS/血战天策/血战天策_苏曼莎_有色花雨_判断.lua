...
tSkillData = 
{
	...
	{nDamageBase = 14200, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_苏曼莎_有色花雨_判断.lua",								--属性值1
		0															--属性值2
	);		
...
function Apply(dwCharacterID,dwSrcID)
	...
	if scene.dwMapID == 175 then
		if player.GetBuff(6924, 1) then
			if not player.GetKungfuMount() then
				player.AddBuff(0, 99, 6925, 1)
			elseif player.GetKungfuMount().dwSkillID == 10028 or player.GetKungfuMount().dwSkillID == 10080 or player.GetKungfuMount().dwSkillID == 10176 then
				player.AddBuff(0, 99, 6926, 1)
			else
				player.AddBuff(0, 99, 6925, 1)
			end
		else
			npc.CastSkill(8279,1)
		end
	else
		if player.GetBuff(6924, 2) then
			if not player.GetKungfuMount() then
				player.AddBuff(0, 99, 6925, 2)
			elseif player.GetKungfuMount().dwSkillID == 10028 or player.GetKungfuMount().dwSkillID == 10080 or player.GetKungfuMount().dwSkillID == 10176 then
				player.AddBuff(0, 99, 6926, 2)
			else
				player.AddBuff(0, 99, 6925, 2)
			end
		else
			npc.CastSkill(8279,2)
		end
	end
end
