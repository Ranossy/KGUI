---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/英雄血战天策_令狐伤_一剑诀·六重.lua
-- 更新时间:	2016/5/20 14:23:42
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 1473500, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/英雄血战天策_令狐伤_一剑诀·六重.lua",
		0
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 7264, 1);		-- 设置1号位Buff
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if player.nCurrentMana < player.nMaxMana * 0.99 then
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcSMS = scene.GetNpcByNickName("sumansha")
	local npcLHS = scene.GetNpcByNickName("linghushang")

	if (npcSMS and npcSMS.bFightState == true) or (npcLHS and npcLHS.bFightState == true) then
		local mapID = npc.GetMapID()
		if mapID == 175 then
			for i = 1, #playerlist do
				local playerOne = GetPlayer(playerlist[i])
				if playerOne then
					if not playerOne.IsAchievementAcquired(3992) then
						playerOne.AcquireAchievement(3992)
					end
				end
			end
		end
		if mapID == 176 then
			for i = 1, #playerlist do
				local playerOne = GetPlayer(playerlist[i])
				if playerOne then
					if not playerOne.IsAchievementAcquired(4012) then
						playerOne.AcquireAchievement(4012)
					end
				end
			end
		end
