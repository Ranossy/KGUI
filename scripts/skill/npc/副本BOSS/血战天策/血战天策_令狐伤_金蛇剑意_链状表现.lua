...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_令狐伤_金蛇剑意_链状表现.lua",								--属性值1
		0															--属性值2
	);				
	
	skill.SetSubsectionSkill( 1, 9, 8595, 1)
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	--skill.AddSlowCheckDestBuff(7154, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7154, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if kungfu.dwSkillID == 10224 or kungfu.dwSkillID == 10225 or kungfu.dwSkillID ==  10014 or kungfu.dwSkillID ==  10080 or kungfu.dwSkillID ==  10081 or kungfu.dwSkillID ==  10175 or kungfu.dwSkillID ==  10176 or kungfu.dwSkillID ==  10021 or kungfu.dwSkillID ==  10028 then
		player.AddBuff(0,99,7153,1)
	end	
	if not scene then
		return
	end
	local npc = scene.GetNpcByNickName("hanguang")
	if not npc then
		return
	end
	if scene.dwMapID == 175 then
		npc.CastSkill(8540, 1, TARGET.PLAYER, dwCharacterID)
	elseif scene.dwMapID == 176 then
		npc.CastSkill(8540, 2, TARGET.PLAYER, dwCharacterID)
	else
		npc.CastSkill(8540, 3, TARGET.PLAYER, dwCharacterID)
	end	
end
