...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_令狐伤_白蛇吐信_施放.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 175 then
		npcYb = scene.CreateNpc(26978, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
	elseif scene.dwMapID == 176 then
		npcYb = scene.CreateNpc(27379, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
	elseif scene.dwMapID == 182 then
		npcYb = scene.CreateNpc(30663, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
	elseif scene.dwMapID == 183 then
		npcYb = scene.CreateNpc(36243, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
	end
	if npcYb then
		npcYb.RunTo(nX, nY)
	end	
