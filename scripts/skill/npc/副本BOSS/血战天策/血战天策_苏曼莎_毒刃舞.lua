...
tSkillData = 
{
	{nDamageBase = 6600, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 7800, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_苏曼莎_毒刃舞.lua",								--属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID,dwSrcID)
	...
	local nBoss = scene.GetNpcByNickName("linghushang")
	if not nBoss then
		return
	end
	if scene.dwMapID == 175 then
		player.AddBuff(nBoss.dwID, nBoss.nLevel, 6927, 1)
		if player.GetBuff(6927, 1) and player.GetBuff(6927, 1).nStackNum == 5 then
			npc.CastSkill(8566,1,TARGET.PLAYER,dwCharacterID)
		end
	else
		player.AddBuff(nBoss.dwID, nBoss.nLevel, 6927, 2)
		if player.GetBuff(6927, 2) and player.GetBuff(6927, 2).nStackNum == 5 then
			npc.CastSkill(8566,2,TARGET.PLAYER,dwCharacterID)
		end
	end
end
