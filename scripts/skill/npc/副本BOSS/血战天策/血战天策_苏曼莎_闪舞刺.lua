...
tSkillData = 
{
	{nDamageBase = 23600, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 27800, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_苏曼莎_闪舞刺.lua",								--属性值1
		0															--属性值2
	);	
	...
	skill.nMaxRadius		= 1200 * LENGTH_BASE;		-- 技能施放的最大距离 
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player.GetBuff(6917, 1) then
		npc.CastSkillXYZ(8287, 2, player.nX, player.nY, player.nZ)
		player.Die()
	end
	if player.GetBuff(6917, 2) then
		npc.CastSkillXYZ(8287, 2, player.nX, player.nY, player.nZ)
		player.Die()
	end	
	if player.GetBuff(6927, 1) then
		npc.CastSkill(8288,1)
	end
	if player.GetBuff(6927, 2) then
		npc.CastSkill(8288,2)
	end
	...
	if scene.dwMapID == 175 then
		if player.GetKungfuMount() then
			if player.GetKungfuMount().dwSkillID == 10144 or player.GetKungfuMount().dwSkillID == 10145 or player.GetKungfuMount().dwSkillID == 10224 or player.GetKungfuMount().dwSkillID == 10225 or player.GetKungfuMount().dwSkillID == 10242 or player.GetKungfuMount().dwSkillID == 10243 then
				if player.nCurrentLife < player.nMaxLife * 0.3 then
					player.AddBuff(0,99,6937,1)
				end
			elseif player.nCurrentLife < player.nMaxLife * 0.5 then
				player.nCurrentMana = 0
			else
				player.AddBuff(0,99,6938,1)
			end
		end
	else
		if player.GetKungfuMount() then
			if player.GetKungfuMount().dwSkillID == 10144 or player.GetKungfuMount().dwSkillID == 10145 or player.GetKungfuMount().dwSkillID == 10224 or player.GetKungfuMount().dwSkillID == 10225 or player.GetKungfuMount().dwSkillID == 10242 or player.GetKungfuMount().dwSkillID == 10243 then
				if player.nCurrentLife < player.nMaxLife * 0.3 then
					player.AddBuff(0,99,6937,2)
				end
			elseif player.nCurrentLife < player.nMaxLife * 0.5 then
				player.nCurrentMana = 0
			else
				player.AddBuff(0,99,6938,2)
			end
		end		
	end
end
