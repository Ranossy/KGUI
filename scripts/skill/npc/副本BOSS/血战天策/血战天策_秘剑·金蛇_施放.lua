...
tSkillData = 
{
	...
	{nDamageBase = 50000, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 21700, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 52200, nDamageRand = 0, nCostMana = 0}, --level 4
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_秘剑·金蛇_施放.lua",								--属性值1
		0															--属性值2
	);	
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 6927, skill.dwLevel);		-- 设置1号位Buff
...
local tDirection = {}
	...
	tDirection[2] = npc.nFaceDirection - 64 % 256
	tDirection[3] = npc.nFaceDirection + 64 % 256
	tDirection[4] = npc.nFaceDirection - 128 % 256
local scene = npc.GetScene()
  ...
  for	i = 1, 4 do  
  	...
  	local npcYb
  	if scene.dwMapID == 175 then  --10人
  		npcYb = scene.CreateNpc(26943, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
  	elseif scene.dwMapID == 176 then	--25人血战
  		npcYb = scene.CreateNpc(27363, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
  	elseif scene.dwMapID == 182 then --10人秦皇陵
  		if npc.GetCustomBoolean(3) then
  			npcYb = scene.CreateNpc(30662, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
  		end
  	elseif scene.dwMapID == 183 then --25人秦皇陵
  		if npc.GetCustomBoolean(3) then
  			npcYb = scene.CreateNpc(36242, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
  		end	
  	end
