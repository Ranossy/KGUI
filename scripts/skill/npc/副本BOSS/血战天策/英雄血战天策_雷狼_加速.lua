...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/英雄血战天策_雷狼_加速.lua", --属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	if player then
		if not player.GetBuff(6896, 2) and not player.GetBuff(6894, 2) and not player.GetBuff(6895, 2) then
			player.Talk(GetEditorString(5, 6453))
			player.AddBuff(0, 99, 6894, 2)
			
		elseif not player.GetBuff(6896, 2) and player.GetBuff(6894, 2) and not player.GetBuff(6895, 2) then
			player.AddBuff(0, 99, 6895, 2)
			player.Talk(GetEditorString(5, 6453))
		end
	end
end
