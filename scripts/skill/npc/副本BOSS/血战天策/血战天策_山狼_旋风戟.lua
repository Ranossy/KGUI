---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_山狼_旋风戟.lua
-- 更新时间:	2013/11/14 10:16:18
-- 更新用户:	ks13376-PC
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_山狼_旋风戟.lua", --属性值1
		0															--属性值2
	);	
    
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(6900, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 175 then
		if player then
			if not player.GetBuff(6900, 1) then
				if player.GetBuff(7184, 1) then
					if player.GetBuff(7184, 1).nStackNum >= 20 then
		...
		if player then
			if not player.GetBuff(6900, 1) then
				...
				player.AddBuff(0, 99, 7184, 1, 32)
				if npc then
					npc.nCurrentLife = npc.nCurrentLife + npc.nMaxLife * 0.001
				end
			end
		end
	end
