...
function Apply(dwCharacterID)
	...
	local nmark = 0
	local nbuffcount = 1
	local nbuffcount_biyou = 0
	
	--取自身伤害buff层数
	if nplayer.GetBuff(6818, 1) then
		nbuffcount = nplayer.GetBuff(6818, 1).nStackNum
	end
	
	if nbuffcount >= 10 then
		for i = 1, 4 do
			if nplayer.GetBuff(6845 + i, 1) then
				nmark = nmark + 1
			end
		end
	end
	...
	--取自身庇佑buff层数
	if nplayer.GetBuff(6817, 1) then
		nbuffcount_biyou = nplayer.GetBuff(6817, 1).nStackNum
	end
	
	if nmark > nbuffcount_biyou then
		for i = 1, nmark - nbuffcount_biyou do
			nplayer.AddBuff(0,99,6817,1)
		end
	end
