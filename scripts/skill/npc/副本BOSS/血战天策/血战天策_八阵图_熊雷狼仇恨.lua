---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_八阵图_熊雷狼仇恨.lua
-- 更新时间:	2013/11/4 17:01:55
-- 更新用户:	liguanghao2
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
    ...
    skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_雷狼_普通攻击.lua",								--属性值1
		0															--属性值2
		);
	--]]
...
function Apply(dwCharacterID)
	...
	if npcBoss then
		if GetCharacterDistance(npc.dwID, npcBoss.dwID) < 320 then
			npc.Yell(GetEditorString(6, 1088))
			npc.ModifyThreat(npcBoss.dwID, 999999)
		else
			npc.ClearAllThreat()
			for i = 1, 5 do
				local npcRou = scene.GetNpcByNickName("yangrou" .. i)
				if npcRou then
					npc.SetTarget(TARGET.NPC, npcRou.dwID)
					return
				end
			end
		end
