...
tSkillData = 
{
	{nDamageBase = 14200, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 16700, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_小怪_香艳绞刑_正面.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID,dwSrcID)
	...
	if npc.GetBuff(7027,1) and npc.GetBuff(7027,1).nStackNum == 5 and player.nCurrentLife < player.nMaxLife * 0.3 then
		player.Die()
	end
	if npc.GetBuff(7027,2) and npc.GetBuff(7027,2).nStackNum == 5 and player.nCurrentLife < player.nMaxLife * 0.3 then
		player.Die()
	end	
	if npc.GetBuff(7027,1) and npc.GetBuff(7027,1).nStackNum >= 3 and player.nCurrentLife < player.nMaxLife * 0.5 then
		player.AddBuff(0,99,7018,1)
	end
	if npc.GetBuff(7027,2) and npc.GetBuff(7027,2).nStackNum >= 3 and player.nCurrentLife < player.nMaxLife * 0.5 then
		player.AddBuff(0,99,7018,2)
	end
	local nResult = FacePoint(player, npc, 180)
	if not nResult then
		npc.CastSkill(8365, 1, TARGET.PLAYER, dwCharacterID)
		return
	end
	if npc.GetBuff(7009, 1) and npc.GetBuff(7009, 1).nStackNum > 2 then
		npc.CastSkill(8365, 1, TARGET.PLAYER, dwCharacterID)
	end
	if npc.GetBuff(7008, 1) and npc.GetBuff(7010, 1) and npc.GetBuff(7010, 1).nStackNum > 1 then
		npc.CastSkill(8365, 3, TARGET.PLAYER, dwCharacterID)
	end
	if npc.GetBuff(7009, 2) and npc.GetBuff(7009, 2).nStackNum > 2 then
		npc.CastSkill(8365, 2, TARGET.PLAYER, dwCharacterID)
	end
	if npc.GetBuff(7008, 2) and npc.GetBuff(7010, 2) and npc.GetBuff(7010, 2).nStackNum > 1 then
		npc.CastSkill(8365, 4, TARGET.PLAYER, dwCharacterID)
	end	
end
...
function FacePoint(player, npc, nRad)
	local x1, y1 = npc.nX, npc.nY
	local x2, y2 = player.nX, player.nY 
	local nRadT = math.atan((y1 - y2) / (x1 - x2))
	if x1 < x2 then
		nRadT = nRadT + math.pi
	end
	local nRadMax = player.nFaceDirection * 2 * math.pi / 256 + nRad * math.pi / 360
	local nRadMin = player.nFaceDirection * 2 * math.pi / 256 - nRad * math.pi / 360
	if nRadT <=nRadMax and nRadT >= nRadMin then
		return true
	end
	return false
end
