---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_保护青骓牧场_绞喉母技能.lua
-- 更新时间:	2013/9/26 15:01:17
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_保护青骓牧场_绞喉母技能.lua", -- 属性值1
		0														-- 属性值2
	);
	skill.AddAttribute(
	...
	skill.AddSlowCheckDestBuff(6901, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID)
	...
	local nRad = 2 * math.pi * (cnpc.nFaceDirection * 360 / 255 - 20) / 360
	if cplayer.nRoleType == 1 then
		cplayer.SetPosition(cnpc.nX - 0.8 * 64 * math.cos(nRad), cnpc.nY - 0.8 * 64 * math.sin(nRad), cnpc.nZ)
	else 
		cplayer.SetPosition(cnpc.nX - 0.5 * 64 * math.cos(nRad), cnpc.nY - 0.5 * 64 * math.sin(nRad), cnpc.nZ)
	end
	cplayer.TurnTo(cnpc.nFaceDirection) 
	cplayer.CastSkill(8243, 1)
end
