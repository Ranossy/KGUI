---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/英雄血战天策_雷狼_忠魂.lua
-- 更新时间:	2013/12/11 14:45:19
-- 更新用户:	liguanghao2
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData = 
{
	{nDamageBase = 61500, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/英雄血战天策_雷狼_忠魂.lua",
		0
	)
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	skill.nHeight = 2 * LENGTH_BASE + 3 * dwSkillLevel * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
...
function Apply(dwCharacterID)
	...
	if npc then
		--------设置变量
		local nZhonghun = 0		--忠魂层数
		local nTimes = 0		--用于记录每次狼之魂已消除忠魂的次数
		local nNum = 0
		---------------------
		...
		if scene then
			...
			if #tPlayer ~= 0 then
				for i = 1, #tPlayer do
					...
					if player then
						local Buff = player.GetBuff(6721, 2)
						if Buff then
							local nNum1 = Buff.nStackNum
							nNum = nNum + nNum1
						end
					end
				end
		...
		local Buff = npc.GetBuff(6720, 2)
		if Buff then
			local npcLeilangZK = scene.GetNpcByNickName("leilangzongkong")
			nZhonghun = Buff.nStackNum
			if npcLeilangZK then
				npcLeilangZK.SetCustomInteger4(29, nZhonghun)
			end
			print("6720=" .. nZhonghun)
		end
		
		local nXiaochutime = npc.GetCustomInteger4(13)
		if nNum >= 30 * nXiaochutime then
			npc.DelBuff(6720, 2)
			nXiaochutime = nXiaochutime + 1
			npc.SetCustomInteger4(13, nXiaochutime)
		end
	end	
end
...
function UnApply(dwCharacterID)
	...
	local npc = GetNpc(dwCharacterID)
	if npc then
		local Buff = npc.GetBuff(6720, 1)
		if Buff then
			local nZhonghun = Buff.nLeftActiveCount
			if nZhonghun == 0 then
				--npc.DelGroupBuff(6720, 1)			--删除忠魂
				npc.AddBuff(7034, 1)				--添加狼魂
			end
		end	
	end
	--]]
