---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_令狐伤_占位二.lua
-- 更新时间:	2013/11/1 16:27:09
-- 更新用户:	liguanghao2
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_令狐伤_占位二.lua",
		0
	);
...
function Apply(dwCharacterID)
	...
	if player then
		...
		if buff and buff.nCustomValue == 2 then
			if npcLhs then
				npcLhs.SetCustomBoolean(51, true)              ----------------QTE变量判断
			end
			RemoteCallToClient(player.dwID, "OnDynamicSkillHighlightChanged", 1, false)
			RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "QTEPanel_HitNotify", 8521)
		else
			
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(6, 1017), 5)
