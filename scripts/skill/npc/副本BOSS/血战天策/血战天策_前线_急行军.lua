...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_前线_急行军.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	local qianxianzongkong = scene.GetNpcByNickName("qianxianzongkong")
	local dongbeishuaiguaikongzhi = scene.GetNpcByNickName("dongbeishuaiguaikongzhi")
	if scene.dwMapID == 175 then
		if qianxianzongkong then
			local nindex = qianxianzongkong.GetCustomInteger4(1)
			for i = 1, nindex + 1 do
				local xiaoguai = scene.GetNpcByNickName("chongfenglangya" .. i)
				if xiaoguai then
					xiaoguai.AddBuff(0, 99, 6669, 1)
				end
			end
		end
	
		if dongbeishuaiguaikongzhi then
			local nindex = dongbeishuaiguaikongzhi.GetCustomInteger4(1)
			for i = 1, nindex + 1 do
				local xiaoguai = scene.GetNpcByNickName("dongbeilangyabing" .. i)
				if xiaoguai then
					xiaoguai.AddBuff(0, 99, 6669, 1)
				end
			end
