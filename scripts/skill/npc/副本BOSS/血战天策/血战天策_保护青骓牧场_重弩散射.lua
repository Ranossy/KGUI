...
function GetSkillLevelData(skill)
    ...
    skill.AddAttribute(
		...
		--"skill/npc/副本BOSS/血战天策/血战天策_保护青雅牧场_重弩标记射击.lua",--属性值1
		"skill/npc/副本BOSS/血战天策/血战天策_保护青骓牧场_重弩散射.lua",
		0															--属性值2
	);		
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7011, 1);		-- 设置1号位Buff
	...
	skill.SetNormalCoolDown(1, 669);	--技能普通CD
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player and npcTarget then
		local tSkillLevel = {
			[175] = 1,
			[176] = 2,
			}
		local scene = player.GetScene()
		local nDirection = GetDirection(player.nX, player.nY, npcTarget.nX, npcTarget.nY)
		local nX = npcTarget.nX - math.cos(nDirection / 128 * math.pi) * nDis
		local nY = npcTarget.nY - math.sin(nDirection / 128 * math.pi) * nDis
		player.PlaySfx(642, player.nX, player.nY, player.nZ + 200, nDirection / 128 * 180 + 90)
		player.PlaySound(476)
		player.SetTimer(36,"scripts/skill/npc/副本BOSS/血战天策/血战天策_保护青骓牧场_重弩散射.lua",npcTarget.dwID,tSkillLevel[scene.dwMapID])
		--player.PlaySfx(639,nX,nY,npcTarget.nZ,nDirection/128 * 180 + 90)
	end
	function OnTimer(player, nParam1, nParam2)
		...
		--nDirection = GetDirection(player.nX, player.nY, npcTarget.nX, npcTarget.nY)
		player.CastSkillXYZ(8592, nParam2, npcTarget.nX, npcTarget.nY, npcTarget.nZ)
		--player.PlaySfx(639,nX,nY,npcTarget.nZ,nDirection/128 * 180 + 90)
	end
	--if player then
		--player.PlaySfx(466, 0, 0, 0)
...
function GetDirection(nX1, nY1, nX2, nY2)
	local nDis = math.sqrt(GetDistanceSq(nX1, nY1, 0, nX2, nY2, 0))
	local nDirection = math.asin((nY2 - nY1) / nDis)
	if nX2 >= nX1 then
		nDirection = (nDirection + 2 * math.pi) /math.pi * 128
	else
		nDirection = (math.pi - nDirection) / math.pi * 128
	end
	nDirection = nDirection % 256
	return nDirection
end
