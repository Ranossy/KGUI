---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_保护青骓牧场_八阵图小怪_遇敌AOE.lua
-- 更新时间:	2013/11/15 1:07:29
-- 更新用户:	liguanghao2
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_保护青骓牧场_八阵图小怪_遇敌AOE.lua",
		0
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--  skill.BindBuff(1, 6707, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSrcID)
	...
	local npcSelf = GetNpc(dwSrcID)
	if npcSelf then
		if npcTarget and (npcTarget.dwTemplateID == 26944 or npcTarget.dwTemplateID == 27364) then
			npcSelf.SetCustomInteger4(5, 1)
			npcSelf.ModifyThreat(npcTarget.dwID, 1)
			npcSelf.SetTarget(TARGET.NPC, npcTarget.dwID)
			npcSelf.TurnToCharacter(npcTarget.dwID)
		elseif npcTarget and (npcTarget.dwTemplateID == 27415 or npcTarget.dwTemplateID == 27469) then
			npcSelf.SetCustomInteger4(5, 2)
			npcSelf.ModifyThreat(npcTarget.dwID, 1)
			npcSelf.SetTarget(TARGET.NPC, npcTarget.dwID)
			npcSelf.TurnToCharacter(npcTarget.dwID)
		elseif npcTarget and (npcTarget.dwTemplateID == 27413 or npcTarget.dwTemplateID == 27461) then
			npcSelf.SetCustomInteger4(5, 3)
			--npcLeilangsuicong.FireAIEvent(2002, 0, 3)
		end
	end
	--[[
	for i = 1, 3 do
		local npcLeilangsuicong = scene.GetNpcByNickName("leilangsuicong" .. i)
		if npcLeilangsuicong then
			if npcTarget and npcTarget.dwTemplateID == 26944 then
				npcLeilangsuicong.SetCustomInteger4(5, 1)
				npcLeilangsuicong.SetTarget(TARGET.NPC, npcTarget.dwID)
				npcLeilangsuicong.TurnToCharacter(npcTarget.dwID)
			elseif npcTarget and npcTarget.dwTemplateID == 27415 then
				npcLeilangsuicong.SetCustomInteger4(5, 2)
				npcLeilangsuicong.SetTarget(TARGET.NPC, npcTarget.dwID)
				npcLeilangsuicong.TurnToCharacter(npcTarget.dwID)
			elseif npcTarget and npcTarget.dwTemplateID == 27413 then
				npcLeilangsuicong.SetCustomInteger4(5, 3)
				--npcLeilangsuicong.FireAIEvent(2002, 0, 3)
			end
		end
	end
	...
	--[[
	if npcTarget and npcTarget.dwTemplateID == 26944 then
		npcBoss.SetCustomInteger4(5, 1)
		npcBoss.SetTarget(TARGET.NPC, npcTarget.dwID)
		npcBoss.TurnToCharacter(npcTarget.dwID)
	elseif npcTarget and npcTarget.dwTemplateID == 27415 then
		npcBoss.SetCustomInteger4(5, 2)
		npcBoss.SetTarget(TARGET.NPC, npcTarget.dwID)
		npcBoss.TurnToCharacter(npcTarget.dwID)
	elseif npcTarget and npcTarget.dwTemplateID == 27413 then
		npcBoss.SetCustomInteger4(5, 3)
		--npcBoss.FireAIEvent(2002, 0, 3)
	end
	--]]
end
