---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_青骓牧场_地煞营_焦油桶布置.lua
-- 更新时间:	2013/10/29 17:18:16
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_青骓牧场_地煞营_焦油桶布置.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if GetDistanceSq(player.nX, player.nY, 0, 39546, 53874, 0) >= (64 * 50) ^ 2 then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(6, 5481), 4)
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 175 then
		scene.CreateNpc(26852, playerthis.nX, playerthis.nY, playerthis.nZ, 0, -1)
	else
		scene.CreateNpc(27312, playerthis.nX, playerthis.nY, playerthis.nZ, 0, -1)
	end
end
