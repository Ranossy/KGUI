...
---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_黑齿元佑_腐骨术.lua
-- 更新时间:	2013/9/9 15:04:42
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
tSkillData = 
{
	{nDamageBase = 4700, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 5600, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_黑齿元佑_腐骨术.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 175 then
		if not player.GetBuff(6771, 1) then
			if player.GetBuff(6770, 1) then
				local buffthis = player.GetBuff(6770, 1)
				if buffthis.nStackNum < 3 then
					player.AddBuff(0, 99, 6770, 1)
				else
					player.DelGroupBuff(6770, 1)
					player.AddBuff(0, 99, 6771, 1)
				end
			else
				player.AddBuff(0, 99, 6770, 1)
			end
		...
		if not player.GetBuff(6771, 2) then
			if player.GetBuff(6770, 2) then
				local buffthis = player.GetBuff(6770, 2)
				if buffthis.nStackNum < 3 then
					player.AddBuff(0, 99, 6770, 2)
				else
					player.DelGroupBuff(6770, 2)
					player.AddBuff(0, 99, 6771, 2)
				end
			else
				player.AddBuff(0, 99, 6770, 2)
			end
