---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/血战天策/血战天策_青骓牧场_地煞营_焦油桶传递.lua
-- 更新时间:	2013/10/22 21:02:30
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_青骓牧场_地煞营_焦油桶传递.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if player.GetItemAmountInAllPackages(5, 20763) == 0 then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(6, 827), 4)
		return false
	end
	...
	local playertarget = GetPlayer(dwTargetID)
	if not playertarget then
		return false
	end
	local nBOX = playertarget.GetFreeRoomSize()
	if nBOX == 0 then
		return false
	end
	if playertarget.GetItemAmountInAllPackages(5, 20763) ~= 0 then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(6, 828), 4)
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local playerthis = GetPlayer(dwSkillSrcID)
	if not playerthis then
		return
	end
	if playerthis.GetItemAmountInAllPackages(5, 20763) == 0 then
		return
	else
		playerthis.CostItemInAllPackage(5, 20763, 1)
		playerthis.SetDynamicSkillGroup(0)
	end
	local nBOX = playertarget.GetFreeRoomSize()
	if nBOX ~= 0 then
		playertarget.AddItem(5, 20763, 1)
		if playertarget.GetDynamicSkillGroup() ~= 84 then
			playertarget.SetDynamicSkillGroup(84)
		end
	end
end
