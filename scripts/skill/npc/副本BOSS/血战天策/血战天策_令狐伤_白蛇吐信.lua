...
tSkillData = 
{
	{nDamageBase = 2800, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 11000, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 4300, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 9300, nDamageRand = 0, nCostMana = 0}, --level 4
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_令狐伤_白蛇吐信.lua",								--属性值1
		0															--属性值2
	);	
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 7169, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 175 then
		npc.AddBuff(0, 99, 7170, 1)
		if player.GetBuff(6927,1) then
			if player.GetBuff(6927, 1).nStackNum < 10 then
				npc.CastSkill(8567, 5, TARGET.PLAYER, dwCharacterID)
			else
				npc.CastSkill(8567,6,TARGET.PLAYER,dwCharacterID)
			end
		end
	elseif scene.dwMapID == 176 then
		npc.AddBuff(0, 99, 7170, 2)
		if player.GetBuff(6927,2) then
			if player.GetBuff(6927, 2).nStackNum < 10 then
				npc.CastSkill(8567, 7, TARGET.PLAYER, dwCharacterID)
			else
				npc.CastSkill(8567,8,TARGET.PLAYER,dwCharacterID)
			end
		end
	elseif scene.dwMapID == 182 then
		npc.AddBuff(0, 99, 7170, 3)
		if player.GetBuff(6927,3) then
			if player.GetBuff(6927, 3).nStackNum < 10 then
				npc.CastSkill(8567, 15, TARGET.PLAYER, dwCharacterID)
			else
				npc.CastSkill(8567,16,TARGET.PLAYER,dwCharacterID)
			end
		end
	else
		npc.AddBuff(0, 99, 7170, 4)
		if player.GetBuff(6927,4) then
			if player.GetBuff(6927, 4).nStackNum < 10 then
				npc.CastSkill(8567, 17, TARGET.PLAYER, dwCharacterID)
			else
				npc.CastSkill(8567,18,TARGET.PLAYER,dwCharacterID)
			end
		end
	end
end
