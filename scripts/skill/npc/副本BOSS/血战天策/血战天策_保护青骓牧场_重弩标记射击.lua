...
tSkillData = 
{
	{nDamageBase = 840000, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 1260000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_保护青骓牧场_重弩标记射击.lua",
		0															--属性值2
	);		
	...
	skill.SetNormalCoolDown(1, 668);	--技能普通CD
	...
	skill.nMaxRadius		= 150 * LENGTH_BASE;		-- 技能施放的最大距离 
	...
	skill.nAreaRadius		= 150 * LENGTH_BASE;		-- 技能作用半径 
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if not player.GetBuff(7231, 1) then
		player.PlaySound(475)
		player.AddBuff(0,99,7231,1,6)
	end
	return nPreResult;
end
...
function Apply(dwCharacterID,dwSrcID)
	...
	if player and npcTarget then
		...
		player.PlaySfx(640, player.nX, player.nY, player.nZ + 200,nDirection/128 * 180 + 90)
		player.PlaySfx(638, npcTarget.nX, npcTarget.nY, npcTarget.nZ, nDirection / 256 * 360 + 90)
		player.PlaySound(474)
	end
	--local scene = player.GetScene()
	--player.CastSkill(8582,tSkillLevel[scene.dwMapID])
end
