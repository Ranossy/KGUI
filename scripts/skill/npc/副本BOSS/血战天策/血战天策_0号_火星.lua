...
tSkillData = 
{
	...
	{nDamageBase = 56000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_0号_火星.lua",								--属性值1
		0															--属性值2
	);	
	----------------- 技能施放Buff需求 ---------------------------------------------
		...
		for i = 1, #playerlist do
			if playerlist[i] ~= dwCharacterID then
				...
				local Ppdistance = GetCharacterDistance(playerlist[i], dwCharacterID)
					if playerTarget and Ppdistance <= 256 then
						if playerTarget.nMoveState ~= MOVE_STATE.ON_DEATH and not playerTarget.GetBuff(6826, 2) then
							playerTarget.AddBuff(0, 99, 6826, 2, 8)
						end
					end
				end
			end
