...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_令狐伤_金蛇盘巢.lua",								--属性值1
		0															--属性值2
	);		
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 182 or scene.dwMapID == 183 then
		nBoss = scene.GetNpcByNickName("linghushang")
	end
	if nBoss then
		if scene.dwMapID == 175 then
			nBoss.AddBuff(0, 99, 7181, 1)
		elseif scene.dwMapID ==176 then
			nBoss.AddBuff(0, 99, 7181, 2)
		elseif scene.dwMapID == 182 then
			nBoss.AddBuff(0, 99, 7181, 3)
		elseif scene.dwMapID == 183 then
			nBoss.AddBuff(0, 99, 7181, 4)
		end
	end
