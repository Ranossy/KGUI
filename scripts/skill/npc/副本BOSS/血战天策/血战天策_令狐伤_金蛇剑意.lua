...
tSkillData = 
{
	...
	{nDamageBase = 25300, nDamageRand = 25300*0.2, nCostMana = 0}, --level 4
...
function GetSkillLevelData(skill)
	...
	--[[skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_令狐伤_金蛇剑意.lua",								--属性值1
		0															--属性值2
	);	--]]
	...
	if dwSkillLevel < 3 then
		...
		skill.nMaxRadius = 100 * LENGTH_BASE;		-- 技能施放的最大距离  秦皇陵需求技能为50尺。
	end
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 175 then
		player.AddBuff(dwCharacterID, npc.nLevel, 6927, 1)
	elseif scene.dwMapID == 176 then
		player.AddBuff(dwCharacterID, npc.nLevel, 6927, 2)
	elseif scene.dwMapID == 182 then
		player.AddBuff(dwCharacterID, npc.nLevel, 6927, 3)
	else
		player.AddBuff(dwCharacterID, npc.nLevel, 6927, 4)
	end
