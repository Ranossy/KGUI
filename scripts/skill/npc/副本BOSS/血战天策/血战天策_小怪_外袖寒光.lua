...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/血战天策/血战天策_小怪_外袖寒光.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID,dwSrcID)
	...
	if target then
		local nTargetDirection = GetLogicDirection(target.nX - npc.nX, target.nY - npc.nY);
		local nNpcDirection = npc.nFaceDirection
		local nDirectionA = math.mod(nTargetDirection + 64 + 256, 256)
		local nDirectionB = math.mod(nTargetDirection-64 + 256, 256)

		if nNpcDirection > nDirectionA or nNpcDirection < nDirectionB then
			target.AddBuff(0,99,7004,1)
		end
	end	
