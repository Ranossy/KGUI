---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/夕颜阁/3号_柳浮云/柳浮云_惊雷.lua
-- 更新时间:	2016/10/10 16:00:14
-- 更新用户:	yangfan14
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 30, nDamageRand = 3, nCostMana = 0}, --level 1
	{nDamageBase = 15900, nDamageRand = 795, nCostMana = 0}, --level 2
	{nDamageBase = 24800, nDamageRand = 1240, nCostMana = 0}, --level 3
...
function GetSkillLevelData(skill)
	...
	if dwSkillLevel == 2 or dwSkillLevel == 3 then
		skill.AddAttribute(
			ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,
			ATTRIBUTE_TYPE.CALL_REPULSED, --击退
			15, -- 击退速度
			0
		)
	end
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/夕颜阁/3号_柳浮云/柳浮云_惊雷.lua", --属性值1
		0															--属性值2
	);
	...
	skill.nAreaRadius = 7.5 * LENGTH_BASE;		-- 技能作用半径
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene then
		local boss = scene.GetNpcByNickName("liufuyun")
		if scene.dwMapID == 256 then
			if  player then
				if player.GetBuff(11412, 1) then
					player.DelBuff(11412, 1)
				end
			end
		end
		if scene.dwMapID == 260 then
			if  player and boss then
				player.AddBuff(boss.dwID, boss.nLevel, 11132, 1)
			end
			local zk = scene.GetNpcByNickName("zongkong_lfy")
			if zk then
