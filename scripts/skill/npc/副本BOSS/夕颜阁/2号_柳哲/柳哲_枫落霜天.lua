---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/夕颜阁/2号_柳哲/柳哲_枫落霜天.lua
-- 更新时间:	2016/10/10 15:56:48
-- 更新用户:	yangfan14
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	...
	{nDamageBase = 20, nDamageRand = 2, nCostMana = 0}, --level 2
	{nDamageBase = 13800, nDamageRand = 345, nCostMana = 0}, --level 3
	{nDamageBase = 13800, nDamageRand = 345, nCostMana = 0}, --level 4
	{nDamageBase = 23100, nDamageRand = 577, nCostMana = 0}, --level 5
	{nDamageBase = 23100, nDamageRand = 577, nCostMana = 0}, --level 6
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/夕颜阁/2号_柳哲/柳哲_枫落霜天.lua", --属性值1
		0															--属性值2
	);
	...
	if dwSkillLevel == 1 or dwSkillLevel == 3 or dwSkillLevel == 5 then
		skill.nAreaRadius = 8 * LENGTH_BASE;		-- 技能作用半径
	end
	if dwSkillLevel == 2 or dwSkillLevel == 4 or dwSkillLevel == 6 then
		skill.nAreaRadius = 9 * LENGTH_BASE;		-- 技能作用半径
	end
	...
	if scene then
		...
		if scene.dwMapID == 257 then
			if  player and boss then
				player.AddBuff(boss.dwID, boss.nLevel, 11091, 2)
			end
		end
		if scene.dwMapID == 260 then
			if  player and boss then
				player.AddBuff(boss.dwID, boss.nLevel, 11091, 3)
				local zk = scene.GetNpcByNickName("zongkong_lz")
				if zk then
					zk.SetCustomInteger1(20, 0)
				end
			end
		end
	end
