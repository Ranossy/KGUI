---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/夕颜阁/2号_柳哲/柳哲_流枫刀意.lua
-- 更新时间:	2016/10/17 0:10:02
-- 更新用户:	yangfan14
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 18, nDamageRand = 2, nCostMana = 0}, --level 1
	{nDamageBase = 8800, nDamageRand = 440, nCostMana = 0}, --level 2
	{nDamageBase = 13200, nDamageRand = 660, nCostMana = 0}, --level 3
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/夕颜阁/2号_柳哲/柳哲_流枫刀意.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if  player then
		player.PlaySfx(1508, player.nX, player.nY, player.nZ + 500)
	end
	...
	if scene then
		...
		if scene.dwMapID == 257 then
			if  player then
				player.DelBuff(11413, 1)
			end
		end
		if scene.dwMapID == 260 then
			if  player and boss then
				player.AddBuff(boss.dwID, boss.nLevel, 11092, 1)
			end
		end
	end
end
