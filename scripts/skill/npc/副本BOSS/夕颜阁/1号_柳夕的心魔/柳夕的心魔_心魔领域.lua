---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/夕颜阁/1号_柳夕的心魔/柳夕的心魔_心魔领域.lua
-- 更新时间:	2016/10/11 11:40:09
-- 更新用户:	yangfan14
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 40, nDamageRand = 4, nCostMana = 0}, --level 1
	{nDamageBase = 2300, nDamageRand = 57, nCostMana = 0}, --level 2
	{nDamageBase = 3300, nDamageRand = 82, nCostMana = 0}, --level 3
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/夕颜阁/1号_柳夕的心魔/柳夕的心魔_心魔领域.lua", --属性值1
		0															--属性值2
	);
	...
	if dwSkillLevel == 3 then
		skill.BindBuff(1, 11409, 1);		-- 设置1号位Buff
	end
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene then
		if scene.dwMapID == 260 then
			if  player and  player.GetBuff(11414, 1) then
				player.DelBuff(11414, 1)
			end
		end
	end
end
