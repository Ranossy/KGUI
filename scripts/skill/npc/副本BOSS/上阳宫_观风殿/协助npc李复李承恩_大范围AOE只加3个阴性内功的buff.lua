---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩_大范围AOE只加3个阴性内功的buff.lua
-- 更新时间:	2016/5/19 15:25:21
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩_大范围AOE只加3个阴性内功的buff.lua", --属性值1
		0															--属性值2
	);
	...
	skill.nTargetCountLimit = 125;				-- 技能作用目标数量限制,(小于0 代表目标数量不限制)
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player then
		if player.GetKungfuMount() ~= nil then
			if player.GetKungfuMount().dwSkillID == 10081 or player.GetKungfuMount().dwSkillID == 10447 or player.GetKungfuMount().dwSkillID == 10242 then---冰心，莫问，焚影
				local scene = player.GetScene()
				local jihuomubiaokongzhi = scene.GetNpcByNickName("jihuomubiaokongzhi")
				if jihuomubiaokongzhi.GetCustomInteger4(16) < 3 then
					player.AddBuff(npc.dwID, npc.nLevel, 10691, 1, 15)
					local step = jihuomubiaokongzhi.GetCustomInteger4(16)
					step = step + 1
					jihuomubiaokongzhi.SetCustomInteger4(16, step)
				end
			end
		end
	end
