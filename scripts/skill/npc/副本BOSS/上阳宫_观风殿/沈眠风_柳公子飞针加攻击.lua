---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_柳公子飞针加攻击.lua
-- 更新时间:	2016/7/6 16:00:23
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 1300, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_柳公子飞针加攻击.lua", -- 属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSrcID)
	...
	scene.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_柳公子飞针加攻击.lua", dwSrcID, dwCharacterID)
end
...
function OnTimer(scene, nParam1, nParam2)
	...
	if scene.dwMapID == 240 or scene.dwMapID == 250 then
		npctarget.AddBuff(npctarget.dwID, npctarget.nLevel, 10949, 1, 30)
	end

	if scene.dwMapID == 248 then
		npctarget.AddBuff(npctarget.dwID, npctarget.nLevel, 10949, 2, 60)
	end
end
