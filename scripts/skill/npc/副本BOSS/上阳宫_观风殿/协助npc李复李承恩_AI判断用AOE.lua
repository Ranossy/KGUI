---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩_AI判断用AOE.lua
-- 更新时间:	2016/5/16 10:54:54
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩_AI判断用AOE.lua", --属性值1
		0															--属性值2
	);
	...
	--skill.BindBuff(1, 8834, 1);		-- 设置1号位Buff
	--elseif dwSkillLevel == 2 then
	--skill.BindBuff(1, 8834, 2)
	--end
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player then
		if player.GetKungfuMount() ~= nil then
			if player.GetKungfuMount().dwSkillID == 10014 or player.GetKungfuMount().dwSkillID == 10021 then---花间和气纯
				if npc then
					local step = npc.GetCustomInteger2(51)
					step = step + 1
					npc.SetCustomInteger2(51, step)
				end
			end

			--if player.GetKungfuMount().dwSkillID == 10062 or player.GetKungfuMount().dwSkillID == 10389 or player.GetKungfuMount().dwSkillID == 10002 or player.GetKungfuMount().dwSkillID == 10243 then---天策，苍云，少林，明教
			if player.GetKungfuMount().dwSkillID == 10175 or player.GetKungfuMount().dwSkillID == 10225 then--毒经，天罗
				if npc then
					local step = npc.GetCustomInteger2(59)
					step = step + 1
					npc.SetCustomInteger2(59, step)
				end
			end
		end
	end
