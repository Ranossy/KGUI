---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_碎骨.lua
-- 更新时间:	2016/4/8 15:01:44
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 52800, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 81000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_碎骨.lua", -- 属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 240 or scene.dwMapID == 250 then
		if playertarget then
			...
			playertarget.AddBuff(npc.dwID, npc.nLevel, 10600, 1, 20)
			playertarget.BeHitted(playertarget.dwID, 5265, 1, 30)
			--scene.SetTimer(1 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_碎骨.lua", playertarget.dwID, 1)
			scene.SetTimer(15, "scripts/skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_碎骨.lua", playertarget.dwID, dwSkillSrcID)
		end

		if npctarget then
			npctarget.AddBuff(npc.dwID, npc.nLevel, 10600, 1, 20)
		end
	end
	...
	if scene.dwMapID == 248 then
		if playertarget then
			--local scene = playertarget.GetScene()
			playertarget.AddBuff(npc.dwID, npc.nLevel, 10600, 2, 60)
			playertarget.BeHitted(playertarget.dwID, 5265, 1, 30)
			--scene.SetTimer(1 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_碎骨.lua", playertarget.dwID, 1)
			scene.SetTimer(15, "scripts/skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_碎骨.lua", playertarget.dwID, dwSkillSrcID)
		end
		...
		if npctarget then
			npctarget.AddBuff(npc.dwID, npc.nLevel, 10600, 2, 60)
		end
	end
end
...
function OnTimer(scene, nParam1, nParam2)
	...
	if scene.dwMapID == 240 or scene.dwMapID == 250 then
		playertarget.AddBuff(npc.dwID, npc.nLevel, 10616, 1, 3)
	end

	if scene.dwMapID == 248 then
		playertarget.AddBuff(npc.dwID, npc.nLevel, 10616, 2, 3)
	end
end
