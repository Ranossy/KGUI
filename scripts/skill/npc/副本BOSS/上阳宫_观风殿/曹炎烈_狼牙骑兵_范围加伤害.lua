---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/曹炎烈_狼牙骑兵_范围加伤害.lua
-- 更新时间:	2016/7/21 9:31:52
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 44010, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 108990, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/曹炎烈_狼牙骑兵_范围加伤害.lua", --属性值1
		0															--属性值2
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7921, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 240 or scene.dwMapID == 250 then
		if npc then
			npc.AddBuff(npc.dwID, npc.nLevel, 11002, 1, 10)
		end

		if npctarget then
			if npctarget.bFightState == true then
				npctarget.AddBuff(npc.dwID, npc.nLevel, 11002, 1, 10)
			end
		end
	end
	...
	if scene.dwMapID == 248 then
		if npc then
			npc.AddBuff(npc.dwID, npc.nLevel, 11002, 2, 10)
		end

		if npctarget then
			if npctarget.bFightState == true then
				npctarget.AddBuff(npc.dwID, npc.nLevel, 11002, 2, 10)
			end
		end
	end
	...
	--[[if npc then
		local scene = npc.GetScene()
		scene.SetTimer(1 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/春明门/春明门_校尉_范围加攻击.lua", dwSrcID, 1)
	end--]]
end

--魔法属性反应用时的执行函数,dwCharacterID是魔法属性作用的目标ID
function UnApply(dwCharacterID, dwSrcID)
