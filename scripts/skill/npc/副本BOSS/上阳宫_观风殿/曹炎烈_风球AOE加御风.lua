---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/曹炎烈_风球AOE加御风.lua
-- 更新时间:	2016/9/7 14:16:04
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/曹炎烈_风球AOE加御风.lua", -- 属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(11170, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc.dwTemplateID == 51102 then
		player.AddBuff(npc.dwID, npc.nLevel, 10572, 1, 35)
		player.AddBuff(npc.dwID, npc.nLevel, 11170, 1, 2)
		player.DelBuff(10554, 1)
		if npc.nCurrentLife <= npc.nMaxLife * 0.05 then
			player.ClearCDTime(204)
			npc.PlaySfx(1461, npc.nX, npc.nY, npc.nZ)
			npc.FireAIEvent(2001, 0, 0)
		else
			npc.nCurrentLife = npc.nCurrentLife - npc.nMaxLife * 0.05
			player.ClearCDTime(204)
			npc.PlaySfx(1461, npc.nX, npc.nY, npc.nZ)
		end
	end

	if npc.dwTemplateID == 54319 then
		player.AddBuff(npc.dwID, npc.nLevel, 10572, 1, 35)
		player.AddBuff(npc.dwID, npc.nLevel, 11170, 1, 2)
		player.DelBuff(10554, 1)
		if npc.nCurrentLife <= npc.nMaxLife * 0.05 then
	...
	if npc.dwTemplateID == 53832 then
		player.AddBuff(npc.dwID, npc.nLevel, 10572, 2, 30)
		player.AddBuff(npc.dwID, npc.nLevel, 11170, 2, 2)
		player.DelBuff(10554, 2)
		if npc.nCurrentLife <= npc.nMaxLife * 0.05 then
			player.ClearCDTime(204)
			npc.PlaySfx(1461, npc.nX, npc.nY, npc.nZ)
			npc.FireAIEvent(2001, 0, 0)
	...
	if npc.dwTemplateID == 53896 then
		npc.PlaySfx(1461, npc.nX, npc.nY, npc.nZ)
		player.AddBuff(npc.dwID, npc.nLevel, 10956, 1)
