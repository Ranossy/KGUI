...
tSkillData =
{
	{nDamageBase = 8000, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/独孤先生/对远程AOE.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local beHitPlayer = GetPlayer(dwCharacterID)

	if beHitPlayer and srcNpc then
		local commonData = DuguData[beHitPlayer.GetMapID()]
		local debuffID = 0
		local debuffLevel = 0
		debuffID = commonData.SuoZuBuffID
		debuffLevel = commonData.SuoZuBuffLevel
		if debuffID ~= 0 and debuffLevel ~= 0 then
			beHitPlayer.AddBuff(srcNpc.dwID, srcNpc.nLevel, debuffID, debuffLevel)
		end
	end
end
