---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/1号关卡/毒虫_随机注入死亡毒液母技能.lua
-- 更新时间:	2016/9/28 19:32:30
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/1号关卡/毒虫_随机注入死亡毒液母技能.lua",
		0
	);
	--[[
   	skill.AddAttribute(
	...
	if scene then
		if scene.dwMapID == 240 or scene.dwMapID == 250 then
			if nRan <= 800 then
				if npcDu then
					if playerTar then
						playerTar.AddBuff(npcDu.dwID, npcDu.nLevel, 11073, 1)
					end
				end
			end
		end
