---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/1号关卡/毒虫_随机DOT母技能.lua
-- 更新时间:	2016/9/28 19:23:06
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/1号关卡/毒虫_随机DOT母技能.lua",
		0
	);
	--[[
   	skill.AddAttribute(
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 11100, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--print(dwCharacterID)
	--print(dwSkillSrcID)
	local nRan = Random(25000)
	local npcDu = GetNpc(dwSkillSrcID)
	...
	if scene then
		...
		if nRan <= 3000 then
			if npcDu then
				if player then
					player.AddBuff(npcDu.dwID, npcDu.nLevel, 11074, 1)
				end
			end
		end
		end
		...
		if nRan <= 5000 then
			if npcDu then
				if player then
					player.AddBuff(npcDu.dwID, npcDu.nLevel, 11074, 2)
				end
			end
		end
		end
