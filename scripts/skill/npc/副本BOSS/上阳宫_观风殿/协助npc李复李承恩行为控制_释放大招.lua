---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩行为控制_释放大招.lua
-- 更新时间:	2016/5/12 15:21:58
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩行为控制_释放大招.lua", -- 属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	skill.SetNormalCoolDown(1, 1108)
	...
	if player then
		...
		if LiChengEn then
			if LiChengEn.GetCustomBoolean(1) == true then
				LiChengEn.SetCustomBoolean(16, true)
			end
		end
	
		if LiFu then
			if LiFu.GetCustomBoolean(1) == true then
				LiFu.SetCustomBoolean(16, true)
			end
		end
	end
end
