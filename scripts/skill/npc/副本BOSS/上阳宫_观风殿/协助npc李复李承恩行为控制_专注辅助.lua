---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩行为控制_专注辅助.lua
-- 更新时间:	2016/5/12 11:02:17
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩行为控制_专注辅助.lua", -- 属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	skill.SetNormalCoolDown(1, 1103)
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		if LiChengEn and LiChengEn.GetCustomBoolean(1) == true then
			LiChengEn.AddBuff(player.dwID, player.nLevel, 10729, 1)
			LiChengEn.DelBuff(10727, 1)
			LiChengEn.DelBuff(10728, 1)
			LiChengEn.PlaySfx(453, LiChengEn.nX, LiChengEn.nY, LiChengEn.nZ)
			if LiChengEn.nMoveState == MOVE_STATE.ON_STAND then
				LiChengEn.DoAction(0, 10060)
			end
		end

		if LiFu and LiFu.GetCustomBoolean(1) == true then
			LiFu.AddBuff(player.dwID, player.nLevel, 10729, 1)
			LiFu.DelBuff(10727, 1)
			LiFu.DelBuff(10728, 1)
			LiFu.PlaySfx(453, LiFu.nX, LiFu.nY, LiFu.nZ)
			if LiFu.nMoveState == MOVE_STATE.ON_STAND then
				LiFu.DoAction(0, 10060)
			end
		end
	end
end
