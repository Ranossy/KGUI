---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_烈焰焚身母技能.lua
-- 更新时间:	2016/10/7 9:16:57
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
 ...
 - 1024,
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_烈焰焚身母技能.lua", -- 属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		if scene.dwMapID == 240 or scene.dwMapID == 248 then
			if not player.GetBuff(11260, 1) then
				player.AddBuff(player.dwID, player.nLevel, 11260, 1)
			end
		end--]]

		if player.GetBuff(11386, 1) then
			if npc then
				player.AddBuff(npc.dwID, npc.nLevel, 11386, 1)
				npc.CastSkill(16565, 3, TARGET.PLAYER, player.dwID)
			end
		end

		if not player.GetBuff(11386, 1) then
			if npc then
				npc.CastSkill(16565, 2, TARGET.PLAYER, player.dwID)
				player.AddBuff(npc.dwID, npc.nLevel, 11386, 1)
			end
		end
	end
