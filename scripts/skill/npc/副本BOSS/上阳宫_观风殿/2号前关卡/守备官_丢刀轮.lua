---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/2号前关卡/守备官_丢刀轮.lua
-- 更新时间:	2016/9/27 20:16:27
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/2号前关卡/守备官_丢刀轮.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local npc1 = GetNpc(dwCharacterID)
	if not npc1 then
		return
	end
	...
	if #playerlist ~= 0 then
		for i = 1, #playerlist do
			...
			if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH  then
				if playertarget.GetBuff(10993, 1) then
					playertarget.DelBuff(10993, 1)
				end
			end
		end
	end
	local MapID = scene.dwMapID
	local npcHZDL = scene.GetNpcByNickName("erhaoguanqiadaolun")
	if not npcHZDL then
		if MapID == 240 then  --10PT
			scene.CreateNpc(51958, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, 100, "erhaoguanqiadaolun")
		else
			scene.CreateNpc(53908, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, 100, "erhaoguanqiadaolun")
		end
	end
end
