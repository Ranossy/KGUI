---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/2号前关卡/刺客_普攻母_单体.lua
-- 更新时间:	2016/9/26 20:36:32
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
Include("scripts/Map/副本通用接口/include/副本通用数据.lua")
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/2号前关卡/刺客_普攻母_单体.lua", --属性值1
		0															--属性值2
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 8084, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local MapID = scene.dwMapID
	local playerlist = scene.GetAllPlayer()
	if #playerlist == 0 then
		return
	end
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		if IsUseMana(player) then
			player.nCurrentMana = player.nCurrentMana - player.nMaxMana * 2 / 100
		end
		if IsUseRage(player) then
			player.nCurrentRage = player.nCurrentRage - player.nMaxRage * 2 / 100
		end
		if IsUseEnergy(player) then
			player.nCurrentEnergy = player.nCurrentEnergy - player.nMaxEnergy * 2 / 100
		end
		if IsUseSMEnergy(player) then
			player.nCurrentSunEnergy = player.nCurrentSunEnergy - player.nMaxSunEnergy * 2 / 100
			player.nCurrentMoonEnergy = player.nCurrentMoonEnergy - player.nMaxMoonEnergy * 2 / 100
		end
		player.PlaySfx(1396, player.nX, player.nY, player.nZ + 500)
		if MapID == 240 then
			npc.CastSkill(16297, 1, TARGET.PLAYER, player.dwID)
			local SZ = npc.szName
			local strFireName = string.sub(SZ, - 1, - 1)
			local nJS = Random(10000)
			if nJS < 1000 then
				local npcEHGQXY = scene.GetNpcByNickName("erhaoguanqiaxuying" .. strFireName)
				if not npcEHGQXY then
					npcEHGQXY = scene.CreateNpc(51952, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, 320, "erhaoguanqiaxuying" .. strFireName)
					npcEHGQXY.AddBuff(npcEHGQXY.dwID, npcEHGQXY.nLevel, 10986, 1, 3600)
					npcEHGQXY.AddBuff(npcEHGQXY.dwID, npcEHGQXY.nLevel, 10991, 1, 3600)
					npcEHGQXY.PlaySfx(142, 0, 0, 0)
					for i = 1, #playerlist do
						local playertarget = GetPlayer(playerlist[i])
						if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH then
							if npcEHGQXY then
								npcEHGQXY.ModifyThreat(playertarget.dwID, 0)
							end
						end
					end
				end
		...
		if MapID == 250 then	--帮会
			...
			if nJS < 1000 then
				local npcEHGQXY = scene.GetNpcByNickName("erhaoguanqiaxuying" .. strFireName)
				if not npcEHGQXY then
					npcEHGQXY = scene.CreateNpc(54389, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, 320, "erhaoguanqiaxuying" .. strFireName)
					npcEHGQXY.AddBuff(npcEHGQXY.dwID, npcEHGQXY.nLevel, 10986, 1, 3600)
					npcEHGQXY.AddBuff(npcEHGQXY.dwID, npcEHGQXY.nLevel, 10991, 1, 3600)
					npcEHGQXY.PlaySfx(142, 0, 0, 0)
					for i = 1, #playerlist do
		...
		if MapID == 248 then	--YX
			npc.CastSkill(16297, 2, TARGET.PLAYER, player.dwID)
			local SZ = npc.szName
			local strFireName = string.sub(SZ, - 1, - 1)
			local nJS = Random(10000)
			if nJS < 1000 then
				local npcEHGQXY = scene.GetNpcByNickName("erhaoguanqiaxuying" .. strFireName)
				if not npcEHGQXY then
					npcEHGQXY = scene.CreateNpc(53902, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, 320, "erhaoguanqiaxuying" .. strFireName)
					npcEHGQXY.AddBuff(npcEHGQXY.dwID, npcEHGQXY.nLevel, 10986, 1, 3600)
					npcEHGQXY.AddBuff(npcEHGQXY.dwID, npcEHGQXY.nLevel, 10991, 1, 3600)
					npcEHGQXY.PlaySfx(142, 0, 0, 0)
					for i = 1, #playerlist do
