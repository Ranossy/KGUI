---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/2号前关卡/弓箭手_普攻_单体.lua
-- 更新时间:	2016/8/16 17:59:32
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 585, nDamageRand = 50, nCostMana = 0}, --level 1
	{nDamageBase = 980, nDamageRand = 50, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/2号前关卡/弓箭手_普攻_单体.lua", --属性值1
		0															--属性值2
	);
	...
	skill.nMaxRadius = 9000 * LENGTH_BASE;		-- 技能施放的最大距离
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if MapID == 240 or MapID == 250 then  --10PT
		player.AddBuff(player.dwID, player.nLevel, 10988, 1)
		local nBuff = player.GetBuff(10988, 1)
		if nBuff and nBuff.nStackNum >= 4 then
			npc.CastSkillXYZ(16285, 1, player.nX, player.nY, player.nZ)
			player.PlaySfx(1404, player.nX, player.nY, player.nZ + 500)
			player.DelGroupBuff(10988, 1)
		end
	end
	if MapID == 248 then	--YX
		player.AddBuff(player.dwID, player.nLevel, 10988, 2)
		local nBuff = player.GetBuff(10988, 2)
		if nBuff and nBuff.nStackNum >= 3 then
			npc.CastSkillXYZ(16285, 2, player.nX, player.nY, player.nZ)
			player.PlaySfx(1404, player.nX, player.nY, player.nZ + 500)
			player.DelGroupBuff(10988, 2)
		end
	end
end
