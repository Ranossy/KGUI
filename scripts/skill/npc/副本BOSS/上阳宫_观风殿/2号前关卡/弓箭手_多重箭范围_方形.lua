---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/2号前关卡/弓箭手_多重箭范围_方形.lua
-- 更新时间:	2016/7/20 15:11:34
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/2号前关卡/弓箭手_多重箭范围_方形.lua", --属性值1
		0															--属性值2
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 8535, 1);		-- 设置1号位Buff
	...
	skill.nAreaRadius = 64 * LENGTH_BASE;		-- 技能作用半径
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcGJS = scene.GetNpcByNickName("erhaoguanqiagongjian" .. strFireName)
	if nSMapId == 240 or nSMapId == 250 then
		if npcGJS then
			npcGJS.CastSkill(16287, 1, TARGET.PLAYER, player.dwID)
		else
		end
	end
	if nSMapId == 248 then
		if npcGJS then
			npcGJS.CastSkill(16287, 2, TARGET.PLAYER, player.dwID)
		else
		end
	end
end
