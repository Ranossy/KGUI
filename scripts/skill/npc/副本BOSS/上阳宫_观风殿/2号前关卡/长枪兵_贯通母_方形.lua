---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/2号前关卡/长枪兵_贯通母_方形.lua
-- 更新时间:	2016/9/26 20:39:06
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/2号前关卡/长枪兵_贯通母_方形.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if MapID == 240 or MapID == 250 then  --10PT
		player.AddBuff(npc.dwID, npc.nLevel, 10990, 1)
		local nBuff = player.GetBuff(10990, 1)
		if nBuff  then
			local nCS = nBuff.nStackNum
			local nJS = Random(10000)
			if nJS <= nCS * 1000 then
				npc.CastSkill(16295, 1, TARGET.PLAYER, player.dwID)
			else
				npc.CastSkill(16294, 1, TARGET.PLAYER, player.dwID)
			end
		else
			npc.CastSkill(16294, 1, TARGET.PLAYER, player.dwID)
		end
	...
	if MapID == 248 then	--YX
		player.AddBuff(npc.dwID, npc.nLevel, 10990, 2)
		local nBuff = player.GetBuff(10990, 2)
		if nBuff  then
			local nCS = nBuff.nStackNum
			local nJS = Random(10000)
			if nJS <= nCS * 1000 then
				npc.CastSkill(16295, 2, TARGET.PLAYER, player.dwID)
			else
				npc.CastSkill(16294, 2, TARGET.PLAYER, player.dwID)
			end
		else
			npc.CastSkill(16294, 2, TARGET.PLAYER, player.dwID)
		end
