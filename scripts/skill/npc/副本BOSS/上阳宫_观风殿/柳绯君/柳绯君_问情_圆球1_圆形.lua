---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/柳绯君/柳绯君_问情_圆球1_圆形.lua
-- 更新时间:	2016/9/27 19:34:31
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 5600, nDamageRand = 200, nCostMana = 0}, --level 1
	{nDamageBase = 7000, nDamageRand = 200, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/柳绯君/柳绯君_问情_圆球1_圆形.lua", --属性值1
		0															--属性值2
	);
	...
	skill.AddSlowCheckDestBuff(10641, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 10641, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.AddBuff(npc.dwID, npc.nLevel, 10645, 1)
	local nBuff = player.GetBuff(10645, 1)
	if nBuff and nBuff.nStackNum >= 4 then
		player.CustomDamage(npc.dwID, 15643, 1, 0, player.nMaxLife * 2)
	end
	local nJS = 1
	local nSMapID = scene.dwMapID
	if nSMapID == 240 or nSMapID == 250 then
		nJS = 1.8
	else
	...
	npc.nCurrentLife = npc.nCurrentLife - npc.nMaxLife / nJS
end
