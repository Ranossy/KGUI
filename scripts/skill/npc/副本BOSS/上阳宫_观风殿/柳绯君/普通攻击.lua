---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/柳绯君/普通攻击.lua
-- 更新时间:	2016/10/4 11:34:23
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 6860, nDamageRand = 500, nCostMana = 0}, --level 1
	{nDamageBase = 35200, nDamageRand = 800, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/柳绯君/普通攻击.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcLFJ = scene.GetNpcByNickName("liufeijun")
	if npcLFJ then
		if MapID == 240 or MapID == 250 then
			if npcLFJ.nCurrentLife / npcLFJ.nMaxLife >= 0.5 then
				player.AddBuff(player.dwID, player.nLevel, 10631, 1)
			else
				player.AddBuff(player.dwID, player.nLevel, 10632, 1)
			end
		else
			if npcLFJ.nCurrentLife / npcLFJ.nMaxLife >= 0.5 then
				player.AddBuff(player.dwID, player.nLevel, 10631, 2)
			else
				player.AddBuff(player.dwID, player.nLevel, 10632, 2)
			end
		end
	end
