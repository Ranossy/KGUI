---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/柳绯君/BUFF/DOT整体.lua
-- 更新时间:	2016/10/4 16:12:34
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		local tBuff = player.GetBuff(10635, 1)
		if not tBuff then
			player.AddBuff(player.dwID, player.nLevel, 10635, 1, 1)
		end
	end
end
...
function UnApply(dwCharacterID)
	...
	if player then
		...
		local npcEHFW = scene.GetNpcByNickName("erhaofanwei")
		if not npcEHFW then
			return
		end
		local tBuff = player.GetBuff(10635, 1)
		if tBuff then
			player.DelGroupBuff(10635, 1)
			local playerlist = scene.GetAllPlayer()
			if #playerlist ~= 0 then
				for i = 1, #playerlist do
					local playertarget = GetPlayer(playerlist[i])
					if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH and playertarget.dwID ~= player.dwID then
						local pbdistance = GetCharacterDistance(playertarget.dwID, player.dwID)
						if (playertarget.nX - player.nX) * (playertarget.nX - player.nX) + (playertarget.nY - player.nY) * (playertarget.nY - player.nY) <= 512 * 512 then
							npcEHFW.SetCustomInteger1(7, 1)
							if scene.dwMapID == 240 or scene.dwMapID == 250 then
								playertarget.AddBuff(npcLFJ.dwID, npcLFJ.nLevel, 10634, 1, 10)
							else
								playertarget.AddBuff(npcLFJ.dwID, npcLFJ.nLevel, 10634, 2, 10)
							end
							
						end
						if pbdistance < 512 then
