---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/柳绯君/BUFF/DOT每跳.lua
-- 更新时间:	2016/9/27 19:26:13
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		if npcLFJ then
			player.PlaySfx(1544, 0, 0, 0)
			if npcLFJ.nCurrentLife / npcLFJ.nMaxLife >= 0.5 then
				npcLFJ.CastSkill(15655, 1, TARGET.PLAYER, player.dwID)
				player.Stop()
			else
				if MapID == 240 or MapID == 250 then
					npcLFJ.nCurrentLife = npcLFJ.nCurrentLife + npcLFJ.nMaxLife * 0.001
				else
					npcLFJ.nCurrentLife = npcLFJ.nCurrentLife + npcLFJ.nMaxLife * 0.001
				end
			end
