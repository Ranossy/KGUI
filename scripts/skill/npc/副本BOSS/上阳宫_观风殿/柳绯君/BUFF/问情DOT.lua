---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/柳绯君/BUFF/问情DOT.lua
-- 更新时间:	2016/8/29 9:43:12
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcEHZL = scene.GetNpcByNickName("erhaozhanglin")
	if npc and npcEHZL and npc.dwID == npcEHZL.dwID then
		return
	end
	...
	if player then
		...
		if npcLFJ then
			...
			if nBuff then
				...
				--player.CustomDamage(npc.dwID, 15643, 1, 0, player.nMaxLife * nCS * 0.001)
			end
		end
	end
end
