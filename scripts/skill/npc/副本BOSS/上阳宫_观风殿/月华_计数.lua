---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/月华_分摊AOE_计数技能.lua
-- 更新时间:	2016/3/29 15:45:23
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/月华_计数.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if playet then
		...
		if scene then
			...
			--if dwMapID == 12345 then
			local yuehua = scene.GetNpcByNickName("yuehua")
			--local jishunpc = scene.GetNpcByNickName("jishuNPC" .. 1)
			if yuehua then
				local i = yuehua.GetCustomInteger1(3)
				local t = i + 1
				yuehua.SetCustomInteger1(3, t)
			--	print(yuehua.GetCustomInteger1(3))
				...
				--	if jishunpc then
				--		i = i + 1
				--		jishunpc.SetCustomInteger1(1, i)
				--	end
				--end
				--end
			end
