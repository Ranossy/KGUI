---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩行为控制_切换角色.lua
-- 更新时间:	2016/5/12 14:52:57
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩行为控制_切换角色.lua", -- 属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	skill.SetNormalCoolDown(1, 1106)
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		if LiFu and LiChengEn then
			if LiFu.GetCustomBoolean(1) == false and LiChengEn.GetCustomBoolean(1) == false then
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(10, 3474), 3)
				player.ClearCDTime(1035)
				return
			end
		end

		if LiFu and LiChengEn then
			if LiFu.bFightState == true or LiChengEn.bFightState == true then
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(10, 4749), 3)
				player.ClearCDTime(1035)
				return
			end
		end
		...
		if LiFu and LiFu.bFightState == false then
			if LiFu.GetCustomBoolean(1) == true then
				LiFu.DelBuff(11345, 1)--独挡一面
				LiFu.DelBuff(11034, 1)
				for i = 1, #playerlist do
					local playertarget = GetPlayer(playerlist[i])
					if playertarget then
						playertarget.DelBuff(11076, 1)
					end
				end
				if LiChengEn then
					if scene.dwMapID == 240 then
						if player.IsAchievementAcquired(5536) == true then---玩家有独当一面成就
							LiChengEn.AddBuff(LiChengEn.dwID, LiChengEn.nLevel, 11345, 1)
						end
					end
					player.CastSkill(16356, 1, TARGET.NPC, LiChengEn.dwID)
				end
			end
		end
		
		if LiChengEn and LiChengEn.bFightState == false then
			if LiChengEn.GetCustomBoolean(1) == true then
				LiChengEn.DelBuff(11345, 1)--独挡一面
				...
				if LiFu then
					if scene.dwMapID == 240 then
						if player.IsAchievementAcquired(5536) == true then---玩家有独当一面成就
							LiFu.AddBuff(LiFu.dwID, LiFu.nLevel, 11345, 1)
						end
					end
					player.CastSkill(16356, 1, TARGET.NPC, LiFu.dwID)
				end
			end
		end
		...
		if LiChengEn and LiChengEn.bFightState == false then
			if LiChengEn.GetCustomBoolean(1) == true then
				LiChengEn.FireAIEvent(2003, 0, 1)
				LiChengEn.SetCustomBoolean(1, false)
			else
				LiChengEn.FireAIEvent(2012, 0, 1)
			end
				
			--if LiChengEn.GetCustomBoolean(1) == false then
			--LiChengEn.FireAIEvent(2012, 0, 1)
			--end
		...
		if LiFu and LiFu.bFightState == false then
			if LiFu.GetCustomBoolean(1) == true then
				LiFu.SetCustomBoolean(1, false)
				LiFu.FireAIEvent(2003, 0, 1)
				--elseif LiFu.GetCustomBoolean(1) == false then
			else
				LiFu.FireAIEvent(2012, 0, 1)
			end
		end
	end
end
