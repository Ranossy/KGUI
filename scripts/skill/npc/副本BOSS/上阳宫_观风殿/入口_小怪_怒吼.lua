---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/入口_小怪_怒吼.lua
-- 更新时间:	2016/8/3 15:13:55
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/入口_小怪_怒吼.lua", -- 属性值1
		0															--属性值2
	);
	...
	--skill.nChannelFrame = 16;	 			-- 通道技持续时间，单位帧数
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 240 or scene.dwMapID == 250 then
		if playertarget then
			playertarget.AddBuff(npc.dwID, npc.nLevel, 11030, 1, 8)
		end

		local npctarget = GetNpc(dwCharacterID)
		if npctarget then
			npctarget.AddBuff(npc.dwID, npc.nLevel, 11030, 1, 8)
		end
	end

	if scene.dwMapID == 248 then
		if playertarget then
			playertarget.AddBuff(npc.dwID, npc.nLevel, 11030, 2, 8)
		end

		local npctarget = GetNpc(dwCharacterID)
		if npctarget then
			npctarget.AddBuff(npc.dwID, npc.nLevel, 11030, 2, 8)
		end
	end
end
