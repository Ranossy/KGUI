---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复_贪功好利_大型爆炸.lua
-- 更新时间:	2016/4/21 14:40:58
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复_贪功好利_大型爆炸.lua", -- 属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npctarget then
		if npctarget.GetBuff(10696, 1) then
			local num = npctarget.GetBuff(10696, 1).nStackNum
			if npc.nCurrentLife > 10000 * 0.2 * num then
				npc.nCurrentLife = npc.nCurrentLife - 10000 * 0.2 * num
			elseif npc.nCurrentLife <= 10000 * 0.2 * num then
				npc.Die()
			end
		end
	end--]]
	local npctarget = GetNpc(dwCharacterID)
	if npctarget then
		npctarget.AddBuff(npctarget.dwID, npctarget.nLevel, 11496, 1, 15)
	end
end
