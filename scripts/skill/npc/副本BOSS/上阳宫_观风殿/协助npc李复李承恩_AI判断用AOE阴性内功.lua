---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩_AI判断用AOE阴性内功.lua
-- 更新时间:	2016/5/19 15:20:34
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩_AI判断用AOE阴性内功.lua", --属性值1
		0															--属性值2
	);
	...
	if player then
		if player.GetKungfuMount() ~= nil then
			if player.GetKungfuMount().dwSkillID == 10081 or player.GetKungfuMount().dwSkillID == 10447 or player.GetKungfuMount().dwSkillID == 10242 then---冰心，莫问，焚影
				if npc then
					local step = npc.GetCustomInteger2(55)
					step = step + 1
					npc.SetCustomInteger2(55, step)
				end
			end
		end
	end
