---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/月华_分身_影打.lua
-- 更新时间:	2016/11/7 15:25:06
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/月华_分身_影打.lua",
		0
	);
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,
...
function Apply(dwCharacterID, dwSkillSrcID)
	local playertar = GetPlayer(dwCharacterID)
	local scene = playertar.GetScene()
	if scene.dwMapID == 248 then
		local yuehua = scene.GetNpcByNickName("yuehua")
		if yuehua and yuehua.bFightState == true then
			playertar.AddBuff(yuehua.dwID, yuehua.nLevel, 11371, 1)
		end
	end
	if not playertar then
		--	print("打击到这")
		return
	end
...
--		return
--	end
	if playertar.nImmuneSkillMoveCount == 0 then
		playertar.BeHitted(playertar.dwID, 5638, 1, 30)
		--print("打击进入")
		--end
