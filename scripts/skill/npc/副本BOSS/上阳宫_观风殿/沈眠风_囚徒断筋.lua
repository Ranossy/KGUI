---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_囚徒断筋.lua
-- 更新时间:	2016/4/8 15:14:18
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	...
	{nDamageBase = 3400, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_囚徒断筋.lua", -- 属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local shenmianfengxiaoguaikongzhi = scene.GetNpcByNickName("shenmianfengxiaoguaikongzhi")
	if shenmianfengxiaoguaikongzhi then
		if shenmianfengxiaoguaikongzhi then
			shenmianfengxiaoguaikongzhi.SetCustomInteger1(1, 1)---完美控制没了
		end
	end
	
	if scene.dwMapID == 240 or scene.dwMapID == 250 then
		if playertarget then
			playertarget.AddBuff(npc.dwID, npc.nLevel, 10599, 1, 2)
		end
	
		if npctarget then
			npctarget.AddBuff(npc.dwID, npc.nLevel, 10599, 1, 2)
		end
	end

	if scene.dwMapID == 248 then
		if playertarget then
			playertarget.AddBuff(npc.dwID, npc.nLevel, 10599, 2, 4)
		end
	
		if npctarget then
			npctarget.AddBuff(npc.dwID, npc.nLevel, 10599, 2, 4)
		end
	end
end
