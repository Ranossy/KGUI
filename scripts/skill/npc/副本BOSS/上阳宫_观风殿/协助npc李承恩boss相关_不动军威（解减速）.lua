---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/协助npc李承恩boss相关_不动军威（解减速）.lua
-- 更新时间:	2016/4/21 16:00:31
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/协助npc李承恩boss相关_不动军威（解减速）.lua", -- 属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npctarget then
		--npctarget.DelBuff(ID, LV)
	end

	local playertarget = GetPlayer(dwCharacterID)
	if playertarget then
		if playertarget.GetBuff(10647, 1) then
			playertarget.DelBuff(10647, 1)
		end
	end
end
