---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩行为控制_集中攻击.lua
-- 更新时间:	2016/5/12 11:08:41
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/协助npc李复李承恩行为控制_集中攻击.lua", --属性值1
		0															--属性值2
	);
	...
	skill.SetNormalCoolDown(1, 1105)
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npctarget then
		...
		if jihuomubiaokongzhi then
			if jihuomubiaokongzhi.GetCustomInteger4(4) ~= nil then
				local mubiaoID = jihuomubiaokongzhi.GetCustomInteger4(4)
				local mubiaoduixiang = GetNpc(mubiaoID)
				if mubiaoduixiang then
					mubiaoduixiang.DelBuff(10781, 1)
				end
			end
			jihuomubiaokongzhi.SetCustomInteger4(4, npctarget.dwID)
		end
		npctarget.AddBuff(player.dwID, player.nLevel, 10781, 1)

		local scene = npctarget.GetScene()
		local LiChengEn = scene.GetNpcByNickName("LiChengEn")
		local LiFu = scene.GetNpcByNickName("LiFu")
		if LiChengEn and LiChengEn.GetCustomBoolean(1) == true then
			LiChengEn.AddBuff(player.dwID, player.nLevel, 10728, 1)
			LiChengEn.DelBuff(10727, 1)
			LiChengEn.DelBuff(10729, 1)
			LiChengEn.PlaySfx(452, LiChengEn.nX, LiChengEn.nY, LiChengEn.nZ)
			if LiChengEn.nMoveState == MOVE_STATE.ON_STAND then
				LiChengEn.DoAction(0, 10062)
			end
		end

		if LiFu and LiFu.GetCustomBoolean(1) == true then
			LiFu.AddBuff(player.dwID, player.nLevel, 10728, 1)
			LiFu.DelBuff(10727, 1)
			LiFu.DelBuff(10729, 1)
			LiFu.PlaySfx(452, LiFu.nX, LiFu.nY, LiFu.nZ)
			if LiFu.nMoveState == MOVE_STATE.ON_STAND then
				LiFu.DoAction(0, 10062)
			end
		end
	end
	...
	if playertarget then
		--if playertarget.nMoveState == MOVE_STATE.ON_DEATH then
			--RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", "无效目标！", 3)
			--return
		--end
		...
		local LiChengEn = scene.GetNpcByNickName("LiChengEn")
		local LiFu = scene.GetNpcByNickName("LiFu")
		local ChouHenKongZhi = scene.GetNpcByNickName("chouhenkongzhi")
		if ChouHenKongZhi then
			ChouHenKongZhi.FireAIEvent(2001, 0, 1)
		end
		
		if LiChengEn and LiChengEn.GetCustomBoolean(1) == true then
			for i = 1, #playerlist do
				local playertarget1 = GetPlayer(playerlist[i])
				if playertarget1 then
					playertarget1.DelBuff(11076, 1)
				end
			end
			playertarget.CastSkill(16356, 1, TARGET.NPC, LiChengEn.dwID)
		end
		...
		if LiFu and LiFu.GetCustomBoolean(1) == true then
			...
			playertarget.CastSkill(16356, 1, TARGET.NPC, LiFu.dwID)
		end
	end
end
