---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_烈焰针对囚徒.lua
-- 更新时间:	2016/4/15 9:38:01
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_观风殿/沈眠风_烈焰针对囚徒.lua", -- 属性值1
		0															--属性值2
	);
	...
	skill.nRectWidth = 9 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npctarget and npctarget.dwTemplateID == 51219 then
		if not npctarget.GetBuff(11275, 1) then
			npctarget.Die()
		end
	end

	if npctarget and npctarget.dwTemplateID == 54329 then
		if not npctarget.GetBuff(11275, 1) then
			npctarget.Die()
		end
	end
	...
	if npctarget then
		if npctarget.dwTemplateID == 53842 then
			if not npctarget.GetBuff(11275, 2) then
				npctarget.Die()
			end
		end
	end
