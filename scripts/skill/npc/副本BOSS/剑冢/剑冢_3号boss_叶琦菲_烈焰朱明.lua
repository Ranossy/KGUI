...
function GetSkillLevelData(skill)
    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	----------------- 魔法属性 -------------------------------------------------
	    skill.AddAttribute(
	   		...
	   		"skill/npc/副本BOSS/剑冢/剑冢_3号boss_叶琦菲_烈焰朱明.lua",	        -- 属性值1
	   		0											                                -- 属性值2
	   		);
	----------------- 技能施放Buff需求 ---------------------------------------------
    ...
    ----------------- 时间相关 -------------------------------------------------
    skill.nPrepareFrames  	= 1.5 * GAME_FPS;				-- 吟唱帧数
...
function Apply(dwCharacterID)
	...
	if player and IsPlayer(dwCharacterID) then
		...
		if scene then
			local npc = scene.GetNpcByNickName("yeqifei")
			local nZ = 1087300
			if npc then
				if player.nZ <= nZ then
					npc.SetTarget(TARGET.PLAYER, player.dwID)
					npc.CastSkill(1892, 7)
				end
			end
		end
	end
