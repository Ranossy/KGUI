---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/天泣林/天泣林_3号_寒冰箭.lua
-- 更新时间:	2015/2/14 10:40:18
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	...
	{nDamageBase = 6800, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/天泣林/天泣林_3号_寒冰箭.lua", --属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	scene.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/天泣林/天泣林_3号_寒冰箭.lua", dwCharacterID, 1)
	--player.PlaySfx(1104, player.nX, player.nY, player.nZ + 500)
	--player.AddBuff(0, 99, 8776, 1, 10)
end
...
function OnTimer(scene, nParam1, nParam2)
	...
	local scene = player.GetScene()
	player.PlaySfx(1104, player.nX, player.nY, player.nZ + 500)
	if scene.dwMapID == 203 then
		player.AddBuff(0, 99, 8776, 1, 10)
	end

	if scene.dwMapID == 224 then
		player.AddBuff(0, 99, 8776, 2, 15)
	end
end
