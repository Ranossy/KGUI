...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
      skill.BindBuff(1, 3877, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	local nBoss = scene.GetNpcByNickName("asaxin")
	if not nBoss then
		return
	end
	if scene.dwMapID ==68 then
		nBoss.CastSkill(3813,1,TARGET.PLAYER,dwCharacterID)
	end
	if scene.dwMapID ==69 then
		nBoss.CastSkill(3813,2,TARGET.PLAYER,dwCharacterID)
	end
	if scene.dwMapID ==70 then
		nBoss.CastSkill(3813,3,TARGET.PLAYER,dwCharacterID)
	end
	if scene.dwMapID ==72 then
		nBoss.CastSkill(3813,4,TARGET.PLAYER,dwCharacterID)
	end	
end
