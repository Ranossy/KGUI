...
tSkillData =
{
	{nDamageBase = 1900*0.8, nDamageRand = 1900*0.4, nBuffLevel = 1},		--level 1
	{nDamageBase = 6700*0.8, nDamageRand = 6700*0.4, nBuffLevel = 2},		--level 2
	{nDamageBase = 6400*0.8, nDamageRand = 6400*0.4, nBuffLevel = 3},		--level 3
	{nDamageBase = 9900*0.8, nDamageRand = 9900*0.4, nBuffLevel = 4},		--level 4
...
function GetSkillLevelData(skill)
  ...
  skill.AddAttribute( 
   ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK,                        
	...
	skill.AddAttribute(                                                           
		...
		0                                                                         
 	   	  ...
 	   	  skill.AddAttribute(
   			...
   			"skill/npc/副本BOSS/荻花圣殿/荻花圣殿_3号BOSS_慕容追风_狮子连爪.lua",								--属性值1
   			0															--属性值2
   	   	  );   
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
    skill.BindBuff(1, 2204, tSkillData[dwSkillLevel].nBuffLevel);		-- 设置1号位Buff
	skill.BindBuff(2, 3876, 1);		-- 设置2号位Buff
	...
	if IsPlayer(dwCharacterID) then
		...
		if player then
			if player.nGender == 1 then
			   player.AddBuff(0,99,3059,1)
			end
			if player.nGender == 2 then
			   player.AddBuff(0,99,3060,1)
			end
		end
	end
end
