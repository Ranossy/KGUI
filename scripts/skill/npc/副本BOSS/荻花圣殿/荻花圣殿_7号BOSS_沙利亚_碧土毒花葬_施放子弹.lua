    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	----------------- 魔法属性 -------------------------------------------------
		...
		if dwSkillLevel == 2 then
   	   	skill.AddAttribute(
   			...
   			"skill/npc/副本BOSS/荻花圣殿/荻花圣殿_7号BOSS_沙利亚_碧土毒花葬_施放子弹.lua",								--属性值1
   			0															--属性值2
   		);
   	elseif dwSkillLevel == 4 then
   	   	skill.AddAttribute(
   			ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK,			-- 属性作用模式
   			ATTRIBUTE_TYPE.EXECUTE_SCRIPT,								-- 魔法属性
   			"skill/npc/副本BOSS/荻花圣殿/25人英雄荻花圣殿_7号BOSS_沙利亚_碧土毒花葬_施放子弹.lua",								--属性值1
   			0															--属性值2
   		);
   	end
...
local tDirection = {}
	...
	for	i = 1, 8 do
		...
		local npcYb = scene.CreateNpc(14609, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, 8 * GLOBAL.GAME_FPS)
		if npcYb then
			npcYb.RunTo(nX, nY)
		end
	end
