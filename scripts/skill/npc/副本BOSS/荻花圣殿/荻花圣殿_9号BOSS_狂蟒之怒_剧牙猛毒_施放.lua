...
tSkillData =
{
	{nDamageBase = 3200*0.8, nDamageRand = 3200*0.4, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	----------------- 魔法属性 -------------------------------------------------
   	   	...
   	   	skill.AddAttribute(
   			...
   			"skill/npc/副本BOSS/荻花圣殿/荻花圣殿_9号BOSS_狂蟒之怒_剧牙猛毒_施放.lua",								--属性值1
   			0															--属性值2
   		);
	...
	----------------- BUFF相关 -------------------------------------------------
  --skill.BindBuff(1, 2219, 1);		-- 设置1号位Buff
	...
	skill.nMaxRadius		= 300 * LENGTH_BASE;		-- 技能施放的最大距离 
...
local tDirection = {}
	...
	tDirection[2] = npc.nFaceDirection - 55 % 256
	tDirection[3] = npc.nFaceDirection + 55 % 256
	tDirection[4] = npc.nFaceDirection - 110 % 256
	tDirection[5] = npc.nFaceDirection + 110 % 256
		...
		for	i = 1, 5 do
			...
			if scene.dwMapID ==120 then
				local npcDy = scene.GetNpcByNickName("duyezhiqiu_" .. i )
				if not npcDy then
					local npcYb = scene.CreateNpc(15658, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1,"duyezhiqiu_" .. i)
					npcYb.RunTo(nX, nY)
				else
					local npcYb = scene.CreateNpc(15658, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1,"duyezhiqiu_" .. i+5)
					npcYb.RunTo(nX, nY)
				end
			elseif scene.dwMapID ==130 then
				local npcDy = scene.GetNpcByNickName("duyezhiqiu_" .. i )
				if not npcDy then
					local npcYb = scene.CreateNpc(16150, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1,"duyezhiqiu_" .. i)
					npcYb.RunTo(nX, nY)
				else
					local npcYb = scene.CreateNpc(16150, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1,"duyezhiqiu_" .. i+5)
					npcYb.RunTo(nX, nY)
				end
			end
		end
