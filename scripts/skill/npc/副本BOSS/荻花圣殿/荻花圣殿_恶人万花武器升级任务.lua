...
function GetSkillLevelData(skill)
    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	skill.AddAttribute(
   		...
   		"skill/npc/副本BOSS/荻花圣殿/荻花圣殿_恶人万花武器升级任务.lua",				-- 属性值1
   		0														-- 属性值2
   	);

	----------------- 技能施放Buff需求 ---------------------------------------------
    --skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
    skill.AddSlowCheckDestBuff(3211,10,BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID)
	...
	if player then
		...
		if nQuestPhase < 3 and nQuestPhase > 0 then
			player.SetQuestValue(nQuestIndex,0,1)
			for i = 1 , 10 do 
				player.DelBuff(3211,2)
			end
		end
	end
end
