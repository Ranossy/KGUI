...
local tDirection = {}
	...
	for	i = 1, 8 do
		local nY = npc.nY + math.floor(math.sin(tDirection[i] / 128 * math.pi) * 60 * 64)
		local nX = npc.nX + math.floor(math.cos(tDirection[i] / 128 * math.pi) * 60 * 64)
		local scene = npc.GetScene()
		if not scene then
			return
		end
		local npcYb = scene.CreateNpc(14981, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, 16 * 16)
		if npcYb then
			npcYb.RunTo(nX, nY)
		end
	end
