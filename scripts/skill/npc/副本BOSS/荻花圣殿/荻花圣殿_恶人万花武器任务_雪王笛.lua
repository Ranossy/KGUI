...
function GetSkillLevelData(skill)
    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	skill.AddAttribute(
   		...
   		"skill/npc/副本BOSS/荻花圣殿/荻花圣殿_恶人万花武器任务_雪王笛.lua",				-- 属性值1
   		0														-- 属性值2
   	);

	----------------- 技能施放Buff需求 ---------------------------------------------
    --skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
      skill.AddSlowCheckDestBuff(3211,1,BUFF_COMPARE_FLAG.GREATER_EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);		-- 需求目标Buff
    
	----------------- BUFF相关 -------------------------------------------------
      skill.BindBuff(1, 3211, 2);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	local cNpc = scene.GetNpcByNickName("bingwangdi")
	if not cNpc then
		return
	end
	if cPlayer.nCurrentLife < cPlayer.nMaxLife * 0.1 then
		cNpc.SetCustomBoolean(2, true)
	else
