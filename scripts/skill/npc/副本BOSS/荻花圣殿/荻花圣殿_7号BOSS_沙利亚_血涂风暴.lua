...
tSkillData =
{
	{nDamageBase = 5500*0.8, nDamageRand = 5500*0.4, nCostMana = 0},		--level 1
	{nDamageBase = 21800*0.8, nDamageRand = 21800*0.4, nCostMana = 0},		--level 2
	{nDamageBase = 19800*0.8, nDamageRand = 19800*0.4, nCostMana = 0},		--level 3
	{nDamageBase = 25800*0.8, nDamageRand = 25800*0.4, nCostMana = 0},		--level 4
   ...
   ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK,                        
	...
	skill.AddAttribute(
  	ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,			-- 属性作用模式
 		...
 		"skill/npc/副本BOSS/荻花圣殿/荻花圣殿_7号BOSS_沙利亚_血涂风暴.lua",				-- 属性值1
 		0														-- 属性值2
 	);
	...
	----------------- BUFF相关 -------------------------------------------------
    --skill.BindBuff(1, 2235, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if IsPlayer(dwCharacterID) then
			...
			if scene.dwMapID == 68 then
				player.nCurrentMana = player.nCurrentMana - 800
			end
			if scene.dwMapID == 70 then
				player.nCurrentMana = player.nCurrentMana - 2000
			end
			if scene.dwMapID == 69 then
				player.nCurrentMana = player.nCurrentMana - 3000
			end
			if scene.dwMapID == 72 then
				player.nCurrentMana = player.nCurrentMana - 5000
			end
	end
end
