---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/雁门关之役/雁门关之役_独孤问俗_沙暴_添加.lua
-- 更新时间:	2014/10/11 15:21:22
-- 更新用户:	wangpengfei1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/雁门关之役/雁门关之役_独孤问俗_沙暴_添加.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID)
	...
	local player_ALL = scene.GetAllPlayer()
	local tplayer_select = {}
	for i = 1, #player_ALL do
		local player_this = GetPlayer(player_ALL[i])
		if player_this then
			if player_this.nMoveState ~= MOVE_STATE.ON_DEATH and not player_this.GetBuff(5491, 1) then
				table.insert(tplayer_select, player_this)
			end
		end
	end
	local nNum = Random(1, #tplayer_select)
	local player_select = tplayer_select[nNum]
	if player_select then
		player_select.AddBuff(0, 99, 8369, 1, 5)
	end
end
