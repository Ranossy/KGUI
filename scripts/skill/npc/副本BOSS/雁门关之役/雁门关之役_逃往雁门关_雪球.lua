---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/雁门关之役/雁门关之役_逃往雁门关_雪球.lua
-- 更新时间:	2014/9/15 11:11:13
-- 更新用户:	ks13376-PC
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 69600, nDamageRand = 0, nCostMana = 0},		--level 1
	{nDamageBase = 230, nDamageRand = 0, nCostMana = 0},		--level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/雁门关之役/雁门关之役_逃往雁门关_雪球.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	if npctarget then
		if npctarget.dwTemplateID == 37077 then
			if npctarget.nCurrentLife > 232100 then
				npctarget.nCurrentLife = npctarget.nCurrentLife - 232100
			else
				npctarget.Die()
			end
		end

		if npctarget.dwTemplateID == 39190 then
			if npctarget.nCurrentLife > 1170 then
				npctarget.nCurrentLife = npctarget.nCurrentLife - 1170
			else
				npctarget.Die()
			end
		end
