---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/雁门关之役/雁门关之役_独孤问俗_沙暴.lua
-- 更新时间:	2014/10/11 15:09:11
-- 更新用户:	wangpengfei1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 50400, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/雁门关之役/雁门关之役_独孤问俗_沙暴.lua", --属性值1
		0															--属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	--skill.AddSlowCheckDestBuff(244, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 8479, 1);		-- 设置1号位Buff
	skill.BindBuff(2, 8525, 1);		-- 设置2号位Buff
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player and playertarget then
		if playertarget.dwID ~= player.dwID then
			playertarget.Stop()
			playertarget.AddBuff(0, 99, 8479, 1, 5)
		end
	end--]]
	--local playertarget = GetPlayer(dwCharacterID)
	...
	--playertarget.AddBuff(0, 99, 8525, 1, 3)
end
