---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/雁门关之役/雁门关之役_逃往雁门关_狼牙兵普通攻击.lua
-- 更新时间:	2014/9/12 14:16:07
-- 更新用户:	ks13376-PC
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 43100, nDamageRand = 2000, nCostMana = 0},		--level 1（内城狼牙）
	{nDamageBase = 200, nDamageRand = 20, nCostMana = 0},		--level 2（狼牙校尉)
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/雁门关之役/雁门关之役_逃往雁门关_狼牙兵普通攻击.lua",								--属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	if nindex <= 10 then
		if player then
			scene.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/雁门关之役/雁门关之役_逃往雁门关_狼牙兵普通攻击.lua", player.dwID, 1)
			--player.PlaySfx(553, player.nX, player.nY, player.nZ + 500)
		end

		if npctarget then
			scene.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/雁门关之役/雁门关之役_逃往雁门关_狼牙兵普通攻击.lua", npctarget.dwID, 1)
		end
	end
end
...
function OnTimer(scene, nParam1, nParam2)
	...
	if playertarget then
		...
		if scene.dwMapID == 195 then
			playertarget.AddBuff(0, 99, 8243, 1)
		end
	end
