---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/雁门关之役/雁门关之役_中田海_风暴_对人.lua
-- 更新时间:	2014/9/26 10:19:03
-- 更新用户:	ks13376-PC
-- 脚本说明:
----------------------------------------------------------------------<
---------------------------------------------------------------------->
...
tSkillData =
{
	{nDamageBase = 30400, nDamageRand = 3200 * 0.4, nCostMana = 0}, --level 1
	{nDamageBase = 300, nDamageRand = 100 * 0.4, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/雁门关之役/雁门关之役_中田海_风暴_对人.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcId)
	...
	--[[if playerT and playerT.nMoveState ~= MOVE_STATE.ON_DASH then
		if playerT.nMoveState ~= MOVE_STATE.ON_DEATH then
			playerT.AddBuff(0, 99, 8331, 1)
		end
	else
		playerT.Stop()
		...
		if npc then
			if scene.dwMapID == 195 then
				npc.CastSkill(9542, 1)
			end

			if scene.dwMapID == 196 then
				npc.CastSkill(9542, 2)
			end
			return
		end
	end--]]
	...
	playerT.SetTimer(4, "scripts/skill/npc/副本BOSS/雁门关之役/雁门关之役_中田海_风暴_对人.lua", dwCharacterID, dwSkillSrcId)
end
...
function OnTimer(player, p1, p2)
	...
	BuffJX = playerT.GetBuff(8331, 1)
	local Buffn
	if BuffJX then
		Buffn = BuffJX.nStackNum
	else
		return
	end
	...
	if scene.dwMapID == 195 then
		if Buffn > 2 then
			playerT.DelGroupBuff(8331, 1)
			playerT.AddBuff(0, 99, 8332, 1, 10)--缴械buff
		end
		local npcF = GetNpc(dwSkillSrcId)
		if npcF then
			if npcF.GetCustomInteger1(5) >= 2 then
				npcF.SetDisappearFrames(8, 0)
			else
				local nID = npcF.GetCustomInteger1(5)
				nID = nID + 1
				npcF.SetCustomInteger1(5, nID)
			end
		end
	end
	...
	if scene.dwMapID == 196 then
		if Buffn > 2 then
			playerT.DelGroupBuff(8331, 1)
			playerT.AddBuff(0, 99, 8332, 1, 5)--缴械buff
		end
		local npcF = GetNpc(dwSkillSrcId)
		if npcF then
			if npcF.GetCustomInteger1(5) >= 2 then
