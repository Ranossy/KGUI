---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/雁门关之役/雁门关之役_独孤问俗_普通攻击.lua
-- 更新时间:	2014/10/29 15:06:14
-- 更新用户:	ks13376-PC
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 51700, nDamageRand = 2000, nCostMana = 0}, --level 1（内城狼牙）
	{nDamageBase = 400, nDamageRand = 50, nCostMana = 0}, --level 2（狼牙校尉)
	{nDamageBase = 27500, nDamageRand = 2000, nCostMana = 0}, --level 3(看守）
	{nDamageBase = 86200, nDamageRand = 2000, nCostMana = 0}, --level 4（狼牙精锐）
	{nDamageBase = 107700, nDamageRand = 2000, nCostMana = 0}, --level 5（袁芳）
	{nDamageBase = 28500, nDamageRand = 2000, nCostMana = 0}, --level 6(狼）
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/雁门关之役/雁门关之役_独孤问俗_普通攻击.lua", --属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	if nindex <= 50 then
		if player then
			scene.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/雁门关之役/雁门关之役_独孤问俗_普通攻击.lua", player.dwID, 1)
			--player.PlaySfx(553, player.nX, player.nY, player.nZ + 500)
		end

		if npctarget then
			scene.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/雁门关之役/雁门关之役_独孤问俗_普通攻击.lua", npctarget.dwID, 1)
		end
	end
end
...
function OnTimer(scene, nParam1, nParam2)
	...
	if playertarget then
		playertarget.PlaySfx(553, playertarget.nX, playertarget.nY, playertarget.nZ + 500)
		if scene.dwMapID == 195 then
			playertarget.AddBuff(0, 99, 8480, 1)
		elseif scene.dwMapID == 196 then
			playertarget.AddBuff(0, 99, 8480, 2)
		end
	end
	...
	if npctarget then
		...
		if scene.dwMapID == 195 then
			npctarget.AddBuff(0, 99, 8480, 1)
		elseif scene.dwMapID == 196 then
			npctarget.AddBuff(0, 99, 8480, 2)
		end
	end
end
