...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	  skill.BindBuff(1, 5829, 1);		-- 设置1号位Buff
...
function UnApply(dwCharacterID)
	...
	local nBuff = npc.GetBuff(5830, 1)
	if not nBuff then
		npc.CastSkill(6222, 1)
		return
	end
	if nBuff.nStackNum < 5 then
		npc.CastSkill(6222, 1)
	elseif nBuff.nStackNum < 12 then
		npc.CastSkill(6222, 2)
	else
		npc.CastSkill(6222, 3)
	end
end
