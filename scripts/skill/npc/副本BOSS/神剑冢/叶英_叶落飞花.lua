---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/神剑冢/叶英_叶落飞花.lua
-- 更新时间:	2013/7/9 14:49:02
-- 更新用户:	liguanghao2
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/神剑冢/叶英_叶落飞花.lua",
		0														
	);
	--[[
   	skill.AddAttribute(
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 6451, 1);		-- 设置1号位Buff
		...
		if scene then
			...
			if #tPlayer <= 5 then
				for i = 1, #tPlayer do
					...
					if cPlayer and cPlayer.nMoveState ~= MOVE_STATE.ON_DEATH and GetCharacterDistance(npc.dwID, cPlayer.dwID) < 2240 then
						cPlayer.AddBuff(0, 99, 6451, 1)
					end
				end
			else
				for i = 1, 5 do
