...
tSkillData =
{
	{nDamageBase = 58000, nDamageRand = 0, nCostMana = 0},		--level 1
	{nDamageBase = 166428, nDamageRand = 0, nCostMana = 0},		--level 2
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	  --skill.BindBuff(1, 6452, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	local nBuff = player.GetBuff(6452, 1)
	if not nBuff then
		return
	end
	if nBuff.nStackNum == 2 then
		player.Die()
	end
	--]]
end
