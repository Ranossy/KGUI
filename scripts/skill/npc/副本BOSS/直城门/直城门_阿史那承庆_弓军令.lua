...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/直城门/直城门_阿史那承庆_弓军令.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	local tTempPlayer1 = {}
	local tTempPlayer2 = {}
	local bType, bID = npc.GetTarget()
	local player_tar
	if bType == TARGET.PLAYER then
		player_tar = GetPlayer(bID)
	elseif bType == TARGET.NPC then
		player_tar = GetNpc(bID)
	end
	if not player_tar then
		return
	end
	for i = 1, #tPlayer do
	...
	if #tTempPlayer1 > 1 then
		for i = 1, #tTempPlayer1 do
			local CPlayer = tTempPlayer1[i]
			if CPlayer then
				if CPlayer.dwID ~= player_tar.dwID then
					tTempPlayer2[#tTempPlayer2 + 1] = CPlayer
				end
			end
		end
	else
		for i = 1, #tTempPlayer1 do
			tTempPlayer2[#tTempPlayer2 + 1] = tTempPlayer1[i]
		end
	end
	for i = 1, 6 do
		local npc_gj = scene.GetNpcByNickName("BOSS3GJ" .. i)
		if npc_gj then
			N_X = N_X + 1
		end
	end
	for i = 1, #tTempPlayer2 do
		print(tTempPlayer2[i].szName)
	end
	if N_X > 3 then
		local N_Y = 0
		for i = 1, 6 do
			local npc_gj = scene.GetNpcByNickName("BOSS3GJ" .. i)
			if npc_gj then
				npc_gj.DoAction(0, 10054)
				N_Y = N_Y + 1
				if N_Y > 1 then
					local CPlayer1 = tTempPlayer2[1]
					local CPlayer2 = tTempPlayer2[2]
					if CPlayer1 then
						CPlayer1.AddBuff(0, 99, 6597, 1)		
					end
					if CPlayer2 then
						CPlayer2.AddBuff(0, 99, 6597, 1)
					end
					break
				end
			end
