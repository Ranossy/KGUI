   	...
   	----------------- 魔法属性 -------------------------------------------------
		skill.AddAttribute(
			ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,
	   		...
	   		"skill/npc/副本BOSS/25人天王殿/九音陀罗曲·刚震.lua",	        	-- 属性值1
			0
			);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	if player and IsPlayer(dwCharacterID) then
		...
		if scene then
			...
			local npc1 = scene.GetNpcByNickName("chiguo")
			if npc then
				local num = player.nZ - npc.nZ
				if num < nDis then
					npc.CastSkill(1895, 1, TARGET.PLAYER, player.dwID)
				end
			end
			if npc1 then
				local num2 = player.nZ - npc1.nZ
				if num2 < nDis then
					npc1.CastSkill(1895, 1, TARGET.PLAYER, player.dwID)
				end
			end
		end
	end
