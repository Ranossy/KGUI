...
tSkillData =
{
	{nBrokenRate = 1},		--level 1
	{nBrokenRate = 0},		--level 2
	{nBrokenRate = 0},		--level 3
	{nBrokenRate = 0},		--level 4
};
--设置武功技能级别相关数值
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
   		...
   		--	"skill/npc/副本BOSS/法王窟/法王窟_青翼蝠王_音波功.lua",								--属性值1
   			"skill/npc/副本BOSS/龙渊泽/龙渊泽_南诏双雄_刺客_技能2_吟唱.lua",
   			0															--属性值2
	  );
   	--[[
   	skill.AddAttribute(
    ...
    skill.nBrokenRate         = tSkillData[dwSkillLevel].nBrokenRate * PERCENT_BASE;		-- 技能被打断的概率,默认1024
...
function Apply(dwCharacterID)
	...
	local xieyunliu = scene.GetNpcByNickName("Boss_4_Assassin")
	if not xieyunliu then
		return
	end
	if math.abs(cPlayer.nZ - 	xieyunliu.nZ) <= nDis then
		xieyunliu.CastSkill(2844, Lv, TARGET.PLAYER, cPlayer.dwID)
	end
end
