    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	skill.AddAttribute(
   		...
   		--	"skill/npc/副本BOSS/法王窟/法王窟_青翼蝠王_音波功.lua",								--属性值1
   			"skill/npc/副本BOSS/龙渊泽/龙渊泽_神策狗男女_女_技能3.lua",
   			0															--属性值2
	  );
   	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID)
	...
	if npc then
		...
		if scene then
			...
			local npcBossMan = scene.GetNpcByNickName("Boss_1_Man")
			if npcBossMan then
				npcBossMan.AddBuff(0, 99, 2664, Lv)
			end
			local npcBossWoman = scene.GetNpcByNickName("Boss_1_Woman")
			if npcBossWoman then
				npcBossWoman.AddBuff(0, 99, 2664, Lv)
			end
		end
	end
end
