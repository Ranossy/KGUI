---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_夜守孤城/箭塔.lua
-- 更新时间:	2014/9/18 15:50:42
-- 更新用户:	xiedixun2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_夜守孤城/英雄箭塔.lua", -- 属性值1
		0														-- 属性值2
	);
	...
	--skill.SetNormalCoolDown(1, 830);	--技能普通CD
	...
	--if dwSkillLevel == 1 then
		--skill.nCostMana = 2;	-- 技能消耗的内力
	--elseif dwSkillLevel == 2 then
	skill.nCostMana = 3;	-- 技能消耗的内力
	--end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local DANYAO = player.GetItemAmount(5, 22221)
	if DANYAO > 2 then
		player.CostItem(5, 22221,3)
	else
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(6, 8525), 3)
		return
	end
	...
	local npc = scene.CreateNpc(27566, nX, nY, nZ, nF, 16)
	if npc then
		npc.AddBuff(0, 99, 3246, 1, 999)
		npc.SetPosition(nX, nY, nZ)
		local buff1 = player.GetBuff(7372, 1)
		local buff2 = player.GetBuff(7373, 1)
		buff1.nCustomValue = GetCharacterDistance(player.dwID, npc.dwID)
		buff2.nCustomValue = npc.dwID
		player.CastSkillXYZ(13158, 1, nX, nY, nZ)
		player.CastSkillXYZ(9151, 1, nX, nY, nZ)
		player.SetTimer(2, "scripts/skill/npc/副本BOSS/太原之战_夜守孤城/箭塔目标技能.lua", 0, 0)
		player.PlaySfx(1060, nX, nY, nZ)
	end	
end
