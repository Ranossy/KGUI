---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_夜守孤城/战斗唐军普攻.lua
-- 更新时间:	2014/11/9 12:39:42
-- 更新用户:	xiedixun2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	...
	{nDamageBase = 10, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_夜守孤城/战斗唐军普攻.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local LANGYA = GetNpc(dwCharacterID)
	if not LANGYA then
		return
	end
	
	local scene = LANGYA.GetScene()
	if not scene then
		return
	end
	...
	if Num < 10 then
		scene.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/太原之战_夜守孤城/战斗唐军普攻.lua", LANGYA.dwID, 0)
	end
end

function OnTimer(scene, nParam1, nParam2)
	local LANGYA = GetNpc(nParam1)
	if not LANGYA then
		return
	end

	LANGYA.PlaySfx(553, LANGYA.nX, LANGYA.nY, LANGYA.nZ + 500)
	LANGYA.PlaySfx(1015, LANGYA.nX, LANGYA.nY, LANGYA.nZ)
end
