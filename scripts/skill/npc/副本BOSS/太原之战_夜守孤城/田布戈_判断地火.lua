---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_夜守孤城/田布戈_判断地火.lua
-- 更新时间:	2015/3/12 11:36:00
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_夜守孤城/田布戈_判断地火.lua", -- 属性值1
		0														-- 属性值2
	);
	...
	----------------- 作用范围 -------------------------------------------------
	skill.nProtectRadius = 34 * LENGTH_BASE;                     -- 环形和矩形AOE的保护距离，范围内不受伤害
	--skill.nHeight = 0 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
	--skill.nRectWidth = 0 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
	skill.nAngleRange = 256;					-- 攻击范围的扇形角度范围
	skill.nAreaRadius = 90 * LENGTH_BASE;		-- 技能作用半径
	skill.nTargetCountLimit = 80;				-- 技能作用目标数量限制,(小于0 代表目标数量不限制)
...
function Apply(dwCharacterID, dwSkillSrcID)
	local DIHUO = GetNpc(dwCharacterID)
	if DIHUO then
		if DIHUO.dwTemplateID == 37014 or DIHUO.dwTemplateID == 41960 or DIHUO.dwTemplateID == 40642 then
			DIHUO.SetDisappearFrames(1, 0)
		end
	end
end
