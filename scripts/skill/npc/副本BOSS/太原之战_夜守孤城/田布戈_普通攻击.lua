---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_夜守孤城/田布戈_普通攻击.lua
-- 更新时间:	2015/3/25 11:37:41
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_夜守孤城/田布戈_普通攻击.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	scene.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/太原之战_夜守孤城/田布戈_普通攻击.lua", npc.dwID, player.dwID)
...
function OnTimer(scene, nParam1, nParam2)
	...
	local MJBUFF = player.GetBuff(8084, 1)
	if MJBUFF then
		local CS = player.GetBuff(8084, 1).nStackNum
		npc.nCurrentLife = npc.nCurrentLife + 25000 * CS * 0.14
		if CS >= 10 then
			player.AddBuff(0, 99, 8536, 1, 3600)
		end
	end

	local MJBUFF2 = player.GetBuff(8084, 2)
	if MJBUFF2 then
		local CS2 = player.GetBuff(8084, 2).nStackNum
		npc.nCurrentLife = npc.nCurrentLife + 105000 * CS2 * 0.14
		if CS2 >= 10 then
			player.AddBuff(0, 99, 8536, 1, 3600)
		end
	end

	local MJBUFF3 = player.GetBuff(8084, 3)
	if MJBUFF3 then
		local CS3 = player.GetBuff(8084, 3).nStackNum
		npc.nCurrentLife = npc.nCurrentLife + 180000 * CS3 * 0.14
		if CS3 >= 10 then
			player.AddBuff(0, 99, 8536, 1, 3600)
		end
	end

	local MJBUFF4 = player.GetBuff(8084, 4)
	if MJBUFF4 then
		local CS4 = player.GetBuff(8084, 4).nStackNum
		npc.nCurrentLife = npc.nCurrentLife + 35000 * CS4 * 0.14
		if CS4 >= 10 then
			player.AddBuff(0, 99, 8536, 1, 3600)
		end
	end
