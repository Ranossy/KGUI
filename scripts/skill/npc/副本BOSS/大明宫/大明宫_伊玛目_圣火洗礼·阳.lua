...
tSkillData =
{
	{nDamageBase = 10600, nDamageRand = 0, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_伊玛目_圣火洗礼·阳.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 164 then
		if player.GetBuff(5404, 1) or player.GetBuff(5404,2) then
			player.Die()
		else
			player.AddBuff(0,99,5403,1)
		end
	else
		if player.GetBuff(5404, 1) or player.GetBuff(5404,2) then
			player.Die()
		else
			player.AddBuff(0,99,5403,2)
		end
