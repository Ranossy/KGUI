...
tSkillData =
{
	{nDamageBase = 9000, nDamageRand = 0, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_傀儡·饿鬼_饿鬼·饥.lua",								--属性值1
		0															--属性值2
	);	
	
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(5576, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 5569, 1);		-- 设置1号位Buff
	skill.BindBuff(2, 5582, 1);		-- 设置2号位Buff
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 164 then
		if player.nCurrentLife > 2400 then
			player.nCurrentLife = player.nCurrentLife - 2400
		else
			player.Die()	
		end
		local npc = scene.GetNpcByNickName("kuilei_egui")
		if not npc then
			return
		end
		if npc.nCurrentLife < npc.nMaxLife - 60000 then
			npc.nCurrentLife = npc.nCurrentLife + 60000
		else
			npc.nCurrentLife = npc.nMaxLife	
		end
	else
		if player.nCurrentLife > 7600 then
			player.nCurrentLife = player.nCurrentLife - 7600
		else
			player.Die()	
		end
		local npc = scene.GetNpcByNickName("kuilei_egui")
		...
		if npc.nCurrentLife < npc.nMaxLife - 200000 then
			npc.nCurrentLife = npc.nCurrentLife + 200000
		else
			npc.nCurrentLife = npc.nMaxLife
		end
	end
