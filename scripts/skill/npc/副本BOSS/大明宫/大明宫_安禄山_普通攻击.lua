...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(                                                           
		...
		skill.AddAttribute(
			...
			"skill/npc/副本BOSS/大明宫/大明宫_安禄山_普通攻击.lua",
			0
		);	
...
function Apply(dwCharacterID)
	...
	local jieduankongzhi = scene.GetNpcByNickName("anlushanjieduankongzhi")
	if jieduankongzhi and jieduankongzhi.GetCustomInteger1(1) == 2 then
		if scene.dwMapID == 164 then
			npc.AddBuff(0, 99, 5634, 1)
		else
			npc.AddBuff(0, 99, 5634, 2)
		end
	end
end
