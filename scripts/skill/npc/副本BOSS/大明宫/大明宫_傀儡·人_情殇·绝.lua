...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 164 then
		if nDistance < 640 then
			if player.GetBuff(5569, 1) then
				if player.GetBuff(5569, 1).nStackNum < 11 then
					if player.nCurrentLife <= 1000 then
						player.Die()
					else
						player.nCurrentLife = player.nCurrentLife - 1000	
					end
				elseif player.GetBuff(5569, 1).nStackNum < 21 then
		...
		if nDistance < 640 then
			if player.GetBuff(5569, 1) then
				if player.GetBuff(5569, 1).nStackNum < 6 then
					if player.nCurrentLife <= 6500 then
						player.Die()
					else
						player.nCurrentLife = player.nCurrentLife - 6500
					end
				elseif player.GetBuff(5569, 1).nStackNum < 10 then
