...
tSkillData =
{
	{nDamageBase = 12200, nDamageRand = 0, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_伊玛目_蓝火赤焰.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	if player.GetBuff(5405, 1)  then
		local num = player.GetBuff(5405, 1).nStackNum
		player.DelGroupBuff(5405,1)
		for i = 1 , num do
			player.AddBuff(0, 99, 5406, 1)
		end
	elseif player.GetBuff(5405, 2)  then
		local num = player.GetBuff(5405, 2).nStackNum + 1
		player.DelGroupBuff(5405,2)
		for i = 1 , num do
			player.AddBuff(0, 99, 5406, 2)
		end
	elseif player.GetBuff(5406, 1)  then
		local num = player.GetBuff(5406, 1).nStackNum
		player.DelGroupBuff(5406,1)
		for i = 1 , num do
			player.AddBuff(0, 99, 5405, 1)
		end
	elseif player.GetBuff(5406, 2)  then
		local num = player.GetBuff(5406, 2).nStackNum + 1
		player.DelGroupBuff(5406,2)
		for i = 1 , num do
			player.AddBuff(0, 99, 5405, 2)
		end
	end
	--[[if player.GetCustomInteger1(1) == 0 and player.GetCustomInteger1(2) == 0 then
		return
	end
	
	if player.GetCustomInteger1(2) ~= 0 then
		local step = player.GetCustomInteger1(2)
		player.SetCustomInteger1(1, step)
		player.SetCustomInteger1(2, 0)
		player.DelGroupBuff(5406, 1)
		for i = 1, step do
			player.AddBuff(0, 99, 5405, 1)
		end
		return
	elseif player.GetCustomInteger1(1) ~= 0 then
