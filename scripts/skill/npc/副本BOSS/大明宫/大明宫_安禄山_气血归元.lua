...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_安禄山_气血归元.lua", --属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 164 then
		npc.nCurrentLife = npc.nCurrentLife + player.nCurrentLife * 3
	else
		if player.nCurrentLife < player.nMaxLife * 0.7 then
			npc.nCurrentLife = npc.nCurrentLife + player.nCurrentLife * 2
		else
			npc.nCurrentLife = npc.nCurrentLife + player.nCurrentLife * 8
		end
	end
end
