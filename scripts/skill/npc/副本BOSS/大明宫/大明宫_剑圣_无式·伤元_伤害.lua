...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_剑圣_无式·伤元_伤害.lua",								--属性值1
		0															--属性值2
	);	
	...
	----------------- BUFF相关 -------------------------------------------------
	  skill.BindBuff(1, 5542, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 164 then
		...
		elseif player.nCurrentMana < player.nMaxMana * 0.8 then
			if player.nCurrentLife > 5500 then
				player.nCurrentLife = player.nCurrentLife - 5500
			else
				player.Die()
			end
		else
		...
		if player.nCurrentMana < player.nMaxMana * 0.4 then
			if player.nCurrentLife > 13000 then
				player.nCurrentLife = player.nCurrentLife - 13000
			else
				player.Die()
			end
		elseif player.nCurrentMana < player.nMaxMana * 0.8 then
			if player.nCurrentLife > 15500 then
				player.nCurrentLife = player.nCurrentLife - 15500
			else
				player.Die()
			end
		else
