---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/大明宫/大明宫_耶律燕_截架.lua
-- 更新时间:	2013/5/31 19:35:58
-- 更新用户:	mengxiangfei
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_耶律燕_截架.lua", --属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(5600, 1, BUFF_COMPARE_FLAG.LESS, 1, BUFF_COMPARE_FLAG.LESS);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(5534, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	skill.SetNormalCoolDown(1, 594);	--技能普通CD
...
function Apply(dwCharacterID, dwSrcID)
	...
	--[[
	local playerthis = GetPlayer(dwSrcID)
	if not playerthis then
		return
	end
	local npcB = scene.GetNpcByNickName("qiu")
	if npcB then
		npcB.SetCustomInteger4(2, 0)
	end
	--]]
	if scene.dwMapID == 165 then
		if player.nCurrentLife < player.nMaxLife * 0.03 then
			player.Die()
		else
