...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_耶律燕_长传.lua",								--属性值1
		0															--属性值2
	);	
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if npc.GetCustomInteger4(2) ~= player.dwID then
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	local scene = player.GetScene()
	local qiu = scene.GetNpcByNickName("qiu")
	if not qiu then
		return
	end
	local scene = player.GetScene()
	scene.SetTimer(0.3*GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/大明宫/大明宫_耶律燕_长传.lua", nX, nY)
end

function OnTimer(scene, nParam1, nParam2)
	local npcQ = scene.GetNpcByNickName("qiu")
	if not npcQ then
		return
	end
	npcQ.RunTo(nParam1, nParam2)
	scene.SetTimer(0.1 * GLOBAL.GAME_FPS, "scripts/Map/大明宫/Event/马球奔跑延时处理.lua", 0, 0)
	--- 帽子戏法成就 ---
	npcQ.AddBuff(0, 99, 6188, 1)
	local player = GetPlayer(npcQ.GetCustomInteger4(2))
	npcQ.SetCustomInteger4(2, 0)
	local buff = npcQ.GetBuff(6188, 1)
	if buff then
		buff.nCustomValue = player.dwID
	end
	npcQ.SetCustomBoolean(6, true)
end
