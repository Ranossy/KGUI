...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
		...
		"skill/npc/副本BOSS/大明宫/大明宫_伊玛目_轰炸_播特效.lua",
		0
	);
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,			-- 属性作用模式
		ATTRIBUTE_TYPE.EXECUTE_SCRIPT,								-- 魔法属性
		"skill/npc/副本BOSS/大明宫/大明宫_伊玛目_轰炸.lua",								--属性值1
		0															--属性值2
	);	
    
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	  skill.AddSlowCheckDestBuff(5453, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 164 then
		if npc.nCurrentLife <= npc.nMaxLife * 0.09 then
			npc.Die()
		end
		npc.nCurrentLife = npc.nCurrentLife - npc.nMaxLife * 0.09
	end
	
	if scene.dwMapID == 165 then
		if npc.nCurrentLife <= npc.nMaxLife * 0.035 then
			npc.Die()
		end
		npc.nCurrentLife = npc.nCurrentLife - npc.nMaxLife * 0.035
	end
end
