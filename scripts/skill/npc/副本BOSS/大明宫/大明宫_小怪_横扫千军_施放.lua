...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_小怪_横扫千军_施放.lua",								--属性值1
		0															--属性值2
	);		
...
function Apply(dwCharacterID)
	...
	if tPlayer then
		local Tar_Player = GetPlayer(tPlayer[Random(1, #tPlayer)])
		if Tar_Player then
			npc.TurnToCharacter(Tar_Player.dwID)
			local nDir = npc.nFaceDirection * 2 * math.pi / 256
			npc.PlaySfx(500, npc.nX + 100 * math.cos(nDir), npc.nY + 100 * math.sin(nDir),  npc.nZ, npc.nFaceDirection * 360 / 255 - 192 *360/255)
			if scene.dwMapID == 164 then
				npc.CastSkill(6271, 1)
			else
				npc.CastSkill(6271, 2)
			end
		end
	end
