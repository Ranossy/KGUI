...
Include("scripts/Map/大明宫/include/boss1.lh")
tSkillData =
{
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_耶律燕_NPC冲撞_眩晕.lua",								--属性值1
		0															--属性值2
	);	
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 5483, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSrcID)
	...
	local npc = scene.GetNpcByNickName("qiu")
	local npcCaster = GetNpc(dwSrcID)
	if not npc then
		return
	end
	...
	if npc.GetCustomInteger4(2) == dwCharacterID then
		npc.SetCustomInteger4(2, 0)
		scene.SetTimer(0.5*GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/大明宫/大明宫_耶律燕_NPC冲撞_眩晕.lua", dwCharacterID, dwSrcID)
	end
end
...
function OnTimer(scene, nParam1, nParam2)
	...
	local npcCaster = GetNpc(nParam2)
	local npc = scene.GetNpcByNickName("qiu")
	if not npc or not npcCaster or not player then
		return
	end
	local  nx, ny = RandomPointPlayer(npcCaster.nX, npcCaster.nY, player, 20 * 64, 60)
	npc.RunTo(nx, ny)
end
