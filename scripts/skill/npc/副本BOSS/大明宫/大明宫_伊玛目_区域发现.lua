...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_伊玛目_区域发现.lua",								--属性值1
		0															--属性值2
	);	
	...
	if dwSkillLevel == 1 or dwSkillLevel == 3 then
		skill.nMaxRadius = 53 * LENGTH_BASE;		-- 技能施放的最大距离
	elseif dwSkillLevel == 2 then
		skill.nMaxRadius = 136 * LENGTH_BASE;		-- 技能施放的最大距离
	elseif dwSkillLevel == 4 then
		skill.nMaxRadius = 140 * LENGTH_BASE;		-- 技能施放的最大距离
	else
	...
	if dwSkillLevel == 1 or dwSkillLevel == 5 then
		skill.nHeight = 20 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
	else
		skill.nHeight = 10 * LENGTH_BASE; 
	end
	
	if dwSkillLevel == 1 then
		skill.nRectWidth = 36 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
	elseif dwSkillLevel == 2 then
		skill.nRectWidth = 54 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
	elseif dwSkillLevel == 3 then
		skill.nRectWidth = 20 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
	elseif dwSkillLevel == 4 then
		skill.nRectWidth = 98 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
	else
		skill.nRectWidth = 16 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
	end
	
	--skill.nAngleRange		= 256;					-- 攻击范围的扇形角度范围
	if dwSkillLevel == 1 or dwSkillLevel == 3 then
		skill.nAreaRadius = 53 * LENGTH_BASE;		-- 技能作用半径
	elseif dwSkillLevel == 2 then
		skill.nAreaRadius = 136 * LENGTH_BASE;		-- 技能作用半径
	elseif dwSkillLevel == 4 then
		skill.nAreaRadius = 140 * LENGTH_BASE;		-- 技能作用半径
	else
		skill.nAreaRadius = 80 * LENGTH_BASE;		-- 技能作用半径
	end	
...
function Apply(dwCharacterID)
	...
	local TrapHashID = GetFileNameHash("scripts/Map/大明宫/trap/发现判断区域.lua")
	local playerlist = scene.GetAllPlayer()
	for i = 1, #playerlist do
		local playertarget = GetPlayer(playerlist[i])
		if playertarget and not playertarget.GetBuff(5455, 1) and playertarget.dwLastScriptID == TrapHashID then
			if playertarget.nMoveState ~= MOVE_STATE.ON_DEATH then
				playertarget.AddBuff(0, 99, 5455, 1)
			end
		end
	end
	local wuliangshangdegaibang = scene.GetNpcByNickName("wuliangshangdegaibang")
	if wuliangshangdegaibang then
		wuliangshangdegaibang.FireAIEvent(2001, 0, 0)
	end
