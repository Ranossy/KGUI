...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_英雄船只散射.lua",								--属性值1
		0															--属性值2
	);	
	...
	skill.SetNormalCoolDown(1, 613);	--技能普通CD
...
function Apply(dwCharacterID)
	...
	player.AddBuff(0, 99, 5894, 1)
	local nBuff = player.GetBuff(5894, 1)
	nBuff.nCustomValue = nZ
	player.SetTimer(16, "scripts/skill/npc/副本BOSS/大明宫/大明宫_英雄船只散射.lua", nX, nY)
end
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	...
	local nY = nParam2
	if nBuff then
		player.CastSkillXYZ(6707, 1, nX, nY, nBuff.nCustomValue)
	end
end
