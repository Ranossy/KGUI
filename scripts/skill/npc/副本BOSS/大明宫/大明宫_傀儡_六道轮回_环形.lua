...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_傀儡_六道轮回.lua",
		0
	);	
    
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	  skill.AddSlowCheckDestBuff(5644, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	  skill.BindBuff(1, 5644, 1);		-- 设置1号位Buff
	...
	----------------- 作用范围 -------------------------------------------------
	  skill.nProtectRadius = 25 * LENGTH_BASE;                     -- 环形和矩形AOE的保护距离，范围内不受伤害
...
function Apply(dwCharacterID)
	...
	if player.nCurrentLife == player.nMaxLife then
		--print("11111")
		player.Die()
	elseif player.nCurrentLife < 100 then
		player.nCurrentLife = player.nMaxLife
	else
		player.nCurrentLife = player.nMaxLife - player.nCurrentLife
	end
	local scene = player.GetScene()
	if not scene then
		return
	end
	local tName = {"kuilei_tian", "kuilei_ren", "kuilei_xiuluo", "kuilei_chusheng", "kuilei_egui", "kuilei_diyu"}
	for i = 1, #tName do
		local npc_kuilei = scene.GetNpcByNickName(tName[i])
		if npc_kuilei then
			if scene.dwMapID == 164 then
				if npc_kuilei.nCurrentLife < npc_kuilei.nMaxLife * 0.995 then
					npc_kuilei.nCurrentLife = npc_kuilei.nCurrentLife + npc_kuilei.nMaxLife * 0.005
				else
					npc_kuilei.nCurrentLife = npc_kuilei.nMaxLife
				end
			else
				if npc_kuilei.nCurrentLife < npc_kuilei.nMaxLife * 0.995 then
					npc_kuilei.nCurrentLife = npc_kuilei.nCurrentLife + npc_kuilei.nMaxLife * 0.005
