...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_剑圣_无式·伤元.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 164 then
		local npcYb = scene.CreateNpc(23872, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, 240)
		if npcYb then
			npcYb.RunTo(nX, nY)
		end
	else
		local npcYb = scene.CreateNpc(25233, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, 240)
		if npcYb then
			npcYb.RunTo(nX, nY)
		end
	end
