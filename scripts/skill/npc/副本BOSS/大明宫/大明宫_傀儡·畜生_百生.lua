...
function GetSkillLevelData(skill)
	...
	skill.BindBuff(2, 5580, 1);		-- 设置2号位Buff
...
function Apply(dwCharacterID)
	...
	local npc = scene.GetNpcByNickName("kuilei_chusheng")
	if not npc then
		return
	end
	if player.nMoveState == MOVE_STATE.ON_STAND then
		if scene.dwMapID == 164 then
			npc.CastSkill(5594, 1, TARGET.PLAYER, dwCharacterID)
		else
			npc.CastSkill(5594, 2, TARGET.PLAYER, dwCharacterID)
		end
	end
end
