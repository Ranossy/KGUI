...
tSkillData =
{
	{nDamageBase = 26000, nDamageRand = 0, nCostMana = 0},		--level 1
	{nDamageBase = 56700, nDamageRand = 0, nCostMana = 0},		--level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(												
		...
		"skill/npc/副本BOSS/大明宫/大明宫_独孤问俗_震地.lua",
		0
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if npc.GetCustomBoolean(1) == false then
		npc.PlaySfx(406, npc.nX, npc.nY, npc.nZ + 64 * 5, npc.nFaceDirection / 256 * 360 + 270)
		local scene = npc.GetScene()
		npc.SetCustomBoolean(1, true)
		scene.SetTimer(5 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/大明宫/大明宫_独孤问俗_震地.lua", 0, 0)
	end	
end

function OnTimer(scene, nParam1, nParam2)
	local npcDG = scene.GetNpcByNickName("duguwensu")
	if npcDG then
		npcDG.SetCustomBoolean(1, false) 
	end
end
