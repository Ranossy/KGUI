...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_剑圣_凡华.lua",								--属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID,dwSrcID)
	...
	local nState = player.GetOTActionState()
	if not nState then
		npc.CastSkill(5565,1,TARGET.PLAYER,dwCharacterID)
	end
	if nState == CHARACTER_OTACTION_TYPE.ACTION_SKILL_PREPARE or nState == CHARACTER_OTACTION_TYPE.ACTION_SKILL_CHANNEL then
		npc.CastSkill(5566, 1, TARGET.PLAYER, dwCharacterID)
	else
		npc.CastSkill(5565,1,TARGET.PLAYER,dwCharacterID)
	end
end
