...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_剑圣_驱幻针.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID,dwSrcID)
	...
	player.DelBuff(5505,2)
	local player1 = GetPlayer(dwSrcID)
	if not player1 then
		return
	end
	if player1.GetKungfuMount().dwSkiiID == 10014 or player1.GetKungfuMount().dwSkiiID == 10015 then
		player.DelBuff(5944, 1)
		player.DelBuff(5944,2)
	end
end
