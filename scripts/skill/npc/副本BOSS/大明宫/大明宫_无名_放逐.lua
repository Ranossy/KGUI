...
tSkillData =
{
	...
	{nDamageBase = 78300, nDamageRand = 0, nCostMana = 0},		--level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_无名_放逐.lua",								--属性值1
		0															--属性值2
	);	
	...
	----------------- BUFF相关 -------------------------------------------------
	  skill.BindBuff(1, 5712, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if player.nMoveState ~= MOVE_STATE.ON_DEATH then
		player.Stop()
		scene.CreateNpc(24064, player.nX, player.nY, player.nZ, 0, -1, "guangzhao_2_" .. player.dwID)
	end
end
