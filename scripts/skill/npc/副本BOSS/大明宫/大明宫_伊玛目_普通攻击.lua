...
tSkillData =
{
	{nDamageBase = 38000, nDamageRand = 0, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_伊玛目_普通攻击.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	local npc = scene.GetNpcByNickName("yimamujieduankongzhi")
	if not npc then
		return
	end
	if scene.dwMapID == 164 then
		if npc.GetCustomInteger1(1) == 2 then
			player.AddBuff(0, 99, 5409, 1)
		else
			player.AddBuff(0,99,5397,1)
		end
	else
		if npc.GetCustomInteger1(1) == 2 then
			player.AddBuff(0, 99, 5409, 2)
		else
			player.AddBuff(0,99,5397,2)
		end		
	end
end
