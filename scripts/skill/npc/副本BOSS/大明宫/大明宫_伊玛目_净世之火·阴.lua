...
tSkillData =
{
	...
	{nDamageBase = 9000, nDamageRand = 0, nCostMana = 0},		--level 3
	{nDamageBase = 9000, nDamageRand = 0, nCostMana = 0},		--level 4
	{nDamageBase = 9000, nDamageRand = 0, nCostMana = 0},		--level 5
	{nDamageBase = 60000, nDamageRand = 0, nCostMana = 0},		--level 6
	{nDamageBase = 18500, nDamageRand = 0, nCostMana = 0},		--level 7
	{nDamageBase = 18500, nDamageRand = 0, nCostMana = 0},		--level 8
	{nDamageBase = 18500, nDamageRand = 0, nCostMana = 0},		--level 9
	{nDamageBase = 18500, nDamageRand = 0, nCostMana = 0},		--level 10
};
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/大明宫/大明宫_伊玛目_净世之火·阴.lua",								--属性值1
		0															--属性值2
	);		
	...
	if dwSkillLevel >= 6 then
		skill.BindBuff(1, 5407, 1);		-- 设置1号位Buff
	end
	--skill.BindBuff(2, 5407, 1);		-- 设置2号位Buff
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 164 then
		local nBuff1 = player.GetBuff(5405, 1)
		if not nBuff1 then
			player.AddBuff(0, 99, 5406, 1)
		else
			local num = nBuff1.nStackNum
			player.DelGroupBuff(5405,1)
			for i = 1, num do
				player.AddBuff(0,99,5406,1)
			end
		end
		--[[if player.GetCustomInteger1(1) == 0 then
			local step = player.GetCustomInteger1(2)
			step = step + 1
			player.SetCustomInteger1(2, step)
			player.DelGroupBuff(5406, 1)
			for i = 1, step do
				player.AddBuff(0, 99, 5406, 1)
			end
		end
		...
		if player.GetCustomInteger1(1) ~= 0 then
			local step = player.GetCustomInteger1(1)
			player.SetCustomInteger1(2, step)
			player.SetCustomInteger1(1, 0)
			player.DelGroupBuff(5405, 1)
			for i = 1, step do
				player.AddBuff(0, 99, 5406, 1)
			end
	...
	if scene.dwMapID == 165 then
		local nBuff1 = player.GetBuff(5405, 2)
		if not nBuff1 then
			player.AddBuff(0, 99, 5406, 2)
		else
			local num = nBuff1.nStackNum + 1
			player.DelGroupBuff(5405,2)
			for i = 1, num do
				player.AddBuff(0,99,5406,2)
			end
		end
