---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/璨翠海厅/2号莫雨_号令红尘.lua
-- 更新时间:	2015/5/21 20:12:26
-- 更新用户:	zhangyan3
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 63600 * 1, nDamageRand = 63600 * 0, nCostMana = 0}, --level 1
	{nDamageBase = 81500 * 1, nDamageRand = 81500 * 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
    ...
    skill.AddAttribute(
		...
		"skill/npc/副本BOSS/璨翠海厅/2号莫雨_号令红尘.lua", -- 属性值1
		0													-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 2254, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if player then
		...
		local npc = scene.GetNpcByNickName("boss_MOYU")
		if npc then
			npc.AddBuff(0, 99, 9159, nbufflv)
		end
		------------------
		if scene.dwMapID == 200 then
			scene.SetCustomBoolean(SCENE_CUSTOM_VALUE_NAME.CANCUIHAITING.CHENGJIU_BUZUNHAOLING, true)--不尊号令
		elseif scene.dwMapID == 222 then
			scene.SetCustomBoolean(SCENE_CUSTOM_VALUE_NAME.TIAOZHANCANCUIHAITING.CHENGJIU_BUZUNHAOLING, true)--不尊号令
		end
	end
