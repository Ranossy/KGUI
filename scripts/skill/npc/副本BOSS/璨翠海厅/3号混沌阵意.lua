---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/璨翠海厅/3号混沌阵意.lua
-- 更新时间:	2015/5/21 19:57:29
-- 更新用户:	zhangyan3
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 24500*1, nDamageRand = 24500*0, nBuffLevel = 1},		--level 1
	{nDamageBase = 34800*1, nDamageRand = 34800*0, nBuffLevel = 2},		--level 2
	{nDamageBase = 3200*0.8, nDamageRand = 3200*0.4, nBuffLevel = 1},		--level 3
...
function GetSkillLevelData(skill)
    ...
    skill.AddAttribute(
		...
		"skill/npc/副本BOSS/璨翠海厅/3号混沌阵意.lua", -- 属性值1
		0													-- 属性值2
		);
	...
	----------------- BUFF相关 -------------------------------------------------
    --skill.BindBuff(1, 2263, tSkillData[dwSkillLevel].nBuffLevel);		-- 设置1号位Buff
	...
	skill.nMaxRadius		= 70 * LENGTH_BASE;		-- 技能施放的最大距离 
...
function Apply(dwCharacterID)
	...
	if player then
		...
		player.SetTimer(8,"scripts/skill/npc/副本BOSS/璨翠海厅/3号混沌阵意.lua",tBuff[nNum][1],nbufflv)
	else
		...
		if npc then
			...
			player1 = GetPlayer(npc.dwEmployer)
			if not player1 then
				return
			end
			local scene1 = player1.GetScene()
			if not scene1 then
				return
			end
			local scene2 = npc.GetScene()
			if not scene2 then
				return
			end			
			if player1 and scene1.dwID == scene2.dwID then
				player1.SetTimer(8,"scripts/skill/npc/副本BOSS/璨翠海厅/3号混沌阵意.lua",tBuff[nNum][1],nbufflv)
			end
		end
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	if boss then
		player.AddBuff(boss.dwID, boss.nLevel, nParam1,nParam2)
	end
	--player.AddBuff(0,99,nParam1,nParam2)
end
