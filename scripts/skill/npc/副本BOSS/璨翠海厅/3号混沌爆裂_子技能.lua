---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/璨翠海厅/3号混沌爆裂_子技能.lua
-- 更新时间:	2015/6/1 20:13:49
-- 更新用户:	zhangyan3
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 56700 * 1, nDamageRand = 56700 * 0, nBuffLevel = 1}, --level 1
	{nDamageBase = 81500 * 1, nDamageRand = 81500 * 0, nBuffLevel = 1}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/璨翠海厅/3号混沌爆裂_子技能.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID)
	...
	for nNum = 1, #tBuff do
		...
		if boss then
			player.AddBuff(boss.dwID, boss.nLevel, tBuff[nNum][1], nbufflv)
		end
	end
	
	if scene.dwMapID == 200 then
		scene.SetCustomBoolean(SCENE_CUSTOM_VALUE_NAME.CANCUIHAITING.CHENGJIU_SUANWUYICE, true)--算无遗策
	elseif scene.dwMapID == 222 then
		scene.SetCustomBoolean(SCENE_CUSTOM_VALUE_NAME.TIAOZHANCANCUIHAITING.CHENGJIU_SUANWUYICE, true)--算无遗策
	end
end
