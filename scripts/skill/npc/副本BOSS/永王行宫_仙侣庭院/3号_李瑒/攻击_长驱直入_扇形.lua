---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_仙侣庭院/3号_李瑒/攻击_长驱直入_扇形.lua
-- 更新时间:	2015/10/11 21:46:09
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
tSkillData =
{
	{nDamage = 5100, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamage = 7600, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamage = 9000, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamage = 11000, nDamageRand = 0, nCostMana = 0}, --level 4
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_仙侣庭院/3号_李瑒/攻击_长驱直入_扇形.lua", --属性值1
		0															--属性值2
	);
	...
	skill.AddSlowCheckDestBuff(9408, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	skill.nAreaRadius = 2.7 * LENGTH_BASE;		-- 技能作用半径
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcSHFW = scene.GetNpcByNickName("sanhaofanwei")
	if not npcSHFW then
		return
	end
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		...
		if MAPID == 220 then
			local MBuff = player.GetBuff(9408, 1)
			if not MBuff then
				player.AddBuff(0, 99, 9408, 1, 3600)
				npc.AddBuff(0, 99, 9407, 1, 3600)
			end
		elseif MAPID == 231 or MAPID == 232 then 
			local MBuff = player.GetBuff(9408, 1)
			if not MBuff then
				player.AddBuff(0, 99, 9408, 1, 3600)
				npc.AddBuff(0, 99, 9407, 2, 3600)
			end
		elseif MAPID == 234 then
			local MBuff = player.GetBuff(9408, 3)
			if not MBuff then
				player.AddBuff(0, 99, 9408, 3, 3600)
				npc.AddBuff(0, 99, 9407, 3, 3600)
			end
		elseif MAPID == 236 then
			local MBuff = player.GetBuff(9408, 4)
			if not MBuff then
				player.AddBuff(0, 99, 9408, 4, 3600)
				npc.AddBuff(0, 99, 9407, 4, 3600)
			end
		end
		
		if MapID == 220 then
			if not player.GetBuff(9402, 1) and not player.GetBuff(9403, 1) and not player.GetBuff(9404, 1) then
				if not npcSHFW.GetBuff(9841, 1) then
					npcSHFW.AddBuff(0, 99, 9841, 1, 3600)
				end
			end
		elseif MAPID == 231 or MAPID == 232 then
			if not player.GetBuff(9402, 1) and not player.GetBuff(9403, 1) and not player.GetBuff(9404, 1) and not player.GetBuff(9405, 1) and not player.GetBuff(9406, 1) then
				if not npcSHFW.GetBuff(9841, 1) then
					npcSHFW.AddBuff(0, 99, 9841, 1, 3600)
				end
			end
		elseif MAPID == 234 then
			if not player.GetBuff(9402, 3) and not player.GetBuff(9403, 3) and not player.GetBuff(9404, 3) and not player.GetBuff(9405, 3) and not player.GetBuff(9406, 3) then
				if not npcSHFW.GetBuff(9841, 1) then
					npcSHFW.AddBuff(0, 99, 9841, 1, 3600)
				end
			end
		elseif MAPID == 236 then
			if not player.GetBuff(9402, 4) and not player.GetBuff(9403, 4) and not player.GetBuff(9404, 4) and not player.GetBuff(9405, 4) and not player.GetBuff(9406, 4) then
				if not npcSHFW.GetBuff(9841, 1) then
					npcSHFW.AddBuff(0, 99, 9841, 1, 3600)
				end
			end
