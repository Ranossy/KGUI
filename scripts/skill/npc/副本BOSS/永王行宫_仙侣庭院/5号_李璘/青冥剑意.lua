...
tSkillData =
{
	...
	{nDamageBase = 65000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_仙侣庭院/5号_李璘/青冥剑意.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local JianQiang = GetNpc(dwSkillSrcID)
	local zkNpc
	
	if JianQiang then
		local scene = JianQiang.GetScene()
		
		if scene then
		    zkNpc = scene.GetNpcByNickName("WHZK")
		end

		if zkNpc then
			zkNpc.SetCustomInteger1(6, 5)
		end
	end
