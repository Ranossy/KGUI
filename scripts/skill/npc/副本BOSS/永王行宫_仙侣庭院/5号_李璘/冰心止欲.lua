...
function GetSkillLevelData(skill)
	...
	local dwSkillLevel = skill.dwLevel;
		...
		"skill/npc/副本BOSS/永王行宫_仙侣庭院/5号_李璘/冰心止欲.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene then
		level = mapToLevel[scene.dwMapID]
	end
	if not level then
		return
	end
	
	srcNpc.CastSkillXYZ(14933, level, targetPlayer.nX, targetPlayer.nY, targetPlayer.nZ)
