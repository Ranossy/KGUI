---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_仙侣庭院/小怪技能/仙侣庭院_小怪_反叛幕僚_击破联合爆炸.lua
-- 更新时间:	2015/8/7 10:41:54
-- 更新用户:	CHENJINGRU
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_仙侣庭院/小怪技能/仙侣庭院_小怪_反叛幕僚_击破联合爆炸.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if playertarget then
		if playertarget.nCurrentLife > playertarget.nMaxLife * 0.1 then
			playertarget.nCurrentLife = playertarget.nCurrentLife - playertarget.nMaxLife * 0.1
		else playertarget.Die()
		end
	end

	if player then
		local scene = player.GetScene()
		scene.SetTimer(0.5 * GLOBAL.GAME_FPS, "skill/npc/副本BOSS/永王行宫_仙侣庭院/小怪技能/仙侣庭院_小怪_反叛幕僚_击破联合爆炸.lua", dwSrcID, 1)
	end
end
...
function OnTimer(scene, nParam1, nParam2)
	local player = GetNpc(nParam1)
	if not player then
		return
	end
	player.PlaySfx(1014, 0, 0, 0)
end
