---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_仙侣庭院/小怪技能/仙侣庭院_小怪_龙图队正_士气爆棚.lua
-- 更新时间:	2015/8/7 15:52:44
-- 更新用户:	CHENJINGRU
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_仙侣庭院/小怪技能/仙侣庭院_小怪_龙图队正_士气爆棚.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if npc then
		npc.AddBuff(0, 99, 9388, 1, 20)
	end

	if npctarget then
		if npctarget.bFightState == true then
			npctarget.AddBuff(0, 99, 9388, 1, 20)
		end
	end

	if npc then
		local scene = npc.GetScene()
		scene.SetTimer(1 * GLOBAL.GAME_FPS, "skill/npc/副本BOSS/永王行宫_仙侣庭院/小怪技能/仙侣庭院_小怪_龙图队正_士气爆棚.lua", dwSrcID, 1)
	end
end
