---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_仙侣庭院/小怪技能/仙侣庭院_小怪_龙图伙长_流血之斧.lua
-- 更新时间:	2015/11/5 18:03:33
-- 更新用户:	chenjingru
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	...
	{nDamageBase = 16500, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 27200, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 31000, nDamageRand = 0, nCostMana = 0}, --level 4
	{nDamageBase = 89000, nDamageRand = 0, nCostMana = 0}, --level 5
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_仙侣庭院/小怪技能/仙侣庭院_小怪_龙图伙长_流血之斧.lua", --属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 9372, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		player.PlaySfx(1014, player.nX, player.nY, player.nZ + 500)
	end
	...
	if MapId == 220 then
		player.AddBuff(0, 99, 9372, 1)
	end

	if MapId == 231 or MapId == 232 then
		player.AddBuff(0, 99, 9372, 2)

	end

	if MapId == 234 then
		player.AddBuff(0, 99, 9372, 3)
	end

	if MapId == 236 then
		player.AddBuff(0, 99, 9372, 4)
	end
end
