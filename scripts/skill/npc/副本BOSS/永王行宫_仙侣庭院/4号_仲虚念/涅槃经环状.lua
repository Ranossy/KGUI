---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_仙侣庭院/4号_仲虚念/涅槃经环状.lua
-- 更新时间:	2015/7/22 17:56:40
-- 更新用户:	WANGZEYU
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_仙侣庭院/4号_仲虚念/涅槃经环状.lua",
		0
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 9761, 1);		-- 设置1号位Buff
	--skill.BindBuff(2, nBuffID, nBuffLevel);		-- 设置2号位Buff
	--skill.BindBuff(3, 9762, dwSkillLevel);		-- 设置3号位Buff
	...
	skill.nHeight = 3 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
	...
	skill.nAreaRadius = 2 * LENGTH_BASE;        -- 技能起始作用环的外圈半径
	skill.nProtectRadius = 0 * LENGTH_BASE;    -- 技能起始作用环的内圈半径，环的起始攻击范围在[skill.nProtectRadius， skill.nAreaRadius]之间，skill.nProtectRadius默认等于0
	skill.nSpreadCircleVelocity = 16; --环每帧运动点数，4就是1尺/秒，可以为负的，负表示朝内收缩
	skill.nSpreadCircleTotalFrame = 4 * GLOBAL.GAME_FPS;  --总的释放帧数 skill.nSpreadCircleVelocity * skill.nSpreadCircleTotalFrame / 64 就是整个技能全过程的作用半径
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 220 then
		TPlayer.Stop()
		TPlayer.AddBuff(0, 99, 9762, 1)
		TPlayer.DelBuff(9761, 1)
		TPlayer.AddBuff(0, 99, 9761, 1)		
	elseif scene.dwMapID == 231 or scene.dwMapID == 232 then
		TPlayer.Stop()
		TPlayer.AddBuff(0, 99, 9762, 2)
		TPlayer.DelBuff(9761, 1)
		TPlayer.AddBuff(0, 99, 9761, 1)
	elseif scene.dwMapID == 234 then
		TPlayer.Stop()
		TPlayer.AddBuff(0, 99, 9762, 3)
		TPlayer.DelBuff(9761, 1)
		TPlayer.AddBuff(0, 99, 9761, 1)
	elseif scene.dwMapID == 236 then
		TPlayer.Stop()
		TPlayer.AddBuff(0, 99, 9762, 4)
		TPlayer.DelBuff(9761, 1)
		TPlayer.AddBuff(0, 99, 9761, 1)
	end
	...
	if Area_Con then
		Area_Con.SetCustomInteger1(10, 1)
	end
end
