...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/南诏皇宫/南诏皇宫_6号_陈和尚_致命格挡_吟唱.lua",								--属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID)
	...
	local nBuff = npc.GetBuff(5068, 1)
	if not nBuff then
		return
	end
	local scene = npc.GetScene()
		if  nBuff.nStackNum < 4 then
			npc.CastSkill(4982, 1)
			npc.DelGroupBuff(5068,1)
		elseif nBuff.nStackNum > 3 and nBuff.nStackNum < 7 then
			npc.CastSkill(4983, 1)
			npc.DelGroupBuff(5068,1)
		else
			npc.CastSkill(4984, 1)
			npc.DelGroupBuff(5068,1)
		end
end
