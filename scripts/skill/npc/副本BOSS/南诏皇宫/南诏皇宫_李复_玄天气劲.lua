    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	----------------- 魔法属性 -------------------------------------------------
     	skill.AddAttribute(
   		...
   		"skill/npc/副本BOSS/南诏皇宫/南诏皇宫_李复_玄天气劲.lua",				
   		0														
   		);
...
function Apply(dwCharacterID)
	...
	if player.GetBuff(4310,1)  or player.GetBuff(4310,2)  then
		player.nCurrentMana = player.nCurrentMana + 8000
		player.AddBuff(0, 99, 4527, 1)
		local buff = player.GetBuff(4527, 1)
		local stack = buff.nStackNum
		if stack == 6 then
			player.AddBuff(0,99,4527,2)
			player.DelBuff(4310, 1)
			player.DelBuff(4310,2)
		end
	end
end
