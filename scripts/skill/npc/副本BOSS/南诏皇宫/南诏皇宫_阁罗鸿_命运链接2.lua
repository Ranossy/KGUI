...
function GetSkillLevelData(skill)
   ...
   ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK,                        
    ...
    --skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
      skill.AddSlowCheckDestBuff(4321, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL);		-- 需求目标Buff
...
function UnApply(dwCharacterID)
	...
	local cNpc = scene.GetNpcByNickName("geluohong")
	if not cNpc then 
		return
	end
	cNpc.FireAIEvent(2003,dwCharacterID)
end
