...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 4521, dwSkillLevel);		-- 设置1号位Buff
...
function UnApply(dwCharacterID)
	...
	local npcBX = scene.GetNpcByNickName("mohu_biaoxian")
	local nP = 0
	...
	if tPlayer then
		for i = 1, #tPlayer do
			...
			if Cplayer then
				if npcBX then
					if Cplayer.nMoveState ~= MOVE_STATE.ON_DEATH and GetCharacterDistance(npcBX.dwID, Cplayer.dwID) < 8 * 64 then
						nP = nP + 1
					end
				end
			end
	...
	if nP ~= 0 then
		if npcBX then
			npcBX.CastSkill(4275, nP+10)
			npcBX.SetDisappearFrames(1 * GLOBAL.GAME_FPS, false)
		end
	end
end
