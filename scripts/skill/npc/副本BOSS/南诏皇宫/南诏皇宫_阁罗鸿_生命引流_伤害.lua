...
tSkillData =
{
	...
	{nDamageBase = 5900, nDamageRand = 0, nCostMana = 0},		--level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/南诏皇宫/南诏皇宫_阁罗鸿_生命引流_伤害.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 140 then
		if cNpc.nCurrentLife > cNpc.nMaxLife * 0.99 then
			cNpc.nCurrentLife = cNpc.nMaxLife
		else
			cNpc.nCurrentLife = cNpc.nCurrentLife + cNpc.nMaxLife * 0.01
		end
	end
	if scene.dwMapID == 155 then
		if cNpc.nCurrentLife > cNpc.nMaxLife * 0.97 then
			cNpc.nCurrentLife = cNpc.nMaxLife
		else
			cNpc.nCurrentLife = cNpc.nCurrentLife + cNpc.nMaxLife * 0.03
		end
	end	
	local nTargetType, nTargetID = cNpc.GetTarget()
	local cPlayer = GetPlayer(nTargetID)
	if not cPlayer then
		return
	end
	cNpc.TurnTo(FaceTo(cNpc,cPlayer))
end
...
function FaceTo(nNpc, nTarget)
	local X1, Y1 = nNpc.nX, nNpc.nY
	local X2, Y2 = nTarget.nX, nTarget.nY
	local nRnd = math.atan((Y1 - Y2) / (X1 - X2 )) * 256 / (2*math.pi)	
	local nFace = 0
