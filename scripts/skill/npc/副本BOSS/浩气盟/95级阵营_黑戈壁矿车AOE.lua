---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/浩气盟/95级阵营_黑戈壁矿车AOE.lua
-- 更新时间:	2015/10/3 20:47:14
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	--skill.AddAttribute(
		...
		--	"Map/ACT_95级黑戈壁阵营活动/include/据点头文件.lua",
		--"skill/npc/副本BOSS/浩气盟/95级阵营_黑戈壁矿车AOE.lua",
		--0
	--);
...
local tKuangChe = {
	AddSelf = 3,--矿车自己人加分
	AddEnemy = 1, --矿车敌人加分
	AddSelfQuest = 3,--矿车自己人任务加分
	AddEnemyQuest = 1,--矿车敌人任务加分
	[47034] = {PQ = 78, },
	[47035] = {PQ = 79, },
}
local tQuest = {
...
function Apply(dwCharacterID, dwSkillSrcID)--这个函数从技能15038，脚本“scripts/skill/npc/副本BOSS/浩气盟/95级阵营_黑戈壁矿车AOE.lua”中调用。
	...
	--if player.GetBuff(961, 1) then
		--return
	--end
	--local nOldValue = player.GetPlayerPQValue(tKuangChe[npc.dwTemplateID].PQ)
	--local nQuestAdd = 0
	--if (player.nCamp == CAMP.GOOD and npc.dwTemplateID == 47034) or (player.nCamp == CAMP.EVIL and  npc.dwTemplateID == 47035) then--与矿车同阵营玩家
		--nOldValue = math.min(100, nOldValue + tKuangChe.AddSelf)
		--nQuestAdd = tKuangChe.AddSelfQuest
	--else--不同阵营玩家
		--nOldValue = math.min(100, nOldValue + tKuangChe.AddEnemy)
		--nQuestAdd = tKuangChe.AddEnemyQuest
	--end
	--player.SetPlayerPQValue(tKuangChe[npc.dwTemplateID].PQ, nOldValue)
	--for i, nQuestID in pairs(tQuest) do
		--if player.GetQuestPhase(nQuestID) == 1 then
			--local nQuestIndex = player.GetQuestIndex(nQuestID)
			--if nQuestIndex then
				--player.SetQuestValue(nQuestIndex, 0, player.GetQuestValue(nQuestIndex, nSlot) + nQuestAdd)
			--end
			--break
		--end
	--end
