---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/浩气盟/90级阵营_牛车AOE.lua
-- 更新时间:	2013/4/1 11:30:00
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/浩气盟/90级阵营_牛车AOE.lua",
		0														
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.bFightState then
		npc.SetCustomUnsigned4(10, GetCurrentTime())
	end
	local buffTX = player.GetBuff(961, 1)
	if player.nCamp == CAMP.GOOD and player.nLevel  >= 90 and not buffTX then
		npc.SetCustomUnsigned2(5, npc.GetCustomUnsigned2(5) + 1)
	end
	if player.nCamp == CAMP.EVIL and player.nLevel  >= 90 and not buffTX then
		npc.SetCustomUnsigned2(7, npc.GetCustomUnsigned2(7) + 1)
	end
end
