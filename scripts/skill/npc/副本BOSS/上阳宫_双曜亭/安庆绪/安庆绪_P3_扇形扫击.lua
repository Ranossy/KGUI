---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_双曜亭/安庆绪/安庆绪_P3_扇形扫击.lua
-- 更新时间:	2016/10/14 10:36:36
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 28100, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_双曜亭/安庆绪/安庆绪_P3_扇形扫击.lua", -- 属性值1
		0															--属性值2
	);
	...
	skill.BindBuff(2, 10871, 1);		-- 设置2号位Buff
	--skill.BindBuff(3, 11217, 2);		-- 设置3号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 241 or scene.dwMapID == 251 then
		...
		if anqingxu then
			player.AddBuff(anqingxu.dwID, anqingxu.nLevel, 11217, 1)
		end
	end
	if scene.dwMapID == 249 then
		local anqingxu = scene.GetNpcByNickName("anqingxu")
		if anqingxu then
			player.AddBuff(anqingxu.dwID, anqingxu.nLevel, 11217, 2)
		end
	end
end
