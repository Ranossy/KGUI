---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_双曜亭/安庆绪/安庆绪_P1_共建偷袭_条形AOE .lua
-- 更新时间:	2016/9/30 17:23:51
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 16380, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 49000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_双曜亭/安庆绪/安庆绪_P1_共建偷袭_条形AOE .lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 10870, 1);		-- 设置1号位Buff
--	skill.BindBuff(2, 11142, 1);		-- 设置2号位Buff
	...
	skill.nAreaRadius = 300 * LENGTH_BASE;		-- 技能作用半径
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 241 or scene.dwMapID == 251 then
		...
		if anqingxu then
			player.AddBuff(anqingxu.dwID, anqingxu.nLevel, 11142, 1)
		end
	end
	if scene.dwMapID == 249 then
		local anqingxu = scene.GetNpcByNickName("anqingxu")
		if anqingxu then
			player.AddBuff(anqingxu.dwID, anqingxu.nLevel, 11142, 2)
		end
	end
end
