---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_双曜亭/安庆绪/安庆绪_毒尸_P2_捶地AOE.lua
-- 更新时间:	2016/10/11 16:17:26
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 8320, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 18200, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_双曜亭/安庆绪/安庆绪_毒尸_P2_捶地AOE.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 10866, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene then
		if scene.dwMapID == 249 then
			local anqingxuzongkong = scene.GetNpcByNickName("anqingxuzongkong")
			local anqingxu = scene.GetNpcByNickName("anqingxu")
			if anqingxu and anqingxu.bFightState == true then
				if anqingxuzongkong then
					anqingxuzongkong.SetCustomInteger1(56, 1)
				end
			end
		end
	end
