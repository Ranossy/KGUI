---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_双曜亭/哥舒翰/六合_扇形.lua
-- 更新时间:	2016/9/30 13:15:15
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_双曜亭/哥舒翰/六合_扇形.lua", --属性值1
		0															--属性值2
	);
	...
	skill.nHeight = 65 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
	--skill.nRectWidth = 0 * LENGTH_BASE;                         -- 矩形AOE的宽度，全宽，不填为nAreaRadius
	skill.nAngleRange = 43;					-- 攻击范围的扇形角度范围
	skill.nAreaRadius = 1000 * LENGTH_BASE;		-- 技能作用半径
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcGSH = scene.GetNpcByNickName("geshuhan")
	if not npcGSH then
		return
	end
	local nSMapId = npc.GetMapID()
	if nSMapId == 241 or nSMapId == 251 then
		local nJS = npcGSH.GetCustomInteger1(6)
		if nJS == 0 then
			npcGSH.SetCustomInteger1(5, npcGSH.GetCustomInteger1(5) + 1)
		else
			local nTargetType, nTargetID = npcGSH.GetTarget()
			local Target = GetPlayer(nTargetID)
			if Target and nTargetType == TARGET.PLAYER then
				if Target.GetBuff(10886, 1) then
					Target.DelGroupBuff(10886, 1)
				end
			end
			local nSS = npcGSH.GetCustomInteger1(5)
			if nSS == 1 then
				npcGSH.CastSkill(15538, 1, TARGET.PLAYER, player.dwID)
			end
			if nSS == 2 then
				npcGSH.CastSkill(15538, 2, TARGET.PLAYER, player.dwID)
			end
			if nSS == 3 then
				npcGSH.CastSkill(15538, 3, TARGET.PLAYER, player.dwID)
			end
			if nSS >= 4 then
				npcGSH.CastSkill(15538, 4, TARGET.PLAYER, player.dwID)
			end
		end
	else
		local nJS = npcGSH.GetCustomInteger1(6)
		if nJS == 0 then
			...
			local nSS = npcGSH.GetCustomInteger1(5)
			if nSS == 1 then
				npcGSH.CastSkill(15538, 5, TARGET.PLAYER, player.dwID)
			end
			if nSS == 2 then
				npcGSH.CastSkill(15538, 6, TARGET.PLAYER, player.dwID)
			end
			if nSS == 3 then
				npcGSH.CastSkill(15538, 7, TARGET.PLAYER, player.dwID)
			end
			if nSS >= 4 then
				npcGSH.CastSkill(15538, 8, TARGET.PLAYER, player.dwID)
			end
		end
	end
end
