---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_双曜亭/哥舒翰/横扫_扇形.lua
-- 更新时间:	2016/9/30 17:12:44
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_双曜亭/哥舒翰/横扫_扇形.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if nSMapId == 241 then
		...
		if nJS == 0 then
			...
			if nSS > 0 and nSS <= 1 then
				npcGSH.CastSkill(15517, 1, TARGET.PLAYER, player.dwID)
			end
			if nSS == 2 then
				npcGSH.CastSkill(15517, 2, TARGET.PLAYER, player.dwID)
			end
			if nSS == 3 then
				npcGSH.CastSkill(15517, 3, TARGET.PLAYER, player.dwID)
			end
			if nSS >= 4 then
				npcGSH.CastSkill(15517, 4, TARGET.PLAYER, player.dwID)
			end
		end
	else
		local nJS = npcGSH.GetCustomInteger1(6)
		if nJS == 0 then
			...
			local nSS = npcGSH.GetCustomInteger1(5)
			if nSS > 0 and nSS <= 1 then
				npcGSH.CastSkill(15517, 5, TARGET.PLAYER, player.dwID)
			end
			if nSS == 2 then
				npcGSH.CastSkill(15517, 6, TARGET.PLAYER, player.dwID)
			end
			if nSS == 3 then
				npcGSH.CastSkill(15517, 7, TARGET.PLAYER, player.dwID)
			end
			if nSS >= 4 then
				npcGSH.CastSkill(15517, 8, TARGET.PLAYER, player.dwID)
			end
		end
	end
