---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_双曜亭/黑齿元佑_黑风掌.lua
-- 更新时间:	2016/4/7 14:10:48
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 12700, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_双曜亭/黑齿元佑_黑风掌.lua", -- 属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSrcID)
	...
	if playertarget then
		...
		scene.SetTimer(0.9 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/上阳宫_双曜亭/黑齿元佑_黑风掌.lua", playertarget.dwID, dwSrcID)
	end
end
...
function OnTimer(scene, nParam1, nParam2)
	...
	if scene.dwMapID == 241 or scene.dwMapID == 251 then
		if playertarget then
			playertarget.PlaySfx(1259, playertarget.nX, playertarget.nY, playertarget.nZ)
			if npc then
				npc.CastSkillXYZ(16492, 1, playertarget.nX, playertarget.nY, playertarget.nZ)
			end
		end
	end
	...
	if scene.dwMapID == 249 then
		if playertarget then
			playertarget.PlaySfx(1259, playertarget.nX, playertarget.nY, playertarget.nZ)
			if npc then
				npc.CastSkillXYZ(16492, 2, playertarget.nX, playertarget.nY, playertarget.nZ)
			end
		end
	end
end
