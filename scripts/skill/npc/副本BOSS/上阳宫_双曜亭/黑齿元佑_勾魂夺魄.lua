---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_双曜亭/黑齿元佑_勾魂夺魄.lua
-- 更新时间:	2016/4/6 19:13:42
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 4320, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 8400, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_双曜亭/黑齿元佑_勾魂夺魄.lua", -- 属性值1
		0															--属性值2
	);
	...
	skill.nTargetCountLimit	= 150;				-- 技能作用目标数量限制,(小于0 代表目标数量不限制)
	...
	skill.nBreakRate			= 1 * PERCENT_BASE;		-- 打断目标施法的概率,基数1024
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--[[if scene.dwMapID == 241 or scene.dwMapID == 251 then
		if playertarget then
			if playertarget.GetBuff(10584, 1) then
				playertarget.AddBuff(npc.dwID, npc.nLevel, 10584, 1, 600)
			elseif playertarget.GetBuff(10585, 1) then
				playertarget.AddBuff(npc.dwID, npc.nLevel, 10585, 1, 600)
			else
				local index = Random(1, 100)
				if index <= 50 then
					playertarget.AddBuff(npc.dwID, npc.nLevel, 10584, 1, 600)
				end
			...
			--if playertarget.GetOTActionState() == 1 or playertarget.GetOTActionState() == 2 then
				--playertarget.AddBuff(npc.dwID, npc.nLevel, 10592, 1, 6)
			--end
		end
	
		local npctarget = GetNpc(dwCharacterID)
		if npctarget then
			if npctarget.GetBuff(10584, 1) then
				npctarget.AddBuff(npc.dwID, npc.nLevel, 10584, 1, 600)
			elseif npctarget.GetBuff(10585, 1) then
				npctarget.AddBuff(npc.dwID, npc.nLevel, 10585, 1, 600)
			else
				local index = Random(1, 100)
				if index <= 50 then
					npctarget.AddBuff(npc.dwID, npc.nLevel, 10584, 1, 600)
	...
	--[[if scene.dwMapID == 249 then
		if playertarget then
			if playertarget.GetBuff(10584, 2) then
				playertarget.AddBuff(npc.dwID, npc.nLevel, 10584, 2, 600)
			elseif playertarget.GetBuff(10585, 2) then
				playertarget.AddBuff(npc.dwID, npc.nLevel, 10585, 2, 600)
			else
				local index = Random(1, 100)
				if index <= 50 then
					playertarget.AddBuff(npc.dwID, npc.nLevel, 10584, 2, 600)
			...
			if playertarget.GetOTActionState() == 1 or playertarget.GetOTActionState() == 2 then
				playertarget.AddBuff(npc.dwID, npc.nLevel, 10592, 2, 5)
			end
		end
	
		local npctarget = GetNpc(dwCharacterID)
		if npctarget then
			if npctarget.GetBuff(10584, 2) then
				npctarget.AddBuff(npc.dwID, npc.nLevel, 10584, 2, 600)
			elseif npctarget.GetBuff(10585, 2) then
				npctarget.AddBuff(npc.dwID, npc.nLevel, 10585, 2, 600)
			else
				local index = Random(1, 100)
				if index <= 50 then
					npctarget.AddBuff(npc.dwID, npc.nLevel, 10584, 2, 600)
