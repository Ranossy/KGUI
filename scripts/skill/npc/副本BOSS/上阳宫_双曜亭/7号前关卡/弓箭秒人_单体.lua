---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_双曜亭/7号前关卡/弓箭秒人_单体.lua
-- 更新时间:	2016/10/4 15:22:56
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_双曜亭/7号前关卡/弓箭秒人_单体.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcQHZK = scene.GetNpcByNickName("qihaoguanqiazongkong")
	if not npcQHZK then
		return
	end
	npcQHZK.SetCustomInteger1(6, 1)
	local MapID = scene.dwMapID
	if MapID == 241 or MapID == 251 then  --10PT
		local nJS = Random(10, 30)
		player.CustomDamage(npc.dwID, 16357, 1, 0, player.nMaxLife * nJS / 100)
		player.PlaySfx(1406, player.nX, player.nY + 80, player.nZ)
	end
	if MapID == 249 then  --25YX
		local nJS = Random(5, 10)
		player.CustomDamage(npc.dwID, 16357, 1, 0, player.nMaxLife * 10 / nJS)
		player.PlaySfx(1406, player.nX, player.nY + 80, player.nZ)
	end
end
