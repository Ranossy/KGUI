---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_双曜亭/7号前关卡/倒油.lua
-- 更新时间:	2016/10/4 17:28:15
-- 更新用户:	gudingyin
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_双曜亭/7号前关卡/倒油.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nJS = 0
	for i = 1, 7 do
		local doodadQHZW = scene.GetDoodadByNickName("qihaozawudui" .. i)
		if doodadQHZW then
			local nJL = (player.nX - doodadQHZW.nX) * (player.nX - doodadQHZW.nX) + (player.nY - doodadQHZW.nY) * (player.nY - doodadQHZW.nY)
			if nJL <= 4 * 64 * 64 and player.nZ >= doodadQHZW.nZ - 1536 then
				--local SZ = doodadQHZW.szName
				--local strFireName = string.sub(SZ, - 1, - 1)
				local npcQHHY = scene.GetNpcByNickName("qihaoguanqiaheiyou" .. i)
				if npcQHHY then
					RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(10, 4680), 4)
					player.SendSystemMessage(GetEditorString(10, 4681))
					return
				else
					if MapID == 241 then
						if player then
							player.AddAchievementCount(5546, 1)	--10人倒油
						end
					else
						if MapID == 249 then
							player.AddAchievementCount(5568, 1)	--25人倒油
						end
					end
					player.AddBuff(player.dwID, player.nLevel, 11062, 1)
					RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(10, 4682), 4)
					scene.CreateNpc(52054, doodadQHZW.nX, doodadQHZW.nY, doodadQHZW.nZ + 2560, 0, - 1, "qihaoguanqiaheiyou" .. i)
				end
			else
				nJS = nJS + 1
			end
	...
	if nJS == 7 then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(10, 4683), 4)
		player.SendSystemMessage(GetEditorString(10, 4683))
		return
	end
	return
end
