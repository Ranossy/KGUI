---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_双曜亭/黑齿元佑_小怪_蜘蛛毒液母技能.lua
-- 更新时间:	2016/7/21 10:46:36
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_双曜亭/黑齿元佑_小怪_蜘蛛毒液母技能.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 241 or scene.dwMapID == 251 then
		if playertarget then
			if npc then
				npc.CastSkill(16323, 1, TARGET.PLAYER, playertarget.dwID)
			end
		end
	end
	...
	if scene.dwMapID == 249 then
		if playertarget then
			if npc then
				npc.CastSkill(16323, 2, TARGET.PLAYER, playertarget.dwID)
			end
		end
	end
