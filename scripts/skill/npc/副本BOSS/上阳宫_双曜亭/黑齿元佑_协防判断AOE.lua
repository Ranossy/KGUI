---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_双曜亭/黑齿元佑_协防判断AOE.lua
-- 更新时间:	2016/3/30 15:52:13
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_双曜亭/黑齿元佑_协防判断AOE.lua", --属性值1
		0															--属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(11088, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 241 or scene.dwMapID == 251 then
		if npctarget then
			if npctarget.dwTemplateID == 51134 or npctarget.dwTemplateID == 51135 or npctarget.dwTemplateID == 54482 or npctarget.dwTemplateID == 54483 then
				if npc then
					if not npc.GetBuff(10583, 1) then
						local heichishuaduhuakongzhi = scene.GetNpcByNickName("heichishuaduhuakongzhi")
						if heichishuaduhuakongzhi then
							heichishuaduhuakongzhi.SetCustomInteger1(1, 1)--破防成就没了
						end
						npc.AddBuff(npc.dwID, npc.nLevel, 10583, 1, 3600)
					end
				end
			end
		end
	...
	if scene.dwMapID == 249 then
		if npctarget then
			if npctarget.dwTemplateID == 53995 or npctarget.dwTemplateID == 53996 then
				if npc then
					if not npc.GetBuff(10583, 2) then
						local heichishuaduhuakongzhi = scene.GetNpcByNickName("heichishuaduhuakongzhi")
						if heichishuaduhuakongzhi then
							heichishuaduhuakongzhi.SetCustomInteger1(1, 1)--破防成就没了
						end
						npc.AddBuff(npc.dwID, npc.nLevel, 10583, 2, 3600)
					end
				end
			end
		end
