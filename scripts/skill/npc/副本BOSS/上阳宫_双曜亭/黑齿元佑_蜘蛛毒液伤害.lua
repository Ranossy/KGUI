---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/上阳宫_双曜亭/黑齿元佑_蜘蛛毒液伤害.lua
-- 更新时间:	2016/4/6 10:14:05
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	...
	{nDamageBase = 7488, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 1560, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 20000, nDamageRand = 0, nCostMana = 0}, --level 4
	{nDamageBase = 11200, nDamageRand = 0, nCostMana = 0}, --level 5
	{nDamageBase = 4900, nDamageRand = 0, nCostMana = 0}, --level 6
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/上阳宫_双曜亭/黑齿元佑_蜘蛛毒液伤害.lua", -- 属性值1
		0															--属性值2
	);
	...
	skill.nMaxRadius		= 120 * LENGTH_BASE;		-- 技能施放的最大距离
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if scene.dwMapID == 241 or scene.dwMapID == 251 then
		if playertarget then
			if playertarget.GetBuff(10585, 1) then
				playertarget.DelBuff(10585, 1)
			else
		...
		if npctarget then
			if npctarget.GetBuff(10585, 1) then
				npctarget.DelBuff(10585, 1)
			else
	...
	if scene.dwMapID == 249 then
		if playertarget then
			if playertarget.GetBuff(10585, 2) then
				playertarget.DelBuff(10585, 2)
			else
		...
		if npctarget then
			if npctarget.GetBuff(10585, 2) then
				npctarget.DelBuff(10585, 2)
			else
