...
function GetSkillLevelData(skill)
	...
	"skill/npc/副本BOSS/唐门密室/汉唐_机甲损坏.lua",				-- 属性值1
	0														-- 属性值2
	);
	skill.AddAttribute(
...
local character = nil
	...
	if character then
		local scene = character.GetScene()
            ...
            scene.CreateNpc(15640,character.nX,character.nY,character.nZ,0,-1);                         
            elseif scene.dwMapID==125 then
            scene.CreateNpc(16014,character.nX,character.nY,character.nZ,0,-1);--此处写Boss名字+"_",只要加入名字,其他信息不变
	    	end
	end
end
