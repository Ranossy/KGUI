...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/持国回忆录/持国回忆录_青龙长老_毒啸_施放子弹.lua",								--属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 131 then
		...
		local npcYb = scene.CreateNpc(17701, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
		if npcYb then
			npcYb.RunTo(nX, nY)
		end
	elseif scene.dwMapID == 148 then
		for i = 1 ,3 do
			...
			local npcYb = scene.CreateNpc(18404, npc.nX, npc.nY, npc.nZ, npc.nFaceDirection, -1)
			if npcYb then
				npcYb.RunTo(nX, nY)
			end
		end
