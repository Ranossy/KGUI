...
function GetSkillLevelData(skill)
    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	skill.AddAttribute(
   		...
   		"skill/npc/副本BOSS/持国回忆录/持国回忆录_鼠王_疯狂车轮.lua",				-- 属性值1
   		0														-- 属性值2
   		);
	...
	----------------- BUFF相关 -------------------------------------------------
    --skill.BindBuff(1, 3516, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
         ...
         if cPlayer then
                   ...
                   local cNpc = scene.GetNpcByNickName("shuwang")

                   if cNpc and cNpc.nMoveState ~= MOVE_STATE.ON_DEATH then				
                            local nNow = cPlayer.nZ - cNpc.nZ

                            local nMax = 2 * 8 * 64

                            if nNow <= nMax then
																		
                            	cNpc.SetTarget(TARGET.PLAYER, cPlayer.dwID);
                                      if scene.dwMapID ==131 then
                                     		cNpc.CastSkill(3572, 1)
                                      elseif scene.dwMapID ==132 then
                               		cNpc.CastSkill(3572, 2)
                                     	end	
