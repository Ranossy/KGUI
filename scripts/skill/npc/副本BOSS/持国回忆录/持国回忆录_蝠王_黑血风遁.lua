...
function GetSkillLevelData(skill)
    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	skill.AddAttribute(
   		...
   		"skill/npc/副本BOSS/持国回忆录/持国回忆录_蝠王_黑血风遁.lua",				-- 属性值1
   		0														-- 属性值2
   	);
    ...
    --skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, BUFF_COMPARE_FLAG.Result, nLevel, BUFF_COMPARE_FLAG.Result);		-- 需求自身Buff
      skill.AddSlowCheckDestBuff(3527, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
    --skill.AddSlowCheckSelfOwnBuff(3527, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);	-- 需求自身属于自己的Buff
	...
	----------------- BUFF相关 -------------------------------------------------
    --skill.BindBuff(1, 3527, 1);		-- 设置1号位Buff
	--skill.BindBuff(2, 3527, 1);		-- 设置2号位Buff
...
function Apply(dwCharacterID)
	...
	--cPlayer.SetPosition(cPlayer.nX, cPlayer.nY, cPlayer.nZ + 4*8*64)
	cPlayer.Jump(true, 0)
	if scene.dwMapID == 131 then
		cPlayer.AddBuff(0, 99, 3527, 1)
	elseif scene.dwMapID == 148 then
		cPlayer.AddBuff(0, 99, 3527, 2)
	end
end
