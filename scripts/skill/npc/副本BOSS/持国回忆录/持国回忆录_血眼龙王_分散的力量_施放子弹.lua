...
function GetSkillLevelData(skill)
	...
	if dwSkillLevel == 1 then
		skill.AddAttribute(
			...
			"skill/npc/副本BOSS/持国回忆录/持国回忆录_血眼龙王_分散的力量_施放子弹.lua",								--属性值1
			0															--属性值2
		);
	elseif dwSkillLevel == 2 then
		skill.AddAttribute(
			ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK,			-- 属性作用模式
			ATTRIBUTE_TYPE.EXECUTE_SCRIPT,								-- 魔法属性
			"skill/npc/副本BOSS/持国回忆录/持国回忆录_血眼龙王_分散的力量_施放子弹_中.lua",								--属性值1
			0															--属性值2
		);
	elseif dwSkillLevel == 3 then
		skill.AddAttribute(
			ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK,			-- 属性作用模式
			ATTRIBUTE_TYPE.EXECUTE_SCRIPT,								-- 魔法属性
			"skill/npc/副本BOSS/持国回忆录/持国回忆录_血眼龙王_分散的力量_施放子弹_高.lua",								--属性值1
			0															--属性值2
		);
	end
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	tDirection[2] = npc.nFaceDirection - 90 % 256
	tDirection[3] = npc.nFaceDirection + 90 % 256
	tDirection[4] = npc.nFaceDirection + 180 % 256
	for i = 1 ,4 do
