...
tSkillData =
{
	{nDamage = 5400, nDamageRand = 270, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/持国回忆录/持国回忆录_蝠王_吸血牙.lua",								--属性值1
		0															--属性值2
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 3530, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID ==131 then
		if cNpc.nCurrentLife < cNpc.nMaxLife - 8000 then
			cNpc.nCurrentLife = cNpc.nCurrentLife + 8000
		else
			cNpc.nCurrentLife = cNpc.nMaxLife
		end
		if npcBoss.nMoveState ~= MOVE_STATE.ON_DEATH then
			if npcBoss.nCurrentLife < npcBoss.nMaxLife - 25000 then
				npcBoss.nCurrentLife = npcBoss.nCurrentLife + 25000
			else
				npcBoss.nCurrentLife = npcBoss.nMaxLife
			end
		end
	end
	if scene.dwMapID == 148 then
		if cNpc.nCurrentLife < cNpc.nMaxLife - 22500 then
			cNpc.nCurrentLife = cNpc.nCurrentLife + 22500
		else
			cNpc.nCurrentLife = cNpc.nMaxLife
		end
		if npcBoss.nMoveState ~= MOVE_STATE.ON_DEATH then
			if npcBoss.nCurrentLife < npcBoss.nMaxLife - 70000 then
				npcBoss.nCurrentLife = npcBoss.nCurrentLife + 70000
			else
				npcBoss.nCurrentLife = npcBoss.nMaxLife
			end
		end
