...
function GetSkillLevelData(skill)
    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	----------------- 魔法属性 ------------------------------------------------
   	   	...
   	   	skill.AddAttribute(
   			...
   			"skill/npc/副本BOSS/持国回忆录/持国回忆录_鼠王_鼠牙巢壳.lua",								--属性值1
   			0															--属性值2
   		);
	...
	----------------- BUFF相关 -------------------------------------------------
      skill.BindBuff(1, 4148, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	local nKungfuMountID = cPlayer.GetKungfuMount().dwSkillID
		if not nKungfuMountID then
			return
		end
	local nBuff = cPlayer.GetBuff(4156, 1)
	if not nBuff then
		return	
	end
	if nBuff.nStackNum > 54 then
		cPlayer.Die()
	end
	if nKungfuMountID == 10144 or nKungfuMountID == 10145 or nKungfuMountID == 10224 or nKungfuMountID == 10225 then
		cPlayer.nCurrentLife = cPlayer.nCurrentLife - 4700
	elseif cPlayer.nCurrentMana > 2500 then
			cPlayer.nCurrentMana = cPlayer.nCurrentMana - 2500
	else
			cPlayer.nCurrentLife = cPlayer.nCurrentLife - 4700
	end
end
