---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_听音商.lua
-- 更新时间:	2015/9/6 10:46:17
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_听音商.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		local buff = player.GetBuff(9657, 0)		--
		if buff then
			local nLevel = buff.nLevel
			player.DelBuff(9657, nLevel)
		end
	end
end
