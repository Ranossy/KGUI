---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_听音羽.lua
-- 更新时间:	2015/9/10 19:46:20
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_听音羽.lua",
		0
	);
	...
	skill.SetNormalCoolDown(1, 946);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		local buff = player.GetBuff(9660, 0)		--宫
		if buff then
			local nLevel = buff.nLevel
			player.DelBuff(9660, nLevel)
		end
	end
end

function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame)
	local player = GetPlayer(nnCharacterID)
	if not player then
		return
	end
	...
	local npc = scene.GetNpcByNickName("hanfeichi")
	if not npc then
		return
	end
	...
	if BuffID == 9647 then
		player.CustomDamage(npc.dwID, 14193, 1, 2, player.nMaxLife * 0.35)
	elseif BuffID == 9648 then
		player.CustomDamage(npc.dwID, 14194, 1, 2, player.nMaxLife * 0.35)
	elseif BuffID == 9649 then
		player.CustomDamage(npc.dwID, 14195, 1, 2, player.nMaxLife * 0.35)
	elseif BuffID == 9650 then
		player.CustomDamage(npc.dwID, 14196, 1, 2, player.nMaxLife * 0.35)
	elseif BuffID == 9651 then
		player.CustomDamage(npc.dwID, 14197, 1, 2, player.nMaxLife * 0.35)
	end
end
