---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_音缠.lua
-- 更新时间:	2015/10/12 17:05:27
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 2220, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 3700, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 3700, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 3900, nDamageRand = 0, nCostMana = 0}, --level 4
	{nDamageBase = 2220, nDamageRand = 0, nCostMana = 0}, --level 5
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_音缠.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(9664, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 9662, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		if tLevel[scene.dwMapID] then
			nID = tLevel[scene.dwMapID]
		end
		local buff = player.GetBuff(9662, 0)
		if buff then
			if buff.nStackNum >= 10 then
				player.AddBuff(0, 99, 9664, 1, 30)
				player.DelGroupBuff(9662, buff.nLevel)
				local scene = player.GetScene()
				if scene then
					local npchm = scene.CreateNpc(nID, player.nX, player.nY, player.nZ, 0, -1, "huanmoft"..player.dwID)
					if npchm then
						npchm.SetCustomUnsigned4(0, player.dwID)
					end
				end
			end
		end
