---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_问莲四式·映日.lua
-- 更新时间:	2015/6/2 9:45:15
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	...
	{nDamageBase = 7400, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 5300, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 8700, nDamageRand = 0, nCostMana = 0}, --level 4
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_问莲四式·映日.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID ~= 221 then
		if scene.dwMapID == 230 or scene.dwMapID == 233 then
			if not player.GetBuff(9126, 2) then
				--player.AddBuff(0, 99, 9126, 2)
			end
		end
	end

	if scene.dwMapID == 237 then
		if not player.GetBuff(9126, 4) then
			player.AddBuff(0, 99, 9126, 4)
		end
	end
end
