---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_落英神剑.lua
-- 更新时间:	2015/10/12 14:04:46
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 7500, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 13000, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 9900, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 13700, nDamageRand = 0, nCostMana = 0}, --level 4
	{nDamageBase = 7500, nDamageRand = 0, nCostMana = 0}, --level 5
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_落英神剑.lua",
		0
	);
	...
	----------------- BUFF相关 -------------------------------------------------
--	skill.BindBuff(1, 9652, nBuffLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		...
		if scene then
			...
			if npcboss then
				npcboss.nCurrentMana = npcboss.nCurrentMana + npcboss.nMaxMana * 0.04
				if player.nCurrentMana >= player.nMaxMana * 0.02 then
					player.nCurrentMana = player.nCurrentMana - player.nMaxMana * 0.02
				end
				if player.dwForceID == 21 or player.dwForceID == 8  then
					if player.nCurrentRage >= 0.05 * player.nMaxRage then
						player.nCurrentRage = player.nCurrentRage - 0.05 * player.nMaxRage
					end
			
				end
				--恢复机值
				if player.dwForceID == 7 then
					if player.nCurrentEnergy >= 0.05 * player.nMaxEnergy then
						player.nCurrentEnergy = player.nCurrentEnergy - 0.05 * player.nMaxEnergy
					end
				...
				if player.dwForceID == 10 then
					if player.nCurrentSunEnergy >= 0.05 * player.nMaxSunEnergy then
						player.nCurrentSunEnergy = player.nCurrentSunEnergy - 0.05 * player.nMaxSunEnergy
					end
					if player.nCurrentMoonEnergy >= 0.05 * player.nMaxMoonEnergy then
						player.nCurrentMoonEnergy = player.nCurrentMoonEnergy - 0.05 * player.nMaxMoonEnergy
					end
			
				end
				player.AddBuff(npcboss.dwID, npcboss.nLevel, 9652, nLevel)
				player.AddBuff(npcboss.dwID, npcboss.nLevel, 9652, nLevel)
				player.AddBuff(npcboss.dwID, npcboss.nLevel, 9652, nLevel)
			end
