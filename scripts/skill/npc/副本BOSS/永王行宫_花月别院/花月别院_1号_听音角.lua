---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_听音角.lua
-- 更新时间:	2015/9/10 19:48:00
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_听音角.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		local buff = player.GetBuff(9658, 0)		--宫
		if buff then
			local nLevel = buff.nLevel
			player.DelBuff(9658, nLevel)
		end
	end
end
