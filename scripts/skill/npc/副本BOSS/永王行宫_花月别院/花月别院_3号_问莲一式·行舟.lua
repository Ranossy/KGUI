---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_问莲一式·行舟.lua
-- 更新时间:	2015/5/29 15:38:55
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData = 
{
	{nDamageBase = 18000, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 20000, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 20000, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 25000, nDamageRand = 0, nCostMana = 0}, --level 4
...
function GetSkillLevelData(skill)
    ...
    skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_问莲一式·行舟.lua",								--属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function GetMinPlayer(cTarget, scene, nNum)
	local tMin = {}
		...
		for i = 1, #tPlayer do
			...
			for i = 1, #tPlayer do
				...
				if cPlayer and cPlayer.nMoveState ~= MOVE_STATE.ON_DEATH then
					local nSpace = GetSpace(cTarget, cPlayer)
					if not nMin then
						nMin = nSpace
						tempPlayer = cPlayer
					else
						if nSpace < nMin then
							tempMin[#tempMin + 1] = tempPlayer.dwID
							nMin = nSpace
							tempPlayer = cPlayer
						else
							tempMin[#tempMin + 1] = cPlayer.dwID
						end
					end
				end
			end
			tMin[#tMin + 1] = {nMin, tempPlayer}
			tPlayer = tempMin
		end
	end
	if nNum > #tMin then
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player then
		...
		scene.SetTimer(0.25 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_问莲一式·行舟.lua", dwSrcID, dwCharacterID)
	...
	if npctarget then
		...
		scene.SetTimer(0.25 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_问莲一式·行舟.lua", dwSrcID, dwCharacterID)
		npctarget.PlaySfx(553, npctarget.nX, npctarget.nY, npctarget.nZ + 500)
	end
end
...
function OnTimer(scene, nParam1, nParam2)
	if scene.dwMapID == 221 then
		local player = GetPlayer(nParam2)
		local npctarget = GetNpc(nParam2)
		--if not player then
			--return
		--end
		if player then
			local npc = GetNpc(nParam1)
			local tPlayer = GetMinPlayer(player, player.GetScene(), 2)
			if #tPlayer == 1 then
				npc.CastSkill(14447, 1, TARGET.PLAYER, player.dwID)
				return
			end
		
			if #tPlayer >= 2 then
				local playertarget = tPlayer[2][2]
				if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH then
					if GetCharacterDistance(player.dwID, playertarget.dwID) <= 256 then
						if npc then
							npc.CastSkill(13928, 1, TARGET.PLAYER, playertarget.dwID)
						end
					elseif GetCharacterDistance(player.dwID, playertarget.dwID) > 256 then
						if npc then
							npc.CastSkill(14447, 1, TARGET.PLAYER, player.dwID)
						end
					end
		...
		if npctarget then
			local npc = GetNpc(nParam1)
			local tPlayer = GetMinPlayer(npctarget, npctarget.GetScene(), 2)
			if #tPlayer >= 1 then
				local playertarget = tPlayer[1][2]
				if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH then
					if GetCharacterDistance(npctarget.dwID, playertarget.dwID) <= 256 then
						if npc then
							npc.CastSkill(13928, 1, TARGET.PLAYER, playertarget.dwID)
						end
					elseif GetCharacterDistance(npctarget.dwID, playertarget.dwID) > 256 then
						if npc then
							npc.CastSkill(14447, 1, TARGET.NPC, npctarget.dwID)
						end
					end
				end
			end
	...
	if scene.dwMapID == 230 or scene.dwMapID == 233 then
		...
		if player then
			...
			if #tPlayer == 1 then
				if npc then
					npc.CastSkill(15084, 2, TARGET.PLAYER, player.dwID)
				end
				return
			end
			...
			if #tPlayer >= 2 then
				...
				if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH then
					if GetCharacterDistance(player.dwID, playertarget.dwID) <= 256 then
						if npc then
							npc.CastSkill(13928, 2, TARGET.PLAYER, playertarget.dwID)
						end
					elseif GetCharacterDistance(player.dwID, playertarget.dwID) > 256 then
						if npc then
							npc.CastSkill(15084, 2, TARGET.PLAYER, player.dwID)
		...
		if npctarget then
			...
			if #tPlayer >= 1 then
				...
				if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH then
					...
					elseif GetCharacterDistance(npctarget.dwID, playertarget.dwID) > 256 then
						if npc then
							npc.CastSkill(15084, 2, TARGET.NPC, npctarget.dwID)
						end
					end
				end
			end
	...
	if scene.dwMapID == 235 then
		...
		if player then
			...
			local tPlayer = GetMinPlayer(player, player.GetScene(), 2)
			if #tPlayer == 1 then
				npc.CastSkill(14447, 3, TARGET.PLAYER, player.dwID)
				return
			end
			...
			if #tPlayer >= 2 then
				...
				if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH then
					if GetCharacterDistance(player.dwID, playertarget.dwID) <= 256 then
						if npc then
							npc.CastSkill(13928, 3, TARGET.PLAYER, playertarget.dwID)
						end
					elseif GetCharacterDistance(player.dwID, playertarget.dwID) > 256 then
						if npc then
							npc.CastSkill(14447, 3, TARGET.PLAYER, player.dwID)
		...
		if npctarget then
			...
			if #tPlayer >= 1 then
				...
				if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH then
					...
					elseif GetCharacterDistance(npctarget.dwID, playertarget.dwID) > 256 then
						if npc then
							npc.CastSkill(14447, 3, TARGET.NPC, npctarget.dwID)
						end
					end
				end
			end
	...
	if scene.dwMapID == 237 then
		...
		if player then
			...
			local tPlayer = GetMinPlayer(player, player.GetScene(), 2)
			if #tPlayer == 1 then
				npc.CastSkill(15084, 4, TARGET.PLAYER, player.dwID)
				return
			end
			...
			if #tPlayer >= 2 then
				...
				if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH then
					if GetCharacterDistance(player.dwID, playertarget.dwID) <= 256 then
						if npc then
							npc.CastSkill(13928, 4, TARGET.PLAYER, playertarget.dwID)
						end
					elseif GetCharacterDistance(player.dwID, playertarget.dwID) > 256 then
						if npc then
							npc.CastSkill(15084, 4, TARGET.PLAYER, player.dwID)
		...
		if npctarget then
			...
			if #tPlayer >= 1 then
				...
				if playertarget and playertarget.nMoveState ~= MOVE_STATE.ON_DEATH then
					...
					elseif GetCharacterDistance(npctarget.dwID, playertarget.dwID) > 256 then
						if npc then
							npc.CastSkill(15084, 4, TARGET.NPC, npctarget.dwID)
						end
					end
				end
			end
