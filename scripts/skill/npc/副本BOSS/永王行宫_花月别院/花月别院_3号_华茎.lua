---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_华茎.lua
-- 更新时间:	2015/6/1 15:00:12
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData = 
{
	...
	{nDamageBase = 2000, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 2000, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 3000, nDamageRand = 0, nCostMana = 0}, --level 4
...
function GetSkillLevelData(skill)
    ...
    skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_华茎.lua",								--属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	scene.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_华茎.lua", dwCharacterID, 1)
...
function OnTimer(scene, nParam1, nParam2)
	...
	player.PlaySfx(1203, player.nX, player.nY, player.nZ - 250)
	if scene.dwMapID == 221 then
		player.AddBuff(0, 99, 8970, 1, 6)
	end

	if scene.dwMapID == 230 or scene.dwMapID == 233 then
		player.AddBuff(0, 99, 8970, 2, 10)
	end

	if scene.dwMapID == 235 then
		player.AddBuff(0, 99, 8970, 3, 5)
	end
