---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_狂暴.lua
-- 更新时间:	2015/10/20 2:14:30
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if #tplayer > 0 then
		local player
		for i = 1, #tplayer do
			player = GetPlayer(tplayer[i])
			if player then
				player.AddBuff(0, 99, 9664, 1, 300)
				player.DelBuff(10038, 1)

				local npchm
				if scene.dwMapID == 230 then
					npchm = scene.CreateNpc(47148, player.nX, player.nY, player.nZ, 0, 300 * 28, "huanmoft"..player.dwID)
				elseif scene.dwMapID == 235 then
					npchm = scene.CreateNpc(48265, player.nX, player.nY, player.nZ, 0, 300 * 28, "huanmoft" .. player.dwID)
				elseif scene.dwMapID == 237 then
					npchm = scene.CreateNpc(48731, player.nX, player.nY, player.nZ, 0, 300 * 28, "huanmoft"..player.dwID)
				end
				if npchm then
