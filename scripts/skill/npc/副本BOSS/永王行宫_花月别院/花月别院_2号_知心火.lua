---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_2号_知心火.lua
-- 更新时间:	2015/5/18 16:29:12
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 2000, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 3000, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 3000, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 3500, nDamageRand = 0, nCostMana = 0}, --level 4
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_2号_知心火.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 221 then---10人花月
		if not player.GetBuff(8896, 1) then
			player.AddBuff(0, 99, 8896, 1, 15)
		end
	end

	if scene.dwMapID == 230 or scene.dwMapID == 233 then-----25人和帮会花月
		if not player.GetBuff(8896, 2) then
			player.AddBuff(0, 99, 8896, 2, 15)
		end
	end

	if scene.dwMapID == 235 then-----10人挑战
		if not player.GetBuff(8896, 3) then
			player.AddBuff(0, 99, 8896, 3, 15)
		end
	end

	if scene.dwMapID == 237 then-----25人挑战
		if not player.GetBuff(8896, 4) then
			player.AddBuff(0, 99, 8896, 4, 15)
		end

		if player.GetBuff(8896, 4) then
			if player.GetBuff(8896, 4).nStackNum < 2 then
				player.AddBuff(0, 99, 8896, 4, 15)
			end
		end
	end
