---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_问莲七式·踏歌.lua
-- 更新时间:	2015/6/8 11:31:58
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_问莲七式·踏歌.lua", --属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(8985, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 221 then
		player.AddBuff(0, 99, 9387, 1)
	end

	if scene.dwMapID == 230 or scene.dwMapID == 233 then
		player.AddBuff(0, 99, 9387, 2)
	end

	if scene.dwMapID == 235 then
		player.AddBuff(0, 99, 9387, 3)
	end

	if scene.dwMapID == 237 then
		player.AddBuff(0, 99, 9387, 4)
	end
	--local scene = player.GetScene()
