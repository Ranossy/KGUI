---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_剑气旋窝加buff.lua
-- 更新时间:	2015/6/2 16:54:29
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 20400, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 48900, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 60000, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 70000, nDamageRand = 0, nCostMana = 0}, --level 4
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_剑气旋窝加buff.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player then
		if scene.dwMapID == 230 or scene.dwMapID == 233 then
			player.AddBuff(0, 99, 8985, 2, 1)
		end

		if scene.dwMapID == 235 then
			player.AddBuff(0, 99, 8985, 3, 1)
		end

		if scene.dwMapID == 237 then
			player.AddBuff(0, 99, 8985, 4, 1)
		end
	end

	if npc then
		local num = npc.GetCustomInteger1(1)
		num = num + 1
		npc.SetCustomInteger1(1, num)
	end
end
