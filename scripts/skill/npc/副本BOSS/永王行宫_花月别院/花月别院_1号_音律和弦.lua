---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_音律和弦.lua
-- 更新时间:	2015/9/18 22:23:24
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 5200, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 9800, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 19200, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 6800, nDamageRand = 0, nCostMana = 0}, --level 4
	{nDamageBase = 16100, nDamageRand = 0, nCostMana = 0}, --level 5
	{nDamageBase = 29400, nDamageRand = 0, nCostMana = 0}, --level 6
	{nDamageBase = 6800, nDamageRand = 0, nCostMana = 0}, --level 7
	{nDamageBase = 13000, nDamageRand = 0, nCostMana = 0}, --level 8
	{nDamageBase = 25400, nDamageRand = 0, nCostMana = 0}, --level 9
	{nDamageBase = 7200, nDamageRand = 0, nCostMana = 0}, --level 10
	{nDamageBase = 17000, nDamageRand = 0, nCostMana = 0}, --level 11
	{nDamageBase = 31000, nDamageRand = 0, nCostMana = 0}, --level 12
	{nDamageBase = 1110, nDamageRand = 0, nCostMana = 0}, --level 13
	{nDamageBase = 2110, nDamageRand = 0, nCostMana = 0}, --level 14
	{nDamageBase = 3110, nDamageRand = 0, nCostMana = 0}, --level1 5
	...
	{nDamageBase = 0, nDamageRand = 0, nCostMana = 0}, --level1 9
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		ATTRIBUTE_TYPE.SKILL_LUNAR_DAMAGE, -- 魔法属性
	...
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK, -- 属性作用模式
		ATTRIBUTE_TYPE.SKILL_LUNAR_DAMAGE_RAND, -- 魔法属性
	...
	skill.AddAttribute(
		...
		ATTRIBUTE_TYPE.CALL_LUNAR_DAMAGE, -- 魔法属性
		0, -- 属性值1
		0											-- 属性值2
	);
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_音律和弦.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 9652, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		...
		if scene then
			...
			if tLevel[scene.dwMapID] then
				nLevel = tLevel[scene.dwMapID]
			end
			local npcboss = scene.GetNpcByNickName("hanfeichi")
			if npcboss then
				if npcboss.GetCustomUnsigned1(9) == 3  then
					player.nCurrentMana = math.max(0,player.nCurrentMana - player.nMaxMana * 0.2)
				end
				if npcboss.GetCustomUnsigned1(9) == 2 and nLevel == 4  then
					player.nCurrentMana = math.max(0,player.nCurrentMana - player.nMaxMana * 0.2)
				end
				if npcboss.GetCustomUnsigned1(9) == 1 and nLevel == 1 then
					player.nCurrentMana = math.max(0,player.nCurrentMana + player.nMaxMana * 0.1)
				end
			end
		end
	end
