---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_协奏曲本体.lua
-- 更新时间:	2015/9/12 23:25:51
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_1号_协奏曲本体.lua",
		0
	);
	...
	skill.nAreaRadius = 216/64 * LENGTH_BASE;        -- 技能起始作用环的外圈半径
	...
	skill.nSpreadCircleVelocity = 160/6; --环每帧运动点数，4就是1尺/秒，可以为负的，负表示朝内收缩
	skill.nSpreadCircleTotalFrame = 6 * GLOBAL.GAME_FPS;  --总的释放帧数
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npchy = scene.GetNpcByNickName("hanfeichihy")
	if not npcboss then
		return
	end
	local DXflag = false
	if npcboss and npchy then
		local disboss = GetCharacterDistance(player.dwID, npcboss.dwID)
		local dishy = GetCharacterDistance(player.dwID, npchy.dwID)
		local nabs = math.abs(disboss - dishy)
		for i = 1, 5 do
			if math.abs(nabs - (i-1) * 40 * 64 / 6 * 2) <= 2 * 64 then
				DXflag = true
				break
			end
		end
	...
	if DXflag then
		player.PlaySfx(1201, 0, 0, 0)
		if nLevel > 2 then
			npcboss.CastSkillXYZ(14573, nLevel, player.nX, player.nY, player.nZ)
		end
		player.nCurrentMana = math.min(player.nMaxMana,player.nCurrentMana + 0.02 * player.nMaxMana)
	else
		if nLevel> 2 then
			npcboss.CastSkillXYZ(14573, nLevel, player.nX, player.nY, player.nZ)
		end
		player.DelBuff(10037,1)
		npcboss.CastSkill(14571, nLevel, TARGET.PLAYER, player.dwID)
		player.nCurrentMana = math.max(0,player.nCurrentMana - 0.05 * player.nMaxMana)
		if player.nCurrentMana < 0.2 * player.nMaxMana then
			npcboss.CastSkill(14571, nLevel, TARGET.PLAYER, player.dwID)
		end
	end
end
