---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_黄色射线.lua
-- 更新时间:	2015/6/2 11:42:24
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/永王行宫_花月别院/花月别院_3号_黄色射线.lua", --属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(8983, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 221 then
		if npctarget and npctarget.dwTemplateID == 43275 then
			...
			if not npctarget.GetBuff(9019, 1) then
				npctarget.AddBuff(0, 99, 8973, 1, 30)
				npctarget.AddBuff(0, 99, 9019, 1)
			end
		end
		...
		if npctarget and npctarget.dwTemplateID ~= 43275 then
			npctarget.Die()
			--local scene = npctarget.GetScene()
			npctarget.AddBuff(0, 99, 8982, 1)
			if not npctarget.GetBuff(9019, 1) then
				npctarget.AddBuff(0, 99, 8981, 1, 10)
				npctarget.AddBuff(0, 99, 9019, 1)
		...
		if player then
			...
			player.AddBuff(0, 99, 8982, 1)
			if not player.GetBuff(9019, 1) then
				player.AddBuff(0, 99, 8981, 1, 10)
				player.AddBuff(0, 99, 9019, 1)
				local npc = GetNpc(dwSrcID)
				if npc then
					if scene.dwMapID ~= 221 then
	...
	if scene.dwMapID == 230 then
		if npctarget and npctarget.dwTemplateID == 47097 then
			...
			if not npctarget.GetBuff(9019, 2) then
				npctarget.AddBuff(0, 99, 8973, 2, 30)
				npctarget.AddBuff(0, 99, 9019, 2)
			end
		end
		...
		if npctarget and npctarget.dwTemplateID ~= 47097 then
			npctarget.Die()
			--local scene = npctarget.GetScene()
			npctarget.AddBuff(0, 99, 8982, 2)
			if not npctarget.GetBuff(9019, 2) then
				npctarget.AddBuff(0, 99, 8981, 2, 10)
				npctarget.AddBuff(0, 99, 9019, 2)
				local npc = GetNpc(dwSrcID)
				if npc then
					if scene.dwMapID ~= 221 then
		...
		if player then
			--local scene = player.GetScene()
			player.AddBuff(0, 99, 8982, 2)
			if not player.GetBuff(9019, 2) then
				player.AddBuff(0, 99, 8981, 2, 10)
				player.AddBuff(0, 99, 9019, 2)
				local npc = GetNpc(dwSrcID)
				if npc then
					if scene.dwMapID ~= 221 then
	...
	if scene.dwMapID == 235 then
		if npctarget and npctarget.dwTemplateID == 48214 then
			...
			if not npctarget.GetBuff(9019, 3) then
				npctarget.AddBuff(0, 99, 8973, 3, 30)
				npctarget.AddBuff(0, 99, 9019, 3)
			end
		end
		...
		if npctarget and npctarget.dwTemplateID ~= 48214 then
			npctarget.Die()
			--local scene = npctarget.GetScene()
			npctarget.AddBuff(0, 99, 8982, 3)
			if not npctarget.GetBuff(9019, 3) then
				npctarget.AddBuff(0, 99, 8981, 3, 10)
				npctarget.AddBuff(0, 99, 9019, 3)
				local npc = GetNpc(dwSrcID)
				if npc then
					if scene.dwMapID ~= 221 then
		...
		if player then
			--local scene = player.GetScene()
			player.AddBuff(0, 99, 8982, 3)
			if not player.GetBuff(9019, 3) then
				player.AddBuff(0, 99, 8981, 3, 10)
				player.AddBuff(0, 99, 9019, 3)
				local npc = GetNpc(dwSrcID)
				if npc then
					if scene.dwMapID ~= 221 then
						--npc.CastSkill(14296, 1, TARGET.PLAYER, player.dwID)
	...
	if scene.dwMapID == 237 then
		if npctarget and npctarget.dwTemplateID == 48680 then
			...
			if not npctarget.GetBuff(9019, 4) then
				npctarget.AddBuff(0, 99, 8973, 4, 30)
				npctarget.AddBuff(0, 99, 9019, 4)
			end
		end
		...
		if npctarget and npctarget.dwTemplateID ~= 48680 then
			npctarget.Die()
			--local scene = npctarget.GetScene()
			npctarget.AddBuff(0, 99, 8982, 4)
			if not npctarget.GetBuff(9019, 4) then
				npctarget.AddBuff(0, 99, 8981, 4, 10)
				npctarget.AddBuff(0, 99, 9019, 4)
				local npc = GetNpc(dwSrcID)
				if npc then
					if scene.dwMapID ~= 221 then
		...
		if player then
			--local scene = player.GetScene()
			player.AddBuff(0, 99, 8982, 4)
			if not player.GetBuff(9019, 4) then
				player.AddBuff(0, 99, 8981, 4, 10)
				player.AddBuff(0, 99, 9019, 4)
				local npc = GetNpc(dwSrcID)
				if npc then
					if scene.dwMapID ~= 221 then
						--npc.CastSkill(14296, 1, TARGET.PLAYER, player.dwID)
