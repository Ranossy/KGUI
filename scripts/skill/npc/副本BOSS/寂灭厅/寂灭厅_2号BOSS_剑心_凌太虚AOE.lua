...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
   			...
   			"skill/npc/副本BOSS/寂灭厅/寂灭厅_2号BOSS_剑心_凌太虚AOE.lua",								--属性值1
   			0															--属性值2
   	);
...
function Apply(dwCharacterID)
	if IsPlayer(dwCharacterID) then
		...
		local npcTarget = scene.GetNpcByNickName("lingtaixu")
		if npcTarget then
			npcTarget.SetTarget(TARGET.PLAYER, player.dwID)
			if scene.dwMapID == 110 then
				npcTarget.CastSkill(3008, 1)
			else
				npcTarget.CastSkill(3008, 2)
			end
		end
	else
		...
		if npc.dwTemplateID == 11149 or npc.dwTemplateID == 13924 then
			npc.AddBuff(0, 99, 2886, 1)
		end
	end
