   ...
   ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK,                        
	...
	skill.AddAttribute(                                                           
  	...
  	skill.AddAttribute(
        ...
        "skill/npc/副本BOSS/毒神殿/毒神殿_鲍穆侠_乙毒·蜈蚣.lua", 			
        0
    );
...
function Apply(dwCharacterID)
         ...
         if player then
					...
					scene.SetCustomBoolean(SCENE_CUSTOM_VALUE_NAME.DUSHENDIAN.BuZhong_WuGong, true)
         			--<<
                   local MaxMP = player.nMaxMana
                   local CurrentMP = player.nCurrentMana
                   if CurrentMP > 100 then
                   	player.nCurrentMana = CurrentMP - MaxMP * 0.1
                   end
         end
end
