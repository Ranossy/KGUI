---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/引仙水榭/李复_流风五连斩.lua
-- 更新时间:	2015/6/11 20:04:15
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/引仙水榭/李复_流风五连斩.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcziji = GetNpc(dwSkillSrcID)
	if not npcziji then
		return
	end
	local scene = npcziji.GetScene()
	if not scene then
		return
	end
	local nLevel = 1
	...
	if npc then
		buff = npc.GetBuff(9032, nLevel)
		if buff then
			num = buff.nStackNum
		end
		if npc.szName == "litan" then
			local scene = npc.GetScene()
			if not scene then
				return
			end
			local npczk = scene.GetNpcByNickName("llzk")
			if npczk then
				npczk.SetCustomBoolean(3, true)
			end
			npc.nCurrentLife = npc.nCurrentLife - npc.nMaxLife * 0.06 * (1 + 0.35 * num)
			npc.AddBuff(0, 99, 9032, nLevel)		
			npcziji.SetDisappearFrames(1)
		end
	end
	
	if player then
	--	print(player.szName)
		npcziji.CastSkillXYZ(13969, nLevel, player.nX,player.nY,player.nZ)
		npcziji.SetDisappearFrames(1)
	end
end
