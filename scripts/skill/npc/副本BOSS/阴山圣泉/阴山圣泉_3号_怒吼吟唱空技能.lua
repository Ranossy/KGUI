---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/阴山圣泉/阴山圣泉_3号_怒吼吟唱空技能.lua
-- 更新时间:	2015/3/25 19:49:40
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 900, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 2600, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/阴山圣泉/阴山圣泉_3号_怒吼吟唱空技能.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID)
	...
	if scene.dwMapID == 204 then
		if not player.GetBuff(8834, 1) then
			player.AddBuff(0, 99, 8834, 1, 3)
		end
	end

	if scene.dwMapID == 227 then
		if not player.GetBuff(8834, 2) then
			player.AddBuff(0, 99, 8834, 2, 3)
		end
	end
end
