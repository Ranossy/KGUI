---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/挖宝5人副本/召唤景.lua
-- 更新时间:	2014/7/8 20:49:19
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
			...
			"skill/npc/副本BOSS/挖宝5人副本/召唤景.lua",
			0
		);
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if player.dwWaistItemIndex ~= 11774 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND 
    	return false
  	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	if not player.GetScene().GetNpcByNickName(szEnemyNickName) then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(6, 7001) .. tNpcName[15419])
		return
	end
	QiXiCreatAllPartner(player, 15419);
end
