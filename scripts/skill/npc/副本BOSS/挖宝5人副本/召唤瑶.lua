---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/挖宝5人副本/召唤瑶.lua
-- 更新时间:	2014/7/8 20:49:24
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
----------------------------------------------------------------------<
...
Include("scripts/Map/节日七夕/include/QiXi_RanQuest.lua")
tSkillData =
{
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
			...
			"skill/npc/副本BOSS/挖宝5人副本/召唤瑶.lua",
			0
		);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7929, 1);		-- 设置1号位Buff
	...
	skill.SetNormalCoolDown(1,  642);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if player.dwWaistItemIndex ~= 11773 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND 
    return false
  end
	return nPreResult;
end
...
local tNpcName = {
		---阿妮的承诺
		[15362] = GetEditorString(6, 6999),
		---阿景的承诺
		[15419] = GetEditorString(2, 9933),
		----姬黄泉的承诺
		[16568] = GetEditorString(2, 9934),
		--瑶瑶的承诺
		[19729] = GetEditorString(6, 7000),
		}
	local player = GetPlayer(dwCharacterID);
	if not player then
		return
	end
	local szEnemyNickName = "QiXiRanQuest" .. player.dwID;
	if not player.GetScene().GetNpcByNickName(szEnemyNickName) then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(6, 7001) .. tNpcName[19729])
		return
	end
	QiXiCreatAllPartner(player, 19729);
end
