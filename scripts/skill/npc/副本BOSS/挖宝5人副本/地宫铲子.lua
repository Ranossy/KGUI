---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/挖宝5人副本/地宫铲子.lua
-- 更新时间:	2015/10/28 11:26:56
-- 更新用户:	linjiajia2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
			...
			"skill/npc/副本BOSS/挖宝5人副本/地宫铲子.lua",
			0
		);
	----------------- 魔法属性 -------------------------------------------------
	--[[
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetFreeRoomSize() < 1 then
		player.SendSystemMessage(GetEditorString(8, 8183))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(8, 8183))
		return
	end
	local tReward = 
	{
	...
	local nR = Random(10000) / 10000
	local nRate = 0
	local bNothing = true
	for i = 1, #tReward do
		if nR <= tReward[i][1] + nRate then
			if player.GetFreeRoomSize() >= 1 and not player.GetBuff(6698, 1)then
				player.DoAction(player.dwID, 20913)
				player.PlaySfx(1, player.nX, player.nY, player.nZ)
				player.AddItem(5, tReward[i][2], 1)
				player.AddBuff(0, 99, 6698, 1, 22 * 60)
				bNothing = false
			end
			break
		else
			nRate=nRate+tReward[i][1]
		end
	end
	player.SetDynamicSkillGroup(0)
	if bNothing then
		player.SendSystemMessage(GetEditorString(1, 2675))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(1, 2675))
	end
end
