---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/流离岛/流离岛挥舞火把.lua
-- 更新时间:	2013/5/14 20:21:34
-- 更新用户:	xiedixun2
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/流离岛/流离岛挥舞火把.lua", -- 属性值1
		0														-- 属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID)
	...
	local n = scene.GetCustomUnsigned1(8)
	
	if npc.dwTemplateID == 24586 then
		n = n + 1
		scene.SetCustomUnsigned1(8, n)
		if npc then
		...
		if n <= 11 then
			...
			for i = 1, #playerlist do
				...
				RemoteCallToClient(playerlist[i], "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(5, 6902) .. n .. "/11", 4)
				playertarget.SendSystemMessage(GetEditorString(5, 6902) .. n .. "/11")
			end
		end
	elseif  npc.dwTemplateID == 24667 then
		--驱散BOSS战中的雾气。
		local a = scene.GetCustomInteger4(11)
		a = a + 1
		scene.SetCustomInteger4(11, a)    --记录驱散数量,与成就相关
	...
	if n >= 11 then    --消灭一定数量后给总控发事件，开启剧情
							
		local ZONGKONG = scene.GetNpcByNickName("juqingzongkong")
		if not ZONGKONG then
			--print("没有剧情总控")
			return				
		end
		if scene.GetCustomBoolean(1) then
			return 
		else
			scene.SetCustomBoolean(1, true)
			ZONGKONG.FireAIEvent(2001, 0, 0)     --一号的剧情开关
		end
		...
		for i = 1, #playerlist do
			local playertarget = GetPlayer(playerlist[i])
			RemoteCallToClient(playerlist[i], "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(5, 4776), 4)
			playertarget.SendSystemMessage(GetEditorString(5, 4776))
		end
	end
