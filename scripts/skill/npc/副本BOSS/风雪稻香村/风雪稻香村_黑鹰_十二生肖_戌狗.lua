---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_黑鹰_十二生肖_戌狗.lua
-- 更新时间:	2014/2/8 17:05:54
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
tSkillData = 
{
	{nDamageBase = 16800, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_黑鹰_十二生肖_戌狗.lua", --属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 7409, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSrcID)
	...
	local scene = npccaster.GetScene()
	if not scene then
		return
	end
	if scene.dwMapID == 177 then
		local buff = playerthis.GetBuff(7409, 1)
		if buff then
			if buff.nStackNum >= 4 then
				npccaster.CastSkill(9065, 1)
			end
		end
	else
		local buff = playerthis.GetBuff(7409, 2)
		if buff then
			if buff.nStackNum >= 4 then
				npccaster.CastSkill(9065, 1)
			end
