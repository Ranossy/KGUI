---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_小怪关卡_特殊腰坠使用.lua
-- 更新时间:	2014/7/17 20:04:51
-- 更新用户:	wangpengfei1
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 50000, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 35000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_小怪关卡_特殊腰坠使用.lua", --属性值1
		0															--属性值2
	);
	...
	skill.SetNormalCoolDown(1, 736);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	local nGua, nNum = GetItemIndex_QuestKey_Gua(player)
	if nGua and nNum then
		local npcHY = scene.GetNpcByNickName("baguahuanying_" .. nNum)
		if npcHY then
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_YELLOW", GetEditorString(6, 7133), 4)
			return false
		end
	else
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_YELLOW", GetEditorString(6, 7134), 4)
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if nGua and nNum then
		local npcHY = scene.GetNpcByNickName("baguahuanying_" .. nNum)
		local npcGJ = scene.GetNpcByNickName("guajinbiaoxian_" .. nNum)
		if not npcHY and npcGJ then
			scene.CreateNpc(36941, npcGJ.nX, npcGJ.nY, npcGJ.nZ, npcGJ.nFaceDirection, -1, "baguahuanying_" .. nNum)
			return
		else
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_YELLOW", GetEditorString(6, 7133), 4)
			--player.SetTimer(2 * GLOBAL.GAME_FPS, "skill/npc/副本BOSS/风雪稻香村/风雪稻香村_小怪关卡_特殊腰坠使用.lua", 0, 0)
			--player.ClearCDTime(329)
			return
		end
	else
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_YELLOW", GetEditorString(6, 7134), 4)	
...
function GetItemIndex_QuestKey_Gua(player)
	local tDataGua_SuanMing = {
		{0, 0, 0, 1},--天
		{0, 0, 1, 8},--泽
		{0, 1, 0, 4},--火
		{0, 1, 1, 6},--雷
	...
	local bYao_r = 0
	local bYao_d = 0
	if player.GetQuestPhase(nG_t) ~= 0 then
		bYao_t = 1
	end
	if player.GetQuestPhase(nG_r) ~= 0 then
		bYao_r = 1
	end
	if player.GetQuestPhase(nG_d) ~= 0 then
		bYao_d = 1
	end

	for i = 1, #tDataGua_SuanMing do
		local tData = tDataGua_SuanMing[i]
		if tData[1] == bYao_t and tData[2] == bYao_r and tData[3] == bYao_d then
			return i, tData[4]
		end
	end
	return nil, nil
