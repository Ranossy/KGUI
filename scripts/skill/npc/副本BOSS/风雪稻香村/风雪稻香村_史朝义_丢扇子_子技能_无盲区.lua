---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_史朝义_丢扇子_子技能_无盲区.lua
-- 更新时间:	2014/2/21 15:13:17
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData = 
{
	{nDamageBase = 26400, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 34800, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_史朝义_丢扇子_子技能_无盲区.lua", --属性值1
		0															--属性值2
	);	
	
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(7418, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	skill.AddSlowCheckDestBuff(7452, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID)
	...
	playerthis.AddBuff(0, 99, 7411, 1)		--跳舞失败
	if not playerthis.GetBuff(7452, 1) then
		playerthis.AddBuff(0, 99, 7415, 1)		--易伤
	end
	playerthis.AddBuff(0, 99, 7413, 1)		--致死
	if playerthis.GetBuff(7414, 1) then	--免致死	
		playerthis.DelGroupBuff(7414, 1)
	end
	if playerthis.GetBuff(7412, 1) then	--跳舞成功
		playerthis.DelGroupBuff(7412, 1)
	end
	if playerthis.GetBuff(7757, 1) then
		playerthis.DelBuff(7757, 1)
	end
end
