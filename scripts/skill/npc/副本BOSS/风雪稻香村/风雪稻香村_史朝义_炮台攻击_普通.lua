---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_史朝义_炮台攻击_普通.lua
-- 更新时间:	2014/3/11 17:02:49
-- 更新用户:	wangpengfei1
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 80000, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 100000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_史朝义_炮台攻击_普通.lua", --属性值1
		0															--属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(7614, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	--skill.SetNormalCoolDown(1, 634);	--技能普通CD
...
function Apply(dwCharacterID, SrcID)
	local playercaster = GetPlayer(SrcID)
	...
	local nNum = SkillDiff(playercaster)
	local buffthis = playercaster.GetBuff(7450, 1)
	local buffthis1 = playercaster.GetBuff(7753, 1)
	if not buffthis1 then
		return
	end
	--print(buffthis1.nStackNum)
	if buffthis then
		local nLevel = math.floor(buffthis.nStackNum / 3) + 1
		if nLevel >= 10 then
			nLevel = 10
		end
		for i = 1, buffthis1.nStackNum do
			playercaster.CastSkill(tSkills[nNum], nLevel)
		end
	else
		for i = 1, buffthis1.nStackNum do
			playercaster.CastSkill(tSkills[nNum], 1)
		end
	end
	playercaster.DelGroupBuff(7753, 1)
end
