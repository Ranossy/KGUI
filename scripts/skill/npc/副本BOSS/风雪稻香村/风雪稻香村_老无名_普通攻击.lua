...
tSkillData =
{
	{nDamageBase = 75600, nDamageRand = 0, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_老无名_普通攻击.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 177 then
		if npc.GetBuff(6640, 1) then
			player.AddBuff(0,99,6641,1)
		end
		if npc.GetBuff(6642, 1) then
			player.AddBuff(0,99,6650,1)
		end
	else
		if npc.GetBuff(6640, 1) then
			player.AddBuff(0,99,6641,2)
		end
		if npc.GetBuff(6642, 1) then
			player.AddBuff(0,99,6650,2)
		end
	end
end
