---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_史朝义_丢扇子_母技能.lua
-- 更新时间:	2014/2/21 14:55:14
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
tSkillData = 
{
	{nDamageBase = 2200, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 11600, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_史朝义_丢扇子_母技能.lua", --属性值1
		0															--属性值2
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7365, dwSkillLevel);		-- 设置1号位Buff
	...
	----------------- 时间相关 -------------------------------------------------
	--skill.nPrepareFrames = 4 * GLOBAL.GAME_FPS;				-- 吟唱帧数
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if playertarget.GetBuff(7452, 1) and not playertarget.GetBuff(7418, 1) then
		local npcZK = scene.GetNpcByNickName("cikeshuabingkongzhi")
		if not npcZK then
			return	
		end
		local nNum = npcZK.GetCustomInteger1(2)
		nNum = nNum + 1
		local npcCK = nil
		if scene.dwMapID == 177 then
			npcCK = scene.CreateNpc(27672, playertarget.nX, playertarget.nY, playertarget.nZ, 0, -1, "zhuijicike" .. nNum)
		else
			npcCK = scene.CreateNpc(36361, playertarget.nX, playertarget.nY, playertarget.nZ, 0, -1, "zhuijicike" .. nNum)
		end
		npcZK.SetCustomInteger1(2, nNum)
		if npcCK then
			npcCK.SetCustomInteger4(1, playertarget.dwID)	
		end
		playertarget.DelBuff(7452, 1)
	end
end
