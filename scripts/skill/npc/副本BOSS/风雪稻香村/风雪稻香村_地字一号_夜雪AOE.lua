---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_地字一号_夜雪AOE.lua
-- 更新时间:	2013/12/12 15:24:21
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_地字一号_夜雪AOE.lua", --属性值1
		0															--属性值2
	);	
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 7287, dwSkillLevel);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npccaster = GetNpc(dwSkillSrcID)
	if not npccaster then
		return
	end
	...
	if playerthis.GetBuff(7295, 1) or playerthis.GetBuff(7296, 1) or playerthis.GetBuff(7295, 2) or playerthis.GetBuff(7296, 2) then
		if scene.dwMapID == 177 then
			playerthis.CustomDamage(npccaster.dwID, 8663, 2, 0, 38900*0.14)
		else
			playerthis.CustomDamage(npccaster.dwID, 8663, 2, 0, 48200*0.14)
		end
		return
	end
	if scene.dwMapID == 177 then
		local buffthis = playerthis.GetBuff(7287, 1)
		if buffthis then
			if buffthis.nStackNum == 10 then
				playerthis.Die()
			end
		end
	else
		local buffthis = playerthis.GetBuff(7287, 2)
		if buffthis then
			if buffthis.nStackNum == 10 then
				playerthis.Die()
			end
