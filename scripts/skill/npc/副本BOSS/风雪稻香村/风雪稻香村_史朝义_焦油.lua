---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_史朝义_焦油.lua
-- 更新时间:	2014/2/10 15:59:45
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_史朝义_焦油.lua", --属性值1
		0															--属性值2
	);
	
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(7643, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if playerthis then
		playerthis.AddBuff(0, 99, 7441, 1)
		local buffthis = playerthis.GetBuff(7441, 1)
		if buffthis then
			if buffthis.nStackNum >= 10 then
				playerthis.Stop()
				playerthis.AddBuff(0, 99, 7445, 1)
			end
		end
	else
		local npcthis = GetNpc(dwCharacterID)
		if npcthis then
			npcthis.AddBuff(0, 99, 7441, 1)
			local buffthis = npcthis.GetBuff(7441, 1)
			if buffthis then
				if buffthis.nStackNum >= 10 then
					npcthis.AddBuff(0, 99, 7445, 1)
				end
			end
		end
	end
