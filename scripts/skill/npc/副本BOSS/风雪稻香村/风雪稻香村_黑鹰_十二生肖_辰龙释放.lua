---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_黑鹰_十二生肖_辰龙释放.lua
-- 更新时间:	2014/1/24 14:55:44
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
tSkillData = 
{
	...
	{nDamageBase = 150000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_黑鹰_十二生肖_辰龙释放.lua",
		0
	);	
...
function Apply(dwCharacterID, dwSrcID)
	...
	if scene.dwMapID == 177 then
		local nRandom = Random(1, 256)
		local x1 = npcthis.nX + math.cos(nRandom * math.pi / 128) * 64 * 40
		local y1 = npcthis.nY + math.sin(nRandom * math.pi / 128) * 64 * 40
		local x2 = npcthis.nX + math.cos(nRandom * math.pi / 128 + math.pi / 2) * 64 * 40
		local y2 = npcthis.nY + math.sin(nRandom * math.pi / 128 + math.pi / 2) * 64 * 40
		local npcLX1 = scene.CreateNpc(27640, npcthis.nX, npcthis.nY, npcthis.nY, 0, 10 * GLOBAL.GAME_FPS, "longxingqijin_1")
		local npcLX2 = scene.CreateNpc(27640, npcthis.nX, npcthis.nY, npcthis.nY, 0, 10 * GLOBAL.GAME_FPS, "longxingqijin_2")
		if npcLX1 then
			npcLX1.RunTo(x1, y1)
		end
		if npcLX2 then
		...
		local x2 = npcthis.nX + math.cos(nRandom * math.pi / 128 + math.pi / 2) * 64 * 40
		local y2 = npcthis.nY + math.sin(nRandom * math.pi / 128 + math.pi / 2) * 64 * 40
		local npcLX1 = scene.CreateNpc(36353, npcthis.nX, npcthis.nY, npcthis.nY, 0, 10 * GLOBAL.GAME_FPS, "longxingqijin_1")
		local npcLX2 = scene.CreateNpc(36353, npcthis.nX, npcthis.nY, npcthis.nY, 0, 10 * GLOBAL.GAME_FPS, "longxingqijin_2")
		if npcLX1 then
			npcLX1.RunTo(x1, y1)
		end
		if npcLX2 then
