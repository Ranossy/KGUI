---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_史朝义_牛车战斗.lua
-- 更新时间:	2014/2/17 15:00:16
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_史朝义_牛车战斗.lua", --属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID, dwSrcID)
	...
	if npcthis.dwTemplateID == 26559 then
		local tplayert = scene.GetAllPlayer()
		if tplayert then
			for i = 1, #tplayert do
				local playerthis = GetPlayer(tplayert[i])
				if playerthis then
