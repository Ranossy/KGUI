---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_史朝义_地刺投掷.lua
-- 更新时间:	2014/3/31 16:27:43
-- 更新用户:	wangpengfei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_史朝义_地刺投掷.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(7694, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL)		-- 需求自身Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nx, ny, nz = playerthis.GetCoordinateSkillTarget()
	--print(nx, ny, nz)
	local npcthis = nil
	if scene.dwMapID == 177 then
		npcthis = scene.CreateNpc(27670, nx, ny, nz, 0, -1)
	else
		npcthis = scene.CreateNpc(36359, nx, ny, nz, 0, -1)
	end
	playerthis.PlaySfx(9, nx, ny, nz)
	if npcthis then
		npcthis.SetForceID(27)
	end
	playerthis.DelBuff(7694, 1)
end
