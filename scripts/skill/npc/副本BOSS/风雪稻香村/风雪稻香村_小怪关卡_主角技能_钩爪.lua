---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_小怪关卡_主角技能_钩爪.lua
-- 更新时间:	2014/2/25 14:19:22
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
tSkillData = 
{
	{nDamageBase = 120000, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_小怪关卡_主角技能_钩爪.lua", --属性值1
		0															--属性值2
	);
	
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(7697, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	skill.SetNormalCoolDown(1, 699);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npctarget.GetBuff(7644, 1) then
		npctarget.DelBuff(7644, 1)
	end
	scene.SetTimer(1.5 * GLOBAL.GAME_FPS, "scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_小怪关卡_主角技能_钩爪.lua", 0, dwCharacterID)
end
...
function OnTimer(scene, nParam1, nParam2)
	...
	if npctarget.dwTemplateID == 26121 or npctarget.dwTemplateID == 26019 or npctarget.dwTemplateID == 36279 or npctarget.dwTemplateID == 36301  then	
		npctarget.FireAIEvent(2011, 0, 0)
		npctarget.AddBuff(0, 99, 7644, 1)
	end
end
