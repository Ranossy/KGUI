---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_黑鹰_十二生肖_午马释放.lua
-- 更新时间:	2014/1/24 15:10:23
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_黑鹰_十二生肖_午马释放.lua", --属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID, dwSrcID)
	...
	for i = 1, #tplayer do
		...
		if playerthis and playerthis.nMoveState ~= MOVE_STATE.ON_DEATH then
			if GetCharacterDistance(playerthis.dwID, npcthis.dwID) <= 6 * 64 then
				nCount = nCount + 1	
			end
		end
	end
	if scene.dwMapID == 177 then
		if nCount + 1 < 10 then
			npcthis.CastSkill(8721, nCount + 1)
		else
			npcthis.CastSkill(8721, 10)
		end
	else
		if nCount < 20 then
			npcthis.CastSkill(8721, nCount + 10)
		else
			npcthis.CastSkill(8721, 30)
		end
	end
end
