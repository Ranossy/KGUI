---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_老无名_冥域.lua
-- 更新时间:	2014/4/1 16:21:39
-- 更新用户:	wangpengfei1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_老无名_冥域.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player then
		...
		player.DieByKiller(dwSrcID)
		local nNum = npccaster.GetCustomInteger1(37)
		nNum = nNum + 1
		if scene.dwMapID == 177 then
			scene.CreateNpc(30368, player.nX, player.nY, player.nZ, 0, -1, "kuileiyouming_" .. nNum)
		else
			scene.CreateNpc(36367, player.nX, player.nY, player.nZ, 0, -1, "kuileiyouming_" .. nNum)
		end
		npccaster.SetCustomInteger1(37, nNum)
		return
	end
	local npcthis = GetNpc(dwCharacterID)
	if npcthis then
		if npcthis.nKind == 10 then
			npcthis.Die()
		end
	end
end
