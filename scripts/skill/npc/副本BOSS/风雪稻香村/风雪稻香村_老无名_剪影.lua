...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_老无名_剪影.lua",								--属性值1
		0															--属性值2
	);	
...
function Apply(dwCharacterID, dwSrcID)
	...
	local nBuff = player.GetBuff(6662, 1)
	local nDamage = 0
	if scene.dwMapID == 177 then
		if not nBuff then
			nDamage = player.nMaxLife * 0.05
		else
			nDamage = player.nMaxLife * (0.05 + nBuff.nStackNum * 0.05)	
		end
	else
		if not nBuff then
			nDamage = player.nMaxLife * 0.05
	...
	player.CustomDamage(npcBoss.dwID, 8080, 1, 1, nDamage)
	player.AddBuff(0, 99, 6662, 1)
end
