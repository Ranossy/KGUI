---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_老无名_傀儡阴阳_连锁伤害.lua
-- 更新时间:	2014/2/26 17:42:17
-- 更新用户:	wangpengfei1
-- 脚本说明:	
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_老无名_傀儡阴阳_连锁伤害.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not playertarget then
		local npcPet = GetNpc(dwCharacterID)
		if npcPet then
			if npcPet.nSpecies == 10 then
				npcPet.Die()
			end
		else
			return
	...
	local tplayer_select = {}
	if tplayer then
		for i = 1, #tplayer do
			local playerthis = GetPlayer(tplayer[i])
			if playerthis then
				if playerthis ~= playertarget and playerthis.nMoveState ~= MOVE_STATE.ON_DEATH then
					table.insert(tplayer_select, playerthis)
				end
			end
		end
	end
	if #tplayer_select == 0 then
		return
	end
	local nRandom = Random(1, #tplayer_select)
	local player_select = tplayer_select[nRandom]
	if player_select then
		player_select.CastSkill(8904, 1, TARGET.PLAYER, playertarget.dwID)
		player_select.AddBuff(0, 99, 7554, 1)
	end
end
