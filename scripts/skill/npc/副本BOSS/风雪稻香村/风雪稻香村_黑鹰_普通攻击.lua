---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_黑鹰_普通攻击.lua
-- 更新时间:	2014/3/27 20:23:33
-- 更新用户:	wangpengfei1
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 66700, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 144100, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雪稻香村/风雪稻香村_黑鹰_普通攻击.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npccast = GetNpc(dwSkillSrcID)
	if not npccast then
		return
	end
	local scene = npccast.GetScene()
	if not scene then
		return
	end
	if scene.dwMapID == 177 then
		local buffthis = npccast.GetBuff(7518, 1)
		if buffthis then
			local nLevel = buffthis.nStackNum
			npccast.CastSkill(8853, nLevel)
		end
	else
		local buffthis = npccast.GetBuff(7518, 1)
		if buffthis then
			local nLevel = buffthis.nStackNum
			npccast.CastSkill(8853, nLevel + 25)
		end
	end
end
