---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雪稻香村/风雪稻香村_黑鹰_十二生肖_亥猪.lua
-- 更新时间:	2014/4/16 20:30:25
-- 更新用户:	wangpengfei1
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 1250000, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 2025000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame)
	local npcthis = GetNpc(nCharacterID)
	if not npcthis then
		return
	end
	local x = npcthis.nX + math.cos(math.pi * 2 * npcthis.nFaceDirection / 256) * 128
	local y = npcthis.nY + math.sin(math.pi * 2 * npcthis.nFaceDirection / 256) * 128
	npcthis.RunTo(x, y)
end
