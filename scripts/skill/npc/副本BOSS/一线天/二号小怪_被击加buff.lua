---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/一线天/二号小怪_被击加buff.lua
-- 更新时间:	2013/3/25 16:57:10
-- 更新用户:	mengxiangfei
-- 脚本说明:	
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/一线天/二号小怪_被击加buff.lua", -- 属性值1
		0														-- 属性值2
	);
...
function Apply(dwCharacterID)
	...
	if cNpc.GetBuff(5339, 1) then
		--有无敌buff不加伤害
		return
	else
		if not cNpc.GetBuff(5338, 1) then
			cNpc.AddBuff(0, 99, 5338, 1, 3600)
		else
			local nbuffcount = cNpc.GetBuff(5338, 1).nStackNum
			--buff最多有50层
			if nbuffcount >= 40 then
				for i = 1, nbuffcount do
					cNpc.DelBuff(5338, 1)
				end
				cNpc.AddBuff(0, 99, 5339, 1, 10)
			else
				cNpc.AddBuff(0, 99, 5338, 1, 3600)
			end
		end
