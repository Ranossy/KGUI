...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
    skill.BindBuff(1, 4370, dwSkillLevel);		-- 设置1号位Buff
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame)
    ...
    if nLeftFrame == 0 then -- 正常结束，剩余时间为0
    	...
    	local result = character.AddBuff(0, 99, 4366, 1)      -- 非正常结束
    	character.nCurrentLife = character.nCurrentLife - character.nMaxLife * 0.02
    end
end;
