---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_锻刀厅/柳哲/3号_柳哲_冰火之舞冰母技能.lua
-- 更新时间:	2017/4/1 15:12:22
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_锻刀厅/柳哲/3号_柳哲_冰火之舞冰母技能.lua", -- 属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local huojianquan = scene.GetNpcByNickName("huojianquan")
	local liuzhezongkong = scene.GetNpcByNickName("liuzhezongkong")
	if scene.dwMapID == 263 or scene.dwMapID == 273 then
		if playertarget then
			if huojianquan then
				if GetCharacterDistance(playertarget.dwID, huojianquan.dwID) <= 384 then
					npc.CastSkill(17289, 1, TARGET.PLAYER, playertarget.dwID)
				else
					npc.CastSkill(17289, 2, TARGET.PLAYER, playertarget.dwID)
	...
	if scene.dwMapID == 271 then
		if playertarget then
			if huojianquan then
				if GetCharacterDistance(playertarget.dwID, huojianquan.dwID) <= 384 then
					npc.CastSkill(17289, 3, TARGET.PLAYER, playertarget.dwID)
				else
					npc.CastSkill(17289, 4, TARGET.PLAYER, playertarget.dwID)
					if liuzhezongkong then
						liuzhezongkong.SetCustomInteger1(40, 1)
					end
				end
