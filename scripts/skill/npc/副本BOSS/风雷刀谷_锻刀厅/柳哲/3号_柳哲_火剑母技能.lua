---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_锻刀厅/柳哲/3号_柳哲_火剑母技能.lua
-- 更新时间:	2017/4/6 15:57:12
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 1110, nDamageRand = 110, nCostMana = 0}, --level 1
	{nDamageBase = 2220, nDamageRand = 220, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_锻刀厅/柳哲/3号_柳哲_火剑母技能.lua", -- 属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local huoMaxnX = 34632
	local huoMinnX = 32710
	local huoMaxnY = 30734
	local huoMinnY = 26823
	...
	if playertarget then
		...
		if scene then
			if scene.dwMapID == 263 or scene.dwMapID == 273 then
				if playertarget.nX >= huoMinnX and playertarget.nX <= huoMaxnX and playertarget.nY >= huoMinnY and playertarget.nY <= huoMaxnY then
					if npc then
						npc.CastSkill(17285, 2, TARGET.PLAYER, playertarget.dwID)
					end
				else
					if npc then
						npc.CastSkill(17285, 1, TARGET.PLAYER, playertarget.dwID)
					end
				end
			else
				if playertarget.nX >= huoMinnX and playertarget.nX <= huoMaxnX and playertarget.nY >= huoMinnY and playertarget.nY <= huoMaxnY then
					if npc then
						npc.CastSkill(17285, 4, TARGET.PLAYER, playertarget.dwID)
					end
				else
					if npc then
						npc.CastSkill(17285, 3, TARGET.PLAYER, playertarget.dwID)
					end
				end
			end
		end
