---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_锻刀厅/柳哲/3号_柳哲_冰剑母技能.lua
-- 更新时间:	2017/4/6 15:51:45
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_锻刀厅/柳哲/3号_柳哲_冰剑母技能.lua", -- 属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local binMaxnX = 32710
	local binMinnX = 30785
	local binMaxnY = 30734
	local binMinnY = 26814
	...
	if playertarget then
		...
		if scene then
			if scene.dwMapID == 263 or scene.dwMapID == 273 then
				if playertarget.nX >= binMinnX and playertarget.nX <= binMaxnX and playertarget.nY >= binMinnY and playertarget.nY <= binMaxnY then
					if npc then
						npc.CastSkill(17286, 2, TARGET.PLAYER, playertarget.dwID)
					end
				else
					if npc then
						npc.CastSkill(17286, 1, TARGET.PLAYER, playertarget.dwID)
					end
				end
			else
				if playertarget.nX >= binMinnX and playertarget.nX <= binMaxnX and playertarget.nY >= binMinnY and playertarget.nY <= binMaxnY then
					if npc then
						npc.CastSkill(17286, 4, TARGET.PLAYER, playertarget.dwID)
					end
				else
					if npc then
						npc.CastSkill(17286, 3, TARGET.PLAYER, playertarget.dwID)
					end
				end
			end
		end
