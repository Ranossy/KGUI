---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_锻刀厅/柳哲/3号_柳哲_冰火之舞火母技能.lua
-- 更新时间:	2017/4/1 15:16:40
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_锻刀厅/柳哲/3号_柳哲_冰火之舞火母技能.lua", -- 属性值1
		0															--属性值2
	);
	...
	skill.nAreaRadius = 200 * LENGTH_BASE;		-- 技能作用半径
	skill.nTargetCountLimit = 250;				-- 技能作用目标数量限制,(小于0 代表目标数量不限制)
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local bingjianquan = scene.GetNpcByNickName("bingjianquan")
	local liuzhezongkong = scene.GetNpcByNickName("liuzhezongkong")
	if scene.dwMapID == 263 or scene.dwMapID == 273 then
		if playertarget then
			if bingjianquan then
				if GetCharacterDistance(playertarget.dwID, bingjianquan.dwID) <= 384 then
					npc.CastSkill(17290, 1, TARGET.PLAYER, playertarget.dwID)
				else
					npc.CastSkill(17290, 2, TARGET.PLAYER, playertarget.dwID)
					if liuzhezongkong then
						liuzhezongkong.SetCustomInteger1(40, 1)
					end
				end
			end
		end

		if not bingjianquan then
			if playertarget then
				npc.CastSkill(17290, 2, TARGET.PLAYER, playertarget.dwID)
				if liuzhezongkong then
					liuzhezongkong.SetCustomInteger1(40, 1)
	...
	if scene.dwMapID == 271 then
		if playertarget then
			if bingjianquan then
				if GetCharacterDistance(playertarget.dwID, bingjianquan.dwID) <= 384 then
					npc.CastSkill(17290, 3, TARGET.PLAYER, playertarget.dwID)
				else
					npc.CastSkill(17290, 4, TARGET.PLAYER, playertarget.dwID)
					if liuzhezongkong then
						liuzhezongkong.SetCustomInteger1(40, 1)
					end
				end
