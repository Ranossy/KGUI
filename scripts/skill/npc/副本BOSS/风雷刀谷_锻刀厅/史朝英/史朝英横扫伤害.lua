---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_锻刀厅/史朝英/史朝英横扫伤害.lua
-- 更新时间:	2017/3/29 12:05:15
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 4680, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_锻刀厅/史朝英/史朝英横扫伤害.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
	...
	if player then
		...
		if scene then
			if scene.dwMapID == 271 then
				...
				if npc then
					player.AddBuff(npc.dwID, npc.nLevel, 11841, 1)
				end
			end
		end
	end
