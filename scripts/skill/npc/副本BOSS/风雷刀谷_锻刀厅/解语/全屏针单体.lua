---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/风雷刀谷_锻刀厅/解语/全屏针单体.lua
-- 更新时间:	2017/1/11 10:34:13
-- 更新用户:	ZHENGYUCHAO
-- 脚本说明:
----------------------------------------------------------------------<
...
Include("scripts/Map/风雷刀谷_锻刀厅/include/解语通用数据.lua")

tSkillData =
{
	{nDamageBase = 7800, nDamageRand = 200, nCostMana = 0}, --level 1
	{nDamageBase = 14000, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		ATTRIBUTE_TYPE.SKILL_POISON_DAMAGE, -- 魔法属性
	...
	skill.AddAttribute(
		...
		ATTRIBUTE_TYPE.CALL_POISON_DAMAGE, -- 魔法属性
		0, -- 属性值1
		0											-- 属性值2
	);
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/风雷刀谷_锻刀厅/解语/全屏针单体.lua", --属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local srcNpc = GetNpc(dwSkillSrcID)
	if not srcNpc then
		return
	end
	local commonData = JieYuData[srcNpc.GetMapID()]
	
	targetPlayer.AddBuff(srcNpc.dwID, srcNpc.nLevel, 11608, commonData.QuanPingZhenDebuffLevel)
	local lianxianBuff = targetPlayer.GetBuff(11608, commonData.QuanPingZhenDebuffLevel)

	local npcX = srcNpc.nX
	local npcY = srcNpc.nY
	local playerX = targetPlayer.nX
	local playerY = targetPlayer.nY

	local distanceAsChi = math.sqrt((npcX - playerX) * (npcX - playerX) + (npcY - playerY) * (npcY - playerY)) / 64
	lianxianBuff.nCustomValue = distanceAsChi
end
