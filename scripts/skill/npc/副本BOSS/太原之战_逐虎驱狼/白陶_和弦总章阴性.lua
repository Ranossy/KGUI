---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_和弦总章阴性.lua
-- 更新时间:	2014/9/19 11:35:23
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_和弦总章阴性.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSrcID)
	...
	local npcKH = scene.GetNpcByNickName("ZhHQL_BaiTao_PiPa")
	local dwSrcBuffID = 0
	if npcKH  then
		dwSrcBuffID = npcKH.dwID
	end
	if player and not player.GetBuff(856,1) then
		player.AddBuff(dwSrcBuffID, 99, 7992, buffLevel)
