---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/狼牙弓箭手.lua
-- 更新时间:	2014/11/8 21:48:09
-- 更新用户:	xiedixun2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/狼牙弓箭手.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	local CANGYUN = GetNpc(dwCharacterID)
	if not CANGYUN then
		return
	end

	local scene = CANGYUN.GetScene()
	if not scene then
		return
	end
	...
	scene.SetTimer(40, "scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/狼牙弓箭手.lua", CANGYUN.dwID, npc.dwID)
end

function OnTimer(scene, nParam1, nParam2)
	local CANGYUN = GetNpc(nParam1)
	if CANGYUN then
		CANGYUN.PlaySfx(1075, CANGYUN.nX + 50, CANGYUN.nY, CANGYUN.nZ + 500)
	end

	--local npc = GetNpc(nParam2)
	--if npc then
		--npc.CastSkill(13462, 1, TARGET.NPC, CANGYUN.dwID)
	--end
end
