---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/挑战_史思明_怒血连掌_第一目标.lua
-- 更新时间:	2015/3/4 15:11:55
-- 更新用户:	wangwei-pc2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 333000, nDamageRand = 10000, nCostMana = 0}, --level 1
	{nDamageBase = 240000, nDamageRand = 5000, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/挑战_史思明_怒血连掌_第一目标.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 8352, dwSkillLevel);		-- 设置1号位Buff
	--skill.BindBuff(2, 8306, dwSkillLevel);		-- 设置2号位Buff
	...
	--skill.nAreaRadius = 6 * LENGTH_BASE;		-- 技能作用半径,120°
...
function Apply(dwCharacterID, dwSkillSrcID)
	local player_MT = GetPlayer(dwCharacterID)
	if not player_MT then
		return
	end
	...
	for i = 1, #tPlayer do
		...
		if CPlayer_ALL and CPlayer_ALL.nMoveState ~= MOVE_STATE.ON_DEATH then
			-------------取6尺内的非当前T目标---------------
			if GetDistanceSq(npc.nX, npc.nY, 0, CPlayer_ALL.nX, CPlayer_ALL.nY, 0) <= (6 * 64) ^ 2 then
				if CPlayer_ALL.dwID ~= player_MT.dwID then
					tTempPlayer1[#tTempPlayer1 + 1] = CPlayer_ALL
				end
			end
		end
	end
	if #tTempPlayer1 > 0 then
		local num = 1
		for i = 1, #tTempPlayer1 do
			if tTempPlayer1[i].nCurrentLife > tTempPlayer1[num].nCurrentLife then
				num = i
			end
		end
		npc.CastSkill(13755, 1, TARGET.PLAYER, tTempPlayer1[num].dwID)
	else
		npc.CastSkill(13756, 1, TARGET.PLAYER, player_MT.dwID)
	end
end
