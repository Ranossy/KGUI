---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_音波球搜寻AOE.lua
-- 更新时间:	2014/7/14 19:25:07
-- 更新用户:	ZHOUMING
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_音波球搜寻AOE.lua", -- 属性值1
		0														-- 属性值2
	);
	---------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSrcID)
	...
	local npcCha = GetNpc(dwCharacterID)
	if not npcSrc then
		return
	elseif player then
		--local scene = npcSrc.GetScene()
	--	Log(npcSrc.szName.."释放搜寻AOE时A到玩家："..player.szName.."!")
		local nNumPlayer = npcSrc.GetCustomUnsigned1(0)
		npcSrc.SetCustomUnsigned1(0, nNumPlayer + 1)
		local nNearestDis = npcSrc.GetCustomUnsigned4(6)
		local nDisThis = math.floor(math.sqrt(GetDistanceSq(npcSrc.nX, npcSrc.nY, npcSrc.nZ, player.nX, player.nY, player.nZ)))
		if nDisThis < nNearestDis then
		--	Log(npcSrc.szName.."释放搜寻AOE时A到玩家："..player.szName.."刷新了最低距离："..nDisThis.."!")
			npcSrc.SetCustomUnsigned4(2, dwCharacterID)
			npcSrc.SetCustomUnsigned4(6,nDisThis)
		end
	elseif npcCha then
		--local scene = npcSrc.GetScene()
		--Log(npcSrc.szName.."释放搜寻AOE时A到NPC："..npcCha.szName.."!")
		local nNumNpc = npcSrc.GetCustomUnsigned1(1)
		npcSrc.SetCustomUnsigned1(1, nNumNpc + 1)
		local nNearestDis = npcSrc.GetCustomUnsigned4(6)
		local nDisThis = math.floor(math.sqrt(GetDistanceSq(npcSrc.nX, npcSrc.nY, npcSrc.nZ, npcCha.nX, npcCha.nY, npcCha.nZ)))
		if nDisThis < nNearestDis then
			--Log(npcSrc.szName.."释放搜寻AOE时A到npc："..player.szName.."刷新了最低距离："..nDisThis.."!")
			npcSrc.SetCustomUnsigned4(2, dwCharacterID)
			npcSrc.SetCustomUnsigned4(6,nDisThis)
		end
	end
