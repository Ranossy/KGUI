---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_阳性音波冲击目标选择.lua
-- 更新时间:	2014/7/14 17:33:35
-- 更新用户:	ZHOUMING
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_阳性音波冲击目标选择.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	if tTargetList and #tTargetList > 0 then
		for i = 1, #tTargetList do
			if i == 1 then--第一个目标特殊处理
				...
				if playerThis then
					playerThis.AddBuff(npc.dwID, 99, 8027, nlevel)
				end
				npc.CastSkill(9394, ii, TARGET.PLAYER, tTargetList[i])--伤害技能
				npc.CastSkill(9395, nlevel, TARGET.PLAYER, tTargetList[i])--链状表现
				...
				if playerThis then
					...
					if i == #tTargetList then--最后一个特殊处理
						local buff8174 = playerThis.GetBuff(8027, nlevel)
						if buff8174 then
				...
				if playerLast then
					local buff8027 = playerLast.GetBuff(8027, nlevel)
					if buff8027 then
						buff8027.nCustomValue = tTargetList[i]
					end
					playerLast.CastSkill(9395, nlevel, TARGET.PLAYER, tTargetList[i])--链状表现
				end
				playerLast = nil
			end
		end
