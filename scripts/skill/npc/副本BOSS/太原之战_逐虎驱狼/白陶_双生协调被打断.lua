---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_双生协调被打断.lua
-- 更新时间:	2014/8/11 16:07:46
-- 更新用户:	ZHOUMING
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_双生协调被打断.lua", -- 属性值1
		0														-- 属性值2
	);
    
	----------------- 技能施放Buff需求 ---------------------------------------------
	  skill.AddSlowCheckSelfBuff(8179, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	 -- skill.BindBuff(1, 6951, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	--print("接下来继续添加双生协调被打断的逻辑!")
	if npc.dwID ~= npcKH.dwID then
		npcKH.Jump()
	end
	if npc.dwID ~= npcPP.dwID then
		npcPP.Jump()
	end
end
