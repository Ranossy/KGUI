---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_雪舞伤害AOE.lua
-- 更新时间:	2014/8/18 14:38:14
-- 更新用户:	ZHOUMING
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	if dwSkillLevel <= 10 then
		nDamage = 24500 * (0.7 + 0.3 * dwSkillLevel) * 0.14
	elseif dwSkillLevel <= 20 then
		nDamage = 28700 * (0.65 + 0.2 * (dwSkillLevel-10)) * 0.14
		dwBuffLevel = 2
	elseif dwSkillLevel <= 30 then
		nDamage = 32800 * (0.65 + 0.35 * (dwSkillLevel-20)) * 0.14
		dwBuffLevel = 3
	elseif dwSkillLevel <= 40 then
		nDamage = 25800 * (0.7 + 0.3 * (dwSkillLevel-30)) * 0.14
		dwBuffLevel = 4
	end
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_雪舞伤害AOE.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(8201, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	skill.AddSlowCheckDestBuff(8198, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff.
	skill.AddSlowCheckDestBuff(8196, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff.
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 8201, 1);		-- 设置1号位Buff
	skill.BindBuff(2, 7992, dwBuffLevel);		-- 设置2号位Buff
...
function Apply(dwCharacterID, dwSrcID)
	...
	if not player or not npcSrc then
		return
	end
	local npcBoss = scene.GetNpcByNickName("ZhHQL_BaiTao_PiPa")
	if npcBoss and npcBoss.GetBuff(8060, 1) then
		local buff= player.GetBuff(8199,nLevel)
	
		if not buff then
			player.AddBuff(npcBoss.dwID, 99, 8199, nLevel)
		else
			player.SetBuffLeftActiveCount(buff.nIndex, buff.nLeftActiveCount + 4)
		end
	end
