---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_双生协调.lua
-- 更新时间:	2014/10/13 11:10:27
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 14580000, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 69984000, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 155520000, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 20412000, nDamageRand = 0, nCostMana = 0}, --level 4
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_双生协调.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- 时间相关 -------------------------------------------------
	if dwSkillLevel <=10 then
		skill.nPrepareFrames = 6 * 16 + 48 * dwSkillLevel;				-- 吟唱帧数
	elseif dwSkillLevel <= 20 then
		skill.nPrepareFrames = 6 * 16 + 48 * (dwSkillLevel-10);				-- 吟唱帧数
	elseif dwSkillLevel <= 30 then
		skill.nPrepareFrames = 6 * 16 + 48 * (dwSkillLevel-20);
	elseif dwSkillLevel <= 40 then
		skill.nPrepareFrames = 6 * 16 + 48 * (dwSkillLevel-30);
	end
...
function Apply(dwCharacterID, dwSrcID)
	...
	if npcKH then
		npcKH.FireAIEvent(2012, npcBoss.dwID, 0)
		npcKH.FireAIEvent(2013, npcBoss.dwID, 0)
	end
	if npcPP then
		npcPP.FireAIEvent(2012, npcBoss.dwID, 0)
		npcPP.FireAIEvent(2013, npcBoss.dwID, 0)
	end
