---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_冰冻AOE.lua
-- 更新时间:	2014/8/18 16:24:00
-- 更新用户:	ZHOUMING
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 32600, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 38300, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 78800, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 34400, nDamageRand = 0, nCostMana = 0}, --level 4
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_冰冻AOE.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(8198, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	elseif dwSkillLevel == 4 then
		skill.nAreaRadius = 2 * LENGTH_BASE;
	end
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player and player.nMoveState~= MOVE_STATE.ON_DEATH and not player.GetBuff(8198,0) then
		local dwBuffSrcID = 0
		...
		if scene then
			if scene.dwMapID == 192 then
				...
				if npcBoss then
					dwBuffSrcID = npcBoss.dwID
					player.Stop()
					--print(player.szName.."frezen")
					player.AddBuff(dwBuffSrcID, 99, 8198, 1)
					local npcbl = scene.GetNpcByNickName("binglao"..player.dwID)
					if not npcbl then
						npcbl = scene.CreateNpc(36861, player.nX, player.nY, player.nZ, 0, -1, "binglao"..player.dwID)
					else
						player.SetPosition(npcbl.nX, npcbl.nY, npcbl.nZ)
					end
					if npcbl then
						npcbl.SetCustomUnsigned4(0, player.dwID)
						local buff8198 = player.GetBuff(8198, 1 )
						if buff8198 then
							buff8198.nCustomValue = npcbl.dwID
						end
					end
				end
			elseif scene.dwMapID == 199 then
				...
				if npcBoss then
					...
					player.Stop()
					--print(player.szName.."frezen")
					player.AddBuff(dwBuffSrcID, 99, 8198, 2)
					local npcbl = scene.GetNpcByNickName("binglao"..player.dwID)
					if not npcbl then
						npcbl = scene.CreateNpc(40937, player.nX, player.nY, player.nZ, 0, -1, "binglao"..player.dwID)
					else
						player.SetPosition(npcbl.nX, npcbl.nY, npcbl.nZ)
					end
					if npcbl then
						npcbl.SetCustomUnsigned4(0, player.dwID)
						local buff8198 = player.GetBuff(8198, 2 )
						if buff8198 then
							buff8198.nCustomValue = npcbl.dwID
						end
					end
			...
			elseif scene.dwMapID == 206 then
				...
				if npcBoss then
					...
					player.Stop()
					--print(player.szName.."frezen")
					player.AddBuff(dwBuffSrcID, 99, 8198, 3)
					local npcbl = scene.GetNpcByNickName("binglao"..player.dwID)
					if not npcbl then
						npcbl = scene.CreateNpc(42157, player.nX, player.nY, player.nZ, 0, -1, "binglao"..player.dwID)
					else
						player.SetPosition(npcbl.nX, npcbl.nY, npcbl.nZ)
					end
					if npcbl then
						npcbl.SetCustomUnsigned4(0, player.dwID)
						local buff8198 = player.GetBuff(8198, 3 )
						if buff8198 then
							buff8198.nCustomValue = npcbl.dwID
						end
					end
			...
			elseif scene.dwMapID == 212 then
				...
				if npcBoss then
					...
					player.Stop()
					--print(player.szName.."frezen")
					player.AddBuff(dwBuffSrcID, 99, 8198, 4)
					local npcbl = scene.GetNpcByNickName("binglao"..player.dwID)
					if not npcbl then
						npcbl = scene.CreateNpc(42685, player.nX, player.nY, player.nZ, 0, -1, "binglao"..player.dwID)
					else
						player.SetPosition(npcbl.nX, npcbl.nY, npcbl.nZ)
					end
					if npcbl then
						npcbl.SetCustomUnsigned4(0, player.dwID)
						local buff8198 = player.GetBuff(8198, 4 )
						if buff8198 then
							buff8198.nCustomValue = npcbl.dwID
						end
					end
