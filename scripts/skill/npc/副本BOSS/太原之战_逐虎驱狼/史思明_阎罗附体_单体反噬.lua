---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/史思明_阎罗附体_单体反噬.lua
-- 更新时间:	2014/9/24 17:09:15
-- 更新用户:	wangwei-pc2
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 6100, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 8620, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/史思明_阎罗附体_单体反噬.lua",
		0
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 8035, dwSkillLevel);		-- 设置1号位Buff
	--skill.BindBuff(2, 8036, dwSkillLevel);		-- 设置2号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetScene().dwMapID == 192 then
		if player.nCurrentLife > 2900 then
			player.CustomDamage(dwSkillSrcID, 13196, 1, 2, 2900*0.14)
		else
			player.CustomDamage(dwSkillSrcID, 13196, 1, 2, 2900*0.14)
			player.DieByKiller(npc.dwID)
		end		
	elseif player.GetScene().dwMapID == 199 then
		if player.nCurrentLife > 3500 then
			player.CustomDamage(dwSkillSrcID, 13196, 1, 2, 3500*0.14)
		else
			player.CustomDamage(dwSkillSrcID, 13196, 1, 2, 3500*0.14)
			player.DieByKiller(npc.dwID)
		end
	elseif player.GetScene().dwMapID == 206 then
		if player.nCurrentLife > 7400 then
			player.CustomDamage(dwSkillSrcID, 13196, 1, 2, 7400*0.14)
		else
			player.CustomDamage(dwSkillSrcID, 13196, 1, 2, 7400*0.14)
			player.DieByKiller(npc.dwID)
		end
	elseif player.GetScene().dwMapID == 212 then
		if player.nCurrentLife > 3100 then
			player.CustomDamage(dwSkillSrcID, 13196, 1, 2, 3100*0.14)
		else
			player.CustomDamage(dwSkillSrcID, 13196, 1, 2, 3100*0.14)
			player.DieByKiller(npc.dwID)
		end	
