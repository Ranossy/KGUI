---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/史思明_十殿阎罗_玩家出掌.lua
-- 更新时间:	2014/10/18 22:04:42
-- 更新用户:	wangwei-pc2
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 45700, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/史思明_十殿阎罗_玩家出掌.lua",
		0
	);
	----------------- 魔法属性 -------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	for i = 1, #playerlist do
		local CPplayer = GetPlayer(playerlist[i])
		if CPplayer then
			local buff = player.GetBuff(8408, 1)
			if buff then
				--把正在传功的人标记，出掌之后不再流失内力和生命。
				buff.nCustomValue = 1
			end
		end
	end
