---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_双生协作搜索.lua
-- 更新时间:	2014/8/14 15:52:15
-- 更新用户:	ZHOUMING
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_双生协作搜索.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSrcID)
	...
	if not npcSrc or tBuffData[npcSrc.dwTemplateID] then
		Log(GetEditorString(6, 7594)..npcSrc.dwTemplateID)
		return
	elseif player then
		local nDis = math.sqrt(GetDistanceSq(npcSrc.nX, npcSrc.nY, 0, player.nX, player.nY, 0))
		local nMinDis = npcSrc.GetCustomUnsigned4(4)
		if nDis < nMinDis then
			print(GetEditorString(6, 849)..player.szName..GetEditorString(6, 7595)..npcSrc.szName..GetEditorString(6, 7596)..nDis)
			npcSrc.SetCustomUnsigned4(0, dwCharacterID)
			npcSrc.SetCustomUnsigned4(4, nDis)
			npcSrc.SetCustomUnsigned1(8, 1)
		end
	elseif npcTarget then
		local nDis = math.sqrt(GetDistanceSq(npcSrc.nX, npcSrc.nY, 0, npcTarget.nX, npcTarget.nY, 0))
		local nMinDis = npcSrc.GetCustomUnsigned4(4)
		if nDis < nMinDis then
			print(GetEditorString(6, 2249)..npcTarget.szName..GetEditorString(6, 7595)..npcSrc.szName..GetEditorString(6, 7596)..nDis)
			npcSrc.SetCustomUnsigned4(0, dwCharacterID)
			npcSrc.SetCustomUnsigned4(4, nDis)
			npcSrc.SetCustomUnsigned1(8, 1)
		end
