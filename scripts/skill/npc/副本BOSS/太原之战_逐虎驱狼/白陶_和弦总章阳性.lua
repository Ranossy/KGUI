---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_和弦总章阳性.lua
-- 更新时间:	2014/9/19 11:35:37
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 10800, nDamageRand = 0, nCostMana = 0}, --level 1
	{nDamageBase = 18400, nDamageRand = 0, nCostMana = 0}, --level 2
	{nDamageBase = 21000, nDamageRand = 0, nCostMana = 0}, --level 3
	{nDamageBase = 11400, nDamageRand = 0, nCostMana = 0}, --level 4
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_和弦总章阳性.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSrcID)
	...
	local buffLevel = tLevel[scene.dwMapID]
	
	local player = GetPlayer(dwCharacterID)
	local npcTarget = GetPlayer(dwCharacterID)
	local npcKH = scene.GetNpcByNickName("ZhHQL_BaiTao_KongHou")
	local dwSrcBuffID = 0
	if npcKH  then
		dwSrcBuffID = npcKH.dwID
	end
	if player and not player.GetBuff(856, 1) then
		player.AddBuff(dwSrcBuffID, 99, 7993, buffLevel)
