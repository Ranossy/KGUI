---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_阴性音波冲击伤害.lua
-- 更新时间:	2014/11/5 19:16:13
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	...
	{nDamageBase = 28700, nDamageRand = 0, nCostMana = 0}, --level 2
...
function GetSkillLevelData(skill)
	...
	if dwSkillLevel <= 10 then
		skill.AddAttribute(
			...
			tSkillData[1].nDamageBase * math.pow(1.2, dwSkillLevel -1) * 0.14,
			0
		);
	elseif dwSkillLevel <= 35 then
		skill.AddAttribute(
			ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK,
			ATTRIBUTE_TYPE.SKILL_LUNAR_DAMAGE,
			tSkillData[2].nDamageBase * math.pow(1.25, dwSkillLevel -11) * 0.14,
			0
			);
	elseif dwSkillLevel <= 60 then
		skill.AddAttribute(
			ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK,
			ATTRIBUTE_TYPE.SKILL_LUNAR_DAMAGE,
			tSkillData[3].nDamageBase * math.pow(1.20, dwSkillLevel -36) * 0.14,
			0
			);
	else
		skill.AddAttribute(
			ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK,
			ATTRIBUTE_TYPE.SKILL_LUNAR_DAMAGE,
			tSkillData[4].nDamageBase * math.pow(1.20, dwSkillLevel -61) * 0.14,
			0
			);
	end
