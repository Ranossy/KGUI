---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_技能被打断.lua
-- 更新时间:	2014/10/10 19:42:24
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_技能被打断.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSrcID)
	--print("daduandaduandaduan")
	local npc = GetNpc(dwCharacterID)
	if npc and not npc.GetBuff(8179, 0) then
		local scene = npc.GetScene()
		if scene then
			if scene.dwMapID == 199 or scene.dwMapID == 206 or scene.dwMapID == 212 then
				return
			end
			...
			if npcKH then
				npcKH.Jump(1, 1)
				npcKH.FireAIEvent(2014, 1, 1)
				print(npcKH.szName.."daduandaduandaduan")
			end
			if npcPP then
				npcPP.Jump(1, 1)
				npcPP.FireAIEvent(2014, 1, 1)
				print(npcPP.szName.."daduandaduandaduan")
			end
			npc.DelBuff(8345,1)
		end
	end
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame)
	print(1234567,nCharacterID,BuffID)
	local npc = GetNpc(nCharacterID)
	if not npc then
		return
	end
	if not BuffID == 8179 then
		return
	end
	npc.Jump(1, 1)
	local buff = npc.GetBuff(BuffID, nBuffLevel)
	local scene = npc.GetScene()
	if scene then
		local npcKH = scene.GetNpcByNickName("ZhHQL_BaiTao_KongHou")
		local npcPP = scene.GetNpcByNickName("ZhHQL_BaiTao_PiPa")
