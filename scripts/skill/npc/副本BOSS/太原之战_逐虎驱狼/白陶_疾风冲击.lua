---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_疾风冲击.lua
-- 更新时间:	2014/11/6 22:09:55
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_疾风冲击.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSrcID)
	...
	local npcKH = scene.GetNpcByNickName("ZhHQL_BaiTao_KongHou")
	local dwSrcBuffID = 0
	if npcKH  then
		dwSrcBuffID = npcKH.dwID
	end
	if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		if player.GetBuff(8196, 0) then

			local npcpp = scene.GetNpcByNickName("ZhHQL_BaiTao_PiPa")

			if npcpp then
				if tLevel[scene.dwMapID] then
					player.DelBuff(8196, tLevel[scene.dwMapID])
					player.Stop()
					print(player.CastSkill(10395, tLevel[scene.dwMapID]))
				end
			end
		else
			local nlevel = 1
			if npcKH then
				local buff8060 = npcKH.GetBuff(8060, 1)
				if buff8060 then
					nlevel = buff8060.nStackNum + 1
					if nlevel > 10 then
						nlevel = 10
					end
				end
			...
			player.Stop()
			npcSrc.CastSkill(13273, nlevel, TARGET.PLAYER, player.dwID)
			player.AddBuff(dwSrcBuffID, 99, 7993, tLevel[scene.dwMapID])
			player.AddBuff(dwSrcBuffID, 99, 7993, tLevel[scene.dwMapID])
			player.AddBuff(dwSrcBuffID, 99, 7993, tLevel[scene.dwMapID])
			player.AddBuff(dwSrcBuffID, 99, 7346, 1, 2)
		end

	elseif npcTarget then
		if npcTarget.dwTemplateID == 36861 or npcTarget.dwTemplateID == 40937 or npcTarget.dwTemplateID == 42157 or npcTarget.dwTemplateID == 42685 then
			npcSrc.FireAIEvent(2003, npcTarget.dwID, 1)
			local dwid = npcTarget.GetCustomUnsigned4(0)
			local player = GetPlayer(dwid)
			if player then
				player.AddBuff(0,99,8542,tLevel[scene.dwMapID])
			end
		end
	end
end
