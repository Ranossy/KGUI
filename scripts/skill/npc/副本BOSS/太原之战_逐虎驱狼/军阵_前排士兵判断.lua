---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/军阵_前排士兵判断.lua
-- 更新时间:	2014/10/16 19:58:14
-- 更新用户:	wangwei-pc2
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 25000, nDamageRand = 2000, nCostMana = 0}, --level 1
	{nDamageBase = 25000, nDamageRand = 2000, nCostMana = 0}, --level 2
...
--[[
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/军阵_前排士兵判断.lua",
		0
	);
	
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(7934, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(7934, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	--skill.nChannelInterval	= 0; 				-- 通道技间隔时间,技能加成
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		player.PlaySfx(553, player.nX, player.nY, player.nZ + 200)
	end
	local npc = GetNpc(dwCharacterID)
	if npc then
		if npc.dwTemplateID == 36837 or npc.dwTemplateID == 40913 or npc.dwTemplateID == 42661 or npc.dwTemplateID == 42133 then
			if npc.GetCustomInteger1(1) == 0 then
				npc.PlaySfx(553, npc.nX, npc.nY, npc.nZ + 400)
				npc.SetDisappearFrames(12, 1)
				npc.Stand()
				npc.DoAction(0, 10001)
				npc.SetCustomInteger1(1, 1)
			end
		end
	end
end
