---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_音波球伤害AOE前置判定.lua
-- 更新时间:	2015/3/16 15:18:16
-- 更新用户:	xueyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
	--给自己挂buff
	if dwSkillLevel == 1 or dwSkillLevel == 2 or dwSkillLevel == 4 then
		skill.AddAttribute(
	...
	if dwSkillLevel == 3 then
		skill.AddAttribute(
			...
			"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_音波球伤害AOE前置判定.lua", -- 属性值1
			0													-- 属性值2
		);
	end
...
function Apply(dwCharacterID, dwSrc)
	local player = GetPlayer(dwCharacterID)
	local buff8822 = player.GetBuff(8822, 1)
	local buff8823 = player.GetBuff(8823, 1)
	local npcscr = GetNpc(dwSrc)

	local buff8822Stack = 0
	local buff8823Stack = 0
	if buff8822 then
		buff8822Stack = buff8822.nStackNum
	end
	if buff8823 then
		buff8823Stack = buff8823.nStackNum
	end
	
	if npcscr.dwTemplateID == 42143 then  --阳性音波球
		player.AddBuff(0, 99, 8822, 1) --挑战版阳性debuff添加
		if buff8823 then
			ClearBuff8822And8823(player)
			player.CustomDamage(npcscr.dwID, 13785, 1, 1, player.nMaxLife * 2)
			--player.Die()
		else
			for i = 1, 3 do
				player.AddBuff(0,99,7993,3)
			end
			local nLevel
			nLevel = buff8822Stack + 1
			if nLevel > 6 then
               nLevel = 6
			end
			npcscr.CastSkill(13785,nLevel,TARGET.PLAYER,dwCharacterID)
		end
	elseif npcscr.dwTemplateID == 42148 then --阴性音波球
		player.AddBuff(0, 99, 8823, 1) --挑战版阴性debuff添加
		if buff8822 then
			ClearBuff8822And8823(player)
			player.CustomDamage(npcscr.dwID, 13786, 1, 2, player.nMaxLife * 2)
			--player.Die()
		else
			for i = 1, 3 do
				player.AddBuff(0,99,7992,3)
			end
			local nLevel
			nLevel = buff8823Stack + 1
			if nLevel > 6 then
               nLevel = 6
			end
			npcscr.CastSkill(13786,nLevel,TARGET.PLAYER,dwCharacterID)
		end
	end
end
...
function ClearBuff8822And8823(player)
	local buff8822 = player.GetBuff(8822, 1)
	local buff8823 = player.GetBuff(8823, 1)
	local buff8822Stack = 0
	local buff8823Stack = 0
	...
	for i = 1, buff8822Stack do
        player.DelBuff(8822,1)
	end
	for i = 1, buff8823Stack do
		player.DelBuff(8823,1)
	end
end
