---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_双生协调搜索.lua
-- 更新时间:	2014/8/20 16:02:33
-- 更新用户:	ZHOUMING
-- 脚本说明:
----------------------------------------------------------------------
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/npc/副本BOSS/太原之战_逐虎驱狼/白陶_双生协调搜索.lua", -- 属性值1
		0														-- 属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcSrc = GetNpc(dwSkillSrcID)
	if npcSrc and playerTarget then
		local nMinTargetDis = npcSrc.GetCustomUnsigned4(4)
		local nDisThis = math.sqrt(GetDistanceSq(npcSrc.nX, npcSrc.nY, npcSrc.nZ, playerTarget.nX, playerTarget.nY, playerTarget.nZ))
		if nDisThis < nMinTargetDis then
			npcSrc.SetCustomUnsigned1(8, 1)
			npcSrc.SetCustomUnsigned4(4, nDisThis)
			npcSrc.SetCustomUnsigned4(0, dwCharacterID)
		end
	elseif npcSrc and npcTarget then
		local nMinTargetDis = npcSrc.GetCustomUnsigned4(4)
		local nDisThis = math.sqrt(GetDistanceSq(npcSrc.nX, npcSrc.nY, npcSrc.nZ, npcTarget.nX, npcTarget.nY, npcTarget.nZ))
		if nDisThis < nMinTargetDis then
			npcSrc.SetCustomUnsigned1(8, 1)
			npcSrc.SetCustomUnsigned4(4, nDisThis)
			npcSrc.SetCustomUnsigned4(0, dwCharacterID)
