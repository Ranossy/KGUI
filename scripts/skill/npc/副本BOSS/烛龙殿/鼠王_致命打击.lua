...
tSkillData =
{
	{nDamage = 21300 * 0.8, nDamageRand = 21300 * 0.4, nCostMana = 1},		--level 1
	{nDamage = 45000 * 0.8, nDamageRand = 45000 * 0.4, nCostMana = 2},		--level 2
...
--	);
	...
	----------------- BUFF相关 -------------------------------------------------
    skill.BindBuff(1, 3562, tSkillData[dwSkillLevel].nCostMana);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if not IsPlayer(dwCharacterID) then
		...
		cplayer = GetNpc(dwCharacterID)

	else
		cplayer = GetPlayer(dwCharacterID)
	end
	local nNum = Random(1000,2000)
	local scene = cplayer.GetScene()
			if not scene then
			return
	end
	if scene.dwMapID==133 then
		nsn=cplayer.GetBuff(3562,2).nStackNum
		if not nsn then
		return
		end

	elseif scene.dwMapID==134 then
		nsn=cplayer.GetBuff(3562,1).nStackNum
		if not nsn then
		return
		end

	end
    local damage = nNum * nsn
	if cplayer and IsPlayer(dwCharacterID)  then
	   if cplayer.nCurrentLife > damage then
		  cplayer.nCurrentLife = cplayer.nCurrentLife - damage
	    else
		   cplayer.nCurrentLife = 0
	    end
    end
end
