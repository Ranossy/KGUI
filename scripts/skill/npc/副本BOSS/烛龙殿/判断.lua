...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/npc/副本BOSS/烛龙殿/判断.lua",				-- 属性值1
   		0														-- 属性值2
   		);
   	--[[
   	skill.AddAttribute(
    ...
    skill.AddSlowCheckDestBuff(4013, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID)
	...
	if player.GetBuff(4013, 1) then
		player.DelBuff(4013,1)
	end
	local scene = player.GetScene()
	local larennpc = scene.GetNpcByNickName("larennpc")
	larennpc.SetTarget(TARGET.PLAYER, player.dwID)
	larennpc.CastSkill(3958,1)
end
