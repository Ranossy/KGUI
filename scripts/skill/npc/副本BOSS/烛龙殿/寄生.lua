	...
	tSkillData =
	{
		{nDamage = 30000, nDamageRand = 0, nCostMana = 0},		--level 1
		{nDamage = 35000, nDamageRand = 0, nCostMana = 0},		--level 2
	...
	function GetSkillLevelData(skill)
	   ...
	   skill.AddAttribute(
		...
		skill.AddAttribute(
			ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK,				-- 属性作用模式
			ATTRIBUTE_TYPE.SKILL_POISON_DAMAGE_RAND,							-- 魔法属性
	...
	local player=GetPlayer(dwCharacterID)
		...
		local npcBOSS = scene.GetNpcByNickName("huangzhizhu")
			if not npcBOSS then
			return
		end
		 if scene.dwMapID==133 then
			npcBOSS.CastSkill(3605,2,TARGET.PLAYER,dwCharacterID)
		 elseif scene.dwMapID==134 then
			npcBOSS.CastSkill(3605,1,TARGET.PLAYER,dwCharacterID)
		 end
