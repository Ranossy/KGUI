    ...
    local dwSkillLevel = skill.dwLevel;
   	...
   	----------------- 魔法属性 -------------------------------------------------
   	 skill.AddAttribute(
   		...
   		"skill/npc/副本BOSS/烛龙殿/产小蜘蛛.lua",				-- 属性值1
   		0														-- 属性值2
   		);
...
local character = nil
	...
	if character then
		...
		if scene.dwMapID == 134 then   
			for i = 1, 4 do
				local zongkong = scene.GetNpcByNickName("chanxiaozhizhu")
				if zongkong then
					local step = zongkong.GetCustomInteger4(1)
					scene.CreateNpc(17133, character.nX, character.nY, character.nZ, 0, -1, "xiaohongzhizhu" .. step)
					step = step + 1
					zongkong.SetCustomInteger4(1, step)
				end
			end
		elseif scene.dwMapID == 133 then
			for i = 1, 4 do
				...
				if zongkong then
					local step = zongkong.GetCustomInteger4(1)
					scene.CreateNpc(17393, character.nX, character.nY, character.nZ, 0, -1, "xiaohongzhizhu" .. step)
					step = step + 1
					zongkong.SetCustomInteger4(1, step)
				end
			end
