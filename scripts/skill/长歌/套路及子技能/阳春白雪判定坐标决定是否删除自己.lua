---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/阳春白雪判定坐标决定是否删除自己.lua
-- 更新时间:	2015/10/25 7:40:39
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nbuff_1 = player.GetBuff(10056, 1)
	local nbuff_2 = player.GetBuff(9320, 1)
	if nbuff_1 and nbuff_2 then
		local nx = nbuff_1.nCustomValue
		local ny = nbuff_2.nCustomValue
		if nx == 0 or ny == 0 then
			nbuff_1.nCustomValue = player.nX
			nbuff_2.nCustomValue = player.nY
		else
			if nbuff_1.nCustomValue ~= player.nX or nbuff_2.nCustomValue ~= player.nY then
