...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--[[
	local A_buff = player.GetBuff(9641, 1)
	if A_buff then
		A_buff.nCustomValue = A_buff.nCustomValue + 1
		local ncount = A_buff.nCustomValue
		--print(ncount % 20)
		if ncount % 50 ~= 1 then
			return
		else
	...
	--]]
	local nCount_qufeng = 0
	for i = 1, 4 do
		if player.IsHaveBuff(9318 + i, 1) then
			nCount_qufeng = nCount_qufeng + 1
		end
	end

	if nCount_qufeng == 0 then
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "FireUIEvent", "CHANGE_ACCUMULATE_STYLE", 0)
	end

	local nSkill = player.GetKungfuMountID()
	
	if nSkill ~= nil then
		--莫问
		if nSkill == 10447 then
			if player.GetSkillLevel(14350) == 1 then
				local nbuffcount = 0
				local nbuff = player.GetBuff(9437, 1)
				if nbuff then
					nbuffcount = nbuff.nStackNum
				end
				local nqufengcount = math.min(player.nAccumulateValue, 5)
				--print("nqufengcount"..nqufengcount)
				--print("nbuffcount"..nbuffcount)
				if nbuffcount > nqufengcount then
					--print(1111111)
					local D_value = nbuffcount - nqufengcount
					for i = 1, D_value do
						player.DelBuff(9437, 1)
					end
				elseif nbuffcount < nqufengcount then
					--print(2222222)
					local D_value_1 = nqufengcount - nbuffcount
					for i = 1, D_value_1 do
						player.AddBuff(player.dwID, player.nLevel, 9437, 1)
					end
				end
			else
				--print(333333)
				if player.IsHaveBuff(9437, 1) then
					player.DelGroupBuff(9437, 1)
				end
			end
		elseif nSkill == 10448 then
				if player.GetSkillLevel(14359) == 1 then
				local nbuffcount = 0
				local nbuff = player.GetBuff(9441, 1)
				if nbuff then
					nbuffcount = nbuff.nStackNum
				end
				local nqufengcount = math.min(player.nAccumulateValue, 5)
				...
				if player.IsHaveBuff(9441, 1) then
