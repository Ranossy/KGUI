...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(14143) == 1 then
		--有奇穴减伤可以作用小队
		player.CastSkillXYZ(14145, 1, target.nX, target.nY, target.nZ)
		--这个是目标范围技能(气场对自己释放，不能是长歌释放，一定注意)
		target.CastSkillXYZ(14515, 1, target.nX, target.nY, target.nZ)
		--附魔团队减伤
		if player.GetSkillLevel(15178) == 1 then
			player.CastSkillXYZ(15179, 1, target.nX, target.nY, target.nZ)
		end
	else
		--正常减伤效果只作用自己
		player.CastSkillXYZ(14144, 1, target.nX, target.nY, target.nZ)
		--这个是目标范围技能(气场对自己释放，不能是长歌释放，一定注意)
		target.CastSkillXYZ(14515, 1, target.nX, target.nY, target.nZ)
		--附魔团队减伤
		if player.GetSkillLevel(15178) == 1 then
	...
	if player.GetSkillLevel(14468) == 1 then
		--player.CastSkillXYZ(14469, 1, target.nX, target.nY, target.nZ)
		target.CastSkillXYZ(14521, 1, target.nX, target.nY, target.nZ)
	end
end
