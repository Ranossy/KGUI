---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_笑傲光阴_对所有玩家目标家buff.lua
-- 更新时间:	2015/8/20 22:59:57
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_笑傲光阴_对所有玩家目标家buff.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 9258, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if IsPlayer(dwSkillSrcID) then
		player = GetPlayer(dwSkillSrcID)
	else
		player = GetNpc(dwSkillSrcID)
	end
	if not player then
		return
	end
	...
	local nbuff = player.GetBuff(9257, 1)
	--print(2222222222)
	if nbuff then
		--print(3333333)
		local castID = nbuff.dwSkillSrcID
		local Cast_player = GetPlayer(castID)
		if Cast_player then
			--print(44444)
			--target.AddBuff(Cast_player.dwID, Cast_player.nLevel, 9258, 1)

			if IsEnemy(dwCharacterID, castID) and not target.GetBuff(10212, 1) then
				target.AddBuff(Cast_player.dwID, Cast_player.nLevel, 9258, 1)
			end

			local playerbest = nil
			local playerlist =	Cast_player.GetPartyMemberList()
			if not playerlist then
				playerlist = {Cast_player.dwID}
			end
			for i = 1, #playerlist do
				if dwCharacterID == playerlist[i] and not target.GetBuff(10212, 1) then
					target.AddBuff(Cast_player.dwID, Cast_player.nLevel, 9258, 1)	
				end
			end
