---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_角hot_正常hot效果.lua
-- 更新时间:	2015/8/10 20:59:46
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
    ...
    --]]
		...
		"skill/长歌/套路及子技能/长歌_角hot_正常hot效果.lua",
		0
		);
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 9463, dwSkillLevel);		-- 设置1号位Buff
	...
	elseif dwSkillLevel < 25 then
		skill.nChannelInterval = (40 + (dwSkillLevel - 9) * 2)*1.1
	else
		skill.nChannelInterval = 95*1.1;     -- 通道技间隔时间
	end
	----------------- 阵法相关 -------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(14406) == 1 then
		target.AddBuff(player.dwID, player.nLevel, 9512, 1, 1)
	end
end
