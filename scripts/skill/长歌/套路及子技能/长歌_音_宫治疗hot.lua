---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_音_宫治疗hot.lua
-- 更新时间:	2015/8/10 19:26:40
-- 更新用户:	mengxiangfei11
-- 脚本说明:
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_音_宫治疗hot.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	-----------------技能施放技能需求-------------------------------------------
	skill.AddCheckSelfLearntSkill(14361, 1, SKILL_COMPARE_FLAG.EQUAL);     --检查比较Caster自己已学习的技能ID和等级
	...
	if dwSkillLevel <= 6 then
		skill.nCostManaBasePercent = 450 * 0.2;	-- 技能消耗的内力
	elseif dwSkillLevel <= 12 then
		skill.nCostManaBasePercent = 450 * 0.4;	-- 技能消耗的内力
	elseif dwSkillLevel <= 18 then
		skill.nCostManaBasePercent = 450 * 0.6;	-- 技能消耗的内力
	elseif dwSkillLevel <= 24 then
		skill.nCostManaBasePercent = 450 * 0.8;	-- 技能消耗的内力
	else
		skill.nCostManaBasePercent = 450;	-- 技能消耗的内力
	end
...
function OnSkillLevelUp(skill, player)
	...
	if player.GetSkillLevel(14137) ~= lv then 
		player.LearnSkillLevel(14137, lv, player.dwID)	--学习宫治疗
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local gong_hot_lev = player.GetSkillLevel(14303)		--取技能宫hot的等级

	if gong_dot_lev ~= 0 then
		--普通dot
		if buff_jiange ~= 1 and buff_xiaoguo ~= 1 then
			player.CastSkill(14389, gong_hot_lev, player.GetSkillTarget())
		end
		--间隔缩短的dot
		if buff_jiange == 1 and buff_xiaoguo ~= 1 then
			player.CastSkill(14390, gong_hot_lev, player.GetSkillTarget())
		end
		--递增的dot
		if buff_jiange ~= 1 and buff_xiaoguo == 1 then
			player.CastSkill(14391, gong_hot_lev, player.GetSkillTarget())
		end
		--递增且间隔缩短的dot
		if buff_jiange == 1 and buff_xiaoguo == 1 then
			player.CastSkill(14393, gong_hot_lev, player.GetSkillTarget())
		end
	end
