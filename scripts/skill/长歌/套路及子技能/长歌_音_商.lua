---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_音_商.lua
-- 更新时间:	2015/8/5 11:54:09
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_音_商.lua", -- 属性值1
		0											-- 属性值2
	);
...
function OnSkillLevelUp(skill, player)
	...
	player.LearnSkillLevel(14450, lv, player.dwID)	--学习剑·商
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local shang_lev = player.GetSkillLevel(14065)		--取技能商的等级

	if shang_lev ~= 0 then
		--普通dot
		if buff_jiange ~= 1 and buff_xiaoguo ~= 1 then
			player.CastSkill(14287, shang_lev, player.GetSkillTarget())
		end
		--间隔缩短的dot
		if buff_jiange == 1 and buff_xiaoguo ~= 1 then
			player.CastSkill(14288, shang_lev, player.GetSkillTarget())
		end
		--递增的dot
		if buff_jiange ~= 1 and buff_xiaoguo == 1 then
			player.CastSkill(14289, shang_lev, player.GetSkillTarget())
		end
		--递增且间隔缩短的dot
		if buff_jiange == 1 and buff_xiaoguo == 1 then
			player.CastSkill(14290, shang_lev, player.GetSkillTarget())
		end
	end
	--参连每个曲风叠加2%基础内功AP
