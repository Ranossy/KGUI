---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_宫治疗子技能.lua
-- 更新时间:	2015/8/10 15:11:48
-- 更新用户:	mengxiangfei11
-- 脚本说明:
...
function GetSkillLevelData(skill)
	...
	elseif dwSkillLevel < 25 then
		skill.nChannelInterval = (32 + (dwSkillLevel - 9) * 3)*1.15
	else
		skill.nChannelInterval = 95*1.15;     -- 通道技间隔时间   
	end
	----------------- 阵法相关 -------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--宫额外治疗效果
	if player.GetSkillLevel(14384) == 1 then
		target.AddBuff(player.dwID, player.nLevel, 11263, 1)
		local nLev = player.GetSkillLevel(14137)
		if nLev ~= 0 then
			player.CastSkillXYZ(16852, nLev, target.nX, target.nY, target.nZ)
		end
	end
