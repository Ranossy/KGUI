...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = player.GetBuff(9268, 1)	--记录命中目标数量的buff
	local buff1 = player.GetBuff(9267, 1)	--记录气血值百分比的buff

	if buff and buff1 then
		local nBuffCount = buff.nStackNum
		local nAverage = buff1.nCustomValue / nBuffCount / 100
		--print(target.szName.."----"..nBuffCount)
		--print(target.szName.."----"..buff1.nCustomValue)
		--print(target.szName.."----"..nAverage)
		if nAverage <= 0.01 then
			nAverage = 0.01
		end
		if target.dwShapeShiftID == 0 and not target.GetBuff(7913,1) then
			target.nCurrentLife = target.nMaxLife * nAverage
		end
	end

	if buff then
		buff.nCustomValue = buff.nCustomValue + 1
		if buff.nStackNum == buff.nCustomValue then
			player.DelBuff(9267, 1)
			player.DelGroupBuff(9268, 1)
		end
	end
end
