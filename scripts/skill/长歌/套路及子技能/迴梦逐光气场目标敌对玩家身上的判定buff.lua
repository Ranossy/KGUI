...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--自身死亡、释放技能的长歌不在线、或者不在同一个场景
	if target.nMoveState == MOVE_STATE.ON_DEATH or not player or (player and player.nMoveState == MOVE_STATE.ON_DEATH) or (player and player.GetScene().dwMapID ~= target.GetScene().dwMapID) then
		local buff_huimeng = target.GetBuff(9272, 1)
		--print(111111)
		if buff_huimeng then
			--print(333333333)
			local nMark = buff_huimeng.nCustomValue
			--玩家中途死亡直接删除影子
			--RemoteCallToClient(target.dwID, "CallUIGlobalFunction", "rlcmd", "destroy shadow 0 "..tostring(nMark))
			local npc_huimeng = GetNpc(nMark)
			if npc_huimeng and npc_huimeng.dwTemplateID == 44766 then
				--print(44444444)
			...
			--删除自身在主人身上的dwID记录buff
			for i = 1, 5 do
				if player then
					local buff_CG = player.GetBuff(9277 + i, 1)
					if buff_CG and buff_CG.dwSkillSrcID == dwCharacterID and player.GetScene().dwMapID == target.GetScene().dwMapID then
