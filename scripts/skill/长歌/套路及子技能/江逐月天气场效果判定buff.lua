...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nbuff = target.GetBuff(10259, 1)
	local nbuffCount = 0
	if nbuff then
		nbuffCount = nbuff.nStackNum
		if nbuffCount > 3 then
			--print(111)
			player.CastSkillXYZ(14149, 1, target.nX, target.nY, target.nZ)
			target.AddBuff(player.dwID, player.nLevel, 10259, 1)
		else
			--print(222)
			player.CastSkillXYZ(14149, 2, target.nX, target.nY, target.nZ)
			target.AddBuff(player.dwID, player.nLevel, 10259, 1)
		end
	else
		--print(3333)
	...
	if player.GetSkillLevel(14349) == 1 then
		...
		player.CastSkillXYZ(14520, 1, target.nX, target.nY, target.nZ)
	end
	
	--奇穴楚济气场AOE效果,1级是15尺半径AOE
	if player.GetSkillLevel(14468) == 1 then
		player.CastSkillXYZ(14469, 2, target.nX, target.nY, target.nZ)
	end
end
