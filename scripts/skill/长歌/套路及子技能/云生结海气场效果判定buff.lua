...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--[[
	local buff = target.GetBuff(9264, 1)
	if buff then
		local nCount = buff.nCustomValue 
		if nCount == 0 and player.GetSkillLevel(14152) == 1 then
			--该buff用于记录平分范围内玩家的百分比值
			player.AddBuff(player.dwID,player.nLevel,9267,1)

			--S_14153释放音域的时候平衡音域范围内所有友方目标奇穴百分比值
			player.CastSkillXYZ(14153, 1, target.nX, target.nY, target.nZ)
			buff.nCustomValue = 1
		end
	end
	--]]
	--对范围内小队目标添加平摊伤害和平摊治疗效果buff(一个技能脚本判定加2个buff)
	player.CastSkillXYZ(14150, 1, target.nX, target.nY, target.nZ)
