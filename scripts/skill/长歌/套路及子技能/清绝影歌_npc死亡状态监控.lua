---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/清绝影歌_npc死亡状态监控.lua
-- 更新时间:	2015/9/11 12:45:59
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player then
		target.SetDisappearFrames(1, 0)
		return
	end
	
	if (player.GetScene().dwMapID ~= target.GetScene().dwMapID) or (player.GetScene().dwID ~= target.GetScene().dwID) then
		--print(11111111)
		target.SetDisappearFrames(1, 0)
	end
	--[[
	elseif target.nMoveState == MOVE_STATE.ON_DEATH or target.nCurrentLife <= target.nMaxLife * 0.01 then
		--print(222222222222222)
		player.SetPosition(target.nX, target.nY, target.nZ)
		--删除长歌自身的buff，并且切换状态
