---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_奇穴楚济_笑傲光阴范围敌对目标添加减疗效果.lua
-- 更新时间:	2015/8/21 19:55:08
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_奇穴楚济_笑傲光阴范围敌对目标添加减疗效果.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	-----------------技能施放技能需求-------------------------------------------
	--skill.AddCheckSelfLearntSkill(14468, 1, SKILL_COMPARE_FLAG.EQUAL);     --检查比较Caster自己已学习的技能ID和等级
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 9514, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if IsPlayer(dwSkillSrcID) then
		npc_cast = GetPlayer(dwSkillSrcID)
	else
		npc_cast = GetNpc(dwSkillSrcID)
	end
	if not npc_cast then
		return
	end
	--print(333)
	local nbuff = npc_cast.GetBuff(9257, 1)
	if nbuff then
		--print(4444)
		local CG_id = nbuff.dwSkillSrcID
		local CG_player = GetPlayer(CG_id)
		if CG_player and CG_player.GetSkillLevel(14468) == 1 and CG_player.GetBuff(9322, 1) then
			--print(555555)
			if IsEnemy(CG_id, dwCharacterID) then
				--print(6666)
				target.AddBuff(CG_player.dwID, CG_player.nLevel, 9514, 1)
			end
		end
	end
end
