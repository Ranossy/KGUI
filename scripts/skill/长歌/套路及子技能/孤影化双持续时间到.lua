...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if shadow_serial ~= 0 then
		--RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "rlcmd", "resume ani as shadow 0 "..tostring(shadow_serial))	--回到影子动作
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "rlcmd", "destroy shadow 0 "..tostring(shadow_serial))			--删除影子
	...
	player.ResetSkillCD(14162)
	--]]
	local player = GetPlayer(dwCharacterID)
	if not player then
		return
	end
	local nbuff = player.GetBuff(9284, 1)
	if nbuff then
		local nbuffleftcount = math.max(nbuff.nLeftActiveCount - 2, 0)
		if player.GetSkillLevel(14164) == 1 then
			RemoteCallToClient(player.dwID, "ChangeSkillSurfaceNum", 17571, nbuffleftcount)
		else
			RemoteCallToClient(player.dwID, "ChangeSkillSurfaceNum", 14162, nbuffleftcount)
		end
	end
end
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	local npc_guiying = GetNpc(nCustomValue)
	if npc_guiying then
	...
	--状态标示清空，技能图标界面
	RemoteCallToClient(player.dwID, "ChangeSkillSurfaceNum", 14522, 0)
