---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/梅花三弄自然消失.lua
-- 更新时间:	2015/8/20 9:06:07
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
		...
		if nLeftFrame ~= 0 then
			...
			if target.nRechargeableAbsorbShieldValue <= 0 then
				...
				if player.GetSkillLevel(14249) == 1 then
					...
					target.SetTimer(4, "scripts/skill/长歌/套路及子技能/梅花三弄自然消失.lua", nLeftCount, 0)
					--print("nLeftCount"..nLeftCount)
				end
			end

			--奇穴绝唱
			local nLeftCount_1 = math.ceil(nLeftFrame / 16)	--落梅debuff为16帧一跳
