---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_曲_阳春白雪.lua
-- 更新时间:	2015/8/15 23:38:40
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK, -- 属性作用模式
		ATTRIBUTE_TYPE.DEL_SINGLE_GROUP_BUFF_BY_ID_AND_LEVEL, -- 魔法属性
		9437, -- 属性值1
		1														-- 属性值2
		);
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_曲_阳春白雪.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(9320, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 9320, 1);		-- 设置1号位Buff
	...
	if dwSkillLevel <= 6 then
		skill.nCostManaBasePercent = 210 * 0.1;	-- 技能消耗的内力
	elseif dwSkillLevel <= 12 then
		skill.nCostManaBasePercent = 210 * 0.2;	-- 技能消耗的内力
	elseif dwSkillLevel <= 18 then
		skill.nCostManaBasePercent = 210 * 0.4;	-- 技能消耗的内力
	elseif dwSkillLevel <= 24 then
		skill.nCostManaBasePercent = 210 * 0.7;	-- 技能消耗的内力
	else
		skill.nCostManaBasePercent = 210;	-- 技能消耗的内力
	end
	----------------- 聚气相关 -------------------------------------------------
	skill.bIsAccumulate = true;				-- 技能是否需要聚气
	skill.nNeedAccumulateCount = 5;				-- 技能需要气点的个数，当skill.bIsAccumulate	= true时生效
	--skill.SetSubsectionSkill(nBeginInterval, nEndInterval, dwSkillID, dwSkillLevel)
	--空技能防止报错
	skill.SetSubsectionSkill(1, 10, 14593, 1)
	----------------- 链状技能相关 ---------------------------------------------
...
function OnSkillLevelUp(skill, player)
	...
	if player.GetSkillLevel(14230) ~= lv then
		player.LearnSkillLevel(14230, lv, player.dwID)
	end

	if player.GetSkillLevel(15090) ~= lv then
		player.LearnSkillLevel(15090, lv, player.dwID)
	end

	if skill.dwLevel == skill.dwMaxLevel then
		player.AcquireAchievement(4653)
	end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "FireUIEvent", "CHANGE_ACCUMULATE_STYLE", 2)

	if player.GetSkillLevel(14470) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 9515, 1)
	end

	--消耗曲风后删除治疗AP加层效果
	if player.GetBuff(9441, 1) then
		player.DelGroupBuff(9441, 1)
	end

	--消耗曲风后删除输出AP加层效果
	if player.GetBuff(9437, 1) then
		player.DelGroupBuff(9437, 1)
	end
end
