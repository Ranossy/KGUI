---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_徵治疗回豆判定.lua
-- 更新时间:	2015/8/28 11:57:08
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_徵治疗回豆判定.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--判定延长跳数buff
	if player.GetBuff(9575, 1) then
		nNeedCount = nNeedCount + 1
	elseif player.GetBuff(9575, 2) then
		nNeedCount = nNeedCount + 2
	elseif player.GetBuff(9575, 3) then
		nNeedCount = nNeedCount + 3
	end
	--print("-------------------")
	--print(nNeedCount)

	--添加徵通道计数buff
	if not player.GetBuff(9576, 1) then
		player.AddBuff(player.dwID, player.nLevel, 9576, 1)
		player.AddBuff(player.dwID, player.nLevel, 9574, 1)

		local nbuff_mark = player.GetBuff(9574, 1)
		--print(nbuff_mark.nStackNum)
		if nbuff_mark and nbuff_mark.nStackNum == nNeedCount then
			--释放回豆技能
			--print("徵积累一个曲风")
			player.CastSkill(14532, 1)
		end
	end
	--print("------------------------------ ")
end
