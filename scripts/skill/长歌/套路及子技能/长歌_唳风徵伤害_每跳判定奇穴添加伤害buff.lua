---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_唳风徵伤害_每跳判定奇穴添加伤害buff.lua
-- 更新时间:	2015/8/28 8:33:30
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_唳风徵伤害_每跳判定奇穴添加伤害buff.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(9570, 1) then
		if not player.GetBuff(9571, 1) then
			--print(2)
			player.AddBuff(player.dwID, player.nLevel, 9571, 1)
			player.SetTimer(4, "scripts/skill/长歌/套路及子技能/长歌_唳风徵伤害_每跳判定奇穴添加伤害buff.lua", 2, 0)
			--player.AddBuff(player.dwID, player.nLevel, 9572, 2)
			--player.DelBuff(9572, 1)
		end
	elseif player.GetBuff(9570, 2) then
		if not player.GetBuff(9571, 1) then
			--print(3)
			player.AddBuff(player.dwID, player.nLevel, 9571, 1)
			player.SetTimer(4, "scripts/skill/长歌/套路及子技能/长歌_唳风徵伤害_每跳判定奇穴添加伤害buff.lua", 3, 0)
			--player.AddBuff(player.dwID, player.nLevel, 9572, 3)
			--player.DelBuff(9572, 2)
		end
	elseif player.GetBuff(9570, 3) then
		if not player.GetBuff(9571, 1) then
			--print(4)
			player.AddBuff(player.dwID, player.nLevel, 9571, 1)
			player.SetTimer(4, "scripts/skill/长歌/套路及子技能/长歌_唳风徵伤害_每跳判定奇穴添加伤害buff.lua", 4, 0)
			--player.AddBuff(player.dwID, player.nLevel, 9572, 4)
			--player.DelBuff(9572, 3)
		end
	elseif player.GetBuff(9570, 4) then
		if not player.GetBuff(9571, 1) then
			--print(5)
			player.AddBuff(player.dwID, player.nLevel, 9571, 1)
			player.SetTimer(4, "scripts/skill/长歌/套路及子技能/长歌_唳风徵伤害_每跳判定奇穴添加伤害buff.lua", 5, 0)
			--player.AddBuff(player.dwID, player.nLevel, 9572, 5)
			--player.DelBuff(9572, 4)
		end
	elseif player.GetBuff(9570, 5) then
		if not player.GetBuff(9571, 1) then
			--print(6)
			player.AddBuff(player.dwID, player.nLevel, 9571, 1)
			--player.AddBuff(player.dwID, player.nLevel, 9572, 6)
			player.SetTimer(4, "scripts/skill/长歌/套路及子技能/长歌_唳风徵伤害_每跳判定奇穴添加伤害buff.lua", 6, 0)
			--player.DelBuff(9572, 5)
		end
	elseif not player.GetBuff(9571, 1) then
		--print(1)
		player.AddBuff(player.dwID, player.nLevel, 9571, 1)
		player.SetTimer(4, "scripts/skill/长歌/套路及子技能/长歌_唳风徵伤害_每跳判定奇穴添加伤害buff.lua", 1, 0)
...
function OnTimer(player, nParam1, nParam2)
	...
	if nParam1 > 1 then
		...
		player.DelBuff(9570, nlev)
	end
	player.AddBuff(player.dwID, player.nLevel, 9570, nParam1)
end
