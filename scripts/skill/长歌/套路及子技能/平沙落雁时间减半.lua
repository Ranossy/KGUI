...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	if nCustomValue == 1 then
		if player then
			target.AddBuff(player.dwID, player.nLevel, 10471, 1)
		else
			target.AddBuff(0, 99, 10471, 1)
		end
	elseif nCustomValue == 2 then
		if player then
			target.AddBuff(player.dwID, player.nLevel, 10472, 1)
		else
			target.AddBuff(0, 99, 10472, 1)
		end
		target.DelBuff(10471,1)
	end
end
