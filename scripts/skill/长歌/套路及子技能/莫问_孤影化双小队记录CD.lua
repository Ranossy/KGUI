---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/莫问_孤影化双小队记录CD.lua
-- 更新时间:	2015/10/23 20:53:00
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	if dwSkillLevel == 1 then
	   	skill.AddAttribute(
	   		...
	   		"skill/长歌/套路及子技能/莫问_孤影化双小队记录CD.lua",											-- 属性值1
	   		0											-- 属性值2
			);
	elseif dwSkillLevel == 2 then
		skill.AddAttribute(
	   		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,					-- 属性作用模式
	   		ATTRIBUTE_TYPE.EXECUTE_SCRIPT,								-- 魔法属性
	   		"skill/长歌/套路及子技能/莫问_孤影化双小队返还CD.lua",											-- 属性值1
	   		0											-- 属性值2
			);
	end
	----------------- 技能施放Buff需求 ---------------------------------------------
	if dwSkillLevel == 1 then
		skill.AddSlowCheckDestBuff(10186, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	elseif dwSkillLevel == 2 then
		skill.AddSlowCheckDestBuff(10186, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	end
	...
	-----------------技能施放技能需求-------------------------------------------
	skill.AddCheckSelfLearntSkill(15175, 1, SKILL_COMPARE_FLAG.EQUAL);     --检查比较Caster自己已学习的技能ID和等级
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	target.AddBuff(player.dwID, player.nLevel, 10186, 1)
	local nbuff = target.GetBuff(10186, 1)
	if nbuff then
		nbuff.nCustomValue = target.nCurrentLife / target.nMaxLife * 100
	end
end
