---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_曲_梅花三弄.lua
-- 更新时间:	2015/7/31 16:48:06
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_曲_梅花三弄.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(9321, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 9321, 1);		-- 设置1号位Buff
...
function OnSkillLevelUp(skill, player)
	...
	if player.GetSkillLevel(14231) ~= lv then
		player.LearnSkillLevel(14231, lv, player.dwID)
	end

	if skill.dwLevel == skill.dwMaxLevel then
		player.AcquireAchievement(4654)
	end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "FireUIEvent", "CHANGE_ACCUMULATE_STYLE", 3)
