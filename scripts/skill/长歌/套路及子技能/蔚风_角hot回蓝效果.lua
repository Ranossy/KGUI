---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/蔚风_角hot回蓝效果.lua
-- 更新时间:	2015/8/15 21:20:29
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nbuff_mark = target.GetBuffByOwner(9512, 1, player.dwID)
	
	if player.GetSkillLevel(14464) == 1 and nbuff_mark then
		player.nCurrentMana = player.nCurrentMana + player.nMaxMana * 0.01
		if IsPlayer(target.dwID) and dwCharacterID ~= dwSkillSrcID then
