...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff_x = target.GetBuff(9300, 1)
	local buff_y = target.GetBuff(9301, 1)
	local buff_z = target.GetBuff(9302, 1)
	local buff_dir = target.GetBuff(9303, 1)

	target.Stop()
	target.Die()
	--RemoteCallToClient(target.dwID, "CallUIGlobalFunction", "rlcmd", "resume ani as shadow 0 3")	--回到影子动作
	--RemoteCallToClient(target.dwID, "CallUIGlobalFunction", "rlcmd", "destroy shadow 0 3")			--删除影子
	RemoteCallToClient(target.dwID, "CallUIGlobalFunction", "rlcmd", "leave camera Post render")	--离开镜头状态
	RemoteCallToClient(target.dwID, "CallUIGlobalFunction", "rlcmd", "set virtual 0 0")				--关闭虚化状态
	--回复坐标
	if buff_x and buff_y and buff_z then
		if buff_dir and buff_dir.nCustomValue == target.GetScene().dwID then
	...
	for i = 1, 6 do
		target.DelBuff(9297 + i, 1)
	end

	--附魔伤害效果
	target.DelBuff(10184, 1)
end
