---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/争簇_徵效果减低间隔变短.lua
-- 更新时间:	2017/3/20 11:58:51
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/争簇_徵效果减低间隔变短.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.AddBuff(player.dwID, player.nLevel, 9523, 1)

	--必须删除增加徵跳数的buff
	for i = 1, 3 do
		if player.GetBuff(9439, i) then
			player.DelBuff(9439, i)
		end
		if player.GetBuff(9503, i) then
			player.DelBuff(9503, i)
		end
		if player.GetBuff(9502, i) then
			player.DelBuff(9502, i)
		end
		if player.GetBuff(9353, i) then
			player.DelBuff(9353, i)
		end
	end
end
