---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_迴梦逐光_范围内敌对目标产生影子效果.lua
-- 更新时间:	2015/7/27 21:30:06
-- 更新用户:	mengxiangfei11
-- 脚本说明:
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	...
   	skill.AddAttribute(
   		...
   		"skill/长歌/套路及子技能/长歌_迴梦逐光_范围内敌对目标产生影子效果.lua",											-- 属性值1
   		0											-- 属性值2
   		);
	...
	--每个玩家同时不能受到多个长歌的影子标记
	skill.AddSlowCheckDestBuff(9271, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	skill.nHeight = 30 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not target or (target.dwShapeShiftID ~= 0 or target.GetBuff(7913,1)) then
		return
	end

	--创建影子，这里要注意影子的序列号，影子持续时间偏长，是靠切换图标buff来控制时间的,但是也应该有自己的时间，保证影子自动删除
	...
	if player.GetSkillLevel(14159) == 1 then
		npc_huimeng = scene.CreateNpc(46140, target.nX, target.nY, target.nZ, target.nFaceDirection, 17 * 16)
		npc_huimeng.SetNpcCopyRepresentByPlayerID(npc_huimeng.dwID, target.dwID)
		--效果期间不再受到其他玩家的技能迴梦逐光影响了,也是玩家解控的手段
		target.AddBuff(player.dwID, player.nLevel, 9271, 1, 17)
		--RemoteCallToClient(target.dwID, "CallUIGlobalFunction", "rlcmd", "create shadow 0 17500 1 "..tostring(shadow_Serial_only))
	else
		npc_huimeng = scene.CreateNpc(46140, target.nX, target.nY, target.nZ, target.nFaceDirection, 9 * 16)
		npc_huimeng.SetNpcCopyRepresentByPlayerID(npc_huimeng.dwID, target.dwID)
		--RemoteCallToClient(target.dwID, "CallUIGlobalFunction", "rlcmd", "create shadow 0 9500 1 "..tostring(shadow_Serial_only))
		--效果期间不再受到其他玩家的技能迴梦逐光影响了,也是玩家解控的手段
		target.AddBuff(player.dwID, player.nLevel, 9271, 1, 9)
	end


	--给目标添加检测判定buff，4帧一跳的buff
	target.AddBuff(player.dwID, player.nLevel, 9272, 1, 260)
	
	--local shadow_Serial_only = 1

	local buff_huimeng = target.GetBuff(9272, 1)
	if buff_huimeng and npc_huimeng then
		buff_huimeng.nCustomValue = npc_huimeng.dwID
	end

	--开始记录目标坐标朝向
	target.AddBuff(player.dwID, player.nLevel, 9273, 1)
	target.AddBuff(player.dwID, player.nLevel, 9274, 1)
	target.AddBuff(player.dwID, player.nLevel, 9275, 1)
	target.AddBuff(player.dwID, player.nLevel, 9276, 1)
	...
	if buff_x then
		buff_x.nCustomValue = target.nX
	end

	if buff_y then
		buff_y.nCustomValue = target.nY
	end

	if buff_z then
		buff_z.nCustomValue = target.nZ
	end

	if buff_dir then
		buff_dir.nCustomValue = target.nFaceDirection
	end

	--记录5个目标的ID在自己身上
	for i = 1, 5 do
		if not player.GetBuff(9277 + i, 1) then
			player.AddBuff(target.dwID, target.nLevel, 9277 + i, 1)
			break
		end
	end
end
