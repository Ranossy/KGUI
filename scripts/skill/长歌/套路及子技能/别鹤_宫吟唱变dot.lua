---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/别鹤_宫吟唱变dot.lua
-- 更新时间:	2015/8/5 21:09:46
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/别鹤_宫吟唱变dot.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--print("加载奇穴宫伤害变为宫dot")
	local lv = player.GetSkillLevel(14064)
	if not player.GetBuff(9377, 1) and lv ~= 0 then
		--print(111)
		RecipeCopy(14064, lv, player, 14302)
		RemoteCallToClient(player.dwID, "OnActionBarSkillReplace", 14064, 14302, lv)	--吟唱换dot
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "NewSkillPanel_RefeshKungfuSkillAt", 14064)
		--RemoteCallToClient(player.dwID, "OnSkillReplace", 14064, 14302, lv)	--吟唱换dot
	end
	--[[
	if lv ~= 0 then
		--print(8)
		player.ForgetSkill(14064)
		player.LearnSkillLevel(14302, lv, player.dwID)
		RecipeCopy(14064, lv, player, 14302)
		RemoteCallToClient(player.dwID, "OnActionBarSkillReplace", 14064, 14302, 1)	--吟唱换dot
	end
	--]]
end
...
function UnApply(dwCharacterID)
	...
	--print("卸除奇穴宫dot变为宫伤害")
	local lv = player.GetSkillLevel(14302)
	if not player.GetBuff(9377, 1) and lv ~= 0 then
		--print(222)
		RecipeCopy(14302, lv, player, 14064)
		RemoteCallToClient(player.dwID, "OnActionBarSkillReplace", 14302, 14064, lv)	--dot换吟唱
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "NewSkillPanel_RefeshKungfuSkillAt", 14064)
		--RemoteCallToClient(player.dwID, "OnSkillReplace", 14302, 14064, lv)	--dot换吟唱
	end
	--[[
	if lv ~= 0 then
		player.ForgetSkill(14302)
		player.LearnSkillLevel(14064, lv, player.dwID)
		RecipeCopy(14302, lv, player, 14064)
		RemoteCallToClient(player.dwID, "OnActionBarSkillReplace", 14302, 14064, 1)	--dot换吟唱		
	end
	--]]
end
