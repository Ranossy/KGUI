---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_云生结海_平衡小队气血值技能.lua
-- 更新时间:	2015/7/27 17:00:09
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_云生结海_平衡小队气血值技能.lua", -- 属性值1
		0											-- 属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(9269, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	--延迟回血buff
	skill.BindBuff(1, 9269, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff = player.GetBuff(9267, 1)
	if buff then
		--print("-----"..target.szName.."------"..buff.nCustomValue)
		buff.nCustomValue = buff.nCustomValue + (target.nCurrentLife / target.nMaxLife) * 100
		--print("-----"..target.szName.."------"..buff.nCustomValue.."----"..target.nCurrentLife.."----"..target.nMaxLife.."----"..target.nCurrentLife / target.nMaxLife)
	end
end
