---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/疏影横斜判定障碍.lua
-- 更新时间:	2017/3/10 17:16:13
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	local npc_yingzi = GetNpc(dwCharacterID)
	if not npc_yingzi then
		return
	end

	local player = npc_yingzi.GetEmployer()
	if not player then
		return
	end
	...
	--buff映射关系
	local nMapping = {
		[10134] = {B_ID = 10148, S_ID = 15039},
		[10135] = {B_ID = 10149, S_ID = 15040},
		[10136] = {B_ID = 10150, S_ID = 15041},
		[10137] = {B_ID = 10151, S_ID = 15042},
		[10138] = {B_ID = 10152, S_ID = 15043},
		[10139] = {B_ID = 10153, S_ID = 15044},
	}

	--释放障碍距离判定技能，更新界面状态
	--local nResult = npc_yingzi.CastSkill(15138, 1, TARGET.PLAYER, player.dwID)
	local Is_pre, S_ID, S_lev, L_time = player.GetSkillPrepareState()
	if Is_pre and S_ID == 3691 or S_ID == 6191 or player.GetOTActionState() == 4 or player.GetOTActionState() == 5 then
		for i = 1, 6 do
			if npc_yingzi.GetBuff(10133 + i, 1) then
				player.AddBuff(player.dwID, player.nLevel, nMapping[10133 + i].B_ID, 1)
				RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "FireUIEvent", "UPDATE_SHADOW_STATE ", nMapping[10133 + i].S_ID, npc_yingzi.dwID, true)
				break
			end
		end
		--print(11111)
		...
		--print(22222)
		local nResult = player.CastSkill(15138, 1, TARGET.NPC, dwCharacterID)
		if nResult == 1 then
			--print(33333)
			for i = 1, 6 do
				if npc_yingzi.GetBuff(10133 + i, 1) then
					if player.GetBuff(nMapping[10133 + i].B_ID, 1) then
						--print(1111111)
						player.DelBuff(nMapping[10133 + i].B_ID, 1)
					end
					--print(22222)
					RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "FireUIEvent", "UPDATE_SHADOW_STATE ", nMapping[10133 + i].S_ID, npc_yingzi.dwID, false)
					break
				end
			end
		else
