---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_气_迴梦逐光.lua
-- 更新时间:	2015/7/27 17:54:44
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_气_迴梦逐光.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	skill.SetNormalCoolDown(1, 903);	--音域技能单独100sGCD
	skill.SetNormalCoolDown(2, 1151);	--音域技能GCD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--切换图标buff，这个buff自然结束后要删除之前敌对目标身上的buff标记效果,16帧作用一次
	if player.GetSkillLevel(14159) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 9277, 1, 16)
	else
		player.AddBuff(player.dwID, player.nLevel, 9277, 1, 8)
	end

	--创建迴梦逐光气场NPC
	local npc
	if player.GetSkillLevel(14159) == 1 then
		npc = scene.CreateNpcWithEmployerID(44766, target.nX, target.nY, target.nZ, 0, 16 * 16, player.dwID, false, player.dwIdentityVisiableID)
		--npc.SetNpcCopyRepresentByPlayerID(npc.dwID, target.dwID)
	else
		--print(target.dwID)
		--print(dwCharacterID)
		npc = scene.CreateNpcWithEmployerID(44766, target.nX, target.nY, target.nZ, 0, 8 * 16, player.dwID, false, player.dwIdentityVisiableID)
		--npc.SetNpcCopyRepresentByPlayerID(npc.dwID, target.dwID)
	end
	if npc then
		--防止其它NPC把气场搜到仇恨范围
		npc.AddBuff(player.dwID, player.nLevel, 9256, 1, 1)

		--持续打效果的buff，buff是4帧一跳,一共无脑加80跳，20秒，buff根据剩余跳数来决定情况
		npc.AddBuff(player.dwID, player.nLevel, 9270, 1, 80)

		--气场释放时产生一个性的记录效果
		player.CastSkillXYZ(14157, 1, npc.nX, npc.nY, npc.nZ)

		local buff_changge_biaoji = player.GetBuff(9277, 1)
		if buff_changge_biaoji then
			buff_changge_biaoji.nCustomValue = npc.dwID
		end

		--奇穴楚济气场AOE效果,2级是10尺半径AOE
		if player.GetSkillLevel(14468) == 1 then
			player.CastSkillXYZ(14469, 2, npc.nX, npc.nY, npc.nZ)
		end
	end
end
