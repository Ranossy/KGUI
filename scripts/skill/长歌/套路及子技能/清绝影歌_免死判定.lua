...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npc_yingge = nil
	if nbuff_mark then
		local yingge_npc_ID = nbuff_mark.nCustomValue
		npc_yingge = GetNpc(yingge_npc_ID)
		if not npc_yingge or (npc_yingge and (player.GetScene().dwMapID ~= npc_yingge.GetScene().dwMapID) or (player.GetScene().dwID ~= npc_yingge.GetScene().dwID)) then
			--print(2222222222)
			--删除长歌自身的buff，并且切换状态
