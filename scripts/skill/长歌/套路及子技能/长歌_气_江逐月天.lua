---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_气_江逐月天.lua
-- 更新时间:	2017/4/17 12:23:24
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_气_江逐月天.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	skill.SetNormalCoolDown(1, 899);	--音域技能单独100sGCD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--创建江逐月天气场NPC
	local npc = scene.CreateNpcWithEmployerID(44764, target.nX, target.nY, target.nZ, 0, 8 * 16, player.dwID, false, player.dwIdentityVisiableID)
	...
	if npc then
		...
		--持续打效果的buff
		npc.AddBuff(player.dwID, player.nLevel, 9262, 1, 8)

		--对范围内小队目标添加暂停CD效果
		--player.CastSkillXYZ(14149, 1, npc.nX, npc.nY, npc.nZ)
		player.CastSkillXYZ(14149, 2, npc.nX, npc.nY, npc.nZ)
		npc.AddBuff(player.dwID, player.nLevel, 10259, 1)

		if player.GetSkillLevel(14349) == 1 then
			--奇穴沁心给友方目标打在音域内的标记buff
			player.CastSkillXYZ(14520, 1, npc.nX, npc.nY, npc.nZ)
		end
