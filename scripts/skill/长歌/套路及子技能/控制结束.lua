---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/控制结束.lua
-- 更新时间:	2017/4/26 10:17:58
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.dwControlPlayerID == 0 then
		player.DelBuff(10173, 1)
	end
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	target.Stop()
	target.nCurrentMana = nCustomValue
	...
	player.EndControlPlayer()
	target.DelBuff(10174, 1)
--这部分功能skill.lh已经注释，由程序功能实现，先注释。
	--[[
	local SkillCDTable = {
