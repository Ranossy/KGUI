---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_曲_阳春白雪主动技能_治疗.lua
-- 更新时间:	2015/10/11 16:44:43
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_曲_阳春白雪主动技能.lua", -- 属性值1
		0											-- 属性值2
		);
	--]]
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(9320, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	skill.AddSlowCheckSelfBuff(9506, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff

	--需求有可释放的标记buff
	skill.AddSlowCheckSelfBuff(9430, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	skill.SetNormalCoolDown(1, 916);	--技能普通CD
	...
	if dwSkillLevel <= 6 then
		skill.nCostManaBasePercent = 120 * 0.1;	-- 技能消耗的内力
	elseif dwSkillLevel <= 12 then
		skill.nCostManaBasePercent = 120 * 0.2;	-- 技能消耗的内力
	elseif dwSkillLevel <= 18 then
		skill.nCostManaBasePercent = 120 * 0.4;	-- 技能消耗的内力
	elseif dwSkillLevel <= 24 then
		skill.nCostManaBasePercent = 120 * 0.7;	-- 技能消耗的内力
	else
		skill.nCostManaBasePercent = 120;	-- 技能消耗的内力
	end
	----------------- 聚气相关 -------------------------------------------------
...
function OnSkillLevelUp(skill, player)
	...
	if player.GetSkillLevel(14070) ~= lv then
		player.LearnSkillLevel(14070, lv, player.dwID)
	end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nlev = player.GetSkillLevel(14230)

	if nlev ~= 0 then
		player.CastSkill(14343, nlev, player.GetSkillTarget())		--阳春白雪伤害
		player.CastSkill(14344, nlev, player.GetSkillTarget())		--阳春白雪治疗
	end
	--]]
end
