---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_音_角hot.lua
-- 更新时间:	2015/8/10 20:53:28
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_音_角hot.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	if dwSkillLevel <= 6 then
		skill.nCostManaBasePercent = 400 * 0.2;	-- 技能消耗的内力
	elseif dwSkillLevel <= 12 then
		skill.nCostManaBasePercent = 400 * 0.4;	-- 技能消耗的内力
	elseif dwSkillLevel <= 18 then
		skill.nCostManaBasePercent = 400 * 0.6;	-- 技能消耗的内力
	elseif dwSkillLevel <= 24 then
		skill.nCostManaBasePercent = 400 * 0.8;	-- 技能消耗的内力
	else
		skill.nCostManaBasePercent = 400;	-- 技能消耗的内力
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local jue_lev = player.GetSkillLevel(14139)		--取技能角hot的等级

	if jue_lev ~= 0 then
		--普通dot
		if buff_jiange ~= 1 and buff_xiaoguo ~= 1 then
			player.CastSkill(14399, jue_lev, player.GetSkillTarget())
		end
		--间隔缩短的dot
		if buff_jiange == 1 and buff_xiaoguo ~= 1 then
			player.CastSkill(14400, jue_lev, player.GetSkillTarget())
		end
		--递增的dot
		if buff_jiange ~= 1 and buff_xiaoguo == 1 then
			player.CastSkill(14401, jue_lev, player.GetSkillTarget())
		end
		--递增且间隔缩短的dot
		if buff_jiange == 1 and buff_xiaoguo == 1 then
			player.CastSkill(14402, jue_lev, player.GetSkillTarget())
		end
	end

	--橙武吞噬角hot
	player.CastSkill(15186, 1, player.GetSkillTarget())
	...
	--蔚风角hot回蓝
	if player.GetSkillLevel(14464) == 1 then
		--if IsPlayer(target.dwID) then
		--只是打一个标记buff，实际效果在角dot中判定
		target.AddBuff(player.dwID, player.nLevel, 9512, 1)
		--end
	end
end
