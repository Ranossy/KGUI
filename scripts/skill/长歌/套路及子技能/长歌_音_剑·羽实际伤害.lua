---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_音_剑·羽实际伤害.lua
-- 更新时间:	2015/8/13 22:30:43
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_音_剑·羽实际伤害.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 9569, 1);		-- 设置1号位Buff
	...
	--skill.nChannelInterval	= 165; 				-- 通道技间隔时间
	...
	elseif dwSkillLevel < 25 then
		skill.nChannelInterval = 60 + (dwSkillLevel - 9) * 10
	else
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(10090, 1) then
		player.DelBuff(10090, 1)
	elseif player.GetBuff(10089, 1) then
		player.DelBuff(10089, 1)
		player.AddBuff(player.dwID, player.nLevel, 10090, 1)
	else
		player.AddBuff(player.dwID, player.nLevel, 10089, 1)
	end
end
