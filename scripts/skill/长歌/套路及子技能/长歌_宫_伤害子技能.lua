---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_宫_伤害子技能.lua
-- 更新时间:	2015/8/9 13:35:06
-- 更新用户:	mengxiangfei11
-- 脚本说明:
...
tSkillData =
{
	{nDamageBase = 100, nDamageRand = 10, nCostMana = 0}, --level 1
	...
	{nDamageBase = 100, nDamageRand = 10, nCostMana = 0}, --level 11
	...
	{nDamageBase = 100, nDamageRand = 10, nCostMana = 0}, --level 21
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_宫_伤害子技能.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	elseif dwSkillLevel < 25 then
		skill.nChannelInterval = 36 + (dwSkillLevel - 9) * 8
	else
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local gong_lev = player.GetSkillLevel(14302)
	
	--商一组buff,应该是4个一组
	if shang_lev ~= 0 then
		if target.GetBuffByOwner(9357, shang_lev, player.dwID) or target.GetBuffByOwner(9358, shang_lev, player.dwID) or target.GetBuffByOwner(9359, shang_lev, player.dwID) or target.GetBuffByOwner(9360, shang_lev, player.dwID) then
	...
	--角一组buff
	if jue_lev ~= 0 then
		if target.GetBuffByOwner(9361, jue_lev, player.dwID) or target.GetBuffByOwner(9362, jue_lev, player.dwID) or target.GetBuffByOwner(9363, jue_lev, player.dwID) or target.GetBuffByOwner(9364, jue_lev, player.dwID) then
			nbuffCount = nbuffCount + 1
		end
	end
	...
	--宫一组buff
	if gong_lev ~= 0 then
		if target.GetBuffByOwner(9373, gong_lev, player.dwID) or target.GetBuffByOwner(9374, gong_lev, player.dwID) or target.GetBuffByOwner(9375, gong_lev, player.dwID) or target.GetBuffByOwner(9376, gong_lev, player.dwID) then
			nbuffCount = nbuffCount + 1
		end
	end
	...
	if player.GetSkillLevel(14282) == 1 then
		--[[
		local buff_now = player.GetBuff(9355, 1)
		local buff_now_count = 0
		if buff_now then
			buff_now_count = buff_now.nStackNum
		end
		if nbuffCount >= buff_now_count and nbuffCount ~= 0 then
