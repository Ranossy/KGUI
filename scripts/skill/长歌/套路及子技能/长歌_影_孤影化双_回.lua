---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_影_孤影化双_回.lua
-- 更新时间:	2015/7/28 14:55:46
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_影_孤影化双_回.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(9506, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	skill.AddSlowCheckSelfBuff(9284, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff_x = player.GetBuff(9285, 1)
	local buff_y = player.GetBuff(9286, 1)
	local buff_z = player.GetBuff(9287, 1)
	local buff_dir = player.GetBuff(9288, 1)
	local buff_hp = player.GetBuff(9289, 1)
	local buff_mp = player.GetBuff(9290, 1)

	--恢复气血
	if buff_hp then
		player.nCurrentLife = buff_hp.nCustomValue
	end

	--恢复内力
	if buff_mp then
		player.nCurrentMana = buff_mp.nCustomValue
	end
	--[[
	--找到自己影子序列
	local shadow_serial = 0
	local buff_guying = player.GetBuff(9283, 1)
	if buff_guying then
		shadow_serial = buff_guying.nCustomValue
	end
	--回复坐标
	if buff_x and buff_y and buff_z then
		...
		if shadow_serial ~= 0 then
			RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "rlcmd", "resume shadow 0 "..tostring(shadow_serial))			--召回影子
		end
	end
	--]]
	...
	local npc_dwID
	if buff_guying then
		npc_dwID = buff_guying.nCustomValue
		local npc_guiying = GetNpc(npc_dwID)
		if npc_guiying then
			npc_guiying.SetDisappearFrames(1, 0)
		end
	end
	...
	if player.GetSkillLevel(15175) == 1 then
		player.CastSkill(15176, 2)
	end

	--删除所有buff效果
	for i = 1, 8 do
		player.DelBuff(9282 + i, 1)
	end

	--气血召唤回影子后减伤和免控效果
	if player.GetSkillLevel(14164) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 9293, 1)
		player.AddBuff(player.dwID, player.nLevel, 9294, 1)
	end
end
