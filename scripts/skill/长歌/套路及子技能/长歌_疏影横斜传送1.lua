---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_疏影横斜传送1.lua
-- 更新时间:	2015/10/3 23:31:27
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(												
		...
		"skill/长歌/套路及子技能/长歌_疏影横斜传送1.lua",
		0									
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(9993, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	skill.AddSlowCheckSelfBuff(9509, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	skill.AddSlowCheckSelfBuff(10148, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nbuff = player.GetBuff(9993, 1)
	if nbuff then
		local npc_dwID = nbuff.nCustomValue
		local yingzi_npc = GetNpc(npc_dwID)
		if yingzi_npc then
			player.PlaySfx(1249, player.nX, player.nY, player.nZ)
			player.PlaySfx(1250, yingzi_npc.nX, yingzi_npc.nY, yingzi_npc.nZ)
			player.SetPosition(yingzi_npc.nX, yingzi_npc.nY, yingzi_npc.nZ)
			player.nCurrentMana = player.nCurrentMana + player.nMaxMana * 0.15
			player.AddBuff(player.dwID, player.nLevel, 10247, 1)
			yingzi_npc.SetDisappearFrames(1, 0)
			--if player.GetBuff(10000, 1) then
				--player.DelBuff(10000, 1)
			--end
			if player.GetSkillLevel(14874) == 1 then
				player.AddBuff(player.dwID, player.nLevel, 9999, 1)
			end
		end
		player.DelBuff(9993, 1)
	end
end
