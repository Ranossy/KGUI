---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_音_徵_治疗.lua
-- 更新时间:	2015/8/10 15:27:58
-- 更新用户:	mengxiangfei11
-- 脚本说明:
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	if dwSkillLevel <= 6 then
		skill.nCostManaBasePercent = 680 * 0.2;	-- 技能消耗的内力
	elseif dwSkillLevel <= 12 then
		skill.nCostManaBasePercent = 680 * 0.4;	-- 技能消耗的内力
	elseif dwSkillLevel <= 18 then
		skill.nCostManaBasePercent = 680 * 0.6;	-- 技能消耗的内力
	elseif dwSkillLevel <= 24 then
		skill.nCostManaBasePercent = 680 * 0.8;	-- 技能消耗的内力
	else
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	for i = 1, 5 do
		if player.GetBuff(9442 + i, 1) then
			player.DelBuff(9442 + i, 1)
		end
	end
	for i = 1, 5 do
		if player.GetBuff(9447 + i, 1) then
			player.DelBuff(9447 + i, 1)
		end
	end
...
function OnSkillLevelUp(skill, player)
	...
	if skill.dwLevel == skill.dwMaxLevel and not player.IsAchievementAcquired(5128) then
		player.AcquireAchievement(5128)
	end
end
	...
	if player.GetSkillLevel(14472) == 1 then
		...
		if (buff_mark and buff_mark.nStackNum < 2) or not buff_mark then
			...
			if jue_leftcount ~= 0 and jue_leftcount >= gong_leftcount and jue_leftcount >= shang_leftcount then
				...
				if IsPlayer(dwCharacterID) then
					player.CastSkill(14500, 1, TARGET.PLAYER, dwCharacterID)
				else
					player.CastSkill(14500, 1, TARGET.NPC, dwCharacterID)
				end
			...
			elseif shang_leftcount ~= 0 and shang_leftcount >= jue_leftcount and shang_leftcount >= gong_leftcount then
				...
				if IsPlayer(dwCharacterID) then
					player.CastSkill(14499, 1, TARGET.PLAYER, dwCharacterID)
				else
					player.CastSkill(14499, 1, TARGET.NPC, dwCharacterID)
				end
				if not buff_mark then
					if player.GetSkillLevel(14352) == 1 then
						player.ResetCD(923)
			...
			elseif gong_leftcount ~= 0 and gong_leftcount >= shang_leftcount and gong_leftcount >= jue_leftcount then
				...
				if IsPlayer(dwCharacterID) then
					player.CastSkill(14498, 1, TARGET.PLAYER, dwCharacterID)
				else
					player.CastSkill(14498, 1, TARGET.NPC, dwCharacterID)
				end
