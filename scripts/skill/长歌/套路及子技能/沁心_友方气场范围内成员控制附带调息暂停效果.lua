---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/沁心_友方气场范围内成员控制附带调息暂停效果.lua
-- 更新时间:	2015/8/20 20:16:29
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff_nmark = player.GetBuff(9549, 1)		--释放者是否在音域中的标识buff
	if buff_nmark then
		--local CG_ID = buff.dwSkillSrcID
		--local CG_player = GetPlayer(CG_ID)
		--if CG_player and CG_player.GetSkillLevel(14349) == 1 then
		target.AddBuff(player.dwID, player.nLevel, 9436, 1)		--给目标添加CD暂停buff
		--end
	end
end
