---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_广雅_宫会心刷新目标dot.lua
-- 更新时间:	2015/8/7 10:37:28
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/长歌/套路及子技能/长歌_广雅_宫会心刷新目标dot.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if shang_lev ~= 0 then
		--刷新正常商dot
		if target.GetBuffByOwner(9357, shang_lev, player.dwID) then
			player.CastSkill(17788, shang_lev, player.GetSkillTarget())
		end
		--刷新缩短间隔商dot
		if target.GetBuffByOwner(9358, shang_lev, player.dwID) then
			player.CastSkill(17789, shang_lev, player.GetSkillTarget())
		end
		--刷新效果递增商dot
		if target.GetBuffByOwner(9359, shang_lev, player.dwID) then
			player.CastSkill(17790, shang_lev, player.GetSkillTarget())
		end
		--刷新递增且缩短间隔商dot
		if target.GetBuffByOwner(9360, shang_lev, player.dwID) then
			player.CastSkill(17791, shang_lev, player.GetSkillTarget())
		end
	end

	if jue_lev ~= 0 then
		--刷新正常角dot
		if target.GetBuffByOwner(9361, jue_lev, player.dwID) then
			--print(232323)
			player.CastSkill(17792, jue_lev, player.GetSkillTarget())
		end
		--刷新缩短间隔角dot
		if target.GetBuffByOwner(9362, jue_lev, player.dwID) then
			player.CastSkill(17793, jue_lev, player.GetSkillTarget())
		end
		--刷新效果递增角dot
		if target.GetBuffByOwner(9363, jue_lev, player.dwID) then
			player.CastSkill(17794, jue_lev, player.GetSkillTarget())
		end
		--刷新递增且缩短间隔角dot
		if target.GetBuffByOwner(9364, jue_lev, player.dwID) then
			player.CastSkill(17795, jue_lev, player.GetSkillTarget())
		end
	end
