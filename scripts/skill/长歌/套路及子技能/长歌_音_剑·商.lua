---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_音_剑·商.lua
-- 更新时间:	2015/10/2 21:07:53
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(9506, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	skill.AddSlowCheckSelfBuff(9509, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	--skill.nCostManaBasePercent = 200;							-- 技能消耗的内力百分比
	if dwSkillLevel <= 6 then
		skill.nCostManaBasePercent = 150 * 0.2;	-- 技能消耗的内力
	elseif dwSkillLevel <= 12 then
		skill.nCostManaBasePercent = 150 * 0.4;	-- 技能消耗的内力
	elseif dwSkillLevel <= 18 then
		skill.nCostManaBasePercent = 150 * 0.6;	-- 技能消耗的内力
	elseif dwSkillLevel <= 24 then
		skill.nCostManaBasePercent = 150 * 0.8;	-- 技能消耗的内力
	else
		skill.nCostManaBasePercent = 150;	-- 技能消耗的内力
	end
	----------------- 聚气相关 -------------------------------------------------
	...
	--skill.nHeight = 0 * LENGTH_BAS E;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
	...
	elseif dwSkillLevel < 25 then
		skill.nChannelInterval = 100 + (dwSkillLevel - 9) * 15
	else
		skill.nChannelInterval = 350;     -- 通道技间隔时间
	end
	----------------- 阵法相关 -------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(9986, 1) then
		player.DelBuff(9986, 1)
	elseif player.GetBuff(9985, 1) then
		player.DelBuff(9985, 1)
		player.AddBuff(player.dwID, player.nLevel, 9986, 1)
	else
		player.AddBuff(player.dwID, player.nLevel, 9985, 1)
	end
end
