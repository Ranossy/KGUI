---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_音_商hot.lua
-- 更新时间:	2015/8/10 20:30:44
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_音_商hot.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	if dwSkillLevel <= 6 then
		skill.nCostManaBasePercent = 380 * 0.2;	-- 技能消耗的内力
	elseif dwSkillLevel <= 12 then
		skill.nCostManaBasePercent = 380 * 0.4;	-- 技能消耗的内力
	elseif dwSkillLevel <= 18 then
		skill.nCostManaBasePercent = 380 * 0.6;	-- 技能消耗的内力
	elseif dwSkillLevel <= 24 then
		skill.nCostManaBasePercent = 380 * 0.8;	-- 技能消耗的内力
	else
		skill.nCostManaBasePercent = 380;	-- 技能消耗的内力
	end
...
function OnSkillLevelUp(skill, player)
	if skill.dwLevel == skill.dwMaxLevel and not player.IsAchievementAcquired(5127) then
		player.AcquireAchievement(5127)
	end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local shang_lev = player.GetSkillLevel(14138)		--取技能商hot的等级

	if shang_lev ~= 0 then
		--普通dot
		if buff_jiange ~= 1 and buff_xiaoguo ~= 1 then
			player.CastSkill(14395, shang_lev, player.GetSkillTarget())
		end
		--间隔缩短的dot
		if buff_jiange == 1 and buff_xiaoguo ~= 1 then
			player.CastSkill(14396, shang_lev, player.GetSkillTarget())
		end
		--递增的dot
		if buff_jiange ~= 1 and buff_xiaoguo == 1 then
			player.CastSkill(14397, shang_lev, player.GetSkillTarget())
		end
		--递增且间隔缩短的dot
		if buff_jiange == 1 and buff_xiaoguo == 1 then
			player.CastSkill(14398, shang_lev, player.GetSkillTarget())
		end
	end
	--参连每个曲风叠加2%基础内功AP
