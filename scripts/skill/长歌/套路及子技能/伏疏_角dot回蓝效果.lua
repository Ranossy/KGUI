---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/伏疏_角dot回蓝效果.lua
-- 更新时间:	2015/8/15 21:46:36
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nbuff_mark = target.GetBuffByOwner(9513, 1, player.dwID)
	
	if player.GetSkillLevel(14465) == 1 and nbuff_mark then
		print(111111)
		player.nCurrentMana = player.nCurrentMana + player.nMaxMana * 0.01
		if IsPlayer(target.dwID) then
			print(22222)
			if target.nCurrentMana > target.nMaxMana * 0.01 then
