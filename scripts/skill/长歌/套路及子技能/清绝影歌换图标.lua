---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/清绝影歌换图标.lua
-- 更新时间:	2015/8/14 15:08:43
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--删除高山流水替换效果
	if target.GetBuff(9377, 1) then
		target.DelBuff(9377, 1)
	end


	local shang_lev = target.GetSkillLevel(14065)
	if shang_lev ~= 0 then
		--替换商
		RemoteCallToClient(target.dwID, "OnActionBarSkillReplace", 14065, 14450, shang_lev)
	end

	local jue_lev = target.GetSkillLevel(14066)
	if jue_lev ~= 0 then
		--替换角
		RemoteCallToClient(target.dwID, "OnActionBarSkillReplace", 14066, 14453, jue_lev)
	end

	local zhi_lev = target.GetSkillLevel(14067)
	if zhi_lev ~= 0 then
		--替换徵
		RemoteCallToClient(target.dwID, "OnActionBarSkillReplace", 14067, 14451, zhi_lev)
	end

	local yu_lev = target.GetSkillLevel(14068)
	if yu_lev ~= 0 then
		--替换羽
		RemoteCallToClient(target.dwID, "OnActionBarSkillReplace", 14068, 14452, yu_lev)
	end

	
	if target.GetSkillLevel(14308) == 1 then
		local gong_lev = target.GetSkillLevel(14302)
		if gong_lev ~= 0 then
			--替换宫dot
			RemoteCallToClient(target.dwID, "OnActionBarSkillReplace", 14302, 14449, gong_lev)
		end
	else
		local gong_lev_1 = target.GetSkillLevel(14064)
		if gong_lev_1 ~= 0 then
			--替换宫吟唱
			RemoteCallToClient(target.dwID, "OnActionBarSkillReplace", 14064, 14449, gong_lev_1)
		end
	end
...
function UnApply(dwCharacterID)
	...
	if shang_lev ~= 0 then
		--替换商
		RemoteCallToClient(target.dwID, "OnActionBarSkillReplace", 14450, 14065, shang_lev)
	end

	local jue_lev = target.GetSkillLevel(14066)
	if jue_lev ~= 0 then
		--替换角
		RemoteCallToClient(target.dwID, "OnActionBarSkillReplace", 14453, 14066, jue_lev)
	end

	local zhi_lev = target.GetSkillLevel(14067)
	if zhi_lev ~= 0 then
		--替换徵
		RemoteCallToClient(target.dwID, "OnActionBarSkillReplace", 14451, 14067, zhi_lev)
	end

	local yu_lev = target.GetSkillLevel(14068)
	if yu_lev ~= 0 then
		--替换羽
		RemoteCallToClient(target.dwID, "OnActionBarSkillReplace", 14452, 14068, yu_lev)
	end
	...
	if target.GetSkillLevel(14308) == 1 then
		...
		if gong_lev ~= 0 then
			--替换宫dot
			RemoteCallToClient(target.dwID, "OnActionBarSkillReplace", 14449, 14302, gong_lev)
		end
	else
		local gong_lev_1 = target.GetSkillLevel(14064)
		if gong_lev_1 ~= 0 then
			--替换宫吟唱
			RemoteCallToClient(target.dwID, "OnActionBarSkillReplace", 14449, 14064, gong_lev_1)
		end
	end
end
