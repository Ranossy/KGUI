---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_影_清绝影歌回收状态.lua
-- 更新时间:	2015/9/11 11:16:37
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/长歌/套路及子技能/长歌_影_清绝影歌回收状态.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if nbuff_mark then
		local yingge_npc_ID = nbuff_mark.nCustomValue
		local npc_yingge = GetNpc(yingge_npc_ID)
		if npc_yingge  and (player.GetScene().dwMapID == npc_yingge.GetScene().dwMapID) and (player.GetScene().dwID == npc_yingge.GetScene().dwID) then
			player.SetPosition(npc_yingge.nX, npc_yingge.nY, npc_yingge.nZ)
			player.nCurrentLife = npc_yingge.nCurrentLife
			npc_yingge.SetDisappearFrames(1, 0)
		end
	end
	...
	--删除影子
	--player.BreakOTSkill()
	print(player.CastSkill(15069, 1))
end
