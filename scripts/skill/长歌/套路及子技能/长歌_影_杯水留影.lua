---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_影_杯水留影.lua
-- 更新时间:	2017/1/16 14:46:50
-- 更新用户:	chenchen6-pc
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_影_杯水留影.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(9506, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(10050, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	skill.SetNormalCoolDown(1, 909);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--添加一个死亡状态,B_9298被删除时把记录坐标的几个buff删除下
	target.AddBuff(player.dwID, player.nLevel, 9298, 1, 36000)

	--记录玩家位置（X、Y、Z、Dir）
	for i = 1, 4 do
		target.AddBuff(player.dwID, player.nLevel, 9299 + i, 1)
	end
	...
	local nTable = {
		...
		target.nY,
		target.nZ,
		target.GetScene().dwID,
	}

	for i = 1, 4 do
		local buff = target.GetBuff(9299 + i, 1)
		if buff then
	...
	if player.GetSkillLevel(14167) == 1 then
		target.AddBuff(player.dwID, player.nLevel, 10439, 1)
	else
		target.DelBuff(10439, 1)
	end

	if player.IsSkillRecipeActive(2112, 1) then
		target.AddBuff(player.dwID, player.nLevel, 10440, 1)
	else
		target.DelBuff(10440, 1)
	end

	if player.GetSkillLevel(14168) == 1 then
		target.AddBuff(player.dwID, player.nLevel, 10441, 1)
	else
		target.DelBuff(10441, 1)
	end

	if player.GetSkillLevel(15172) == 1 then
		target.AddBuff(player.dwID, player.nLevel, 10442, 1)
	else
