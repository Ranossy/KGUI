---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/平沙控制目标持续添加仇恨.lua
-- 更新时间:	2016/5/19 15:43:10
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local npcY = scene.GetNpcByNickName("aaa")
	if not npcY then
		npcY = scene.CreateNpc(24535, player.nX, player.nY, player.nZ, 0, - 1, "aaa")
	end

	if npcY then
		npcY.ModifyThreat(target.dwID, 0)
		npcY.SetPosition(target.nX,target.nY,target.nZ)
	end
	
	--target.CustomDamage(player.dwID, 13840, 1, 1, 0)
	--print(player.CastSkill(14287, 1, TARGET.PLAYER, dwSkillSrcID))
	--]]
end
