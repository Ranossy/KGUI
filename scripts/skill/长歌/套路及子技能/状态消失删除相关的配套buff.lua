---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/状态消失删除相关的配套buff.lua
-- 更新时间:	2015/8/15 17:20:52
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function UnApply(dwCharacterID)
	...
	--删除减伤
	if player.GetBuff(9510, 1) then
		player.DelBuff(9510, 1)
	end

	--删除免控
	if player.GetBuff(9511, 1) then
		player.DelBuff(9511, 1)
	end

	--删除禁止释放宫徵的buff
	if player.GetBuff(9552, 1) then
