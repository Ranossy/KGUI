---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/长歌/套路及子技能/长歌_音_角.lua
-- 更新时间:	2015/8/5 14:54:35
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/长歌/套路及子技能/长歌_音_角.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	if dwSkillLevel <= 6 then
		skill.nCostManaBasePercent = 190 * 0.2;	-- 技能消耗的内力
	elseif dwSkillLevel <= 12 then
		skill.nCostManaBasePercent = 190 * 0.4;	-- 技能消耗的内力
	elseif dwSkillLevel <= 18 then
		skill.nCostManaBasePercent = 190 * 0.6;	-- 技能消耗的内力
	elseif dwSkillLevel <= 24 then
		skill.nCostManaBasePercent = 190 * 0.8;	-- 技能消耗的内力
	else
		skill.nCostManaBasePercent = 190;	-- 技能消耗的内力
	end
...
function OnSkillLevelUp(skill, player)
	...
	player.LearnSkillLevel(14453, lv, player.dwID)	--学习剑·角
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local jue_lev = player.GetSkillLevel(14066)		--取技能角的等级

	if jue_lev ~= 0 then
		--普通dot
		if buff_jiange ~= 1 and buff_xiaoguo ~= 1 then
			player.CastSkill(14291, jue_lev, player.GetSkillTarget())
		end
		--间隔缩短的dot
		if buff_jiange == 1 and buff_xiaoguo ~= 1 then
			player.CastSkill(14292, jue_lev, player.GetSkillTarget())
		end
		--递增的dot
		if buff_jiange ~= 1 and buff_xiaoguo == 1 then
			player.CastSkill(14293, jue_lev, player.GetSkillTarget())
		end
		--递增且间隔缩短的dot
		if buff_jiange == 1 and buff_xiaoguo == 1 then
			player.CastSkill(14294, jue_lev, player.GetSkillTarget())
		end
	end

	--CW效果吞噬角
	player.CastSkill(15185, 1, player.GetSkillTarget())
	...
	--伏疏角dot降低目标内力并回复自身内力
	if player.GetSkillLevel(14465) == 1 then
		--if IsPlayer(target.dwID) then
		--标记buff，判定在dot上
		target.AddBuff(player.dwID, player.nLevel, 9513, 1)
		--end
	end
end
