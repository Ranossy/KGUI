...
function GetSkillLevelData(skill)
    ...
    -- 技能CD, CoolDownIndex为CD位(共3个), nCoolDownID为CoolDownList.tab内的CDID
      skill.SetNormalCoolDown(1, 488);	--技能普通CD
...
function Apply(dwCharacterID)
	...
	if nResaultA == 1 and nRX == 0 and nRY ==0 and nRZ ==0 then		--若3D障碍可达，则传至炮台后面
		cPlayer.Stop()
		...
		--cPlayer.TurnTo(cNpc.nFaceDirection)
		cPlayer.TurnToCharacter(cNpc.dwID)
		cPlayer.SetTimer(16, "scripts/skill/唐门/鬼斧神工_子技能.lua", cNpc.dwID, 0)
		--cPlayer.CastSkill(3398,1)
		cPlayer.AddBuff(0,99,3425,1,2)
	else						--若3D障碍不可达，则传至炮台位置
		cPlayer.Stop()
		--cPlayer.TurnTo(cNpc.nFaceDirection)
		cPlayer.TurnToCharacter(cNpc.dwID)
