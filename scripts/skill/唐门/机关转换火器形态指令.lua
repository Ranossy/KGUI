...
tSkillData =
{
	{nDamage = 0, nDamageRand = 0, nCostMana = 0},		--level 1
...
function GetSkillLevelData(skill)
    ...
    --skill.SetNormalCoolDown(1, 492);	--技能普通CD
	 --skill.SetNormalCoolDown(2, 483);	--技能普通CD
	   skill.SetNormalCoolDown(3, 1016);
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	local battery = GetNpc(player.dwBatteryID)
	if not battery or battery.dwTemplateID ~= 16174 then
		return false
	end
    return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	--cNpc.SetDisappearFrames(176, 0) 
	local battery = scene.CreateBattery(16177, cNpc.nX, cNpc.nY, cNpc.nZ, cPlayer.nFaceDirection, cPlayer.dwID, false)
	battery.AddBuff(0, 99, 3486, 1)
	battery.AddBuff(0,99,6111,1)
	cPlayer.CastSkill(3305, 1, TARGET.NPC, battery.dwID)			--为炮台上镇派效果
	cPlayer.SetTimer(48, "scripts/skill/唐门/机关转换火器形态指令.lua", battery.dwID, 0)
	battery.SetModelID(26326)
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	...
	if npc then
		npc.SetModelID(26292)
		npc.AddBuff(0,99,3403,1)
	end
end
