---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/唐门/解除迷神钉.lua
-- 更新时间:	2015/10/2 23:13:15
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
	...
	--print(22222)
	local nbuff = target.GetBuff(3224, 1)
	if nbuff then
		local playerID = nbuff.dwSkillSrcID
		local player = GetPlayer(playerID)
		if player then
			if player.GetSkillLevel(14853) == 1 then
				if nbuff.nLeftActiveCount <= 3 then
					--print(111111)
					target.DelBuff(3224, 1)		
				end
			else
				target.DelBuff(3224, 1)
			end
