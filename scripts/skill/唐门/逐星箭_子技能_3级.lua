...
function GetSkillLevelData(skill)
	...
	skill.AddSlowCheckDestOwnBuff(2237, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL);	-- 需求目标属于自己的Buff   

	-----------------技能施放技能需求-------------------------------------------
	--skill.AddCheckSelfLearntSkill(dwSkillID, dwSkillLevel, LevelCompareFlag);     --检查比较Caster自己已学习的技能ID和等级
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 3200, 1);		-- 设置1号位Buff
	...
	if dwSkillLevel < 10 then
		skill.nChannelInterval = 28*1.7;
	elseif dwSkillLevel < 28 then
		skill.nChannelInterval = (28 + (dwSkillLevel - 9) * 31)*1.7
	else
		skill.nChannelInterval =60*1.2; 
	end
