...
function GetSkillLevelData(skill)
	...
	-----------------技能施放技能需求-------------------------------------------
     skill.AddCheckSelfLearntSkill(6874, 1, SKILL_COMPARE_FLAG.EQUAL);     --检查比较Caster自己已学习的技能ID和等级
	----------------- BUFF相关 -------------------------------------------------
    ...
    --skill.nPrepareFrames  	= 48;				-- 吟唱帧数
      skill.nChannelInterval	= 272*0.1 * dwSkillLevel; 				-- 通道技间隔时间 
	...
	if dwSkillLevel == 1 then
    	skill.nTargetLifePercentMin	= 90;			-- 血量最小值>=
    	skill.nTargetLifePercentMax	= 100;			-- 血量最大值<=
	elseif dwSkillLevel == 2 then
		skill.nTargetLifePercentMin	= 80;			-- 血量最小值>=
		skill.nTargetLifePercentMax = 90;			-- 血量最大值<=
	elseif dwSkillLevel == 3 then
		skill.nTargetLifePercentMin	= 70;			-- 血量最小值>=
		skill.nTargetLifePercentMax = 80;			-- 血量最大值<=
	elseif dwSkillLevel == 4 then
		skill.nTargetLifePercentMin	= 60;			-- 血量最小值>=
		skill.nTargetLifePercentMax = 70;			-- 血量最大值<=
	elseif dwSkillLevel == 5 then
		skill.nTargetLifePercentMin	= 50;			-- 血量最小值>=
		skill.nTargetLifePercentMax = 60;			-- 血量最大值<=
	elseif dwSkillLevel == 6 then
		skill.nTargetLifePercentMin	= 40;			-- 血量最小值>=
		skill.nTargetLifePercentMax = 50;			-- 血量最大值<=
	elseif dwSkillLevel == 7 then
		skill.nTargetLifePercentMin	= 30;			-- 血量最小值>=
		skill.nTargetLifePercentMax = 40;			-- 血量最大值<=
	elseif dwSkillLevel == 8 then
		skill.nTargetLifePercentMin	= 20;			-- 血量最小值>=
		skill.nTargetLifePercentMax = 30;			-- 血量最大值<=
	elseif dwSkillLevel == 9 then
		skill.nTargetLifePercentMin	= 10;			-- 血量最小值>=
		skill.nTargetLifePercentMax = 20;			-- 血量最大值<=
	elseif dwSkillLevel == 10 then
		skill.nTargetLifePercentMin	= 0;			-- 血量最小值>=
		skill.nTargetLifePercentMax = 10;			-- 血量最大值<=
	end
	...
	----------------- 武器伤害相关 ---------------------------------------------
	skill.nWeaponDamagePercent		= 1536 * dwSkillLevel;			-- 武器伤害百分比,对外功伤害有用。填0表示此次外功攻击不计算武器伤害,1024为100%
