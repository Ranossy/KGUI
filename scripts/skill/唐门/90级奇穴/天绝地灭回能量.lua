---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/唐门/90级奇穴/天绝地灭回能量.lua
-- 更新时间:	2014/7/2 22:30:23
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function UnApply(dwCharacterID)
	...
	if player.GetSkillLevel(9350) == 1 then
		local buffcount
		local buff = player.GetBuff(7999, 1)
		if buff then
			buffcount = buff.nStackNum
			if buffcount == 1 then
				player.nCurrentEnergy = player.nCurrentEnergy + 20
			elseif buffcount > 1 then
				player.nCurrentEnergy = player.nCurrentEnergy + 40
			end
			player.DelGroupBuff(7999, 1)
		end
	end
