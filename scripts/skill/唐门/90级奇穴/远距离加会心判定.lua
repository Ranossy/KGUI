---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/唐门/90级奇穴/远距离加会心判定.lua
-- 更新时间:	2015/10/2 17:40:05
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if IsEnemy(dwCharacterID, targetID) and GetDistanceSq(player.nX, player.nY, player.nZ, target.nX, target.nY, target.nZ) > 15 * 64 * 15 * 64 and player.bFightState ==true then
		if not player.IsHaveBuff(9981, 1) and player.nMoveState ~= MOVE_STATE.ON_DEATH then
			player.AddBuff(player.dwID,player.nLevel,9981,1)
		end
	else
		if not IsPlayer(targetID) and target.dwTemplateID == 15959 then
			return
		end
		if player.IsHaveBuff(9981, 1) then
			player.DelBuff(9981, 1)
		end
	end
end
