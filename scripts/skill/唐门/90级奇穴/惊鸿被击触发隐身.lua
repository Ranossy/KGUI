---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/唐门/90级奇穴/惊鸿被击触发隐身.lua
-- 更新时间:	2017/4/9 17:29:39
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/唐门/90级奇穴/惊鸿被击触发隐身.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 3203, 1);		-- 设置1号位Buff
	skill.BindBuff(2, 3443, 1);			-- 设置2号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--添加隐身
	player.Stop()
	player.AddBuff(player.dwID, player.nLevel, 3203, 1, 5)
	
	--处理方士状态透明度显示
	if player.GetBuff(10827, 1) then
		if player.GetBuff(11494, 1) then
			player.DelBuff(11494, 1)
		end
	end

	--允许触发穷尽九泉
	if player.GetSkillLevel(6874) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 7158, 1, 1)
	end
end
