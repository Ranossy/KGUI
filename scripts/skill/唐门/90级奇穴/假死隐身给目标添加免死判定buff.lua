---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/唐门/90级奇穴/假死隐身给目标添加免死判定buff.lua
-- 更新时间:	2015/10/3 0:47:02
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.GetBuff(9990, 1) then
		if not player.GetBuff(9989, 1) and player.nMoveState ~= MOVE_STATE.ON_DEATH then
			player.AddBuff(player.dwID,player.nLevel,9989,1)
		end
	else
		if player.GetBuff(9989, 1) then
