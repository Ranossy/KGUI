---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/唐门/90级奇穴/判定血量触发实际假死隐身.lua
-- 更新时间:	2017/3/16 16:44:47
-- 更新用户:	chenchen6-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/唐门/90级奇穴/判定血量触发实际假死隐身.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(14858) == 1 and player.nCurrentLife <= player.nMaxLife * 0.1 and not player.GetBuff(9990, 1) and player.nMoveState ~= MOVE_STATE.ON_DEATH and player.bFightState == true then
		player.CastSkill(17513, 1)
		--player.DelBuff(9989, 1)
		--player.AddBuff(player.dwID, player.nLevel, 9990, 1)
		--player.AddBuff(player.dwID, player.nLevel, 9991, 1)
	end
end
