---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/唐门/90级奇穴/追命箭AOE.lua
-- 更新时间:	2013/4/30 11:13:50
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData = 
{
	{nDamageBase = 96*0.95, 	nDamageRand = 96*0.1,  nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckDestOwnBuff(5782, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);
	...
	elseif dwSkillLevel < 28 then
		...
		skill.nChannelInterval =90;     -- 通道技间隔时间   
	end
	--]]
	if dwSkillLevel < 10 then
		skill.nChannelInterval = 100*1.1*1.1*0.5;
	elseif dwSkillLevel < 28 then
		skill.nChannelInterval = (100 + (dwSkillLevel - 9) * 6)*1.1*1.1*0.5
	else
		skill.nChannelInterval =272*1.1*1.1*0.5;     -- 通道技间隔时间   
	end
