---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/唐门/套路及子技能/飞星遁影判定障碍.lua
-- 更新时间:	2017/4/13 12:21:47
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	local npc_feixing = GetNpc(dwCharacterID)
	if not npc_feixing then
		return
	end


	local player = GetPlayer(npc_feixing.GetCustomUnsigned4(0))
	if not player then
		return
	end
	...
	--buff映射关系
	local nMapping = {
		[11888] = {B_ID = 11898, S_ID = 17587},
		[11889] = {B_ID = 11899, S_ID = 17588},
	}
	...
	--print(1)
	if Is_pre and S_ID == 3691 or S_ID == 3109 or S_ID == 6191 or S_ID == 3095 or S_ID == 3096 or player.GetOTActionState() == 4 or player.GetOTActionState() == 5 or player.GetOTActionState() == 2 then
		--print(2)
		for i = 1, 2 do
			if npc_feixing.GetBuff(11887 + i, 1) then
				--print(3)
				player.AddBuff(player.dwID, player.nLevel, nMapping[11887 + i].B_ID, 1)
				RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "FireUIEvent", "UPDATE_HIDDEN_STATE ", nMapping[11887 + i].S_ID, npc_feixing.dwID, true)
				--print(111111)
				break
			end
		end
		...
		--print(22222)
		local nResult = player.CastSkill(17595, 1, TARGET.NPC, dwCharacterID)
		--print(nResult)
		if nResult == 1 then
			--print(22222)
			for i = 1, 2 do
				if npc_feixing.GetBuff(11887 + i, 1) then
					if player.GetBuff(nMapping[11887 + i].B_ID, 1) then
						--print(99997)
						player.DelBuff(nMapping[11887 + i].B_ID, 1)
					end
						--print(99998)
					RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "FireUIEvent", "UPDATE_HIDDEN_STATE ", nMapping[11887 + i].S_ID, npc_feixing.dwID, false)
					--print(99999)
					break
				end
			end
