---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/唐门/唐门_弓弩_裂石矢.lua
-- 更新时间:	2013/5/12 10:29:01
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData = 
{
	...
	{nDamageBase = 56 * 0.95, nDamageRand = 56 * 0.1, nCostMana = 0}, --level 2
	{nDamageBase = 94 * 0.95, nDamageRand = 94 * 0.1, nCostMana = 0}, --level 3
	{nDamageBase = 135 * 0.95, nDamageRand = 135 * 0.1, nCostMana = 0}, --level 4
...
function OnSkillLevelUp(skill, player)
	if skill.dwLevel == skill.dwMaxLevel then
		player.AcquireAchievement(2712)
	end
...
function Apply(dwCharacterID,dwSrcID)
	...
	if player.GetSkillLevel(5713) == 1 then
		player.AddBuff(dwSrcID, player.nLevel, 5888, 1, 1)
		if not player.GetBuff(5887, 1) then
			player.AddBuff(dwSrcID, player.nLevel, 5887, 1, 1)
		end
	end
end
