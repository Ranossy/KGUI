...
function GetSkillLevelData(skill)
    ...
    -- 技能CD, CoolDownIndex为CD位(共3个), nCoolDownID为CoolDownList.tab内的CDID
      skill.SetNormalCoolDown(1, 491);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	local target
		if not targetID or not targetType then
			return SKILL_RESULT_CODE.NEED_TARGET
		end
		...
		if not target then
			return SKILL_RESULT_CODE.NO_TARGET
		end	
	local nDistance = GetCharacterDistance(battery.dwID, target.dwID)
	if battery.dwTemplateID == 16175 and nDistance > 25 * LENGTH_BASE then
		return SKILL_RESULT_CODE.TOO_FAR_TARGET
	end
	if battery.dwTemplateID == 16176 and nDistance > 25 * LENGTH_BASE then
		return SKILL_RESULT_CODE.TOO_FAR_TARGET
	end
    return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	local battery = GetNpc(cPlayer.dwBatteryID)
	if not battery then
		return
	end
	battery.FireAIEvent(2001, 0, 0) --攻击
	local pType, pID = cPlayer.GetTarget()
	local bType, bID = battery.GetTarget()
	if pID ~= bID  then
		battery.SetTarget(TARGET.NPC, battery.dwID)
		cPlayer.SetTimer(24, "scripts/skill/唐门/机关攻击指令.lua", battery.dwID, 0)		
	end
end
