---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/唐门/唐门_弓弩_转落七星.lua
-- 更新时间:	2015/10/19 21:29:06
-- 更新用户:	mengxiangfei11
-- 脚本说明:
...
tSkillData =
{
	{nDamageBase = 200, nDamageRand = 15, nCostMana = 0}, --level 1
};
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 10608, 1);		-- 设置1号位Buff
	...
	----------------- 设置Cool down --------------------------------------------
	-- 公共CD
	--skill.SetPublicCoolDown(974);							-- 公共CD 1.5秒
	-- 技能CD, CoolDownIndex为CD位(共3个), nCoolDownID为CoolDownList.tab内的CDID
	skill.SetNormalCoolDown(1, 971);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(10133, 1) then
		player.DelBuff(10133, 1)
	elseif player.GetBuff(10132, 1) then
		player.DelBuff(10132, 1)
		player.AddBuff(player.dwID, player.nLevel, 10133, 1)
	else
		player.AddBuff(player.dwID, player.nLevel, 10132, 1)
	end
end
