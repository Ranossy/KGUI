...
function GetSkillLevelData(skill)
   ...
   --skill.SetPublicCoolDown(16);							-- 公共CD 1.5秒
    -- 技能CD, CoolDownIndex为CD位(共3个), nCoolDownID为CoolDownList.tab内的CDID
      skill.SetNormalCoolDown(1, 487);	--技能普通CD
...
function OnSkillLevelUp(skill, player)
	if player.GetSkillLevel(3357) < 1 then
		player.LearnSkill(3357)
	end
end
...
function Apply(dwCharacterID)
	...
	for i = 1, maxcount do
		local npc = scene.CreateBomb(16000, nX+5*(i-1), nY+5*(i-1), nZ, 0, 960,player.dwID)
		if npc  then		
			npc.SetModelID(26123)
			npc.AddBuff(0,99,3361,1)
			npc.AddBuff(player.dwID, player.nLevel, 3342, 1)	
			player.SetTimer(32, "scripts/skill/唐门/唐门_机关_暗藏杀机.lua", npc.dwID, 0)
			--npc.AddBuff(0,99,3361,1)
		end
	end
end
