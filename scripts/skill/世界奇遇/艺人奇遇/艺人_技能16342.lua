---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/世界奇遇/艺人奇遇/艺人_技能16342.lua
-- 更新时间:	2016/8/5 18:21:40
-- 更新用户:	YANGYUBO
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 2500000, nDamageRand = 200000, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	skill.AddAttribute(
		...
		"skill/世界奇遇/艺人奇遇/艺人_技能16342.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 魔法属性 -------------------------------------------------
	--[[
	...
	--skill.AddSlowCheckDestBuff(5468, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	skill.nAreaRadius = 5000 * LENGTH_BASE;		-- 技能作用半径
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	--要在技能编辑器中勾选调用脚本
	local mubiao = player.GetSelectCharacter()
	if mubiao and mubiao.dwTemplateID == 52099 then
		return nPreResult;
	else
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		local nQuestID = 15656
		local nQuestPhase = player.GetQuestPhase(nQuestID)
		local nQuestIndex = player.GetQuestIndex(nQuestID)
		if npc.dwTemplateID == 52099 then
			player.SetTimer(1 * GLOBAL.GAME_FPS, "scripts/skill/世界奇遇/艺人奇遇/艺人_技能16342.lua", npc.dwID, 0)
			player.PlaySfx(1358, npc.nX, npc.nY, npc.nZ)
		end
	end
end
