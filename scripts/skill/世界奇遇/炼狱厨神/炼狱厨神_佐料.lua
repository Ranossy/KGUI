---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/世界奇遇/炼狱厨神/炼狱厨神_佐料.lua
-- 更新时间:	2015/10/13 0:45:03
-- 更新用户:	kelin-PC
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(10045, 1) then
		RemoteCallToClient(player.dwID, "OnDynamicSkillHighlightChanged", 2, false)
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "QTEPanel_HitNotify", 15086)

		player.DelBuff(10045, 1)
		player.SendSystemMessage(GetEditorString(8, 6398))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(8, 6398), 4)
	end
end
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	if nLeftFrame == 0 then
		...
		g_QYSpicyFish.QTEFailed(player)
	end
end
