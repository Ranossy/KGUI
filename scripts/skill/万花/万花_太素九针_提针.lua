---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/万花/万花_太素九针_提针.lua
-- 更新时间:	2017/3/16 10:37:10
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	elseif dwSkillLevel < 28 then
		...
		skill.nChannelInterval = 143 * 1.1;     -- 通道技间隔时间
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(5788) == 1 then
		--for i = 1, sum do
	...
	--提针对自身的额外效果
	local nlev = player.GetSkillLevel(138)
	if player.GetSkillLevel(9325) == 1 then
		player.CastSkill(9327, nlev)
