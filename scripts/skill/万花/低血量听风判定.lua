---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/万花/低血量听风判定.lua
-- 更新时间:	2016/9/21 11:46:40
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.GetBuff(11268, 1) and player.nCurrentLife <= player.nMaxLife * 0.4 and player.nMoveState ~= MOVE_STATE.ON_DEATH and player.bFightState == true then
		local nLeft_frame = player.GetCDLeft(446)
		if nLeft_frame ~= 0 then
			--player.ResetCD(446)
			player.ClearCDTime(446)
			player.AddBuff(player.dwID, player.nLevel, 11268, 1)
		end
	end
end
