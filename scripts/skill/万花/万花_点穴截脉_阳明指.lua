---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/万花/万花_点穴截脉_阳明指.lua
-- 更新时间:	2015/9/27 20:55:22
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nAddRage = 0, nCostRage = 10, nDamageBase = 20, nDamageRand = 5, }, -- Level 1
	{nAddRage = 0, nCostRage = 10, nDamageBase = 25, nDamageRand = 5, }, -- Level 2
	{nAddRage = 0, nCostRage = 10, nDamageBase = 30, nDamageRand = 5, }, -- Level 3
	{nAddRage = 0, nCostRage = 10, nDamageBase = 35, nDamageRand = 5, }, -- Level 4
	{nAddRage = 0, nCostRage = 10, nDamageBase = 40, nDamageRand = 5, }, -- Level 5
	{nAddRage = 0, nCostRage = 10, nDamageBase = 45, nDamageRand = 5, }, -- Level 6
	{nAddRage = 0, nCostRage = 10, nDamageBase = 50, nDamageRand = 5, }, -- Level 7
	{nAddRage = 0, nCostRage = 10, nDamageBase = 54, nDamageRand = 5, }, -- Level 8
	{nAddRage = 0, nCostRage = 10, nDamageBase = 58, nDamageRand = 5, }, -- Level 9
	{nAddRage = 0, nCostRage = 10, nDamageBase = 62, nDamageRand = 10, }, -- Level 10
	{nAddRage = 0, nCostRage = 10, nDamageBase = 66, nDamageRand = 11, }, -- Level 11
	{nAddRage = 0, nCostRage = 10, nDamageBase = 70, nDamageRand = 12, }, -- Level 12
	{nAddRage = 0, nCostRage = 10, nDamageBase = 74, nDamageRand = 13, }, -- Level 13
	{nAddRage = 0, nCostRage = 10, nDamageBase = 78, nDamageRand = 14, }, -- Level 14
	{nAddRage = 0, nCostRage = 10, nDamageBase = 82, nDamageRand = 15, }, -- Level 15
...
function GetSkillLevelData(skill)
	...
	if dwSkillLevel < 10 then
		skill.nChannelInterval = 48 * 1.3;
	elseif dwSkillLevel < 28 then
		skill.nChannelInterval = (48 + (dwSkillLevel - 9) * 4) * 1.3
	else
		skill.nChannelInterval = 130 * 1.3;     -- 通道技间隔时间
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--优先删除保护buff（保护buff只有在加放歌的时候发现玩家处于吟唱状态才有的）
	if player.GetBuff(8597, 1) then
		player.DelBuff(8597, 1)
		--通用删除瞬发buff
	elseif player.GetBuff(412, 1) then
		--删水月
		player.DelBuff(412, 1)
	elseif player.GetBuff(932, 1) then
		--删阵法迅涌
		player.DelBuff(932, 1)
	elseif player.GetBuff(8491, 1) then
		player.DelBuff(8491, 1)
	end
	...
	local lv_yangming = player.GetSkillLevel(179)
	local nTime = 0
	
	if lv_shangyang ~= 0 and target.GetBuffByOwner(666, lv_shangyang, player.dwID)	then
		nTime = nTime + 1
	end

	if lv_lancui ~= 0 and target.GetBuffByOwner(711, lv_lancui, player.dwID) then
		nTime = nTime + 1
	end

	if lv_zhonglin ~= 0 and target.GetBuffByOwner(714, lv_zhonglin, player.dwID) then
		nTime = nTime + 1	
	end
	
	if player.GetSkillLevel(5767) == 1 then
		if	nTime == 0 then
			player.CastSkill(14941, lv_yangming, player.GetSkillTarget())
		elseif nTime == 1 then
			player.CastSkill(14942, lv_yangming, player.GetSkillTarget())
		elseif nTime == 2 then
			player.CastSkill(14943, lv_yangming, player.GetSkillTarget())
		elseif nTime == 3 then
			player.CastSkill(14944, lv_yangming, player.GetSkillTarget())
		end
	else
		player.CastSkill(14941, lv_yangming, player.GetSkillTarget())
	end
	...
	--附魔效果，阳明指叠快雪伤害标记
	if player.GetSkillLevel(9464) == 1 and player.GetKungfuMount() ~= nil and player.GetKungfuMount().dwSkillID == 10021 then
	...
	if player.GetSkillLevel(5774) == 1 then
		...
		if target.GetBuffByOwner(666, lv_shangyang, player.dwID) then
			...
			player.CastSkillXYZ(14647, lv_shangyang, target.nX, target.nY, target.nZ)
		end
		...
		if target.GetBuffByOwner(711, lv_lancui, player.dwID) then
			...
			player.CastSkillXYZ(14648, lv_lancui, target.nX, target.nY, target.nZ)
		end
		...
		if target.GetBuffByOwner(714, lv1, player.dwID) then
			...
			player.CastSkillXYZ(14649, lv1, target.nX, target.nY, target.nZ)
		end
	end
