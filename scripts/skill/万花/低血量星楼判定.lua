---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/万花/低血量星楼判定.lua
-- 更新时间:	2016/9/21 12:33:38
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.nCurrentLife <= player.nMaxLife * 0.4 and not player.GetBuff(11271, 1) and player.nMoveState ~= MOVE_STATE.ON_DEATH and player.bFightState == true then
		local nLeft_frame = player.GetCDLeft(51)
		if nLeft_frame ~= 0 then
			player.ClearCDTime(51)
			player.AddBuff(player.dwID, player.nLevel, 11271, 1)
		end
	end
end
