---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/万花/离经_长针溅射握针效果.lua
-- 更新时间:	2017/3/16 10:59:14
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	--skill.AddSlowCheckDestOwnBuff(631, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);	-- 需求目标属于自己的Buff
	...
	--skill.BindBuff(1, 631, dwSkillLevel);
	----------------- 设置Cool down --------------------------------------------
	-- 公共CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if nlev ~= 0 and not target.GetBuffByOwner(631, nlev, player.dwID) then
		if IsPlayer(dwCharacterID) then
			player.CastSkill(14665, nlev, TARGET.PLAYER, target.dwID)
		else
			player.CastSkill(14665, nlev, TARGET.NPC, target.dwID)
		end
	end
end
