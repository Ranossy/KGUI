---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/万花/锋针不死清除.lua
-- 更新时间:	2013/5/20 16:12:17
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/万花/锋针不死清除.lua", -- 属性值1
		0														-- 属性值2
	);
    
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(5970, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
...
function Apply(dwCharacterID)
	...
	local dwSkillSrcID = player.GetBuff(5970, 1).dwSkillSrcID
	...
	if player.GetScene().dwID == target.GetScene().dwID and player.GetScene().dwMapID == target.GetScene().dwMapID then
		if dwSkillSrcID == dwCharacterID or IsParty(dwSkillSrcID, dwCharacterID) then
			...
			target.PlaySfx(505, target.nX, target.nY, target.nZ)
			--[[
			--附魔离经心法下缝针额外效果
