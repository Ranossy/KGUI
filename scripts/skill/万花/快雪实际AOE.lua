---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/万花/快雪实际AOE.lua
-- 更新时间:	2015/9/13 15:48:47
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	if dwSkillLevel < 10 then
		skill.nChannelInterval = 44 * 0.9;
	elseif dwSkillLevel < 29 then
		skill.nChannelInterval = (44 + (dwSkillLevel - 9) * 2) * 0.9
	else
		skill.nChannelInterval = 84 * 0.9;     -- 通道技间隔时间
	end
		...
		if nRnd <= 4000 then
			...
			if buff_shangyang then
				--if buff_shangyang.nCustomValue < 1 then
					...
					target.SetBuffNextActiveFrame(buff_shangyang.nIndex, 1 + GetLogicFrameCount())
			...
			if buff_lanchui then
				--if buff_lanchui.nCustomValue < 1 then
					...
					target.SetBuffNextActiveFrame(buff_lanchui.nIndex, 1 + GetLogicFrameCount())
			...
			if buff_zhonglin then
				--if buff_zhonglin.nCustomValue < 1 then
					...
					target.SetBuffNextActiveFrame(buff_zhonglin.nIndex,1 + GetLogicFrameCount())
