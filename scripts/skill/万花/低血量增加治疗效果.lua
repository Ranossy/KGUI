---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/万花/低血量增加治疗效果.lua
-- 更新时间:	2016/9/26 4:40:14
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nCount = 0
	if player.nMoveState ~= MOVE_STATE.ON_DEATH and player.bFightState == true then
		if player.nCurrentLife <= player.nMaxLife * 0.2 then
			nCount = 4
		elseif player.nCurrentLife <= player.nMaxLife * 0.4 then
			nCount = 3
		elseif player.nCurrentLife <= player.nMaxLife * 0.6 then
			nCount = 2
		elseif player.nCurrentLife <= player.nMaxLife * 0.8 then
			nCount = 1
		end
	end

	if nCount == 0 then
		player.DelGroupBuff(11321, 1)
	else
		local nbuffmark = player.GetBuff(11321, 1)
		local nbuffcount = 0
		if nbuffmark then
			nbuffcount = nbuffmark.nStackNum
		end
		if nCount > nbuffcount then
			for i = 1, nCount - nbuffcount do
				player.AddBuff(player.dwID, player.nLevel, 11321, 1)
			end
		elseif nCount < nbuffcount then
			for i = 1, nbuffcount - nCount do
				player.DelBuff(11321,1)
			end
		end
	end
end
