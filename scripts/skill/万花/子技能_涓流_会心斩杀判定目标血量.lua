---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/万花/子技能_涓流_会心斩杀判定目标血量.lua
-- 更新时间:	2015/9/13 12:47:47
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/万花/子技能_涓流_会心斩杀判定目标血量.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.GetBuff(9722, 1) and target.nCurrentLife <= target.nMaxLife * 0.35 and not player.GetBuff(9723, 1) then
		for i = 1, 10 do
			player.AddBuff(player.dwID, player.nLevel, 9722, 1)
			player.AddBuff(player.dwID, player.nLevel, 9723, 1)
		end
	else
		--if not player.GetBuff(9723) then
			player.DelBuff(9722, 1)
			player.DelBuff(9723, 1)
		--end
	end
end
