---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/万花/90级奇穴/治疗计数.lua
-- 更新时间:	2013/4/23 17:27:25
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
	...
	player.AddBuff(dwCharacterID, player.nLevel, 5678, 1)
	local buff = player.GetBuff(5678, 1)
	if not buff then
		return
	end
	if buff.nStackNum == 5 then
		player.AddBuff(dwCharacterID, player.nLevel, 5679, 1)
		for i = 1, 5 do
			player.DelBuff(5678, 1)
		end
	end
end
