---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/万花/万花_太素九针_折叶笼花.lua
-- 更新时间:	2015/9/28 21:28:12
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/万花/万花_太素九针_折叶笼花.lua",											-- 属性值1
   		0											-- 属性值2
   		);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(203, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	skill.AddSlowCheckDestBuff(7525, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	skill.AddSlowCheckDestBuff(7561, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	skill.SetNormalCoolDown(1, 967);	--技能普通CD
	...
	skill.nCostManaBasePercent = 308;	-- 技能消耗的内力
	----------------- 聚气相关 -------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if target.GetSkillLevel(14607) == 1 and target.GetBuff(9703, 1) then
		target.DelBuff(9703,1)
		target.AddBuff(player.dwID, player.nLevel, 9933, 1, 24)
	else
		target.AddBuff(player.dwID, player.nLevel, 9933, 1, 24)
	end

	ModityCDToUI(player, 14963, 0, 0)
end
