...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nlev = player.GetSkillLevel(190)
	if nlev == 0 then
		return
	end
	
	local buff = target.GetBuffByOwner(711, nlev, player.dwID)


	if buff and player.GetSkillLevel(6690) == 1 then
		local nNum = Random(1, 10000)
		if nNum <= 2500 then
			local Is_pre, S_ID, S_lev, L_time = player.GetSkillPrepareState()
			if Is_pre and S_ID == 179 then
				player.AddBuff(player.dwID,player.nLevel,8597,1)
				player.AddBuff(player.dwID,player.nLevel,8597,1)
			end
			player.AddBuff(player.dwID, player.nLevel, 8491, 1)
			--player.AddBuff(player.dwID, player.nLevel, 9060, 1)
		end
	end
end
