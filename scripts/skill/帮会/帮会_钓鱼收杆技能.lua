---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/帮会/帮会_钓鱼收杆技能.lua
-- 更新时间:	2017/5/16 17:57:32
-- 更新用户:	zhengfeng5
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
	...
	local buff_ShangGou = player.GetBuff(SHANGGOUBUFF, 1)
	if buff_ShangGou then
		--print("上钩buff剩余时间：" .. buff_ShangGou.nLeftActiveCount)
		player.DoAction(dwCharacterID, SHOUGAN_ACT)
		--if player.GetBuff(YUERBUFF_BHYE, 1) then
			player.SetTimer(16 * 2, "scripts/skill/帮会/帮会_钓鱼收杆技能.lua", YUERBUFF_BHYE, 1)--有收获
		--end		
		return
	end
