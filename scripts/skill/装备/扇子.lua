---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/扇子.lua
-- 更新时间:	2017/3/15 11:56:17
-- 更新用户:	LINGJIAJIA-PC
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	if tScirptPath[dwSkillLevel] then
		skill.AddAttribute(
			ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK,
			ATTRIBUTE_TYPE.EXECUTE_SCRIPT,
			tScirptPath[dwSkillLevel],
			0
			);
	end
	----------------- 魔法属性 -------------------------------------------------
...
-- 注意，最终以脚本返回的结果为准
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能19371
	if player.dwWaistItemIndex < 19368 or  player.dwWaistItemIndex > 19371  then 
		return false
	end
	return nPreResult;
end
