---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/月饼使用.lua
-- 更新时间:	2014/8/4 15:19:54
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	--print(1)
	--if player.dwWaistItemIndex ~= 11801 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND
if player.dwBackItemIndex ~= 11801 then
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	player.SetTimer(48,"scripts/skill/装备/月饼使用.lua",player.nX,player.nY)
	--local nX1,nY1 = GetPosition(player.nX, player.nY, player.nFaceDirection, 50, math.pi/2)
	--scene.CreateNpc(36984, nX1, nY1, player.nZ,player.nFaceDirection,13*GAME_FPS)
	--local nFace = player.nFaceDirection + 128
	--if nFace > 255 then
		--nFace = 255 - 128
	--end
	--local nX2,nY2 = GetPosition(player.nX, player.nY, player.nFaceDirection, 50, math.pi*3/2)
	--scene.CreateNpc(36984,nX2, nY2, player.nZ,nFace,13*GAME_FPS)
end
function OnTimer(player, nParam1, nParam2)
	local scene = player.GetScene()
	scene.CreateNpc(36984, nParam1, nParam2, player.nZ,player.nFaceDirection,10*GAME_FPS)
end
