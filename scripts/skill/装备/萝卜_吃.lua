---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/萝卜_吃.lua
-- 更新时间:	2014/5/27 16:02:28
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if player.dwWaistItemIndex ~= 11654 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	player_Target = GetPlayer(dwTargetID)
	if not player_Target then
		return
	end
	player.RemovePendent(11654)
	player_Target.DoAction(0, 13217)
	if not player.IsPendentExist(11655) then
		player.AddPendent(11655)
	end

	player.SetRepresentID(EQUIPMENT_REPRESENT.WAIST_EXTEND, 181)
	RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_WARNING_YELLOW", GetEditorString(6, 5514), 5)
end
