---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/背部挂件_九霄环佩.lua
-- 更新时间:	2014/8/8 16:20:31
-- 更新用户:	zhangyan3
-- 脚本说明:
----------------------------------------------------------------------<
-- 创建人   :  CBG
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if ArtistActionCheck(player, 11802) then
		return nPreResult
	end
	-- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND
	if player.dwBackItemIndex ~= 11802 and player.dwBackItemIndex ~= 19257 then --加入限时版判断 
