---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/帐篷挂件.lua
-- 更新时间:	2014/11/20 22:43:39
-- 更新用户:	zhangyan3
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if player.dwBackItemIndex ~= 13126 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND
		return false
	end
...
function Apply(dwCharacterID)
	...
	--if not player.GetBuff(7194, 1) then
		...
		--	player.AddBuff(0, 99, 7194, 1) --CD
			...
			--监控BUFF
			--换动作BUFF
			player.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/装备/帐篷挂件.lua", 0, 0)
	...
	--else
	--	RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", "还请侠士稍等片刻", 4)
	--	return
	--end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	if nParam1 == 0 then
		...
		if npc1 and npc2 and npc3 and npc4 then
			...
			if nNum >= 150 then
				...
				player.Stop()
			--	player.AddBuff(0, 99, 8659, 1, 900)--东道主
				player.AddBuff(0, 99, 8640, 1, 900) --监控
			--	player.DoAction(0, 10107)
			--	player.PlaySfx(1087, 0, 0, 0)
				player.SetDynamicSkillGroup(184)
				--player.PlaySfx(1087,0,0,0)
				player.SetTimer(0.7 * GLOBAL.GAME_FPS, "scripts/skill/装备/帐篷挂件.lua", nParam1 + 1, 0)
				return
			end
		end
	end
	if nParam1 == 1 then
		player.PlaySfx(1087, 0, 0, 0)
		player.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/装备/帐篷挂件.lua", nParam1 + 1, 0)
		return
	end
	if nParam1 >= 2 then
		if nParam1 == 2 then
		...
		local buff2 = player.GetBuff(8659, 1)--主人
		if not buff2 then
			player.DelBuff(8640, 1)--监控
			player.SetDynamicSkillGroup(0)
			player.DoAction(0, 10107)	
		...
		--player.AddBuff(0, 99, 8640, 1, 3600) --监控
	...
	--			print(200001)
		...
		--player.AddBuff(0, 99, 8640, 1, 6000) --监控
		--player.AddBuff(0, 99, 8659, 1, 6000)--东道主
		player.SetDynamicSkillGroup(184)
		player.SetTimer(1 * GLOBAL.GAME_FPS, "scripts/skill/装备/帐篷挂件.lua", nParam1 + 1, 0)
		return
	end
end
