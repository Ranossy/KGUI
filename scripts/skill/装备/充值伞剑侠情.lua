---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/充值伞剑侠情.lua
-- 更新时间:	2014/2/17 15:46:42
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if player.dwBackItemIndex ~= 5846 and player.dwBackItemIndex ~= 11076 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND 
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	if player.GetSingleRepresentID(EQUIPMENT_REPRESENT.BACK_EXTEND) == 115 then
