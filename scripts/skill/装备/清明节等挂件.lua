---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/清明节等挂件.lua
-- 更新时间:	2013/4/2 14:29:22
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if player.dwBackItemIndex ~= 8407 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND 
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	if player.GetBuff(1422, 2) then 
		player.DelBuff(1422, 2)	
		return
	end
	if player.GetSingleRepresentID(EQUIPMENT_REPRESENT.BACK_EXTEND) == 135 then
		player.SetRepresentID(EQUIPMENT_REPRESENT.BACK_EXTEND, 0)
	end
	player.DoAction(0, 10102)
	player.AddBuff(0, 99, 1422, 2)
end
