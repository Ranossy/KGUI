---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/帐篷_桃花帐篷.lua
-- 更新时间:	2017/5/17 21:44:38
-- 更新用户:	ZHANGYAN0-PC
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 1207);
...
function Apply(dwCharacterID)
	...
	if npc then
		player.SendSystemMessage(GetEditorString(11, 6527))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(11, 6528), 4)
		return
	else
...
--		player.AddBuff(0, 99, 8640, 1, 10) --监控
		...
		player.ClearCDTime(1207)---------------------------------------------------------------

		player.DoCustomOTAction(0, 1, GLOBAL.GAME_FPS * 1, TARGET.PLAYER, player.dwID, "scripts/skill/装备/帐篷_桃花帐篷.lua", GetEditorString(11, 6529))
		...
		--	player.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/装备/挂件相关/背挂_帐篷.lua", 0, 0)
	end
end
...
function OnCustomEvent(player, target)
	...
	if npc1 and npc2 and npc3 and npc4 then
		...
		if nNum >= 150 then
			...
			if player.GetSingleRepresentID(EQUIPMENT_REPRESENT.BACK_EXTEND) == 393 then
			...
			local npc = scene.GetNpcByNickName("qq_fxf0520" .. player.dwID)
			if not npc then
				scene.CreateNpc(57093, player.nX, player.nY, player.nZ, player.nFaceDirection, 16 * 3, "qq_fxf0520" .. player.dwID)
			end

			--player.SetTimer(1.5 * GLOBAL.GAME_FPS, "scripts/skill/装备/挂件相关/背挂_帐篷.lua", 1, 0)
			------------------------------
			--player.Sprint(false)
			--player.Stop()

			player.AddBuff(0, 99, 10262, 1)
			player.AddBuff(0, 99, 11973, 1)----------------------------------------------------------------------------------------------------
			local buff_fj = player.GetBuff(10796, 1)--副驾
			if buff_fj then
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(7, 456), 4)
			...
			if npc then
				...
				npc.SetDisappearFrames(16 * 60 * 10, 0)
