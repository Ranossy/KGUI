---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/表现_披风_蓝白款.lua
-- 更新时间:	2016/8/22 20:34:13
-- 更新用户:	FENGMENGJIE
-- 脚本说明:
----------------------------------------------------------------------<
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/表现_披风_蓝白款.lua",
		0
	);

	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 1090);
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	--if dwRepresentID1 ~= 553 then
		--if dwRepresentID1 ~= 555  or dwRepresentID2 ~= 107  or dwRepresentID3 ~= 107  or dwRepresentID4 ~= 60 then
			--return false
		--end
	--end
	
	if not player.IsReplaceRepresentMatchCondition(5, 553) then--是否可执行切换5表现位（上衣）到553表现ID
		return false
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--
	local dwRepresentID1 = player.GetRepresentID(EQUIPMENT_REPRESENT.CHEST_STYLE)  --外装
	if player.IsReplaceRepresentMatchCondition(5, 553) then--是否能通过包身换成 表现553（553位包身状态）
		if dwRepresentID1 == 555 then
			player.PlaySfx(1424, player.nX, player.nY, player.nZ + 100)--player.PlaySfx(19, 0, 0, 0)
			player.ReplaceRepresent(5, 553)--执行包身切换处理
		elseif dwRepresentID1 == 553 then
	...
	if dwRepresentID1 == 555  and dwRepresentID2 == 107 and dwRepresentID3 == 107  and dwRepresentID4 == 60 then   -------非包身状态-----
		player.PlaySfx(1424, player.nX, player.nY, player.nZ + 100)--player.PlaySfx(19, 0, 0, 0)
		player.SwitchRepresent(INVENTORY_INDEX.EQUIP, EQUIPMENT_INVENTORY.CHEST)
	elseif dwRepresentID1 == 553  then   -------包身状态-----
		player.PlaySfx(1424, player.nX, player.nY, player.nZ + 100 )
		player.SwitchRepresent(INVENTORY_INDEX.EQUIP, EQUIPMENT_INVENTORY.CHEST)
	end
	--]]
