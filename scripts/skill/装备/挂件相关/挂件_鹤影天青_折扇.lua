---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/挂件_鹤影天青_折扇.lua
-- 更新时间:	2017/5/18 23:33:33
-- 更新用户:	ZHANGYAN0-PC
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/挂件_鹤影天青_折扇.lua",
		0
	);

	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 1209);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.nRoleType == ROLE_TYPE.STANDARD_MALE then
		player.AddBuff(0, 99, 11902, 1, 11)
	...
	elseif player.nRoleType == ROLE_TYPE.LITTLE_GIRL then
		player.AddBuff(0, 99, 11902, 1, 8)
	end
	player.ApplyDoEmotionAction(109)--表情动作ID
end
