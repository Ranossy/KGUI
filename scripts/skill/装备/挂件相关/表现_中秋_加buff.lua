---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/表现_中秋_加buff.lua
-- 更新时间:	2016/9/7 15:19:03
-- 更新用户:	FENGMENGJIE
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/表现_中秋_加buff.lua",
		0
	);

	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 1121);
...
function CanCast(player, nPreResult) -- 判断玩家的状态，以判断是否可以发出技能
	...
	local dwRepresentID = player.GetRepresentID(EQUIPMENT_REPRESENT.CHEST_STYLE)
	if  dwRepresentID ~= 559 and dwRepresentID ~= 560 then
		return false
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--
	local dwRepresentID = player.GetRepresentID(EQUIPMENT_REPRESENT.CHEST_STYLE)
	local buff1 = player.GetBuff(11141, 1)
	local buff2 = player.GetBuff(11120, 1)
	if not buff1  and  not buff2 then
		if dwRepresentID ==559 or dwRepresentID == 560 then
			player.AddBuff(0, 99, 11141, 1, 3600)
		end
	end
