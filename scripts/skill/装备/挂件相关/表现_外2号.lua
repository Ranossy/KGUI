---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/表现_外2号.lua
-- 更新时间:	2015/8/13 10:47:19
-- 更新用户:	zhangyan3
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/表现_外2号.lua",
		0
	);
	
	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 940);
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if not (dwRepresentID == 516 or dwRepresentID == 517) then 
		return false
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if dwRepresentID == 516 then --非带帽子状态
		if player.nRoleType == ROLE_TYPE.LITTLE_BOY then
			player.SetTimer(2 * 16, "scripts/skill/装备/挂件相关/表现_外2号.lua", 0, 0)
		elseif player.nRoleType == ROLE_TYPE.LITTLE_GIRL then
			player.SetTimer(1 * 16, "scripts/skill/装备/挂件相关/表现_外2号.lua", 0, 0)
		elseif player.nRoleType == ROLE_TYPE.STANDARD_FEMALE then
			player.SetTimer(1.5 * 16, "scripts/skill/装备/挂件相关/表现_外2号.lua", 0, 0)
		elseif player.nRoleType == ROLE_TYPE.STANDARD_MALE then
			player.SetTimer(1 * 16, "scripts/skill/装备/挂件相关/表现_外2号.lua", 0, 0)	
		end
	elseif dwRepresentID == 517 then--带帽子状态
		if player.nRoleType == ROLE_TYPE.LITTLE_BOY then
			player.DoAction(player.dwID, 13288)  --正太用动作
			player.SetTimer(2 * 16, "scripts/skill/装备/挂件相关/表现_外2号.lua", 1, 0)
		else
			player.DoAction(player.dwID, 11471) --动作ID[脱帽]
			player.SetTimer(0.5 * 16, "scripts/skill/装备/挂件相关/表现_外2号.lua", 1, 0)
		end	
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	if nParam1 == 0 then
		...
		--
	--	player.SetTimer(2, "scripts/skill/装备/挂件相关/表现_外2号.lua", 11, 0)
		return
	end
	...
	--if nParam1 == 11 then
		--player.DoAction(player.dwID, 13288) --动作ID
	--end
end
