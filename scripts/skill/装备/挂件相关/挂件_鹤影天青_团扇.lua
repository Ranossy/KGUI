---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/挂件_鹤影天青_团扇.lua
-- 更新时间:	2017/5/18 23:36:32
-- 更新用户:	ZHANGYAN0-PC
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/挂件_鹤影天青_团扇.lua",
		0
	);

	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 1208);
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	local dwRepresentID = player.GetRepresentID(EQUIPMENT_REPRESENT.WAIST_EXTEND)
	if dwRepresentID ~= 320 then
		return false
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.nRoleType == ROLE_TYPE.STANDARD_MALE then
		player.AddBuff(0, 99, 11902, 1, 10)
	elseif player.nRoleType == ROLE_TYPE.STANDARD_FEMALE then
		player.AddBuff(0, 99, 11902, 1, 16)
	elseif player.nRoleType == ROLE_TYPE.LITTLE_BOY then
		player.AddBuff(0, 99, 11902, 1, 9)
	elseif player.nRoleType == ROLE_TYPE.LITTLE_GIRL then
		player.AddBuff(0, 99, 11902, 1, 12)
	end
	
	player.ApplyDoEmotionAction(108)--表情动作ID
end
