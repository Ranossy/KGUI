---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/表现_披风_黑甲款.lua
-- 更新时间:	2016/8/11 14:54:14
-- 更新用户:	FENGMENGJIE
-- 脚本说明:
----------------------------------------------------------------------<
--[[
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/表现_披风_黑甲款.lua",
		0
	);

	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 1091);
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if not player.IsReplaceRepresentMatchCondition(5, 554) then--检查是否可以执行包身切换
		return false
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--
	local dwRepresentID1 = player.GetRepresentID(EQUIPMENT_REPRESENT.CHEST_STYLE)  --外装
	if  player.IsReplaceRepresentMatchCondition(5, 554) then--是否能通过包身换成 表现554（554位包身状态）
		if dwRepresentID1 == 556 then
			...
			player.ReplaceRepresent(5, 554)--执行包身切换处理
		elseif dwRepresentID1 == 554 then
			if player.nRoleType == ROLE_TYPE.LITTLE_BOY or player.nRoleType == ROLE_TYPE.LITTLE_GIRL then
				player.PlaySfx(1425, player.nX, player.nY, player.nZ - 100)
			else
				player.PlaySfx(1425, player.nX, player.nY, player.nZ + 200)
