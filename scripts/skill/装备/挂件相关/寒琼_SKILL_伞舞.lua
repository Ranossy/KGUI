---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/寒琼_SKILL_伞舞.lua
-- 更新时间:	2016/8/19 11:25:22
-- 更新用户:	jiangzhefei1
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/寒琼_SKILL_伞舞.lua",
		0
	);

	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 1043);
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	------------------------------------------------------------
	if player.dwBackItemIndex ~= 18380 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND
		return false
	end

	local buff = player.GetBuff(1575, 16)
	if not buff then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(10, 4790), 4)
		player.SendSystemMessage(GetEditorString(10, 4790))
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--
	player.ApplyDoEmotionAction(75)--表情动作ID
	if player.nRoleType == ROLE_TYPE.LITTLE_GIRL then
		player.SetTimer(254+6,"scripts/skill/装备/挂件相关/寒琼_SKILL_伞舞.lua",0,0)
	elseif player.nRoleType == ROLE_TYPE.LITTLE_BOY then
		player.SetTimer(218+5,"scripts/skill/装备/挂件相关/寒琼_SKILL_伞舞.lua",0,0)
	elseif player.nRoleType == ROLE_TYPE.STANDARD_MALE then
		player.SetTimer(206+5,"scripts/skill/装备/挂件相关/寒琼_SKILL_伞舞.lua",0,0)
	elseif player.nRoleType == ROLE_TYPE.STANDARD_FEMALE then
		player.SetTimer(206+6,"scripts/skill/装备/挂件相关/寒琼_SKILL_伞舞.lua",0,0)
	end	
end
...
function OnTimer(player, param1, param2)
	if player then
		player.TurnTo(player.nFaceDirection+5)
	end
end
