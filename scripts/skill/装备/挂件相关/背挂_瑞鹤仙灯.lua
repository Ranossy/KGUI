---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/背挂_瑞鹤仙灯.lua
-- 更新时间:	2017/4/18 14:58:34
-- 更新用户:	ZHANGYAN0-PC
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 1200);
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if player.dwBackItemIndex ~= 19438 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	local npcX = scene.GetNpcByNickName("rhxd_zy201704" .. player.dwID)
	if npcX then
		...
		player.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/装备/挂件相关/背挂_瑞鹤仙灯.lua", 0, 0)
	end
end
	...
	if nParam1 == 0 then
		...
		if npc1 and npc2 and npc3 and npc4 then
			...
			if nNum >= 150 then
				...
				player.ClearCDTime(1200)
				return
			end
			-----------------------------

			player.AddBuff(0, 99, 11913, 1, 65)
			player.SetDynamicSkillGroup(336)

			if player.GetSingleRepresentID(EQUIPMENT_REPRESENT.BACK_EXTEND) == 389 then
			...
			local npcX = scene.GetNpcByNickName("rhxd_zy201704" .. player.dwID)
			if not npcX then
				scene.CreateNpc(56759, player.nX, player.nY, player.nZ, player.nFaceDirection, 65 * 16, "rhxd_zy201704" .. player.dwID) --多加5秒以便AI运行
