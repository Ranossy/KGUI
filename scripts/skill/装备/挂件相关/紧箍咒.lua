---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/紧箍咒.lua
-- 更新时间:	2016/1/29 11:17:34
-- 更新用户:	FENGMENGJIE
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/紧箍咒.lua",
		0
	);

	skill.SetPublicCoolDown(16);							-- 公共CD 1.5秒
	skill.SetPublicCoolDown(1028);	--862	736
...
function CanCast(player, nPreResult) -- 判断玩家的状态，以判断是否可以发出技能
	...
	------------------------------------------------------------
	if ArtistActionCheck(player, 17235) then
		return nPreResult
	end
	
	if player.dwFaceItemIndex ~=17235  then --
		return false
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--
	player.DoAction(player.dwID, 13308) -- 动作ID
end
