---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/表现_春节_加buff.lua
-- 更新时间:	2017/1/17 16:39:12
-- 更新用户:	FENGMENGJIE-PC
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/表现_春节_加buff.lua",
		0
	);

	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 1176);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--
	local dwRepresentID = player.GetRepresentID(EQUIPMENT_REPRESENT.CHEST_STYLE)
	local buff1 = player.GetBuff(11629, 1)
	if not buff1  then
		if dwRepresentID == 574 then
			player.AddBuff(0, 99, 11629, 1, 3600)
		end
	end
