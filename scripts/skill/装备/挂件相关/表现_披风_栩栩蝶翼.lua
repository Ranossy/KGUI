---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/表现_披风_栩栩蝶翼.lua
-- 更新时间:	2016/10/13 19:20:06
-- 更新用户:	FENGMENGJIE
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/表现_披风_栩栩蝶翼.lua",
		0
	);

	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 1147);
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	------------------------------------------------------------
	local dwRepresentID = player.GetRepresentID(EQUIPMENT_REPRESENT.BACK_CLOAK_EXTEND)
	if dwRepresentID ~= 64 then
		return false
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.SetTimer(5, "scripts/skill/装备/挂件相关/表现_披风_栩栩蝶翼.lua", 0, 0)
--[[
	local nT_Value = -90 --360
	local nT = math.floor(player.nFaceDirection/256 * 360 - 180)
	local nT_Now = nT + nT_Value

	if player.nRoleType == ROLE_TYPE.LITTLE_BOY or player.nRoleType == ROLE_TYPE.LITTLE_GIRL then
		player.PlaySfx(1534, player.nX, player.nY, player.nZ + 400, nT_Now)
	else
		player.PlaySfx(1534, player.nX, player.nY, player.nZ, nT_Now)
	end
--]]
	player.ApplyDoEmotionAction(78)
end
...
function OnTimer(player, nP1, NP2)
	...
	if player.nRoleType == ROLE_TYPE.LITTLE_BOY or player.nRoleType == ROLE_TYPE.LITTLE_GIRL then
		player.PlaySfx(1541, player.nX, player.nY, player.nZ, nT_Now)
	else
		player.PlaySfx(1534, player.nX, player.nY, player.nZ, nT_Now)
	end
end
