---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/背挂_秋千2_交互.lua
-- 更新时间:	2017/2/9 19:25:43
-- 更新用户:	FENGMENGJIE-PC
-- 脚本说明:
----------------------------------------------------------------------<
-------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 1180);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local scene = caster.GetScene()
	if not scene then
		return
	end
	------主驾动作-------------
	local buff1 = caster.GetBuff(11687, 1)
	if buff1 then
		local npc = scene.GetNpcByNickName("QIUQ_fxf0124" .. caster.dwID)
		if npc then
			local dwPlayer_fjID = npc.GetPlayerIDBySocketIndex(1)
			local target1 = GetPlayer(dwPlayer_fjID)
			if dwPlayer_fjID == nil then
				RemoteCallToClient(caster.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(10, 1300), 4)
				caster.ClearCDTime(1180)
			end
			if target1 then
				if target1.nRoleType == ROLE_TYPE.STANDARD_MALE then
					caster.CastSkill(15967, 1)
				elseif target1.nRoleType == ROLE_TYPE.STANDARD_FEMALE then
					caster.CastSkill(15968, 1)
				elseif target1.nRoleType == ROLE_TYPE.LITTLE_BOY then
					caster.CastSkill(15969, 1)
				else
					caster.CastSkill(15970, 1)
				end
			end
		end
	end
	...
	----------------------------------
	------副驾动作--------------------
	local buff2 = caster.GetBuff(11688, 1)
	if buff2 then
		local buff3 = caster.GetBuff(11686, 1)
		if buff3 then
			local owner = GetPlayer(buff3.nCustomValue)
			if owner then
				if owner.nRoleType == ROLE_TYPE.STANDARD_MALE then
					caster.CastSkill(15971, 1)
				elseif owner.nRoleType == ROLE_TYPE.STANDARD_FEMALE then
					caster.CastSkill(15972, 1)
				elseif owner.nRoleType == ROLE_TYPE.LITTLE_BOY then
					caster.CastSkill(15973, 1)
				else
					caster.CastSkill(15974, 1)
				end
			end
		end
	end
