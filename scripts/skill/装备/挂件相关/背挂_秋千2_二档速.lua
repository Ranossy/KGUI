---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/背挂_秋千2_二档速.lua
-- 更新时间:	2017/2/9 19:22:52
-- 更新用户:	FENGMENGJIE-PC
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	local buff = player.GetBuff(11687, 1)
	if not buff then
...
function Apply(dwCharacterID)
	...
	local buff1 = npc.GetBuff(11689, 1)
	if buff1 then
		npc.DelBuff(11689, 1)
	end

	local buff2 = npc.GetBuff(11690, 1)
	if buff2 then
		npc.DelBuff(11690, 1)
	end

	npc.AddBuff(0, 99, 11691, 1, 1000) -- 一档速
