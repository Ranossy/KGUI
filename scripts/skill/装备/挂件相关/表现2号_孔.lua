---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/表现2号_孔.lua
-- 更新时间:	2015/8/13 10:47:35
-- 更新用户:	zhangyan3
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/表现2号_孔.lua",
		0
	);

	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 933);
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if not (dwRepresentID == 25 or dwRepresentID == 31) then 
		return false
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--
	player.ApplyDoEmotionAction(50)--表情动作ID
end
