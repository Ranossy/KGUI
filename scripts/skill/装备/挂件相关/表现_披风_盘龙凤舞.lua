---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/表现_披风_盘龙凤舞.lua
-- 更新时间:	2017/1/16 15:24:15
-- 更新用户:	ZHANGYAN0-PC
-- 脚本说明:
----------------------------------------------------------------------<
----------------------------------------------------------------------<
--[[
// 导出脚本函数
// 函数名  : IsReplaceRepresentMatchCondition
// 函数描述: 判断是否满足换表现的条件，可以让玩家某个表现位（nRepresentIndex）换成对应表现ID（dwRepresentID）
// 参数列表: nRepresentIndex, dwRepresentID
// 返回值  : bool
// 备注    :
// 示例    : IsReplaceRepresentMatchCondition(nRepresentIndex, dwRepresentID)
huanglei(黄蕾) 09-08 17:43:32
// 导出脚本函数
// 函数名  : ReplaceRepresent
// 函数描述: 让玩家某个表现位（nRepresentIndex）换成对应表现ID（dwRepresentID），当然要满足表里的条件才行
// 参数列表: nRepresentIndex, dwRepresentID
// 返回值  : bool
// 备注    :
// 示例    : ReplaceRepresent(nRepresentIndex, dwRepresentID)
huanglei(黄蕾) 09-08 17:43:41
// 函数名  : RecoverRepresent
// 函数描述: 恢复玩家某个表现位（nRepresentIndex）的表现ID，会把对应的color表现位的表现也恢复
// 参数列表: nRepresentIndex
// 返回值  : bool
// 备注    : 如果穿着外装，就恢复成外装的表现，否则就恢复成装备的表现，只针对有外观表现的表现位
 // 示例    : RecoverRepresent(nRepresentIndex)
--]]
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/表现_披风_盘龙凤舞.lua",
		0
	);

	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 1175);------------------------------------------------------------------------[][][][][][][][][][[]][
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	--EQUIPMENT_REPRESENT.R_SHOULDER_EXTEND	--右肩
	...
	--if dwRepresentID1 ~= 573 then
	--if dwRepresentID1 ~= 572  or dwRepresentID2 ~= 107  or dwRepresentID3 ~= 107  or dwRepresentID4 ~= 60 then
	--return false
	--end
	--end

	if not player.IsReplaceRepresentMatchCondition(5, 573) then--是否可执行切换5表现位（上衣）到573表现ID-------------------------[][][][][][][][][][[]][
		return false
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	for i = 1, 100 do
		...
		player.DelBuff(9320, 1) --阳春白雪删除
	end
	------------------------------------------------------------[][][][][][][]
	local dwRepresentID_San = 572
	local dwRepresentID_Bao = 573
	local tSfx = {
		[1] = {8660, 0, 0, 0},
		[2] = {8658, 0, 0, 0},
		[5] = {8659, 0, 0, 0},
	...
	-------------------------------------------------------------[][][][][][][]
	local dwRepresentID1 = player.GetRepresentID(EQUIPMENT_REPRESENT.CHEST_STYLE)  --外装
	if player.IsReplaceRepresentMatchCondition(5, dwRepresentID_Bao) then--是否能通过包身换成 表现573（573位包身状态）
		if dwRepresentID1 == dwRepresentID_San then
			if  player.nRoleType == ROLE_TYPE.STANDARD_FEMALE then    ---标女
				player.PlaySfx(tSfx[2][1], player.nX + tSfx[2][2], player.nY + tSfx[2][3], player.nZ + tSfx[2][4])
			elseif player.nRoleType == ROLE_TYPE.LITTLE_GIRL then    ---萝莉
				player.PlaySfx(tSfx[6][1], player.nX + tSfx[6][2], player.nY + tSfx[6][3], player.nZ + tSfx[6][4])
			elseif player.nRoleType == ROLE_TYPE.STANDARD_MALE then    ---成男
				player.PlaySfx(tSfx[1][1], player.nX + tSfx[1][2], player.nY + tSfx[1][3], player.nZ + tSfx[1][4])
			elseif player.nRoleType == ROLE_TYPE.LITTLE_BOY then    ---正太
				player.PlaySfx(tSfx[5][1], player.nX + tSfx[5][2], player.nY + tSfx[5][3], player.nZ + tSfx[5][4])
			end
			player.ReplaceRepresent(5, dwRepresentID_Bao)--执行包身切换处理
		elseif dwRepresentID1 == dwRepresentID_Bao then
			if  player.nRoleType == ROLE_TYPE.STANDARD_FEMALE then    ---标女
				player.PlaySfx(tSfx[2][1], player.nX + tSfx[2][2], player.nY + tSfx[2][3], player.nZ + tSfx[2][4])
			elseif player.nRoleType == ROLE_TYPE.LITTLE_GIRL then    ---萝莉
				player.PlaySfx(tSfx[6][1], player.nX + tSfx[6][2], player.nY + tSfx[6][3], player.nZ + tSfx[6][4])
			...
			player.RecoverRepresent(5)--执行包身切换处理
		end
	end
	...
	if dwRepresentID1 == 572  and dwRepresentID2 == 107 and dwRepresentID3 == 107  and dwRepresentID4 == 60 then   -------非包身状态-----
		player.PlaySfx(1424, player.nX, player.nY, player.nZ + 100)--player.PlaySfx(19, 0, 0, 0)
		player.SwitchRepresent(INVENTORY_INDEX.EQUIP, EQUIPMENT_INVENTORY.CHEST)
	elseif dwRepresentID1 == 573  then   -------包身状态-----
		player.PlaySfx(1424, player.nX, player.nY, player.nZ + 100 )
		player.SwitchRepresent(INVENTORY_INDEX.EQUIP, EQUIPMENT_INVENTORY.CHEST)
	end
	--]]
end
