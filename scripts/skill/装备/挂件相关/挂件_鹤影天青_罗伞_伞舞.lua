---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/挂件_鹤影天青_罗伞_伞舞.lua
-- 更新时间:	2017/5/18 23:39:21
-- 更新用户:	ZHANGYAN0-PC
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/挂件_鹤影天青_罗伞_伞舞.lua",
		0
	);

	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 1210);
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if dwRepresentID ~= 395 then
		return false
	end
	if player.GetBuff(1575, 0) then
		return false
	end
	--检查换装
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	elseif player.nRoleType == ROLE_TYPE.STANDARD_FEMALE then
		player.AddBuff(0, 99, 11902, 1, 29)
	...
	player.ApplyDoEmotionAction(110)--表情动作ID
end
