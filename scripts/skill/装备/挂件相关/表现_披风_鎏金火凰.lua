---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件相关/表现_披风_鎏金火凰.lua
-- 更新时间:	2016/10/13 19:19:43
-- 更新用户:	FENGMENGJIE
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/装备/挂件相关/表现_披风_鎏金火凰.lua",
		0
	);

	skill.SetPublicCoolDown(938);
	skill.SetNormalCoolDown(1, 1146);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.nRoleType == ROLE_TYPE.LITTLE_BOY or player.nRoleType == ROLE_TYPE.LITTLE_GIRL then
		player.PlaySfx(1540, player.nX, player.nY, player.nZ, nT_Now)
	else
		player.PlaySfx(1533, player.nX, player.nY, player.nZ, nT_Now)
	end

	player.ApplyDoEmotionAction(77)
end
