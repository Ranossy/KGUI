---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/挂件_七彩风车使用.lua
-- 更新时间:	2016/5/25 19:24:00
-- 更新用户:	xutong3
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	--if player.dwWaistItemIndex ~= 17521 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND
	if player.dwBackItemIndex ~= 17521 then 
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	if player.GetBuff(10801, 1) then
		return
	end
--	player.DoAction(0, 13217)
	if player.GetItemAmount(5, 26718) < 1 then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(10, 1216), 4)
		return
	end
	
	player.AddBuff(player.dwID, player.nLevel, 10801, 1)
	player.SetDynamicSkillGroup(259)
	player.CostItem(5,26718,1)
	-- player.GetSingleRepresentID(EQUIPMENT_REPRESENT.BACK_EXTEND) == 135
end
