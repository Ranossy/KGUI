---------------------------------------------------------------------->
-- 脚本名称:	G:/zhugan/client/scripts/skill/装备/罗伞戏鱼渊.lua
-- 更新时间:	2012-10-24 10:05:21
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if player.dwBackItemIndex ~= 6915 and player.dwBackItemIndex ~= 19250 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND 
		return false
	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	if player.GetSingleRepresentID(EQUIPMENT_REPRESENT.BACK_EXTEND) == 120 then
	...
	if player.GetBuff(1422, 1)  or player.GetBuff(1426, 1)  or player.GetBuff(1426, 2) or  player.GetBuff(1528, 1) then			--中秋活动中秋礼服、灯笼buff, 成就系统提灯笼Buff
		...
		player.DoAction(0, 11347)					--播放开伞动作
		player.AddBuff(0, 99, 1576, 7)				--添加开伞后加持伞换动作Buff
	elseif  player.GetBuff(1575, 1) or player.GetBuff(1575, 2) or player.GetBuff(1575, 3) or player.GetBuff(1575, 4) or player.GetBuff(1575, 5) or player.GetBuff(1575, 6) or player.GetBuff(1575, 7) then			--成就系统提灯笼Buff
		player.AddBuff(0, 99, 1593, 1, 1) --驱除所有换模、换动作的Buff
