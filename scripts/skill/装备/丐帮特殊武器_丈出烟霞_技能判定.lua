---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/丐帮特殊武器_丈出烟霞_技能判定.lua
-- 更新时间:	2015/1/6 14:00:48
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	...
   	skill.AddAttribute(
   		...
   		"skill/装备/丐帮特殊武器_丈出烟霞_技能判定.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npc.nLevel <= 94 then
		if player.dwForceID == 1 then
			--打阳性内功伤害
			player.CastSkill(13641, 1, player.GetSkillTarget())
			--player.AddBuff(player.dwID, player.nLevel, 8715, 1)
		elseif (player.dwForceID == 2 and player.GetKungfuMount() ~= nil and player.GetKungfuMount().dwSkillID == 10021) or (player.dwForceID == 4 and player.GetKungfuMount() ~= nil and player.GetKungfuMount().dwSkillID == 10014) then
			--打混元内功伤害
			player.CastSkill(13642, 1, player.GetSkillTarget())
			--player.AddBuff(player.dwID, player.nLevel, 8715, 1)
		elseif (player.dwForceID == 5 and player.GetKungfuMount() ~= nil and player.GetKungfuMount().dwSkillID == 10081) or (player.dwForceID == 10) then
			--打阴性内功伤害
			player.CastSkill(13643, 1, player.GetSkillTarget())
			--player.AddBuff(player.dwID, player.nLevel, 8715, 1)
		elseif (player.dwForceID == 6 and player.GetKungfuMount() ~= nil and player.GetKungfuMount().dwSkillID == 10175) or (player.dwForceID == 7 and player.GetKungfuMount() ~= nil and player.GetKungfuMount().dwSkillID == 10225) then
			--打毒性内功伤害
			player.CastSkill(13644, 1, player.GetSkillTarget())
			--player.AddBuff(player.dwID, player.nLevel, 8715, 1)
		elseif not(player.dwForceID == 2 and player.GetKungfuMount() ~= nil and player.GetKungfuMount().dwSkillID == 10028) and not (player.dwForceID == 5 and player.GetKungfuMount() ~= nil and player.GetKungfuMount().dwSkillID == 10080) and not (player.dwForceID == 6 and player.GetKungfuMount() ~= nil and player.GetKungfuMount().dwSkillID == 10176) then
			--打外功伤害
			player.CastSkill(13645, 1, player.GetSkillTarget())
			--player.AddBuff(player.dwID, player.nLevel, 8715, 1)
		end
	end
end
