---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/装备/面具舞蹈201503月.lua
-- 更新时间:	2015/3/16 11:43:10
-- 更新用户:	zhangyan3
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetPublicCoolDown(862);	--862	736					
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if ArtistActionCheck(player, 10182) then
		return nPreResult
	end
	
	if player.dwFaceItemIndex ~= 13248 and player.dwFaceItemIndex ~= 19235 then --增加限时面挂的判断处理
		return false
	end
...
function Apply(dwCharacterID)
	...
	if ArtistActionCheck(player, 10182) then
	    player.DoAction(player.dwID, 55168)
	end
