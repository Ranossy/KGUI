---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/七秀/七秀_剑舞_通用添加剑舞之剑主天地回3层剑舞.lua
-- 更新时间:	2014/11/5 21:36:07
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/七秀/七秀_剑舞_通用添加剑舞之剑主天地回3层剑舞.lua",
		0
	);
	skill.AddAttribute(
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 2921, 1)			-- 设置Buff
	...
	if player.GetBuff(1383, 2) or player.GetBuff(8528,1) then
		...
		for i = 1, 7 do
			if player.GetBuff(409, i) then
				...
				stackNum = nBuff.nStackNum
				if stackNum >= 7  then
					if not player.GetBuff(5865,1) then
						player.AddBuff(dwCharacterID, player.nLevel, 5865, 1, 1)
					end
				else
					if player.GetBuff(5865, 1) then
						player.DelBuff(5865, 1)
					end
				end
				if stackNum >= 4  then
					if not player.GetBuff(5866,1) then
						player.AddBuff(dwCharacterID, player.nLevel, 5866, 1, 1)
					end
				else
					if player.GetBuff(5866, 1) then
						player.DelBuff(5866, 1)
					end
				end
			end
