---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/七秀/橙武翔舞.lua
-- 更新时间:	2013/8/1 16:26:08
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(2584, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);		-- 需求自身Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 680, 29);			-- 设置Buff
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK, -- 属性作用模式
		ATTRIBUTE_TYPE.EXECUTE_SCRIPT, -- 魔法属性
		"skill/七秀/橙武翔舞.lua", -- 属性值1
		0														-- 属性值2
	);
	--skill.AddAttribute(
	--ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK, -- 属性作用模式
	...
	--skill.nCostManaBasePercent = 0.5 * (1 + dwSkillLevel / 7) * 34;	-- 技能消耗的内力
	----------------- 聚气相关 -------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.SetTimer(1, "scripts/skill/七秀/橙武翔舞.lua", dwCharacterID, 0)
end
...
function OnTimer(player, value1, value2)
	local target
	if IsPlayer(value1) then
		target = GetPlayer(value1)
		player.CastSkill(6955, 1, TARGET.PLAYER, value1)
	else
		target = GetNpc(value1) 
		player.CastSkill(6955, 1, TARGET.NPC, value1)
	end
end
