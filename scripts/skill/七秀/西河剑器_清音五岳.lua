---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/七秀/西河剑器_清音五岳.lua
-- 更新时间:	2013/6/5 3:38:20
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	--]]
	skill.AddSlowCheckSelfBuff(10121, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
...
function OnSkillLevelUp(skill, player)
	...
	if player.GetSkillLevel(15127) ~= lv then
		player.LearnSkillLevel(15127, lv, player.dwID)
	end
	
	--满级新手指引任务
	local nQuestIndex = player.GetQuestIndex(13253)
	local nQuestPhase = player.GetQuestPhase(13253)
