---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/七秀/90级奇穴/上元完结效果提前触发.lua
-- 更新时间:	2014/7/1 19:52:53
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nlev = player.GetSkillLevel(556)
	if target.nCurrentLife <= target.nMaxLife * 0.5 then
		if player.GetSkillLevel(9319) == 1 and not player.GetBuff(8218,1) then
			if IsPlayer(dwCharacterID) then
				player.CastSkill(9320, nlev, TARGET.PLAYER, target.dwID)
			else
				player.CastSkill(9320, nlev, TARGET.NPC, target.dwID)
			end
			player.AddBuff(player.dwID,player.nLevel,8218,1)
			local buff = target.GetBuffByOwner(3830, nlev, player.dwID)
			if buff then
