---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/七秀/90级奇穴/繁音急节AOE.lua
-- 更新时间:	2015/6/16 17:29:03
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/七秀/90级奇穴/繁音急节AOE.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(5859) == 1 then
		target.AddBuff(player.dwID, player.nLevel, 9075, nlev)
	else
		target.AddBuff(player.dwID, player.nLevel, 538, nlev)
	end
	--]]
if player.GetSkillLevel(14710) == 1 and player.GetSkillLevel(14715) == 1 then
			target.AddBuff(player.dwID, player.nLevel, 9769, 1,15)	--45%版本，只有一级
		else
			if player.GetSkillLevel(14710) == 1 then
				target.AddBuff(player.dwID, player.nLevel, 9769, 1, 30) --45%版本，30秒
			elseif player.GetSkillLevel(14715) == 1 then
				target.AddBuff(player.dwID, player.nLevel, 538, nlev,15)--30%版本，15秒
			else
				target.AddBuff(player.dwID, player.nLevel, 538, nlev,30) --30%版本，30秒	
			end
		end
	end
