---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/七秀/90级奇穴/天地消失额外添加3秒效果.lua
-- 更新时间:	2015/9/15 19:59:26
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	if player.GetSkillLevel(14723) == 1 and target.nCurrentLife <= target.nMaxLife * 0.5 then
		if BuffID == 684 then
			target.AddBuff(player.dwID, player.nLevel, 9775, 1, 3)
		elseif BuffID == 9781 then
			target.AddBuff(player.dwID, player.nLevel, 9782, 1, 3)
		end
	end
end
...
function OnDetach(nCharacterID, BuffID, nBuffLevel, nLeftFrame)
	...
	if nLeftFrame ~= 0 and player.nMoveState ~= 16 and player.GetSkillLevel(5862) == 1 then
		player.ModifyCoolDown(119, - 320)
	end
end
