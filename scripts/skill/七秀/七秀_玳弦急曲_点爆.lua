---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/七秀/七秀_玳弦急曲_点爆.lua
-- 更新时间:	2013/5/1 10:00:50
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	--skill.AddSlowCheckDestOwnBuff(2920, 3, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);	-- 需求目标属于自己的Buff   
...
function Apply(dwCharacterID)
	...
	if player.GetSkillLevel(5860) == 1 then
		if player.GetSkillLevel(9345) == 1 then
			player.CastSkill(9421, lv)
		else
			player.CastSkill(9114, lv)
		end
	end
	--点爆效果
	if player.GetSkillLevel(5869) == 1 then
		if player.GetSkillLevel(9345) == 1 then
			--点爆4层
			player.CastSkill(9428, 1)
		else
			--点爆3层
			player.CastSkill(3889, 1)
		end
	end
end
