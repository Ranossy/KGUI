---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/七秀/镇派/鹊踏枝期间闪避回蓝.lua
-- 更新时间:	2015/2/5 11:54:49
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	...
   	skill.AddAttribute(
   		...
   		"skill/七秀/镇派/鹊踏枝期间闪避回蓝.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(13678) == 1 then
		...
		local nlev = player.GetSkillLevel(546)
		if nlev ~= 0 then
			local rest, nleftframe, ntotalframe = player.GetSkillCDProgress(546, nlev)
			if rest == true and nleftframe ~= 0 and not player.GetBuff(8743,1) then
				player.ClearCDTime(116)
				player.AddBuff(player.dwID,player.nLevel,8743,1)
			end
		end
	end
end
