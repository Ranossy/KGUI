---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/轻功/长_鱼头_QRU.lua
-- 更新时间:	2015/10/6 17:57:36
-- 更新用户:	zhangyan3
-- 脚本说明:
----------------------------------------------------------------------<
...
--	skill.SetNormalCoolDown(1, 979);	--技能普通CD
...
function Apply(dwCharacterID)
	...
	-------------------------------------------------
	local nH, nType = player.GetAltitude()--二段跳高度
	if nH < 1600 then
		return
	end
	------------------------------------------------------
	local buff = player.GetBuff(10191, 1)
	if buff then
		return
	end
	...
	player.AddBuff(0, 99, 9891, 1, 60 * 10)

	--player.SetPassiveVelocityXY(300)
	--player.SetPassiveVelocityZ(-60)
	--player.LockPassiveDirectionXYByFace()

	local nNumValue = 50 * 100
