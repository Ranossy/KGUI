...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		ATTRIBUTE_TYPE.SET_JUMP_COUNT,--修改跳数为指定值
		10,
		1
		);
	--[[
...
function Apply(dwCharacterID)
	...
	if player then
		if player.nMoveState ~= MOVE_STATE.ON_FLY_JUMP then
			if player.nPitchDirection > -14 then
				player.PitchTo(-14)
			end	
			player.SetTurnRange(128, 128, -14, -64)
			player.AddBuff(0, 99, 8633, 1, 30)
		--	player.AddBuff(0, 99, 8311, 1, 30)
			player.FlyJump()
		end
--		player.AddBuff(0,99,8311,1,2000)
	end
end
