---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/经脉/基础系/基础系_内功切换.lua
-- 更新时间:	2017/4/1 11:00:36
-- 更新用户:	XIAXIANBO1
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if player.bInArenaQueue then
		player.SendSystemMessage(GetEditorString(11, 4663))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(11, 4622), 4)
		return false
	end	
	if player.GetBuff(11837, 1) then
		player.SendSystemMessage(GetEditorString(11, 4664))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(11, 4664), 4)
		return false
	end
	if player.GetBuff(12009, 1) then
		player.SendSystemMessage(GetEditorString(11, 6709))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(11, 6709), 4)
		return false
	end
	local tBuff = {
		{4052, 1},
	...
	for i = 1, #tBuff do
		if player.GetBuff(tBuff[i][1], tBuff[i][2]) then
			player.DelBuff(tBuff[i][1], tBuff[i][2])
		end
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.bInArenaQueue then
		player.SendSystemMessage(GetEditorString(11, 4663))
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(11, 4663), 4)
		return
	end
	if player.GetBuff(11837, 1) then
		player.SendSystemMessage(GetEditorString(11, 4664))
	...
	local BJbuff = player.GetBuff(8290, 1)
	local KungfuID
	if BJbuff then
		KungfuID = BJbuff.nCustomValue
	end
	player.UmountKungfu()
	local lv = player.GetSkillLevel(KungfuID)
	if lv ~= 0 then
		player.MountKungfu(KungfuID, lv)
	end
