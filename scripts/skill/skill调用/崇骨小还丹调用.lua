---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/skill调用/崇骨小还丹调用.lua
-- 更新时间:	06/07/10 14:20:46
-- 更新用户:	zhouhengda2
-- 脚本说明:	
----------------------------------------------------------------------<
...
function Apply(dwCharacterID)  --返还效果结束时候调用，返还效果必须按不同BUFF分别调用
	...
	if buff then
		...
		if nStackNum > 1 then 
			...
			player.SendSystemMessage(SKILL_STRING_TABLE[31387] .. nStackNum - 1 .. SKILL_STRING_TABLE[31371])
		elseif nStackNum == 1 then
			player.SendSystemMessage(SKILL_STRING_TABLE[31388])
		end
	end
end
