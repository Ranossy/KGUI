---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/套路及子技能/无咎被击重置龙啸.lua
-- 更新时间:	2016/6/29 22:41:10
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.nCurrentLife <= player.nMaxLife * 0.4 and player.nMoveState ~= MOVE_STATE.ON_DEATH and player.bFightState == true then
		local nleftframe, nCDCount = player.GetCDLeft(603)
		if nleftframe ~= 0 and not player.GetBuff(10926, 1) then
			player.ClearCDTime(603)
			player.AddBuff(player.dwID, player.nLevel, 10926, 1)
		end
	end
end
