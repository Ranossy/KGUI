---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/丐帮_蜀犬吠日·戳.lua
-- 更新时间:	2013/6/8 14:16:01
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetDelaySubSkill(1 * 0.528, 6353, dwSkillLevel)
	skill.SetDelaySubSkill(18 * 0.528, 6351, dwSkillLevel)	
	skill.SetDelaySubSkill(24 * 0.528, 6352, dwSkillLevel)	
	skill.SetDelaySubSkill(45 * 0.528, 6354, dwSkillLevel)	
	-----------------技能渐变相关-------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	--player.AddBuff(player.dwID, player.nLevel, 5754, 1, 31)
...
function Apply(dwCharacterID)
	...
	player.AddBuff(player.dwID, player.nLevel, 5754, 1, 31)
	if target.nImmuneSkillMoveCount == 0 then
		target.BeHitted(player.dwID, 5351, 1, 66)
	end
	if player.GetSkillLevel(6815) ~= 1 then
	...
	--target.SetTimer(3, "scripts/skill/丐帮/丐帮_蜀犬吠日·戳.lua", 0, 0)
end
