---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/丐帮_拔狗朝天·绊.lua
-- 更新时间:	2013/6/8 14:19:10
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK, --回蓝5%
		ATTRIBUTE_TYPE.CALL_BUFF_STACK_5,
		5997,
		1
	);
	skill.AddAttribute(
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(5523, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);			-- 需求自身Buff
	...
	skill.SetDelaySubSkill(6 * 0.528, 6377, dwSkillLevel)
	skill.SetDelaySubSkill(21 * 0.528, 6378, dwSkillLevel)
	----------------- BUFF相关 -------------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if target.nImmuneSkillMoveCount == 0 and player.GetBuff(5523,1)  then
...
function Apply(dwCharacterID)
	...
	if target.nImmuneSkillMoveCount == 0 then
		target.BeHitted(player.dwID, 5354, 1, 64)
	end
	if player.GetSkillLevel(6845) == 1 then
