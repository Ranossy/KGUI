---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/丐帮_神龙摆尾_击退.lua
-- 更新时间:	2013/6/8 11:59:21
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	--);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(5984, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);		-- 需求自身Buff
	...
	skill.SetDelaySubSkill(2, 6358, dwSkillLevel)
	skill.SetDelaySubSkill(5, 6359, dwSkillLevel)
	skill.SetDelaySubSkill(11, 6360, dwSkillLevel)	
	-----------------技能渐变相关-------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if target.nImmuneSkillMoveCount == 0 then
		...
		--player.AdjustPlayerDistance(target.dwID, 100)
	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	if target.nImmuneSkillMoveCount == 0 then
		target.BeHitted(player.dwID, 5367, 1, 41)
	end
