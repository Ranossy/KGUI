---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/丐帮_龙战于野.lua
-- 更新时间:	2014/11/26 11:47:20
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(  --处理冲刺时丢失目标的情况
	...
	skill.SetNormalCoolDown(1, 620);	--技能普通CD
	...
	skill.nCostManaMaxPercent = 0.2 * 1024;					-- 技能消耗的内力
...
function OnSkillLevelUp(skill, player)
	if skill.dwLevel == skill.dwMaxLevel then
		player.AcquireAchievement(3447)
	end
end
...
function Apply(dwCharacterID)
	...
	if player.GetBuff(8676, 1) then
		local nbuff = player.GetBuff(8676, 1)
		nbuff.nCustomValue = Target.dwID
	end
...
function UnApply(dwCharacterID)
	local player = GetPlayer(dwCharacterID)
	local dwSkillLevel = player.GetSkillLevel(5266)   --获得母技能 龙跃于渊 的等级
	if player.GetBuff(8676, 1) then
		local nbuff = player.GetBuff(8676, 1)
		local nID = nbuff.nCustomValue
		local targetN = nil
		if nID ~= 0 then
			if IsPlayer(nID) then
				targetN = GetPlayer(nID)
				if targetN then
					player.SetTarget(TARGET.PLAYER, targetN.dwID)
					player.CastSkill(5527, dwSkillLevel,TARGET.PLAYER,targetN.dwID)				
				end
			else
				targetN = GetNpc(nID)
				if targetN then
					player.SetTarget(TARGET.NPC,targetN.dwID)
					player.CastSkill(5527, dwSkillLevel,TARGET.NPC,targetN.dwID)				
				end
			end
			if not targetN then
				player.CastSkill(5527, dwSkillLevel)
			end
		else
			player.CastSkill(5527, dwSkillLevel)
		end
		player.DelBuff(8676,1)
	else
		player.CastSkill(5527, dwSkillLevel)
	end

	--[[
		if player.IsSkillRecipeActive(1602, 1) then
		player.AddBuff(dwCharacterID, player.nLevel, 6579, 1, 1)
	--]]
	end
