---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/丐帮_御鸿于天.lua
-- 更新时间:	2015/9/26 19:06:12
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 100, nDamageRand = 20, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddSlowCheckSelfOwnBuff(9922, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);	-- 需求自身属于自己的Buff
	--skill.AddSlowCheckDestOwnBuff(9922, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);	-- 需求目标属于自己的Buff
	...
	skill.SetNormalCoolDown(1, 961);	--技能普通CD
	...
	skill.nCostManaMaxPercent = 0.1 * 1024;					-- 技能消耗的内力
	----------------- 聚气相关 -------------------------------------------------
...
function OnSkillLevelUp(skill, player)
	if player.GetSkillLevel(14928) ~= 1 then
		player.LearnSkillLevel(14928, 1, player.dwID)
	end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nlev = 0
	if player.GetSkillLevel(6824) == 1 then
		nlev = player.GetSkillLevel(8490)
	else
		nlev = player.GetSkillLevel(5638)	
	end
	

	if nlev ~= 0 then
		local nbuff_1 = target.GetBuffByOwner(6367, nlev, player.dwID)
		local nbuff_8 = target.GetBuffByOwner(6401, nlev, player.dwID)

		if nbuff_1 then
			player.AddBuff(player.dwID, player.nLevel, 9922, 1)	--一层标记buff
			target.DelBuffByIndex(nbuff_1.nIndex)
			player.AddBuff(player.dwID, player.nLevel, 10221, 1, 1)
		end

		if nbuff_8 then
			player.AddBuff(player.dwID, player.nLevel, 9922, 2)	--八层标记buff
			local nbuff_mark = player.GetBuff(9922, 2)
			if nbuff_mark then
				nbuff_mark.nCustomValue = nbuff_8.nStackNum
			end

			for i = 1, nbuff_8.nStackNum do
				player.AddBuff(player.dwID, player.nLevel, 10221, 1, nbuff_8.nStackNum)
			end
			for i = 1, 8 do
				target.DelBuffByIndex(nbuff_8.nIndex)
			end
		end
	end

	ModityCDToUI(player, 14927, 0, 0)
end
