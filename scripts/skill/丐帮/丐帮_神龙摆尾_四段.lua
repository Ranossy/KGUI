---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/丐帮_神龙摆尾_四段.lua
-- 更新时间:	2013/6/8 12:01:36
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(5987, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);		-- 需求自身Buff
	...
	skill.SetDelaySubSkill(20, 6365, dwSkillLevel)	
	-----------------技能渐变相关-------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if target.nImmuneSkillMoveCount == 0 and player.GetBuff(5987, 1) then
...
function Apply(dwCharacterID)
	...
	if target.nImmuneSkillMoveCount == 0 then
		target.BeHitted(player.dwID, 5370, 1, 39)
	end
	...
	if player.GetSkillLevel(6831) == 1 then
		...
		if not nbuff or (nbuff and nbuffcount < 4) then
