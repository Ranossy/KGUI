---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/丐帮_龙跃于渊.lua
-- 更新时间:	2013/5/25 10:53:49
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK, --标识
	...
	--苍云反会有问题
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK, --免控
		ATTRIBUTE_TYPE.CALL_BUFF,
		6015,
		1
	);	
	skill.AddAttribute(												
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK, -- 龙跃冲刺
		ATTRIBUTE_TYPE.CAST_SKILL_TARGET_DST,
		6872,
		1									
	); 	
	skill.AddAttribute(												
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK, -- 奇穴龙跃标记
		ATTRIBUTE_TYPE.CAST_SKILL,
		6923,
		1									
	); 		
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 6364, 1);		-- 设置1号位Buff
...
function UnApply(dwCharacterID)
	...
	local dwSkillLevel = player.GetSkillLevel(5262)   --获得母技能 龙跃于渊 的等级

	if player.GetBuff(7848, 1) then
		local nbuff = player.GetBuff(7848, 1)
		...
		if nID ~= 0 then
			if IsPlayer(nID) then
				...
				if targetN then
					...
					player.CastSkill(5363, dwSkillLevel,TARGET.PLAYER,targetN.dwID)				
				end
			else
				targetN = GetNpc(nID)
				if targetN then
					player.SetTarget(TARGET.NPC,targetN.dwID)
					player.CastSkill(5363, dwSkillLevel,TARGET.NPC,targetN.dwID)				
				end
			end
			if not targetN then
				player.CastSkill(5363, dwSkillLevel)
			end
		else
			player.CastSkill(5363, dwSkillLevel)
		end
		player.DelBuff(7848,1)
	else
		player.CastSkill(5363, dwSkillLevel)
	end
end
