...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--[[
	if player.nCurrentLife <= player.nMaxLife * 0.3 and not player.GetBuff(9854, 1) then
		--print(1111)
		local nlev = player.GetSkillLevel(5269)
		if nlev ~= 0 then
			local rest, nleftframe, ntotalframe = player.GetSkillCDProgress(5269, nlev)
			if rest == true and nleftframe ~= 0 then
				--print(2222222)
				player.ClearCDTime(621)
				player.ClearCDTime(88)
				player.ClearCDTime(606)
				player.AddBuff(player.dwID,player.nLevel,9854,1)
			end
		end
	end
	--]]
