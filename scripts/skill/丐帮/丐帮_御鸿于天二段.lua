---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/丐帮_御鸿于天二段.lua
-- 更新时间:	2015/9/26 18:51:01
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddSlowCheckSelfOwnBuff(9922, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL);	-- 需求自身属于自己的Buff
	--skill.AddSlowCheckDestOwnBuff(9922, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL);	-- 需求目标属于自己的Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--local nlev = player.GetSkillLevel(5638)
	local nbuff_1 = player.GetBuff(9922, 1)
	local nbuff_8 = player.GetBuff(9922, 2)

	if nbuff_1 and nlev ~= 0 then
		
		player.DelGroupBuff(10221, 1)
		player.CastSkill(14930, nlev, player.GetSkillTarget())
		player.DelBuff(9922, 1)
	end

	if nbuff_8 and nlev ~= 0 then
		print(1111)
		player.DelGroupBuff(10221, 1)
		local nTime = nbuff_8.nCustomValue
		print(nTime)
		for i = 1 ,nTime do
			print(player.CastSkill(14931, nlev, player.GetSkillTarget()))
		end
		player.DelBuff(9922, 2)
	end
end
