---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/笑醉狂计数buff回血.lua
-- 更新时间:	2014/10/22 19:51:22
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	if player.GetSkillLevel(6829) == 1 and player.nMoveState ~= MOVE_STATE.ON_DEATH then
		player.nCurrentLife = player.nCurrentLife + player.nMaxLife * 0.08 * nCustomValue
	end
	--]]
	if player.GetSkillLevel(6829) == 1 then
		--player.nCurrentLife = player.nCurrentLife + player.nMaxLife * 0.08 * nCustomValue
		local nTime = 10 - nStackNum
		--print(nTime)
		if nTime > 0 then
			for i = 1, nTime do
				--每次降低8秒调息时间
				player.CastSkill(14624, 1)
