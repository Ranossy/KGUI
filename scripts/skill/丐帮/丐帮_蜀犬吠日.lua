---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/丐帮_蜀犬吠日.lua
-- 更新时间:	2013/9/16 12:55:28
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/丐帮/丐帮_蜀犬吠日回蓝.lua", --奇穴+30%蓝
		0														
	);	
	skill.AddAttribute(
	...
	skill.SetNormalCoolDown(1, 596);	--技能普通CD
...
function OnSkillLevelUp(skill, player)
	if skill.dwLevel == skill.dwMaxLevel then
		player.AcquireAchievement(3442)
	end	
end
...
function Apply(dwCharacterID)
	...
	ModityCDToUI(player, 5257, 0, 0)
end

--魔法属性反应用时的执行函数,dwCharacterID是魔法属性作用的目标ID
function UnApply(dwCharacterID)
	local player = GetPlayer(dwCharacterID)
	local dwSkillLevel = player.GetSkillLevel(5257)   --获得母技能 蜀犬吠日 的等级
	player.CastSkill(5351, dwSkillLevel)
end
