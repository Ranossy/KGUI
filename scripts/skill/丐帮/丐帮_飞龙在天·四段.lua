...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	  skill.AddSlowCheckSelfBuff(5992, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	skill.SetDelaySubSkill(1, 6604, dwSkillLevel)	
	skill.SetDelaySubSkill(3, 6605, dwSkillLevel)	
	skill.SetDelaySubSkill(5, 6606, dwSkillLevel)	
	skill.SetDelaySubSkill(7, 6607, dwSkillLevel)
	skill.SetDelaySubSkill(9, 6608, dwSkillLevel)	
	skill.SetDelaySubSkill(12, 6609, dwSkillLevel)		
	skill.SetDelaySubSkill(17, 6610, dwSkillLevel)		
	-----------------技能渐变相关-------------------------------------------
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	--player.AddBuff(0,99,5754,1,20)
	player.TurnToCharacter(target.dwID)
	if target.nImmuneSkillMoveCount == 0 and player.GetBuff(5992, 1) then
		target.TurnToCharacter(player.dwID)
		--player.AdjustPlayerDistance(target.dwID,72)
	end	
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	if target.nImmuneSkillMoveCount == 0 then
		target.BeHitted(player.dwID, 5605, 1, 36)
	end
end
