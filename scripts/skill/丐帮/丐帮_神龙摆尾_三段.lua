---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/丐帮_神龙摆尾_三段.lua
-- 更新时间:	2013/6/8 12:01:38
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(5986, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);		-- 需求自身Buff
	...
	skill.SetDelaySubSkill(8, 6362, dwSkillLevel)
	skill.SetDelaySubSkill(19, 6363, dwSkillLevel)
	skill.SetDelaySubSkill(27, 6364, dwSkillLevel)	
	-----------------技能渐变相关-------------------------------------------
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 6381, 1);		-- 设置1号位Buff
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if target.nImmuneSkillMoveCount == 0 and player.GetBuff(5986, 1) then
		target.TurnToCharacter(player.dwID)
		--player.AdjustPlayerDistance(target.dwID,83)
	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	local buff = player.GetBuff(6381, 1)
	local n
	if buff then
		n = buff.nStackNum
	end
	if player.GetSkillLevel(6831) ~= 1 then
		if not buff or n < 4 then
			player.AddBuff(dwCharacterID, player.nLevel, 6381, 1, 1)
		end
	else
		if not buff or n < 7 then
			player.AddBuff(dwCharacterID, player.nLevel, 6381, 1, 1)
			player.AddBuff(dwCharacterID, player.nLevel, 6381, 1, 1)
		end
	end
	if target.nImmuneSkillMoveCount == 0 then
		target.BeHitted(player.dwID, 5369, 1, 39)
	end

	--叠加破防
	if player.GetSkillLevel(6831) == 1 then
		local nbuff = player.GetBuff(9920, 1)
		local nbuffcount = 0
		if nbuff then
			nbuffcount = nbuff.nStackNum
		end

		if not nbuff or (nbuff and nbuffcount < 3) then
			player.AddBuff(player.dwID, player.nLevel, 9920, 1)
		end
	end
end
