---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/酒中仙.lua
-- 更新时间:	2017/3/20 14:08:01
-- 更新用户:	chenchen6-pc
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.SetNormalCoolDown(1, 605);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if nPreResult == SKILL_RESULT_CODE.SUCCESS and  player.GetSkillLevel(15211) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 10258, 1)
	end
	if player.GetSkillLevel(6846) == 1 then
		player.AddBuff(0, 99, 6369, 1)
	end
	return nPreResult;
end
...
function Apply(dwCharacterID)
	...
	local i = 10
	local run_levl = 1
	if player.IsSkillRecipeActive(1608, 1) then
		i = i + 1
		run_levl = run_levl + 1
	end
	if player.IsSkillRecipeActive(1609, 1) then
		i = i + 1
		run_levl = run_levl + 1
	end
	if player.GetSkillLevel(14625) == 1 then
		i = i + 6
	end

	--小队添加会心效果
	if player.IsInParty() then
		player.CastSkill(8564, 1)
	end

	--自身添加buff效果
	player.AddBuff(dwCharacterID, player.nLevel, 5994, 1, i)

	--奇穴额外提升会效
	if player.GetSkillLevel(14625) == 1 then
		player.AddBuff(dwCharacterID, player.nLevel, 9719, 1, i)
	end

	if player.GetSkillLevel(15211) == 1 then
		player.AddBuff(dwCharacterID, player.nLevel, 10258, 1, 1)
	end
	--每秒10%内力
	player.nCurrentMana = player.nCurrentMana + player.nMaxMana * 0.1

	--移动速度提高10%
	if player.IsSkillRecipeActive(1607, 1) then
		player.AddBuff(dwCharacterID, player.nLevel, 6581, run_levl, 1)
	end

	--计数buff
	player.AddBuff(dwCharacterID, player.nLevel, 6435, 1, 1)    --读完5跳的标记
	local buff = player.GetBuff(6435, 1)
	if not buff then
		return
	end
	if buff.nStackNum == 5 then
		player.AddBuff(dwCharacterID, player.nLevel, 6433, 1, 1)    --读完5跳放醉逍遥BUFF技能
		if player.GetSkillLevel(6840) == 1 then
			player.nCurrentMana = player.nCurrentMana + player.nMaxMana * 0.5
		end
		if player.GetSkillLevel(14628) == 1 then
