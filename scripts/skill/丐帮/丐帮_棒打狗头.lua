---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/丐帮_棒打狗头.lua
-- 更新时间:	2013/5/22 16:32:59
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
	...
	ModityCDToUI(player, 5259, 0, 0)
end
--魔法属性反应用时的执行函数,dwCharacterID是魔法属性作用的目标ID
function UnApply(dwCharacterID)
	local player = GetPlayer(dwCharacterID)
	local dwSkillLevel = player.GetSkillLevel(5259)   --获得母技能 蜀犬吠日 的等级
	if player.GetBuff(8675, 1) then
		local nbuff = player.GetBuff(8675, 1)
		...
		if nID ~= 0 then
			if IsPlayer(nID) then
				...
				if targetN then
					...
					player.CastSkill(5356, dwSkillLevel,TARGET.PLAYER,targetN.dwID)				
				end
			else
				targetN = GetNpc(nID)
				if targetN then
					player.SetTarget(TARGET.NPC,targetN.dwID)
					player.CastSkill(5356, dwSkillLevel,TARGET.NPC,targetN.dwID)				
				end
			end
			if not targetN then
				player.CastSkill(5356, dwSkillLevel)
			end
		else
			player.CastSkill(5356, dwSkillLevel)
		end
		player.DelBuff(8675,1)
	else
		player.CastSkill(5356, dwSkillLevel)
	end
end
