---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/丐帮/丐帮_烟雨行·前复制用于母技能.lua
-- 更新时间:	2017/3/22 10:33:47
-- 更新用户:	chenchen6-pc
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	skill.BindBuff(1, 6459, 1);		-- 设置1号位Buff
	...
	--skill.SetNormalCoolDown(1, 606);	--技能普通CD
	skill.SetNormalCoolDown(2, 88);	--技能普通CD
	--skill.SetCheckCoolDown(3, 621);	--只检查不走的CD
	----------------- 经验升级相关 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	if player.GetSkillLevel(9295) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 7979, 1)
	end

	--重置棒打狗头调息时间
	if player.GetSkillLevel(14631) == 1 then
		player.ClearCDTime(597)
		ModityCDToUI(player, 5259, 0, 0)
	end
	--ModityCDToUI(player, 5259, 0, 0)
	player.SetTimer(1, GetEditorString(11, 4307), 0, 0)
end
