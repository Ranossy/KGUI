---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/纯阳决_化三清.lua
-- 更新时间:	2013/4/22 21:05:21
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData = 
{
	{nDamageBase = 0, nDamageRand = 0, nCostMana = 176}, --level 1
};
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/纯阳/纯阳决_化三清.lua", -- 属性值1
		0															-- 属性值2
	);
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK, -- 属性作用模式
		ATTRIBUTE_TYPE.EXECUTE_SCRIPT, -- 魔法属性
		"skill/纯阳/90级奇穴/气场通用瞬发.lua", -- 属性值1
		0															-- 属性值2
	);
    --[[
    skill.AddAttribute(
	...
	--skill.nCostMana      	= tSkillData[dwSkillLevel].nCostMana * 0.7;					-- 技能消耗的内力
...
function Apply(dwCharacterID)
	...
	if player.GetSkillLevel(14829) == 1 or player.GetSkillLevel(14834) == 1 then
		npc = scene.CreateGas(
			dwCharacterID, 3080, 576, 3574,
			614, 1, 0, 3140, 1, 0, true,
			617, 1, 0, 3141, 1, 0, true,
			160, 1, 24, 0, 0, 0, false,
			9956, 1, 0, 0, 0, 0, false,
			455, 1, false,
			322, 1, false,
			0, 0, false,
		14992, 1, false)
	...
	if npc then
		...
		if player.GetSkillLevel(9352) == 1 and nSkill ~= nil and nSkill.dwSkillID == 10015 then
			if player.GetSkillLevel(14829) == 1 or player.GetSkillLevel(14834) == 1 then
				npc.AddBuff(player.dwID, player.nLevel, 8097, 1, 12)
			else
				npc.AddBuff(player.dwID, player.nLevel, 8097, 1, 8)
			end
		end
		player.SetTimer(45, "scripts/skill/纯阳/纯阳决_化三清.lua", npc.dwID, 0)
	end
end
