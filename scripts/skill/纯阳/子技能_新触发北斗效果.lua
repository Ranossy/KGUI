---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/子技能_新触发北斗效果.lua
-- 更新时间:	2015/10/1 14:39:05
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--print(222222)
	local nbuff_1 = player.GetBuff(9968, 1)

	if nbuff_1 then
		--print(3333)
		local castID = nbuff_1.dwSkillSrcID
		local Caster = GetPlayer(castID)
		if not Caster then
			return
		end

		if Caster.GetSkillLevel(5833) == 1 then
			player.AddBuff(Caster.dwID, Caster.nLevel, 2755, 2, 1)
			player.DelBuff(9959, 3)
			player.DelBuff(9959, 4)
			player.DelBuff(9968, 1)
		end
	end
end
