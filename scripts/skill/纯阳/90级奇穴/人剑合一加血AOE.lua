---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/90级奇穴/人剑合一加血AOE.lua
-- 更新时间:	2013/4/18 18:58:22
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/纯阳/90级奇穴/人剑合一加血AOE.lua", -- 连协技
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(5806) == 1 then
		if player.IsInMyGroupOrTeam(dwCharacterID) then
			if not target.GetBuff(7913, 1) and target.dwShapeShiftID == 0 then
				target.nCurrentLife = target.nCurrentLife + target.nMaxLife * 0.01
