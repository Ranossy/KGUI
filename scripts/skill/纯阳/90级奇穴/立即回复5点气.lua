---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/90级奇穴/立即回复5点气.lua
-- 更新时间:	2013/6/6 5:42:14
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	if dwSkillLevel == 2 then
		skill.AddSlowCheckSelfBuff(6341, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	end
	...
	----------------- BUFF相关 -------------------------------------------------
	if dwSkillLevel == 1 then
		--skill.BindBuff(1, 5648, 1);		-- 设置1号位Buff
	elseif dwSkillLevel == 2 then
		skill.BindBuff(1, 6341, 1);	
	end
...
function Apply(dwCharacterID)
	...
	--local buff = player.GetBuff(5648, 1) 
	if player.GetSkillLevel(5799) == 1 then
		--if not buff then
			if i <= 25 then
				player.AddBuff(dwCharacterID, player.nLevel, 5868, 1, 1)
			end
		--end
	end
	--buff.nCustomValue
