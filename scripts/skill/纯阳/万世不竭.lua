---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/万世不竭.lua
-- 更新时间:	2013/12/13 15:08:06
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData = 
{
	{nDamageBase = 867 * 0.95, nDamageRand = 867 * 0.1, nCostMana = 0}, --level 1
	{nDamageBase = 137 * 0.95, nDamageRand = 137 * 0.1, nCostMana = 0}, --level 2
	{nDamageBase = 178 * 0.95, nDamageRand = 178 * 0.1, nCostMana = 0}, --level 3
	{nDamageBase = 219 * 0.95, nDamageRand = 219 * 0.1, nCostMana = 0}, --level 4
	{nDamageBase = 260 * 0.95, nDamageRand = 260 * 0.1, nCostMana = 0}, --level 5
	{nDamageBase = 301 * 0.95, nDamageRand = 301 * 0.1, nCostMana = 0}, --level 6
	{nDamageBase = 342 * 0.95, nDamageRand = 342 * 0.1, nCostMana = 0}, --level 7
	{nDamageBase = 383 * 0.95, nDamageRand = 383 * 0.1, nCostMana = 0}, --level 8
...
function GetSkillLevelData(skill)
	...
	--);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(6422, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL);	
	...
	skill.SetNormalCoolDown(1, 623);	--技能普通CD
	skill.SetCheckCoolDown(1, 444);	--只检查不走的CD
	----------------- 经验升级相关 ---------------------------------------------
	skill.nCostManaBasePercent = 410*0.85;	-- 技能消耗的内力
	...
	skill.nNeedAccumulateCount = 10;				-- 技能需要气点的个数，当skill.bIsAccumulate	= true时生效
	...
	--skill.nChannelInterval = 100 + (dwSkillLevel - 9) * 6
	--else
	--skill.nChannelInterval = 220;     -- 通道技间隔时间   
	--end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(6425,1) then
		player.DelBuff(6425, 1)
	...
	if IsPlayer(dwCharacterID) then
		...
		player.CastSkill(8560, 1, TARGET.PLAYER, dwCharacterID)
		player.CastSkill(8560, 2, TARGET.PLAYER, dwCharacterID)
		player.CastSkill(8560, 3, TARGET.PLAYER, dwCharacterID)
		player.CastSkill(8560, 4, TARGET.PLAYER, dwCharacterID)
	else
		--target = GetNpc(dwCharacterID)
		player.CastSkill(8560, 1, TARGET.NPC, dwCharacterID)
		player.CastSkill(8560, 2, TARGET.NPC, dwCharacterID)
		player.CastSkill(8560, 3, TARGET.NPC, dwCharacterID)
		player.CastSkill(8560, 4, TARGET.NPC, dwCharacterID)
		--local target= player.GetSelectCharacter()
	end
	--local tID = target.dwID
	if player.GetSkillLevel(6905) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 10612, 1)
	end

	if player.GetSkillLevel(6908) == 1 then
		if IsPlayer(dwCharacterID) then
			player.CastSkill(6909, 1, TARGET.PLAYER, dwCharacterID)
		else
			player.CastSkill(6909, 1, TARGET.NPC, dwCharacterID)
		end
	end
end
