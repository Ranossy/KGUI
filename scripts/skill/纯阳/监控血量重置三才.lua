---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/监控血量重置三才.lua
-- 更新时间:	2015/9/30 22:36:44
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.nCurrentLife <= player.nMaxLife * 0.4 and not player.GetBuff(9961, 1) and player.nMoveState ~= MOVE_STATE.ON_DEATH and player.bFightState == true then
		local nLeftFrame = player.GetCDLeft(160)
		if nLeftFrame ~= 0 and not player.GetBuff(9961,1) then
			player.ClearCDTime(160)
			player.AddBuff(player.dwID,player.nLevel,9961,1)
		end
	end
end
