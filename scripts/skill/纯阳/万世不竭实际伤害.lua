---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/万世不竭实际伤害.lua
-- 更新时间:	2016/4/11 20:47:41
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		tSkillData[dwSkillLevel].nDamageBase * (1 + 0.3 * (dwSkillLevel - 1)) / 4,
		0
	);
	...
	skill.AddAttribute(
		...
		tSkillData[dwSkillLevel].nDamageRand * (1 + 0.3 * (dwSkillLevel - 1)) / 4,
		0
	);
	...
	skill.AddSlowCheckSelfOwnBuff(10612, dwSkillLevel - 1, BUFF_COMPARE_FLAG.EQUAL, nLevel, BUFF_COMPARE_FLAG.EQUAL);	-- 需求自身属于自己的Buff
	--skill.AddSlowCheckDestOwnBuff(6424, dwSkillLevel-1, BUFF_COMPARE_FLAG.EQUAL, nLevel, BUFF_COMPARE_FLAG.EQUAL);	-- 需求目标属于自己的Buff
	...
	----------------- 经验升级相关 ---------------------------------------------
	--skill.nCostManaBasePercent = 680;	-- 技能消耗的内力
	...
	--else
	skill.nChannelInterval = 260*0.9*(1+0.3*(dwSkillLevel-1))*1.15-- 通道技间隔时间
	--end
