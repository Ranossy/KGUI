---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/镇派/附魔_六合独尊AOE与单体转换.lua
-- 更新时间:	2014/7/2 14:27:14
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/纯阳/镇派/附魔_六合独尊AOE与单体转换.lua",											-- 属性值1
   		0											-- 属性值2
		);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID)
	...
	local lv = player.GetSkillLevel(368)
	if lv ~= 0 then
		player.ForgetSkill(368)
		player.LearnSkillLevel(9335, lv, dwCharacterID)
		RemoteCallToClient(player.dwID, "OnActionBarSkillReplace", 368, 9335, lv)
	end
end
...
function UnApply(dwCharacterID)
	...
	local lv = player.GetSkillLevel(9335)
	if lv ~= 0 then
		player.ForgetSkill(9335)
		player.LearnSkillLevel(368, lv, dwCharacterID)
		RemoteCallToClient(player.dwID, "OnActionBarSkillReplace", 9335, 368, lv)
	end
end
