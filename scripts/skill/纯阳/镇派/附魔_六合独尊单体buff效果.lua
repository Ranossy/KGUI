---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/镇派/附魔_六合独尊单体buff效果.lua
-- 更新时间:	2014/7/2 15:33:09
-- 更新用户:	mengxiangfei
-- 脚本说明:
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/纯阳/镇派/附魔_六合独尊单体buff效果.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID)
	...
	if lv ~= 0 then
		...
		player.LearnSkillLevel(9341, lv, dwCharacterID)
		RemoteCallToClient(player.dwID, "OnActionBarSkillReplace", 368, 9341, lv)
	end
end
...
function UnApply(dwCharacterID)
	...
	local lv = player.GetSkillLevel(9341)
	if lv ~= 0 then
		player.ForgetSkill(9341)
		player.LearnSkillLevel(368, lv, dwCharacterID)
		RemoteCallToClient(player.dwID, "OnActionBarSkillReplace", 9341, 368, lv)
	end
end
