---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/玄门_每秒执行.lua
-- 更新时间:	2017/4/12 17:07:16
-- 更新用户:	xueyan3
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not Is_pre then
		npc.CastSkill(17583, 1)
		player.CastSkill(17585, 1)
	end

	--print("dwConvexHullID = " .. npc.dwConvexHullID)

	local num = npc.GetCustomUnsigned1(0)
	if num > 0 then
		local npcjh
		local npcjhnextid
		local npcjhnext
		for i = 1, num do
			npcjh = GetNpc(npc.GetCustomUnsigned4(4 * i - 3))
			npcjhnextid = npc.GetCustomUnsigned4(4 * i + 1)
			npcjhnext = GetNpc(npcjhnextid)
			if npcjh and npcjhnext then
			--	print("nomal")
				npcjh.CastSkill(17574, 1, TARGET.NPC, npcjhnextid)
			elseif npcjh and npc.GetCustomUnsigned4(1) ~= 0 and npcjh.dwID ~= npc.GetCustomUnsigned4(1) then
				npcjh.CastSkill(17574, 1, TARGET.NPC, npc.GetCustomUnsigned4(1))
			else
				local employerplayer = npc.GetEmployer()
				if not employerplayer then
					return
				end
				...
				scene.RemoveConvexHull(npc.dwConvexHullID)
