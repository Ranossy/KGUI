---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/玄门_增益.lua
-- 更新时间:	2017/4/11 19:18:42
-- 更新用户:	xueyan3
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 11884, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not scene.IsInConvexHull(buff.nCustomValue, playertarget.nX, playertarget.nY) then
		return
	end
	
	if playertarget.GetSkillLevel(17586) == 1 then
		playertarget.AddBuff(player.dwID, player.nLevel, 11884, 1)
	end
end
