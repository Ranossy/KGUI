---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/子技能_七星拱瑞被击事件.lua
-- 更新时间:	2015/9/30 22:11:08
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/纯阳/子技能_七星拱瑞被击事件.lua",											-- 属性值1
   		0											-- 属性值2
   		);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local player
	for i = 1, 4 do
		local nbuff = target.GetBuff(9959, i)
		if nbuff then
			local castID = nbuff.dwSkillSrcID
			if IsPlayer(castID) then
				player = GetPlayer(castID)
			else
				player = GetNpc(castID)
			end
			print(i)
			print(castID)
			if not player then
			...
			--触发北斗
			if player.GetSkillLevel(5833) == 1 then
				print(33333)
				target.AddBuff(player.dwID, player.nLevel, 2755, 2)
			end

			target.DelBuff(9959, i)
			break
		end
	end
end
