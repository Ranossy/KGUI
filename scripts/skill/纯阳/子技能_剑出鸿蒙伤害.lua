---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/子技能_剑出鸿蒙伤害.lua
-- 更新时间:	2015/10/24 18:52:54
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nAddRage = 0, nCostRage = 0, nDamageBase = 100, nDamageRand = 8}, -- Level 1
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	for i = 1, 6 do
		if player.GetBuff(10207, i) then
			if  i == 6 then
				player.DelBuff(10207, i)
			else
				--player.DelBuff(10207, i)
				player.AddBuff(player.dwID, player.nLevel, 10207, i + 1)
				if  i == 5 then
					player.DelBuff(10207, i + 1)
				end
				--player.SetTimer(6, "scripts/skill/纯阳/子技能_剑出鸿蒙伤害.lua", i, 0)
			end
			break
		end
	end
...
function OnTimer(player, nParam1, nParam2)
	...
	player.AddBuff(player.dwID, player.nLevel, 10207, nParam1 + 1)
end
