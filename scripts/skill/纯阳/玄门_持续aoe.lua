---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/纯阳/玄门_持续aoe.lua
-- 更新时间:	2017/4/11 18:35:03
-- 更新用户:	xueyan3
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/纯阳/玄门_持续aoe.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);		-- 需求自身Buff
	skill.AddSlowCheckDestBuff(11885, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if lv ~= 0 and IsEnemy(player.dwID, dwCharacterID) then
		if IsPlayer(dwCharacterID) then
			player.CastSkill(17584, lv, TARGET.PLAYER, dwCharacterID)
			target.AddBuff(player.dwID,player.nLevel,11885,1)
		else
			player.CastSkill(17584, lv, TARGET.NPC, dwCharacterID)
		end
	end
end
