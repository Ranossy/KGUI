...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff_jiwei = player.GetBuff(8273, 1)
	if buff_jiwei then
		local HP_old 
		if buff_jiwei.nCustomValue > 1 then
			HP_old = buff_jiwei.nCustomValue / 100
		else
			HP_old = buff_jiwei.nCustomValue
		end
		local HP_new = player.nCurrentLife / player.nMaxLife
		--if HP_old ~= 0 then
		if HP_old - HP_new >= 0.35 then
		...
		--end
		buff_jiwei.nCustomValue = HP_new * 100
	end
end
