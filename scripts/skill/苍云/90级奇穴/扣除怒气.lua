...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.bFightState and not player.IsHaveBuff(8664, 1) then
		if player.nCurrentRage <= 1 then
		...
		if player.nMoveState ~= MOVE_STATE.ON_DEATH then
			if player.nCurrentEnergy < player.nMaxEnergy - 2 then
				player.nCurrentEnergy = player.nCurrentEnergy + 2
			else
	...
	if nSkill ~= nil and nSkill == 10389 then
		if not player.IsHaveBuff(8481, 1) and (player.nCurrentRage >= 10 or player.IsHaveBuff(10087, 1)) then
			player.AddBuff(player.dwID, player.nLevel, 8481, 1, 100)
		end

		if player.IsHaveBuff(8481, 1) and player.nCurrentRage < 10 and not player.IsHaveBuff(10087, 1) then
			player.DelBuff(8481, 1)
		end
	end
	...
	if player.nPoseState == 2 then
		if not player.IsHaveBuff(8571,1) then
			player.AddBuff(player.dwID,player.nLevel,8571,1)
		end
	else
		if player.IsHaveBuff(8571, 1) then
