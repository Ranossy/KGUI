...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--有星火低血量触发回血奇穴
	if player.GetSkillLevel(13129) == 1 then
		if player.nCurrentLife < player.nMaxLife * 0.3 and not player.GetBuff(8269, 1) and player.nMoveState ~= MOVE_STATE.ON_DEATH then
			--添加回血buff
			player.AddBuff(player.dwID, player.nLevel, 8269, 1)
		end

		if player.nCurrentLife >= player.nMaxLife * 0.3 or player.bFightState == false then
			--删除回血buff
			if player.GetBuff(8269, 1) then
				player.DelBuff(8269, 1)
			end
		end
	end

	--有嗜血低血量触发回血奇穴
	if player.GetSkillLevel(13163) == 1 then
		if player.nCurrentLife < player.nMaxLife * 0.3 and not player.GetBuff(8285, 1) and not player.GetBuff(7913, 1) and player.nMoveState ~= MOVE_STATE.ON_DEATH and player.bFightState == true then
			--添加回血buff加强版本
			player.AddBuff(player.dwID, player.nLevel, 8285, 1)
		end

		if player.nCurrentLife >= player.nMaxLife * 0.3 or player.bFightState == false then
			--删除回血buff加强版
			if player.GetBuff(8285, 1) then
				player.DelBuff(8285, 1)
			end
		end
	end

	--千险奇穴低血量触发解控免控效果
	if player.GetSkillLevel(13172) == 1 then
		--施放解控技能并添加免控效果
		if player.nCurrentLife < player.nMaxLife * 0.3 and not player.GetBuff(8293, 1) and player.nMoveState ~= MOVE_STATE.ON_DEATH and player.bFightState == true then
			--对自身施放解控技能
			player.CastSkill(13173, 1)
			--添加免控buff
			player.AddBuff(player.dwID, player.nLevel, 8293, 1)
		end

		--删除免控buff
		if player.nCurrentLife >= player.nMaxLife * 0.3 or player.bFightState == false then
			if player.GetBuff(8293, 1) then
				player.DelBuff(8293, 1)
			end
		end
	end
end
