...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local CSnum = 0
	local Num = 0
	local XSLbuff = player.GetBuff(8470, 1) --记录吸收量的BUFF
	local CSbuff = player.GetBuff(8471, 1)  --记录击中了多少个人
	
	if XSLbuff then
		XSLnum = XSLbuff.nCustomValue * 0.5  --吸收量
	end
	if CSbuff then
		CSnum = CSbuff.nStackNum   --击中人数
	end
	if XSLnum > 1000 then
		Num = math.floor((XSLnum - 1000) / (1000 * 0.2)) + 1
	else
		Num = 1
	end
	local NumZ = Num + CSnum
	if NumZ > 126 then
		NumZ = 126
	elseif NumZ == 0 then
		NumZ = 1
	end
	player.CastSkill(13411, NumZ)  --目前最大为126级，现在伤害大概写到
	--print(111111)
	player.DelBuff(8287, 1)
end
