---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_风雷引_立盾.lua
-- 更新时间:	2014/9/22 19:55:56
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 8303, 1);		-- 设置1号位Buff
	...
	----------------- 设置Cool down --------------------------------------------
	-- 公共CD
	skill.SetPublicCoolDown(836);							-- 公共CD 1.5秒
	-- 技能CD, CoolDownIndex为CD位(共3个), nCoolDownID为CoolDownList.tab内的CDID
	skill.SetNormalCoolDown(1, 810);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.GetBuff(8265, 1) then  --不在盾墙姿态下为2秒
		player.AddBuff(0, 99, 8303, 1, 2)
		player.AddBuff(0, 99, 8674, 1, 2) --盾立发招音效
	else
		player.AddBuff(0, 99, 8303, 1, 3)  --盾墙下用盾立为3秒
		player.AddBuff(0, 99, 8674, 1, 2) --盾立发招音效
	end
	...
	--if player.GetSkillLevel(13073) == 1 and lv ~= 0 and (player.GetBuff(8244, lv) or player.GetBuff(8245, lv)) then
		--if player.nCurrentLife <= player.nMaxLife * nRageCost * 0.9985 then
			--player.nCurrentLife = player.nCurrentLife + player.nMaxLife * nRageCost * 0.0015
		--else
			--player.nCurrentLife = player.nMaxLife
		--end
	--end

	ModityCDToUI(player, 13067, 0, 0)
end
