---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_苍雪刀_血刀.lua
-- 更新时间:	2014/9/17 10:05:09
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_苍雪刀_血刀.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(8265, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 盾墙
	skill.AddSlowCheckSelfBuff(8274, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 血刀需求
...
--	skill.AddSlowCheckDestOwnBuff(8249, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL,);	-- 需求目标属于自己的Buff
	...
	skill.SetNormalCoolDown(1, 836);	--隐刀0.5秒
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local lvZD = player.GetSkillLevel(13054)  --斩刀的等级
	if lvZD >= 1 then
		if nTarget.GetBuffByOwner(8249, lvZD, player.dwID) then
			local LXbuff = nTarget.GetBuffByOwner(8249, lvZD, player.dwID)  --流血BUFF
			if LXbuff.nCustomValue == 1 then -- 当前的流血为强流血，刷新标记位
				player.AddBuff(0, 99, 8390, 1, 1) --用于标记当前攻击的目标是有强流血的
			end
			player.CastSkill(13143, 1) --放溅射
		else
			player.CastSkill(13144, 1) --放伤害
		end
	end

	local XDBJbuff = player.GetBuff(8274, 1)
	if XDBJbuff then
		XDBJbuff.nCustomValue = XDBJbuff.nCustomValue + 1
		if XDBJbuff.nCustomValue == 2 then
			player.DelBuff(8274, 1)
		end
	end
-----------------------------------------------------血怒回血
