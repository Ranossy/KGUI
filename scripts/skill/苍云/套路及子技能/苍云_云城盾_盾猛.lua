---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_云城盾_盾猛.lua
-- 更新时间:	2014/9/14 15:09:49
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_云城盾_盾猛.lua", -- 属性值1
		0											-- 属性值2
	);
	--回复怒气
	skill.AddAttribute(
	...
	skill.AddSlowCheckSelfBuff(8873, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	skill.SetNormalCoolDown(1, 803);	--技能普通CD
...
function OnSkillLevelUp(skill, player)
	if skill.dwLevel == skill.dwMaxLevel then
		player.AcquireAchievement(4306)
	end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--防御奇穴盾猛获得的岌危效果
	if player.GetSkillLevel(13141) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 8273, 1)
	end
	--防御奇穴盾猛获得的激昂效果
	if player.GetSkillLevel(13356) == 1 then
		player.AddBuff(player.dwID, player.nLevel, 8418, 1, 6)
	end

	--点了奇穴13103锋鸣后就没冲刺及后续的效果了
	if player.GetSkillLevel(13103) ~= 1 then
		player.CastSkill(13269, 1, player.GetSkillTarget())  --盾猛冲刺
	end
end
