...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local BJbuff = player.GetBuff(8470, 1)
	local t = player.GetAbsorbInfo()
	for k, v in ipairs(t) do
		--if v.BuffID == 8279 and v.level == 1 then
			BJbuff.nCustomValue = v.nAbsorbUsedValue  --普通版吸收60%
		--elseif v.BuffID == 8425 and v.level == 1 then
			--BJbuff.nCustomValue = v.nAbsorbUsedValue  --加强版吸收100%
		--end
	end
end
