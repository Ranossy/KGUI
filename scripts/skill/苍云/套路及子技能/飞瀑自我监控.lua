---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/飞瀑自我监控.lua
-- 更新时间:	2014/11/26 16:11:13
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if IsPlayer(dwCharacterID) then
		self = GetPlayer(dwCharacterID)
	else
		self = GetNpc(dwCharacterID)
	end

	local owner = GetPlayer(dwSkillSrcID)
	if owner then
		if owner.nPoseState == 2 then
			self.DelGroupBuff(8678, 1)
		end
	end
end
