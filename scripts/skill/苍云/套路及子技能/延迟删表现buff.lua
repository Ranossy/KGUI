---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/延迟删表现buff.lua
-- 更新时间:	2015/11/8 14:11:18
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	if nbuffCount ~= nbuff_markCount then
		...
		elseif nbuffCount > nbuff_markCount then
			...
			for i = 1, nChange do
				player.AddBuff(player.dwID, player.nLevel, 8487, 1)
			end
		elseif nbuffCount < nbuff_markCount then
			local nChange = nbuff_markCount - nbuffCount
			for i = 1, nChange do
				player.DelBuff(8487, 1)
			end
		end
	end
end
