---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_苍雪刀_盾飞弹射子技能.lua
-- 更新时间:	2014/10/16 21:56:52
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(  --防御奇穴当关
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK,
		ATTRIBUTE_TYPE.CAST_SKILL_TARGET_DST,
		13369,
		dwSkillLevel
	);
	skill.AddAttribute(  --添加虚弱
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK,
		ATTRIBUTE_TYPE.CAST_SKILL_TARGET_DST,
		13540,
		dwSkillLevel
	);
	skill.AddAttribute(  --降低目标输出
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK,
		ATTRIBUTE_TYPE.CAST_SKILL_TARGET_DST,
		13541,
		dwSkillLevel
	);
	skill.AddAttribute(  --难行减速
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK,
		ATTRIBUTE_TYPE.CAST_SKILL_TARGET_DST,
		13542,
		dwSkillLevel
		);
	skill.AddAttribute(  --飞瀑回怒气
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_NOT_ROLLBACK,
		ATTRIBUTE_TYPE.CAST_SKILL_TARGET_DST,
		13578,
		dwSkillLevel
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_苍雪刀_盾飞弹射子技能.lua",
		0
	);
	
	skill.AddAttribute( --攻击系统无敌目标时候的处理
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK,
		ATTRIBUTE_TYPE.EXECUTE_SCRIPT,
		"skill/苍云/套路及子技能/盾飞切姿态.lua",
		0
	);
	...
	elseif dwSkillLevel < 18 then
		skill.nChannelInterval = 16 + (dwSkillLevel - 4) * 1;
	else
		skill.nChannelInterval = 35;     -- 通道技间隔时间
	end
	...
	skill.nBulletVelocity = 100;				-- 子弹速度，单位 点/帧
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nTarget
	if IsPlayer(dwCharacterID) then  --可以拆
	...
	--if IsPlayer(dwCharacterID) then  --可以拆
	--nTarget = GetPlayer(dwCharacterID)
	--if player.GetSkillLevel(13368) == 1 then --防御奇穴，当关，强制目标攻击自身3秒
	--if not nTarget.GetBuff(8430, 1) then
	--player.CastSkill(13369, lvDF, TARGET.PLAYER, nTarget.dwID)  --盾飞嘲讽技能
	--end
	--end
	--else
	--nTarget = GetNpc(dwCharacterID)
	--if player.GetSkillLevel(13368) == 1 then --防御奇穴，当关，强制目标攻击自身3秒
	--if not nTarget.GetBuff(8430, 1) then
	--player.CastSkill(13369, lvDF, TARGET.NPC, nTarget.dwID) --盾飞嘲讽技能
	--end
	--end
	--end

	--local lvZD = player.GetSkillLevel(13054) --可以拆
	--if not nTarget.GetBuffByOwner(8249, lvZD, player.dwID) then
	--nTarget.AddBuff(player.dwID, player.nLevel, 8248, 1, 1) --目标没自己的流血，则加虚弱
	--end

	--if player.GetSkillLevel(13320) == 1 then --可以拆
	--nTarget.AddBuff(player.dwID, player.nLevel, 8397, 1, 15) -- 降低目标输出
	--end

	--if player.GetSkillLevel(13374) == 1 then  --盾飞减速   --可以拆
	--if not nTarget.GetBuff(8493, 1) then  --内置CD
	--if not nTarget.GetBuff(8492, 1) then
	--nTarget.AddBuff(player.dwID, player.nLevel, 8492, 1, 6) -- 目标减速60%
	--nTarget.AddBuff(player.dwID, player.nLevel, 8493, 1, 60)--内置CD
	--end
	--end	
	--end

	if player.IsSkillRecipeActive(1959,1) then
		nTarget.AddBuff(player.dwID, player.nLevel, 8394, 1, 1)
	end
end
