---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_云城盾_杨旌沙场.lua
-- 更新时间:	2017/4/14 18:26:51
-- 更新用户:	chenchen6-pc
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddSlowCheckDestBuff(10215, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	skill.AddSlowCheckDestBuff(10212, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求目标Buff
	...
	-----------------技能施放姿态需求(苍云)----------------------------------------
	skill.nNeedPoseState = 2    --检查姿态，1为擎刀姿态，2为擎盾姿态
	...
	skill.SetNormalCoolDown(1, 986);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if IsPlayer(target.dwID) then
		target = GetPlayer(target.dwID)
	else
		target = GetNpc(target.dwID)
		return SKILL_RESULT_CODE.FAILED
	end
	

	local KungFu = {10062, 10389, 10448, 10028, 10080, 10002, 10243, 10176}
	local nSkill = target.GetKungfuMount()
	local nReady = 0
	for i = 1, #KungFu do
		if nSkill ~= nil and nSkill.dwSkillID == KungFu[i] then
			nReady = nReady + 1
			break
		end
	end

	if nReady == 1 or (target and (target.dwShapeShiftID ~= 0 or target.GetBuff(4964, 2) or target.GetBuff(7913, 1))) then
		RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_YELLOW", GetEditorString(9, 590), 4)
		return SKILL_RESULT_CODE.FAILED
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
   ...
   --删除笑傲光阴BUFF
	if target.GetBuff(9258, 1) then
		target.DelBuff(9258, 1)
	end
	if player.GetBuff(9258, 1) then
		player.DelBuff(9258, 1)
	end
	--由苍云方进行检测判定
	player.AddBuff(target.dwID, target.nLevel, 10212, 1)
	local nbuff_1 = player.GetBuff(10212, 1)
	if nbuff_1 then
		nbuff_1.nCustomValue = player.nCurrentLife / player.nMaxLife * 100
	end

	player.AddBuff(target.dwID, target.nLevel, 10219, 1)
	target.AddBuff(target.dwID, target.nLevel, 10212, 1)
	local nbuff_2 = player.GetBuff(10219, 1)
	if nbuff_2 then
		nbuff_2.nCustomValue = target.nCurrentLife / target.nMaxLife * 100
	end

	--print("-------------")
	--print(nbuff_1.nCustomValue)
	--print(nbuff_2.nCustomValue)
	--print("---------------------")
	--双方满血
	player.nCurrentLife = player.nMaxLife
	target.nCurrentLife = target.nMaxLife

	target.AddBuff(target.dwID, target.nLevel, 10215, 1)
