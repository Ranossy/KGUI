---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_寒铁诀_血怒.lua
-- 更新时间:	2014/10/15 17:45:44
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_寒铁诀_血怒.lua
-- 更新时间:	2014/9/12 16:55:30
-- 更新用户:	mengxiangfei
-- 脚本说明:
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_寒铁诀_血怒.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	----------------- 设置Cool down --------------------------------------------
	-- 公共CD
	skill.SetPublicCoolDown(815);							-- 公共CD 0.5秒受急速加成
	-- 技能CD, CoolDownIndex为CD位(共3个), nCoolDownID为CoolDownList.tab内的CDID
	skill.SetNormalCoolDown(1, 799);	--技能普通CD
	skill.SetNormalCoolDown(2, 816);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if player.GetSkillLevel(13304) == 1 then  --奇穴愤恨，扣10%血
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.CastSkill(13300, 1) --血怒第1层
	player.CastSkill(13301, 1)--血怒第2层
	player.CastSkill(13302, 1) --血怒第3层

	if player.GetSkillLevel(14848) == 1 then
		player.nCurrentLife = player.nCurrentLife + player.nMaxLife * 0.1
	end
	
	if player.GetSkillLevel(13563) == 1 then  --学习了血影的标记技能
		if not player.GetBuff(8666, 1) then --不同奇穴下的血影时间长短不同
			if player.GetSkillLevel(13304) == 1 then --25秒基础的血怒
				player.PlaySfx(1089, player.nX, player.nY, player.nZ)
				player.CastSkill(13564, 1) --25秒的
			else
				player.PlaySfx(1089, player.nX, player.nY, player.nZ)
				player.CastSkill(13566, 1) --10秒的
			end
		end
	end
	
	ModityCDToUI(player, 13040, 0, 0)
end
