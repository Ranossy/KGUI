...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		tSkillData[dwSkillLevel].nDamageBase*3/5,
		0
	);
	...
	skill.AddAttribute(
		...
		tSkillData[dwSkillLevel].nDamageRand*3/5,
		0
	);
	...
	--skill.AddSlowCheckDestOwnBuff(8249, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL, 1, BUFF_COMPARE_FLAG.GREATER_EQUAL);	-- 需求目标属于自己的Buff
	-----------------技能施放技能需求-------------------------------------------
	--skill.AddCheckSelfLearntSkill(13109, 1, SKILL_COMPARE_FLAG.EQUAL);     --检查比较Caster自己已学习的技能ID和等级
	...
	if dwSkillLevel < 5 then
		skill.nChannelInterval = 16*2.2;
	elseif dwSkillLevel < 16 then
		skill.nChannelInterval = (16 + (dwSkillLevel - 4) * 5)*2.2;
	else
		skill.nChannelInterval = 100*2.2;     -- 通道技间隔时间
	end
	----------------- 阵法相关 -------------------------------------------------
