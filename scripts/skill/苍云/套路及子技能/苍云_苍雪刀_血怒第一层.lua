---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_苍雪刀_血怒第一层.lua
-- 更新时间:	2014/10/15 16:38:17
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_苍雪刀_血怒第一层.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(8381, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);	  	--第一层需求
	skill.AddSlowCheckSelfBuff(8382, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		--第二层需求
	skill.AddSlowCheckSelfBuff(8383, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		--第三层需求
	...
	----------------- 设置Cool down --------------------------------------------
	-- 公共CD
	--skill.SetPublicCoolDown(815);		-- 公共CD 1秒
	-- 技能CD, CoolDownIndex为CD位(共3个), nCoolDownID为CoolDownList.tab内的CDID
	skill.SetNormalCoolDown(1, 817);	--技能普通CD
	...
	skill.nMaxRadius = 1000 * LENGTH_BASE;		-- 技能施放的最大距离
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--血怒免控时间可累加
	if player.GetSkillLevel(13395) == 1 then
		player.AddBuff(player.dwID,player.nLevel,8449,1)
	end
	...
	--血怒延长buff跳数秘籍
	local nNum = 0
	local tRecipeList = {1867, 1868, 1869}
	for i = 1, #tRecipeList do
		if player.IsSkillRecipeActive(tRecipeList[i],1) then
			nNum = nNum + 1
		end
	end
	...
	if nSkill ~= nil then
		if nSkill.dwSkillID == 10390 then	--输出心法
			if player.GetSkillLevel(13304) == 1 then --奇穴愤恨，扣血20%
				if player.nCurrentLife > player.nMaxLife * 0.1 then
					player.nCurrentLife = player.nCurrentLife - player.nMaxLife * 0.1
				else
					player.nCurrentLife = 1
				end
				if player.GetBuff(8384, 1) then
					player.AddBuff(player.dwID, player.nLevel, 8385, 1, 25 + nNum) --添加15%AP效果
				--	player.CastSkill(13343, 1) --额外恢复25点怒气
				else
					player.DelGroupBuff(8385, 1)
					player.AddBuff(player.dwID, player.nLevel, 8385, 1, 25 + nNum)
				--	player.CastSkill(13343, 1) --额外恢复25点怒气
				end
			else
				if  player.GetBuff(8384, 1) then
					player.AddBuff(player.dwID, player.nLevel, 8244, 1, 10 + nNum) --添加10%AP效果	
				else
					player.DelGroupBuff(8244, 1)
					player.AddBuff(player.dwID, player.nLevel, 8244, 1, 10 + nNum) --添加10%AP效果	
				end
			end
			
		elseif nSkill.dwSkillID == 10389 then--防御心法
			if player.GetSkillLevel(13304) == 1 then --奇穴愤恨，扣血20%
				if player.nCurrentLife > player.nMaxLife * 0.1 then
					player.nCurrentLife = player.nCurrentLife - player.nMaxLife * 0.1
				else
				...
				if  player.GetBuff(8384, 1) then
					player.AddBuff(player.dwID, player.nLevel, 8386, 1, 25 + nNum) --防御心法添加75%内外防效果
				--	player.CastSkill(13343, 1) --额外恢复25点怒气
				else
					player.DelGroupBuff(8386, 1)
					player.AddBuff(player.dwID, player.nLevel, 8386, 1, 25 + nNum) --防御心法添加75%内外防效果
					--player.CastSkill(13343, 1) --额外恢复25点怒气
				end
				...
				if  player.GetBuff(8384, 1) then
					player.AddBuff(player.dwID, player.nLevel, 8245, 1, 10 + nNum)--防御心法添加50%内外防效果
				else
					player.DelGroupBuff(8245, 1)
					player.AddBuff(player.dwID, player.nLevel, 8245, 1, 10 + nNum)--防御心法添加50%内外防效果
				end
			end

			--防御奇穴血怒额外提高威胁值和被疗伤效果
			if player.GetSkillLevel(13142) == 1 then
				player.AddBuff(player.dwID, player.nLevel, 8275, 1, 10 + nNum)
			end
		end
	end

	player.AddBuff(0, 99, 8580, 1, 10 + nNum)  --血怒表现
	
	--回血秘籍
	if player.IsSkillRecipeActive(1873, 1) and not player.GetBuff(7913, 1) then
		player.nCurrentLife = player.nCurrentLife + player.nMaxLife * 0.02
	end
	if player.IsSkillRecipeActive(1874, 1) and not player.GetBuff(7913, 1) then
		player.nCurrentLife = player.nCurrentLife + player.nMaxLife * 0.03
	end
	
	player.AddBuff(0, 99, 8382, 1, 1) --第二层需求
	player.AddBuff(0, 99, 8384, 1, 1) --1秒连按判断
end
