...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		tSkillData[dwSkillLevel].nDamageBase * 3/3, -- 属性值1
		0											-- 属性值2
	);
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK, -- 属性作用模式
		ATTRIBUTE_TYPE.SKILL_PHYSICS_DAMAGE_RAND, -- 魔法属性
		tSkillData[dwSkillLevel].nDamageRand * 3/3, -- 属性值1
		0											-- 属性值2
	);
	skill.AddAttribute(
	...
	if dwSkillLevel < 5 then
		skill.nChannelInterval = 40*2.5;
	elseif dwSkillLevel < 25 then
		skill.nChannelInterval = (40 + (dwSkillLevel - 4) * 4)*2.5;
	else
		skill.nChannelInterval = 150*2.5;     -- 通道技间隔时间
	end
	----------------- 阵法相关 -------------------------------------------------
