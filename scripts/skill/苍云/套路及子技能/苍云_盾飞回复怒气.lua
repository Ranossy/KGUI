---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_盾飞回复怒气.lua
-- 更新时间:	2014/11/26 16:03:32
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute( --攻击系统无敌目标时候的处理
		...
		"skill/苍云/套路及子技能/苍云_盾飞回复怒气.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckDestBuff(8492, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);
	--skill.AddSlowCheckDestBuff(8493, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);
	--skill.AddSlowCheckSelfOwnBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);	-- 需求自身属于自己的Buff
	--skill.AddSlowCheckDestOwnBuff(dwBuffID, nStackNum, eCompareFlag, nLevel, eLevelCompareFlag);	-- 需求目标属于自己的Buff

	-----------------技能施放技能需求-------------------------------------------
	skill.AddCheckSelfLearntSkill(13372, 1, SKILL_COMPARE_FLAG.EQUAL);     --检查比较Caster自己已学习的技能ID和等级
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 8492, 1);		-- 减速
	--skill.BindBuff(2, 8493, 1);		-- 监控
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.AddBuff(0, 99, 8678, 1, 80)  --20秒的监控
	local buff = player.GetBuff(8678, 1)  --次数标记BUFF
	if buff then
		if buff.nStackNum < 6 then
			player.nCurrentRage = player.nCurrentRage + 4  --回复2点怒气
		end
	end
end
