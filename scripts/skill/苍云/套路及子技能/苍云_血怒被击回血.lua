---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_血怒被击回血.lua
-- 更新时间:	2015/9/24 23:27:09
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/苍云/套路及子技能/苍云_血怒被击回血.lua",											-- 属性值1
   		0											-- 属性值2
   		);
	...
	-----------------技能施放技能需求-------------------------------------------
	skill.AddCheckSelfLearntSkill(14847, 1, SKILL_COMPARE_FLAG.EQUAL);     --检查比较Caster自己已学习的技能ID和等级
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(14847) == 1 and player.nCurrentLife <= player.nMaxLife * 0.35 and not player.GetBuff(9936, 1) then
		local nbuff_1 = player.GetBuff(8245, 1)
		local nbuff_2 = player.GetBuff(8386, 1)

		if nbuff_1 then
			local nbuffcount = nbuff_1.nStackNum
			if not player.GetBuff(9888, 1) then
				player.nCurrentLife = player.nCurrentLife + player.nMaxLife * 0.02 * nbuffcount
			end
		end

		if nbuff_2 then
			local nbuffcount = nbuff_2.nStackNum
			if not player.GetBuff(9888, 1) then
				player.nCurrentLife = player.nCurrentLife + player.nMaxLife * 0.02 * nbuffcount
			end
		end

		player.AddBuff(player.dwID, player.nLevel, 9936, 1)
	end
end
