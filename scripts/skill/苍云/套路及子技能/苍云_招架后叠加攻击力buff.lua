---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_招架后叠加攻击力buff.lua
-- 更新时间:	2014/10/22 21:08:10
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_招架后叠加攻击力buff.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nNum = player.nParryValue
	local nCeng = math.floor(nNum / 300*0.4)
	
	--内置CD期间不刷新了
	if player.GetBuff(8462, 1) and (player.GetBuff(8271, 1) or player.GetBuff(8437, 1)) then
		return
	end
	
	player.DelGroupBuff(8271,1)
	player.DelBuff(8437, 1)

	if nCeng >= 1 then
		for i = 1, nCeng do
			player.AddBuff(0, 99, 8271, 1, 8)  --8秒叠加层数的AP加成
		end
	end
	player.AddBuff(0, 99, 8437, 1, 8)  --显示用寒甲
	player.AddBuff(0,99,8462,1)	--		2秒内置CDbuff
end
