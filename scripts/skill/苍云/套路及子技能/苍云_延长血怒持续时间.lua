---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_延长血怒持续时间.lua
-- 更新时间:	2015/9/25 11:13:33
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/苍云/套路及子技能/苍云_延长血怒持续时间.lua",											-- 属性值1
   		0											-- 属性值2
   		);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nbuff = player.GetBuff(8244, 1)
	local nbuff_2 = player.GetBuff(8385, 1)

	if nbuff and nbuff.nCustomValue < 3 then
		local nLeftCount = nbuff.nLeftActiveCount
		player.SetBuffLeftActiveCount(nbuff.nIndex, nLeftCount + 2)
		nbuff.nCustomValue = nbuff.nCustomValue + 1
	end

	if nbuff_2 and nbuff_2.nCustomValue < 3 then
		local nLeftCount = nbuff_2.nLeftActiveCount
		player.SetBuffLeftActiveCount(nbuff_2.nIndex, nLeftCount + 2)
		nbuff_2.nCustomValue = nbuff_2.nCustomValue + 1
	end
end
