---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_苍雪刀_血刀溅射.lua
-- 更新时间:	2014/10/14 11:57:06
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_苍雪刀_血刀溅射.lua",
		0
		);
	...
	skill.AddAttribute(
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK,
		ATTRIBUTE_TYPE.EXECUTE_SCRIPT,
		"skill/苍云/套路及子技能/苍云_苍雪刀_血刀纯伤害.lua",
		0
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(8390, 1) then
		...
		if IsPlayer(dwCharacterID) then
			...
			player.CastSkill(13308, lvZD, TARGET.PLAYER, nTarget.dwID) --强流血
		else
			nTarget = GetNpc(dwCharacterID)
			player.CastSkill(13308, lvZD, TARGET.NPC, nTarget.dwID) --强流血
		end
	else
		local nTarget
		if IsPlayer(dwCharacterID) then
			nTarget = GetPlayer(dwCharacterID)
			player.CastSkill(13295, lvZD, TARGET.PLAYER, nTarget.dwID) --弱流血
		else
			nTarget = GetNpc(dwCharacterID)
			player.CastSkill(13295, lvZD, TARGET.NPC, nTarget.dwID) --弱流血
		end
	end
end
