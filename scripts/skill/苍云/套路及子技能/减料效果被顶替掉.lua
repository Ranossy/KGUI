---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/减料效果被顶替掉.lua
-- 更新时间:	2015/11/8 11:04:47
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	target.AddBuff(target.dwID, target.nLevel, 10244, 1)
	--target.SetTimer(2, "scripts/skill/苍云/套路及子技能/减料效果被顶替掉.lua", 0, 0)
end
...
function OnTimer(player, nParam1, nParam2)
	...
	local nbuffCount = 0
	for i = 1, 3 do
		if player.GetBuff(10243, i) then
			nbuffCount = i
			break
		end
	end
	...
	local nbuff = player.GetBuff(8487, 1)
	if nbuff then
		nbuff_markCount = nbuff.nStackNum
	end
	if nbuffCount ~= nbuff_markCount then
		if nbuffCount == 0 then
			player.DelGroupBuff(8487, 1)
