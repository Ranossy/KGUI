---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_云城盾_盾击眩晕降急速.lua
-- 更新时间:	2015/11/8 10:20:34
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
--------------脚本文件开始------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_云城盾_盾击眩晕降急速.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	-----------------技能施放技能需求-------------------------------------------
	skill.AddCheckSelfLearntSkill(13111, 1, SKILL_COMPARE_FLAG.EQUAL);     --检查比较Caster自己已学习的技能ID和等级
	...
	----------------- BUFF相关 -------------------------------------------------
	--降低加速buff
	--skill.BindBuff(1, 8487, 1);		--减疗
	--眩晕效果
	--skill.BindBuff(2, 8257, 1);		-- 封轻功
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if target.GetBuff(10243, 3) then
		target.AddBuff(player.dwID, player.nLevel, 10243, 3)
	elseif target.GetBuff(10243, 2) then
		target.AddBuff(player.dwID, player.nLevel, 10243, 3)
		target.DelBuff(10243, 2)
	elseif target.GetBuff(10243, 1) then
		target.AddBuff(player.dwID, player.nLevel, 10243, 2)
		target.DelBuff(10243, 1)
	else
		target.AddBuff(player.dwID, player.nLevel, 10243, 1)
	end

	local nbuffCount = 0
	for i = 1, 3 do
		if target.GetBuff(10243, i) then
			nbuffCount = i
			break
		end
	end

	local nbuff_markCount = 0
	local nbuff_mark = target.GetBuff(8487, 1)
	if nbuff_mark then
		nbuff_markCount = nbuff_mark.nStackNum
	end

	if nbuffCount == 0 then
		target.DelGroupBuff(8487, 1)
	elseif nbuffCount == nbuff_markCount and nbuffCount == 3 then
		target.AddBuff(player.dwID, player.nLevel, 8487, 1)
	elseif nbuffCount > nbuff_markCount then
		local nChange = nbuffCount - nbuff_markCount
		for i = 1, nChange do
			target.AddBuff(player.dwID, player.nLevel, 8487, 1)
		end
	elseif nbuffCount < nbuff_markCount then
		local nChange = nbuff_markCount - nbuffCount
		for i = 1, nChange do
			target.DelBuff(8487, 1)
		end
	end
end
