---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/沙场循环判定.lua
-- 更新时间:	2015/10/25 2:53:57
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not target then
		--print(7777777777)
		player.DelBuff(10212, 1)
		return
	end

	--苍云胜利
	if (player.nCurrentLife > player.nMaxLife * 0.2 and target.nCurrentLife <= target.nMaxLife * 0.2) or (player.nCurrentLife <= player.nMaxLife * 0.2 and target.nCurrentLife <= target.nMaxLife * 0.2 and player.nCurrentLife / player.nMaxLife >= target.nCurrentLife / target.nMaxLife) then
		--苍云释放小队增益效果
		player.CastSkill(15197, 1)
		local nbuff_1 = player.GetBuff(10212, 1)
		if nbuff_1 then
			player.nCurrentLife = player.nMaxLife * nbuff_1.nCustomValue / 100
		end
