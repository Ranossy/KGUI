---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_苍雪刀_盾飞.lua
-- 更新时间:	2014/10/16 22:25:41
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_苍雪刀_盾飞.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(8391, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需要不再刀姿态下
	...
	----------------- 设置Cool down --------------------------------------------
	-- 公共CD
	skill.SetPublicCoolDown(822);							-- 公共CD 1.5秒
	-- 技能CD, CoolDownIndex为CD位(共3个), nCoolDownID为CoolDownList.tab内的CDID
	skill.SetNormalCoolDown(1, 818);	--技能普通CD
	skill.SetNormalCoolDown(2, 820);	--技能普通CD
	...
	--skill.SetDelaySubSkill(5, 13352, 1) --隐藏盾
	skill.SetDelaySubSkill(5, 13316, dwSkillLevel) --弹射子技能
	...
	--skill.nMaxHitCount = 19
	--skill.nMaxHitRadius = 30 * 64
	----------------- 施放距离 -------------------------------------------------
	...
	--skill.nBulletVelocity = 40;				-- 子弹速度，单位 点/帧
...
function OnSkillLevelUp(skill, player)
	player.LearnSkillLevel(13051, skill.dwLevel, player.dwID)
	if skill.dwLevel == skill.dwMaxLevel then
		player.AcquireAchievement(4307)
	end
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--切刀0.5秒内无法使用盾猛
	player.AddBuff(0, 99, 8873, 1)

	--加绝刀伤害判定buff
	player.AddBuff(0, 99, 9176, 1)
	...
	if not nTarget.GetBuffByOwner(8249, lvZD, player.dwID) then   --有自身的流血
		nTarget.AddBuff(player.dwID, player.nLevel, 8248, 1, 1) --飞出盾牌即立马带上虚弱，优化闪刀的手感	
	end

	if player.GetSkillLevel(13397) == 1 then  --雷云打断
		player.CastSkill(13398, 1) --打断子技能
	end
	--[[
	if player.GetSkillLevel(13366) == 1 then  --从容，盾飞提高10%基础攻击力
		player.AddBuff(0, 99, 8423, 1, 40) --40秒从容
	end
	--]]
	if player.GetBuff(8265, 1) then
		if player.GetSkillLevel(13181) == 1 then  --肃列
			if not player.GetBuff(8302, 1) then
				player.SetTimer(4, "scripts/skill/苍云/套路及子技能/苍云_苍雪刀_盾飞.lua", 0, 0)
	...
	player.DelGroupBuff(8678, 1) --删除飞瀑的监控
	--删除盾墙姿态
	if player.GetBuff(8265, 1) then
