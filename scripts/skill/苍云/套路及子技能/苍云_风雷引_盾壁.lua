---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_风雷引_盾壁.lua
-- 更新时间:	2014/9/19 19:44:28
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_风雷引_盾壁.lua", -- 属性值1
		0											-- 属性值2
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 8279, dwSkillLevel);		-- 设置1号位Buff
	...
	skill.SetNormalCoolDown(1, 807);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.AddBuff(0, 99, 8470, 1, 48) --12秒记数BUFF，4帧一跳 
	
	
	--防御战意奇穴盾壁回血盾效果加强
	if player.GetSkillLevel(13171) == 1 then
		player.nCurrentLife = player.nCurrentLife + player.nMaxLife * 0.2
		player.AddBuff(player.dwID, player.nLevel, 8292, 1, 8)
		return
	end
	if player.GetSkillLevel(13367) == 1 then  --百炼  100%血量的吸收盾
		player.AddBuff(player.dwID, player.nLevel, 8425, 1, 8)
		return
	end
	--普通版本盾壁吸收盾
		
	local nlv = player.GetSkillLevel(13070)
	player.AddBuff(player.dwID, player.nLevel, 8279, nlv)
	

	ModityCDToUI(player, 13070, 0, 0)
end
