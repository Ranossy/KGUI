---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_云城盾_盾挡被击删buff.lua
-- 更新时间:	2014/10/24 15:51:20
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_云城盾_盾挡被击删buff.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(14845) == 1 then
		if player.GetSkillLevel(13421) == 1 then
			for i = 1, 11 do
				local nbuff = player.GetBuff(8448, i)
				if nbuff then
					local nMark = nbuff.nCustomValue
					if nMark < 1 then
						nbuff.nCustomValue = nbuff.nCustomValue + 1
					else
						player.DelBuff(8488, i)
					end
					break
				end
			end
			...
			for i = 1, 11 do
				local nbuff = player.GetBuff(8499, i)
				if nbuff then
					--print(1111)
					local nMark = nbuff.nCustomValue
					if nMark < 1 then
						nbuff.nCustomValue = nbuff.nCustomValue + 1
						--print(nbuff.nCustomValue)
					else
						--print(222222)
						--print(nbuff.nCustomValue)
						player.DelBuff(8499, i)
					end
					break
				end
			end
		...
		for i = 1, 11 do
			if player.GetBuff(8448, i) then
				player.DelBuff(8448, i)
			end
			if player.GetBuff(8499, i) then
				player.DelBuff(8499, i)
			end
		end
	end
