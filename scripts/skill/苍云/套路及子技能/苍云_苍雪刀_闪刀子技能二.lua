---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_苍雪刀_闪刀子技能二.lua
-- 更新时间:	2014/9/17 17:45:08
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_苍雪刀_闪刀子技能二.lua",
		0
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(8065, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(8283, 1) then  --删除表现一的闪刀标记
		player.DelBuff(8283, 1)
	end
	local lvYD = player.GetSkillLevel(13130)  --隐刀
	local lvXD = player.GetSkillLevel(13131)  --血刀
	...
	if lvYD >= 1 then
		player.AddBuff(player.dwID, player.nLevel, 8270, 1, 7)  --闪刀更换为隐刀的buff维持7秒
	end
	if lvXD >= 1 then
		player.AddBuff(player.dwID, player.nLevel, 8274, 1, 4)  --闪刀更换为血刀的buff维持4秒
	end
	if lvGL >= 1 then
		if npc.GetBuffByOwner(8249, lvZD, player.dwID) then
			local nCount = player.GetBuff(8249, lvZD).nLeftActiveCount     --自身流血的剩余跳数
			--需要程序接口对其进行强化，伤害提高300%
		end
	end
	if lvNT >= 1 then
		if npc.GetBuffByOwner(8248, 1, player.dwID) then
			player.AddBuff(player.dwID, player.nLevel, 8276, 1, 10) --10秒绝刀伤害提高30%
		end
	end
end
