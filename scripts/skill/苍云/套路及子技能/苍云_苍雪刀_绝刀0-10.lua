...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		tSkillData[dwSkillLevel].nDamageBase*1.45/4,											-- 属性值1
   		0										-- 属性值2
   		);
   	skill.AddAttribute(
   		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_SELF_AND_ROLLBACK,					-- 属性作用模式
   		ATTRIBUTE_TYPE.SKILL_PHYSICS_DAMAGE_RAND,								-- 魔法属性
   		tSkillData[dwSkillLevel].nDamageRand*1.45/4,											-- 属性值1
   		0										-- 属性值2
		);
	skill.AddAttribute(
	...
	if dwSkillLevel < 5 then
		skill.nChannelInterval = 60*0.9*0.75*1.45*0.9;
	elseif dwSkillLevel < 20 then
		skill.nChannelInterval = (60 + (dwSkillLevel - 4) * 12)*0.9*0.75*1.45*0.9;
	else
		skill.nChannelInterval = 250*0.75*1.45*0.9;     -- 通道技间隔时间
	end
	----------------- 阵法相关 -------------------------------------------------
	...
	skill.nSelfLifePercentMax	= 10;				-- 血量最大值<=
