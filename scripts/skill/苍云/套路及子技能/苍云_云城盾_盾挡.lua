---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_云城盾_盾挡.lua
-- 更新时间:	2015/9/24 20:09:23
-- 更新用户:	mengxiangfei11
-- 脚本说明:
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_云城盾_盾挡.lua", -- 属性值1
		0											-- 属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(8265, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	skill.AddSlowCheckSelfBuff(8481, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	skill.SetNormalCoolDown(1, 833);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	--最少需要10点怒气
	if player.nCurrentRage < 10 and not player.GetBuff(10087, 1) then
		return SKILL_RESULT_CODE.FAILED
	end
	return nPreResult;
end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nTime = 0
	if player.GetSkillLevel(14845) == 1 then
		nTime = nTime + 4
	end

	local nRageCost = 0

	if player.GetSkillLevel(14846) == 1 and player.GetBuff(10087, 1) then
		local nAdd_lev = 0
		if player.GetSkillLevel(13136) == 1 then
			nAdd_lev = nAdd_lev + 1
		end

		if player.GetSkillLevel(13421) == 1 then  --盾挡收益提高50%
			player.AddBuff(player.dwID, player.nLevel, 8448, 10 + nAdd_lev, 6 + nTime)
		else
			player.AddBuff(player.dwID, player.nLevel, 8499, 10 + nAdd_lev, 6 + nTime) --弱版本
		end
		player.DelBuff(10087, 1)
		--player.AddBuff(player.dwID, player.nLevel, 9887, 1)
		--player.nCurrentRage = player.nCurrentRage + 10	--返还消耗的10点
	else
		--技能先消耗了10点
		local nlev = math.floor(player.nCurrentRage / 10) + 1
		if nlev ~= 0 then
			if nlev < 11 then
				if player.GetSkillLevel(13421) == 1 then  --盾挡收益提高50% --千山
					player.AddBuff(player.dwID, player.nLevel, 8448, nlev, 6 + nTime)
				else
					player.AddBuff(player.dwID, player.nLevel, 8499, nlev, 6 + nTime) --弱版本
				end
			else
				if player.GetSkillLevel(13421) == 1 then  --盾挡收益提高50%
					player.AddBuff(player.dwID, player.nLevel, 8448, 11, 6 + nTime)
				else
					player.AddBuff(player.dwID, player.nLevel, 8499, 11, 6 + nTime) --弱版本
				end
			end
			player.nCurrentRage = player.nCurrentRage - 10 * (nlev - 1)

			nRageCost = 10 * nlev
		end
	end

	if player.GetSkillLevel(13422) == 1 then  --振奋
		player.CastSkill(13423, 1)
	end

	--盾压消耗30点怒气，有奇穴13073 赤心 回血
