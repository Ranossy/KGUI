---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/石虎判定血量重置盾壁.lua
-- 更新时间:	2015/9/24 20:03:39
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local buff_9885 = player.GetBuff(9885, 1)
	if buff_9885 and buff_9885.nLeftActiveCount < 100 and player.GetSkillLevel(14841) > 0 then
		buff_9885.nLeftActiveCount = 9999
	end
	if player.nCurrentLife <= player.nMaxLife * 0.4 then
		--[[
		local nlev = player.GetSkillLevel(13070)
		if nlev ~= 0 then
			local rest, nleftframe, ntotalframe = player.GetSkillCDProgress(13070, nlev)
			if rest == true and nleftframe ~= 0 and not player.GetBuff(9886, 1) then
				player.ClearCDTime(807)
				player.AddBuff(player.dwID, player.nLevel, 9886, 1)
			end
		end
		--]]
		--807为盾壁CD
		local nleftframe, nCDCount = player.GetCDLeft(807)
		if nleftframe ~= 0 and not player.GetBuff(9886, 1) then
			player.ClearCDTime(807)
			player.AddBuff(player.dwID, player.nLevel, 9886, 1)
		end
	end
