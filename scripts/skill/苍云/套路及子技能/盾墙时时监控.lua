...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local cengshu
	if BJbuff then
		cengshu = BJbuff.nCustomValue
	end

	
	if player.GetSkillLevel(13555)  == 1 then  --苍云武器特殊附魔
		player.CastSkill(13554, 1)
	end

	
	if player.nCurrentEnergy < 1 then  --格挡值空了
		--删除盾墙姿态
		----------------------------------------------------盾墙姿态附带的效果
	...
	if not player.GetBuff(8300, cengshu) and not player.GetBuff(8421, cengshu) then  --手动点掉减伤
		--删除盾墙姿态
		----------------------------------------------------盾墙姿态附带的效果
		player.DelBuff(8265, 1) --免控
		player.DelBuff(8400, 1) --换动作
	...
	if not player.GetBuff(8265, 1) then  --手动点掉免控后
		--删除盾墙姿态
		----------------------------------------------------盾墙姿态附带的效果
		player.DelBuff(8265, 1) --免控
		player.DelBuff(8400, 1) --换动作
		...
		if player.GetSkillLevel(13181) == 1 then
			if not player.GetBuff(8302, 1) then
				player.AddBuff(player.dwID, player.nLevel, 8302, 1) --肃列的免疫控制
				player.DelBuff(8486, 1) --删除时时监控
			end	
		end
		if player.GetSkillLevel(13364) == 1 then --肆意，防御奇穴，盾墙消失后团队无威胁buff
			player.CastSkill(13365, 1)
	...
	if player.GetBuff(8391, 1) then  --卡飞盾的情况
