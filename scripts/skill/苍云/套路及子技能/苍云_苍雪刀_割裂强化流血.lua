---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/苍云/套路及子技能/苍云_苍雪刀_割裂强化流血.lua
-- 更新时间:	2014/10/16 10:19:43
-- 更新用户:	zhouyixiao2
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/苍云/套路及子技能/苍云_苍雪刀_割裂强化流血.lua",
		0
		);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 8249, dwSkillLevel);		-- 设置1号位Buff
	...
	if dwSkillLevel < 10 then
		skill.nChannelInterval = 16 * 1.5*1.5;
	elseif dwSkillLevel < 22 then
		skill.nChannelInterval = (16 + (dwSkillLevel - 9) * 4) *1.5*1.5;
	else
		skill.nChannelInterval = 70 * 1.5*1.5;     -- 通道技间隔时间
	end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if nTarget then
		if nTarget.GetBuffByOwner(8249, lvZD, player.dwID) then   --目标身上有自身加强版的流血
			local LXbuff = nTarget.GetBuffByOwner(8249, lvZD, player.dwID)
			local TXbuff = player.GetBuff(8389, 1)
			if TXbuff then
				nTiaoShu = TXbuff.nCustomValue
			end
			if LXbuff then
				if nTiaoShu > 0 then
					nTarget.SetBuffLeftActiveCount(LXbuff.nIndex, nTiaoShu)
					player.SetTimer(4, "scripts/skill/苍云/套路及子技能/苍云_苍雪刀_割裂强化流血.lua", nTarget.dwID, 0)	
				end
			end
		end
	end
...
function OnTimer(player, nParam1, nParam2)
	...
	if IsPlayer(nParam1) then
		nTarget = GetPlayer(nParam1)
	else
		nTarget = GetNpc(nParam1)
	end
	if not target then
		return
	end
	local LXbuff = nTarget.GetBuffByOwner(8249, lvZD, player.dwID)
	if LXbuff then
		LXbuff.nCustomValue = 1   --当变量标记为1时，表明此时的8249为强流血
		player.DelBuff(8389, 1) --删除记录跳数用的标记buff
	end
end
