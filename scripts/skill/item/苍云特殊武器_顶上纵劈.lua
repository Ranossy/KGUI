---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/item/苍云特殊武器_顶上纵劈.lua
-- 更新时间:	2016/3/24 10:24:41
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/item/苍云特殊武器_顶上纵劈.lua", --属性值1
		0															--属性值2
	);
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 5500, dwSkillLevel);		-- 设置1号位Buff
	...
	skill.nHeight = 28 * LENGTH_BASE;                            -- AOE的高度，全高，圆柱体AOE中不填为2倍的nAreaRadius，矩形AOE中不填为nAreaRadius
...
function Apply(dwCharacterID, dwSrcID)
	...
	if player then
		npc.CastSkill(15671, 1, TARGET.PLAYER, player.dwID)
		if player.GetBuff(10565, 1) then---有无畏
			player.AddBuff(0, 99, 10568, 1, 5)---加寻首
			player.nCurrentRage = player.nCurrentRage + player.nMaxRage * 0.3
		end
	end
end
