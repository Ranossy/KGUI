---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/item/苍云特殊武器_炎帝反击火球AOE母技能.lua
-- 更新时间:	2016/10/10 17:54:11
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/item/苍云特殊武器_炎帝反击火球AOE母技能.lua", -- 属性值1
		0															--属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local playerID = npc.GetCustomInteger4(4)
	local player = GetPlayer(playerID)
	if player then
		if npctarget then
			player.CastSkill(17032, 1, TARGET.NPC, npctarget.dwID)
		end
	end

	if npc.nCurrentLife <= npc.nMaxLife * 0.07 then
		npc.Die()
	else
		npc.nCurrentLife = npc.nCurrentLife - npc.nMaxLife * 0.07
	end
end
