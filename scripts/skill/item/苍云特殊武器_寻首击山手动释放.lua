---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/item/苍云特殊武器_寻首击山手动释放.lua
-- 更新时间:	2016/9/8 17:51:11
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/item/苍云特殊武器_寻首击山手动释放.lua", -- 属性值1
		0											-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(11183, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		player.AddBuff(0, 99, 11185, 1)--寻首辅助
		...
		player.DelBuff(10828, 1)
		--player.CastSkill(16493, 1)
		--local nDir = player.nFaceDirection
		--local N_X = player.nX + 35 * math.cos(2 * nDir * math.pi / 255) 
		--local N_Y = player.nY + 35 * math.sin(2 * nDir * math.pi / 255)
		--player.RunTo(N_X, N_Y)
		player.SetTimer(0.2 * GLOBAL.GAME_FPS, "scripts/skill/item/苍云特殊武器_寻首击山手动释放.lua", dwSrcID, 1)
	end
end
