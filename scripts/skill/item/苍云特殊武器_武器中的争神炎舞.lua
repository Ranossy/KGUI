---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/item/苍云特殊武器_武器中的争神炎舞.lua
-- 更新时间:	2016/6/7 17:14:41
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
tSkillData =
{
	{nDamageBase = 20000, nDamageRand = 0, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/item/苍云特殊武器_武器中的争神炎舞.lua", -- 属性值1
		0															--属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	--skill.AddSlowCheckSelfBuff(10823, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		player.PlaySfx(63, X2, Y2, player.nZ + 512)
		player.PlayCharacterSound(19201)
		--player.PlaySfx(1453, X2, Y2, player.nZ + 512, 360 * player.nFaceDirection / 256 + 90)
		player.PlaySfx(1455, X2, Y2, player.nZ+128, 360 * player.nFaceDirection / 256 -180)
		player.PlaySfx(1449, X2, Y2, player.nZ + 128, 360 * player.nFaceDirection / 256 - 180)
		player.PlaySfx(1456, X2, Y2, player.nZ, 360 * player.nFaceDirection / 256 + 180)
		player.SetTimer(0.4 * GLOBAL.GAME_FPS, "scripts/skill/item/苍云特殊武器_武器中的争神炎舞.lua", dwSrcID, 1)
	end

	if npctarget then
		if npctarget.GetBuff(11181, 1) then
			if player then
				npctarget.AddBuff(player.dwID, player.nLevel, 11182, 2, 15)
			end
		else
			npctarget.AddBuff(player.dwID, player.nLevel, 11182, 1, 15)
		end
	end
end
...
function OnTimer(player, nParam1, nParam2)
	...
	local X2 = X1 + H * math.cos(F/128 * math.pi)
	local Y2 = Y1 + H * math.sin(F / 128 * math.pi)
	player.CastSkill(16718, 1)
	player.PlayCharacterSound(19202)
	player.PlaySfx(1463, X2, Y2, player.nZ+128, 360 * player.nFaceDirection / 256 -180)
