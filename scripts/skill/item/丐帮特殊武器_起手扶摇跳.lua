---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/item/丐帮特殊武器_起手扶摇跳.lua
-- 更新时间:	2017/2/10 15:03:07
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/item/丐帮特殊武器_起手扶摇跳.lua", -- 属性值1
		0															--属性值2
	)
	--[[
   	skill.AddAttribute(
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	...
	if nType == TARGET.NPC then
		...
		if npc then
			...
			if nDis <= 10 * 64 then
				if not player.GetBuff(7049, 0) and not player.GetBuff(7525, 0) and not player.GetBuff(7561, 0) and not player.GetBuff(9982, 0) then
					return nPreResult;
				end
			end
		end
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if nType == TARGET.NPC then
		local npc = GetNpc(nID)
		if npc then
			local nDis = GetCharacterDistance(npc.dwID, player.dwID)
			if nDis <= 10 * 64 then
				if not IsPlayer(npc.dwID) then
					if player and player.nMoveState ~= MOVE_STATE.ON_DEATH then
						player.Stop()
						player.PlaySfx(1532, player.nX, player.nY, player.nZ)
						player.AddBuff(player.dwID, player.nLevel, 208, 11,30)
						player.Jump(0, true)
						player.AddBuff(player.dwID, player.nLevel, 11415, 1)
					end
				end
			end
		end
