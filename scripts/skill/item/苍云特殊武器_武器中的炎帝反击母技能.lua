---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/item/苍云特殊武器_武器中的炎帝反击母技能.lua
-- 更新时间:	2016/8/17 16:12:37
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/item/苍云特殊武器_武器中的炎帝反击母技能.lua", -- 属性值1
		0											-- 属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(11106, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		player.DoAction(0, 60125)
		--player.DelBuff(8400, 1)
		player.CastSkill(16720, 1)--炎帝反击增
		player.DelBuff(11036, 1)---删除持盾buff
		player.DelBuff(11036, 2)---删除持盾buff
		player.DelBuff(8576, 1)---盾墙无法再次盾墙标记
		player.DelBuff(10821, 1)---免控
		player.DelBuff(11106, 1)--炎帝反击开始图标为黑
		player.DelBuff(10824, 1)--特殊技能栏内技能变亮被击响应buff
		player.DelBuff(11197, 1)---删除魂吸buff
		player.SetDynamicSkillGroup(0)
		player.PlayCharacterSound(19192)
		local scene = player.GetScene()
		scene.CreateNpc(51507, player.nX, player.nY, player.nZ, player.nFaceDirection, 128,  player.dwID .. "yandiqiu")
		local yandiqiu = scene.GetNpcByNickName(player.dwID .. "yandiqiu")
		if yandiqiu then
			yandiqiu.SetCustomInteger4(4, player.dwID)
		end
		...
		player.PlaySfx(1454, X2, Y2, player.nZ + 128, 360 * player.nFaceDirection / 256 + 90)
		player.SetTimer(0.5 * GLOBAL.GAME_FPS, "scripts/skill/item/苍云特殊武器_武器中的炎帝反击母技能.lua", dwSrcID, 1)
	end
end
