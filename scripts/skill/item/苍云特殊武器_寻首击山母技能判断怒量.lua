---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/item/苍云特殊武器_寻首击山母技能判断怒量.lua
-- 更新时间:	2016/3/25 9:58:00
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/item/苍云特殊武器_寻首击山母技能判断怒量.lua", -- 属性值1
		0															--属性值2
	);
	...
	----------------- 设置Cool down --------------------------------------------
	-- 公共CD
	skill.SetNormalCoolDown(1, 1102)
...
function Apply(dwCharacterID, dwSrcID)
	...
	player.PlayCharacterSound(19198)
	local X1 = player.nX
		local Y1 = player.nY
		local H = 450
	...
	if player.nCurrentRage < player.nMaxRage then
		player.CastSkill(15680, 1)
		player.DoAction(0,60122)
		player.nCurrentRage = 0
	end

	if player.nCurrentRage == player.nMaxRage then
		player.CastSkill(15681, 1)
		player.DoAction(0,60122)
		player.nCurrentRage = 0
	end
end
