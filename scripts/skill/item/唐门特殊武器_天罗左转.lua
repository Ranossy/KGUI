...
function Apply(dwCharacterID)
	...
	local tbuff = target.GetBuff(4170, 1)
	local tID = tbuff.dwSkillSrcID 
	local tnpc = scene.GetNpcByNickName("cannon_"..tID)
	if not tnpc then 
		return
	end
	tnpc.TurnTo(tnpc.nFaceDirection + 64)
	target.AddBuff(0,99,4184,1)
end
