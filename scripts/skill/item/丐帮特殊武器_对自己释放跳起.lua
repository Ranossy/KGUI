---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/item/丐帮特殊武器_对自己释放跳起.lua
-- 更新时间:	2016/10/11 23:23:01
-- 更新用户:	ZHANGGUANYI
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/item/丐帮特殊武器_对自己释放跳起.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
		...
		if nType == TARGET.NPC then
			...
			if npc then
				if not IsPlayer(npc.dwID) then
					...
					player.AddBuff(player.dwID, player.nLevel, 11396, 1)
					player.AddBuff(player.dwID, player.nLevel, 11395, 1)
					player.AddBuff(player.dwID, player.nLevel, 11416, 1)
				end
			end
		end
		--print("oye")
