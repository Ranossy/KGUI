---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/item/长歌门特殊武器_舞剑.lua
-- 更新时间:	2016/9/20 17:24:15
-- 更新用户:	yangcheng3
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/item/长歌门特殊武器_舞剑.lua", -- 属性值1
		0															--属性值2
	);
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(11198, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	skill.SetNormalCoolDown(1, 1100);	--技能普通CD
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player then
		...
		player.PlaySfx(1479, X2, Y2, player.nZ + 512, 360 * player.nFaceDirection / 256 + 90)
		
		local scene = player.GetScene()
		local yingzi3 = scene.GetNpcByNickName(player.dwID .. "yingzi3")
		local yingzi2 = scene.GetNpcByNickName(player.dwID .. "yingzi2")
		if yingzi3 then
			yingzi3.AddBuff(0, 99, 10527, 1, 30)
		end
		
		if yingzi2 then
			if yingzi2.GetCustomInteger4(16) >= 3 then
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(10, 4891), 2)
				return
			end
			local step = yingzi2.GetCustomInteger4(16)
			step = step + 1
			yingzi2.SetCustomInteger4(16, step)
			--print(yingzi2.GetCustomInteger4(16))
		end
		...
		if yingzi3 then
			...
			local step = yingzi2.GetCustomInteger4(16)
			if step == 1 then
				yingzi3.PlaySfx(1429, yingzi3.nX, yingzi3.nY, yingzi3.nZ)
			end
			if step == 2 or step == 3 then
				player.PlaySfx(1486, X2, Y2, player.nZ + 512, 360 * player.nFaceDirection / 256 - 90)
			end
		end

		if yingzi3 then
			local step = yingzi3.GetCustomInteger4(20)
			step = step + 1
			yingzi3.SetCustomInteger4(20, step)
		end
	end
end
