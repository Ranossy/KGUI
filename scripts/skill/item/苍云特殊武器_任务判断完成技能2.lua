---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/item/苍云特殊武器_任务判断完成技能2.lua
-- 更新时间:	2016/3/24 15:17:44
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/item/苍云特殊武器_任务判断完成技能2.lua",
		0
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nQuestID = 15119
	local nQuestPhase = player.GetQuestPhase(nQuestID)
	local nQuestIndex = player.GetQuestIndex(nQuestID)
	if nQuestPhase ~= 1 or not nQuestIndex then
		return
	end

	local nValue = player.GetQuestValue(nQuestIndex, 1)
	player.SetQuestValue(nQuestIndex, 1, nValue + 1)
end
