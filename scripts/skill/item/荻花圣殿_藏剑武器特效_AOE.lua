...
tSkillData =
{
	{nDamage = 3000*0.8, nDamageRand = 3000*0.4, nCostMana = 0},		--level 1
	{nDamage = 0, nDamageRand = 0, nCostMana = 0},		--level 2
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
      skill.BindBuff(1, 3650, 1);		-- 设置1号位Buff
    ...
    --skill.nChannelInterval	= 960; 				-- 通道技间隔时间 
...
function Apply(dwCharacterID)
    ...
    --player.SetPosition(player.nX, player.nY, player.nZ - 4*8*64) 
 	--player.SetPosition(player.nX, player.nY, 0)
 	player.CastSkill(3353,1)
	--player.SetTimer(9, "scripts/skill/item/荻花圣殿_藏剑武器特效_AOE.lua", 0, 0)
	local nDis = 2								--距离
	local nDir = 20								--角度
	local nRad = 2 * math.pi * (player.nFaceDirection * 360 / 255 - nDir) / 360
		player.PlaySfx(121, player.nX + nDis * 64 * math.cos(nRad), player.nY + nDis * 64 * math.sin(nRad), player.nZ)
	for i = 1, 4 do
		player.PlaySfx(117, player.nX + nDis * 64 * math.cos(nRad), player.nY + nDis * 64 * math.sin(nRad), player.nZ - 1000, 90 * i + player.nFaceDirection * 360 / 255 - 40)
	end	
	player.SetTimer(9, "scripts/skill/item/荻花圣殿_藏剑武器特效_AOE.lua", 0, 0)
end
...
function OnTimer(player, nParam1, nParam2)				-- 一定时间后生效
	...
	local nDir = 20								--角度
	local nRad = 2 * math.pi * (player.nFaceDirection * 360 / 255 - nDir) / 360
 	player.PlaySfx(120, player.nX + nDis * 64 * math.cos(nRad), player.nY + nDis * 64 * math.sin(nRad), player.nZ)
end
