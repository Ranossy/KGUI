---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/item/苍云特殊武器_判断被击者距离启动技能.lua
-- 更新时间:	2016/6/8 10:11:57
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/item/苍云特殊武器_判断被击者距离启动技能.lua", -- 属性值1
		0															--属性值2
	);
	--[[
   	skill.AddAttribute(
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if npctarget then
		if GetCharacterDistance(npctarget.dwID, dwSkillSrcID) <= 384 then
			if player then
