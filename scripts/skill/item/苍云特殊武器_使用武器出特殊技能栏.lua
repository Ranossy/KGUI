---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/item/苍云特殊武器_使用武器出特殊技能栏.lua
-- 更新时间:	2016/6/7 14:43:35
-- 更新用户:	zhangdongen
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/item/苍云特殊武器_使用武器出特殊技能栏.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	player.AddBuff(0, 99, 10821, 1, 40)  --免控
	--player.AddBuff(0, 99, 8400, 1, 40) --换动作（需要新的buffID)
	--player.AddBuff(0, 99, 11036, 1, 40) --换动作（需要新的buffID)
	player.AddBuff(0, 99, 8576, 1, 40) --盾墙无法再次盾墙
	player.AddBuff(0, 99, 10823, 1, 40) --争神炎舞开始图标为黑
	player.AddBuff(0, 99, 10825, 1, 40) --寻首击山开始图标为黑
	player.AddBuff(0, 99, 11106, 1, 40) --炎帝反击开始图标为黑
	player.AddBuff(0, 99, 11183, 1, 40) --寻首击山手动开始图标为黑
	player.AddBuff(0, 99, 10824, 1, 40) --特殊技能栏内技能变亮被击响应buff
	player.AddBuff(0, 99, 11197, 1, 40)--魂吸buff
	player.SetDynamicSkillGroup(262)
	player.PlayCharacterSound(19191)
	player.PlaySfx(1440,player.nX,player.nY,player.nZ + 756)
	player.SetTimer(0.7 * GLOBAL.GAME_FPS, "scripts/skill/item/苍云特殊武器_使用武器出特殊技能栏.lua", dwSrcID, 1)
...
function OnTimer(player, nParam1, nParam2)
	player.PlayCharacterSound(19192)
	player.PlayCharacterSound(19193)
	player.PlaySfx(63, 0, 0, 0)
	player.PlaySfx(1439,player.nX,player.nY,player.nZ+256)
	local X1 = player.nX
	local Y1 = player.nY
	local H = 50
	local F = player.nFaceDirection
	local X2 = X1 + H * math.cos(F/128 * math.pi)
	local Y2 = Y1 + H * math.sin(F / 128 * math.pi)
	player.PlaySfx(563, X2, Y2, player.nZ, 360 * player.nFaceDirection / 256 )
	player.PlaySfx(1436, X2, Y2, player.nZ, 360 * player.nFaceDirection / 256 )
	player.PlaySfx(1519, player.nX, player.nY, player.nZ, 360 * player.nFaceDirection / 256 )
	player.CastSkillXYZ(16715, 1, X2, Y2, player.nZ)
	player.AddBuff(0, 99, 11346, 1, 160)
	player.AddBuff(0, 99, 11036, 1, 40)
end
