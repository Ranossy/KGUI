---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/五毒/五毒_物件_仙王蛊鼎_镇派2.lua
-- 更新时间:	2013/4/29 14:29:32
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
	...
	if owner then 
		...
		if bIsPartyMember or player.dwID == owner.dwID then
			if  player.GetBuff(3448, 1) then 
				...
				if owner.GetSkillLevel(6039) == 1 then
					player.AddBuff(doodad.dwOwnerID, owner.nLevel, 5776, 1, 1)
				end
				player.AddBuff(0, 99, 2515, 3, 0)
