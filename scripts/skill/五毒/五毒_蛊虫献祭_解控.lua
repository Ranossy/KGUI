...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 2544, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID)
	...
	if pet then 
		...
		if PetType ==  9956 then 				--蝎子
			player.AddBuff(0,99,2307,1)
			pet.SetDisappearFrames(3)
			pet.DieByKiller(pet.dwID)
		elseif PetType == 9999 then 				--蟾蜍
			player.AddBuff(0,99,2307,2)
			pet.SetDisappearFrames(3)
			pet.DieByKiller(pet.dwID)
		elseif PetType == 9998 then 				--蛇
			
			player.AddBuff(0,99,2307,3)
			pet.SetDisappearFrames(16)
			if pet.GetBuff(2829, 2) then
				pet.DelBuff(2829, 2)
			end
			pet.DieByKiller(pet.dwID)
		elseif PetType == 9996 then 				--蜈蚣
			player.CastSkillXYZ(332, 1, pet.nX, pet.nY, pet.nZ)
			pet.SetDisappearFrames(3)
			pet.DieByKiller(pet.dwID)
