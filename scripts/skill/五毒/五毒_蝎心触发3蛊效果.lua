---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/五毒/五毒_蝎心触发3蛊效果.lua
-- 更新时间:	2015/10/18 19:41:00
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local lv1 = player.GetSkillLevel(2209)
	local lv2 = player.GetSkillLevel(6621)
	local nlev = math.max(lv1, lv2)
	
	if nlev ~= 0 then
		--触发夺命
		if target.GetBuff(2307, 1) or target.GetBuff(10118, 1) then
			player.CastSkill(2455, nlev, player.GetSkillTarget())
		end
		--触发迷心
		if not player.GetBuff(4678, 1) and (target.GetBuff(2308, 1) or target.GetBuff(10118, 1)) then
			player.CastSkill(2460, nlev, player.GetSkillTarget())
		end
		--触发枯残
		if target.GetBuff(2309, 1) or target.GetBuff(10118, 1) then
			player.CastSkill(2465, nlev, player.GetSkillTarget())
		end
	end
end
