---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/五毒/宠物死亡AOE伤害.lua
-- 更新时间:	2014/10/24 12:06:03
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData =
{
	{nDamageBase = 501, nDamageRand = 50, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 6223, 1);		-- 设置1号位Buff
	...
	skill.nChannelInterval = 100*0.65; 				-- 通道技间隔时间
...
function Apply(dwCharacterID)
	...
	--[[
	if not target.GetBuff(8444, 1) then
		target.AddBuff(0, 99, 6223, 1)
		target.AddBuff(0, 99, 8444, 1)
	end
	--]]
	if target.GetBuff(9173, 1) then
		return
	elseif target.GetBuff(9172, 1) then
		target.AddBuff(0, 99, 6223, 1, 2)
		target.DelBuff(9172, 1)
		target.AddBuff(0, 99, 9173, 1)
	else
		target.AddBuff(0, 99, 6223, 1, 4)
