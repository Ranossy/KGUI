...
function Apply(dwCharacterID)
	...
	if player then
		...
		local pet = nil
		if nResault == 1 then 
			local pet = scene.CreatePet(9999, nCX, nCY, nZ, nDir, - 1, dwCharacterID);
			if not pet then
				return
			end
			
			pet.AddBuff(pet.dwID, pet.nLevel, 2565, 6)
			player.CastSkill(2970, 1, TARGET.NPC, pet.dwID)			--为宠物上蟾蜍镇派效果
			...
			local pet = scene.CreatePet(9999, nX, nY, nZ, nDir, -1, dwCharacterID);
			pet.AddBuff(pet.dwID, pet.nLevel, 2565, 6)
			player.CastSkill(2970, 1, TARGET.NPC, pet.dwID)			--为宠物上蟾蜍镇派效果
			player.CastSkill(2978, 1, TARGET.NPC, pet.dwID)			--为宠物上公共镇派效果
			pet.nCurrentLife = pet.nMaxLife
