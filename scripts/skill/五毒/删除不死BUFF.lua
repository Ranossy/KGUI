---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/五毒/删除不死BUFF.lua
-- 更新时间:	2015/9/20 18:03:28
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.nSelfLifePercentMax = 2;				-- 血量最大值<=
...
function Apply(dwCharacterID)
	...
	if player.nCurrentLife < player.nMaxLife * 0.01 then
		local npet = player.GetPet()
		if not npet then
			return
		end
		if player.GetSkillLevel(6636) == 1 then  --宠物死亡AOE伤害
			npet.CastSkill(6638, 1)
		end
		if player.GetSkillLevel(6637) == 1 then --宠物死亡AOE格挡
			npet.CastSkill(6639, 1)
		end
		if npet then
			npet.Die()
		end
		player.nCurrentLife = player.nMaxLife * 0.3
		player.DelBuff(6235, 1)
		player.AddBuff(player.dwID, player.nLevel, 6236, 1)
	end
end
