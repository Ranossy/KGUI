...
function GetSkillLevelData(skill)
	...
	----------------- 技能施放Buff需求 ---------------------------------------------
	  skill.AddSlowCheckSelfBuff(5790, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
...
function Apply(dwCharacterID)
	...
	--[[
	--B_5970 缝针免死
	if not player.GetBuff(5970, 1) and not player.GetBuff(6235,1) and not player.GetBuff(6236,1) then
		player.AddBuff(0,99,6235,1)
	end
	--]]

	if player.GetSkillLevel(6652) == 1 and player.GetPet() and not player.GetBuff(6236, 1) then
		player.AddBuff(player.dwID, player.nLevel, 6235, 1)
	else
