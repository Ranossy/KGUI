---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/五毒/五毒_五仙图录_蛇影.lua
-- 更新时间:	2015/10/18 20:36:37
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
--增加额外内功AP加成2倍
	...
	--0																-- 属性值2
	--);
	skill.AddAttribute(													-- dot触发
		ATTRIBUTE_EFFECT_MODE.EFFECT_TO_DEST_NOT_ROLLBACK, -- 属性作用模式
		ATTRIBUTE_TYPE.EXECUTE_SCRIPT, -- 魔法属性
		"skill/五毒/五毒_五仙图录_蛇影.lua", --属性值1
		0										--属性值2
	);
	skill.AddAttribute(													-- dot触发
	...
	if dwSkillLevel <= 6 then
		skill.nCostManaBasePercent = 386 * 0.2;	-- 技能消耗的内力
	elseif dwSkillLevel <= 12 then
		skill.nCostManaBasePercent = 386 * 0.4;	-- 技能消耗的内力
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local lv1 = player.GetSkillLevel(2211)
	local lv2 = player.GetSkillLevel(13430)
	local nlev = math.max(lv1, lv2)
	
	if nlev ~= 0 then
		--触发夺命
		if target.GetBuff(2307, 1) or target.GetBuff(10118, 1) then
			player.CastSkill(2457, nlev, player.GetSkillTarget())
		end
		--触发迷心
		if target.GetBuff(2308, 1) or target.GetBuff(10118, 1) then
			player.CastSkill(2462, nlev, player.GetSkillTarget())
		end
		--触发枯残
		if target.GetBuff(2309, 1) or target.GetBuff(10118, 1) then
			player.CastSkill(2467, nlev, player.GetSkillTarget())
		end
	end
end
