...
function Apply(dwCharacterID, dwSrcID)
	local nPet = GetNpc(dwSrcID)
	if not nPet then
		return
	end
	local player = nPet.GetEmployer()
	if not player then
		return
	end
	local nTarget = GetNpc(dwCharacterID)
	if nTarget then
		player.CastSkill(6648,1,TARGET.NPC,dwCharacterID)
	end
	local nTarget = GetPlayer(dwCharacterID)
	if nTarget then
		player.CastSkill(6648,1,TARGET.PLAYER,dwCharacterID)
	end
end
