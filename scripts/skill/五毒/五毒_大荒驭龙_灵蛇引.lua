---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/五毒/五毒_大荒驭龙_灵蛇引.lua
-- 更新时间:	2013/4/29 11:07:40
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
	...
	if player then
		...
		local pet = nil
		if nResault == 1 then 
			local pet = scene.CreatePet(9998, nCX, nCY, nZ, nDir, - 1, dwCharacterID);
			if not pet then
				return
			end
			pet.AddBuff(pet.dwID, pet.nLevel, 2565, 1)
			player.CastSkill(2969, 1, TARGET.NPC, pet.dwID)			--为宠物上蛇镇派效果
			player.CastSkill(2978, 1, TARGET.NPC, pet.dwID)			--为宠物上公共镇派效果
			pet.nCurrentLife = pet.nMaxLife
			if player.GetSkillLevel(6024) == 1 then
				pet.AddBuff(dwCharacterID, player.nLevel, 5768, 1)
			end
			...
			local pet = scene.CreatePet(9998, nX, nY, nZ, nDir, -1, dwCharacterID);
			pet.AddBuff(pet.dwID, pet.nLevel, 2565, 1)
			player.CastSkill(2969, 1, TARGET.NPC, pet.dwID)			--为宠物上蛇镇派效果
			player.CastSkill(2978, 1, TARGET.NPC, pet.dwID)			--为宠物上公共镇派效果
			pet.nCurrentLife = pet.nMaxLife
