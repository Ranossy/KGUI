...
function Apply(dwCharacterID)
	...
	if player then
		...
		local pet = nil
		if nResault == 1 then 
			local pet = scene.CreatePet(12944, nCX, nCY, nZ, nDir, - 1, dwCharacterID);
			if not pet then
				return
			end
			pet.FireAIEvent(2002, 0, 2) --无视
			pet.FireAIEvent(2001, 0, 0) --跟随
			player.CastSkill(2978, 1, TARGET.NPC, pet.dwID)			--为宠物上公共镇派效果
			pet.nCurrentLife = pet.nMaxLife
			pet.AddBuff(0, 99, 2998, 1)
			...
			if player.GetSkillLevel(6636) == 1 then
				pet.AddBuff(pet.dwID, pet.nLevel, 6223, 1)
			end
			...
			local pet = scene.CreatePet(12944, nX, nY, nZ, nDir, -1, dwCharacterID);
			pet.FireAIEvent(2002, 0, 2) --无视
			pet.FireAIEvent(2001, 0, 0) --跟随
			player.CastSkill(2978, 1, TARGET.NPC, pet.dwID)			--为宠物上公共镇派效果
			pet.nCurrentLife = pet.nMaxLife
