---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/五毒/五毒_金蟾吐瑞_治疗子技能.lua
-- 更新时间:	2013/4/29 15:03:39
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
tSkillData = 
{
	{nDamageBase = 502 * 0.95, nDamageRand = 502 * 0.1, nCostMana = 0}, --level 1
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/五毒/五毒_金蟾吐瑞_治疗子技能.lua", -- 属性值1
		0														-- 属性值2
	);
	----------------- 技能施放Buff需求 ---------------------------------------------
	...
	skill.nChannelInterval = 64 * 1.15; 				-- 通道技间隔时间 
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(6049) == 1 then
		target.nCurrentMana = target.nCurrentMana + target.nMaxMana * 0.01
	end

	if player.GetSkillLevel(14868) == 1 and target.nCurrentLife <= target.nMaxLife * 0.4 then
		--local type, ID = player.GetSkillTarget()
		--print(ID)
		player.CastSkill(14896, 1, player.GetSkillTarget())
	end

	if player.GetSkillLevel(6883) == 1 then
