---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/五毒/凤凰蛊不死清除.lua
-- 更新时间:	2014/7/19 9:43:19
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/五毒/凤凰蛊不死清除.lua", -- 属性值1
		0														-- 属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckSelfBuff(8104, 1, BUFF_COMPARE_FLAG.EQUAL, 1, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	skill.nSelfLifePercentMax = 1;				-- 血量最大值<=
...
function Apply(dwCharacterID)
	...
	local dwSkillSrcID = player.GetBuff(8104, 1).dwSkillSrcID
	...
	if player.GetScene().dwID == target.GetScene().dwID and player.GetScene().dwMapID == target.GetScene().dwMapID then
		if dwSkillSrcID == dwCharacterID or IsParty(dwSkillSrcID, dwCharacterID) then
			player.AddBuff(player.dwID, player.nLevel, 8103, 1)
			player.SetDynamicSkillGroup(167)
			player.nCurrentLife = player.nMaxLife
		end
	end
	player.DelBuff(8104, 1)
end
