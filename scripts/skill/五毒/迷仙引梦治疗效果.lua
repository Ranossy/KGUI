---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/五毒/迷仙引梦治疗效果.lua
-- 更新时间:	2015/10/19 19:58:16
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local player = target.GetEmployer()
	if not player then
		return
	end

	if player.IsSkillRecipeActive(4109,1) then
		player.CastSkillXYZ(15747, 1, target.nX, target.nY, target.nZ)
	end

	target.AddBuff(player.dwID, player.nLevel, 10235, 1)
		
	local nlev = 0
	local nbuff_markCount = 0
	
	local nbuff = target.GetBuff(10235, 1)
	local nbuff_mark = player.GetBuff(10614, 1)
	
	if nbuff then
		nlev = nbuff.nStackNum
	else
		nlev = 1
	end
	...
	if nlev ~= 0 then
		if nbuff_mark and nbuff_markCount < 5 then
			--少目标强化技能ID
			player.CastSkillXYZ(15749, nlev, target.nX, target.nY, target.nZ)
		else
			player.CastSkillXYZ(15134, nlev, target.nX, target.nY, target.nZ)
		end
		player.DelGroupBuff(10614, 1)
	end
end
