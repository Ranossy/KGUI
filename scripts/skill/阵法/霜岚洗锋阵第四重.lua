---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/阵法/霜岚洗锋阵第四重.lua
-- 更新时间:	2016/9/5 20:34:59
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/阵法/霜岚洗锋阵第四重.lua", -- 属性值1
		0											-- 属性值2
	);

	----------------- 技能施放Buff需求 ---------------------------------------------
	skill.AddSlowCheckDestBuff(11380, 0, BUFF_COMPARE_FLAG.EQUAL, 0, BUFF_COMPARE_FLAG.EQUAL);		-- 需求自身Buff
	...
	----------------- BUFF相关 -------------------------------------------------
	  skill.BindBuff(1, 11380, 1);		-- 设置1号位Buff
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(10814, 1) then
		player.nCurrentEnergy = player.nCurrentEnergy + math.min(1, player.nMaxEnergy - player.nCurrentEnergy)
		--print(player.nCurrentEnergy)
	elseif player.GetBuff(10815, 1) then
		player.nCurrentRage = player.nCurrentRage + math.min(1, player.nMaxRage - player.nCurrentRage)
		--print(player.nCurrentRage)
	elseif player.GetBuff(10816, 1)then
		player.nCurrentSunEnergy = player.nCurrentSunEnergy + math.min(1, player.nMaxSunEnergy - player.nCurrentSunEnergy)
		--print(player.nCurrentSunEnergy)
	end
end
