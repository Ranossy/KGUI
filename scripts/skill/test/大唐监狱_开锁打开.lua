---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/test/大唐监狱_开锁打开.lua
-- 更新时间:	2013/6/19 17:44:18
-- 更新用户:	gaoweilong1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
	...
	if player then
		local buff = player.GetBuff(6412, 1)
		RemoteCallToClient(player.dwID, "CallUIGlobalFunction", "QTEPanel_HitNotify", 7966)
		if buff and buff.nCustomValue == 3 then
			RemoteCallToClient(player.dwID, "OnDynamicSkillHighlightChanged", 3, false)
			local buff_Learnd = player.GetBuff(6404, 1)
			if not buff_Learnd then
				player.AddBuff(0, 99, 6404, 1, 1)
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(6, 417), 5)
			else
				RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_GREEN", GetEditorString(5, 7076), 5)
				local doodad = GetDoodad(buff_Learnd.nCustomValue)
				if doodad then
					doodad.OpenDoor()
					player.PlaySound(296)
					player.PlaySound(298)
					-- player.SetTimer(GLOBAL.GAME_FPS * 10, "scripts/Map/大唐监狱/doodad/牢门.lua", doodad.dwID, 1)
				end
			end
		else
			RemoteCallToClient(player.dwID, "OnOutputWarningMessage", "MSG_NOTICE_RED", GetEditorString(6, 416), 5)
		end
