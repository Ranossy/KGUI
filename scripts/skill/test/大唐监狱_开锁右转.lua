---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/test/大唐监狱_开锁右转.lua
-- 更新时间:	2013/6/19 17:44:09
-- 更新用户:	gaoweilong1
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID)
	...
	if player then
		...
		if buff and buff.nCustomValue == 2 then
			player.PlaySound(295)
			RemoteCallToClient(player.dwID, "OnDynamicSkillHighlightChanged", 2, false)
			player.SetTimer(1 * GLOBAL.GAME_FPS, "scripts/Map/大唐监狱/include/QTE开锁_OnTimer.lua", 0, 0)
		else
			player.PlaySound(297)
