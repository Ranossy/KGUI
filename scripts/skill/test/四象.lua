---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/test/四象.lua
-- 更新时间:	2013/11/21 16:25:39
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.nBreakRate			= 80 * PERCENT_BASE;		-- 打断目标施法的概率,基数1024
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if buff then
		...
		if scene then
			zongkong = scene.GetNpcByNickName("FZHD_ZongKong")
			if zongkong then
				local nStuck = zongkong.GetCustomUnsigned4(0) - 4
				if nStuck < 0 then
					nStuck = 0
				end
				zongkong.SetCustomUnsigned4(0,nStuck)
			end
		end
	end
end
