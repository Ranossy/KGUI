---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/test/小白测试4_单人空中.lua
-- 更新时间:	2016/6/6 23:44:31
-- 更新用户:	zhangyan-pc
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/test/小白测试4_单人空中.lua",								--属性值1
		0															--属性值2
	);	
...
if not noTarget and player.IsFollowController() == false then
		...
		------------------------------------------
		if bValue then
		   player.CastSkill(6157, 1)	
		   return
		end
	end

if player.IsRunOnWater() == 1 then		
	player.CastSkill(6489, 1)	
    return
end

if player.nMoveState == MOVE_STATE.ON_RUN then		
	player.CastSkill(6488, 1)
    return
end

if player.nNextJumpCount ~= 0 then
	return
end

if player.IsFollowController() == false then
    if player.nJumpCount == 2 then
    	--print(777)
    	player.AddBuff(0,99,11922,1,30)
		player.CastSkill(6416, 1)--滑翔
	elseif player.nJumpCount == 3 then
		--print(888)
		player.AddBuff(0,99,11922,1,30)	
		player.CastSkill(6418, 1)--上跳
	elseif player.nJumpCount == 4 then
		player.AddBuff(0,99,11922,1,30)	
		--print(999)
		player.CastSkill(6419, 1)--俯冲
	end
else
	local followplayer = GetPlayer(player.dwFollowID)
	if not followplayer then
		return
	end

    if player.nJumpCount == 2 then	
		player.CastSkill(6422, 1)
		followplayer.CastSkill(6784, 1)
	elseif player.nJumpCount == 3 then	
		player.CastSkill(6423, 1)
		followplayer.CastSkill(6785, 1)
	elseif player.nJumpCount == 4 then	
		player.CastSkill(6424, 1)
		followplayer.CastSkill(6786, 1)
    end
end
