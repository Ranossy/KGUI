---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/test/咸鱼挂件起始.lua
-- 更新时间:	2013/12/11 14:25:08
-- 更新用户:	taoli
-- 脚本说明:	
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- BUFF相关 -------------------------------------------------
	--skill.BindBuff(1, 7175, 1);		-- 设置1号位Buff
	...
	skill.SetNormalCoolDown(1, 714);	--技能普通CD
...
function CanCast(player, nPreResult)    --判断玩家的状态，以判断是否可以发出技能
	if ArtistActionCheck(player, 10959) then
		return nPreResult
	end
	
	if player.dwWaistItemIndex ~= 10959 then -- 检查是不是装了背部挂件，腰部用 EQUIPMENT_REPRESENT.WAIST_EXTEND 
		return false
	end
	if nPreResult then
		player.Stop()
		player.AddBuff(player.dwID, player.nLevel, 7177, 1, 1)
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if not player.bFightState then
		player.Stop()
		player.AddBuff(dwSkillSrcID, target.nLevel, 7175, 1, 20)
	end
end
