---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/天策/号令三军三鼓衔接.lua
-- 更新时间:	2015/10/22 15:35:11
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function OnRemove(nCharacterID, BuffID, nBuffLevel, nLeftFrame, nCustomValue, dwSkillSrcID, nStackNum, nBuffIndex, dwCasterID, dwCasterSkillID)
	...
	if BuffID == 10175 then
		if nBuffLevel == 1 then
			player.AddBuff(player.dwID, player.nLevel, 10175, 2)
		elseif nBuffLevel == 2 then
			player.AddBuff(player.dwID, player.nLevel, 10175, 3)
		end
	end
end
