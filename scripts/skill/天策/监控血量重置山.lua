---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/天策/监控血量重置山.lua
-- 更新时间:	2016/5/14 14:55:52
-- 更新用户:	XIAXIANBO
-- 脚本说明:
----------------------------------------------------------------------<
	...
	if player.nCurrentLife <= player.nMaxLife * 0.35 then
		...
		if nlev ~= 0 then
			...
			if skill_target then
				...
				if nleftframe ~= 0 and not player.GetBuff(9840, 1) and player.nMoveState ~= MOVE_STATE.ON_DEATH and player.bFightState == true then
			...
			--[[
			if rest == true and nleftframe > 16 and not player.GetBuff(9840, 1) and player.nMoveState ~= MOVE_STATE.ON_DEATH then
				player.ClearCDTime(149)
				player.AddBuff(player.dwID,player.nLevel,9840,1)
			end
			--]]
