---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/天策/镇派/天策_风虎奇穴_释放技能添加计数效果.lua
-- 更新时间:	2015/3/19 15:36:40
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	...
   	skill.AddAttribute(
   		...
   		"skill/天策/镇派/天策_风虎奇穴_释放技能添加计数效果.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetBuff(8835, 1) then
		local nbuff = player.GetBuff(8837, 1)
		local nbuffcount
		if nbuff then
			nbuffcount = nbuff.nStackNum
			--添加buff的技能就已经添加了一次
			if nbuffcount >= 3 then
				player.DelBuff(8835, 1)
				player.DelGroupBuff(8837, 1)
			else
				player.AddBuff(player.dwID, player.nLevel, 8837, 1)
			end
		else
			player.AddBuff(player.dwID, player.nLevel, 8837, 1)
		end
