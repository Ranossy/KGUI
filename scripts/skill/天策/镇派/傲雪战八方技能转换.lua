---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/天策/镇派/傲雪战八方技能转换.lua
-- 更新时间:	2014/7/16 19:07:45
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		"skill/天策/镇派/傲雪战八方技能转换.lua", -- 属性值1
		0											-- 属性值2
	);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(5669) == 1 then
		...
		if lv ~= 0 then
			...
			player.LearnSkillLevel(9441, lv, dwCharacterID)
			RecipeCopy(6520, lv, player, 9441)
			RemoteCallToClient(player.dwID, "OnActionBarSkillReplace", 6520, 9441, lv)
		end
	else
		--如果玩家马上战八方
		...
		if lv ~= 0 then
			...
			player.LearnSkillLevel(9440, lv, dwCharacterID)
			RecipeCopy(427, lv, player, 9440)
			RemoteCallToClient(player.dwID, "OnActionBarSkillReplace", 427, 9440, lv)
		end
	end
end
...
function UnApply(dwCharacterID)
	...
	if player.GetSkillLevel(5669) == 1 then
		...
		if lv ~= 0 then
			...
			RecipeCopy(9441, lv, player, 6520) 
			RemoteCallToClient(player.dwID, "OnActionBarSkillReplace", 9441, 6520, lv)
		end
	else
		--马上战八方奇穴
		...
		if lv ~= 0 then
			...
			RecipeCopy(9440, lv, player, 427)
			RemoteCallToClient(player.dwID, "OnActionBarSkillReplace", 9440, 427, lv)
		end
	end
end
