---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/天策/90级奇穴/被会心延长1秒虎的时间.lua
-- 更新时间:	2015/9/21 16:27:32
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/天策/90级奇穴/被会心延长1秒虎的时间.lua",											-- 属性值1
   		0											-- 属性值2
   		);
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	if player.GetSkillLevel(14822) == 1 then
		local nlev = player.GetSkillLevel(422)
		if nlev ~= 0 then
			local nbuff = player.GetBuff(203, nlev)
			local nLeftCount = nbuff.nLeftActiveCount
			local nTime = nbuff.nCustomValue
			if nTime <= 4 then
				player.SetBuffLeftActiveCount(nbuff.nIndex, nLeftCount + 1)
