---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/天策/90级奇穴/重置守如山调息时间.lua
-- 更新时间:	2015/9/20 0:30:51
-- 更新用户:	mengxiangfei11
-- 脚本说明:
----------------------------------------------------------------------<
...
function GetSkillLevelData(skill)
	...
	----------------- 魔法属性 -------------------------------------------------
   	skill.AddAttribute(
   		...
   		"skill/天策/90级奇穴/重置守如山调息时间.lua",											-- 属性值1
   		0											-- 属性值2
   		);
	...
	-----------------技能施放技能需求-------------------------------------------
	skill.AddCheckSelfLearntSkill(14802, 1, SKILL_COMPARE_FLAG.EQUAL);     --检查比较Caster自己已学习的技能ID和等级
	...
	skill.nSelfLifePercentMax	= 35;				-- 血量最大值<=
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	--print(1111111)
	if player.nCurrentLife <= player.nMaxLife * 0.35 then
		--print(222222)
		local nlev = player.GetSkillLevel(413)
		if nlev ~= 0 then
			local skill_target = GetSkill(413, nlev)
			if skill_target then
				local CD_ID = skill_target.GetNormalCooldownID(1)
				local nleftframe = player.GetCDLeft(CD_ID)
				if nleftframe ~= 0 and not player.GetBuff(9840, 1) and player.nMoveState ~= MOVE_STATE.ON_DEATH then
					player.ClearCDTime(149)
					player.AddBuff(player.dwID,player.nLevel,9840,1)
				end
			end
			--[[
			local rest, nleftframe, ntotalframe = player.GetSkillCDProgress(413, nlev)
			if rest == true and nleftframe > 16 and not player.GetBuff(9840, 1) then
				--print(4444)
				player.ClearCDTime(149)
				player.AddBuff(player.dwID,player.nLevel,9840,1)
			end
