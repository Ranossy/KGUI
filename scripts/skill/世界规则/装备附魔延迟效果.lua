---------------------------------------------------------------------->
-- 脚本名称:	scripts/skill/世界规则/装备附魔延迟效果.lua
-- 更新时间:	2014/8/14 8:56:21
-- 更新用户:	mengxiangfei
-- 脚本说明:
----------------------------------------------------------------------<
------------------------------------------------
...
function Apply(dwCharacterID, dwSkillSrcID)
	...
	local nbuff = player.GetBuff(8186, 1)
	if nbuff and nbuff.nLeftActiveCount <= 3 then
		if nbuff.nCustomValue == 1 then
			if player.GetSkillLevel(9437) ~= 1 and player.GetKungfuMount() ~= nil and player.GetKungfuMount().dwSkillID == 10026  then
				player.LearnSkillLevel(9437, 1, player.dwID)
			end
		elseif nbuff.nCustomValue == 2 then
			if player.GetSkillLevel(9352) ~= 1 then
				player.LearnSkillLevel(9352, 1, player.dwID)
			end
		elseif nbuff.nCustomValue == 3 then
			if player.GetSkillLevel(9464) ~= 1 then
				player.LearnSkillLevel(9464, 1, player.dwID)
			end

			local lv = player.GetSkillLevel(2636)
			if lv ~= 0 then
				player.LearnSkillLevel(9465, lv, player.dwID)
				player.LearnSkillLevel(9466, lv, player.dwID)
			end
		elseif nbuff.nCustomValue == 4 then
			if player.GetSkillLevel(9477) ~= 1 then
				player.LearnSkillLevel(9477, 1, player.dwID)
			end
		elseif nbuff.nCustomValue == 5 then
			if player.GetSkillLevel(9334) ~= 1 then
				player.LearnSkillLevel(9334, 1, player.dwID)
			end
		elseif nbuff.nCustomValue == 6 then
			if player.GetSkillLevel(9338) ~= 1 then
				player.LearnSkillLevel(9338, 1, player.dwID)
			end
		end

		player.DelBuff(8186,1)
	end
end
